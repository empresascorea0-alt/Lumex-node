...
void atexit_handler ()
{
	lumex::runtime_files::cleanup ();
}
}

void lumex::runtime_files::create (std::filesystem::path const & path, std::string const & contents)
{
...
void lumex::runtime_files::cleanup ()
{
...
void lumex::runtime_files::create_pid_file (std::filesystem::path const & path)
{
	auto pid = boost::this_process::get_id ();
	lumex::runtime_files::create (path, std::to_string (pid));
}

void lumex::runtime_files::create_runtime_info (std::filesystem::path const & path, runtime_info const & info)
{
	...
	lumex::runtime_files::create (path, oss.str ());
}