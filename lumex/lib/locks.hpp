...
#define USING_LUMEX_TIMED_LOCKS (LUMEX_TIMED_LOCKS > 0)

#if USING_LUMEX_TIMED_LOCKS
...
{
class mutex;
extern lumex::mutex * mutex_to_filter;
extern lumex::mutex mutex_to_filter_mutex;
...
template <typename Mutex>
void output_if_held_long_enough (lumex::timer<std::chrono::milliseconds> & timer, Mutex & mutex);

#ifndef LUMEX_TIMED_LOCKS_IGNORE_BLOCKED
template <typename Mutex>
void output_if_blocked_long_enough (lumex::timer<std::chrono::milliseconds> & timer, Mutex & mutex);
#endif
...
template <>
class lock_guard<lumex::mutex> final
{
public:
	explicit lock_guard (lumex::mutex & mutex_a);
	~lock_guard () noexcept;
...
template <typename Mutex, typename = std::enable_if_t<std::is_same<Mutex, lumex::mutex>::value>>
...
/** Assumes std implementations of std::condition_variable never actually call lumex::unique_lock::lock/unlock,
...
public:
	...
	void wait (lumex::unique_lock<lumex::mutex> & lt);

	template <typename Pred>
	void wait (lumex::unique_lock<lumex::mutex> & lk, Pred pred)
	{
		while (!pred ())
		{
			wait (lk);
	...
	std::cv_status wait_until (lumex::unique_lock<lumex::mutex> & lk, std::chrono::time_point<Clock, Duration> const & timeout_time)
	{
		if (!lk.mut || !lk.owns)
		{
	...
	bool wait_until (lumex::unique_lock<lumex::mutex> & lk, std::chrono::time_point<Clock, Duration> const & timeout_time, Pred pred)
	{
		while (!pred ())
		{
	...
	void wait_for (lumex::unique_lock<lumex::mutex> & lk, std::chrono::duration<Rep, Period> const & rel_time)
	{
		wait_until (lk, std::chrono::steady_clock::now () + rel_time);
	}

	template <typename Rep, typename Period, typename Pred>
	bool wait_for (lumex::unique_lock<lumex::mutex> & lk, std::chrono::duration<Rep, Period> const & rel_time, Pred pred)
	{
...
public:
	...
	{
		lumex::unique_lock<lumex::mutex> lk (mutex);
		obj = other;
		return obj;
	}
...
private:
	T obj;
	mutable lumex::mutex mutex; // Mutex needs to be mutable to allow locking in const methods
};

/** Shared ownership wrapper around lumex::locked<T>. Safe to capture by value in callbacks. */
template <class T>
class shared_locked
{
public:
	...
	shared_locked () :
		impl (std::make_shared<lumex::locked<T>> ())
	{
	}
	...
	shared_locked (Arg && arg, Args &&... args) :
		impl (std::make_shared<lumex::locked<T>> (std::forward<Arg> (arg), std::forward<Args> (args)...))
	{
	}
...
private:
	std::shared_ptr<lumex::locked<T>> impl;
};
}
