...
public:
	...
	std::unique_ptr<lumex::container_info_component> to_legacy (std::string const & name) const
	{
		auto composite = std::make_unique<lumex::container_info_composite> (name);

		// Add entries as leaf components
		for (const auto & entry : entries_m)
		{
			lumex::container_info_entry info{ entry.name, entry.size, entry.sizeof_element };
			composite->add_component (std::make_unique<lumex::container_info_leaf> (info));
		}
