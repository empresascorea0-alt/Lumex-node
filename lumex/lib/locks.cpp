...
{
	static lumex::mutex cout_mutex;
	auto stacktrace = lumex::generate_stacktrace ();
...
void output_if_held_long_enough (lumex::timer<std::chrono::milliseconds> & timer, Mutex & mutex)
{
	...
	if (time_held >= std::chrono::milliseconds (LUMEX_TIMED_LOCKS))
	{
		std::unique_lock lk (lumex::mutex_to_filter_mutex);
		if (!lumex::any_filters_registered () || (lumex::mutex_to_filter == &mutex))
		{
			lk.unlock ();
			output ("held", time_held, mutex);
		}
	}
	if (timer.current_state () != lumex::timer_state::stopped)
	{
		timer.stop ();
	}
}
...
void output_if_blocked_long_enough (lumex::timer<std::chrono::milliseconds> & timer, Mutex & mutex)
{
	...
	if (time_blocked >= std::chrono::milliseconds (LUMEX_TIMED_LOCKS))
	{
		std::unique_lock lk (lumex::mutex_to_filter_mutex);
		if (!lumex::any_filters_registered () || (lumex::mutex_to_filter == &mutex))
		{
...
lock_guard<lumex::mutex>::lock_guard (lumex::mutex & mutex) :
	mut (mutex)
{
	timer.start ();
...
lock_guard<lumex::mutex>::~lock_guard () noexcept
{
	mut.unlock ();
...
template class unique_lock<lumex::mutex>;
...
void condition_variable::wait (lumex::unique_lock<lumex::mutex> & lk)
{
	if (!lk.mut || !lk.owns)
	{
		throw (std::system_error (std::make_error_code (std::errc::operation_not_permitted)));
...
lumex::mutex * mutex_to_filter{ nullptr };
lumex::mutex mutex_to_filter_mutex;

bool should_be_filtered (char const * name)
{
	return std::strcmp (name, xstr (LUMEX_TIMED_LOCKS_FILTER)) == 0;
}

bool any_filters_registered ()
{
	return std::strcmp ("", xstr (LUMEX_TIMED_LOCKS_FILTER)) != 0;
}
}
#endif

char const * lumex::mutex_identifier (mutexes mutex)
{
	switch (mutex)
	{
		case mutexes::active:
