...
lumex::tomlconfig::tomlconfig () :
	tree (cpptoml::make_table ())
{
	error = std::make_shared<lumex::error> ();
}

lumex::tomlconfig::tomlconfig (std::shared_ptr<cpptoml::table> const & tree_a, std::shared_ptr<lumex::error> const & error_a) :
...
void lumex::tomlconfig::doc (std::string const & key, std::string const & doc)
{
	tree->document (key, doc);
}

lumex::error & lumex::tomlconfig::read (std::filesystem::path const & path_a)
{
...
lumex::error & lumex::tomlconfig::read (std::istream & stream_overrides, std::filesystem::path const & path_a)
{
	std::fstream stream;
...
lumex::error & lumex::tomlconfig::read (std::istream & stream_a)
{
	std::stringstream stream_override_empty;
	stream_override_empty << std::endl;
...
lumex::error & lumex::tomlconfig::read (std::istream & stream_first_a, std::istream & stream_second_a)
{
	try
	{
...
void lumex::tomlconfig::write (std::filesystem::path const & path_a)
{
	std::fstream stream;
	open_or_create (stream, path_a.string ());
	write (stream);
...
void lumex::tomlconfig::write (std::ostream & stream_a) const
{
...
void lumex::tomlconfig::open_or_create (std::fstream & stream_a, std::string const & path_a)
{
...
std::shared_ptr<cpptoml::table> lumex::tomlconfig::get_tree ()
{
	return tree;
}

/** Returns true if the toml table is empty */
bool lumex::tomlconfig::empty () const
{
	return tree->empty ();
}

std::optional<lumex::tomlconfig> lumex::tomlconfig::get_optional_child (std::string const & key_a)
{
...
lumex::tomlconfig lumex::tomlconfig::get_required_child (std::string const & key_a)
{
	if (!tree->contains (key_a))
	{
...
lumex::tomlconfig & lumex::tomlconfig::put_child (std::string const & key_a, lumex::tomlconfig & conf_a)
{
	tree->insert (key_a, conf_a.get_tree ());
	return *this;
}

lumex::tomlconfig & lumex::tomlconfig::replace_child (std::string const & key_a, lumex::tomlconfig & conf_a)
{
	tree->erase (key_a);
...
bool lumex::tomlconfig::has_key (std::string const & key_a)
{
	return tree->contains (key_a);
}

/** Erase the property of given key */
lumex::tomlconfig & lumex::tomlconfig::erase (std::string const & key_a)
{
	tree->erase (key_a);
	return *this;
}

std::shared_ptr<cpptoml::array> lumex::tomlconfig::create_array (std::string const & key, std::optional<char const *> documentation_a)
{
	if (!has_key (key))
	{
 ...
 */
void lumex::tomlconfig::erase_default_values (tomlconfig & defaults_a)
{
...
std::string lumex::tomlconfig::merge_defaults (lumex::tomlconfig & current_config, lumex::tomlconfig & default_config)
{
...
std::string lumex::tomlconfig::to_string (bool comment_values)
{
...
lumex::tomlconfig & lumex::tomlconfig::get_config (bool optional, std::string const & key, uint8_t & target, uint8_t default_value)
{
	try
	{
...
lumex::tomlconfig & lumex::tomlconfig::get_config (bool optional, std::string const & key, bool & target, bool default_value)
{
...
void lumex::tomlconfig::erase_defaults (std::shared_ptr<cpptoml::table> const & base, std::shared_ptr<cpptoml::table> const & other, std::shared_ptr<cpptoml::table> const & update_target)
{
...
lumex::tomlconfig & lumex::tomlconfig::get_config (bool optional, std::string key, boost::asio::ip::address_v6 & target, boost::asio::ip::address_v6 const & default_value)
{
	try
	{
		if (tree->contains_qualified (key))
