...
lumex::rpc_config::rpc_config (lumex::network_constants & network_constants) :
	rpc_process{ network_constants },
	address{ boost::asio::ip::address_v6::loopback ().to_string () }
{
}

lumex::rpc_config::rpc_config (lumex::network_constants & network_constants, uint16_t port_a, bool enable_control_a) :
	rpc_process{ network_constants },
	address{ boost::asio::ip::address_v6::loopback ().to_string () },
	port{ port_a },
...
lumex::error lumex::rpc_config::serialize_toml (lumex::tomlconfig & toml) const
{
	...
	lumex::tomlconfig rpc_process_l;
...
lumex::rpc_process_config::rpc_process_config (lumex::network_constants & network_constants) :
...
namespace lumex
{
lumex::error read_rpc_config_toml (std::filesystem::path const & data_path_a, lumex::rpc_config & config_a, std::vector<std::string> const & config_overrides)
{
	lumex::error error;
	auto toml_config_path = lumex::get_rpc_toml_config_path (data_path_a);
...
std::string get_default_rpc_filepath ()
{
	...
	// Construct the lumex_rpc executable file path based on where the currently running executable is found.
