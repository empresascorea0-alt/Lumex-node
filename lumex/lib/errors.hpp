...
REGISTER_ERROR_CODES (lumex, error_common);
REGISTER_ERROR_CODES (lumex, error_blocks);
REGISTER_ERROR_CODES (lumex, error_rpc);
REGISTER_ERROR_CODES (lumex, error_process);
REGISTER_ERROR_CODES (lumex, error_config);
...
public:
	error () = default;
	error (lumex::error const &) = default;
	error (lumex::error &&) = default;

	error (std::error_code);
	error (std::string message);
	error (std::exception const &);

	error & operator= (lumex::error const &);
	error & operator= (lumex::error &&);
	...
	error & then (std::function<lumex::error &()> next);
	...
	error & set (std::string message, std::error_code code = lumex::error_common::generic);
...
/**
 * A type that manages a lumex::error.
 * The default return type is lumex::error&, though shared_ptr<lumex::error> is a good option in cases
 * where shared error state is desirable.
 */
template <typename RET_TYPE = lumex::error &>
class error_aware
{
	static_assert (std::is_same<RET_TYPE, lumex::error &>::value || std::is_same<RET_TYPE, std::shared_ptr<lumex::error>>::value, "Must be lumex::error& or shared_ptr<lumex::error>");
