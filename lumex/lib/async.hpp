...
{
public:
	explicit cancellation (lumex::async::strand & strand) :
		strand{ strand },
...
{
public:
	explicit condition (lumex::async::strand & strand) :
		strand{ strand },
...
private:
	struct shared_state
	{
		...
		explicit shared_state (lumex::async::strand & strand) :
			timer{ strand } {};
	};
...
public:
	...
	explicit task (lumex::async::strand & strand) :
		strand{ strand },
		cancellation{ strand }
	{
	}
	...
	task (lumex::async::strand & strand, Func && func) :
		strand{ strand },
		cancellation{ strand }
	{
	...
	task (lumex::async::strand & strand, Func && func) :
		strand{ strand },
		cancellation{ strand },
		condition{ std::make_unique<lumex::async::condition> (strand) }
	{
...
private:
	...
	lumex::async::cancellation cancellation;
	std::unique_ptr<lumex::async::condition> condition; // Optional, but needs a fixed address
};
}