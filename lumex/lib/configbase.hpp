...
class configbase : public lumex::error_aware<>
{
public:
	configbase () = default;
	configbase (std::shared_ptr<lumex::error> const & error_a) :
		error (error_a)
	{
	}

	/** Returns the current error */
	lumex::error & get_error () override
	{
		return *error;
	}
...
protected:
	...
	/** We're a lumex::error_aware type. Child nodes share the error state. */
	std::shared_ptr<lumex::error> error;
