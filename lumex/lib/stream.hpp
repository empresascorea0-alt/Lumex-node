...
#include <lumex/lib/char_traits.hpp>
...
template <typename T>
bool try_read (lumex::stream & stream_a, T & value_a)
{
...
template <typename T>
void read (lumex::stream & stream_a, T & value)
{
...
inline void read (lumex::stream & stream_a, std::vector<uint8_t> & value_a, size_t size_a)
{
...
template <typename T>
void write (lumex::stream & stream_a, T const & value_a)
{
...
inline void write (lumex::stream & stream_a, std::vector<uint8_t> const & value_a)
{
...
inline bool at_end (lumex::stream & stream)
{
	uint8_t junk;
	auto end (lumex::try_read (stream, junk));
	return end;
}
 ...
 */
template <typename T>
void write_big_endian (lumex::stream & stream, T const & value)
{
	lumex::write (stream, boost::endian::native_to_big (value));
}

template <typename T>
void read_big_endian (lumex::stream & stream, T & value)
{
	T tmp;
	lumex::read (stream, tmp);
