...
public:
	...
	virtual const lumex::store::transaction & base_txn () const = 0;

	// Conversion operator to const lumex::store::transaction&
	virtual operator const lumex::store::transaction & () const = 0;
...
{
	lumex::store::write_guard guard; // Guard should be released after the transaction
	lumex::store::write_transaction txn;
...
public:
	write_transaction (lumex::store::write_transaction && txn_a, lumex::store::write_guard && guard_a) noexcept :
	...
	const lumex::store::transaction & base_txn () const override
	{
		return txn;
	}
	...
	// Conversion operator to const lumex::store::transaction&
	operator const lumex::store::transaction & () const override
	{
		return txn;
	}

	// Additional conversion operator specific to lumex::store::write_transaction
	operator const lumex::store::write_transaction & () const
	{
		return txn;
	}
};
...
{
	lumex::store::read_transaction txn;

public:
	explicit read_transaction (lumex::store::read_transaction && t) noexcept :
		txn{ std::move (t) }
	{
	}
	...
	// Additional conversion operator specific to lumex::store::read_transaction
	operator const lumex::store::read_transaction & () const
	{
		return txn;
	}
};
