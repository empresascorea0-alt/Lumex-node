...
void lumex::ledger::populate_topo_index ()
{
	release_assert (!flags.topo_index, "populate_topo_index called when topo_index is already enabled");

	logger.info (lumex::log::type::ledger_upgrade, "Populating topology index...");
	...
	size_t const batch_size_compute = lumex::is_dev_run () ? 2 : 1024 * 1024;
	size_t const batch_size_populate = lumex::is_dev_run () ? 2 : 16 * 1024 * 1024;
	size_t const max_depth = lumex::is_dev_run () ? 16 : 16 * 1024 * 1024;

	// Phase 1: compute topo_height for every block via bounded DFS, write to sideband only
	logger.info (lumex::log::type::ledger_upgrade, "Phase 1: computing topology heights for {} blocks...", total_blocks);
	{
		...
		auto is_resolved = [&] (std::shared_ptr<lumex::block> const & blk) -> bool {
			if (!blk)
			{
		...
		auto get_dependencies = [&] (std::shared_ptr<lumex::block> const & blk) -> std::array<std::shared_ptr<lumex::block>, 2> {
			auto const dep_hashes = blk->dependencies ();
			std::array<std::shared_ptr<lumex::block>, 2> deps{};
		...
		auto compute_topo = [] (std::array<std::shared_ptr<lumex::block>, 2> const & deps) -> uint64_t {
			uint64_t topo = 1;
		...
		auto verify_topo = [&] (std::shared_ptr<lumex::block> const & blk) {
		...
		auto resolve = [&] (std::shared_ptr<lumex::block> const & blk) -> bool {
			...
			{
				logger.info (lumex::log::type::ledger_upgrade, "Topology resolve progress: {} / {} blocks ({:.1f}%)", resolved, total_blocks, lumex::log::percentage (resolved, total_blocks));
				last_log = now;
			}

			if (resolved % batch_size_compute == 0)
			{
				logger.debug (lumex::log::type::ledger_upgrade, "Committing batch of {} blocks...", batch_size_compute);
				...
				logger.debug (lumex::log::type::ledger_upgrade, "Transaction refresh took {}ms", refresh_ms);
			}
		...
		for (; crawler; ++crawler, ++processed)
		{
			...
			{
				logger.info (lumex::log::type::ledger_upgrade, "Processing progress: {} / {} blocks ({:.1f}%)", processed, total_blocks, lumex::log::percentage (processed, total_blocks));
			}
			...
			lumex::bounded_dfs_result dfs_result;
			do
			{
				dfs_result = lumex::bounded_dfs (bws.block, max_depth, is_resolved, get_dependencies, resolve);
				if (dfs_result.overflow)
				{
					logger.debug (lumex::log::type::ledger_upgrade, "Partially resolved {} dependencies for block {}, continuing...", dfs_result.resolved, hash);
				}
			} while (dfs_result.overflow);
		}

		logger.info (lumex::log::type::ledger_upgrade, "Resolved {} blocks", resolved);
	}

	// Phase 1 verification: every block must carry a non-zero topo_height with valid ordering against its dependencies
	logger.info (lumex::log::type::ledger_upgrade, "Verifying topology heights...");
	{
		auto txn = tx_begin_read ();
		size_t verified = 0;
		for (auto it = store.block.begin (txn), end = store.block.end (txn); it != end; ++it)
		{
			...
			{
				logger.info (lumex::log::type::ledger_upgrade, "Topology verification progress: {} / {} blocks ({:.1f}%)", verified, total_blocks, lumex::log::percentage (verified, total_blocks));
			}
		}
	}
	...
	logger.info (lumex::log::type::ledger_upgrade, "Phase 2: populating topology table for {} blocks...", total_blocks);
	{
		...
		while (crawler)
		{
			...
			if (now - last_log >= std::chrono::seconds (5))
			{
				logger.info (lumex::log::type::ledger_upgrade, "Topology table progress: {} / {} blocks ({:.1f}%)", processed, total_blocks, lumex::log::percentage (processed, total_blocks));
				last_log = now;
			}

			if (processed % batch_size_populate == 0)
			{
				logger.debug (lumex::log::type::ledger_upgrade, "Committing batch of {} blocks...", batch_size_populate);
				auto const refresh_start = std::chrono::steady_clock::now ();
		...
		logger.info (lumex::log::type::ledger_upgrade, "Done populating topology table for {} blocks", processed);
	}
	...
	logger.info (lumex::log::type::ledger_upgrade, "Topology index populated for {} blocks", total_blocks);
}

void lumex::ledger::drop_topo_index ()
{
	release_assert (flags.topo_index, "drop_topo_index called when topo_index is not enabled");

	logger.info (lumex::log::type::ledger_upgrade, "Dropping topology index...");
	...
	logger.info (lumex::log::type::ledger_upgrade, "Topology index dropped");
}
