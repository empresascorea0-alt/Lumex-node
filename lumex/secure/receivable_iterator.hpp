...
public:
	...
	receivable_iterator (secure::transaction const & transaction, Set const & set, std::optional<std::pair<lumex::pending_key, lumex::pending_info>> const & item);
...
public: // Dereferencing, undefined behavior when called on an end () iterator
	...
	std::pair<lumex::pending_key, lumex::pending_info> const & operator* () const;
	std::pair<lumex::pending_key, lumex::pending_info> const * operator->() const;
...
private:
	...
	std::optional<std::pair<lumex::pending_key, lumex::pending_info>> item;
};
}
