...
TEST (wallet, construction)
{
	...
	auto wallet_l = system.nodes[0]->wallets.create (lumex::random_wallet_id ());
	auto key_result = wallet_l->deterministic_insert ();
	ASSERT_TRUE (key_result);
	auto key = key_result.value ();
	auto wallet = std::make_shared<lumex_qt::wallet> (*test_application, processor, *system.nodes[0], wallet_l, key);
	wallet->start ();
...
// https://github.com/lumexcurrency/lumex-node/issues/3235
TEST (wallet, DISABLED_status)
{
	...
	auto wallet = std::make_shared<lumex_qt::wallet> (*test_application, processor, *system.nodes[0], wallet_l, key.pub);
	wallet->start ();
	auto wallet_has = [wallet] (lumex_qt::status_types status_ty) {
	...
	auto outer_node = lumex::test::add_outer_node (system);
	lumex::test::establish_tcp (system, *system.nodes[0], outer_node->network.endpoint ());
	...
	ASSERT_FALSE (wallet_has (lumex_qt::status_types::synchronizing));
	system.deadline_set (25s);
	while (!wallet_has (lumex_qt::status_types::synchronizing))
	{
	...
	ASSERT_TRUE (wallet_has (lumex_qt::status_types::disconnected));
}
...
TEST (wallet, status_with_peer)
{
	...
	ASSERT_TRUE (wallet_has (lumex_qt::status_types::nominal));
}
...
TEST (wallet, startup_balance)
{
	...
	ASSERT_EQ ("Balance: 0 lumex", wallet->self.balance_label->text ().toStdString ());
}
...
TEST (wallet, select_account)
{
	...
	auto wallet = std::make_shared<lumex_qt::wallet> (*test_application, processor, *system.nodes[0], wallet_l, key1);
	wallet->start ();
...
TEST (wallet, password_change)
{
	lumex_qt::eventloop_processor processor;
	lumex::test::system system (1);
	ASSERT_TRUE (system.wallet (0)->insert_adhoc (lumex::keypair ().prv));
	lumex::account account = system.wallet (0)->accounts ().front ();
	auto wallet (std::make_shared<lumex_qt::wallet> (*test_application, processor, *system.nodes[0], system.wallet (0), account));
	wallet->start ();
	QTest::mouseClick (wallet->settings_button, Qt::LeftButton);
...
TEST (wallet, send)
{
	lumex_qt::eventloop_processor processor;
	lumex::test::system system (2);
	ASSERT_TRUE (system.wallet (0)->insert_adhoc (lumex::dev::genesis_key.prv));
	auto key1 = system.wallet (1)->insert_adhoc (lumex::keypair ().prv).value ();
	auto account (lumex::dev::genesis_key.pub);
	auto wallet (std::make_shared<lumex_qt::wallet> (*test_application, processor, *system.nodes[0], system.wallet (0), account));
	wallet->start ();
	ASSERT_NE (wallet->rendering_ratio, lumex::raw_ratio);
	QTest::mouseClick (wallet->send_blocks, Qt::LeftButton);
	...
	lumex::uint128_t amount (wallet->node.balance (key1));
...
// Issue for investigating it: https://github.com/lumexcurrency/lumex-node/issues/3642
TEST (wallet, DISABLED_process_block)
{
	...
	lumex::send_block send (latest, key1.pub, 0, lumex::dev::genesis_key.prv, lumex::dev::genesis_key.pub, *system.work.generate (latest));
...
TEST (wallet, create_send)
{
	...
	QTest::keyClicks (wallet->block_creation.account, lumex::dev::genesis_key.pub.to_account ().c_str ());
	...
	bool error (false);
	auto send = std::make_shared<lumex::state_block> (error, tree1);
	ASSERT_FALSE (error);
	ASSERT_EQ (lumex::block_status::progress, system.nodes[0]->process (send));
	ASSERT_EQ (lumex::block_status::old, system.nodes[0]->process (send));
}
...
TEST (wallet, create_open_receive)
{
	...
	lumex::test::system system (1);
	ASSERT_TRUE (system.wallet (0)->insert_adhoc (lumex::dev::genesis_key.prv));
	system.wallet (0)->send_action (lumex::dev::genesis_key.pub, key.pub, 100);
	lumex::block_hash latest1 (system.nodes[0]->latest (lumex::dev::genesis_key.pub));
	system.wallet (0)->send_action (lumex::dev::genesis_key.pub, key.pub, 100);
	lumex::block_hash latest2 (system.nodes[0]->latest (lumex::dev::genesis_key.pub));
	...
	QTest::keyClicks (wallet->block_creation.representative, lumex::dev::genesis_key.pub.to_account ().c_str ());
	QTest::mouseClick (wallet->block_creation.create, Qt::LeftButton);
	...
	bool error (false);
	auto open = std::make_shared<lumex::state_block> (error, tree1);
	ASSERT_FALSE (error);
	ASSERT_EQ (lumex::block_status::progress, system.nodes[0]->process (open));
	ASSERT_EQ (lumex::block_status::old, system.nodes[0]->process (open));
	...
	bool error2 (false);
	auto receive = std::make_shared<lumex::state_block> (error2, tree2);
	ASSERT_FALSE (error2);
	ASSERT_EQ (lumex::block_status::progress, system.nodes[0]->process (receive));
	ASSERT_EQ (lumex::block_status::old, system.nodes[0]->process (receive));
}
...
TEST (wallet, create_change)
{
	...
	boost::property_tree::read_json (istream, tree1);
	bool error (false);
	auto change = std::make_shared<lumex::state_block> (error, tree1);
	ASSERT_FALSE (error);
	ASSERT_EQ (lumex::block_status::progress, system.nodes[0]->process (change));
	ASSERT_EQ (lumex::block_status::old, system.nodes[0]->process (change));
}
...
TEST (history, short_text)
{
	...
	lumex::ledger ledger (*store, lumex::dev::network_params, stats, logger);
	{
		...
		auto send = std::make_shared<lumex::send_block> (latest, lumex::dev::genesis_key.pub, 0, lumex::dev::genesis_key.prv, lumex::dev::genesis_key.pub, *system.work.generate (latest));
		ASSERT_EQ (lumex::block_status::progress, ledger.process (transaction, send));
		auto receive = std::make_shared<lumex::receive_block> (send->hash (), send->hash (), lumex::dev::genesis_key.prv, lumex::dev::genesis_key.pub, *system.work.generate (send->hash ()));
		ASSERT_EQ (lumex::block_status::progress, ledger.process (transaction, receive));
		auto change = std::make_shared<lumex::change_block> (receive->hash (), key.pub, lumex::dev::genesis_key.prv, lumex::dev::genesis_key.pub, *system.work.generate (receive->hash ()));
		ASSERT_EQ (lumex::block_status::progress, ledger.process (transaction, change));
	}
	lumex_qt::history history (ledger, lumex::dev::genesis_key.pub, *wallet);
	history.refresh ();
...
TEST (history, pruned_source)
{
	...
	ledger.pruning = true;
	lumex::block_hash next_pruning;
	// Basic pruning for legacy blocks. Previous block is pruned, source is pruned
	{
		auto transaction = ledger.tx_begin_write ();
		auto latest (ledger.any.account_head (transaction, lumex::dev::genesis_key.pub));
		auto send1 = std::make_shared<lumex::send_block> (latest, lumex::dev::genesis_key.pub, lumex::dev::constants.genesis_amount - 100, lumex::dev::genesis_key.prv, lumex::dev::genesis_key.pub, *system.work.generate (latest));
		ASSERT_EQ (lumex::block_status::progress, ledger.process (transaction, send1));
		auto send2 = std::make_shared<lumex::send_block> (send1->hash (), key.pub, lumex::dev::constants.genesis_amount - 200, lumex::dev::genesis_key.prv, lumex::dev::genesis_key.pub, *system.work.generate (send1->hash ()));
		ASSERT_EQ (lumex::block_status::progress, ledger.process (transaction, send2));
		auto receive1 = std::make_shared<lumex::receive_block> (send2->hash (), send1->hash (), lumex::dev::genesis_key.prv, lumex::dev::genesis_key.pub, *system.work.generate (send2->hash ()));
		ASSERT_EQ (lumex::block_status::progress, ledger.process (transaction, receive1));
		auto open = std::make_shared<lumex::open_block> (send2->hash (), key.pub, key.pub, key.prv, key.pub, *system.work.generate (key.pub));
	...
	ASSERT_NE (wallet->rendering_ratio, lumex::raw_ratio);
	wallet->rendering_ratio = lumex::raw_ratio;
	// Genesis account pruned values
	lumex_qt::history history1 (ledger, lumex::dev::genesis_key.pub, *wallet);
	history1.refresh ();
	...
	ASSERT_EQ (lumex::account (0).to_account (), account1->text ().toStdString ());
	...
	lumex_qt::history history2 (ledger, key.pub, *wallet);
	history2.refresh ();
	...
	ASSERT_EQ (lumex::dev::genesis_key.pub.to_account (), account3->text ().toStdString ());
	...
	ASSERT_EQ (lumex::account (0).to_account (), account4->text ().toStdString ());
	...
	ASSERT_EQ (lumex::account (0).to_account (), account5->text ().toStdString ());
	...
	// Pruning for state blocks. Previous block is pruned, source is pruned
	{
		auto transaction = ledger.tx_begin_write ();
		auto latest (ledger.any.account_head (transaction, lumex::dev::genesis_key.pub));
		auto send3 = std::make_shared<lumex::state_block> (lumex::dev::genesis_key.pub, latest, lumex::dev::genesis_key.pub, lumex::dev::constants.genesis_amount - 200, key.pub, lumex::dev::genesis_key.prv, lumex::dev::genesis_key.pub, *system.work.generate (latest));
		...
		auto receive2 = std::make_shared<lumex::state_block> (key.pub, latest_key, key.pub, 200, send3->hash (), key.prv, key.pub, *system.work.generate (latest_key));
	...
	ASSERT_EQ (lumex::dev::genesis_key.pub.to_account (), account7->text ().toStdString ());
	// Amount is unknown because previous block (receive1) is removed
	...
	// Pruning for state blocks. Change state block
	{
		auto transaction = ledger.tx_begin_write ();
		auto latest (ledger.any.account_head (transaction, lumex::dev::genesis_key.pub));
		auto change = std::make_shared<lumex::state_block> (lumex::dev::genesis_key.pub, latest, key.pub, lumex::dev::constants.genesis_amount - 200, 0, lumex::dev::genesis_key.prv, lumex::dev::genesis_key.pub, *system.work.generate (latest));
		ASSERT_EQ (lumex::block_status::progress, ledger.process (transaction, change));
		ledger.cement (transaction, latest);
		ASSERT_EQ (1, ledger.pruning_action (transaction, latest, 2));
	}
	...
	ASSERT_EQ (lumex::account (0).to_account (), account9->text ().toStdString ());
	// Amount is unknown because previous block (receive1) is removed
	...
	ASSERT_TRUE (system.wallet (0)->insert_adhoc (key2.prv));
	{
		auto transaction = ledger.tx_begin_write ();
		auto latest_key (ledger.any.account_head (transaction, key.pub));
		auto send4 = std::make_shared<lumex::state_block> (key.pub, latest_key, key.pub, 100, key2.pub, key.prv, key.pub, *system.work.generate (latest_key));
		ASSERT_EQ (lumex::block_status::progress, ledger.process (transaction, send4));
		auto open2 = std::make_shared<lumex::state_block> (key2.pub, 0, key2.pub, 100, send4->hash (), key2.prv, key2.pub, *system.work.generate (key2.pub));
		ASSERT_EQ (lumex::block_status::progress, ledger.process (transaction, open2));
		ledger.cement (transaction, latest_key);
		ASSERT_EQ (1, ledger.pruning_action (transaction, latest_key, 2));
	}
	...
	lumex_qt::history history3 (ledger, key2.pub, *wallet);
	history3.refresh ();
...
TEST (wallet, startup_work)
{
	...
	wallet->start ();
	QTest::mouseClick (wallet->show_advanced, Qt::LeftButton);
	auto work1_result = wallet->wallet_m->get_work (lumex::dev::genesis_key.pub);
	...
	auto has_work = [&] (lumex::public_key const & account) {
		auto result = wallet->wallet_m->get_work (account);
		return result && result.value () != 0;
	};
	ASSERT_TIMELY (10s, has_work (lumex::dev::genesis_key.pub));
}
...
TEST (wallet, import)
{
	...
	auto path{ lumex::unique_path () / "wallet.json" };
	{
		std::ofstream stream;
		stream.open (path.string ().c_str ());
		stream << json;
	}
	auto wallet (std::make_shared<lumex_qt::wallet> (*test_application, processor, *system.nodes[1], system.wallet (1), key2.pub));
	wallet->start ();
	QTest::mouseClick (wallet->show_advanced, Qt::LeftButton);
	ASSERT_EQ (wallet->advanced.window, wallet->main_stack->currentWidget ());
	QTest::mouseClick (wallet->accounts_button, Qt::LeftButton);
...
TEST (wallet, republish)
{
	...
	lumex::block_hash hash;
	{
		...
		auto latest (node1.ledger.any.account_head (transaction, lumex::dev::genesis_key.pub));
		auto block = std::make_shared<lumex::send_block> (latest, key.pub, 0, lumex::dev::genesis_key.prv, lumex::dev::genesis_key.pub, *system.work.generate (latest));
		hash = block->hash ();
		ASSERT_EQ (lumex::block_status::progress, node1.ledger.process (transaction, block));
	}
	...
	auto account (lumex::dev::genesis_key.pub);
	auto wallet (std::make_shared<lumex_qt::wallet> (*test_application, processor, node1, system.wallet (0), account));
	wallet->start ();
	QTest::mouseClick (wallet->show_advanced, Qt::LeftButton);
	ASSERT_EQ (wallet->advanced.window, wallet->main_stack->currentWidget ());
	QTest::mouseClick (wallet->advanced.block_viewer, Qt::LeftButton);
...
TEST (wallet, ignore_empty_adhoc)
{
	...
	lumex::keypair key1;
	ASSERT_TRUE (system.wallet (0)->insert_adhoc (key1.prv));
	auto wallet (std::make_shared<lumex_qt::wallet> (*test_application, processor, *system.nodes[0], system.wallet (0), key1.pub));
	wallet->start ();
	QTest::mouseClick (wallet->show_advanced, Qt::LeftButton);
	ASSERT_EQ (wallet->advanced.window, wallet->main_stack->currentWidget ());
	QTest::mouseClick (wallet->accounts_button, Qt::LeftButton);
	ASSERT_EQ (wallet->accounts.window, wallet->main_stack->currentWidget ());
	QTest::keyClicks (wallet->accounts.account_key_line, lumex::dev::genesis_key.prv.to_string ().c_str ());
	QTest::mouseClick (wallet->accounts.account_key_button, Qt::LeftButton);
	ASSERT_EQ (1, wallet->accounts.model->rowCount ());
...
TEST (wallet, change_seed)
{
	...
	auto wallet (std::make_shared<lumex_qt::wallet> (*test_application, processor, *system.nodes[0], system.wallet (0), wallet_key));
	wallet->start ();
	QTest::mouseClick (wallet->show_advanced, Qt::LeftButton);
	ASSERT_EQ (wallet->advanced.window, wallet->main_stack->currentWidget ());
	QTest::mouseClick (wallet->accounts_button, Qt::LeftButton);
...
TEST (wallet, seed_work_generation)
{
	...
	lumex::test::system system (1);
	auto key1 = system.wallet (0)->deterministic_insert ().value ();
	auto wallet (std::make_shared<lumex_qt::wallet> (*test_application, processor, *system.nodes[0], system.wallet (0), key1));
	wallet->start ();
	QTest::mouseClick (wallet->show_advanced, Qt::LeftButton);
	ASSERT_EQ (wallet->advanced.window, wallet->main_stack->currentWidget ());
	QTest::mouseClick (wallet->accounts_button, Qt::LeftButton);
	...
	QTest::keyClicks (wallet->import.seed, seed.to_string ().c_str ());
	QTest::keyClicks (wallet->import.clear_line, "clear keys");
	lumex::result<uint64_t> work_result = lumex::error (lumex::error_common::account_not_found_wallet);
	QTest::mouseClick (wallet->import.import_seed, Qt::LeftButton);
	system.deadline_set (10s);
	...
	auto transaction = system.nodes[0]->ledger.tx_begin_read ();
	ASSERT_GE (lumex::dev::network_params.work.difficulty (lumex::work_version::work_1, system.nodes[0]->ledger.latest_root (transaction, pub), work_result.value ()), system.nodes[0]->default_difficulty (lumex::work_version::work_1));
}
...
TEST (wallet, import_locked)
{
	...
	auto seed2 = system.wallet (0)->get_seed ();
	ASSERT_FALSE (seed2);
	ASSERT_EQ (seed2.error (), lumex::error_common::wallet_locked);
...
TEST (wallet, epoch_2_validation)
{
	...
	auto account (lumex::dev::genesis_key.pub);
	auto wallet (std::make_shared<lumex_qt::wallet> (*test_application, processor, *node, system.wallet (0), account));
	wallet->start ();
	wallet->client_window->show ();
	...
	auto create_and_process = [&] () -> lumex::block_hash {
		wallet->block_creation.create->click ();
		std::string json (wallet->block_creation.block->toPlainText ().toStdString ());
		...
		boost::property_tree::read_json (istream, tree1);
		bool error (false);
		auto block = std::make_shared<lumex::state_block> (error, tree1);
		EXPECT_FALSE (error);
		EXPECT_EQ (lumex::block_status::progress, node->process (block));
		return block->hash ();
	};

	auto do_send = [&] (lumex::public_key const & destination) -> lumex::block_hash {
		wallet->block_creation.send->click ();
		wallet->block_creation.account->setText (lumex::dev::genesis_key.pub.to_account ().c_str ());
	...
	auto do_open = [&] (lumex::block_hash const & source, lumex::public_key const & account) -> lumex::block_hash {
		wallet->block_creation.open->click ();
	...
	auto do_receive = [&] (lumex::block_hash const & source) -> lumex::block_hash {
		wallet->block_creation.receive->click ();
		wallet->block_creation.source->setText (source.to_string ().c_str ());
		return create_and_process ();
	};

	auto do_change = [&] (lumex::public_key const & account, lumex::public_key const & representative) -> lumex::block_hash {
		wallet->block_creation.change->click ();
	...
	while (++tries < max_tries)
	{
		...
		do_receive (send2);
		do_change (key.pub, lumex::dev::genesis_key.pub);
	}
}
