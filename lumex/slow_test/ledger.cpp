...
namespace
{
void rollback_degenerate_impl (lumex::test::ledger_context & ctx, size_t num_accounts)
{
	auto & ledger = ctx.ledger ();
	auto & store = ctx.store ();
	auto & pool = ctx.pool ();

	std::vector<lumex::keypair> keys (num_accounts); // Keys for A0 to A(N-1)

	// Track the head block hash for each account involved (Genesis + A0..A(N-1))
	std::vector<lumex::block_hash> current_head_hashes (num_accounts + 1);
	current_head_hashes[0] = lumex::dev::genesis->hash (); // Index 0 for Genesis

	// Track balances (needed for creating send blocks)
	std::vector<lumex::uint128_t> current_balances (num_accounts + 1);
	current_balances[0] = lumex::dev::constants.genesis_amount; // Index 0 for Genesis

	lumex::block_hash first_send_hash; // Store the hash of the first send (G->A0)
	lumex::block_hash first_receive_hash; // Store the hash of the first receive (A0 open)
	...
	std::cout << "Creating and processing " << num_accounts << " send/receive pairs..." << std::endl;
	{
		...
		for (uint32_t i = 0; i < num_accounts; ++i)
		{
			...
			lumex::keypair sender_key = (i == 0) ? lumex::dev::genesis_key : keys[i - 1];
			lumex::block_hash sender_previous_hash = current_head_hashes[sender_account_index];
			lumex::uint128_t sender_balance = current_balances[sender_account_index];
			...
			ASSERT_EQ (lumex::block_status::progress, ledger.process (transaction, send_block)) << "Failed processing send block " << i;
			...
			ASSERT_EQ (lumex::block_status::progress, ledger.process (transaction, receive_block)) << "Failed processing receive block " << i;
	...
	// Verify state after processing
	{
		...
		ASSERT_EQ (ledger.any.account_balance (transaction, lumex::dev::genesis_key.pub), lumex::dev::constants.genesis_amount - 1); // Genesis only sent once to A0
		for (uint32_t i = 0; i < num_accounts; ++i)
		{
		...
		auto pending_info = ledger.any.pending_get (transaction, lumex::pending_key (keys[0].pub, first_send_hash));
	...
	std::deque<std::shared_ptr<lumex::block>> rolled_back_list;
	{
		auto transaction = ledger.tx_begin_write ();
	...
	// Verify final state
	{
		...
		ASSERT_EQ (ledger.any.account_balance (transaction, lumex::dev::genesis_key.pub), lumex::dev::constants.genesis_amount - 1);
		ASSERT_EQ (ledger.weight (lumex::dev::genesis_key.pub), lumex::dev::constants.genesis_amount - 1); // Unreceived 1 raw send
		...
		ASSERT_EQ (pending_info->source, lumex::dev::genesis_key.pub);
