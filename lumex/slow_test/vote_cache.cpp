...
namespace
{
lumex::keypair setup_rep (lumex::test::system & system, lumex::node & node, lumex::uint128_t amount)
{
	...
	lumex::test::confirm (node.ledger, open->hash ());

	return key;
}

std::vector<lumex::keypair> setup_reps (lumex::test::system & system, lumex::node & node, int count)
{
	const lumex::uint128_t weight = lumex::Klumex_ratio * 1000;
	std::vector<lumex::keypair> reps;
 ...
 */
std::vector<std::shared_ptr<lumex::block>> setup_blocks (lumex::test::system & system, lumex::node & node, int count)
{
	auto latest = node.latest (lumex::dev::genesis_key.pub);
	auto balance = node.balance (lumex::dev::genesis_key.pub);

	std::vector<std::shared_ptr<lumex::block>> sends;
	std::vector<std::shared_ptr<lumex::block>> receives;
	for (int n = 0; n < count; ++n)
	{
		if (n % 10000 == 0)
	...
	EXPECT_TRUE (lumex::test::process (node, sends));
	EXPECT_TRUE (lumex::test::process (node, receives));

	// Confirm whole genesis chain at once
	lumex::test::confirm (node.ledger, sends.back ()->hash ());
...
TEST (vote_cache, perf_singlethreaded)
{
	...
	rate.observe (node, lumex::stat::type::vote_cache, lumex::stat::detail::vote_processed, lumex::stat::dir::in);
	...
	for (int n = 0; n < vote_count; ++n)
	{
		...
		for (int i = 0; i < single_vote_reps; ++i)
		{
			...
			auto vote = lumex::test::make_vote (reps[rep_idx], hashes);
	...
	std::cout << "total votes processed: " << node.stats.count (lumex::stat::type::vote_cache, lumex::stat::detail::vote_processed, lumex::stat::dir::in) << std::endl;

	// Ensure we processed all the votes
	ASSERT_EQ (node.stats.count (lumex::stat::type::vote_cache, lumex::stat::detail::vote_processed, lumex::stat::dir::in), vote_count * single_vote_size * single_vote_reps);
