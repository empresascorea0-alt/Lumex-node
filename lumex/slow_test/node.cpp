 ...
 */
size_t manually_count_pruned_blocks (lumex::store::ledger_store & store)
{
	size_t count = 0;
...
TEST (system, generate_mass_activity_long)
{
	...
	node_config.enable_voting = false; // Prevent blocks cementing
	auto node = system.add_node (node_config);
	lumex::thread_runner runner (system.io_ctx, system.logger, system.nodes[0]->config.io_threads);
	system.wallet (0)->insert_adhoc (lumex::dev::genesis_key.prv);
...
TEST (system, receive_while_synchronizing)
{
	std::vector<boost::thread> threads;
	{
		...
		ASSERT_TRUE (wallet->insert_adhoc (lumex::dev::genesis_key.prv)); // For voting
		auto insert_result = wallet->insert_adhoc (key.prv);
		ASSERT_TRUE (insert_result);
		ASSERT_EQ (key.pub, insert_result.value ());
		node1->start ();
		system.nodes.push_back (node1);
		ASSERT_NE (nullptr, lumex::test::establish_tcp (system, *node1, node->network.endpoint ()));
		node1->workers.post_delayed (std::chrono::milliseconds (200), ([&system, &key] () {
			auto hash (system.wallet (0)->send_sync (lumex::dev::genesis_key.pub, key.pub, system.nodes[0]->config.receive_minimum.number ()));
...
TEST (ledger, deep_account_compute)
{
	...
	lumex::keypair key;
	auto balance (lumex::dev::constants.genesis_amount - 1);
...
TEST (wallet, multithreaded_send_async)
{
	std::vector<boost::thread> threads;
	{
		...
		wallet_l->insert_adhoc (lumex::dev::genesis_key.prv);
		...
		for (auto i (0); i < num_of_threads; ++i)
		{
			threads.push_back (boost::thread ([wallet_l, &key, num_of_threads, num_of_sends] () {
				...
				{
					wallet_l->send_async (lumex::dev::genesis_key.pub, key.pub, 1000, [] (std::shared_ptr<lumex::block> const & block_a) {
		...
		ASSERT_TIMELY_EQ (1000s, system.nodes[0]->balance (lumex::dev::genesis_key.pub), (lumex::dev::constants.genesis_amount - num_of_threads * num_of_sends * 1000));
	}
	for (auto i (threads.begin ()), n (threads.end ()); i != n; ++i)
	{
		i->join ();
		...
		threads.push_back (boost::thread ([&system] () {
			for (auto i (0); i != 1000; ++i)
			{
				...
				for (auto j (0); j != 10; ++j)
				{
					...
					system.nodes[0]->store.account.put (transaction, account, lumex::account_info ());
				}
			}
		}));
	}
...
TEST (node, fork_storm)
{
	...
	lumex::test::system system (node_count, lumex::transport::transport_type::tcp, flags);
	system.wallet (0)->insert_adhoc (lumex::dev::genesis_key.prv);
	auto previous (system.nodes[0]->latest (lumex::dev::genesis_key.pub));
	auto balance (system.nodes[0]->balance (lumex::dev::genesis_key.pub));
	...
	for (auto node_j : system.nodes)
	{
		...
		for (auto node_i : system.nodes)
		{
			...
			ASSERT_EQ (lumex::block_status::progress, send_result);
			lumex::keypair rep;
			auto open = builder
						.open ()
						.source (previous)
			...
			ASSERT_EQ (lumex::block_status::progress, open_result);
			auto transaction (node_i->store.tx_begin_read ());
			node_i->network.flood_block (open, lumex::transport::traffic_type::test);
		}
	}
	auto again (true);
	...
	while (again)
	{
		auto empty = 0;
		auto single = 0;
		std::for_each (system.nodes.begin (), system.nodes.end (), [&] (std::shared_ptr<lumex::node> const & node_a) {
		...
		for (auto i (nodes.begin ()), n (nodes.end ()); i != n; ++i)
		{
			switch (*i)
			{
				...
				case 1:
					...
					{
						++message_count;
						auto entry (lumex::random_pool::generate_word32 (0, node_count - 1));
						switch (nodes[entry])
						{
							case 0:
								nodes[entry] = 1;
...
TEST (store, unchecked_load)
{
	...
	lumex::block_builder builder;
	std::shared_ptr<lumex::block> block = builder
										 .send ()
										 .previous (0)
										 .destination (0)
										 .balance (0)
...
TEST (store, vote_load)
{
	...
	{
		auto vote = std::make_shared<lumex::vote> (lumex::dev::genesis_key.pub, lumex::dev::genesis_key.prv, i, 0, std::vector<lumex::block_hash>{ i });
		node.vote_processor.vote (vote, std::make_shared<lumex::transport::inproc::channel> (node, node));
	}
}
...
TEST (store, pruned_load)
{
	...
	boost::unordered_set<lumex::block_hash> hashes;
	{
...
TEST (wallets, rep_scan)
{
	...
	node.wallets.foreach_representative ([] (lumex::public_key const & pub_a, lumex::raw_key const & prv_a) {
	});
...
TEST (node, mass_vote_by_hash)
{
	...
	for (auto i (0); i < 10000; ++i)
	{
		auto block = builder
					 ...
					 .previous (previous)
					 .representative (lumex::dev::genesis_key.pub)
					 .balance (lumex::dev::constants.genesis_amount - (i + 1) * lumex::Klumex_ratio)
					 .link (key.pub)
					 .sign (lumex::dev::genesis_key.prv, lumex::dev::genesis_key.pub)
	...
	{
		system.nodes[0]->block_processor.add (*i, lumex::block_source::test);
	}
}
...
TEST (confirmation_height, many_accounts_single_confirmation)
{
	...
	// The number of frontiers should be more than the lumex::confirmation_height::unbounded_cutoff to test the amount of blocks confirmed is correct.
	auto const num_accounts = lumex::confirmation_height::unbounded_cutoff * 2 + 50;
	lumex::keypair last_keypair = lumex::dev::genesis_key;
	lumex::block_builder builder;
	auto last_open_hash = node->latest (lumex::dev::genesis_key.pub);
	{
		...
		for (auto i = num_accounts - 1; i > 0; --i)
		{
			...
			auto open = builder
						...
						.build ();
			ASSERT_EQ (lumex::block_status::progress, node->ledger.process (transaction, open));
	...
	ASSERT_EQ (node->ledger.stats.count (lumex::stat::type::confirmation_height, lumex::stat::detail::blocks_cemented, lumex::stat::dir::in), num_accounts * 2 - 2);
	ASSERT_EQ (node->ledger.stats.count (lumex::stat::type::confirmation_height, lumex::stat::detail::blocks_cemented_bounded, lumex::stat::dir::in), num_accounts * 2 - 2);
	ASSERT_EQ (node->ledger.stats.count (lumex::stat::type::confirmation_height, lumex::stat::detail::blocks_cemented_unbounded, lumex::stat::dir::in), 0);

	ASSERT_TIMELY_EQ (40s, (node->ledger.cemented_count () - 1), node->stats.count (lumex::stat::type::confirmation_observer, lumex::stat::dir::out));
}
...
TEST (confirmation_height, many_accounts_many_confirmations)
{
	...
	auto const num_accounts = lumex::confirmation_height::unbounded_cutoff * 2 + 50;
	auto latest_genesis = node->latest (lumex::dev::genesis_key.pub);
	lumex::block_builder builder;
	std::vector<std::shared_ptr<lumex::open_block>> open_blocks;
	{
		auto transaction = node->ledger.tx_begin_write ();
		for (auto i = num_accounts - 1; i > 0; --i)
		{
	...
	ASSERT_TIMELY_EQ (1500s, node->stats.count (lumex::stat::type::confirmation_height, lumex::stat::detail::blocks_cemented, lumex::stat::dir::in), num_blocks_to_confirm);

	auto num_confirmed_bounded = node->ledger.stats.count (lumex::stat::type::confirmation_height, lumex::stat::detail::blocks_cemented_bounded, lumex::stat::dir::in);
	ASSERT_GE (num_confirmed_bounded, lumex::confirmation_height::unbounded_cutoff);
	ASSERT_EQ (node->ledger.stats.count (lumex::stat::type::confirmation_height, lumex::stat::detail::blocks_cemented_unbounded, lumex::stat::dir::in), num_blocks_to_confirm - num_confirmed_bounded);

	ASSERT_TIMELY_EQ (60s, (node->ledger.cemented_count () - 1), node->stats.count (lumex::stat::type::confirmation_observer, lumex::stat::dir::out));
	...
	ASSERT_TIMELY_EQ (20s, (node->ledger.cemented_count () - 1), node->stats.count (lumex::stat::type::confirmation_observer, lumex::stat::dir::out));
}
...
TEST (confirmation_height, long_chains)
{
	...
	auto const num_blocks = lumex::confirmation_height::unbounded_cutoff * 2 + 50;
	...
	auto send = builder
				...
				.balance (lumex::dev::constants.genesis_amount - lumex::Klumex_ratio + num_blocks + 1)
				.sign (lumex::dev::genesis_key.prv, lumex::dev::genesis_key.pub)
	...
	auto previous_destination_chain_hash = open->hash ();
	{
		...
		for (auto i = num_blocks - 1; i > 0; --i)
		{
			auto send = builder
						...
						.destination (key1.pub)
						.balance (lumex::dev::constants.genesis_amount - lumex::Klumex_ratio + i + 1)
						.sign (lumex::dev::genesis_key.prv, lumex::dev::genesis_key.pub)
			...
			auto receive = builder
						   ...
						   .build ();
			ASSERT_EQ (lumex::block_status::progress, node->ledger.process (transaction, receive));
	...
	auto receive1 = builder
					...
					.previous (previous_genesis_chain_hash)
					.representative (lumex::dev::genesis_key.pub)
					.balance (lumex::dev::constants.genesis_amount - lumex::Klumex_ratio + 1)
					.link (send1->hash ())
					.sign (lumex::dev::genesis_key.prv, lumex::dev::genesis_key.pub)
					.work (*system.work.generate (previous_genesis_chain_hash))
					.build ();
...

	{
		...
		ASSERT_EQ (lumex::block_status::progress, node->ledger.process (transaction, receive1));
	...
	ASSERT_TRUE (info);
	lumex::confirmation_height_info confirmation_height_info;
	ASSERT_FALSE (node->store.confirmation_height.get (transaction, lumex::dev::genesis_key.pub, confirmation_height_info));
	...
	ASSERT_EQ (cemented_count, node->ledger.cemented_count ());
	ASSERT_EQ (node->ledger.stats.count (lumex::stat::type::confirmation_height, lumex::stat::detail::blocks_cemented, lumex::stat::dir::in), num_blocks * 2 + 2);
	ASSERT_EQ (node->ledger.stats.count (lumex::stat::type::confirmation_height, lumex::stat::detail::blocks_cemented_bounded, lumex::stat::dir::in), num_blocks * 2 + 2);
	ASSERT_EQ (node->ledger.stats.count (lumex::stat::type::confirmation_height, lumex::stat::detail::blocks_cemented_unbounded, lumex::stat::dir::in), 0);
...
TEST (confirmation_height, dynamic_algorithm)
{
	...
	lumex::keypair key;
	system.wallet (0)->insert_adhoc (lumex::dev::genesis_key.prv);
	auto const num_blocks = lumex::confirmation_height::unbounded_cutoff;
	auto latest_genesis = lumex::dev::genesis;
	std::vector<std::shared_ptr<lumex::state_block>> state_blocks;
	lumex::block_builder builder;
	...
	ASSERT_EQ (node->ledger.stats.count (lumex::stat::type::confirmation_height, lumex::stat::detail::blocks_cemented, lumex::stat::dir::in), num_blocks);
	ASSERT_EQ (node->ledger.stats.count (lumex::stat::type::confirmation_height, lumex::stat::detail::blocks_cemented_bounded, lumex::stat::dir::in), 1);
	ASSERT_EQ (node->ledger.stats.count (lumex::stat::type::confirmation_height, lumex::stat::detail::blocks_cemented_unbounded, lumex::stat::dir::in), num_blocks - 1);
}
...
TEST (confirmation_height, many_accounts_send_receive_self)
{
	...
	std::vector<std::shared_ptr<lumex::open_block>> open_blocks;
	{
		...
		for (auto i = 0; i < num_accounts; ++i)
		{
			...
			auto send = builder
						...
						.previous (latest_genesis)
						.destination (key.pub)
						.balance (lumex::dev::constants.genesis_amount - 1 - i)
						.sign (lumex::dev::genesis_key.prv, lumex::dev::genesis_key.pub)
						.work (*system.work.generate (latest_genesis))
						.build ();
			ASSERT_EQ (lumex::block_status::progress, node->ledger.process (transaction, send));
	...
	while (node->stats.count (lumex::stat::type::confirmation_height, lumex::stat::detail::blocks_cemented, lumex::stat::dir::in) != num_blocks_to_confirm)
	{
		ASSERT_NO_ERROR (system.poll ());
	}

	std::vector<std::shared_ptr<lumex::send_block>> send_blocks;
	std::vector<std::shared_ptr<lumex::receive_block>> receive_blocks;
	...
	while ((node->ledger.cemented_count () - 1) != node->stats.count (lumex::stat::type::confirmation_observer, lumex::stat::dir::out))
	{
		ASSERT_NO_ERROR (system.poll ());
	}
...
TEST (confirmation_height, many_accounts_send_receive_self_no_elections)
{
	...
	lumex::ledger ledger (*store, lumex::dev::network_params, stats, logger);
	lumex::store::write_queue write_database_queue;
	lumex::work_pool pool{ lumex::dev::network_params.network, std::numeric_limits<unsigned>::max () };
	std::atomic<bool> stopped{ false };
	boost::latch initialized_latch{ 0 };

	lumex::block_hash block_hash_being_processed{ 0 };
	lumex::store::write_queue write_queue;

	lumex::node_config node_config;
	lumex::unchecked_map unchecked{ 0, stats, false };
	lumex::ledger_notifications ledger_notifications{ node_config, stats, logger };
	lumex::cementing_set_config cementing_set_config{};
	lumex::cementing_set cementing_set{ cementing_set_config, ledger, ledger_notifications, stats, logger };

	auto const num_accounts = 100000;

	auto latest_genesis = lumex::dev::genesis->hash ();
	std::vector<lumex::keypair> keys;
	std::vector<std::shared_ptr<lumex::open_block>> open_blocks;
	...
	auto num_blocks_to_confirm = num_accounts * 2;
	while (stats.count (lumex::stat::type::confirmation_height, lumex::stat::detail::blocks_cemented, lumex::stat::dir::in) != num_blocks_to_confirm)
	{
		ASSERT_NO_ERROR (system.poll ());
	}
	...
	// Now add all send/receive blocks
	{
		...
		for (int i = 0; i < open_blocks.size (); ++i)
		{
			...
			ASSERT_EQ (lumex::block_status::progress, ledger.process (transaction, send_blocks.back ()));
			ASSERT_EQ (lumex::block_status::progress, ledger.process (transaction, receive_blocks.back ()));
		}
	}
...
{
public:
	lumex::test::counted_completion write_completion{ 0 };
...
TEST (telemetry, ongoing_requests)
{
	...
	ASSERT_EQ (0, node_client->stats.count (lumex::stat::type::bootstrap, lumex::stat::detail::telemetry_ack, lumex::stat::dir::in));
	ASSERT_EQ (0, node_client->stats.count (lumex::stat::type::bootstrap, lumex::stat::detail::telemetry_req, lumex::stat::dir::out));

	ASSERT_TIMELY (20s, node_client->stats.count (lumex::stat::type::message, lumex::stat::detail::telemetry_ack, lumex::stat::dir::in) == 1 && node_server->stats.count (lumex::stat::type::message, lumex::stat::detail::telemetry_ack, lumex::stat::dir::in) == 1);

	// Wait till the next ongoing will be called, and add a 1s buffer for the actual processing
	auto time = std::chrono::steady_clock::now ();
	ASSERT_TIMELY (10s, std::chrono::steady_clock::now () >= (time + lumex::dev::network_params.network.telemetry_cache_cutoff + 1s));

	ASSERT_EQ (2, node_client->stats.count (lumex::stat::type::message, lumex::stat::detail::telemetry_ack, lumex::stat::dir::in));
	ASSERT_EQ (2, node_client->stats.count (lumex::stat::type::message, lumex::stat::detail::telemetry_req, lumex::stat::dir::in));
	ASSERT_EQ (2, node_client->stats.count (lumex::stat::type::message, lumex::stat::detail::telemetry_req, lumex::stat::dir::out));
	ASSERT_EQ (2, node_server->stats.count (lumex::stat::type::message, lumex::stat::detail::telemetry_ack, lumex::stat::dir::in));
	ASSERT_EQ (2, node_server->stats.count (lumex::stat::type::message, lumex::stat::detail::telemetry_req, lumex::stat::dir::in));
	ASSERT_EQ (2, node_server->stats.count (lumex::stat::type::message, lumex::stat::detail::telemetry_req, lumex::stat::dir::out));
}
...
TEST (telemetry, under_load)
{
	...
	auto send = builder
				...
				.previous (latest_genesis)
				.representative (lumex::dev::genesis_key.pub)
				.balance (lumex::dev::constants.genesis_amount - num_blocks)
				.link (key.pub)
				.sign (lumex::dev::genesis_key.prv, lumex::dev::genesis_key.pub)
				.work (*system.work.generate (latest_genesis))
				.build ();
	...
	auto thread_func = [key1, &system, node, num_blocks] (lumex::keypair const & keypair, lumex::block_hash const & latest, lumex::uint128_t const initial_amount) {
	...
	std::thread thread1 (thread_func, lumex::dev::genesis_key, latest_genesis, lumex::dev::constants.genesis_amount - num_blocks);
	...
	{
		ASSERT_EQ (0, node->stats.count (lumex::stat::type::telemetry, lumex::stat::detail::failed_send_telemetry_req));
		ASSERT_EQ (0, node->stats.count (lumex::stat::type::telemetry, lumex::stat::detail::request_within_protection_cache_zone));
		ASSERT_EQ (0, node->stats.count (lumex::stat::type::telemetry, lumex::stat::detail::unsolicited_telemetry_ack));
		ASSERT_EQ (0, node->stats.count (lumex::stat::type::telemetry, lumex::stat::detail::no_response_received));
	}
}
 ...
 * The telemetry reply is saved in the callback and then it is also requested via lumex::telemetry::get_metrics().
...
TEST (telemetry, cache_read_and_timeout)
{
	...
	ASSERT_EQ (1, node_client->stats.count (lumex::stat::type::message, lumex::stat::detail::telemetry_ack, lumex::stat::dir::in));
	ASSERT_EQ (0, node_client->stats.count (lumex::stat::type::message, lumex::stat::detail::telemetry_req, lumex::stat::dir::in));
	ASSERT_EQ (1, node_client->stats.count (lumex::stat::type::message, lumex::stat::detail::telemetry_req, lumex::stat::dir::out));
	ASSERT_EQ (0, node_server->stats.count (lumex::stat::type::message, lumex::stat::detail::telemetry_ack, lumex::stat::dir::in));
	ASSERT_EQ (1, node_server->stats.count (lumex::stat::type::message, lumex::stat::detail::telemetry_req, lumex::stat::dir::in));
	ASSERT_EQ (0, node_server->stats.count (lumex::stat::type::message, lumex::stat::detail::telemetry_req, lumex::stat::dir::out));
...
TEST (telemetry, many_nodes)
{
	...
	auto const num_nodes = lumex::memory_intensive_instrumentation () ? 4 : 10;
	for (auto i = 0; i < num_nodes; ++i)
	{
		lumex::node_config node_config = system.default_config ();
		...
		auto node = std::make_shared<lumex::node> (lumex::unique_path (), node_config, system.work, node_flags);
		node->start ();
	...
	lumex::test::wait_peer_connections (system);
	...
	std::vector<lumex::messages::telemetry_data> telemetry_datas;
	...
	for (auto & data : telemetry_datas)
	{
		...
		ASSERT_EQ (data.protocol_version, lumex::dev::network_params.network.protocol_version);
		ASSERT_GE (data.bandwidth_cap, 100000);
		ASSERT_LT (data.bandwidth_cap, 100000 + system.nodes.size ());
		ASSERT_EQ (data.major_version, lumex::get_major_node_version ());
		ASSERT_EQ (data.minor_version, lumex::get_minor_node_version ());
		ASSERT_EQ (data.patch_version, lumex::get_patch_node_version ());
		ASSERT_EQ (data.pre_release_version, lumex::get_pre_release_node_version ());
		ASSERT_EQ (data.maker, lumex::messages::telemetry_maker::nf_node);
		ASSERT_LT (data.uptime, 100);
		ASSERT_EQ (data.genesis_block, lumex::dev::genesis->hash ());
		ASSERT_LE (data.timestamp, std::chrono::system_clock::now ());
		ASSERT_EQ (data.active_difficulty, system.nodes.front ()->default_difficulty (lumex::work_version::work_1));
	}
	...
	auto perform_test = [] (size_t const batch_size) {
		...
		for (auto & accounts : all)
		{
			for (auto & info : *accounts)
			{
				...
				auto block = builder
							 ...
							 .representative (lumex::dev::genesis_key.pub)
							 .sign (lumex::dev::genesis_key.prv, lumex::dev::genesis_key.pub)
							 .work (*node.work_generate_blocking (latest, node_config.network_params.work.threshold (lumex::work_version::work_1, lumex::block_details (lumex::epoch::epoch_0, false, false, false))))
							 .build (ec);
				ASSERT_FALSE (ec);
				ASSERT_NE (nullptr, block);
		...
		for (auto const & info : opened)
		{
			...
			auto block = builder
						 ...
						 .work (*node.work_generate_blocking (info.key.pub, node_config.network_params.work.threshold (lumex::work_version::work_1, lumex::block_details (lumex::epoch::epoch_0, false, false, false))))
						 .build (ec);
			ASSERT_FALSE (ec);
			ASSERT_NE (nullptr, block);
			ASSERT_EQ (lumex::block_status::progress, node.process (block));
		...
		while (node.ledger.block_count () != block_count_before + total_to_upgrade)
		{
			...
			ASSERT_FALSE (node.epoch_upgrader.start (epoch_signer.prv, lumex::epoch::epoch_1, upgrade_count, threads));
			// Already ongoing - should fail
			ASSERT_TRUE (node.epoch_upgrader.start (epoch_signer.prv, lumex::epoch::epoch_1, upgrade_count, threads));
			system.deadline_set (60s);
...
TEST (node, mass_block_new)
{
	...
	// Upgrade to epoch_2
	system.upgrade_genesis_epoch (node, lumex::epoch::epoch_1);
	system.upgrade_genesis_epoch (node, lumex::epoch::epoch_2);

	auto next_block_count = num_blocks + 3;
	auto process_all = [&] (std::vector<std::shared_ptr<lumex::state_block>> const & blocks_a) {
	...
	lumex::keypair key;
	std::vector<lumex::keypair> keys (num_blocks);
	lumex::state_block_builder builder;
	std::vector<std::shared_ptr<lumex::state_block>> send_blocks;
	auto send_threshold (lumex::dev::network_params.work.threshold (lumex::work_version::work_1, lumex::block_details (lumex::epoch::epoch_2, true, false, false)));
	auto latest_genesis = node.latest (lumex::dev::genesis_key.pub);
	for (auto i = 0; i < num_blocks; ++i)
	{
		auto send = builder.make_block ()
					...
					.link (keys[i].pub)
					.sign (lumex::dev::genesis_key.prv, lumex::dev::genesis_key.pub)
					.work (*system.work.generate (lumex::work_version::work_1, latest_genesis, send_threshold))
					.build ();
		latest_genesis = send->hash ();
	...
	std::vector<std::shared_ptr<lumex::state_block>> open_blocks;
	auto receive_threshold (lumex::dev::network_params.work.threshold (lumex::work_version::work_1, lumex::block_details (lumex::epoch::epoch_2, false, true, false)));
	for (auto i = 0; i < num_blocks; ++i)
	{
		...
		auto open = builder.make_block ()
					...
					.sign (key.prv, key.pub)
					.work (*system.work.generate (lumex::work_version::work_1, key.pub, receive_threshold))
					.build ();
		open_blocks.push_back (std::move (open));
	}
	...
	std::vector<std::shared_ptr<lumex::state_block>> send_blocks2;
	for (auto i = 0; i < num_blocks; ++i)
	{
		auto const & key = keys[i];
		...
		auto send2 = builder.make_block ()
					 ...
					 .link (key.pub)
					 .sign (key.prv, key.pub)
					 .work (*system.work.generate (lumex::work_version::work_1, latest->hash (), send_threshold))
					 .build ();
	...
	std::vector<std::shared_ptr<lumex::state_block>> receive_blocks;
	for (auto i = 0; i < num_blocks; ++i)
	{
		auto const & key = keys[i];
		...
		auto send2 = builder.make_block ()
					 ...
					 .link (latest->hash ())
					 .sign (key.prv, key.pub)
					 .work (*system.work.generate (lumex::work_version::work_1, latest->hash (), receive_threshold))
					 .build ();
...
TEST (node, aggressive_flooding)
{
	...
	std::vector<std::pair<std::shared_ptr<lumex::node>, std::shared_ptr<lumex::wallet::wallet>>> nodes_wallets;
	nodes_wallets.resize (!lumex::memory_intensive_instrumentation () ? 5 : 3);
	...
	auto large_amount = (lumex::dev::constants.genesis_amount / 2) / nodes_wallets.size ();
	std::vector<std::shared_ptr<lumex::block>> genesis_blocks;
	for (auto & node_wallet : nodes_wallets)
	{
		lumex::keypair keypair;
		node_wallet.second->set_representative (keypair.pub);
		node_wallet.second->insert_adhoc (keypair.prv);
		auto block (wallet1.send_action (lumex::dev::genesis_key.pub, keypair.pub, large_amount));
		ASSERT_NE (nullptr, block);
	...
	for (auto & node_wallet : nodes_wallets)
	{
		for (auto const & block : genesis_blocks)
		{
			...
			ASSERT_TRUE (lumex::block_status::progress == process_result || lumex::block_status::old == process_result);
		}
		ASSERT_EQ (node1.latest (lumex::dev::genesis_key.pub), node_wallet.first->latest (lumex::dev::genesis_key.pub));
		ASSERT_EQ (genesis_blocks.back ()->hash (), node_wallet.first->latest (lumex::dev::genesis_key.pub));
		// Confirm blocks for rep crawler & receiving
		lumex::test::confirm (node_wallet.first->ledger, genesis_blocks.back ());
	}
	lumex::test::confirm (node1.ledger, genesis_blocks.back ());
	...
	ASSERT_TIMELY (!lumex::slow_instrumentation () ? 10s : 40s, all_received ());

	ASSERT_TIMELY_EQ (!lumex::slow_instrumentation () ? 10s : 40s, node1.ledger.block_count (), 1 + 2 * nodes_wallets.size ());

	// Wait until the main node sees all representatives
	ASSERT_TIMELY_EQ (!lumex::slow_instrumentation () ? 10s : 40s, node1.rep_crawler.principal_representatives ().size (), nodes_wallets.size ());

	// Generate blocks and ensure they are sent to all representatives
	lumex::state_block_builder builder;
	std::shared_ptr<lumex::state_block> block{};
	{
		auto transaction = node1.ledger.tx_begin_read ();
		block = builder.make_block ()
				.account (lumex::dev::genesis_key.pub)
				.representative (lumex::dev::genesis_key.pub)
				.previous (node1.ledger.any.account_head (transaction, lumex::dev::genesis_key.pub))
				.balance (node1.ledger.any.account_balance (transaction, lumex::dev::genesis_key.pub).value ().number () - 1)
				.link (lumex::dev::genesis_key.pub)
				.sign (lumex::dev::genesis_key.prv, lumex::dev::genesis_key.pub)
				.work (*node1.work_generate_blocking (node1.ledger.any.account_head (transaction, lumex::dev::genesis_key.pub)))
				.build ();
	}
	// Processing locally goes through the aggressive block flooding path
	ASSERT_EQ (lumex::block_status::progress, node1.process_local (block).value ());

	auto all_have_block = [&nodes_wallets] (lumex::block_hash const & hash_a) {
	...
	ASSERT_TIMELY (!lumex::slow_instrumentation () ? 5s : 25s, all_have_block (block->hash ()));

	// Do the same for a wallet block
	auto wallet_block = wallet1.send_sync (lumex::dev::genesis_key.pub, lumex::dev::genesis_key.pub, 10);
	ASSERT_TIMELY (!lumex::slow_instrumentation () ? 5s : 25s, all_have_block (wallet_block));
...
{
	lumex::test::system system (lumex::memory_intensive_instrumentation () ? 4 : 10);
	lumex::keypair key2;
	system.wallet (0)->insert_adhoc (lumex::dev::genesis_key.prv);
	...
	ASSERT_TIMELY (3.5min, std::all_of (system.nodes.begin (), system.nodes.end (), [&] (std::shared_ptr<lumex::node> const & node_a) { return !node_a->balance (key2.pub).is_zero (); }));
	system.stop ();
	for (auto node : system.nodes)
	{
...
TEST (system, block_sequence)
{
	...
	lumex::test::system system;
	std::deque<lumex::keypair> reps;
	for (auto i = 0; i < pr_count; ++i)
	{
		reps.push_back (lumex::keypair{});
	}
	system.ledger_initialization_set (reps, lumex::Klumex_ratio);
	...
	for (auto rep : reps)
	{
		system.wallet (0);
		config.peering_port = system.get_available_port ();
		auto pr = system.add_node (config, flags, lumex::transport::transport_type::tcp, rep);
	...
	while (std::any_of (system.nodes.begin (), system.nodes.end (), [] (std::shared_ptr<lumex::node> const & node) {
	...
	for (auto i = 0; i < block_count; ++i)
	{
		...
		auto block = wallet->send_action (lumex::dev::genesis_key.pub, key.pub, 1);
	...
	while (!done)
	{
		...
		done = std::all_of (system.nodes.begin (), system.nodes.end (), [&blocks] (std::shared_ptr<lumex::node> node) { return node->block_confirmed (blocks.back ()->hash ()); });
		system.poll ();
	}
}
} // namespace lumex
