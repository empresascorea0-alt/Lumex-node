#include <lumex/boost/asio/connect.hpp>
...
void write_config_files (std::filesystem::path const & data_path, int index)
{
	...
	toml.write (lumex::get_node_toml_config_path (data_path));

	lumex::rpc_config rpc_config{ daemon_config.node.network_params.network };
	rpc_config.port = rpc_port_start + index;
	rpc_config.enable_control = true;
	rpc_config.rpc_process.ipc_port = ipc_port_start + index;

	// Write rpc config
	lumex::tomlconfig toml_rpc;
	rpc_config.serialize_toml (toml_rpc);
	toml_rpc.write (lumex::get_rpc_toml_config_path (data_path));
}
...
int main (int argc, char * const * argv)
{
	...
	description.add_options ()
		...
		("node_path", boost::program_options::value<std::string> (), "The path to the lumex_node to test")
		("rpc_path", boost::program_options::value<std::string> (), "The path to the lumex_rpc to test");
	// clang-format on
	...
	{
		auto node_filepath = running_executable_filepath.parent_path () / "lumex_node";
	...
	{
		std::cerr << "lumex_node executable could not be found in " << node_path << std::endl;
		return 1;
	}
	...
	{
		auto rpc_filepath = running_executable_filepath.parent_path () / "lumex_rpc";
		if (running_executable_filepath.has_extension ())
		{
	...
	{
		std::cerr << "lumex_rpc executable could not be found in " << rpc_path << std::endl;
		return 1;
	}
	...
	std::string current_network{ lumex::dev::network_params.network.get_current_network_as_string () };
	...
	std::thread t ([send_count, &ioc, &primary_node_results, &resolver, &node_count, &destination_count] () {
		...
		wallet_add_rpc (ioc, primary_node_results, wallet, lumex::dev::genesis_key.prv.to_string ());
		...
		for (auto i = 0; i < send_count; ++i)
		{
			...
			auto send_receive = std::make_shared<send_receive_impl> (ioc, wallet, lumex::dev::genesis_key.pub.to_account (), destination_account->as_string, send_calls_remaining, primary_node_results);
			boost::asio::strand<boost::asio::io_context::executor_type> strand{ ioc.get_executor () };
			boost::asio::post (strand,
			[send_receive] () {
				send_receive->start ();
	...
	lumex::thread_runner runner (ioc_shared, lumex::default_logger (), simultaneous_process_calls);
	t.join ();
	runner.join ();
