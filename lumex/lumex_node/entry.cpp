#include <lumex/boost/process/process.hpp>
...
int main (int argc, char * const * argv)
{
	...
	description.add_options ()
		...
		("config", boost::program_options::value<std::vector<lumex::config_key_value_pair>>()->multitoken(), "Pass node configuration values. This takes precedence over any values in the configuration file. This option can be repeated multiple times.")
		("rpcconfig", boost::program_options::value<std::vector<lumex::config_key_value_pair>>()->multitoken(), "Pass rpc configuration values. This takes precedence over any values in the configuration file. This option can be repeated multiple times.")
		...
		("debug_output_last_backtrace_dump", "Displays the contents of the latest backtrace in the event of a lumex_node crash")
		("debug_generate_crash_report", "Consolidates the lumex_node_backtrace.dump file. Requires addr2line installed on Linux")
		...
		("debug_profile_process", "Profile active blocks processing (only for lumex_dev_network)")
		...
		("debug_profile_votes", "Profile votes processing (only for lumex_dev_network)")
		("debug_profile_frontiers_confirmation", "Profile frontiers confirmation speed (only for lumex_dev_network)")
		...
		("pid_file", boost::program_options::value<std::string> ()->implicit_value ("lumex_node.pid"), "Write process ID to file. Optional path argument, defaults to lumex_node.pid in current directory")
	...
	if (auto existing = vm.find ("pid_file"); existing != vm.end ())
	{
		...
		lumex::runtime_files::create_pid_file (pid_filepath);
	...
	if (ec == lumex::error_cli::unknown_command)
	{
		...
		{
			if (lumex::get_active_network () != lumex::network_type::lumex_dev_network)
			{
				auto node_flags = lumex::inactive_node_flag_defaults ();
				lumex::update_flags (node_flags, vm);
				node_flags.generate_cache.reps = true;
				lumex::inactive_node inactive_node (data_path, node_flags);
				...
				auto get_total = [] (decltype (bootstrap_weights.representatives) const & reps) -> lumex::uint128_union {
					return std::accumulate (reps.begin (), reps.end (), lumex::uint128_t{ 0 }, [] (auto sum, auto const & rep) { return sum + rep.second; });
				};
				...
				{
					std::vector<std::pair<lumex::account, lumex::uint128_t>> sorted;
				...
				{
					lumex::account rep;
					lumex::uint128_union hardcoded;
					lumex::uint128_union ledger;
					lumex::uint128_union diff;
					std::string get_entry () const
					{
						return boost::str (boost::format ("representative %1% hardcoded %2% ledger %3% mismatch %4%")
						% rep.to_account () % hardcoded.format_balance (lumex::lumex_ratio, 0, true) % ledger.format_balance (lumex::lumex_ratio, 0, true) % diff.format_balance (lumex::lumex_ratio, 0, true));
					}
				};
				...
				std::transform (hardcoded.begin (), hardcoded.end (), std::back_inserter (mismatched), [&ledger] (auto const & rep) {
					...
					lumex::uint128_t ledger_weight = (ledger_rep == ledger.end () ? 0 : ledger_rep->second);
				...
				lumex::uint128_union const mismatch_total = std::accumulate (mismatched.begin (), mismatched.end (), lumex::uint128_t{ 0 }, [] (auto sum, mismatched_t const & sample) { return sum + sample.diff.number (); });
				lumex::uint128_union const mismatch_mean = mismatch_total.number () / mismatched.size ();

				lumex::uint512_union mismatch_variance = std::accumulate (mismatched.begin (), mismatched.end (), lumex::uint512_t (0), [M = mismatch_mean.number (), N = mismatched.size ()] (lumex::uint512_t sum, mismatched_t const & sample) {
					auto x = sample.diff.number ();
					lumex::uint512_t const mean_diff = x > M ? x - M : M - x;
					lumex::uint512_t const sqr = mean_diff * mean_diff;
					return sum + sqr;
				})
				/ mismatched.size ();

				lumex::uint128_union const mismatch_stddev = lumex::narrow_cast<lumex::uint128_t> (boost::multiprecision::sqrt (mismatch_variance.number ()));

				auto const outlier_threshold = std::max (lumex::Klumex_ratio, mismatch_mean.number () + 1 * mismatch_stddev.number ());
				...
				auto const newcomer_threshold = std::max (lumex::Klumex_ratio, mismatch_mean.number ());
				std::vector<std::pair<lumex::account, lumex::uint128_t>> newcomers;
				...
				auto newcomer_entry = [] (auto const & rep) {
					return boost::str (boost::format ("representative %1% hardcoded --- ledger %2%") % rep.first.to_account () % lumex::uint128_union (rep.second).format_balance (lumex::lumex_ratio, 0, true));
				};

				std::cout << boost::str (boost::format ("hardcoded weight %1% Mlumex at %2% blocks\nledger weight %3% Mlumex at %4% blocks\nmismatched\n\tsamples %5%\n\ttotal %6% Mlumex\n\tmean %7% Mlumex\n\tsigma %8% Mlumex\n")
				% total_hardcoded.format_balance (lumex::lumex_ratio, 0, true)
				% hardcoded_height
				% total_ledger.format_balance (lumex::lumex_ratio, 0, true)
				% ledger_height
				% mismatched.size ()
				% mismatch_total.format_balance (lumex::lumex_ratio, 0, true)
				% mismatch_mean.format_balance (lumex::lumex_ratio, 0, true)
				% mismatch_stddev.format_balance (lumex::lumex_ratio, 0, true));
				...
				// Log more data
				auto const log_threshold = lumex::Klumex_ratio;
		...
		else if (vm.count ("debug_bootstrap_generate"))
		{
			...
			if (key_it != vm.end ())
			{
				lumex::uint256_union key;
				if (!key.decode_hex (key_it->second.as<std::string> ()))
				{
					lumex::keypair genesis (key.to_string ());
					lumex::work_pool work{ network_params.network, std::numeric_limits<unsigned>::max () };
					std::cout << "Genesis: " << genesis.prv.to_string () << "\n"
							  << "Public: " << genesis.pub.to_string () << "\n"
							  << "Account: " << genesis.pub.to_account () << "\n";
					lumex::keypair landing;
					...
					lumex::uint128_t balance (std::numeric_limits<lumex::uint128_t>::max ());
					lumex::open_block genesis_block (reinterpret_cast<lumex::block_hash const &> (genesis.pub), genesis.pub, genesis.pub, genesis.prv, genesis.pub, *work.generate (lumex::work_version::work_1, genesis.pub, network_params.work.epoch_1));
					std::cout << genesis_block.to_json ();
					std::cout.flush ();
					lumex::block_hash previous (genesis_block.hash ());
					for (auto i (0); i != 8; ++i)
					{
						lumex::uint128_t yearly_distribution (lumex::uint128_t (1) << (127 - (i == 7 ? 6 : i)));
						...
						for (auto j (0); j != 52; ++j)
						{
							...
							lumex::send_block send (previous, landing.pub, balance, genesis.prv, genesis.pub, *work.generate (lumex::work_version::work_1, previous, network_params.work.epoch_1));
							previous = send.hash ();
		...
		else if (vm.count ("debug_dump_representatives"))
		{
			...
			auto node = inactive_node.node;
			auto transaction (node->store.tx_begin_read ());
			lumex::uint128_t total;
			auto rep_amounts = node->ledger.rep_weights.get_rep_amounts ();
			std::map<lumex::account, lumex::uint128_t> ordered_reps (rep_amounts.begin (), rep_amounts.end ());
		...
		else if (vm.count ("debug_dump_frontier_unchecked_dependents"))
		{
			...
			auto transaction (node->store.tx_begin_read ());
			std::unordered_set<lumex::block_hash> frontier_hashes;
			...
			node->unchecked.for_each ([&frontier_hashes] (lumex::unchecked_key const & key, lumex::unchecked_info const & info) {
		...
		{
			uint64_t difficulty{ lumex::work_thresholds::publish_full.base };
			...
			if (multiplier_it != vm.end ())
			{
				try
				{
					...
					difficulty = lumex::difficulty::from_multiplier (multiplier, difficulty);
				}
			...
			else
			{
				...
				{
					if (lumex::from_string_hex (difficulty_it->second.as<std::string> (), difficulty))
					{
			...
			if (!result)
			{
				std::cerr << boost::str (boost::format ("Starting generation profiling. Difficulty: %1$#x (%2%x from base difficulty %3$#x)\n") % difficulty % lumex::to_string (lumex::difficulty::to_multiplier (difficulty, lumex::work_thresholds::publish_full.base), 4) % lumex::work_thresholds::publish_full.base);
				while (!result)
				{
					block.hashables.previous.qwords[0] += 1;
					auto begin1 (std::chrono::high_resolution_clock::now ());
					block.block_work_set (*work.generate (lumex::work_version::work_1, block.root (), difficulty));
					auto end1 (std::chrono::high_resolution_clock::now ());
			...
			if (!error)
			{
				...
				if (!result)
				{
					...
					if (!error)
					{
						...
						{
							lumex::logger logger;
							lumex::opencl_config config (platform, device, threads);
							auto opencl = lumex::opencl_work::create (true, config, logger, network_params.work);
							...
							for (uint64_t i (0); true; ++i)
							{
								block.hashables.previous.qwords[0] += 1;
								auto begin1 (std::chrono::high_resolution_clock::now ());
								block.block_work_set (*work_pool.generate (lumex::work_version::work_1, block.root (), difficulty));
								auto end1 (std::chrono::high_resolution_clock::now ());
								std::cerr << boost::str (boost::format ("%|1$ 12d|\n") % std::chrono::duration_cast<std::chrono::microseconds> (end1 - begin1).count ());
							}
						}
		...
		{
			if (std::filesystem::exists ("lumex_node_backtrace.dump"))
			{
				// There is a backtrace, so output the contents
				std::ifstream ifs ("lumex_node_backtrace.dump");
		...
		else if (vm.count ("debug_generate_crash_report"))
		{
			if (std::filesystem::exists ("lumex_node_backtrace.dump"))
			{
				...
				std::string crash_report_filename = "lumex_node_crash_report.txt";
				...
				ofs << st;
#else
				// Read all the lumex node files
				boost::system::error_code err;
				auto running_executable_filepath = boost::dll::program_location (err);
				if (!err)
				{
					auto num = 0;
					auto format = boost::format ("lumex_node_crash_load_address_dump_%1%.txt");
					...
					{
						std::ofstream ofs (crash_report_filename);
						lumex::set_secure_perm_file (crash_report_filename);
					}
					...
					// Now run using relative addresses. This will give actual results for other dlls, the results from the lumex_node executable.
					run_addr2line (true);

					if (std::find (system_codes.begin (), system_codes.end (), 0) == system_codes.end ())
					{
						std::cerr << "Error: Check that addr2line is installed and that lumex_node_crash_load_address_dump_*.txt files exist." << std::endl;
						result = -1;
					}
				}
				else
			...
			{
				std::cerr << "Error: lumex_node_backtrace.dump could not be found";
				result = -1;
			}
		}
		...
		{
			lumex::keypair key;
			lumex::uint256_union message;
			auto signature = lumex::sign_message (key.prv, key.pub, message);
			auto begin (std::chrono::high_resolution_clock::now ());
			for (auto i (0u); i < 1000; ++i)
			{
				lumex::validate_message (key.pub, message, signature);
			}
		...
		else if (vm.count ("debug_profile_process"))
		{
			...
			lumex::update_flags (node_flags, vm);
			lumex::inactive_node inactive_node (lumex::unique_path (), data_path, node_flags);
			auto node = inactive_node.node;
			...
			// Generating keys
			std::vector<lumex::keypair> keys (num_accounts);
			std::vector<lumex::root> frontiers (num_accounts);
			std::vector<lumex::uint128_t> balances (num_accounts, 1000000000);
			// Generating blocks
			...
			for (auto i (0); i != num_accounts; ++i)
			{
				...
				auto send = builder.state ()
							...
							.work (*node->work.generate (lumex::work_version::work_1, genesis_latest, node->network_params.work.epoch_1))
							.build ();
				...
				auto open = builder.state ()
							...
							.work (*node->work.generate (lumex::work_version::work_1, keys[i].pub, node->network_params.work.epoch_1))
							.build ();
			...
			for (auto i (0); i != num_iterations; ++i)
			{
				for (auto j (0); j != num_accounts; ++j)
				{
					...
					auto send = builder.state ()
								...
								.work (*node->work.generate (lumex::work_version::work_1, frontiers[j], node->network_params.work.epoch_1))
								.build ();
					...
					auto receive = builder.state ()
								   ...
								   .work (*node->work.generate (lumex::work_version::work_1, frontiers[other], node->network_params.work.epoch_1))
								   .build ();
			...
			lumex::timer<std::chrono::seconds> timer_l (lumex::timer_state::started);
		...
		{
			lumex::cli::run_block_processing_benchmark (vm, data_path);
		}
		else if (vm.count ("benchmark_cementing"))
		{
			lumex::cli::run_cementing_benchmark (vm, data_path);
		}
		else if (vm.count ("benchmark_elections"))
		{
			lumex::cli::run_elections_benchmark (vm, data_path);
		}
		else if (vm.count ("benchmark_pipeline"))
		{
			lumex::cli::run_pipeline_benchmark (vm, data_path);
		}
		else if (vm.count ("debug_profile_votes"))
		{
			...
			lumex::node_flags node_flags;
			lumex::update_flags (node_flags, vm);
			lumex::node_wrapper node_wrapper (lumex::unique_path (), data_path, node_flags);
			auto node = node_wrapper.node;

			lumex::block_hash genesis_latest (node->latest (lumex::dev::genesis_key.pub));
			lumex::uint128_t genesis_balance (std::numeric_limits<lumex::uint128_t>::max ());
			// Generating keys
			std::vector<lumex::keypair> keys (num_representatives);
			lumex::uint128_t balance ((node->config.online_weight_minimum.number () / num_representatives) + 1);
			...
			// Generating votes
			std::deque<std::shared_ptr<lumex::vote>> votes;
			for (auto j (0); j != num_representatives; ++j)
			{
				uint64_t sequence (1);
				for (auto & i : blocks)
				{
					auto vote (std::make_shared<lumex::vote> (keys[j].pub, keys[j].prv, sequence, 0, std::vector<lumex::block_hash> (1, i->hash ())));
		...
		else if (vm.count ("debug_profile_frontiers_confirmation"))
		{
			...
			lumex::work_pool work{ network_params.network, std::numeric_limits<unsigned>::max () };
			auto path1 (lumex::unique_path ());
			auto path2 (lumex::unique_path ());
			...
			auto error = lumex::read_node_config_toml (data_path, daemon_config, config_overrides);

			lumex::node_config config1 = daemon_config.node;
			...
			auto node1 (std::make_shared<lumex::node> (path1, config1, work, flags, 0));
			lumex::block_hash genesis_latest (node1->latest (lumex::dev::genesis_key.pub));
			lumex::uint128_t genesis_balance (std::numeric_limits<lumex::uint128_t>::max ());
			// Generating blocks
			std::deque<std::shared_ptr<lumex::block>> blocks;
			for (auto i (0); i != count; ++i)
			{
				...
				auto send = builder.state ()
							...
							.link (key.pub)
							.sign (lumex::dev::genesis_key.prv, lumex::dev::genesis_key.pub)
							.work (*work.generate (lumex::work_version::work_1, genesis_latest, lumex::dev::network_params.work.epoch_1))
							.build ();
				...
				auto open = builder.state ()
							...
							.link (genesis_latest)
							.sign (key.prv, key.pub)
							.work (*work.generate (lumex::work_version::work_1, key.pub, lumex::dev::network_params.work.epoch_1))
							.build ();
			...
			node1->block_processor.add_many (blocks, lumex::block_source::test);
			auto iteration (0);
			...
			// Start new node
			lumex::node_config config2 = daemon_config.node;
			...
			auto node2 (std::make_shared<lumex::node> (path2, config2, work, flags, 1));
			node2->start ();
			std::cout << boost::str (boost::format ("Processing %1% blocks (test node)\n") % (count * 2));
			node2->block_processor.add_many (blocks, lumex::block_source::test);
			blocks.clear ();
			...
			auto wallet (node1->wallets.create (lumex::random_wallet_id ()));
		...
		else if (vm.count ("debug_random_feed"))
		{
			 ...
			 *
			 *   ./lumex_node --debug_random_feed | dieharder -a -g 200
			 */
			lumex::raw_key seed;
			for (;;)
			{
		...
		else if (vm.count ("debug_rpc"))
		{
			...
			auto node_flags = lumex::inactive_node_flag_defaults ();
			lumex::update_flags (node_flags, vm);
			node_flags.generate_cache = lumex::generate_cache_flags::all_enabled ();
			lumex::inactive_node inactive_node_l (data_path, node_flags);

			lumex::node_rpc_config config;
			lumex::ipc::ipc_server server (*inactive_node_l.node, config);
			auto handler_l (std::make_shared<lumex::json_handler> (*inactive_node_l.node, config, command_l.str (), response_handler_l));
		...
		else if (vm.count ("validate_blocks") || vm.count ("debug_validate_blocks"))
		{
			...
			std::deque<std::pair<lumex::account, lumex::account_info>> accounts;
			...
			auto print_error_message = [&silent, &errors] (std::string const & error_message_a) {
				if (!silent)
				{
					static lumex::mutex cerr_mutex;
					lumex::lock_guard<lumex::mutex> lock{ cerr_mutex };
			...
			auto check_account = [&print_error_message, &silent, &count, &block_count] (std::shared_ptr<lumex::node> const & node, lumex::secure::read_transaction const & transaction, lumex::account const & account, lumex::account_info const & info) {
				++count;
				...
				lumex::block_hash calculated_hash (0);
				...
				lumex::account calculated_representative{};
				while (!hash.is_zero () && block != nullptr)
				{
					...
					if (block->type () == lumex::block_type::open || block->type () == lumex::block_type::state)
					{
					...
					if (height == 0 && block->type () != lumex::block_type::open && block->type () != lumex::block_type::state)
					{
					...
					if (validate_message (account, hash, block->block_signature ()))
					{
						...
						{
							auto & state_block (static_cast<lumex::state_block &> (*block.get ()));
					...
					if (block->type () == lumex::block_type::open || block->type () == lumex::block_type::change || block->type () == lumex::block_type::state)
					{
			...
			count = 0;
			finished = false;
			std::deque<std::pair<lumex::pending_key, lumex::pending_info>> pending;

			auto check_pending = [&print_error_message, &silent, &count] (std::shared_ptr<lumex::node> const & node, lumex::secure::read_transaction const & transaction, lumex::pending_key const & key, lumex::pending_info const & info) {
				++count;
				...
				else
				{
					...
					if (auto state = dynamic_cast<lumex::state_block *> (block.get ()))
					{
						if (state->is_send ())
						{
					...
					else if (auto send = dynamic_cast<lumex::send_block *> (block.get ()))
					{
		...
		else if (vm.count ("debug_profile_bootstrap"))
		{
			...
			lumex::update_flags (node_flags, vm);
			lumex::inactive_node node (lumex::unique_path (), node_flags);
			auto begin (std::chrono::high_resolution_clock::now ());
			uint64_t block_count (0);
			size_t count (0);
			std::deque<std::shared_ptr<lumex::block>> epoch_open_blocks;
			{
				auto node_flags = lumex::inactive_node_flag_defaults ();
				lumex::update_flags (node_flags, vm);
				node_flags.generate_cache.block_count = true;
				...
				for (auto i (source_node->store.account.begin (transaction)), n (source_node->store.account.end (transaction)); i != n; ++i)
				{
					...
					while (!hash.is_zero ())
					{
						...
						if (block != nullptr)
						{
							...
							node.node->block_processor.add (block, lumex::block_source::test);
							if (block->type () == lumex::block_type::state && block->previous ().is_zero () && source_node->ledger.is_epoch_link (block->link_field ().value ()))
							{
			...
			while (node.node->ledger.block_count () != block_count)
			{
				...
				{
					node.node->block_processor.add_many (epoch_open_blocks, lumex::block_source::test);
				}
		...
		else if (vm.count ("debug_account_versions"))
		{
			auto inactive_node = lumex::default_inactive_node (data_path, vm);
			auto node = inactive_node->node;
			auto const epoch_count = lumex::normalized_epoch (lumex::epoch::max) + static_cast<std::underlying_type<lumex::epoch>::type> (1);
			// Cache the accounts in a collection to make searching quicker against unchecked keys. Group by epoch
			lumex::locked<std::vector<boost::unordered_set<lumex::account>>> opened_account_versions_shared (epoch_count);
			using opened_account_versions_t = decltype (opened_account_versions_shared)::value_type;
			node->store.account.for_each_par (
			[&opened_account_versions_shared, epoch_count] (lumex::store::read_transaction const & /*unused*/, auto i, auto n) {
				// First cache locally
				...
				for (; i != n; ++i)
				{
					...
					auto epoch_idx = lumex::normalized_epoch (account_info.epoch ());
			...
			boost::unordered_set<lumex::account> opened_accounts;
			{
				auto opened_account_versions = opened_account_versions_shared.lock ();
			...
			lumex::locked<boost::unordered_map<lumex::account, std::underlying_type_t<lumex::epoch>>> unopened_highest_pending_shared;
			using unopened_highest_pending_t = decltype (unopened_highest_pending_shared)::value_type;
			node->store.pending.for_each_par (
			[&unopened_highest_pending_shared, &opened_accounts] (lumex::store::read_transaction const & /*unused*/, auto i, auto n) {
				// First cache locally
				...
				for (; i != n; ++i)
				{
					...
					if (!exists)
					{
						...
						auto epoch = lumex::normalized_epoch (info.epoch);
