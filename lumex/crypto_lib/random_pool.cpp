...
void lumex::random_pool::generate_block (unsigned char * output, size_t size)
{
	auto & pool = get_pool ();
	pool.GenerateBlock (output, size);
}

unsigned lumex::random_pool::generate_word32 (unsigned min, unsigned max)
{
	auto & pool = get_pool ();
	return pool.GenerateWord32 (min, max);
}

uint64_t lumex::random_pool::generate_word64 (uint64_t min, uint64_t max)
{
	auto & pool = get_pool ();
...
unsigned char lumex::random_pool::generate_byte ()
{
	auto & pool = get_pool ();
	return pool.GenerateByte ();
}

CryptoPP::AutoSeededRandomPool & lumex::random_pool::get_pool ()
{
