...
TEST (conflicts, add_existing)
{
	...
	// create a send block to send all of the lumex supply to key1
	lumex::block_builder builder;
	auto send1 = builder
				 .send ()
				 .previous (lumex::dev::genesis->hash ())
	...
	lumex::test::start_election (system, node1, send1->hash ());
...
TEST (conflicts, add_two)
{
	...
	auto gk = lumex::dev::genesis_key;

	// create 2 new accounts, that receive 1 raw each, all blocks are force confirmed
	auto [send1, open1] = lumex::test::setup_new_account (system, *node, 1, gk, key1, gk.pub, true);
	auto [send2, open2] = lumex::test::setup_new_account (system, *node, 1, gk, key2, gk.pub, true);
	ASSERT_EQ (5, node->ledger.cemented_count ());

	// send 1 raw to account key3 from key1
	auto send_a = lumex::state_block_builder ()
				  .account (key1.pub)
				  .previous (open1->hash ())
	...
	auto send_b = lumex::state_block_builder ()
				  .account (key2.pub)
				  .previous (open2->hash ())
				  .representative (lumex::dev::genesis_key.pub)
				  .balance (0)
	...
	ASSERT_TRUE (lumex::test::process (*node, { send_a, send_b }));
	ASSERT_TRUE (lumex::test::start_elections (system, *node, { send_a, send_b }));
...
TEST (vote_uniquer, vbh_one)
{
	...
	auto vote1 = lumex::test::make_vote (key, { hashes }, 0, 0);
	auto vote2 (std::make_shared<lumex::vote> (*vote1));
...
TEST (vote_uniquer, vbh_two)
{
	...
	auto block1 = builder
				  ...
				  .work (0)
				  .build ();
	std::vector<lumex::block_hash> hashes1;
	...
	auto block2 = builder
				  ...
				  .work (0)
				  .build ();
	std::vector<lumex::block_hash> hashes2;
	hashes2.push_back (block2->hash ());
	auto vote1 = lumex::test::make_vote (key, { hashes1 }, 0, 0);
	auto vote2 = lumex::test::make_vote (key, { hashes2 }, 0, 0);
	ASSERT_EQ (vote1, uniquer.unique (vote1));
...
TEST (vote_uniquer, cleanup)
{
	lumex::vote_uniquer uniquer;
	lumex::keypair key;
	auto vote1 = std::make_shared<lumex::vote> (key.pub, key.prv, 0, 0, std::vector<lumex::block_hash>{ lumex::block_hash{ 0 } });
	auto vote2 = std::make_shared<lumex::vote> (key.pub, key.prv, lumex::vote::timestamp_min * 1, 0, std::vector<lumex::block_hash>{ lumex::block_hash{ 0 } });
