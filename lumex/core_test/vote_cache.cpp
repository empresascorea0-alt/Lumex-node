...
namespace
{
std::map<lumex::account, lumex::uint128_t> & rep_to_weight_map ()
{
	static std::map<lumex::account, lumex::uint128_t> map;
	return map;
}

std::function<lumex::uint128_t (lumex::account const & rep)> rep_weight_query ()
{
	return [] (lumex::account const & rep) { return rep_to_weight_map ()[rep]; };
}

void register_rep (lumex::account const & rep, lumex::uint128_t weight)
{
	auto & map = rep_to_weight_map ();
	map[rep] = weight;
}

lumex::keypair create_rep (lumex::uint128_t weight)
{
	lumex::keypair key;
...
TEST (vote_cache, construction)
{
	...
	lumex::vote_cache_config cfg;
	lumex::vote_cache vote_cache{ cfg, system.stats };
	ASSERT_EQ (0, vote_cache.size ());
	ASSERT_TRUE (vote_cache.empty ());
	auto hash1 = lumex::test::random_hash ();
...
TEST (vote_cache, insert_one_hash)
{
	...
	auto hash1 = lumex::test::random_hash ();
	auto vote1 = lumex::test::make_vote (rep1, { hash1 }, 1024 * 1024);
...
TEST (vote_cache, insert_one_hash_many_votes)
{
	...
	auto vote1 = lumex::test::make_vote (rep1, { hash1 }, 1 * 1024 * 1024);
	auto vote2 = lumex::test::make_vote (rep2, { hash1 }, 2 * 1024 * 1024);
	auto vote3 = lumex::test::make_vote (rep3, { hash1 }, 3 * 1024 * 1024);
	vote_cache.insert (vote1);
...
TEST (vote_cache, insert_many_hashes_many_votes)
{
	...
	auto hash1 = lumex::test::random_hash ();
	auto hash2 = lumex::test::random_hash ();
	auto hash3 = lumex::test::random_hash ();
	...
	auto vote1 = lumex::test::make_vote (rep1, { hash1 }, 1024 * 1024);
	auto vote2 = lumex::test::make_vote (rep2, { hash2 }, 1024 * 1024);
	auto vote3 = lumex::test::make_vote (rep3, { hash3 }, 1024 * 1024);
	auto vote4 = lumex::test::make_vote (rep4, { hash1 }, 1024 * 1024);
...
TEST (vote_cache, insert_duplicate)
{
	...
	auto vote1 = lumex::test::make_vote (rep1, { hash1 }, 1 * 1024 * 1024);
	auto vote2 = lumex::test::make_vote (rep1, { hash1 }, 1 * 1024 * 1024);
	vote_cache.insert (vote1);
	vote_cache.insert (vote2);
	ASSERT_EQ (1, vote_cache.size ());
}
...
TEST (vote_cache, insert_newer)
{
	...
	ASSERT_EQ (peek1.size (), 1);
	ASSERT_EQ (peek1.front (), vote1);
	auto vote2 = lumex::test::make_final_vote (rep1, { hash1 });
	vote_cache.insert (vote2);
...
TEST (vote_cache, insert_older)
{
	...
	auto hash1 = lumex::test::random_hash ();
	auto rep1 = create_rep (9);
	auto vote1 = lumex::test::make_vote (rep1, { hash1 }, 2 * 1024 * 1024);
	vote_cache.insert (vote1);
	auto peek1 = vote_cache.find (hash1);
	ASSERT_EQ (peek1.size (), 1);
	ASSERT_EQ (peek1.front (), vote1);
...
TEST (vote_cache, age_cutoff)
{
	...
	auto hash1 = lumex::test::random_hash ();
	auto rep1 = create_rep (9);
	auto vote1 = lumex::test::make_vote (rep1, { hash1 }, 3);
	vote_cache.insert (vote1);
	ASSERT_EQ (1, vote_cache.size ());
	ASSERT_FALSE (vote_cache.find (hash1).empty ());
	...
	auto check = [&] () {
		...
		vote_cache.top (0);
		return system.stats.count (lumex::stat::type::vote_cache, lumex::stat::detail::cleanup);
	};
