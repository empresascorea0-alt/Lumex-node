...
TEST (toml, diff)
{
	lumex::tomlconfig defaults, other;
...
TEST (toml, base_override)
{
	...
	ASSERT_EQ (t.get_error (), lumex::error_config::missing_value);
	t.get_error ().clear ();

	// Query uint16 that's too big, make sure we have an error
	t.get_required<uint16_t> ("node.too_big", port);
	ASSERT_TRUE (t.get_error ());
	ASSERT_EQ (t.get_error (), lumex::error_config::invalid_value);
}

TEST (toml, put)
{
	lumex::tomlconfig config;
	lumex::tomlconfig config_node;
...
TEST (toml_config, daemon_config_update_array)
{
	...
	lumex::daemon_config c{ data_path, lumex::dev::network_params };
...
TEST (toml_config, rpc_config_deserialize_defaults)
{
	...
	lumex::tomlconfig t;
	t.read (ss);
	lumex::rpc_config conf{ lumex::dev::network_params.network };
	lumex::rpc_config defaults{ lumex::dev::network_params.network };
...
TEST (toml_config, daemon_config_deserialize_defaults)
{
	...
	lumex::tomlconfig t;
	t.read (ss);
	lumex::daemon_config conf;
	lumex::daemon_config defaults;
	conf.deserialize_toml (t);
...
TEST (toml_config, daemon_config_deserialize_no_defaults)
{
	...
	preconfigured_representatives = ["lumex_3arg3asgtigae3xckabaaewkx3bzsh7nwz7jkmjos79ihyaxwphhm6qgjps4"]
	receive_minimum = "999"
...
TEST (toml_config, daemon_read_config)
{
	...
	ASSERT_FALSE (std::filesystem::exists (lumex::get_node_toml_config_path (path)));
	ASSERT_FALSE (lumex::read_node_config_toml (path, config));
	{
		auto error = lumex::read_node_config_toml (path, config, invalid_overrides1);
		ASSERT_TRUE (error);
		ASSERT_EQ (error.get_message (), expected_message1);
	}
	{
		auto error = lumex::read_node_config_toml (path, config, invalid_overrides2);
		ASSERT_TRUE (error);
	...
	lumex::tomlconfig toml;
	toml.write (lumex::get_node_toml_config_path (path));

	// Reading when there is a config file
	ASSERT_TRUE (std::filesystem::exists (lumex::get_node_toml_config_path (path)));
	ASSERT_FALSE (lumex::read_node_config_toml (path, config));
	{
		auto error = lumex::read_node_config_toml (path, config, invalid_overrides1);
		ASSERT_TRUE (error);
...
TEST (toml_config, log_config_defaults)
{
	...
	lumex::tomlconfig toml;
	toml.read (ss);
	lumex::log_config confg{};
	lumex::log_config defaults{};
...
TEST (toml_config, merge_config_files)
{
	...
	lumex::tomlconfig merged_toml;
	lumex::daemon_config default_config{ ".", network_params };
	lumex::daemon_config current_config{ ".", network_params };
	lumex::daemon_config merged_config{ ".", network_params };
