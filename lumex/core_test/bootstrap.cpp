...
TEST (bootstrap, account_base)
{
	...
	lumex::test::system system{ 1, lumex::transport::transport_type::tcp, flags };
	...
	auto send1 = builder.make_block ()
				 ...
				 .build ();
	ASSERT_EQ (lumex::block_status::progress, node0.process (send1));
...
TEST (bootstrap, account_inductive)
{
	...
	ASSERT_EQ (lumex::block_status::progress, node0.process (send1));
	ASSERT_EQ (lumex::block_status::progress, node0.process (send2));
...
TEST (bootstrap, trace_base)
{
	...
	ASSERT_EQ (lumex::block_status::progress, node0.process (send1));
	ASSERT_EQ (lumex::block_status::progress, node0.process (receive1));
...
TEST (bootstrap, frontier_scan)
{
	...
	std::vector<std::shared_ptr<lumex::block>> opens;
	std::vector<std::shared_ptr<lumex::block>> updates;
	{
		auto source = lumex::dev::genesis_key;
		auto latest = lumex::dev::genesis->hash ();
		auto balance = lumex::dev::genesis->balance ().number ();
	...
	ASSERT_TRUE (lumex::test::process (node0, updates));
...
TEST (bootstrap, frontier_scan_pending)
{
	...
	auto & node0 = *system.add_node (config, flags);
	ASSERT_TRUE (lumex::test::process (node0, opens));
...
TEST (bootstrap, frontier_scan_cannot_prioritize)
{
	...
	std::vector<std::shared_ptr<lumex::block>> sends;
	std::vector<std::shared_ptr<lumex::block>> opens;
	std::vector<std::shared_ptr<lumex::block>> sends2;
	std::vector<std::shared_ptr<lumex::block>> opens2;
	{
		auto source = lumex::dev::genesis_key;
		auto latest = lumex::dev::genesis->hash ();
		auto balance = lumex::dev::genesis->balance ().number ();
	...
	auto & node0 = *system.add_node (config, flags);
	ASSERT_TRUE (lumex::test::process (node0, sends2));
	ASSERT_TRUE (lumex::test::process (node0, opens2));
