...
lumex::bounded_dfs_result run_dfs (graph_t const & graph, int start, size_t max_depth, std::vector<int> & resolve_order)
{
	std::set<int> resolved;
	return lumex::bounded_dfs (
	start, max_depth,
	[&] (int const & node) {
...
{
	// Test with std::array return type (like lumex::block::dependencies())
	std::set<int> resolved;
	std::vector<int> order;
	auto result = lumex::bounded_dfs (
	3, 100,
