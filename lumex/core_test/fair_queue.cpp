...
{
	lumex::fair_queue<source_enum, int> queue;
...
{
	lumex::fair_queue<int, source_enum> queue;
...
TEST (fair_queue, source_channel)
{
	...
	lumex::fair_queue<int, source_enum, std::shared_ptr<lumex::transport::channel>> queue;
	queue.priority_query = [] (auto const &) { return 1; };
	queue.max_size_query = [] (auto const &) { return 999; };

	auto channel1 = lumex::test::fake_channel (system.node (0));
	auto channel2 = lumex::test::fake_channel (system.node (0));
	auto channel3 = lumex::test::fake_channel (system.node (0));
