...
struct migration_test_data
{
	...
	std::map<lumex::account, lumex::account_info> accounts;
	std::map<lumex::pending_key, lumex::pending_info> pending;
	std::map<lumex::account, lumex::uint128_t> rep_weights;
	std::map<uint64_t, lumex::amount> online_weights;
	std::set<lumex::block_hash> pruned;
	std::vector<std::pair<lumex::endpoint_key, uint64_t>> peers;
	std::map<lumex::account, lumex::confirmation_height_info> confirmation_heights;
	std::map<lumex::qualified_root, lumex::block_hash> final_votes;
};
...
migration_test_data populate_ledger_for_migration (lumex::store::ledger_store & store)
{
	...
	// Generate blocks
	auto previous = lumex::dev::genesis->hash ();
	for (size_t i = 0; i < num_blocks; ++i)
	{
		auto block = lumex::state_block_builder ()
					 .account (lumex::dev::genesis_key.pub)
					 .previous (previous)
					 .representative (lumex::dev::genesis_key.pub)
					 .balance (lumex::dev::constants.genesis_amount - (i + 1) * 1000)
					 .link (lumex::account (i + 1))
	...
	{
		lumex::account acc{ i + 1 };
		lumex::account_info info{ data.blocks[0]->hash (), lumex::dev::genesis_key.pub,
			data.blocks[0]->hash (), 1000, lumex::seconds_since_epoch (),
			1, lumex::epoch::epoch_0 };
	...
	{
		lumex::pending_key key{ lumex::account (i + 1), data.blocks[i]->hash () };
		lumex::pending_info info{ lumex::dev::genesis_key.pub, 1000, lumex::epoch::epoch_0 };
	...
	{
		data.online_weights[i * 1000 + 100] = lumex::amount (i * 1000000);
	}

	// Populate pruned
	for (size_t i = 0; i < num_pruned; ++i)
	{
		data.pruned.insert (lumex::block_hash{ i + 10000 });
	}
	...
	for (size_t i = 0; i < num_peers; ++i)
	{
		...
		lumex::endpoint_key key (addr.to_bytes (), static_cast<uint16_t> (i + 1000));
	...
	{
		lumex::qualified_root qroot{ lumex::root{ i + 1 }, lumex::block_hash{ i + 5000 } };
		data.final_votes[qroot] = lumex::block_hash{ i + 10000 };
	}
	...
	// Write all data to store
	{
		...
		{
			data.blocks[i]->sideband_set (lumex::block_sideband{
			lumex::dev::genesis_key.pub,
			lumex::dev::constants.genesis_amount - (i + 1) * 1000,
			i + 2, lumex::seconds_since_epoch (), lumex::epoch::epoch_0,
...
TEST (migrations, lmdb_to_rocksdb)
{
	...
	auto data_path = lumex::unique_path ();

	migration_test_data expected;
	std::map<lumex::store::table, size_t> src_counts;

	// Create and populate LMDB store
	{
		auto store = lumex::test::make_store (data_path);

		// Initialize with genesis block
		lumex::ledger::seed_genesis (*store, store->tx_begin_write (), lumex::dev::constants);
	...
	// Run migration
	ASSERT_NO_THROW (lumex::migrate_lmdb_to_rocksdb (data_path));

	// Open RocksDB store and verify
	auto rocksdb_path = lumex::database_path_for_backend (data_path, lumex::database_backend::rocksdb);
	auto rocksdb_backend = std::make_unique<lumex::store::rocksdb::backend_rocksdb> (rocksdb_path, lumex::rocksdb_config{}, logger);
	lumex::store::ledger_store rocksdb_store{ std::move (rocksdb_backend), lumex::store::open_mode::read_only, stats, logger };
...
TEST (migrations, double_migration_fails)
{
	...
	ASSERT_THROW (lumex::migrate_lmdb_to_rocksdb (data_path), std::runtime_error);
}
