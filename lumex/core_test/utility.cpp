...
#include <lumex/lib/relaxed_atomic.hpp>
...
TEST (optional_ptr, basic)
{
	...
	lumex::optional_ptr<valtype> opt;
	ASSERT_FALSE (opt);
...
TEST (filesystem, remove_all_files)
{
	...
	lumex::remove_all_files_in_dir (path);
...
TEST (filesystem, move_all_files)
{
	...
	lumex::move_all_files_to_dir (dummy_directory, path);
...
{
	lumex::relaxed_atomic_integral<uint32_t> atomic{ 0 };
	ASSERT_EQ (0, atomic++);
	ASSERT_EQ (1, atomic);
	ASSERT_EQ (2, ++atomic);
	ASSERT_EQ (2, atomic);
...
{
	lumex::pending_key one{ 1, 2 };
	lumex::pending_key two{ 1, 3 };
	lumex::pending_key three{ 2, 1 };
	ASSERT_LT (one, two);
	ASSERT_LT (one, three);
	ASSERT_LT (two, three);
	lumex::pending_key one_same{ 1, 2 };
	ASSERT_EQ (std::hash<lumex::pending_key>{}(one), std::hash<lumex::pending_key>{}(one_same));
	ASSERT_NE (std::hash<lumex::pending_key>{}(one), std::hash<lumex::pending_key>{}(two));
}
