...
public:
	...
	{
		auto chain = lumex::test::setup_chain (system, node, count);
		blocks.insert (blocks.end (), chain.begin (), chain.end ());
	}

	std::shared_ptr<lumex::block> next_block ()
	{
	...
	std::shared_ptr<lumex::election> random_election (lumex::election_behavior behavior = lumex::election_behavior::priority)
	{
		return std::make_shared<lumex::election> (node, next_block (), behavior);
	}
};
}
...
TEST (active_elections_index, insert)
{
	...
	auto election1 = context.random_election (lumex::election_behavior::priority);
	auto election2 = context.random_election (lumex::election_behavior::hinted);
	auto election3 = context.random_election (lumex::election_behavior::optimistic);
	...
	index.insert (election1, lumex::election_behavior::priority, 1, 100);
	ASSERT_EQ (index.size (), 1);
	ASSERT_TRUE (index.exists (election1));
	ASSERT_TRUE (index.exists (election1->qualified_root));
	ASSERT_EQ (index.size (lumex::election_behavior::priority), 1);
	ASSERT_EQ (index.size (lumex::election_behavior::priority, 1), 1);

	// Insert second election with different behavior
	index.insert (election2, lumex::election_behavior::hinted, 2, 200);
	ASSERT_EQ (index.size (), 2);
	ASSERT_TRUE (index.exists (election2));
	ASSERT_EQ (index.size (lumex::election_behavior::hinted), 1);
	ASSERT_EQ (index.size (lumex::election_behavior::hinted, 2), 1);

	// Insert third election
	index.insert (election3, lumex::election_behavior::optimistic, 1, 50);
	ASSERT_EQ (index.size (), 3);
	ASSERT_EQ (index.size (lumex::election_behavior::optimistic), 1);
	ASSERT_EQ (index.size (lumex::election_behavior::optimistic, 1), 1);
}
...
TEST (active_elections_index, erase)
{
	...
	// Insert elections
	index.insert (election1, lumex::election_behavior::priority, 1, 100);
	index.insert (election2, lumex::election_behavior::priority, 1, 200);
	index.insert (election3, lumex::election_behavior::hinted, 2, 300);

	ASSERT_EQ (index.size (), 3);
	ASSERT_EQ (index.size (lumex::election_behavior::priority), 2);
	ASSERT_EQ (index.size (lumex::election_behavior::priority, 1), 2);
	...
	ASSERT_EQ (index.size (), 0);
	ASSERT_EQ (index.size (lumex::election_behavior::priority), 0);
	ASSERT_EQ (index.size (lumex::election_behavior::hinted), 0);
}
...
TEST (active_elections_index, update)
{
	...
	index.update (election1, lumex::election_behavior::optimistic);

	ASSERT_EQ (index.size (), 2);
	ASSERT_EQ (index.size (lumex::election_behavior::priority), 0);
	ASSERT_EQ (index.size (lumex::election_behavior::optimistic), 1);
	ASSERT_EQ (index.size (lumex::election_behavior::hinted), 1);
	ASSERT_EQ (index.size (lumex::election_behavior::priority, 1), 0);
	ASSERT_EQ (index.size (lumex::election_behavior::optimistic, 1), 1);
	...
	index.update (election2, lumex::election_behavior::hinted);
	ASSERT_EQ (index.size (lumex::election_behavior::hinted), 1);
	ASSERT_EQ (index.size (lumex::election_behavior::hinted, 2), 1);
}
...
TEST (active_elections_index, election_lookup)
{
	...
	ASSERT_EQ (info1->behavior, lumex::election_behavior::priority);
...
TEST (active_elections_index, size_operations)
{
	...
	index.insert (e1, lumex::election_behavior::priority, 1, 100);
	index.insert (e2, lumex::election_behavior::priority, 1, 200);
	index.insert (e3, lumex::election_behavior::priority, 2, 300);
	index.insert (e4, lumex::election_behavior::hinted, 1, 400);
	index.insert (e5, lumex::election_behavior::optimistic, 3, 500);

	// Test total size
	ASSERT_EQ (index.size (), 5);

	// Test size by behavior
	ASSERT_EQ (index.size (lumex::election_behavior::priority), 3);
	ASSERT_EQ (index.size (lumex::election_behavior::hinted), 1);
	ASSERT_EQ (index.size (lumex::election_behavior::optimistic), 1);
	ASSERT_EQ (index.size (lumex::election_behavior::manual), 0);

	// Test size by behavior and bucket
	ASSERT_EQ (index.size (lumex::election_behavior::priority, 1), 2);
	ASSERT_EQ (index.size (lumex::election_behavior::priority, 2), 1);
	ASSERT_EQ (index.size (lumex::election_behavior::priority, 3), 0);
	ASSERT_EQ (index.size (lumex::election_behavior::hinted, 1), 1);
	ASSERT_EQ (index.size (lumex::election_behavior::optimistic, 3), 1);
}
...
TEST (active_elections_index, last)
{
	...
	// Test empty index
	auto [empty_election, empty_priority] = index.last (lumex::election_behavior::priority, 1);
	ASSERT_EQ (empty_election, nullptr);
	ASSERT_EQ (empty_priority, std::numeric_limits<lumex::priority_timestamp>::max ());
	...
	index.insert (e1, lumex::election_behavior::priority, 1, 300); // Highest priority value in bucket 1
	index.insert (e2, lumex::election_behavior::priority, 1, 100);
	index.insert (e3, lumex::election_behavior::priority, 1, 200);
	index.insert (e4, lumex::election_behavior::priority, 2, 50); // Only election in bucket 2

	// Test last for bucket 1 (should return e1 with priority 300 - highest value)
	auto [top1_election, top1_priority] = index.last (lumex::election_behavior::priority, 1);
	ASSERT_EQ (top1_election, e1);
	ASSERT_EQ (top1_priority, 300);

	// Test last for bucket 2 (should return e4 with priority 50)
	auto [top2_election, top2_priority] = index.last (lumex::election_behavior::priority, 2);
	ASSERT_EQ (top2_election, e4);
	ASSERT_EQ (top2_priority, 50);

	// Test last for empty bucket
	auto [top3_election, top3_priority] = index.last (lumex::election_behavior::priority, 3);
	ASSERT_EQ (top3_election, nullptr);
	ASSERT_EQ (top3_priority, std::numeric_limits<lumex::priority_timestamp>::max ());

	// Test last for different behavior
	auto e5 = context.random_election ();
	index.insert (e5, lumex::election_behavior::hinted, 1, 75);
	auto [top4_election, top4_priority] = index.last (lumex::election_behavior::hinted, 1);
...
TEST (active_elections_index, list_with_cutoff)
{
	...
	index.insert (e1, lumex::election_behavior::priority, 1, 100);
	index.insert (e2, lumex::election_behavior::priority, 2, 200);
	index.insert (e3, lumex::election_behavior::hinted, 1, 300);
	...
	std::set<std::shared_ptr<lumex::election>> processed (elections_list.begin (), elections_list.end ());
...
TEST (active_elections_index, clear)
{
	...
	index.insert (e2, lumex::election_behavior::priority, 2, 200);
	index.insert (e3, lumex::election_behavior::hinted, 1, 300);
	index.insert (e4, lumex::election_behavior::optimistic, 3, 400);
	...
	ASSERT_EQ (index.size (lumex::election_behavior::priority), 0);
	ASSERT_EQ (index.size (lumex::election_behavior::hinted), 0);
	ASSERT_EQ (index.size (lumex::election_behavior::optimistic), 0);
	ASSERT_EQ (index.size (lumex::election_behavior::manual), 0);
	...
	index.insert (e1, lumex::election_behavior::manual, 0, 50);
	ASSERT_EQ (index.size (), 1);
	ASSERT_TRUE (index.exists (e1));
	ASSERT_EQ (index.size (lumex::election_behavior::manual), 1);
}