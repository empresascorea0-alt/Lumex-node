...
TEST (ledger_cement, single)
{
	...
	lumex::block_hash latest1 (node->latest (lumex::dev::genesis_key.pub));
	...
	ASSERT_EQ (1, node->store.confirmation_height.get (transaction, lumex::dev::genesis_key.pub).value ().height);
	ASSERT_EQ (lumex::dev::genesis->hash (), node->store.confirmation_height.get (transaction, lumex::dev::genesis_key.pub).value ().frontier);
	...
	ASSERT_EQ (2, node->ledger.cemented.account_height (transaction, lumex::dev::genesis_key.pub));
	ASSERT_EQ (send1->hash (), node->ledger.cemented.account_head (transaction, lumex::dev::genesis_key.pub));
	...
	ASSERT_EQ (1, node->stats.count (lumex::stat::type::confirmation_height, lumex::stat::detail::blocks_cemented, lumex::stat::dir::in));
...
TEST (ledger_cement, multiple_accounts)
{
	...
	ASSERT_EQ (lumex::block_status::progress, node->ledger.process (transaction, open2));
	ASSERT_EQ (lumex::block_status::progress, node->ledger.process (transaction, open3));

	ASSERT_EQ (lumex::block_status::progress, node->ledger.process (transaction, send4));
	ASSERT_EQ (lumex::block_status::progress, node->ledger.process (transaction, send5));

	ASSERT_EQ (lumex::block_status::progress, node->ledger.process (transaction, receive1));
	ASSERT_EQ (lumex::block_status::progress, node->ledger.process (transaction, send6));
	ASSERT_EQ (lumex::block_status::progress, node->ledger.process (transaction, receive2));
	...
	auto receive3 = builder
					...
					.build ();
	ASSERT_EQ (lumex::block_status::progress, node->ledger.process (transaction, receive3));
	auto confirmed = node->ledger.cement (transaction, receive3->hash ());
	ASSERT_EQ (10, confirmed.size ());
	ASSERT_EQ (10, node->stats.count (lumex::stat::type::confirmation_height, lumex::stat::detail::blocks_cemented, lumex::stat::dir::in));
	ASSERT_EQ (11, node->ledger.cemented_count ());

	ASSERT_TRUE (node->ledger.cemented.block_exists (transaction, receive3->hash ()));
	ASSERT_EQ (4, node->ledger.any.account_get (transaction, lumex::dev::genesis_key.pub).value ().block_count);
	ASSERT_EQ (4, node->ledger.cemented.account_height (transaction, lumex::dev::genesis_key.pub));
	ASSERT_EQ (send3->hash (), node->ledger.cemented.account_head (transaction, lumex::dev::genesis_key.pub));
	...
	ASSERT_TRUE (node->ledger.rollback (transaction, node->ledger.any.account_head (transaction, lumex::dev::genesis_key.pub)));
...
TEST (ledger_cement, send_receive_between_2_accounts)
{
	...
	ASSERT_EQ (lumex::block_status::progress, node->ledger.process (transaction, receive4));
	...
	ASSERT_EQ (7, node->ledger.any.account_get (transaction, lumex::dev::genesis_key.pub).value ().block_count);
	ASSERT_EQ (6, node->ledger.cemented.account_height (transaction, lumex::dev::genesis_key.pub));
	ASSERT_EQ (send5->hash (), node->ledger.cemented.account_head (transaction, lumex::dev::genesis_key.pub));
...
TEST (ledger_cement, send_receive_self)
{
	...
	ASSERT_EQ (6, node->stats.count (lumex::stat::type::confirmation_height, lumex::stat::detail::blocks_cemented, lumex::stat::dir::in));

	ASSERT_TRUE (node->ledger.cemented.block_exists (transaction, receive3->hash ()));
	ASSERT_EQ (8, node->ledger.any.account_get (transaction, lumex::dev::genesis_key.pub).value ().block_count);
	ASSERT_EQ (7, node->ledger.cemented.account_height (transaction, lumex::dev::genesis_key.pub));
	ASSERT_EQ (receive3->hash (), node->ledger.cemented.account_head (transaction, lumex::dev::genesis_key.pub));
...
TEST (ledger_cement, all_block_types)
{
	...
	auto state_receive3 = builder
						  ...
						  .previous (send1->hash ())
						  .representative (lumex::dev::genesis_key.pub)
						  .balance (lumex::dev::constants.genesis_amount - lumex::Klumex_ratio * 2 + 1)
	...
	ASSERT_EQ (lumex::block_status::progress, node->ledger.process (transaction, state_open));

	ASSERT_EQ (lumex::block_status::progress, node->ledger.process (transaction, send2));
	ASSERT_EQ (lumex::block_status::progress, node->ledger.process (transaction, state_receive));

	ASSERT_EQ (lumex::block_status::progress, node->ledger.process (transaction, state_send));
	ASSERT_EQ (lumex::block_status::progress, node->ledger.process (transaction, receive));
	ASSERT_EQ (lumex::block_status::progress, node->ledger.process (transaction, change));
	ASSERT_EQ (lumex::block_status::progress, node->ledger.process (transaction, state_change));

	ASSERT_EQ (lumex::block_status::progress, node->ledger.process (transaction, epoch));
	ASSERT_EQ (lumex::block_status::progress, node->ledger.process (transaction, epoch1));

	ASSERT_EQ (lumex::block_status::progress, node->ledger.process (transaction, state_send1));
	ASSERT_EQ (lumex::block_status::progress, node->ledger.process (transaction, state_receive2));

	ASSERT_EQ (lumex::block_status::progress, node->ledger.process (transaction, state_send2));
	ASSERT_EQ (lumex::block_status::progress, node->ledger.process (transaction, state_send3));

	ASSERT_EQ (lumex::block_status::progress, node->ledger.process (transaction, state_send4));
	ASSERT_EQ (lumex::block_status::progress, node->ledger.process (transaction, state_receive3));

	auto confirmed = node->ledger.cement (transaction, state_send2->hash ());
	ASSERT_EQ (15, confirmed.size ());
	ASSERT_EQ (15, node->stats.count (lumex::stat::type::confirmation_height, lumex::stat::detail::blocks_cemented, lumex::stat::dir::in));
	...
	ASSERT_LE (4, node->ledger.any.account_get (transaction, lumex::dev::genesis_key.pub).value ().block_count);
	ASSERT_EQ (3, node->ledger.cemented.account_height (transaction, lumex::dev::genesis_key.pub));
	ASSERT_EQ (send1->hash (), node->ledger.cemented.account_head (transaction, lumex::dev::genesis_key.pub));
...
TEST (ledger_cement, conflict_rollback_cemented)
{
	...
	auto const genesis_hash = lumex::dev::genesis->hash ();
				  ...
				  .build ();
	{
		...
		ASSERT_EQ (lumex::block_status::progress, node1->ledger.process (transaction, fork1a));
		node1->ledger.cement (transaction, fork1a->hash ());
	}
	ASSERT_TRUE (lumex::test::confirmed (*node1, { fork1a }));
	...
	ASSERT_TIMELY_EQ (5s, 1, (count = node1->stats.count (lumex::stat::type::ledger, lumex::stat::detail::rollback_failed)));
	ASSERT_TRUE (lumex::test::confirmed (*node1, { fork1a->hash () })); // fork1a should still remain after the rollback failed event
}
...
TEST (ledger_cement, observers)
{
	...
	ASSERT_EQ (1, node1->stats.count (lumex::stat::type::confirmation_height, lumex::stat::detail::blocks_cemented, lumex::stat::dir::in));
...
TEST (ledger_cement, pruned_source)
{
	...
	auto store = lumex::test::make_store (system.logger, system.stats, path);

	// Topo index is incompatible with pruning
	lumex::ledger ledger (*store, lumex::dev::network_params, system.stats, system.logger, lumex::ledger_options{ .enable_topo_index = false });
	ledger.pruning = true;
...
TEST (ledger_cementDeathTest, rollback_cemented_block)
{
	...
	if (!lumex::running_within_valgrind ())
	{
		lumex::test::system system;
		...
		lumex::ledger ledger (*store, lumex::dev::network_params, system.stats, system.logger);
		lumex::store::write_queue write_queue;
		lumex::work_pool pool{ lumex::dev::network_params.network, std::numeric_limits<unsigned>::max () };
		lumex::keypair key1;
		lumex::block_builder builder;
