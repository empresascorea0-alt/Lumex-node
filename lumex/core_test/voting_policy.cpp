...
TEST (voting_policy, vote_genesis)
{
	...
	lumex::voting_policy policy{ ledger };

	auto txn = ledger.tx_begin_read ();
	auto result = policy.vote (txn, *lumex::dev::genesis);
	ASSERT_TRUE (result);
	ASSERT_EQ (result->qualified_root (), lumex::dev::genesis->qualified_root ());
	ASSERT_EQ (result->hash (), lumex::dev::genesis->hash ());
	ASSERT_EQ (result->type (), lumex::vote_type::normal);
}
...
TEST (voting_policy, vote_receive_previous_cemented_source_not)
{
	...
	ASSERT_EQ (lumex::block_status::progress, ledger.process (txn, receive2));
...
TEST (voting_policy, vote_change_cemented)
{
	...
	ASSERT_EQ (lumex::block_status::progress, ledger.process (ledger.tx_begin_write (), change1));
...
TEST (voting_policy, vote_change_not_cemented)
{
	...
	auto txn = ledger.tx_begin_write ();
	ASSERT_EQ (lumex::block_status::progress, ledger.process (txn, send1));
	ASSERT_EQ (lumex::block_status::progress, ledger.process (txn, change1));
...
TEST (voting_policy, vote_epoch_cemented)
{
	...
	ASSERT_EQ (final_result->type (), lumex::vote_type::final);
}
...
TEST (voting_policy, vote_upgrades_to_final)
{
	...
	ASSERT_EQ (normal_result->type (), lumex::vote_type::normal);
	...
	ASSERT_EQ (upgraded_result->type (), lumex::vote_type::final);
...
TEST (voting_policy, vote_upgrades_to_final_fork)
{
	...
	auto txn = ledger.tx_begin_write ();
	ASSERT_EQ (lumex::block_status::progress, ledger.process (txn, send1a));
	...
	ASSERT_EQ (lumex::block_status::progress, ledger.process (txn, send1b));

	// vote on the fork upgrades to final with the originally recorded hash
	auto result = policy.vote (txn, *send1b);
	ASSERT_TRUE (result);
	ASSERT_EQ (result->type (), lumex::vote_type::final);
...
TEST (voting_policy, reply_final_eligible)
{
	...
	lumex::keypair rep;
	lumex::voting_policy::vote_signer_t signer = [&rep] (auto const & callback) { callback (rep.pub, rep.prv); };
	auto votes = policy.sign (lumex::vote_type::final, { *result }, signer);
...
TEST (voting_policy_DeathTest, sign_type_mismatch)
{
	...
	ASSERT_DEATH_IF_SUPPORTED (policy.sign (lumex::vote_type::final, { *normal_permit }, signer), "");
}
...
TEST (voting_policy, sign_empty)
{
	...
	lumex::voting_policy::vote_signer_t signer = [&signer_called] (auto const & callback) {
		signer_called = true;
		lumex::keypair key;
		callback (key.pub, key.prv);
	};

	auto votes = policy.sign (lumex::vote_type::normal, {}, signer);
...
TEST (voting_policy, sign_single_permit)
{
	...
	lumex::voting_policy::vote_signer_t signer = [&rep] (auto const & callback) { callback (rep.pub, rep.prv); };
	lumex::millis_t ts = 99008; // 16-byte aligned for clean assertion
	auto votes = policy.sign (lumex::vote_type::normal, { *permit }, signer, ts);

	ASSERT_EQ (votes.size (), 1);
	ASSERT_EQ (votes[0]->hashes.size (), 1);
	ASSERT_EQ (votes[0]->hashes[0], send1->hash ());
	ASSERT_EQ (votes[0]->account, rep.pub);
	ASSERT_EQ (votes[0]->timestamp (), ts & lumex::vote::timestamp_mask);
...
TEST (voting_policy, sign_multiple_permits)
{
	...
	lumex::voting_policy::vote_signer_t signer = [&rep] (auto const & callback) { callback (rep.pub, rep.prv); };
	auto votes = policy.sign (lumex::vote_type::normal, { *permit1, *permit2 }, signer, 99008);
...
TEST (voting_policy, sign_multiple_signers)
{
	...
	lumex::voting_policy::vote_signer_t signer = [&rep1, &rep2] (auto const & callback) {
		callback (rep1.pub, rep1.prv);
		callback (rep2.pub, rep2.prv);
	};
	auto votes = policy.sign (lumex::vote_type::normal, { *permit }, signer, 99008);
...
TEST (voting_policy, sign_final_type)
{
	...
	lumex::voting_policy::vote_signer_t signer = [&rep] (auto const & callback) { callback (rep.pub, rep.prv); };
	auto votes = policy.sign (lumex::vote_type::final, { *permit }, signer);

	ASSERT_EQ (votes.size (), 1);
	ASSERT_TRUE (votes[0]->is_final ());
	ASSERT_EQ (votes[0]->duration_bits (), lumex::vote::duration_max);
	ASSERT_FALSE (votes[0]->validate ());
}
...
TEST (voting_policy, sign_no_signers)
{
	...
	lumex::voting_policy::vote_signer_t empty_signer = [] (auto const &) {};
	auto votes = policy.sign (lumex::vote_type::normal, { *permit }, empty_signer, 99008);
...
TEST (voting_policy, full_send_receive_sign)
{
	...
	// Sign normal vote
	auto normal_votes = policy.sign (lumex::vote_type::normal, { *normal_permit }, signer, 99008);
	ASSERT_EQ (normal_votes.size (), 1);
	ASSERT_FALSE (normal_votes[0]->is_final ());
	ASSERT_FALSE (normal_votes[0]->validate ());

	// Sign final vote
	auto final_votes = policy.sign (lumex::vote_type::final, { *final_permit }, signer);
...
TEST (voting_policy, final_vote_fork_safety)
{
	...
	lumex::voting_policy::vote_signer_t signer = [&rep] (auto const & callback) { callback (rep.pub, rep.prv); };
	auto votes_a = policy.sign (lumex::vote_type::final, { *permit_a }, signer);
	...
	ASSERT_EQ (normal_fork->type (), lumex::vote_type::final);
