...
struct test_context
{
	lumex::vote_rebroadcaster_config config;
	lumex::vote_rebroadcaster_index index;

	explicit test_context (lumex::vote_rebroadcaster_config config_a = {}) :
		config{ config_a },
		index{ config }
	{
	}
...
TEST (vote_rebroadcaster_index, basic_vote_tracking)
{
	...
	lumex::keypair key;
	std::vector<lumex::block_hash> hashes = { lumex::block_hash{ 1 } };
	auto vote = lumex::test::make_vote (key, hashes);

	auto result = index.check_and_record (vote, lumex::uint128_t{ 100 }, now);

	ASSERT_EQ (result, lumex::vote_rebroadcaster_index::result::ok);
...
TEST (vote_rebroadcaster_index, duplicate_vote_rejection)
{
	...
	auto result1 = index.check_and_record (vote, lumex::uint128_t{ 100 }, now);
	ASSERT_EQ (result1, lumex::vote_rebroadcaster_index::result::ok);

	// Same vote should be rejected
	auto result2 = index.check_and_record (vote, lumex::uint128_t{ 100 }, now);
	ASSERT_EQ (result2, lumex::vote_rebroadcaster_index::result::already_rebroadcasted);

	// Even after time threshold
	auto result3 = index.check_and_record (vote, lumex::uint128_t{ 100 }, now + 60min);
	ASSERT_EQ (result3, lumex::vote_rebroadcaster_index::result::already_rebroadcasted);
}
...
TEST (vote_rebroadcaster_index, rebroadcast_timing)
{
	...
	// Initial vote
	auto vote1 = lumex::test::make_vote (key, hashes, 1000);
	auto result1 = index.check_and_record (vote1, lumex::uint128_t{ 100 }, now);
	ASSERT_EQ (result1, lumex::vote_rebroadcaster_index::result::ok);

	// Try rebroadcast immediately - should be rejected
	auto vote2 = lumex::test::make_vote (key, hashes, 1500);
	auto result2 = index.check_and_record (vote2, lumex::uint128_t{ 100 }, now);
	ASSERT_EQ (result2, lumex::vote_rebroadcaster_index::result::rebroadcast_unnecessary);

	// Try after threshold - should be accepted
	auto vote3 = lumex::test::make_vote (key, hashes, 2500);
	auto result3 = index.check_and_record (vote3, lumex::uint128_t{ 100 }, now + 2000ms);
	ASSERT_EQ (result3, lumex::vote_rebroadcaster_index::result::ok);
}
...
TEST (vote_rebroadcaster_index, final_vote_override)
{
	...
	auto final_vote = lumex::test::make_final_vote (key, hashes);
	auto result2 = index.check_and_record (final_vote, lumex::uint128_t{ 100 }, now);
	ASSERT_EQ (result2, lumex::vote_rebroadcaster_index::result::ok);
...
TEST (vote_rebroadcaster_index, representative_limit)
{
	...
	std::vector<lumex::keypair> keys (4);
	std::vector<lumex::block_hash> hashes = { lumex::block_hash{ 1 } };

	// Add first rep (weight 100)
	auto vote1 = lumex::test::make_vote (keys[0], hashes);
	auto result1 = index.check_and_record (vote1, lumex::uint128_t{ 100 }, now);
	ASSERT_EQ (result1, lumex::vote_rebroadcaster_index::result::ok);
	ASSERT_EQ (index.representatives_count (), 1);

	// Add second rep (weight 200)
	auto vote2 = lumex::test::make_vote (keys[1], hashes);
	auto result2 = index.check_and_record (vote2, lumex::uint128_t{ 200 }, now);
	ASSERT_EQ (result2, lumex::vote_rebroadcaster_index::result::ok);
	ASSERT_EQ (index.representatives_count (), 2);

	// Try to add third rep with lower weight - should be rejected
	auto vote3 = lumex::test::make_vote (keys[2], hashes);
	auto result3 = index.check_and_record (vote3, lumex::uint128_t{ 50 }, now);
	ASSERT_EQ (result3, lumex::vote_rebroadcaster_index::result::representatives_full);
	ASSERT_EQ (index.representatives_count (), 2);

	// Add third rep with higher weight - should replace lowest weight
	auto vote4 = lumex::test::make_vote (keys[3], hashes);
	auto result4 = index.check_and_record (vote4, lumex::uint128_t{ 300 }, now);
	ASSERT_EQ (result4, lumex::vote_rebroadcaster_index::result::ok);
...
TEST (vote_rebroadcaster_index, history_limit)
{
	...
	{
		std::vector<lumex::block_hash> hash = { lumex::block_hash{ i } };
		auto vote = lumex::test::make_vote (key, hash);
		index.check_and_record (vote, lumex::uint128_t{ 100 }, now);
	}

	ASSERT_EQ (index.total_history (), 2);
	ASSERT_FALSE (index.contains_block (key.pub, lumex::block_hash{ 0 })); // Oldest was removed
	ASSERT_TRUE (index.contains_block (key.pub, lumex::block_hash{ 1 }));
	ASSERT_TRUE (index.contains_block (key.pub, lumex::block_hash{ 2 }));
}
...
TEST (vote_rebroadcaster_index, cleanup)
{
	...
	// Add two reps
	auto vote1 = lumex::test::make_vote (key1, hashes);
	auto vote2 = lumex::test::make_vote (key2, hashes);
	index.check_and_record (vote1, lumex::uint128_t{ 100 }, now);
	index.check_and_record (vote2, lumex::uint128_t{ 200 }, now);

	// Cleanup with rep1 becoming non-principal
	auto cleanup_count = index.cleanup ([&key1] (lumex::account const & account) {
		return std::make_pair (
		account == key1.pub ? false : true,
		account == key1.pub ? lumex::uint128_t{ 0 } : lumex::uint128_t{ 200 });
	});
...
TEST (vote_rebroadcaster_index, weight_updates)
{
	...
	index.cleanup ([] (lumex::account const &) {
		return std::make_pair (true, lumex::uint128_t{ 200 });
	});

	// Add new rep with lower weight - should be rejected due to updated weight
	auto vote2 = lumex::test::make_vote (key2, hashes);
	auto result = index.check_and_record (vote2, lumex::uint128_t{ 150 }, now);
	ASSERT_EQ (result, lumex::vote_rebroadcaster_index::result::representatives_full);
}
...
TEST (vote_rebroadcaster, basic_rebroadcast)
{
	...
	auto block = lumex::test::setup_chain (system, node, 1, lumex::dev::genesis_key, false).front ();

	// Start election for the block
	ASSERT_TRUE (lumex::test::start_election (system, node, block->hash ()));

	// Genesis should be viewed as a principal representative
	ASSERT_NE (lumex::rep_tier::none, node.rep_tiers.tier (lumex::dev::genesis_key.pub));

	// Create a vote from genesis (principal representative)
	auto vote = lumex::test::make_vote (lumex::dev::genesis_key, { block->hash () });
	...
	ASSERT_TIMELY (5s, node.stats.count (lumex::stat::type::vote_rebroadcaster, lumex::stat::detail::queued) > 0);

	// Verify processing and rebroadcast stats
	ASSERT_TIMELY (5s, node.stats.count (lumex::stat::type::vote_rebroadcaster, lumex::stat::detail::process) > 0);
	ASSERT_TIMELY (5s, node.stats.count (lumex::stat::type::vote_rebroadcaster, lumex::stat::detail::rebroadcast) > 0);
}
...
TEST (vote_rebroadcaster, local_representative_votes_skipped)
{
	...
	ASSERT_TIMELY (5s, node.wallets.reps ().exists (lumex::dev::genesis_key.pub));
	...
	node.stats.clear ();
	ASSERT_TIMELY (5s, node.stats.count (lumex::stat::type::vote_rebroadcaster, lumex::stat::detail::refresh) > 0);
	node.stats.clear ();
	...
	ASSERT_ALWAYS_EQ (250ms, 0, node.stats.count (lumex::stat::type::vote_rebroadcaster, lumex::stat::detail::queued));
}
...
TEST (vote_rebroadcaster, disabled_when_principal_representative)
{
	...
	node.stats.clear ();
	ASSERT_TIMELY (5s, node.stats.count (lumex::stat::type::vote_rebroadcaster, lumex::stat::detail::refresh) > 1);

	// Create a different representative key with enough weight to be tier 1+
	lumex::keypair other_rep;
	auto const send_amount = lumex::Klumex_ratio * 1000;
	...
	auto send = lumex::state_block_builder ()
				...
				.link (other_rep.pub)
				.balance (lumex::dev::constants.genesis_amount - send_amount)
	...
	auto open = lumex::state_block_builder ()
	...
	ASSERT_TRUE (lumex::test::start_election (system, node, send->hash ()));

	// Wait for rep tiers to recognize other_rep
	ASSERT_TIMELY (10s, node.rep_tiers.tier (other_rep.pub) != lumex::rep_tier::none);

	// Create a vote from other_rep (external principal representative)
	auto vote = lumex::test::make_vote (other_rep, { send->hash () });
...
TEST (vote_rebroadcaster, non_principal_rep_votes_rejected)
{
	...
	lumex::keypair random_key;
	ASSERT_EQ (lumex::rep_tier::none, node.rep_tiers.tier (random_key.pub));

	auto vote = lumex::test::make_vote (random_key, { block->hash () });
	...
	ASSERT_TIMELY (5s, node.stats.count (lumex::stat::type::vote_processor, lumex::stat::detail::process) > 0);
...
TEST (vote_rebroadcaster, duplicate_vote_ignored)
{
	...
	auto tier = node.rep_tiers.tier (lumex::dev::genesis_key.pub);
	ASSERT_NE (lumex::rep_tier::none, tier);
	...
	ASSERT_EQ (1, node.stats.count (lumex::stat::type::vote_rebroadcaster, lumex::stat::detail::queued));
}
