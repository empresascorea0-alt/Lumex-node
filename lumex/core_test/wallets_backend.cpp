...
namespace
{
std::unique_ptr<lumex::wallet::wallets_backend> make_wallets_backend (std::filesystem::path const & path)
{
	return std::make_unique<lumex::wallet::lmdb::wallets_backend_lmdb> (path);
}

std::unique_ptr<lumex::wallet::wallets_backend> make_wallets_backend ()
{
	return make_wallets_backend (lumex::unique_path () / "wallet.ldb");
}

// Convenience: collect all (account, wallet_value) pairs from an iterator range
auto collect_accounts (lumex::store::iterator begin, lumex::store::iterator const & end) -> std::vector<std::pair<lumex::account, lumex::wallet::wallet_value>>
{
	std::vector<std::pair<lumex::account, lumex::wallet::wallet_value>> result;
	for (auto it = std::move (begin); it != end; ++it)
	{
		result.emplace_back (static_cast<lumex::account> (lumex::store::db_val{ it->first }), lumex::wallet::wallet_value{ lumex::store::db_val{ it->second } });
	}
	return result;
}

// Convenience: collect raw keys from an iterator range as strings
auto collect_keys (lumex::store::iterator begin, lumex::store::iterator const & end) -> std::vector<std::string>
{
	std::vector<std::string> result;
	for (auto it = std::move (begin); it != end; ++it)
	{
		result.push_back (lumex::bytes_to_string (it->first));
	}
	return result;
}

lumex::wallet::wallet_value make_value (uint64_t seed)
{
	lumex::raw_key key;
	key = seed;
	return lumex::wallet::wallet_value{ key, seed };
}
}
...
{
	auto backend = make_wallets_backend ();
	auto id = lumex::random_wallet_id ().to_string ();
...
TEST (wallets_backend, wallet_open_or_create_distinct_ids_distinct_handles)
{
	...
	auto handle_a = backend->wallet_open_or_create (wallet_txn, lumex::random_wallet_id ().to_string ());
	auto handle_b = backend->wallet_open_or_create (wallet_txn, lumex::random_wallet_id ().to_string ());
...
TEST (wallets_backend, wallet_drop_clears_handle_and_data)
{
	...
	auto id = lumex::random_wallet_id ().to_string ();
	{
		...
		backend->entry_put (wallet_txn, handle, lumex::account{ 7 }, make_value (42));
		ASSERT_TRUE (backend->entry_exists (wallet_txn, handle, lumex::account{ 7 }));
	...
	}
	{
		...
		ASSERT_FALSE (backend->entry_exists (wallet_txn, handle, lumex::account{ 7 }));
	}
}
...
TEST (wallets_backend, wallet_drop_empty_handle)
{
	auto backend = make_wallets_backend ();
	auto wallet_txn = backend->tx_begin_write ();
	lumex::wallet::wallet_handle handle;
	ASSERT_FALSE (handle.valid ());
	backend->wallet_drop (wallet_txn, handle);
	ASSERT_FALSE (handle.valid ());
}
...
TEST (wallets_backend, entry_get_missing_returns_nullopt)
{
	auto backend = make_wallets_backend ();
	auto wallet_txn = backend->tx_begin_write ();
	auto handle = backend->wallet_open_or_create (wallet_txn, lumex::random_wallet_id ().to_string ());
	auto value = backend->entry_get (wallet_txn, handle, lumex::account{ 1 });
...
TEST (wallets_backend, entry_put_get_roundtrip)
{
	...
	backend->entry_put (wallet_txn, handle, lumex::account{ 9 }, original);

	auto retrieved = backend->entry_get (wallet_txn, handle, lumex::account{ 9 });
	ASSERT_TRUE (retrieved.has_value ());
	auto retrieved_value = lumex::wallet::wallet_value{ *retrieved };
...
TEST (wallets_backend, entry_put_overwrites)
{
	...
	backend->entry_put (wallet_txn, handle, lumex::account{ 1 }, make_value (10));
	backend->entry_put (wallet_txn, handle, lumex::account{ 1 }, make_value (20));

	auto retrieved = backend->entry_get (wallet_txn, handle, lumex::account{ 1 });
	ASSERT_TRUE (retrieved.has_value ());
	ASSERT_EQ (lumex::wallet::wallet_value{ *retrieved }.work, 20);
}
...
TEST (wallets_backend, entry_del_removes)
{
	...
	backend->entry_put (wallet_txn, handle, lumex::account{ 5 }, make_value (1));
	ASSERT_TRUE (backend->entry_exists (wallet_txn, handle, lumex::account{ 5 }));

	backend->entry_del (wallet_txn, handle, lumex::account{ 5 });
	ASSERT_FALSE (backend->entry_exists (wallet_txn, handle, lumex::account{ 5 }));
	ASSERT_FALSE (backend->entry_get (wallet_txn, handle, lumex::account{ 5 }).has_value ());
}
...
TEST (wallets_backend, entry_exists_distinguishes_present_and_missing)
{
	...
	ASSERT_FALSE (backend->entry_exists (wallet_txn, handle, lumex::account{ 99 }));
	backend->entry_put (wallet_txn, handle, lumex::account{ 99 }, make_value (1));
	ASSERT_TRUE (backend->entry_exists (wallet_txn, handle, lumex::account{ 99 }));
}
...
TEST (wallets_backend, entries_isolated_per_wallet)
{
	...
	backend->entry_put (wallet_txn, handle_a, lumex::account{ 1 }, make_value (100));
	backend->entry_put (wallet_txn, handle_b, lumex::account{ 1 }, make_value (200));

	auto from_a = backend->entry_get (wallet_txn, handle_a, lumex::account{ 1 });
	auto from_b = backend->entry_get (wallet_txn, handle_b, lumex::account{ 1 });
	ASSERT_TRUE (from_a.has_value ());
	ASSERT_TRUE (from_b.has_value ());
	ASSERT_EQ (lumex::wallet::wallet_value{ *from_a }.work, 100);
	ASSERT_EQ (lumex::wallet::wallet_value{ *from_b }.work, 200);
}
...
TEST (wallets_backend, entries_visible_in_read_txn_after_write_commit)
{
	...
	auto id = lumex::random_wallet_id ().to_string ();
	{
		auto wallet_txn = backend->tx_begin_write ();
		auto handle = backend->wallet_open_or_create (wallet_txn, id);
		backend->entry_put (wallet_txn, handle, lumex::account{ 11 }, make_value (1));
	}
	auto wallet_txn = backend->tx_begin_read ();
	auto handle = backend->wallet_open_or_create (backend->tx_begin_write (), id);
	auto value = backend->entry_get (wallet_txn, handle, lumex::account{ 11 });
	ASSERT_TRUE (value.has_value ());
	ASSERT_EQ (lumex::wallet::wallet_value{ *value }.work, 1);
}
...
TEST (wallets_backend, entries_iterate_sorted)
{
	...
	for (auto seed : seeds)
	{
		backend->entry_put (wallet_txn, handle, lumex::account{ seed }, make_value (seed));
	}
	std::vector<lumex::account> inserted;
	...
	std::set<lumex::account> got;
	for (auto const & e : entries)
	{
		got.insert (e.first);
	}
	std::set<lumex::account> expected{ inserted.begin (), inserted.end () };
...
TEST (wallets_backend, entries_iterate_lower_bound)
{
	...
	{
		backend->entry_put (wallet_txn, handle, lumex::account{ i }, make_value (i));
	}

	// Lower bound at 10 must skip 1 and 5.
	std::vector<lumex::account> seen;
	for (auto it = backend->entries_begin (wallet_txn, handle, lumex::store::db_val{ lumex::account{ 10 } }),
			  end = backend->entries_end (wallet_txn, handle);
		 it != end;
		 ++it)
	{
		seen.push_back (static_cast<lumex::account> (lumex::store::db_val{ it->first }));
	}
	ASSERT_EQ (seen, (std::vector<lumex::account>{ lumex::account{ 10 }, lumex::account{ 20 }, lumex::account{ 30 } }));

	// Lower bound at 11 must skip 10 too.
	seen.clear ();
	for (auto it = backend->entries_begin (wallet_txn, handle, lumex::store::db_val{ lumex::account{ 11 } }),
			  end = backend->entries_end (wallet_txn, handle);
		 it != end;
		 ++it)
	{
	...
	ASSERT_EQ (seen, (std::vector<lumex::account>{ lumex::account{ 20 }, lumex::account{ 30 } }));
}
...
TEST (wallets_backend, entries_iterate_lower_bound_past_end)
{
	...
	backend->entry_put (wallet_txn, handle, lumex::account{ 1 }, make_value (1));
	backend->entry_put (wallet_txn, handle, lumex::account{ 2 }, make_value (2));

	auto begin = backend->entries_begin (wallet_txn, handle, lumex::store::db_val{ lumex::account{ 100 } });
...
{
	auto backend = make_wallets_backend ();
	auto kept = lumex::random_wallet_id ().to_string ();
	auto dropped = lumex::random_wallet_id ().to_string ();
...
TEST (wallets_backend, index_iterate_lower_bound)
{
	...
	auto wallet_txn = backend->tx_begin_read ();
	auto seen = collect_keys (backend->index_begin (wallet_txn, lumex::store::db_val{ id_mid }), backend->index_end (wallet_txn));
...
{
	auto backend = make_wallets_backend ();
	lumex::block_hash hash{ 0xdeadbeef };
	{
		auto wallet_txn = backend->tx_begin_write ();
	...
	ASSERT_TRUE (retrieved.has_value ());
	ASSERT_EQ (static_cast<lumex::block_hash> (*retrieved), hash);
}
...
TEST (wallets_backend, send_action_id_put_overwrites)
{
	auto backend = make_wallets_backend ();
	auto wallet_txn = backend->tx_begin_write ();
	lumex::block_hash first{ 1 };
	lumex::block_hash second{ 2 };
	...
	auto retrieved = backend->send_action_id_get (wallet_txn, "id-a");
	ASSERT_TRUE (retrieved.has_value ());
	ASSERT_EQ (static_cast<lumex::block_hash> (*retrieved), second);
}
...
TEST (wallets_backend, send_action_ids_clear_empties_table)
{
	...
	{
		auto wallet_txn = backend->tx_begin_write ();
		backend->send_action_id_put (wallet_txn, "id-a", lumex::block_hash{ 1 });
		backend->send_action_id_put (wallet_txn, "id-b", lumex::block_hash{ 2 });
	}
	{
		auto wallet_txn = backend->tx_begin_write ();
...
TEST (wallets_backend, send_action_ids_clear_preserves_table_for_future_puts)
{
	auto backend = make_wallets_backend ();
	{
		...
		backend->send_action_id_put (wallet_txn, "id-a", lumex::block_hash{ 1 });
		backend->send_action_ids_clear (wallet_txn);
		ASSERT_TRUE (backend->send_action_id_put (wallet_txn, "id-b", lumex::block_hash{ 2 }));
	}
	auto wallet_txn = backend->tx_begin_read ();
	auto retrieved = backend->send_action_id_get (wallet_txn, "id-b");
	ASSERT_TRUE (retrieved.has_value ());
	ASSERT_EQ (static_cast<lumex::block_hash> (*retrieved), lumex::block_hash{ 2 });
}
...
{
	auto path = lumex::unique_path () / "wallet.ldb";
	auto id = lumex::random_wallet_id ().to_string ();

	{
		...
		auto wallet_txn = backend->tx_begin_write ();
		auto handle = backend->wallet_open_or_create (wallet_txn, id);
		backend->entry_put (wallet_txn, handle, lumex::account{ 42 }, make_value (777));
		backend->send_action_id_put (wallet_txn, "id-x", lumex::block_hash{ 0xfeedface });
	}
	{
		auto backend = make_wallets_backend (path);
		auto wallet_txn = backend->tx_begin_write ();
		...
		auto entry = backend->entry_get (wallet_txn, handle, lumex::account{ 42 });
		ASSERT_TRUE (entry.has_value ());
		ASSERT_EQ (lumex::wallet::wallet_value{ *entry }.work, 777);

		auto hash = backend->send_action_id_get (wallet_txn, "id-x");
		ASSERT_TRUE (hash.has_value ());
		ASSERT_EQ (static_cast<lumex::block_hash> (*hash), lumex::block_hash{ 0xfeedface });
	}
}
