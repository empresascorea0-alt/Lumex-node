...
TEST (cli, key_create)
{
	...
	lumex::raw_key private_key;
	private_key.decode_hex (private_key_str);

	auto public_key = lumex::pub_key (private_key);
...
{
	std::vector<lumex::config_key_value_pair> key_value_pairs;
	auto config_overrides = lumex::config_overrides (key_value_pairs);
	ASSERT_TRUE (config_overrides.empty ());
	key_value_pairs.push_back ({ "key", "value" });
	config_overrides = lumex::config_overrides (key_value_pairs);
...
TEST (cli, enable_rpc_flag)
{
	...
	char const * argv[] = { "lumex_node", "--enable_rpc" };
	boost::program_options::variables_map vm;
