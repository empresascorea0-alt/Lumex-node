...
TEST (object_stream, object_writer_basic)
{
	...
	lumex::object_stream obs{ ss };
	obs.write ("object_field", [] (lumex::object_stream & obs) {
...
TEST (object_stream, object_writer_nested)
{
	...
	obs.write ("object_field", [] (lumex::object_stream & obs) {
		...
		obs.write ("nested_object", [] (lumex::object_stream & obs) {
...
TEST (object_stream, array_writer_basic)
{
	...
	lumex::object_stream obs{ ss };
	obs.write ("array_field", [] (lumex::array_stream & ars) {
...
{
public:
	lumex::uint256_union uint256_union_field{ 0 };
	lumex::block_hash block_hash{ 0 };

	void operator() (lumex::object_stream & obs) const
	{
...
TEST (object_stream, array_writer_objects)
{
	...
	lumex::object_stream obs{ ss };
	obs.write ("array_field", [&objects] (lumex::array_stream & ars) {
		ars.write (objects);
	});
...
public:
	...
	void operator() (lumex::array_stream & ars) const
	{
		ars.write (values);
	}
};
...
namespace lumex
{
using builtin_array_with_pair = std::vector<std::pair<lumex::block_hash, int>>;

void stream_as (std::pair<lumex::block_hash, int> const & entry, lumex::object_stream & obs)
{
...
TEST (object_stream, builtin_array)
{
	...
	array.push_back ({ lumex::block_hash{ 1 }, 1 });
	array.push_back ({ lumex::block_hash{ 2 }, 2 });
	array.push_back ({ lumex::block_hash{ 3 }, 3 });
...
{
	using namespace lumex::object_stream_adapters;

	std::stringstream ss1, ss2;

	streamable_object test_object;
	ss1 << test_object; // Using automatic ostream adapter (in `lumex::ostream_operators`)
	ss2 << lumex::streamed (test_object); // Using explicit ostream adapter
...
{
	streamable_object test_object;
	auto str = fmt::format ("{}", lumex::streamed (test_object)); // Using explicit fmt adapter
...
TEST (object_stream, print_range)
{
	...
	std::stringstream ss1, ss2;
	ss1 << lumex::streamed_range (objects);
	ss2 << fmt::format ("{}", lumex::streamed_range (objects));
