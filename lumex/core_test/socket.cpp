...
#include <lumex/boost/asio/ip/network_v6.hpp>
...
TEST (socket_functions, limited_subnet_address)
{
	...
	auto network = lumex::transport::socket_functions::get_ipv6_subnet_address (address.to_v6 (), 32); // network prefix = 32.
...
{
	auto address = boost::asio::ip::make_address ("a41d:b7b2:8298:cf45:672e:bd1a:e7fb:f713");
	auto first_address = lumex::transport::socket_functions::first_ipv6_subnet_address (address.to_v6 (), 32); // network prefix = 32.
...
{
	auto address = boost::asio::ip::make_address ("a41d:b7b2:8298:cf45:672e:bd1a:e7fb:f713");
	auto last_address = lumex::transport::socket_functions::last_ipv6_subnet_address (address.to_v6 (), 32); // network prefix = 32.
...
TEST (socket_functions, count_subnetwork_connections)
{
	...
	auto connection0 = std::make_shared<lumex::transport::tcp_socket> (*node);
	auto connection1 = std::make_shared<lumex::transport::tcp_socket> (*node);
	auto connection2 = std::make_shared<lumex::transport::tcp_socket> (*node);
	auto connection3 = std::make_shared<lumex::transport::tcp_socket> (*node);
	auto connection4 = std::make_shared<lumex::transport::tcp_socket> (*node);
	auto connection5 = std::make_shared<lumex::transport::tcp_socket> (*node);
	auto connection6 = std::make_shared<lumex::transport::tcp_socket> (*node);

	lumex::transport::address_socket_mmap connections_per_address;
	...
	ASSERT_EQ (4, lumex::transport::socket_functions::count_subnetwork_connections (connections_per_address, address1.to_v6 (), 32));
}
...
TEST (socket, disconnection_of_silent_connections)
{
	...
	std::promise<std::shared_ptr<lumex::transport::tcp_socket>> server_data_socket_promise;
	std::future<std::shared_ptr<lumex::transport::tcp_socket>> server_data_socket_future = server_data_socket_promise.get_future ();
	...
	ASSERT_EQ (0, node->stats.count (lumex::stat::type::tcp, lumex::stat::detail::tcp_io_timeout_drop, lumex::stat::dir::in));
	// Asserts the silent checker worked.
	ASSERT_LE (1, node->stats.count (lumex::stat::type::tcp, lumex::stat::detail::tcp_silent_connection_drop, lumex::stat::dir::in));
}
...
TEST (socket, DISABLED_drop_policy)
{
	...
	lumex::inactive_node inactivenode (lumex::unique_path (), node_flags);
	...
	auto func = [&] (size_t total_message_count) {
		...
		auto client = std::make_shared<lumex::transport::tcp_socket> (*node);
		...
		[&] (boost::system::error_code const & ec_a) mutable {
			for (int i = 0; i < total_message_count; i++)
			{
				...
				client->async_write (lumex::shared_const_buffer (std::move (buff)), [&] (boost::system::error_code const & ec, size_t size_a) {
					if (!ec)
					{
						++completed_writes;
					}
...
TEST (socket, DISABLED_concurrent_writes)
{
	...
	using reader_callback_t = std::function<void (std::shared_ptr<lumex::transport::tcp_socket> const &)>;
	reader_callback_t reader = [&completed_reads, &total_message_count, &reader] (std::shared_ptr<lumex::transport::tcp_socket> const & socket_a) {
	...
	std::vector<std::shared_ptr<lumex::transport::tcp_socket>> connections;
	...
	accept_callback_t accept_callback = [&] (boost::system::error_code const & ec, boost::asio::ip::tcp::socket socket) {
		if (!ec)
		{
			auto new_connection = std::make_shared<lumex::transport::tcp_socket> (*node, std::move (socket));
	...
	std::vector<std::shared_ptr<lumex::transport::tcp_socket>> clients;
	...
	for (int i = 0; i < client_count; i++)
	{
		client_threads.emplace_back ([&client, &message_count] () {
			for (int i = 0; i < message_count; i++)
			{
				...
				buff.push_back ('A' + i);
				client->async_write (lumex::shared_const_buffer (std::move (buff)));
			}
		});
	}
...
TEST (socket_timeout, connect)
{
	...
	std::shared_ptr<lumex::node> node = system.nodes[0];
	...
	auto socket = std::make_shared<lumex::transport::tcp_socket> (*node);
...
TEST (socket_timeout, read)
{
	...
	ASSERT_TRUE (ec);
	ASSERT_EQ (1, node->stats.count (lumex::stat::type::tcp, lumex::stat::detail::tcp_read_error));

	// check that the socket was closed due to tcp_io_timeout timeout
	ASSERT_EQ (1, node->stats.count (lumex::stat::type::tcp, lumex::stat::detail::tcp_io_timeout_drop, lumex::stat::dir::in));
}
...
TEST (socket_timeout, write)
{
	...
	auto socket = std::make_shared<lumex::transport::tcp_socket> (*node, lumex::transport::socket_endpoint::client); // socket with a max queue size much larger than OS buffers
	...
	// check that the callback was called and we got an error
	ASSERT_TRUE (ec);
	ASSERT_LE (1, node->stats.count (lumex::stat::type::tcp, lumex::stat::detail::tcp_write_error));

	// check that the socket was closed due to tcp_io_timeout timeout
	ASSERT_LE (1, node->stats.count (lumex::stat::type::tcp, lumex::stat::detail::tcp_io_timeout_drop, lumex::stat::dir::out));
}
...
TEST (socket_timeout, read_overlapped)
{
	...
	acceptor.async_accept (newsock, [&newsock] (boost::system::error_code const & ec_a) {
		...
		auto buffer = std::make_shared<std::vector<uint8_t>> (1);
		lumex::async_write (newsock, lumex::shared_const_buffer (buffer), [] (boost::system::error_code const & ec_a, size_t size_a) {
			EXPECT_TRUE (!ec_a);
			EXPECT_EQ (size_a, 1);
		});
	});
	...
	// check that the callback was called and we got an error
	ASSERT_LE (1, node->stats.count (lumex::stat::type::tcp, lumex::stat::detail::tcp_read_error));

	// check that the socket was closed due to tcp_io_timeout timeout
	ASSERT_LE (1, node->stats.count (lumex::stat::type::tcp, lumex::stat::detail::tcp_io_timeout_drop, lumex::stat::dir::in));
}
...
TEST (socket_timeout, write_overlapped)
{
	...
	// eventually, the all tcp queues should fill up and async_write will not be able to progress
	// and the timeout should kick in and close the socket, which will cause the async_write to return an error
	auto socket = std::make_shared<lumex::transport::tcp_socket> (*node, lumex::transport::socket_endpoint::client);
	...
	auto [ec_1, size_1] = socket->blocking_write (lumex::shared_const_buffer{ buffer1 }, 1);
	ASSERT_FALSE (ec_1);
	ASSERT_EQ (size_1, 1);
	...
	{
		auto [ec_2, size_2] = socket->blocking_write (lumex::shared_const_buffer{ buffer2 }, 128 * 1024);
		if (ec_2)
		{
			ec = ec_2;
			break;
