...
TEST (tcp_listener, max_connections)
{
	...
	auto client1 = std::make_shared<lumex::transport::tcp_socket> (*node);
	client1->async_connect (node->network.endpoint (), connect_handler);

	auto client2 = std::make_shared<lumex::transport::tcp_socket> (*node);
	client2->async_connect (node->network.endpoint (), connect_handler);

	auto client3 = std::make_shared<lumex::transport::tcp_socket> (*node);
	client3->async_connect (node->network.endpoint (), connect_handler);

	ASSERT_TIMELY_EQ (5s, node->stats.count (lumex::stat::type::tcp_listener, lumex::stat::detail::accept_success), 2);
	ASSERT_ALWAYS_EQ (1s, node->stats.count (lumex::stat::type::tcp_listener, lumex::stat::detail::accept_success), 2);
...
TEST (tcp_listener, max_connections_per_ip)
{
	...
	std::vector<std::shared_ptr<lumex::transport::tcp_socket>> client_list;
	...
	ASSERT_TIMELY_EQ (5s, node->stats.count (lumex::stat::type::tcp_listener, lumex::stat::detail::accept_success), max_ip_connections);
	ASSERT_TIMELY_EQ (5s, node->stats.count (lumex::stat::type::tcp_listener_rejected, lumex::stat::detail::max_per_ip), 1);
...
TEST (tcp_listener, max_connections_per_subnetwork)
{
	...
	ASSERT_TIMELY_EQ (5s, node->stats.count (lumex::stat::type::tcp_listener, lumex::stat::detail::accept_success), max_subnetwork_connections);
	ASSERT_TIMELY_EQ (5s, node->stats.count (lumex::stat::type::tcp_listener_rejected, lumex::stat::detail::max_per_subnetwork), 1);
...
TEST (tcp_listener, max_peers_per_ip)
{
	...
	ASSERT_TIMELY_EQ (5s, node->stats.count (lumex::stat::type::tcp_listener, lumex::stat::detail::accept_success), max_ip_connections + 1);
	ASSERT_TIMELY_EQ (5s, node->stats.count (lumex::stat::type::tcp_listener_rejected, lumex::stat::detail::max_per_ip), 0);
	ASSERT_TIMELY_EQ (5s, connection_attempts, max_ip_connections + 1);
}
...
TEST (tcp_listener, node_id_handshake)
{
	...
	auto socket (std::make_shared<lumex::transport::tcp_socket> (*system.nodes[0]));
	auto bootstrap_endpoint (system.nodes[0]->tcp_listener.endpoint ());
	auto cookie (system.nodes[0]->network.syn_cookies.assign (lumex::transport::map_tcp_to_endpoint (bootstrap_endpoint)));
	ASSERT_TRUE (cookie);
	lumex::messages::node_id_handshake::query_payload query{ *cookie };
	lumex::messages::node_id_handshake node_id_handshake{ lumex::dev::network_params.network, query };
	...
	lumex::messages::node_id_handshake::response_payload response_zero{ 0 };
	lumex::messages::node_id_handshake node_id_handshake_response{ lumex::dev::network_params.network, std::nullopt, response_zero };
...
TEST (tcp_listener, timeout_empty)
{
	...
	auto socket (std::make_shared<lumex::transport::tcp_socket> (*node0));
...
TEST (tcp_listener, timeout_node_id_handshake)
{
	...
	auto node0 = system.add_node (config);
	auto socket (std::make_shared<lumex::transport::tcp_socket> (*node0));
	auto cookie (node0->network.syn_cookies.assign (lumex::transport::map_tcp_to_endpoint (node0->tcp_listener.endpoint ())));
	ASSERT_TRUE (cookie);
	lumex::messages::node_id_handshake::query_payload query{ *cookie };
	lumex::messages::node_id_handshake node_id_handshake{ lumex::dev::network_params.network, query };
	auto channel = std::make_shared<lumex::transport::tcp_channel> (*node0, socket);
	socket->async_connect (node0->tcp_listener.endpoint (), [&node_id_handshake, channel] (boost::system::error_code const & ec) {
		ASSERT_FALSE (ec);
		channel->send (node_id_handshake, lumex::transport::traffic_type::test, [] (boost::system::error_code const & ec, size_t size_a) {
			ASSERT_FALSE (ec);
		});
	});
	ASSERT_TIMELY (5s, node0->stats.count (lumex::stat::type::tcp_server, lumex::stat::detail::node_id_handshake) != 0);
	ASSERT_TIMELY_EQ (5s, node0->tcp_listener.connection_count (), 1);
	ASSERT_TIMELY_EQ (10s, node0->tcp_listener.connection_count (), 0);
}