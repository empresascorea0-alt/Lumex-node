#include <lumex/lib/blockbuilders.hpp>
#include <lumex/lib/blocks.hpp>
#include <lumex/lib/files.hpp>
#include <lumex/lib/logging.hpp>
#include <lumex/lib/numbers.hpp>
#include <lumex/lib/stats.hpp>
#include <lumex/lib/threading.hpp>
#include <lumex/lib/vote.hpp>
#include <lumex/node/active_elections.hpp>
#include <lumex/node/backlog_scan.hpp>
#include <lumex/node/block_processor.hpp>
#include <lumex/node/election.hpp>
#include <lumex/node/nodeconfig.hpp>
#include <lumex/node/scheduler/component.hpp>
#include <lumex/node/scheduler/priority.hpp>
#include <lumex/node/transport/inproc.hpp>
#include <lumex/node/unchecked_map.hpp>
#include <lumex/node/vote_processor.hpp>
#include <lumex/node/vote_router.hpp>
#include <lumex/secure/ledger_set_any.hpp>
#include <lumex/secure/ledger_set_cemented.hpp>
#include <lumex/store/ledger/account.hpp>
#include <lumex/store/ledger/block.hpp>
#include <lumex/store/ledger/confirmation_height.hpp>
#include <lumex/store/ledger/final_vote.hpp>
#include <lumex/store/ledger/online_weight.hpp>
#include <lumex/store/ledger/peer.hpp>
#include <lumex/store/ledger/pending.hpp>
#include <lumex/store/ledger/pruned.hpp>
#include <lumex/store/ledger/rep_weight.hpp>
#include <lumex/store/ledger/topology.hpp>
#include <lumex/store/ledger/version.hpp>
#include <lumex/store/lmdb/backend_lmdb.hpp>
#include <lumex/store/rocksdb/backend_rocksdb.hpp>
#include <lumex/test_common/ledger_context.hpp>
#include <lumex/test_common/make_store.hpp>
#include <lumex/test_common/system.hpp>
#include <lumex/test_common/testutil.hpp>
#include <lumex/weights/bootstrap_weights.hpp>

#include <gtest/gtest.h>

#include <limits>

using namespace std::chrono_literals;

// Init returns an error if it can't open files at the path
TEST (ledger, construct)
{
	auto ctx = lumex::test::ledger_empty ();
}

// Ledger can be initialized and returns a basic query for an empty account
TEST (ledger, empty)
{
	auto ctx = lumex::test::ledger_empty ();
	auto & ledger = ctx.ledger ();
	auto & store = ctx.store ();
	auto transaction = ledger.tx_begin_read ();
	lumex::account account;
	auto balance (ledger.any.account_balance (transaction, account));
	ASSERT_FALSE (balance);
}

// Ledger can be initialized and returns a basic query for an empty account
TEST (ledger, confirmed_unconfirmed_view)
{
	auto ctx = lumex::test::ledger_empty ();
	auto & ledger = ctx.ledger ();
	auto & unconfirmed = ledger;
	auto & cemented = ledger.cemented;
}

// Genesis account should have the max balance on empty initialization
TEST (ledger, genesis_balance)
{
	auto ctx = lumex::test::ledger_empty ();
	auto & ledger = ctx.ledger ();
	auto & store = ctx.store ();
	auto transaction = ledger.tx_begin_write ();
	auto balance = ledger.any.account_balance (transaction, lumex::dev::genesis_key.pub);
	ASSERT_EQ (lumex::dev::constants.genesis_amount, balance);
	auto info = ledger.any.account_get (transaction, lumex::dev::genesis_key.pub);
	ASSERT_TRUE (info);
	ASSERT_EQ (1, ledger.account_count ());
	// Frontier time should have been updated when genesis balance was added
	ASSERT_GE (lumex::seconds_since_epoch (), info->modified);
	ASSERT_LT (lumex::seconds_since_epoch () - info->modified, 10);
	// Genesis block should be confirmed by default
	lumex::confirmation_height_info confirmation_height_info;
	ASSERT_FALSE (store.confirmation_height.get (transaction, lumex::dev::genesis_key.pub, confirmation_height_info));
	ASSERT_EQ (confirmation_height_info.height, 1);
	ASSERT_EQ (confirmation_height_info.frontier, lumex::dev::genesis->hash ());
}

TEST (ledger, process_modifies_sideband)
{
	auto ctx = lumex::test::ledger_empty ();
	auto & ledger = ctx.ledger ();
	auto & store = ctx.store ();
	auto & pool = ctx.pool ();
	lumex::block_builder builder;
	auto send1 = builder
				 .state ()
				 .account (lumex::dev::genesis_key.pub)
				 .previous (lumex::dev::genesis->hash ())
				 .representative (lumex::dev::genesis_key.pub)
				 .balance (lumex::dev::constants.genesis_amount - lumex::Klumex_ratio)
				 .link (lumex::dev::genesis_key.pub)
				 .sign (lumex::dev::genesis_key.prv, lumex::dev::genesis_key.pub)
				 .work (*pool.generate (lumex::dev::genesis->hash ()))
				 .build ();
	ASSERT_EQ (lumex::block_status::progress, ledger.process (ledger.tx_begin_write (), send1));
	ASSERT_EQ (send1->sideband ().timestamp, ledger.any.block_get (ledger.tx_begin_read (), send1->hash ())->sideband ().timestamp);
}

// Create a send block and publish it.
TEST (ledger, process_send)
{
	auto ctx = lumex::test::ledger_empty ();
	auto & ledger = ctx.ledger ();
	auto & store = ctx.store ();
	auto transaction = ledger.tx_begin_write ();
	auto & pool = ctx.pool ();
	auto info1 = ledger.any.account_get (transaction, lumex::dev::genesis_key.pub);
	ASSERT_TRUE (info1);
	lumex::keypair key2;
	lumex::block_builder builder;
	auto send = builder
				.send ()
				.previous (info1->head)
				.destination (key2.pub)
				.balance (50)
				.sign (lumex::dev::genesis_key.prv, lumex::dev::genesis_key.pub)
				.work (*pool.generate (info1->head))
				.build ();
	lumex::block_hash hash1 = send->hash ();
	ASSERT_EQ (1, info1->block_count);
	// This was a valid block, it should progress.
	auto return1 = ledger.process (transaction, send);
	ASSERT_EQ (lumex::block_status::progress, return1);
	ASSERT_EQ (lumex::dev::genesis_key.pub, send->sideband ().account);
	ASSERT_EQ (2, send->sideband ().height);
	ASSERT_EQ (lumex::dev::constants.genesis_amount - 50, ledger.any.block_amount (transaction, hash1));
	ASSERT_EQ (lumex::dev::genesis_key.pub, send->account ());
	ASSERT_EQ (50, ledger.any.account_balance (transaction, lumex::dev::genesis_key.pub).value ().number ());
	ASSERT_EQ (lumex::dev::constants.genesis_amount - 50, ledger.account_receivable (transaction, key2.pub));
	auto info2 = ledger.any.account_get (transaction, lumex::dev::genesis_key.pub);
	ASSERT_TRUE (info2);
	ASSERT_EQ (2, info2->block_count);
	auto latest6 = ledger.any.block_get (transaction, info2->head);
	ASSERT_NE (nullptr, latest6);
	auto latest7 = dynamic_cast<lumex::send_block *> (latest6.get ());
	ASSERT_NE (nullptr, latest7);
	ASSERT_EQ (*send, *latest7);
	// Create an open block opening an account accepting the send we just created
	auto open = builder
				.open ()
				.source (hash1)
				.representative (key2.pub)
				.account (key2.pub)
				.sign (key2.prv, key2.pub)
				.work (*pool.generate (key2.pub))
				.build ();
	lumex::block_hash hash2 (open->hash ());
	// This was a valid block, it should progress.
	auto return2 = ledger.process (transaction, open);
	ASSERT_EQ (lumex::block_status::progress, return2);
	ASSERT_EQ (key2.pub, open->sideband ().account);
	ASSERT_EQ (lumex::dev::constants.genesis_amount - 50, open->sideband ().balance.number ());
	ASSERT_EQ (1, open->sideband ().height);
	ASSERT_EQ (lumex::dev::constants.genesis_amount - 50, ledger.any.block_amount (transaction, hash2));
	ASSERT_EQ (lumex::block_status::progress, return2);
	ASSERT_EQ (key2.pub, open->account ());
	ASSERT_EQ (lumex::dev::constants.genesis_amount - 50, ledger.any.block_amount (transaction, hash2));
	ASSERT_EQ (lumex::dev::constants.genesis_amount - 50, ledger.any.account_balance (transaction, key2.pub));
	ASSERT_EQ (0, ledger.account_receivable (transaction, key2.pub));
	ASSERT_EQ (50, ledger.weight (lumex::dev::genesis_key.pub));
	ASSERT_EQ (lumex::dev::constants.genesis_amount - 50, ledger.weight (key2.pub));
	auto info3 = ledger.any.account_get (transaction, lumex::dev::genesis_key.pub);
	ASSERT_TRUE (info3);
	auto latest2 = ledger.any.block_get (transaction, info3->head);
	ASSERT_NE (nullptr, latest2);
	auto latest3 = dynamic_cast<lumex::send_block *> (latest2.get ());
	ASSERT_NE (nullptr, latest3);
	ASSERT_EQ (*send, *latest3);
	auto info4 = ledger.any.account_get (transaction, key2.pub);
	ASSERT_TRUE (info4);
	auto latest4 = ledger.any.block_get (transaction, info4->head);
	ASSERT_NE (nullptr, latest4);
	auto latest5 = dynamic_cast<lumex::open_block *> (latest4.get ());
	ASSERT_NE (nullptr, latest5);
	ASSERT_EQ (*open, *latest5);
	ASSERT_FALSE (ledger.rollback (transaction, hash2));
	auto info5 = ledger.any.account_get (transaction, key2.pub);
	ASSERT_FALSE (info5);
	auto pending1 = ledger.any.pending_get (transaction, lumex::pending_key (key2.pub, hash1));
	ASSERT_TRUE (pending1);
	ASSERT_EQ (lumex::dev::genesis_key.pub, pending1->source);
	ASSERT_EQ (lumex::dev::constants.genesis_amount - 50, pending1->amount.number ());
	ASSERT_FALSE (ledger.any.account_balance (transaction, key2.pub));
	ASSERT_EQ (lumex::dev::constants.genesis_amount - 50, ledger.account_receivable (transaction, key2.pub));
	ASSERT_EQ (50, ledger.any.account_balance (transaction, lumex::dev::genesis_key.pub).value ().number ());
	ASSERT_EQ (50, ledger.weight (lumex::dev::genesis_key.pub));
	ASSERT_EQ (0, ledger.weight (key2.pub));
	auto info6 = ledger.any.account_get (transaction, lumex::dev::genesis_key.pub);
	ASSERT_TRUE (info6);
	ASSERT_EQ (hash1, info6->head);
	ASSERT_FALSE (ledger.rollback (transaction, info6->head));
	ASSERT_EQ (lumex::dev::constants.genesis_amount, ledger.weight (lumex::dev::genesis_key.pub));
	auto info7 = ledger.any.account_get (transaction, lumex::dev::genesis_key.pub);
	ASSERT_TRUE (info7);
	ASSERT_EQ (1, info7->block_count);
	ASSERT_EQ (info1->head, info7->head);
	ASSERT_FALSE (ledger.any.pending_get (transaction, lumex::pending_key (key2.pub, hash1)));
	ASSERT_EQ (lumex::dev::constants.genesis_amount, ledger.any.account_balance (transaction, lumex::dev::genesis_key.pub).value ().number ());
	ASSERT_EQ (0, ledger.account_receivable (transaction, key2.pub));
	ASSERT_EQ (store.account.count (transaction), ledger.account_count ());
}

TEST (ledger, process_receive)
{
	auto ctx = lumex::test::ledger_empty ();
	auto & ledger = ctx.ledger ();
	auto & store = ctx.store ();
	auto transaction = ledger.tx_begin_write ();
	auto & pool = ctx.pool ();
	auto info1 = ledger.any.account_get (transaction, lumex::dev::genesis_key.pub);
	ASSERT_TRUE (info1);
	lumex::keypair key2;
	lumex::block_builder builder;
	auto send = builder
				.send ()
				.previous (info1->head)
				.destination (key2.pub)
				.balance (50)
				.sign (lumex::dev::genesis_key.prv, lumex::dev::genesis_key.pub)
				.work (*pool.generate (info1->head))
				.build ();
	lumex::block_hash hash1 (send->hash ());
	ASSERT_EQ (lumex::block_status::progress, ledger.process (transaction, send));
	lumex::keypair key3;
	auto open = builder
				.open ()
				.source (hash1)
				.representative (key3.pub)
				.account (key2.pub)
				.sign (key2.prv, key2.pub)
				.work (*pool.generate (key2.pub))
				.build ();
	lumex::block_hash hash2 (open->hash ());
	auto return1 = ledger.process (transaction, open);
	ASSERT_EQ (lumex::block_status::progress, return1);
	ASSERT_EQ (key2.pub, open->account ());
	ASSERT_EQ (key2.pub, open->sideband ().account);
	ASSERT_EQ (lumex::dev::constants.genesis_amount - 50, open->sideband ().balance.number ());
	ASSERT_EQ (1, open->sideband ().height);
	ASSERT_EQ (lumex::dev::constants.genesis_amount - 50, ledger.any.block_amount (transaction, hash2));
	ASSERT_EQ (lumex::dev::constants.genesis_amount - 50, ledger.weight (key3.pub));
	auto send2 = builder
				 .send ()
				 .previous (hash1)
				 .destination (key2.pub)
				 .balance (25)
				 .sign (lumex::dev::genesis_key.prv, lumex::dev::genesis_key.pub)
				 .work (*pool.generate (hash1))
				 .build ();
	lumex::block_hash hash3 = send2->hash ();
	ASSERT_EQ (lumex::block_status::progress, ledger.process (transaction, send2));
	auto receive = builder
				   .receive ()
				   .previous (hash2)
				   .source (hash3)
				   .sign (key2.prv, key2.pub)
				   .work (*pool.generate (hash2))
				   .build ();
	auto hash4 = receive->hash ();
	auto return2 = ledger.process (transaction, receive);
	ASSERT_EQ (key2.pub, receive->sideband ().account);
	ASSERT_EQ (lumex::dev::constants.genesis_amount - 25, receive->sideband ().balance.number ());
	ASSERT_EQ (2, receive->sideband ().height);
	ASSERT_EQ (25, ledger.any.block_amount (transaction, hash4).value ().number ());
	ASSERT_EQ (lumex::block_status::progress, return2);
	ASSERT_EQ (key2.pub, receive->account ());
	ASSERT_EQ (hash4, ledger.any.account_head (transaction, key2.pub));
	ASSERT_EQ (25, ledger.any.account_balance (transaction, lumex::dev::genesis_key.pub).value ().number ());
	ASSERT_EQ (0, ledger.account_receivable (transaction, key2.pub));
	ASSERT_EQ (lumex::dev::constants.genesis_amount - 25, ledger.any.account_balance (transaction, key2.pub));
	ASSERT_EQ (lumex::dev::constants.genesis_amount - 25, ledger.weight (key3.pub));
	ASSERT_FALSE (ledger.rollback (transaction, hash4));
	ASSERT_FALSE (ledger.any.block_successor (transaction, hash2));
	ASSERT_EQ (25, ledger.any.account_balance (transaction, lumex::dev::genesis_key.pub).value ().number ());
	ASSERT_EQ (25, ledger.account_receivable (transaction, key2.pub));
	ASSERT_EQ (lumex::dev::constants.genesis_amount - 50, ledger.any.account_balance (transaction, key2.pub));
	ASSERT_EQ (lumex::dev::constants.genesis_amount - 50, ledger.weight (key3.pub));
	ASSERT_EQ (hash2, ledger.any.account_head (transaction, key2.pub));
	auto pending1 = ledger.any.pending_get (transaction, lumex::pending_key (key2.pub, hash3));
	ASSERT_TRUE (pending1);
	ASSERT_EQ (lumex::dev::genesis_key.pub, pending1->source);
	ASSERT_EQ (25, pending1->amount.number ());
	ASSERT_EQ (store.account.count (transaction), ledger.account_count ());
}

TEST (ledger, rollback_receiver)
{
	auto ctx = lumex::test::ledger_empty ();
	auto & ledger = ctx.ledger ();
	auto & store = ctx.store ();
	auto transaction = ledger.tx_begin_write ();
	auto & pool = ctx.pool ();
	auto info1 = ledger.any.account_get (transaction, lumex::dev::genesis_key.pub);
	ASSERT_TRUE (info1);
	lumex::keypair key2;
	lumex::block_builder builder;
	auto send = builder
				.send ()
				.previous (info1->head)
				.destination (key2.pub)
				.balance (50)
				.sign (lumex::dev::genesis_key.prv, lumex::dev::genesis_key.pub)
				.work (*pool.generate (info1->head))
				.build ();
	lumex::block_hash hash1 (send->hash ());
	ASSERT_EQ (lumex::block_status::progress, ledger.process (transaction, send));
	lumex::keypair key3;
	auto open = builder
				.open ()
				.source (hash1)
				.representative (key3.pub)
				.account (key2.pub)
				.sign (key2.prv, key2.pub)
				.work (*pool.generate (key2.pub))
				.build ();
	lumex::block_hash hash2 (open->hash ());
	ASSERT_EQ (lumex::block_status::progress, ledger.process (transaction, open));
	ASSERT_EQ (hash2, ledger.any.account_head (transaction, key2.pub));
	ASSERT_EQ (50, ledger.any.account_balance (transaction, lumex::dev::genesis_key.pub).value ().number ());
	ASSERT_EQ (lumex::dev::constants.genesis_amount - 50, ledger.any.account_balance (transaction, key2.pub));
	ASSERT_EQ (50, ledger.weight (lumex::dev::genesis_key.pub));
	ASSERT_EQ (0, ledger.weight (key2.pub));
	ASSERT_EQ (lumex::dev::constants.genesis_amount - 50, ledger.weight (key3.pub));
	ASSERT_FALSE (ledger.rollback (transaction, hash1));
	ASSERT_EQ (lumex::dev::constants.genesis_amount, ledger.any.account_balance (transaction, lumex::dev::genesis_key.pub));
	ASSERT_FALSE (ledger.any.account_balance (transaction, key2.pub));
	ASSERT_EQ (lumex::dev::constants.genesis_amount, ledger.weight (lumex::dev::genesis_key.pub));
	ASSERT_EQ (0, ledger.weight (key2.pub));
	ASSERT_EQ (0, ledger.weight (key3.pub));
	ASSERT_FALSE (ledger.any.account_get (transaction, key2.pub));
	ASSERT_EQ (store.account.count (transaction), ledger.account_count ());
	ASSERT_FALSE (ledger.any.pending_get (transaction, lumex::pending_key{ key2.pub, hash1 }));
}

TEST (ledger, rollback_representation)
{
	auto ctx = lumex::test::ledger_empty ();
	auto & ledger = ctx.ledger ();
	auto & store = ctx.store ();
	auto transaction = ledger.tx_begin_write ();
	auto & pool = ctx.pool ();
	lumex::keypair key5;
	lumex::block_builder builder;
	auto change1 = builder
				   .change ()
				   .previous (lumex::dev::genesis->hash ())
				   .representative (key5.pub)
				   .sign (lumex::dev::genesis_key.prv, lumex::dev::genesis_key.pub)
				   .work (*pool.generate (lumex::dev::genesis->hash ()))
				   .build ();
	ASSERT_EQ (lumex::block_status::progress, ledger.process (transaction, change1));
	lumex::keypair key3;
	auto change2 = builder
				   .change ()
				   .previous (change1->hash ())
				   .representative (key3.pub)
				   .sign (lumex::dev::genesis_key.prv, lumex::dev::genesis_key.pub)
				   .work (*pool.generate (change1->hash ()))
				   .build ();
	ASSERT_EQ (lumex::block_status::progress, ledger.process (transaction, change2));
	lumex::keypair key2;
	auto send1 = builder
				 .send ()
				 .previous (change2->hash ())
				 .destination (key2.pub)
				 .balance (50)
				 .sign (lumex::dev::genesis_key.prv, lumex::dev::genesis_key.pub)
				 .work (*pool.generate (change2->hash ()))
				 .build ();
	ASSERT_EQ (lumex::block_status::progress, ledger.process (transaction, send1));
	lumex::keypair key4;
	auto open = builder
				.open ()
				.source (send1->hash ())
				.representative (key4.pub)
				.account (key2.pub)
				.sign (key2.prv, key2.pub)
				.work (*pool.generate (key2.pub))
				.build ();
	ASSERT_EQ (lumex::block_status::progress, ledger.process (transaction, open));
	auto send2 = builder
				 .send ()
				 .previous (send1->hash ())
				 .destination (key2.pub)
				 .balance (1)
				 .sign (lumex::dev::genesis_key.prv, lumex::dev::genesis_key.pub)
				 .work (*pool.generate (send1->hash ()))
				 .build ();
	ASSERT_EQ (lumex::block_status::progress, ledger.process (transaction, send2));
	auto receive1 = builder
					.receive ()
					.previous (open->hash ())
					.source (send2->hash ())
					.sign (key2.prv, key2.pub)
					.work (*pool.generate (open->hash ()))
					.build ();
	ASSERT_EQ (lumex::block_status::progress, ledger.process (transaction, receive1));
	ASSERT_EQ (1, ledger.weight (key3.pub));
	ASSERT_EQ (lumex::dev::constants.genesis_amount - 1, ledger.weight (key4.pub));
	auto info1 = ledger.any.account_get (transaction, key2.pub);
	ASSERT_TRUE (info1);
	ASSERT_EQ (key4.pub, info1->representative);
	ASSERT_FALSE (ledger.rollback (transaction, receive1->hash ()));
	auto info2 = ledger.any.account_get (transaction, key2.pub);
	ASSERT_TRUE (info2);
	ASSERT_EQ (key4.pub, info2->representative);
	ASSERT_EQ (0, ledger.weight (key2.pub));
	ASSERT_EQ (lumex::dev::constants.genesis_amount - 50, ledger.weight (key4.pub));
	ASSERT_FALSE (ledger.rollback (transaction, open->hash ()));
	ASSERT_EQ (1, ledger.weight (key3.pub));
	ASSERT_EQ (0, ledger.weight (key4.pub));
	ledger.rollback (transaction, send1->hash ());
	ASSERT_EQ (lumex::dev::constants.genesis_amount, ledger.weight (key3.pub));
	auto info3 = ledger.any.account_get (transaction, lumex::dev::genesis_key.pub);
	ASSERT_TRUE (info3);
	ASSERT_EQ (key3.pub, info3->representative);
	ASSERT_FALSE (ledger.rollback (transaction, change2->hash ()));
	auto info4 = ledger.any.account_get (transaction, lumex::dev::genesis_key.pub);
	ASSERT_TRUE (info4);
	ASSERT_EQ (key5.pub, info4->representative);
	ASSERT_EQ (lumex::dev::constants.genesis_amount, ledger.weight (key5.pub));
	ASSERT_EQ (0, ledger.weight (key3.pub));
}

TEST (ledger, receive_rollback)
{
	auto ctx = lumex::test::ledger_empty ();
	auto & ledger = ctx.ledger ();
	auto & store = ctx.store ();
	auto transaction = ledger.tx_begin_write ();
	auto & pool = ctx.pool ();
	lumex::block_builder builder;
	auto send = builder
				.send ()
				.previous (lumex::dev::genesis->hash ())
				.destination (lumex::dev::genesis_key.pub)
				.balance (lumex::dev::constants.genesis_amount - lumex::Klumex_ratio)
				.sign (lumex::dev::genesis_key.prv, lumex::dev::genesis_key.pub)
				.work (*pool.generate (lumex::dev::genesis->hash ()))
				.build ();
	ASSERT_EQ (lumex::block_status::progress, ledger.process (transaction, send));
	auto receive = builder
				   .receive ()
				   .previous (send->hash ())
				   .source (send->hash ())
				   .sign (lumex::dev::genesis_key.prv, lumex::dev::genesis_key.pub)
				   .work (*pool.generate (send->hash ()))
				   .build ();
	ASSERT_EQ (lumex::block_status::progress, ledger.process (transaction, receive));
	ASSERT_FALSE (ledger.rollback (transaction, receive->hash ()));
}

TEST (ledger, process_duplicate)
{
	auto ctx = lumex::test::ledger_empty ();
	auto & ledger = ctx.ledger ();
	auto & store = ctx.store ();
	auto transaction = ledger.tx_begin_write ();
	auto & pool = ctx.pool ();
	auto info1 = ledger.any.account_get (transaction, lumex::dev::genesis_key.pub);
	ASSERT_TRUE (info1);
	lumex::keypair key2;
	lumex::block_builder builder;
	auto send = builder
				.send ()
				.previous (info1->head)
				.destination (key2.pub)
				.balance (50)
				.sign (lumex::dev::genesis_key.prv, lumex::dev::genesis_key.pub)
				.work (*pool.generate (info1->head))
				.build ();
	lumex::block_hash hash1 (send->hash ());
	ASSERT_EQ (lumex::block_status::progress, ledger.process (transaction, send));
	ASSERT_EQ (lumex::block_status::old, ledger.process (transaction, send));
	auto open = builder
				.open ()
				.source (hash1)
				.representative (1)
				.account (key2.pub)
				.sign (key2.prv, key2.pub)
				.work (*pool.generate (key2.pub))
				.build ();
	ASSERT_EQ (lumex::block_status::progress, ledger.process (transaction, open));
	ASSERT_EQ (lumex::block_status::old, ledger.process (transaction, open));
}

TEST (ledger, representative_genesis)
{
	auto ctx = lumex::test::ledger_empty ();
	auto & ledger = ctx.ledger ();
	auto & store = ctx.store ();
	auto transaction = ledger.tx_begin_write ();
	auto latest = ledger.any.account_head (transaction, lumex::dev::genesis_key.pub);
	ASSERT_FALSE (latest.is_zero ());
	ASSERT_EQ (lumex::dev::genesis->hash (), ledger.representative_block (transaction, latest));
}

TEST (ledger, weight)
{
	auto ctx = lumex::test::ledger_empty ();
	auto & ledger = ctx.ledger ();
	ASSERT_EQ (lumex::dev::constants.genesis_amount, ledger.weight (lumex::dev::genesis_key.pub));
}

TEST (ledger, representative_change)
{
	auto ctx = lumex::test::ledger_empty ();
	auto & ledger = ctx.ledger ();
	auto & store = ctx.store ();
	auto transaction = ledger.tx_begin_write ();
	lumex::keypair key2;
	auto & pool = ctx.pool ();
	ASSERT_EQ (lumex::dev::constants.genesis_amount, ledger.weight (lumex::dev::genesis_key.pub));
	ASSERT_EQ (0, ledger.weight (key2.pub));
	auto info1 = ledger.any.account_get (transaction, lumex::dev::genesis_key.pub);
	ASSERT_TRUE (info1);
	lumex::block_builder builder;
	auto block = builder
				 .change ()
				 .previous (info1->head)
				 .representative (key2.pub)
				 .sign (lumex::dev::genesis_key.prv, lumex::dev::genesis_key.pub)
				 .work (*pool.generate (info1->head))
				 .build ();
	auto return1 (ledger.process (transaction, block));
	ASSERT_EQ (lumex::block_status::progress, return1);
	ASSERT_EQ (0, ledger.any.block_amount (transaction, block->hash ()).value ().number ());
	ASSERT_EQ (lumex::dev::genesis_key.pub, block->account ());
	ASSERT_EQ (0, ledger.weight (lumex::dev::genesis_key.pub));
	ASSERT_EQ (lumex::dev::constants.genesis_amount, ledger.weight (key2.pub));
	auto info2 = ledger.any.account_get (transaction, lumex::dev::genesis_key.pub);
	ASSERT_TRUE (info2);
	ASSERT_EQ (block->hash (), info2->head);
	ASSERT_FALSE (ledger.rollback (transaction, info2->head));
	auto info3 = ledger.any.account_get (transaction, lumex::dev::genesis_key.pub);
	ASSERT_TRUE (info3);
	ASSERT_EQ (info1->head, info3->head);
	ASSERT_EQ (lumex::dev::constants.genesis_amount, ledger.weight (lumex::dev::genesis_key.pub));
	ASSERT_EQ (0, ledger.weight (key2.pub));
}

TEST (ledger, send_fork)
{
	auto ctx = lumex::test::ledger_empty ();
	auto & ledger = ctx.ledger ();
	auto & store = ctx.store ();
	lumex::keypair key2;
	lumex::keypair key3;
	auto transaction = ledger.tx_begin_write ();
	auto & pool = ctx.pool ();
	auto info1 = ledger.any.account_get (transaction, lumex::dev::genesis_key.pub);
	ASSERT_TRUE (info1);
	lumex::block_builder builder;
	auto block = builder
				 .send ()
				 .previous (info1->head)
				 .destination (key2.pub)
				 .balance (100)
				 .sign (lumex::dev::genesis_key.prv, lumex::dev::genesis_key.pub)
				 .work (*pool.generate (info1->head))
				 .build ();
	ASSERT_EQ (lumex::block_status::progress, ledger.process (transaction, block));
	auto block2 = builder
				  .send ()
				  .previous (info1->head)
				  .destination (key3.pub)
				  .balance (0)
				  .sign (lumex::dev::genesis_key.prv, lumex::dev::genesis_key.pub)
				  .work (*pool.generate (info1->head))
				  .build ();
	ASSERT_EQ (lumex::block_status::fork, ledger.process (transaction, block2));
}

TEST (ledger, receive_fork)
{
	auto ctx = lumex::test::ledger_empty ();
	auto & ledger = ctx.ledger ();
	auto & store = ctx.store ();
	lumex::keypair key2;
	lumex::keypair key3;
	auto transaction = ledger.tx_begin_write ();
	auto & pool = ctx.pool ();
	auto info1 = ledger.any.account_get (transaction, lumex::dev::genesis_key.pub);
	ASSERT_TRUE (info1);
	lumex::block_builder builder;
	auto block = builder
				 .send ()
				 .previous (info1->head)
				 .destination (key2.pub)
				 .balance (100)
				 .sign (lumex::dev::genesis_key.prv, lumex::dev::genesis_key.pub)
				 .work (*pool.generate (info1->head))
				 .build ();
	ASSERT_EQ (lumex::block_status::progress, ledger.process (transaction, block));
	auto block2 = builder
				  .open ()
				  .source (block->hash ())
				  .representative (key2.pub)
				  .account (key2.pub)
				  .sign (key2.prv, key2.pub)
				  .work (*pool.generate (key2.pub))
				  .build ();
	ASSERT_EQ (lumex::block_status::progress, ledger.process (transaction, block2));
	auto block3 = builder
				  .change ()
				  .previous (block2->hash ())
				  .representative (key3.pub)
				  .sign (key2.prv, key2.pub)
				  .work (*pool.generate (block2->hash ()))
				  .build ();
	ASSERT_EQ (lumex::block_status::progress, ledger.process (transaction, block3));
	auto block4 = builder
				  .send ()
				  .previous (block->hash ())
				  .destination (key2.pub)
				  .balance (0)
				  .sign (lumex::dev::genesis_key.prv, lumex::dev::genesis_key.pub)
				  .work (*pool.generate (block->hash ()))
				  .build ();
	ASSERT_EQ (lumex::block_status::progress, ledger.process (transaction, block4));
	auto block5 = builder
				  .receive ()
				  .previous (block2->hash ())
				  .source (block4->hash ())
				  .sign (key2.prv, key2.pub)
				  .work (*pool.generate (block2->hash ()))
				  .build ();
	ASSERT_EQ (lumex::block_status::fork, ledger.process (transaction, block5));
}

TEST (ledger, open_fork)
{
	auto ctx = lumex::test::ledger_empty ();
	auto & ledger = ctx.ledger ();
	auto & store = ctx.store ();
	lumex::keypair key2;
	lumex::keypair key3;
	auto transaction = ledger.tx_begin_write ();
	auto & pool = ctx.pool ();
	auto info1 = ledger.any.account_get (transaction, lumex::dev::genesis_key.pub);
	ASSERT_TRUE (info1);
	lumex::block_builder builder;
	auto block = builder
				 .send ()
				 .previous (info1->head)
				 .destination (key2.pub)
				 .balance (100)
				 .sign (lumex::dev::genesis_key.prv, lumex::dev::genesis_key.pub)
				 .work (*pool.generate (info1->head))
				 .build ();
	ASSERT_EQ (lumex::block_status::progress, ledger.process (transaction, block));
	auto block2 = builder
				  .open ()
				  .source (block->hash ())
				  .representative (key2.pub)
				  .account (key2.pub)
				  .sign (key2.prv, key2.pub)
				  .work (*pool.generate (key2.pub))
				  .build ();
	ASSERT_EQ (lumex::block_status::progress, ledger.process (transaction, block2));
	auto block3 = builder
				  .open ()
				  .source (block->hash ())
				  .representative (key3.pub)
				  .account (key2.pub)
				  .sign (key2.prv, key2.pub)
				  .work (*pool.generate (key2.pub))
				  .build ();
	ASSERT_EQ (lumex::block_status::fork, ledger.process (transaction, block3));
}

TEST (ledger, representation_changes)
{
	auto store{ lumex::test::make_store () };
	lumex::keypair key1;
	lumex::rep_weights rep_weights{ store->rep_weight };
	ASSERT_EQ (0, rep_weights.get (key1.pub));
	rep_weights.put (key1.pub, 1);
	ASSERT_EQ (1, rep_weights.get (key1.pub));
	rep_weights.put (key1.pub, 2);
	ASSERT_EQ (2, rep_weights.get (key1.pub));
}

TEST (ledger, delete_rep_weight_of_zero)
{
	auto store{ lumex::test::make_store () };
	lumex::rep_weights rep_weights{ store->rep_weight };
	auto txn{ store->tx_begin_write () };
	rep_weights.add (txn, 1, 100);
	rep_weights.add (txn, 2, 200);
	ASSERT_EQ (2, rep_weights.size ());
	rep_weights.move_add_sub (txn, 2, 100, 3, 120);
	ASSERT_EQ (3, rep_weights.size ());
	ASSERT_EQ (3, store->rep_weight.count (txn));

	// Reduce rep weights to 0
	rep_weights.sub (txn, 1, 100);
	ASSERT_EQ (2, rep_weights.size ());
	ASSERT_EQ (2, store->rep_weight.count (txn));

	rep_weights.move_add_sub (txn, 3, 120, 2, 100);
	ASSERT_EQ (1, rep_weights.size ());
	ASSERT_EQ (1, store->rep_weight.count (txn));

	rep_weights.sub (txn, 2, 200);
	ASSERT_EQ (0, rep_weights.size ());
	ASSERT_EQ (0, store->rep_weight.count (txn));
}

TEST (ledger, rep_cache_min_weight)
{
	auto store{ lumex::test::make_store () };
	lumex::uint128_t min_weight{ 10 };
	lumex::rep_weights rep_weights{ store->rep_weight, min_weight };
	auto txn{ store->tx_begin_write () };

	// one below min weight
	rep_weights.add (txn, 1, 9);
	ASSERT_EQ (0, rep_weights.size ());
	ASSERT_EQ (1, store->rep_weight.count (txn));

	// exactly min weight
	rep_weights.add (txn, 1, 1);
	ASSERT_EQ (1, rep_weights.size ());
	ASSERT_EQ (1, store->rep_weight.count (txn));

	// above min weight
	rep_weights.add (txn, 1, 1);
	ASSERT_EQ (1, rep_weights.size ());
	ASSERT_EQ (1, store->rep_weight.count (txn));

	// fall blow min weight
	rep_weights.sub (txn, 1, 5);
	ASSERT_EQ (0, rep_weights.size ());
	ASSERT_EQ (1, store->rep_weight.count (txn));
}

TEST (ledger, representation)
{
	auto ctx = lumex::test::ledger_empty ();
	auto & ledger = ctx.ledger ();
	auto & store = ctx.store ();
	auto & rep_weights = ledger.rep_weights;
	auto transaction = ledger.tx_begin_write ();
	auto & pool = ctx.pool ();
	ASSERT_EQ (lumex::dev::constants.genesis_amount, rep_weights.get (lumex::dev::genesis_key.pub));
	lumex::keypair key2;
	lumex::block_builder builder;
	auto block1 = builder
				  .send ()
				  .previous (lumex::dev::genesis->hash ())
				  .destination (key2.pub)
				  .balance (lumex::dev::constants.genesis_amount - 100)
				  .sign (lumex::dev::genesis_key.prv, lumex::dev::genesis_key.pub)
				  .work (*pool.generate (lumex::dev::genesis->hash ()))
				  .build ();
	ASSERT_EQ (lumex::block_status::progress, ledger.process (transaction, block1));
	ASSERT_EQ (lumex::dev::constants.genesis_amount - 100, rep_weights.get (lumex::dev::genesis_key.pub));
	lumex::keypair key3;
	auto block2 = builder
				  .open ()
				  .source (block1->hash ())
				  .representative (key3.pub)
				  .account (key2.pub)
				  .sign (key2.prv, key2.pub)
				  .work (*pool.generate (key2.pub))
				  .build ();
	ASSERT_EQ (lumex::block_status::progress, ledger.process (transaction, block2));
	ASSERT_EQ (lumex::dev::constants.genesis_amount - 100, rep_weights.get (lumex::dev::genesis_key.pub));
	ASSERT_EQ (0, rep_weights.get (key2.pub));
	ASSERT_EQ (100, rep_weights.get (key3.pub));
	auto block3 = builder
				  .send ()
				  .previous (block1->hash ())
				  .destination (key2.pub)
				  .balance (lumex::dev::constants.genesis_amount - 200)
				  .sign (lumex::dev::genesis_key.prv, lumex::dev::genesis_key.pub)
				  .work (*pool.generate (block1->hash ()))
				  .build ();
	ASSERT_EQ (lumex::block_status::progress, ledger.process (transaction, block3));
	ASSERT_EQ (lumex::dev::constants.genesis_amount - 200, rep_weights.get (lumex::dev::genesis_key.pub));
	ASSERT_EQ (0, rep_weights.get (key2.pub));
	ASSERT_EQ (100, rep_weights.get (key3.pub));
	auto block4 = builder
				  .receive ()
				  .previous (block2->hash ())
				  .source (block3->hash ())
				  .sign (key2.prv, key2.pub)
				  .work (*pool.generate (block2->hash ()))
				  .build ();
	ASSERT_EQ (lumex::block_status::progress, ledger.process (transaction, block4));
	ASSERT_EQ (lumex::dev::constants.genesis_amount - 200, rep_weights.get (lumex::dev::genesis_key.pub));
	ASSERT_EQ (0, rep_weights.get (key2.pub));
	ASSERT_EQ (200, rep_weights.get (key3.pub));
	lumex::keypair key4;
	auto block5 = builder
				  .change ()
				  .previous (block4->hash ())
				  .representative (key4.pub)
				  .sign (key2.prv, key2.pub)
				  .work (*pool.generate (block4->hash ()))
				  .build ();
	ASSERT_EQ (lumex::block_status::progress, ledger.process (transaction, block5));
	ASSERT_EQ (lumex::dev::constants.genesis_amount - 200, rep_weights.get (lumex::dev::genesis_key.pub));
	ASSERT_EQ (0, rep_weights.get (key2.pub));
	ASSERT_EQ (0, rep_weights.get (key3.pub));
	ASSERT_EQ (200, rep_weights.get (key4.pub));
	lumex::keypair key5;
	auto block6 = builder
				  .send ()
				  .previous (block5->hash ())
				  .destination (key5.pub)
				  .balance (100)
				  .sign (key2.prv, key2.pub)
				  .work (*pool.generate (block5->hash ()))
				  .build ();
	ASSERT_EQ (lumex::block_status::progress, ledger.process (transaction, block6));
	ASSERT_EQ (lumex::dev::constants.genesis_amount - 200, rep_weights.get (lumex::dev::genesis_key.pub));
	ASSERT_EQ (0, rep_weights.get (key2.pub));
	ASSERT_EQ (0, rep_weights.get (key3.pub));
	ASSERT_EQ (100, rep_weights.get (key4.pub));
	ASSERT_EQ (0, rep_weights.get (key5.pub));
	lumex::keypair key6;
	auto block7 = builder
				  .open ()
				  .source (block6->hash ())
				  .representative (key6.pub)
				  .account (key5.pub)
				  .sign (key5.prv, key5.pub)
				  .work (*pool.generate (key5.pub))
				  .build ();
	ASSERT_EQ (lumex::block_status::progress, ledger.process (transaction, block7));
	ASSERT_EQ (lumex::dev::constants.genesis_amount - 200, rep_weights.get (lumex::dev::genesis_key.pub));
	ASSERT_EQ (0, rep_weights.get (key2.pub));
	ASSERT_EQ (0, rep_weights.get (key3.pub));
	ASSERT_EQ (100, rep_weights.get (key4.pub));
	ASSERT_EQ (0, rep_weights.get (key5.pub));
	ASSERT_EQ (100, rep_weights.get (key6.pub));
	auto block8 = builder
				  .send ()
				  .previous (block6->hash ())
				  .destination (key5.pub)
				  .balance (0)
				  .sign (key2.prv, key2.pub)
				  .work (*pool.generate (block6->hash ()))
				  .build ();
	ASSERT_EQ (lumex::block_status::progress, ledger.process (transaction, block8));
	ASSERT_EQ (lumex::dev::constants.genesis_amount - 200, rep_weights.get (lumex::dev::genesis_key.pub));
	ASSERT_EQ (0, rep_weights.get (key2.pub));
	ASSERT_EQ (0, rep_weights.get (key3.pub));
	ASSERT_EQ (0, rep_weights.get (key4.pub));
	ASSERT_EQ (0, rep_weights.get (key5.pub));
	ASSERT_EQ (100, rep_weights.get (key6.pub));
	auto block9 = builder
				  .receive ()
				  .previous (block7->hash ())
				  .source (block8->hash ())
				  .sign (key5.prv, key5.pub)
				  .work (*pool.generate (block7->hash ()))
				  .build ();
	ASSERT_EQ (lumex::block_status::progress, ledger.process (transaction, block9));
	ASSERT_EQ (lumex::dev::constants.genesis_amount - 200, rep_weights.get (lumex::dev::genesis_key.pub));
	ASSERT_EQ (0, rep_weights.get (key2.pub));
	ASSERT_EQ (0, rep_weights.get (key3.pub));
	ASSERT_EQ (0, rep_weights.get (key4.pub));
	ASSERT_EQ (0, rep_weights.get (key5.pub));
	ASSERT_EQ (200, rep_weights.get (key6.pub));
}

TEST (ledger, double_open)
{
	lumex::logger logger;
	lumex::stats stats{ logger };
	auto store = lumex::test::make_store (logger, stats);
	lumex::ledger ledger (*store, lumex::dev::network_params, stats, logger);
	auto transaction = ledger.tx_begin_write ();
	lumex::work_pool pool{ lumex::dev::network_params.network, std::numeric_limits<unsigned>::max () };
	lumex::keypair key2;
	lumex::block_builder builder;
	auto send1 = builder
				 .send ()
				 .previous (lumex::dev::genesis->hash ())
				 .destination (key2.pub)
				 .balance (1)
				 .sign (lumex::dev::genesis_key.prv, lumex::dev::genesis_key.pub)
				 .work (*pool.generate (lumex::dev::genesis->hash ()))
				 .build ();
	ASSERT_EQ (lumex::block_status::progress, ledger.process (transaction, send1));
	auto open1 = builder
				 .open ()
				 .source (send1->hash ())
				 .representative (key2.pub)
				 .account (key2.pub)
				 .sign (key2.prv, key2.pub)
				 .work (*pool.generate (key2.pub))
				 .build ();
	ASSERT_EQ (lumex::block_status::progress, ledger.process (transaction, open1));
	auto open2 = builder
				 .open ()
				 .source (send1->hash ())
				 .representative (lumex::dev::genesis_key.pub)
				 .account (key2.pub)
				 .sign (key2.prv, key2.pub)
				 .work (*pool.generate (key2.pub))
				 .build ();
	ASSERT_EQ (lumex::block_status::fork, ledger.process (transaction, open2));
}

TEST (ledger, double_receive)
{
	auto ctx = lumex::test::ledger_empty ();
	auto & ledger = ctx.ledger ();
	auto & store = ctx.store ();
	auto transaction = ledger.tx_begin_write ();
	auto & pool = ctx.pool ();
	lumex::keypair key2;
	lumex::block_builder builder;
	auto send1 = builder
				 .send ()
				 .previous (lumex::dev::genesis->hash ())
				 .destination (key2.pub)
				 .balance (1)
				 .sign (lumex::dev::genesis_key.prv, lumex::dev::genesis_key.pub)
				 .work (*pool.generate (lumex::dev::genesis->hash ()))
				 .build ();
	ASSERT_EQ (lumex::block_status::progress, ledger.process (transaction, send1));
	auto open1 = builder
				 .open ()
				 .source (send1->hash ())
				 .representative (key2.pub)
				 .account (key2.pub)
				 .sign (key2.prv, key2.pub)
				 .work (*pool.generate (key2.pub))
				 .build ();
	ASSERT_EQ (lumex::block_status::progress, ledger.process (transaction, open1));
	auto receive1 = builder
					.receive ()
					.previous (open1->hash ())
					.source (send1->hash ())
					.sign (key2.prv, key2.pub)
					.work (*pool.generate (open1->hash ()))
					.build ();
	ASSERT_EQ (lumex::block_status::unreceivable, ledger.process (transaction, receive1));
}

TEST (votes, check_signature)
{
	lumex::test::system system;
	lumex::node_config node_config = system.default_config ();
	node_config.online_weight_minimum = std::numeric_limits<lumex::uint128_t>::max ();
	auto & node1 = *system.add_node (node_config);
	lumex::keypair key1;
	lumex::block_builder builder;
	auto send1 = builder
				 .send ()
				 .previous (lumex::dev::genesis->hash ())
				 .destination (key1.pub)
				 .balance (lumex::dev::constants.genesis_amount - 100)
				 .sign (lumex::dev::genesis_key.prv, lumex::dev::genesis_key.pub)
				 .work (0)
				 .build ();
	node1.work_generate_blocking (*send1);
	{
		auto transaction = node1.ledger.tx_begin_write ();
		ASSERT_EQ (lumex::block_status::progress, node1.ledger.process (transaction, send1));
	}
	auto election1 = lumex::test::start_election (system, node1, send1->hash ());
	ASSERT_EQ (1, election1->votes ().size ());
	auto vote1 = lumex::test::make_vote (lumex::dev::genesis_key, { send1 }, lumex::vote::timestamp_min * 1, 0);
	vote1->signature.bytes[0] ^= 1;
	ASSERT_EQ (lumex::vote_code::invalid, node1.vote_processor.vote_blocking (vote1, std::make_shared<lumex::transport::inproc::channel> (node1, node1)));
	vote1->signature.bytes[0] ^= 1;
	ASSERT_EQ (lumex::vote_code::vote, node1.vote_processor.vote_blocking (vote1, std::make_shared<lumex::transport::inproc::channel> (node1, node1)));
	ASSERT_EQ (lumex::vote_code::replay, node1.vote_processor.vote_blocking (vote1, std::make_shared<lumex::transport::inproc::channel> (node1, node1)));
}

TEST (votes, add_one)
{
	lumex::test::system system (1);
	auto & node1 (*system.nodes[0]);
	lumex::keypair key1;
	lumex::block_builder builder;
	auto send1 = builder
				 .send ()
				 .previous (lumex::dev::genesis->hash ())
				 .destination (key1.pub)
				 .balance (lumex::dev::constants.genesis_amount - 100)
				 .sign (lumex::dev::genesis_key.prv, lumex::dev::genesis_key.pub)
				 .work (0)
				 .build ();
	node1.work_generate_blocking (*send1);
	auto transaction = node1.ledger.tx_begin_write ();
	ASSERT_EQ (lumex::block_status::progress, node1.ledger.process (transaction, send1));
	node1.start_election (send1);
	ASSERT_TIMELY (5s, node1.active.election (send1->qualified_root ()));
	auto election1 = node1.active.election (send1->qualified_root ());
	ASSERT_EQ (1, election1->votes ().size ());
	auto vote1 = lumex::test::make_vote (lumex::dev::genesis_key, { send1 }, lumex::vote::timestamp_min * 1, 0);
	ASSERT_EQ (lumex::vote_code::vote, node1.vote_router.vote (vote1).at (send1->hash ()));
	auto vote2 = lumex::test::make_vote (lumex::dev::genesis_key, { send1 }, lumex::vote::timestamp_min * 2, 0);
	ASSERT_EQ (lumex::vote_code::ignored, node1.vote_router.vote (vote2).at (send1->hash ())); // Ignored due to vote cooldown
	ASSERT_EQ (2, election1->votes ().size ());
	auto votes1 (election1->votes ());
	auto existing1 (votes1.find (lumex::dev::genesis_key.pub));
	ASSERT_NE (votes1.end (), existing1);
	ASSERT_EQ (send1->hash (), existing1->second.hash);
	auto winner (*election1->tally ().begin ());
	ASSERT_EQ (*send1, *winner.second);
	ASSERT_EQ (lumex::dev::constants.genesis_amount - 100, winner.first);
}

namespace lumex
{
// Higher timestamps change the vote
TEST (votes, add_existing)
{
	lumex::test::system system;
	lumex::node_config node_config = system.default_config ();
	node_config.online_weight_minimum = lumex::dev::constants.genesis_amount;
	node_config.backlog_scan->enable = false;
	auto & node1 = *system.add_node (node_config);
	lumex::keypair key1;
	lumex::block_builder builder;
	std::shared_ptr<lumex::block> send1 = builder.state ()
										 .account (lumex::dev::genesis_key.pub)
										 .previous (lumex::dev::genesis->hash ())
										 .representative (lumex::dev::genesis_key.pub) // No representative, blocks can't confirm
										 .balance (lumex::dev::constants.genesis_amount / 2 - lumex::Klumex_ratio)
										 .link (key1.pub)
										 .work (0)
										 .sign (lumex::dev::genesis_key.prv, lumex::dev::genesis_key.pub)
										 .build ();
	node1.work_generate_blocking (*send1);
	ASSERT_EQ (lumex::block_status::progress, node1.ledger.process (node1.ledger.tx_begin_write (), send1));
	auto election1 = lumex::test::start_election (system, node1, send1->hash ());
	auto vote1 = lumex::test::make_vote (lumex::dev::genesis_key, { send1 }, lumex::vote::timestamp_min * 1, 0);
	ASSERT_EQ (lumex::vote_code::vote, node1.vote_router.vote (vote1).at (send1->hash ()));
	// Block is already processed from vote
	ASSERT_TRUE (node1.active.publish (send1));
	ASSERT_EQ (lumex::vote::timestamp_min * 1, election1->last_votes[lumex::dev::genesis_key.pub].timestamp);
	lumex::keypair key2;
	std::shared_ptr<lumex::block> send2 = builder.state ()
										 .account (lumex::dev::genesis_key.pub)
										 .previous (lumex::dev::genesis->hash ())
										 .representative (lumex::dev::genesis_key.pub) // No representative, blocks can't confirm
										 .balance (lumex::dev::constants.genesis_amount / 2 - lumex::Klumex_ratio)
										 .link (key2.pub)
										 .work (0)
										 .sign (lumex::dev::genesis_key.prv, lumex::dev::genesis_key.pub)
										 .build ();
	node1.work_generate_blocking (*send2);
	ASSERT_FALSE (node1.active.publish (send2));
	ASSERT_TIMELY (5s, node1.active.active (*send2));
	auto vote2 = lumex::test::make_vote (lumex::dev::genesis_key, { send2 }, lumex::vote::timestamp_min * 2, 0);
	// Pretend we've waited the timeout
	auto vote_info1 = election1->get_last_vote (lumex::dev::genesis_key.pub);
	vote_info1.time = std::chrono::steady_clock::now () - std::chrono::seconds (20);
	election1->set_last_vote (lumex::dev::genesis_key.pub, vote_info1);
	ASSERT_EQ (lumex::vote_code::vote, node1.vote_router.vote (vote2).at (send2->hash ()));
	ASSERT_EQ (lumex::vote::timestamp_min * 2, election1->last_votes[lumex::dev::genesis_key.pub].timestamp);
	// Also resend the old vote, and see if we respect the timestamp
	auto vote_info2 = election1->get_last_vote (lumex::dev::genesis_key.pub);
	vote_info2.time = std::chrono::steady_clock::now () - std::chrono::seconds (20);
	election1->set_last_vote (lumex::dev::genesis_key.pub, vote_info2);
	ASSERT_EQ (lumex::vote_code::replay, node1.vote_router.vote (vote1).at (send1->hash ()));
	ASSERT_EQ (lumex::vote::timestamp_min * 2, election1->votes ()[lumex::dev::genesis_key.pub].timestamp);
	auto votes (election1->votes ());
	ASSERT_EQ (2, votes.size ());
	ASSERT_NE (votes.end (), votes.find (lumex::dev::genesis_key.pub));
	ASSERT_EQ (send2->hash (), votes[lumex::dev::genesis_key.pub].hash);
	ASSERT_EQ (*send2, *election1->tally ().begin ()->second);
}

// Lower timestamps are ignored
TEST (votes, add_old)
{
	lumex::test::system system (1);
	auto & node1 (*system.nodes[0]);
	lumex::keypair key1;
	lumex::block_builder builder;
	auto send1 = builder
				 .send ()
				 .previous (lumex::dev::genesis->hash ())
				 .destination (key1.pub)
				 .balance (0)
				 .sign (lumex::dev::genesis_key.prv, lumex::dev::genesis_key.pub)
				 .work (0)
				 .build ();
	node1.work_generate_blocking (*send1);
	auto transaction = node1.ledger.tx_begin_write ();
	ASSERT_EQ (lumex::block_status::progress, node1.ledger.process (transaction, send1));
	node1.start_election (send1);
	ASSERT_TIMELY (5s, node1.active.election (send1->qualified_root ()));
	auto election1 = node1.active.election (send1->qualified_root ());
	auto vote1 = lumex::test::make_vote (lumex::dev::genesis_key, { send1 }, lumex::vote::timestamp_min * 2, 0);
	auto channel (std::make_shared<lumex::transport::inproc::channel> (node1, node1));
	node1.vote_processor.vote_blocking (vote1, channel);
	lumex::keypair key2;
	auto send2 = builder
				 .send ()
				 .previous (lumex::dev::genesis->hash ())
				 .destination (key2.pub)
				 .balance (0)
				 .sign (lumex::dev::genesis_key.prv, lumex::dev::genesis_key.pub)
				 .work (0)
				 .build ();
	node1.work_generate_blocking (*send2);
	auto vote2 = std::make_shared<lumex::vote> (lumex::dev::genesis_key.pub, lumex::dev::genesis_key.prv, lumex::vote::timestamp_min * 1, 0, std::vector<lumex::block_hash>{ send2->hash () });
	auto vote_info = election1->get_last_vote (lumex::dev::genesis_key.pub);
	vote_info.time = std::chrono::steady_clock::now () - std::chrono::seconds (20);
	election1->set_last_vote (lumex::dev::genesis_key.pub, vote_info);
	node1.vote_processor.vote_blocking (vote2, channel);
	ASSERT_EQ (2, election1->votes ().size ());
	auto votes (election1->votes ());
	ASSERT_NE (votes.end (), votes.find (lumex::dev::genesis_key.pub));
	ASSERT_EQ (send1->hash (), votes[lumex::dev::genesis_key.pub].hash);
	ASSERT_EQ (*send1, *election1->winner ());
}
}

// Lower timestamps are accepted for different accounts
// Test disabled because it's failing intermittently.
// PR in which it got disabled: https://github.com/lumexcurrency/lumex-node/pull/3629
// Issue for investigating it: https://github.com/lumexcurrency/lumex-node/issues/3631
TEST (votes, DISABLED_add_old_different_account)
{
	lumex::test::system system (1);
	auto & node1 (*system.nodes[0]);
	lumex::keypair key1;
	lumex::block_builder builder;
	auto send1 = builder
				 .send ()
				 .previous (lumex::dev::genesis->hash ())
				 .destination (key1.pub)
				 .balance (0)
				 .sign (lumex::dev::genesis_key.prv, lumex::dev::genesis_key.pub)
				 .work (0)
				 .build ();
	node1.work_generate_blocking (*send1);
	auto send2 = builder
				 .send ()
				 .previous (send1->hash ())
				 .destination (key1.pub)
				 .balance (0)
				 .sign (lumex::dev::genesis_key.prv, lumex::dev::genesis_key.pub)
				 .work (0)
				 .build ();
	node1.work_generate_blocking (*send2);
	ASSERT_EQ (lumex::block_status::progress, node1.process (send1));
	ASSERT_EQ (lumex::block_status::progress, node1.process (send2));
	ASSERT_TRUE (lumex::test::start_elections (system, node1, { send1, send2 }));
	auto election1 = node1.active.election (send1->qualified_root ());
	ASSERT_NE (nullptr, election1);
	auto election2 = node1.active.election (send2->qualified_root ());
	ASSERT_NE (nullptr, election2);
	ASSERT_EQ (1, election1->votes ().size ());
	ASSERT_EQ (1, election2->votes ().size ());
	auto vote1 = lumex::test::make_vote (lumex::dev::genesis_key, { send1 }, lumex::vote::timestamp_min * 2, 0);
	auto channel (std::make_shared<lumex::transport::inproc::channel> (node1, node1));
	auto vote_result1 (node1.vote_processor.vote_blocking (vote1, channel));
	ASSERT_EQ (lumex::vote_code::vote, vote_result1);
	ASSERT_EQ (2, election1->votes ().size ());
	ASSERT_EQ (1, election2->votes ().size ());
	auto vote2 = lumex::test::make_vote (lumex::dev::genesis_key, { send2 }, lumex::vote::timestamp_min * 1, 0);
	auto vote_result2 (node1.vote_processor.vote_blocking (vote2, channel));
	ASSERT_EQ (lumex::vote_code::vote, vote_result2);
	ASSERT_EQ (2, election1->votes ().size ());
	ASSERT_EQ (2, election2->votes ().size ());
	auto votes1 (election1->votes ());
	auto votes2 (election2->votes ());
	ASSERT_NE (votes1.end (), votes1.find (lumex::dev::genesis_key.pub));
	ASSERT_NE (votes2.end (), votes2.find (lumex::dev::genesis_key.pub));
	ASSERT_EQ (send1->hash (), votes1[lumex::dev::genesis_key.pub].hash);
	ASSERT_EQ (send2->hash (), votes2[lumex::dev::genesis_key.pub].hash);
	ASSERT_EQ (*send1, *election1->winner ());
	ASSERT_EQ (*send2, *election2->winner ());
}

// The voting cooldown is respected
TEST (votes, add_cooldown)
{
	lumex::test::system system (1);
	auto & node1 (*system.nodes[0]);
	lumex::keypair key1;
	lumex::block_builder builder;
	auto send1 = builder
				 .send ()
				 .previous (lumex::dev::genesis->hash ())
				 .destination (key1.pub)
				 .balance (0)
				 .sign (lumex::dev::genesis_key.prv, lumex::dev::genesis_key.pub)
				 .work (0)
				 .build ();
	node1.work_generate_blocking (*send1);
	auto transaction = node1.ledger.tx_begin_write ();
	ASSERT_EQ (lumex::block_status::progress, node1.ledger.process (transaction, send1));
	node1.start_election (send1);
	ASSERT_TIMELY (5s, node1.active.election (send1->qualified_root ()));
	auto election1 = node1.active.election (send1->qualified_root ());
	auto vote1 = lumex::test::make_vote (lumex::dev::genesis_key, { send1 }, lumex::vote::timestamp_min * 1, 0);
	auto channel (std::make_shared<lumex::transport::inproc::channel> (node1, node1));
	node1.vote_processor.vote_blocking (vote1, channel);
	lumex::keypair key2;
	auto send2 = builder
				 .send ()
				 .previous (lumex::dev::genesis->hash ())
				 .destination (key2.pub)
				 .balance (0)
				 .sign (lumex::dev::genesis_key.prv, lumex::dev::genesis_key.pub)
				 .work (0)
				 .build ();
	node1.work_generate_blocking (*send2);
	auto vote2 = lumex::test::make_vote (lumex::dev::genesis_key, { send2 }, lumex::vote::timestamp_min * 2, 0);
	node1.vote_processor.vote_blocking (vote2, channel);
	ASSERT_EQ (2, election1->votes ().size ());
	auto votes (election1->votes ());
	ASSERT_NE (votes.end (), votes.find (lumex::dev::genesis_key.pub));
	ASSERT_EQ (send1->hash (), votes[lumex::dev::genesis_key.pub].hash);
	ASSERT_EQ (*send1, *election1->winner ());
}

// Query for block successor
TEST (ledger, successor)
{
	lumex::test::system system (1);
	auto & node1 (*system.nodes[0]);
	lumex::keypair key1;
	lumex::block_builder builder;
	auto send1 = builder
				 .send ()
				 .previous (lumex::dev::genesis->hash ())
				 .destination (key1.pub)
				 .balance (0)
				 .sign (lumex::dev::genesis_key.prv, lumex::dev::genesis_key.pub)
				 .work (0)
				 .build ();
	node1.work_generate_blocking (*send1);
	auto transaction = node1.ledger.tx_begin_write ();
	ASSERT_EQ (lumex::block_status::progress, node1.ledger.process (transaction, send1));
	ASSERT_EQ (*send1, *node1.ledger.any.block_get (transaction, node1.ledger.any.block_successor (transaction, lumex::qualified_root (lumex::root (0), lumex::dev::genesis->hash ())).value ()));
	ASSERT_EQ (*lumex::dev::genesis, *node1.ledger.any.block_get (transaction, node1.ledger.any.block_successor (transaction, lumex::dev::genesis->qualified_root ()).value ()));
	ASSERT_FALSE (node1.ledger.any.block_successor (transaction, lumex::qualified_root (0)));
}

TEST (ledger, fail_change_old)
{
	auto ctx = lumex::test::ledger_empty ();
	auto & ledger = ctx.ledger ();
	auto & store = ctx.store ();
	auto transaction = ledger.tx_begin_write ();
	auto & pool = ctx.pool ();
	lumex::keypair key1;
	lumex::block_builder builder;
	auto block = builder
				 .change ()
				 .previous (lumex::dev::genesis->hash ())
				 .representative (key1.pub)
				 .sign (lumex::dev::genesis_key.prv, lumex::dev::genesis_key.pub)
				 .work (*pool.generate (lumex::dev::genesis->hash ()))
				 .build ();
	auto result1 = ledger.process (transaction, block);
	ASSERT_EQ (lumex::block_status::progress, result1);
	auto result2 = ledger.process (transaction, block);
	ASSERT_EQ (lumex::block_status::old, result2);
}

TEST (ledger, fail_change_gap_previous)
{
	auto ctx = lumex::test::ledger_empty ();
	auto & ledger = ctx.ledger ();
	auto & store = ctx.store ();
	auto transaction = ledger.tx_begin_write ();
	auto & pool = ctx.pool ();
	lumex::keypair key1;
	lumex::block_builder builder;
	auto block = builder
				 .change ()
				 .previous (1)
				 .representative (key1.pub)
				 .sign (lumex::dev::genesis_key.prv, lumex::dev::genesis_key.pub)
				 .work (*pool.generate (lumex::root (1)))
				 .build ();
	auto result1 = ledger.process (transaction, block);
	ASSERT_EQ (lumex::block_status::gap_previous, result1);
}

TEST (ledger, fail_state_bad_signature)
{
	auto ctx = lumex::test::ledger_empty ();
	auto & ledger = ctx.ledger ();
	auto & store = ctx.store ();
	auto transaction = ledger.tx_begin_write ();
	auto & pool = ctx.pool ();
	lumex::block_builder builder;
	auto block = builder
				 .state ()
				 .account (lumex::dev::genesis_key.pub)
				 .previous (lumex::dev::genesis->hash ())
				 .representative (lumex::dev::genesis_key.pub)
				 .balance (0)
				 .link (lumex::dev::genesis_key.pub)
				 .sign (lumex::keypair ().prv, 0)
				 .work (*pool.generate (lumex::dev::genesis->hash ()))
				 .build ();
	auto result1 = ledger.process (transaction, block);
	ASSERT_EQ (lumex::block_status::bad_signature, result1);
}

TEST (ledger, fail_epoch_bad_signature)
{
	auto ctx = lumex::test::ledger_empty ();
	auto & ledger = ctx.ledger ();
	auto & store = ctx.store ();
	auto transaction = ledger.tx_begin_write ();
	auto & pool = ctx.pool ();
	lumex::block_builder builder;
	auto block = builder
				 .state ()
				 .account (lumex::dev::genesis_key.pub)
				 .previous (lumex::dev::genesis->hash ())
				 .representative (lumex::dev::genesis_key.pub)
				 .balance (lumex::dev::constants.genesis_amount)
				 .link (ledger.epoch_link (lumex::epoch::epoch_1))
				 .sign (lumex::dev::genesis_key.prv, lumex::dev::genesis_key.pub)
				 .work (*pool.generate (lumex::dev::genesis->hash ()))
				 .build ();
	block->signature.bytes[0] ^= 1;
	auto result1 = ledger.process (transaction, block);
	ASSERT_EQ (lumex::block_status::bad_signature, result1); // Fails epoch signature
	block->signature.bytes[0] ^= 1;
	auto result2 = ledger.process (transaction, block);
	ASSERT_EQ (lumex::block_status::progress, result2); // Succeeds with epoch signature
}

TEST (ledger, fail_change_bad_signature)
{
	auto ctx = lumex::test::ledger_empty ();
	auto & ledger = ctx.ledger ();
	auto & store = ctx.store ();
	auto transaction = ledger.tx_begin_write ();
	auto & pool = ctx.pool ();
	lumex::keypair key1;
	lumex::block_builder builder;
	auto block = builder
				 .change ()
				 .previous (lumex::dev::genesis->hash ())
				 .representative (key1.pub)
				 .sign (lumex::keypair ().prv, 0)
				 .work (*pool.generate (lumex::dev::genesis->hash ()))
				 .build ();
	auto result1 = ledger.process (transaction, block);
	ASSERT_EQ (lumex::block_status::bad_signature, result1);
}

TEST (ledger, fail_change_fork)
{
	auto ctx = lumex::test::ledger_empty ();
	auto & ledger = ctx.ledger ();
	auto & store = ctx.store ();
	auto transaction = ledger.tx_begin_write ();
	auto & pool = ctx.pool ();
	lumex::keypair key1;
	lumex::block_builder builder;
	auto block1 = builder
				  .change ()
				  .previous (lumex::dev::genesis->hash ())
				  .representative (key1.pub)
				  .sign (lumex::dev::genesis_key.prv, lumex::dev::genesis_key.pub)
				  .work (*pool.generate (lumex::dev::genesis->hash ()))
				  .build ();
	auto result1 = ledger.process (transaction, block1);
	ASSERT_EQ (lumex::block_status::progress, result1);
	lumex::keypair key2;
	auto block2 = builder
				  .change ()
				  .previous (lumex::dev::genesis->hash ())
				  .representative (key2.pub)
				  .sign (lumex::dev::genesis_key.prv, lumex::dev::genesis_key.pub)
				  .work (*pool.generate (lumex::dev::genesis->hash ()))
				  .build ();
	auto result2 = ledger.process (transaction, block2);
	ASSERT_EQ (lumex::block_status::fork, result2);
}

TEST (ledger, fail_send_old)
{
	auto ctx = lumex::test::ledger_empty ();
	auto & ledger = ctx.ledger ();
	auto & store = ctx.store ();
	auto transaction = ledger.tx_begin_write ();
	auto & pool = ctx.pool ();
	lumex::keypair key1;
	lumex::block_builder builder;
	auto block = builder
				 .send ()
				 .previous (lumex::dev::genesis->hash ())
				 .destination (key1.pub)
				 .balance (1)
				 .sign (lumex::dev::genesis_key.prv, lumex::dev::genesis_key.pub)
				 .work (*pool.generate (lumex::dev::genesis->hash ()))
				 .build ();
	auto result1 = ledger.process (transaction, block);
	ASSERT_EQ (lumex::block_status::progress, result1);
	auto result2 = ledger.process (transaction, block);
	ASSERT_EQ (lumex::block_status::old, result2);
}

TEST (ledger, fail_send_gap_previous)
{
	auto ctx = lumex::test::ledger_empty ();
	auto & ledger = ctx.ledger ();
	auto & store = ctx.store ();
	auto transaction = ledger.tx_begin_write ();
	auto & pool = ctx.pool ();
	lumex::keypair key1;
	lumex::block_builder builder;
	auto block = builder
				 .send ()
				 .previous (1)
				 .destination (key1.pub)
				 .balance (1)
				 .sign (lumex::dev::genesis_key.prv, lumex::dev::genesis_key.pub)
				 .work (*pool.generate (lumex::root (1)))
				 .build ();
	auto result1 = ledger.process (transaction, block);
	ASSERT_EQ (lumex::block_status::gap_previous, result1);
}

TEST (ledger, fail_send_bad_signature)
{
	auto ctx = lumex::test::ledger_empty ();
	auto & ledger = ctx.ledger ();
	auto & store = ctx.store ();
	auto transaction = ledger.tx_begin_write ();
	auto & pool = ctx.pool ();
	lumex::keypair key1;
	lumex::block_builder builder;
	auto block = builder
				 .send ()
				 .previous (lumex::dev::genesis->hash ())
				 .destination (key1.pub)
				 .balance (1)
				 .sign (lumex::keypair ().prv, 0)
				 .work (*pool.generate (lumex::dev::genesis->hash ()))
				 .build ();
	auto result1 = ledger.process (transaction, block);
	ASSERT_EQ (lumex::block_status::bad_signature, result1);
}

TEST (ledger, fail_send_negative_spend)
{
	auto ctx = lumex::test::ledger_empty ();
	auto & ledger = ctx.ledger ();
	auto & store = ctx.store ();
	auto transaction = ledger.tx_begin_write ();
	auto & pool = ctx.pool ();
	lumex::keypair key1;
	lumex::block_builder builder;
	auto block1 = builder
				  .send ()
				  .previous (lumex::dev::genesis->hash ())
				  .destination (key1.pub)
				  .balance (1)
				  .sign (lumex::dev::genesis_key.prv, lumex::dev::genesis_key.pub)
				  .work (*pool.generate (lumex::dev::genesis->hash ()))
				  .build ();
	ASSERT_EQ (lumex::block_status::progress, ledger.process (transaction, block1));
	lumex::keypair key2;
	auto block2 = builder
				  .send ()
				  .previous (block1->hash ())
				  .destination (key2.pub)
				  .balance (2)
				  .sign (lumex::dev::genesis_key.prv, lumex::dev::genesis_key.pub)
				  .work (*pool.generate (block1->hash ()))
				  .build ();
	ASSERT_EQ (lumex::block_status::negative_spend, ledger.process (transaction, block2));
}

TEST (ledger, fail_send_fork)
{
	auto ctx = lumex::test::ledger_empty ();
	auto & ledger = ctx.ledger ();
	auto & store = ctx.store ();
	auto transaction = ledger.tx_begin_write ();
	auto & pool = ctx.pool ();
	lumex::keypair key1;
	lumex::block_builder builder;
	auto block1 = builder
				  .send ()
				  .previous (lumex::dev::genesis->hash ())
				  .destination (key1.pub)
				  .balance (1)
				  .sign (lumex::dev::genesis_key.prv, lumex::dev::genesis_key.pub)
				  .work (*pool.generate (lumex::dev::genesis->hash ()))
				  .build ();
	ASSERT_EQ (lumex::block_status::progress, ledger.process (transaction, block1));
	lumex::keypair key2;
	auto block2 = builder
				  .send ()
				  .previous (lumex::dev::genesis->hash ())
				  .destination (key2.pub)
				  .balance (1)
				  .sign (lumex::dev::genesis_key.prv, lumex::dev::genesis_key.pub)
				  .work (*pool.generate (lumex::dev::genesis->hash ()))
				  .build ();
	ASSERT_EQ (lumex::block_status::fork, ledger.process (transaction, block2));
}

TEST (ledger, fail_open_old)
{
	auto ctx = lumex::test::ledger_empty ();
	auto & ledger = ctx.ledger ();
	auto & store = ctx.store ();
	auto transaction = ledger.tx_begin_write ();
	auto & pool = ctx.pool ();
	lumex::keypair key1;
	lumex::block_builder builder;
	auto block1 = builder
				  .send ()
				  .previous (lumex::dev::genesis->hash ())
				  .destination (key1.pub)
				  .balance (1)
				  .sign (lumex::dev::genesis_key.prv, lumex::dev::genesis_key.pub)
				  .work (*pool.generate (lumex::dev::genesis->hash ()))
				  .build ();
	ASSERT_EQ (lumex::block_status::progress, ledger.process (transaction, block1));
	auto block2 = builder
				  .open ()
				  .source (block1->hash ())
				  .representative (1)
				  .account (key1.pub)
				  .sign (key1.prv, key1.pub)
				  .work (*pool.generate (key1.pub))
				  .build ();
	ASSERT_EQ (lumex::block_status::progress, ledger.process (transaction, block2));
	ASSERT_EQ (lumex::block_status::old, ledger.process (transaction, block2));
}

TEST (ledger, fail_open_gap_source)
{
	auto ctx = lumex::test::ledger_empty ();
	auto & ledger = ctx.ledger ();
	auto & store = ctx.store ();
	auto transaction = ledger.tx_begin_write ();
	auto & pool = ctx.pool ();
	lumex::keypair key1;
	lumex::block_builder builder;
	auto block2 = builder
				  .open ()
				  .source (1)
				  .representative (1)
				  .account (key1.pub)
				  .sign (key1.prv, key1.pub)
				  .work (*pool.generate (key1.pub))
				  .build ();
	auto result2 = ledger.process (transaction, block2);
	ASSERT_EQ (lumex::block_status::gap_source, result2);
}

TEST (ledger, fail_open_bad_signature)
{
	auto ctx = lumex::test::ledger_empty ();
	auto & ledger = ctx.ledger ();
	auto & store = ctx.store ();
	auto transaction = ledger.tx_begin_write ();
	auto & pool = ctx.pool ();
	lumex::keypair key1;
	lumex::block_builder builder;
	auto block1 = builder
				  .send ()
				  .previous (lumex::dev::genesis->hash ())
				  .destination (key1.pub)
				  .balance (1)
				  .sign (lumex::dev::genesis_key.prv, lumex::dev::genesis_key.pub)
				  .work (*pool.generate (lumex::dev::genesis->hash ()))
				  .build ();
	ASSERT_EQ (lumex::block_status::progress, ledger.process (transaction, block1));
	auto block2 = builder
				  .open ()
				  .source (block1->hash ())
				  .representative (1)
				  .account (key1.pub)
				  .sign (key1.prv, key1.pub)
				  .work (*pool.generate (key1.pub))
				  .build ();
	block2->signature.clear ();
	ASSERT_EQ (lumex::block_status::bad_signature, ledger.process (transaction, block2));
}

TEST (ledger, fail_open_fork_previous)
{
	auto ctx = lumex::test::ledger_empty ();
	auto & ledger = ctx.ledger ();
	auto & store = ctx.store ();
	auto transaction = ledger.tx_begin_write ();
	auto & pool = ctx.pool ();
	lumex::keypair key1;
	lumex::block_builder builder;
	auto block1 = builder
				  .send ()
				  .previous (lumex::dev::genesis->hash ())
				  .destination (key1.pub)
				  .balance (1)
				  .sign (lumex::dev::genesis_key.prv, lumex::dev::genesis_key.pub)
				  .work (*pool.generate (lumex::dev::genesis->hash ()))
				  .build ();
	ASSERT_EQ (lumex::block_status::progress, ledger.process (transaction, block1));
	auto block2 = builder
				  .send ()
				  .previous (block1->hash ())
				  .destination (key1.pub)
				  .balance (0)
				  .sign (lumex::dev::genesis_key.prv, lumex::dev::genesis_key.pub)
				  .work (*pool.generate (block1->hash ()))
				  .build ();
	ASSERT_EQ (lumex::block_status::progress, ledger.process (transaction, block2));
	auto block3 = builder
				  .open ()
				  .source (block1->hash ())
				  .representative (1)
				  .account (key1.pub)
				  .sign (key1.prv, key1.pub)
				  .work (*pool.generate (key1.pub))
				  .build ();
	ASSERT_EQ (lumex::block_status::progress, ledger.process (transaction, block3));
	auto block4 = builder
				  .open ()
				  .source (block2->hash ())
				  .representative (1)
				  .account (key1.pub)
				  .sign (key1.prv, key1.pub)
				  .work (*pool.generate (key1.pub))
				  .build ();
	ASSERT_EQ (lumex::block_status::fork, ledger.process (transaction, block4));
	ASSERT_EQ (store.account.count (transaction), ledger.account_count ());
}

TEST (ledger, fail_open_account_mismatch)
{
	auto ctx = lumex::test::ledger_empty ();
	auto & ledger = ctx.ledger ();
	auto & store = ctx.store ();
	auto transaction = ledger.tx_begin_write ();
	auto & pool = ctx.pool ();
	lumex::keypair key1;
	lumex::block_builder builder;
	auto block1 = builder
				  .send ()
				  .previous (lumex::dev::genesis->hash ())
				  .destination (key1.pub)
				  .balance (1)
				  .sign (lumex::dev::genesis_key.prv, lumex::dev::genesis_key.pub)
				  .work (*pool.generate (lumex::dev::genesis->hash ()))
				  .build ();
	ASSERT_EQ (lumex::block_status::progress, ledger.process (transaction, block1));
	lumex::keypair badkey;
	auto block2 = builder
				  .open ()
				  .source (block1->hash ())
				  .representative (1)
				  .account (badkey.pub)
				  .sign (badkey.prv, badkey.pub)
				  .work (*pool.generate (badkey.pub))
				  .build ();
	ASSERT_NE (lumex::block_status::progress, ledger.process (transaction, block2));
	ASSERT_EQ (store.account.count (transaction), ledger.account_count ());
}

TEST (ledger, fail_receive_old)
{
	auto ctx = lumex::test::ledger_empty ();
	auto & ledger = ctx.ledger ();
	auto & store = ctx.store ();
	auto transaction = ledger.tx_begin_write ();
	auto & pool = ctx.pool ();
	lumex::keypair key1;
	lumex::block_builder builder;
	auto block1 = builder
				  .send ()
				  .previous (lumex::dev::genesis->hash ())
				  .destination (key1.pub)
				  .balance (1)
				  .sign (lumex::dev::genesis_key.prv, lumex::dev::genesis_key.pub)
				  .work (*pool.generate (lumex::dev::genesis->hash ()))
				  .build ();
	ASSERT_EQ (lumex::block_status::progress, ledger.process (transaction, block1));
	auto block2 = builder
				  .send ()
				  .previous (block1->hash ())
				  .destination (key1.pub)
				  .balance (0)
				  .sign (lumex::dev::genesis_key.prv, lumex::dev::genesis_key.pub)
				  .work (*pool.generate (block1->hash ()))
				  .build ();
	ASSERT_EQ (lumex::block_status::progress, ledger.process (transaction, block2));
	auto block3 = builder
				  .open ()
				  .source (block1->hash ())
				  .representative (1)
				  .account (key1.pub)
				  .sign (key1.prv, key1.pub)
				  .work (*pool.generate (key1.pub))
				  .build ();
	ASSERT_EQ (lumex::block_status::progress, ledger.process (transaction, block3));
	auto block4 = builder
				  .receive ()
				  .previous (block3->hash ())
				  .source (block2->hash ())
				  .sign (key1.prv, key1.pub)
				  .work (*pool.generate (block3->hash ()))
				  .build ();
	ASSERT_EQ (lumex::block_status::progress, ledger.process (transaction, block4));
	ASSERT_EQ (lumex::block_status::old, ledger.process (transaction, block4));
}

TEST (ledger, fail_receive_gap_source)
{
	auto ctx = lumex::test::ledger_empty ();
	auto & ledger = ctx.ledger ();
	auto & store = ctx.store ();
	auto transaction = ledger.tx_begin_write ();
	auto & pool = ctx.pool ();
	lumex::keypair key1;
	lumex::block_builder builder;
	auto block1 = builder
				  .send ()
				  .previous (lumex::dev::genesis->hash ())
				  .destination (key1.pub)
				  .balance (1)
				  .sign (lumex::dev::genesis_key.prv, lumex::dev::genesis_key.pub)
				  .work (*pool.generate (lumex::dev::genesis->hash ()))
				  .build ();
	auto result1 = ledger.process (transaction, block1);
	ASSERT_EQ (lumex::block_status::progress, result1);
	auto block2 = builder
				  .send ()
				  .previous (block1->hash ())
				  .destination (key1.pub)
				  .balance (0)
				  .sign (lumex::dev::genesis_key.prv, lumex::dev::genesis_key.pub)
				  .work (*pool.generate (block1->hash ()))
				  .build ();
	auto result2 = ledger.process (transaction, block2);
	ASSERT_EQ (lumex::block_status::progress, result2);
	auto block3 = builder
				  .open ()
				  .source (block1->hash ())
				  .representative (1)
				  .account (key1.pub)
				  .sign (key1.prv, key1.pub)
				  .work (*pool.generate (key1.pub))
				  .build ();
	auto result3 = ledger.process (transaction, block3);
	ASSERT_EQ (lumex::block_status::progress, result3);
	auto block4 = builder
				  .receive ()
				  .previous (block3->hash ())
				  .source (1)
				  .sign (key1.prv, key1.pub)
				  .work (*pool.generate (block3->hash ()))
				  .build ();
	auto result4 = ledger.process (transaction, block4);
	ASSERT_EQ (lumex::block_status::gap_source, result4);
}

TEST (ledger, fail_receive_overreceive)
{
	auto ctx = lumex::test::ledger_empty ();
	auto & ledger = ctx.ledger ();
	auto & store = ctx.store ();
	auto transaction = ledger.tx_begin_write ();
	auto & pool = ctx.pool ();
	lumex::keypair key1;
	lumex::block_builder builder;
	auto block1 = builder
				  .send ()
				  .previous (lumex::dev::genesis->hash ())
				  .destination (key1.pub)
				  .balance (1)
				  .sign (lumex::dev::genesis_key.prv, lumex::dev::genesis_key.pub)
				  .work (*pool.generate (lumex::dev::genesis->hash ()))
				  .build ();
	auto result1 = ledger.process (transaction, block1);
	ASSERT_EQ (lumex::block_status::progress, result1);
	auto block2 = builder
				  .open ()
				  .source (block1->hash ())
				  .representative (1)
				  .account (key1.pub)
				  .sign (key1.prv, key1.pub)
				  .work (*pool.generate (key1.pub))
				  .build ();
	auto result3 = ledger.process (transaction, block2);
	ASSERT_EQ (lumex::block_status::progress, result3);
	auto block3 = builder
				  .receive ()
				  .previous (block2->hash ())
				  .source (block1->hash ())
				  .sign (key1.prv, key1.pub)
				  .work (*pool.generate (block2->hash ()))
				  .build ();
	auto result4 = ledger.process (transaction, block3);
	ASSERT_EQ (lumex::block_status::unreceivable, result4);
}

TEST (ledger, fail_receive_bad_signature)
{
	auto ctx = lumex::test::ledger_empty ();
	auto & ledger = ctx.ledger ();
	auto & store = ctx.store ();
	auto transaction = ledger.tx_begin_write ();
	auto & pool = ctx.pool ();
	lumex::keypair key1;
	lumex::block_builder builder;
	auto block1 = builder
				  .send ()
				  .previous (lumex::dev::genesis->hash ())
				  .destination (key1.pub)
				  .balance (1)
				  .sign (lumex::dev::genesis_key.prv, lumex::dev::genesis_key.pub)
				  .work (*pool.generate (lumex::dev::genesis->hash ()))
				  .build ();
	auto result1 = ledger.process (transaction, block1);
	ASSERT_EQ (lumex::block_status::progress, result1);
	auto block2 = builder
				  .send ()
				  .previous (block1->hash ())
				  .destination (key1.pub)
				  .balance (0)
				  .sign (lumex::dev::genesis_key.prv, lumex::dev::genesis_key.pub)
				  .work (*pool.generate (block1->hash ()))
				  .build ();
	auto result2 = ledger.process (transaction, block2);
	ASSERT_EQ (lumex::block_status::progress, result2);
	auto block3 = builder
				  .open ()
				  .source (block1->hash ())
				  .representative (1)
				  .account (key1.pub)
				  .sign (key1.prv, key1.pub)
				  .work (*pool.generate (key1.pub))
				  .build ();
	auto result3 = ledger.process (transaction, block3);
	ASSERT_EQ (lumex::block_status::progress, result3);
	auto block4 = builder
				  .receive ()
				  .previous (block3->hash ())
				  .source (block2->hash ())
				  .sign (lumex::keypair ().prv, 0)
				  .work (*pool.generate (block3->hash ()))
				  .build ();
	auto result4 = ledger.process (transaction, block4);
	ASSERT_EQ (lumex::block_status::bad_signature, result4);
}

TEST (ledger, fail_receive_gap_previous_opened)
{
	auto ctx = lumex::test::ledger_empty ();
	auto & ledger = ctx.ledger ();
	auto & store = ctx.store ();
	auto transaction = ledger.tx_begin_write ();
	auto & pool = ctx.pool ();
	lumex::keypair key1;
	lumex::block_builder builder;
	auto block1 = builder
				  .send ()
				  .previous (lumex::dev::genesis->hash ())
				  .destination (key1.pub)
				  .balance (1)
				  .sign (lumex::dev::genesis_key.prv, lumex::dev::genesis_key.pub)
				  .work (*pool.generate (lumex::dev::genesis->hash ()))
				  .build ();
	auto result1 = ledger.process (transaction, block1);
	ASSERT_EQ (lumex::block_status::progress, result1);
	auto block2 = builder
				  .send ()
				  .previous (block1->hash ())
				  .destination (key1.pub)
				  .balance (0)
				  .sign (lumex::dev::genesis_key.prv, lumex::dev::genesis_key.pub)
				  .work (*pool.generate (block1->hash ()))
				  .build ();
	auto result2 = ledger.process (transaction, block2);
	ASSERT_EQ (lumex::block_status::progress, result2);
	auto block3 = builder
				  .open ()
				  .source (block1->hash ())
				  .representative (1)
				  .account (key1.pub)
				  .sign (key1.prv, key1.pub)
				  .work (*pool.generate (key1.pub))
				  .build ();
	auto result3 = ledger.process (transaction, block3);
	ASSERT_EQ (lumex::block_status::progress, result3);
	auto block4 = builder
				  .receive ()
				  .previous (1)
				  .source (block2->hash ())
				  .sign (key1.prv, key1.pub)
				  .work (*pool.generate (lumex::root (1)))
				  .build ();
	auto result4 = ledger.process (transaction, block4);
	ASSERT_EQ (lumex::block_status::gap_previous, result4);
}

TEST (ledger, fail_receive_gap_previous_unopened)
{
	auto ctx = lumex::test::ledger_empty ();
	auto & ledger = ctx.ledger ();
	auto & store = ctx.store ();
	auto transaction = ledger.tx_begin_write ();
	auto & pool = ctx.pool ();
	lumex::keypair key1;
	lumex::block_builder builder;
	auto block1 = builder
				  .send ()
				  .previous (lumex::dev::genesis->hash ())
				  .destination (key1.pub)
				  .balance (1)
				  .sign (lumex::dev::genesis_key.prv, lumex::dev::genesis_key.pub)
				  .work (*pool.generate (lumex::dev::genesis->hash ()))
				  .build ();
	auto result1 = ledger.process (transaction, block1);
	ASSERT_EQ (lumex::block_status::progress, result1);
	auto block2 = builder
				  .send ()
				  .previous (block1->hash ())
				  .destination (key1.pub)
				  .balance (0)
				  .sign (lumex::dev::genesis_key.prv, lumex::dev::genesis_key.pub)
				  .work (*pool.generate (block1->hash ()))
				  .build ();
	auto result2 = ledger.process (transaction, block2);
	ASSERT_EQ (lumex::block_status::progress, result2);
	auto block3 = builder
				  .receive ()
				  .previous (1)
				  .source (block2->hash ())
				  .sign (key1.prv, key1.pub)
				  .work (*pool.generate (lumex::root (1)))
				  .build ();
	auto result3 = ledger.process (transaction, block3);
	ASSERT_EQ (lumex::block_status::gap_previous, result3);
}

TEST (ledger, fail_receive_fork_previous)
{
	auto ctx = lumex::test::ledger_empty ();
	auto & ledger = ctx.ledger ();
	auto & store = ctx.store ();
	auto transaction = ledger.tx_begin_write ();
	auto & pool = ctx.pool ();
	lumex::keypair key1;
	lumex::block_builder builder;
	auto block1 = builder
				  .send ()
				  .previous (lumex::dev::genesis->hash ())
				  .destination (key1.pub)
				  .balance (1)
				  .sign (lumex::dev::genesis_key.prv, lumex::dev::genesis_key.pub)
				  .work (*pool.generate (lumex::dev::genesis->hash ()))
				  .build ();
	auto result1 = ledger.process (transaction, block1);
	ASSERT_EQ (lumex::block_status::progress, result1);
	auto block2 = builder
				  .send ()
				  .previous (block1->hash ())
				  .destination (key1.pub)
				  .balance (0)
				  .sign (lumex::dev::genesis_key.prv, lumex::dev::genesis_key.pub)
				  .work (*pool.generate (block1->hash ()))
				  .build ();
	auto result2 = ledger.process (transaction, block2);
	ASSERT_EQ (lumex::block_status::progress, result2);
	auto block3 = builder
				  .open ()
				  .source (block1->hash ())
				  .representative (1)
				  .account (key1.pub)
				  .sign (key1.prv, key1.pub)
				  .work (*pool.generate (key1.pub))
				  .build ();
	auto result3 = ledger.process (transaction, block3);
	ASSERT_EQ (lumex::block_status::progress, result3);
	lumex::keypair key2;
	auto block4 = builder
				  .send ()
				  .previous (block3->hash ())
				  .destination (key1.pub)
				  .balance (1)
				  .sign (key1.prv, key1.pub)
				  .work (*pool.generate (block3->hash ()))
				  .build ();
	auto result4 = ledger.process (transaction, block4);
	ASSERT_EQ (lumex::block_status::progress, result4);
	auto block5 = builder
				  .receive ()
				  .previous (block3->hash ())
				  .source (block2->hash ())
				  .sign (key1.prv, key1.pub)
				  .work (*pool.generate (block3->hash ()))
				  .build ();
	auto result5 = ledger.process (transaction, block5);
	ASSERT_EQ (lumex::block_status::fork, result5);
}

TEST (ledger, fail_receive_received_source)
{
	auto ctx = lumex::test::ledger_empty ();
	auto & ledger = ctx.ledger ();
	auto & store = ctx.store ();
	auto transaction = ledger.tx_begin_write ();
	auto & pool = ctx.pool ();
	lumex::keypair key1;
	lumex::block_builder builder;
	auto block1 = builder
				  .send ()
				  .previous (lumex::dev::genesis->hash ())
				  .destination (key1.pub)
				  .balance (2)
				  .sign (lumex::dev::genesis_key.prv, lumex::dev::genesis_key.pub)
				  .work (*pool.generate (lumex::dev::genesis->hash ()))
				  .build ();
	auto result1 = ledger.process (transaction, block1);
	ASSERT_EQ (lumex::block_status::progress, result1);
	auto block2 = builder
				  .send ()
				  .previous (block1->hash ())
				  .destination (key1.pub)
				  .balance (1)
				  .sign (lumex::dev::genesis_key.prv, lumex::dev::genesis_key.pub)
				  .work (*pool.generate (block1->hash ()))
				  .build ();
	auto result2 = ledger.process (transaction, block2);
	ASSERT_EQ (lumex::block_status::progress, result2);
	auto block6 = builder
				  .send ()
				  .previous (block2->hash ())
				  .destination (key1.pub)
				  .balance (0)
				  .sign (lumex::dev::genesis_key.prv, lumex::dev::genesis_key.pub)
				  .work (*pool.generate (block2->hash ()))
				  .build ();
	auto result6 = ledger.process (transaction, block6);
	ASSERT_EQ (lumex::block_status::progress, result6);
	auto block3 = builder
				  .open ()
				  .source (block1->hash ())
				  .representative (1)
				  .account (key1.pub)
				  .sign (key1.prv, key1.pub)
				  .work (*pool.generate (key1.pub))
				  .build ();
	auto result3 = ledger.process (transaction, block3);
	ASSERT_EQ (lumex::block_status::progress, result3);
	lumex::keypair key2;
	auto block4 = builder
				  .send ()
				  .previous (block3->hash ())
				  .destination (key1.pub)
				  .balance (1)
				  .sign (key1.prv, key1.pub)
				  .work (*pool.generate (block3->hash ()))
				  .build ();
	auto result4 = ledger.process (transaction, block4);
	ASSERT_EQ (lumex::block_status::progress, result4);
	auto block5 = builder
				  .receive ()
				  .previous (block4->hash ())
				  .source (block2->hash ())
				  .sign (key1.prv, key1.pub)
				  .work (*pool.generate (block4->hash ()))
				  .build ();
	auto result5 = ledger.process (transaction, block5);
	ASSERT_EQ (lumex::block_status::progress, result5);
	auto block7 = builder
				  .receive ()
				  .previous (block3->hash ())
				  .source (block2->hash ())
				  .sign (key1.prv, key1.pub)
				  .work (*pool.generate (block3->hash ()))
				  .build ();
	auto result7 = ledger.process (transaction, block7);
	ASSERT_EQ (lumex::block_status::fork, result7);
}

TEST (ledger, latest_empty)
{
	auto ctx = lumex::test::ledger_empty ();
	auto & ledger = ctx.ledger ();
	auto & store = ctx.store ();
	lumex::keypair key;
	auto transaction = ledger.tx_begin_read ();
	auto latest = ledger.any.account_head (transaction, key.pub);
	ASSERT_TRUE (latest.is_zero ());
}

TEST (ledger, latest_root)
{
	auto ctx = lumex::test::ledger_empty ();
	auto & ledger = ctx.ledger ();
	auto & store = ctx.store ();
	auto transaction = ledger.tx_begin_write ();
	auto & pool = ctx.pool ();
	lumex::keypair key;
	ASSERT_EQ (key.pub, ledger.latest_root (transaction, key.pub).as_account ());
	auto hash1 = ledger.any.account_head (transaction, lumex::dev::genesis_key.pub);
	lumex::block_builder builder;
	auto send = builder
				.send ()
				.previous (hash1)
				.destination (0)
				.balance (1)
				.sign (lumex::dev::genesis_key.prv, lumex::dev::genesis_key.pub)
				.work (*pool.generate (hash1))
				.build ();
	ASSERT_EQ (lumex::block_status::progress, ledger.process (transaction, send));
	ASSERT_EQ (send->hash (), ledger.latest_root (transaction, lumex::dev::genesis_key.pub).as_block_hash ());
}

TEST (ledger, change_representative_move_representation)
{
	auto ctx = lumex::test::ledger_empty ();
	auto & ledger = ctx.ledger ();
	auto & store = ctx.store ();
	lumex::keypair key1;
	auto transaction = ledger.tx_begin_write ();
	auto & pool = ctx.pool ();
	ASSERT_EQ (lumex::dev::constants.genesis_amount, ledger.weight (lumex::dev::genesis_key.pub));
	lumex::block_builder builder;
	auto send = builder
				.send ()
				.previous (lumex::dev::genesis->hash ())
				.destination (key1.pub)
				.balance (0)
				.sign (lumex::dev::genesis_key.prv, lumex::dev::genesis_key.pub)
				.work (*pool.generate (lumex::dev::genesis->hash ()))
				.build ();
	ASSERT_EQ (lumex::block_status::progress, ledger.process (transaction, send));
	ASSERT_EQ (0, ledger.weight (lumex::dev::genesis_key.pub));
	lumex::keypair key2;
	auto change = builder
				  .change ()
				  .previous (send->hash ())
				  .representative (key2.pub)
				  .sign (lumex::dev::genesis_key.prv, lumex::dev::genesis_key.pub)
				  .work (*pool.generate (send->hash ()))
				  .build ();
	ASSERT_EQ (lumex::block_status::progress, ledger.process (transaction, change));
	lumex::keypair key3;
	auto open = builder
				.open ()
				.source (send->hash ())
				.representative (key3.pub)
				.account (key1.pub)
				.sign (key1.prv, key1.pub)
				.work (*pool.generate (key1.pub))
				.build ();
	ASSERT_EQ (lumex::block_status::progress, ledger.process (transaction, open));
	ASSERT_EQ (lumex::dev::constants.genesis_amount, ledger.weight (key3.pub));
}

TEST (ledger, send_open_receive_rollback)
{
	auto ctx = lumex::test::ledger_empty ();
	auto & ledger = ctx.ledger ();
	auto & store = ctx.store ();
	auto transaction = ledger.tx_begin_write ();
	auto & pool = ctx.pool ();
	auto info1 = ledger.any.account_get (transaction, lumex::dev::genesis_key.pub);
	ASSERT_TRUE (info1);
	lumex::keypair key1;
	lumex::block_builder builder;
	auto send1 = builder
				 .send ()
				 .previous (info1->head)
				 .destination (key1.pub)
				 .balance (lumex::dev::constants.genesis_amount - 50)
				 .sign (lumex::dev::genesis_key.prv, lumex::dev::genesis_key.pub)
				 .work (*pool.generate (info1->head))
				 .build ();
	auto return1 = ledger.process (transaction, send1);
	ASSERT_EQ (lumex::block_status::progress, return1);
	auto send2 = builder
				 .send ()
				 .previous (send1->hash ())
				 .destination (key1.pub)
				 .balance (lumex::dev::constants.genesis_amount - 100)
				 .sign (lumex::dev::genesis_key.prv, lumex::dev::genesis_key.pub)
				 .work (*pool.generate (send1->hash ()))
				 .build ();
	auto return2 = ledger.process (transaction, send2);
	ASSERT_EQ (lumex::block_status::progress, return2);
	lumex::keypair key2;
	auto open = builder
				.open ()
				.source (send2->hash ())
				.representative (key2.pub)
				.account (key1.pub)
				.sign (key1.prv, key1.pub)
				.work (*pool.generate (key1.pub))
				.build ();
	auto return4 = ledger.process (transaction, open);
	ASSERT_EQ (lumex::block_status::progress, return4);
	auto receive = builder
				   .receive ()
				   .previous (open->hash ())
				   .source (send1->hash ())
				   .sign (key1.prv, key1.pub)
				   .work (*pool.generate (open->hash ()))
				   .build ();
	auto return5 = ledger.process (transaction, receive);
	ASSERT_EQ (lumex::block_status::progress, return5);
	lumex::keypair key3;
	ASSERT_EQ (100, ledger.weight (key2.pub));
	ASSERT_EQ (lumex::dev::constants.genesis_amount - 100, ledger.weight (lumex::dev::genesis_key.pub));
	ASSERT_EQ (0, ledger.weight (key3.pub));
	auto change1 = builder
				   .change ()
				   .previous (send2->hash ())
				   .representative (key3.pub)
				   .sign (lumex::dev::genesis_key.prv, lumex::dev::genesis_key.pub)
				   .work (*pool.generate (send2->hash ()))
				   .build ();
	auto return6 = ledger.process (transaction, change1);
	ASSERT_EQ (lumex::block_status::progress, return6);
	ASSERT_EQ (100, ledger.weight (key2.pub));
	ASSERT_EQ (0, ledger.weight (lumex::dev::genesis_key.pub));
	ASSERT_EQ (lumex::dev::constants.genesis_amount - 100, ledger.weight (key3.pub));
	ASSERT_FALSE (ledger.rollback (transaction, receive->hash ()));
	ASSERT_EQ (50, ledger.weight (key2.pub));
	ASSERT_EQ (0, ledger.weight (lumex::dev::genesis_key.pub));
	ASSERT_EQ (lumex::dev::constants.genesis_amount - 100, ledger.weight (key3.pub));
	ASSERT_FALSE (ledger.rollback (transaction, open->hash ()));
	ASSERT_EQ (0, ledger.weight (key2.pub));
	ASSERT_EQ (0, ledger.weight (lumex::dev::genesis_key.pub));
	ASSERT_EQ (lumex::dev::constants.genesis_amount - 100, ledger.weight (key3.pub));
	ASSERT_FALSE (ledger.rollback (transaction, change1->hash ()));
	ASSERT_EQ (0, ledger.weight (key2.pub));
	ASSERT_EQ (0, ledger.weight (key3.pub));
	ASSERT_EQ (lumex::dev::constants.genesis_amount - 100, ledger.weight (lumex::dev::genesis_key.pub));
	ASSERT_FALSE (ledger.rollback (transaction, send2->hash ()));
	ASSERT_EQ (0, ledger.weight (key2.pub));
	ASSERT_EQ (0, ledger.weight (key3.pub));
	ASSERT_EQ (lumex::dev::constants.genesis_amount - 50, ledger.weight (lumex::dev::genesis_key.pub));
	ASSERT_FALSE (ledger.rollback (transaction, send1->hash ()));
	ASSERT_EQ (0, ledger.weight (key2.pub));
	ASSERT_EQ (0, ledger.weight (key3.pub));
	ASSERT_EQ (lumex::dev::constants.genesis_amount - 0, ledger.weight (lumex::dev::genesis_key.pub));
}

TEST (ledger, bootstrap_weights)
{
	auto ctx = lumex::test::ledger_empty ();
	auto & ledger = ctx.ledger ();
	auto & store = ctx.store ();
	auto & pool = ctx.pool ();

	lumex::keypair rep_key1, rep_key2, throwaway;

	// Genesis key has all the voting weight
	ASSERT_EQ (std::numeric_limits<lumex::uint128_t>::max (), ledger.weight (lumex::dev::genesis_key.pub));

	// Throwaway transaction to reduce genesis weight
	{
		auto transaction = ledger.tx_begin_write ();
		auto genesis_info = ledger.any.account_get (transaction, lumex::dev::genesis_key.pub);
		ASSERT_TRUE (genesis_info);
		lumex::block_builder builder;
		auto send = builder
					.send ()
					.previous (genesis_info->head)
					.destination (throwaway.pub)
					.balance (std::numeric_limits<lumex::uint128_t>::max () - 50)
					.sign (lumex::dev::genesis_key.prv, lumex::dev::genesis_key.pub)
					.work (*pool.generate (genesis_info->head))
					.build ();
		ASSERT_EQ (lumex::block_status::progress, ledger.process (transaction, send));
	}

	// Override bootstrap weights
	ledger.bootstrap_weights.max_blocks = 4;
	ledger.bootstrap_weights.representatives[rep_key1.pub] = 1000;

	// Should use bootstrap weights
	ASSERT_EQ (2, ledger.block_count ());
	ASSERT_FALSE (ledger.bootstrap_height_reached ());
	ASSERT_EQ (0, ledger.weight (lumex::dev::genesis_key.pub));
	ASSERT_EQ (1000, ledger.weight (rep_key1.pub));
	{
		auto snapshot = ledger.rep_weights_snapshot ();
		ASSERT_EQ (1, snapshot.size ());
		ASSERT_EQ (1000, snapshot[rep_key1.pub]);
	}

	// Open normal representative account, should not use bootstrap weights anymore
	{
		auto transaction = ledger.tx_begin_write ();
		auto genesis_info = ledger.any.account_get (transaction, lumex::dev::genesis_key.pub);
		ASSERT_TRUE (genesis_info);
		lumex::block_builder builder;
		auto send = builder
					.send ()
					.previous (genesis_info->head)
					.destination (rep_key2.pub)
					.balance (std::numeric_limits<lumex::uint128_t>::max () - 100)
					.sign (lumex::dev::genesis_key.prv, lumex::dev::genesis_key.pub)
					.work (*pool.generate (genesis_info->head))
					.build ();
		ASSERT_EQ (lumex::block_status::progress, ledger.process (transaction, send));

		auto open = builder
					.open ()
					.source (send->hash ())
					.representative (rep_key2.pub)
					.account (rep_key2.pub)
					.sign (rep_key2.prv, rep_key2.pub)
					.work (*pool.generate (rep_key2.pub))
					.build ();
		ASSERT_EQ (lumex::block_status::progress, ledger.process (transaction, open));
	}
	ASSERT_EQ (4, ledger.block_count ());
	ASSERT_TRUE (ledger.bootstrap_height_reached ());
	ASSERT_EQ (std::numeric_limits<lumex::uint128_t>::max () - 100, ledger.weight (lumex::dev::genesis_key.pub));
	ASSERT_EQ (0, ledger.weight (rep_key1.pub));
	ASSERT_EQ (50, ledger.weight (rep_key2.pub));
	{
		auto snapshot = ledger.rep_weights_snapshot ();
		ASSERT_EQ (2, snapshot.size ());
		ASSERT_EQ (std::numeric_limits<lumex::uint128_t>::max () - 100, snapshot[lumex::dev::genesis_key.pub]);
		ASSERT_EQ (50, snapshot[rep_key2.pub]);
	}
}

TEST (ledger, block_destination_source)
{
	auto ctx = lumex::test::ledger_empty ();
	auto & ledger = ctx.ledger ();
	auto & store = ctx.store ();
	auto transaction = ledger.tx_begin_write ();
	auto & pool = ctx.pool ();
	lumex::keypair dest;
	lumex::uint128_t balance (lumex::dev::constants.genesis_amount);
	balance -= lumex::Klumex_ratio;
	lumex::block_builder builder;
	auto block1 = builder
				  .send ()
				  .previous (lumex::dev::genesis->hash ())
				  .destination (dest.pub)
				  .balance (balance)
				  .sign (lumex::dev::genesis_key.prv, lumex::dev::genesis_key.pub)
				  .work (*pool.generate (lumex::dev::genesis->hash ()))
				  .build ();
	balance -= lumex::Klumex_ratio;
	auto block2 = builder
				  .send ()
				  .previous (block1->hash ())
				  .destination (lumex::dev::genesis_key.pub)
				  .balance (balance)
				  .sign (lumex::dev::genesis_key.prv, lumex::dev::genesis_key.pub)
				  .work (*pool.generate (block1->hash ()))
				  .build ();
	balance += lumex::Klumex_ratio;
	auto block3 = builder
				  .receive ()
				  .previous (block2->hash ())
				  .source (block2->hash ())
				  .sign (lumex::dev::genesis_key.prv, lumex::dev::genesis_key.pub)
				  .work (*pool.generate (block2->hash ()))
				  .build ();
	balance -= lumex::Klumex_ratio;
	auto block4 = builder
				  .state ()
				  .account (lumex::dev::genesis_key.pub)
				  .previous (block3->hash ())
				  .representative (lumex::dev::genesis_key.pub)
				  .balance (balance)
				  .link (dest.pub)
				  .sign (lumex::dev::genesis_key.prv, lumex::dev::genesis_key.pub)
				  .work (*pool.generate (block3->hash ()))
				  .build ();
	balance -= lumex::Klumex_ratio;
	auto block5 = builder
				  .state ()
				  .account (lumex::dev::genesis_key.pub)
				  .previous (block4->hash ())
				  .representative (lumex::dev::genesis_key.pub)
				  .balance (balance)
				  .link (lumex::dev::genesis_key.pub)
				  .sign (lumex::dev::genesis_key.prv, lumex::dev::genesis_key.pub)
				  .work (*pool.generate (block4->hash ()))
				  .build ();
	balance += lumex::Klumex_ratio;
	auto block6 = builder
				  .state ()
				  .account (lumex::dev::genesis_key.pub)
				  .previous (block5->hash ())
				  .representative (lumex::dev::genesis_key.pub)
				  .balance (balance)
				  .link (block5->hash ())
				  .sign (lumex::dev::genesis_key.prv, lumex::dev::genesis_key.pub)
				  .work (*pool.generate (block5->hash ()))
				  .build ();
	ASSERT_EQ (lumex::block_status::progress, ledger.process (transaction, block1));
	ASSERT_EQ (lumex::block_status::progress, ledger.process (transaction, block2));
	ASSERT_EQ (lumex::block_status::progress, ledger.process (transaction, block3));
	ASSERT_EQ (lumex::block_status::progress, ledger.process (transaction, block4));
	ASSERT_EQ (lumex::block_status::progress, ledger.process (transaction, block5));
	ASSERT_EQ (lumex::block_status::progress, ledger.process (transaction, block6));
	ASSERT_EQ (balance, ledger.any.block_balance (transaction, block6->hash ()));
	ASSERT_EQ (dest.pub, block1->destination ());
	ASSERT_FALSE (block1->source_field ());
	ASSERT_EQ (lumex::dev::genesis_key.pub, block2->destination ());
	ASSERT_FALSE (block2->source_field ());
	ASSERT_FALSE (block3->destination_field ());
	ASSERT_EQ (block2->hash (), block3->source ());
	ASSERT_EQ (dest.pub, block4->destination ());
	ASSERT_FALSE (block4->source_field ());
	ASSERT_EQ (lumex::dev::genesis_key.pub, block5->destination ());
	ASSERT_FALSE (block5->source_field ());
	ASSERT_FALSE (block6->destination_field ());
	ASSERT_EQ (block5->hash (), block6->source ());
}

TEST (ledger, state_account)
{
	auto ctx = lumex::test::ledger_empty ();
	auto & ledger = ctx.ledger ();
	auto & store = ctx.store ();
	auto transaction = ledger.tx_begin_write ();
	auto & pool = ctx.pool ();
	lumex::block_builder builder;
	auto send1 = builder
				 .state ()
				 .account (lumex::dev::genesis_key.pub)
				 .previous (lumex::dev::genesis->hash ())
				 .representative (lumex::dev::genesis_key.pub)
				 .balance (lumex::dev::constants.genesis_amount - lumex::Klumex_ratio)
				 .link (lumex::dev::genesis_key.pub)
				 .sign (lumex::dev::genesis_key.prv, lumex::dev::genesis_key.pub)
				 .work (*pool.generate (lumex::dev::genesis->hash ()))
				 .build ();
	ASSERT_EQ (lumex::block_status::progress, ledger.process (transaction, send1));
	ASSERT_EQ (lumex::dev::genesis_key.pub, ledger.any.block_account (transaction, send1->hash ()).value ());
}

TEST (ledger, state_send_receive)
{
	auto ctx = lumex::test::ledger_empty ();
	auto & ledger = ctx.ledger ();
	auto & store = ctx.store ();
	auto transaction = ledger.tx_begin_write ();
	auto & pool = ctx.pool ();
	lumex::block_builder builder;
	auto send1 = builder
				 .state ()
				 .account (lumex::dev::genesis_key.pub)
				 .previous (lumex::dev::genesis->hash ())
				 .representative (lumex::dev::genesis_key.pub)
				 .balance (lumex::dev::constants.genesis_amount - lumex::Klumex_ratio)
				 .link (lumex::dev::genesis_key.pub)
				 .sign (lumex::dev::genesis_key.prv, lumex::dev::genesis_key.pub)
				 .work (*pool.generate (lumex::dev::genesis->hash ()))
				 .build ();
	ASSERT_EQ (lumex::block_status::progress, ledger.process (transaction, send1));
	ASSERT_TRUE (ledger.any.block_exists (transaction, send1->hash ()));
	auto send2 = ledger.any.block_get (transaction, send1->hash ());
	ASSERT_NE (nullptr, send2);
	ASSERT_EQ (*send1, *send2);
	ASSERT_EQ (lumex::dev::constants.genesis_amount - lumex::Klumex_ratio, ledger.any.block_balance (transaction, send1->hash ()));
	ASSERT_EQ (lumex::Klumex_ratio, ledger.any.block_amount (transaction, send1->hash ()));
	ASSERT_EQ (lumex::dev::constants.genesis_amount - lumex::Klumex_ratio, ledger.weight (lumex::dev::genesis_key.pub));
	ASSERT_TRUE (ledger.any.pending_get (transaction, lumex::pending_key{ lumex::dev::genesis_key.pub, send1->hash () }));
	ASSERT_EQ (2, send2->sideband ().height);
	ASSERT_TRUE (send2->is_send ());
	ASSERT_FALSE (send2->is_receive ());
	ASSERT_FALSE (send2->sideband ().details.is_epoch);
	auto receive1 = builder
					.state ()
					.account (lumex::dev::genesis_key.pub)
					.previous (send1->hash ())
					.representative (lumex::dev::genesis_key.pub)
					.balance (lumex::dev::constants.genesis_amount)
					.link (send1->hash ())
					.sign (lumex::dev::genesis_key.prv, lumex::dev::genesis_key.pub)
					.work (*pool.generate (send1->hash ()))
					.build ();
	ASSERT_EQ (lumex::block_status::progress, ledger.process (transaction, receive1));
	ASSERT_TRUE (ledger.any.block_exists (transaction, receive1->hash ()));
	auto receive2 = ledger.any.block_get (transaction, receive1->hash ());
	ASSERT_NE (nullptr, receive2);
	ASSERT_EQ (*receive1, *receive2);
	ASSERT_EQ (lumex::dev::constants.genesis_amount, ledger.any.block_balance (transaction, receive1->hash ()));
	ASSERT_EQ (lumex::Klumex_ratio, ledger.any.block_amount (transaction, receive1->hash ()));
	ASSERT_EQ (lumex::dev::constants.genesis_amount, ledger.weight (lumex::dev::genesis_key.pub));
	ASSERT_FALSE (ledger.any.pending_get (transaction, lumex::pending_key{ lumex::dev::genesis_key.pub, send1->hash () }));
	ASSERT_EQ (store.account.count (transaction), ledger.account_count ());
	ASSERT_EQ (3, receive2->sideband ().height);
	ASSERT_FALSE (receive2->is_send ());
	ASSERT_TRUE (receive2->is_receive ());
	ASSERT_FALSE (receive2->sideband ().details.is_epoch);
}

TEST (ledger, state_receive)
{
	auto ctx = lumex::test::ledger_empty ();
	auto & ledger = ctx.ledger ();
	auto & store = ctx.store ();
	auto transaction = ledger.tx_begin_write ();
	auto & pool = ctx.pool ();
	lumex::block_builder builder;
	auto send1 = builder
				 .send ()
				 .previous (lumex::dev::genesis->hash ())
				 .destination (lumex::dev::genesis_key.pub)
				 .balance (lumex::dev::constants.genesis_amount - lumex::Klumex_ratio)
				 .sign (lumex::dev::genesis_key.prv, lumex::dev::genesis_key.pub)
				 .work (*pool.generate (lumex::dev::genesis->hash ()))
				 .build ();
	ASSERT_EQ (lumex::block_status::progress, ledger.process (transaction, send1));
	ASSERT_TRUE (ledger.any.block_exists (transaction, send1->hash ()));
	auto send2 = ledger.any.block_get (transaction, send1->hash ());
	ASSERT_NE (nullptr, send2);
	ASSERT_EQ (*send1, *send2);
	ASSERT_EQ (lumex::dev::constants.genesis_amount - lumex::Klumex_ratio, ledger.any.block_balance (transaction, send1->hash ()));
	ASSERT_EQ (lumex::Klumex_ratio, ledger.any.block_amount (transaction, send1->hash ()));
	ASSERT_EQ (lumex::dev::constants.genesis_amount - lumex::Klumex_ratio, ledger.weight (lumex::dev::genesis_key.pub));
	auto receive1 = builder
					.state ()
					.account (lumex::dev::genesis_key.pub)
					.previous (send1->hash ())
					.representative (lumex::dev::genesis_key.pub)
					.balance (lumex::dev::constants.genesis_amount)
					.link (send1->hash ())
					.sign (lumex::dev::genesis_key.prv, lumex::dev::genesis_key.pub)
					.work (*pool.generate (send1->hash ()))
					.build ();
	ASSERT_EQ (lumex::block_status::progress, ledger.process (transaction, receive1));
	ASSERT_TRUE (ledger.any.block_exists (transaction, receive1->hash ()));
	auto receive2 = ledger.any.block_get (transaction, receive1->hash ());
	ASSERT_NE (nullptr, receive2);
	ASSERT_EQ (*receive1, *receive2);
	ASSERT_EQ (lumex::dev::constants.genesis_amount, ledger.any.block_balance (transaction, receive1->hash ()));
	ASSERT_EQ (lumex::Klumex_ratio, ledger.any.block_amount (transaction, receive1->hash ()));
	ASSERT_EQ (lumex::dev::constants.genesis_amount, ledger.weight (lumex::dev::genesis_key.pub));
	ASSERT_EQ (3, receive2->sideband ().height);
	ASSERT_FALSE (receive2->is_send ());
	ASSERT_TRUE (receive2->is_receive ());
	ASSERT_FALSE (receive2->sideband ().details.is_epoch);
}

TEST (ledger, state_rep_change)
{
	auto ctx = lumex::test::ledger_empty ();
	auto & ledger = ctx.ledger ();
	auto & store = ctx.store ();
	auto transaction = ledger.tx_begin_write ();
	auto & pool = ctx.pool ();
	lumex::keypair rep;
	lumex::block_builder builder;
	auto change1 = builder
				   .state ()
				   .account (lumex::dev::genesis_key.pub)
				   .previous (lumex::dev::genesis->hash ())
				   .representative (rep.pub)
				   .balance (lumex::dev::constants.genesis_amount)
				   .link (0)
				   .sign (lumex::dev::genesis_key.prv, lumex::dev::genesis_key.pub)
				   .work (*pool.generate (lumex::dev::genesis->hash ()))
				   .build ();
	ASSERT_EQ (lumex::block_status::progress, ledger.process (transaction, change1));
	ASSERT_TRUE (ledger.any.block_exists (transaction, change1->hash ()));
	auto change2 = ledger.any.block_get (transaction, change1->hash ());
	ASSERT_NE (nullptr, change2);
	ASSERT_EQ (*change1, *change2);
	ASSERT_EQ (lumex::dev::constants.genesis_amount, ledger.any.block_balance (transaction, change1->hash ()));
	ASSERT_EQ (0, ledger.any.block_amount (transaction, change1->hash ()).value ().number ());
	ASSERT_EQ (0, ledger.weight (lumex::dev::genesis_key.pub));
	ASSERT_EQ (lumex::dev::constants.genesis_amount, ledger.weight (rep.pub));
	ASSERT_EQ (2, change2->sideband ().height);
	ASSERT_FALSE (change2->is_send ());
	ASSERT_FALSE (change2->is_receive ());
	ASSERT_FALSE (change2->sideband ().details.is_epoch);
}

TEST (ledger, state_open)
{
	auto ctx = lumex::test::ledger_empty ();
	auto & ledger = ctx.ledger ();
	auto & store = ctx.store ();
	auto transaction = ledger.tx_begin_write ();
	auto & pool = ctx.pool ();
	lumex::keypair destination;
	lumex::block_builder builder;
	auto send1 = builder
				 .state ()
				 .account (lumex::dev::genesis_key.pub)
				 .previous (lumex::dev::genesis->hash ())
				 .representative (lumex::dev::genesis_key.pub)
				 .balance (lumex::dev::constants.genesis_amount - lumex::Klumex_ratio)
				 .link (destination.pub)
				 .sign (lumex::dev::genesis_key.prv, lumex::dev::genesis_key.pub)
				 .work (*pool.generate (lumex::dev::genesis->hash ()))
				 .build ();
	ASSERT_EQ (lumex::block_status::progress, ledger.process (transaction, send1));
	ASSERT_TRUE (ledger.any.block_exists (transaction, send1->hash ()));
	auto send2 = ledger.any.block_get (transaction, send1->hash ());
	ASSERT_NE (nullptr, send2);
	ASSERT_EQ (*send1, *send2);
	ASSERT_EQ (lumex::dev::constants.genesis_amount - lumex::Klumex_ratio, ledger.any.block_balance (transaction, send1->hash ()));
	ASSERT_EQ (lumex::Klumex_ratio, ledger.any.block_amount (transaction, send1->hash ()));
	ASSERT_EQ (lumex::dev::constants.genesis_amount - lumex::Klumex_ratio, ledger.weight (lumex::dev::genesis_key.pub));
	ASSERT_TRUE (ledger.any.pending_get (transaction, lumex::pending_key{ destination.pub, send1->hash () }));
	auto open1 = builder
				 .state ()
				 .account (destination.pub)
				 .previous (0)
				 .representative (lumex::dev::genesis_key.pub)
				 .balance (lumex::Klumex_ratio)
				 .link (send1->hash ())
				 .sign (destination.prv, destination.pub)
				 .work (*pool.generate (destination.pub))
				 .build ();
	ASSERT_EQ (lumex::block_status::progress, ledger.process (transaction, open1));
	ASSERT_FALSE (ledger.any.pending_get (transaction, lumex::pending_key{ destination.pub, send1->hash () }));
	ASSERT_TRUE (ledger.any.block_exists (transaction, open1->hash ()));
	auto open2 = ledger.any.block_get (transaction, open1->hash ());
	ASSERT_NE (nullptr, open2);
	ASSERT_EQ (*open1, *open2);
	ASSERT_EQ (lumex::Klumex_ratio, ledger.any.block_balance (transaction, open1->hash ()));
	ASSERT_EQ (lumex::Klumex_ratio, ledger.any.block_amount (transaction, open1->hash ()));
	ASSERT_EQ (lumex::dev::constants.genesis_amount, ledger.weight (lumex::dev::genesis_key.pub));
	ASSERT_EQ (ledger.account_count (), store.account.count (transaction));
	ASSERT_EQ (1, open2->sideband ().height);
	ASSERT_FALSE (open2->is_send ());
	ASSERT_TRUE (open2->is_receive ());
	ASSERT_FALSE (open2->sideband ().details.is_epoch);
}

// Make sure old block types can't be inserted after a state block.
TEST (ledger, send_after_state_fail)
{
	auto ctx = lumex::test::ledger_empty ();
	auto & ledger = ctx.ledger ();
	auto & store = ctx.store ();
	auto transaction = ledger.tx_begin_write ();
	auto & pool = ctx.pool ();
	lumex::block_builder builder;
	auto send1 = builder
				 .state ()
				 .account (lumex::dev::genesis_key.pub)
				 .previous (lumex::dev::genesis->hash ())
				 .representative (lumex::dev::genesis_key.pub)
				 .balance (lumex::dev::constants.genesis_amount - lumex::Klumex_ratio)
				 .link (lumex::dev::genesis_key.pub)
				 .sign (lumex::dev::genesis_key.prv, lumex::dev::genesis_key.pub)
				 .work (*pool.generate (lumex::dev::genesis->hash ()))
				 .build ();
	ASSERT_EQ (lumex::block_status::progress, ledger.process (transaction, send1));
	auto send2 = builder
				 .send ()
				 .previous (send1->hash ())
				 .destination (lumex::dev::genesis_key.pub)
				 .balance (lumex::dev::constants.genesis_amount - (2 * lumex::Klumex_ratio))
				 .sign (lumex::dev::genesis_key.prv, lumex::dev::genesis_key.pub)
				 .work (*pool.generate (send1->hash ()))
				 .build ();
	ASSERT_EQ (lumex::block_status::block_position, ledger.process (transaction, send2));
}

// Make sure old block types can't be inserted after a state block.
TEST (ledger, receive_after_state_fail)
{
	auto ctx = lumex::test::ledger_empty ();
	auto & ledger = ctx.ledger ();
	auto & store = ctx.store ();
	auto transaction = ledger.tx_begin_write ();
	auto & pool = ctx.pool ();
	lumex::block_builder builder;
	auto send1 = builder
				 .state ()
				 .account (lumex::dev::genesis_key.pub)
				 .previous (lumex::dev::genesis->hash ())
				 .representative (lumex::dev::genesis_key.pub)
				 .balance (lumex::dev::constants.genesis_amount - lumex::Klumex_ratio)
				 .link (lumex::dev::genesis_key.pub)
				 .sign (lumex::dev::genesis_key.prv, lumex::dev::genesis_key.pub)
				 .work (*pool.generate (lumex::dev::genesis->hash ()))
				 .build ();
	ASSERT_EQ (lumex::block_status::progress, ledger.process (transaction, send1));
	auto receive1 = builder
					.receive ()
					.previous (send1->hash ())
					.source (send1->hash ())
					.sign (lumex::dev::genesis_key.prv, lumex::dev::genesis_key.pub)
					.work (*pool.generate (send1->hash ()))
					.build ();
	ASSERT_EQ (lumex::block_status::block_position, ledger.process (transaction, receive1));
}

// Make sure old block types can't be inserted after a state block.
TEST (ledger, change_after_state_fail)
{
	auto ctx = lumex::test::ledger_empty ();
	auto & ledger = ctx.ledger ();
	auto & store = ctx.store ();
	auto transaction = ledger.tx_begin_write ();
	auto & pool = ctx.pool ();
	lumex::block_builder builder;
	auto send1 = builder
				 .state ()
				 .account (lumex::dev::genesis_key.pub)
				 .previous (lumex::dev::genesis->hash ())
				 .representative (lumex::dev::genesis_key.pub)
				 .balance (lumex::dev::constants.genesis_amount - lumex::Klumex_ratio)
				 .link (lumex::dev::genesis_key.pub)
				 .sign (lumex::dev::genesis_key.prv, lumex::dev::genesis_key.pub)
				 .work (*pool.generate (lumex::dev::genesis->hash ()))
				 .build ();
	ASSERT_EQ (lumex::block_status::progress, ledger.process (transaction, send1));
	lumex::keypair rep;
	auto change1 = builder
				   .change ()
				   .previous (send1->hash ())
				   .representative (rep.pub)
				   .sign (lumex::dev::genesis_key.prv, lumex::dev::genesis_key.pub)
				   .work (*pool.generate (send1->hash ()))
				   .build ();
	ASSERT_EQ (lumex::block_status::block_position, ledger.process (transaction, change1));
}

TEST (ledger, state_unreceivable_fail)
{
	auto ctx = lumex::test::ledger_empty ();
	auto & ledger = ctx.ledger ();
	auto & store = ctx.store ();
	auto transaction = ledger.tx_begin_write ();
	auto & pool = ctx.pool ();
	lumex::block_builder builder;
	auto send1 = builder
				 .send ()
				 .previous (lumex::dev::genesis->hash ())
				 .destination (lumex::dev::genesis_key.pub)
				 .balance (lumex::dev::constants.genesis_amount - lumex::Klumex_ratio)
				 .sign (lumex::dev::genesis_key.prv, lumex::dev::genesis_key.pub)
				 .work (*pool.generate (lumex::dev::genesis->hash ()))
				 .build ();
	ASSERT_EQ (lumex::block_status::progress, ledger.process (transaction, send1));
	ASSERT_TRUE (ledger.any.block_exists (transaction, send1->hash ()));
	auto send2 = ledger.any.block_get (transaction, send1->hash ());
	ASSERT_NE (nullptr, send2);
	ASSERT_EQ (*send1, *send2);
	ASSERT_EQ (lumex::dev::constants.genesis_amount - lumex::Klumex_ratio, ledger.any.block_balance (transaction, send1->hash ()));
	ASSERT_EQ (lumex::Klumex_ratio, ledger.any.block_amount (transaction, send1->hash ()));
	ASSERT_EQ (lumex::dev::constants.genesis_amount - lumex::Klumex_ratio, ledger.weight (lumex::dev::genesis_key.pub));
	auto receive1 = builder
					.state ()
					.account (lumex::dev::genesis_key.pub)
					.previous (send1->hash ())
					.representative (lumex::dev::genesis_key.pub)
					.balance (lumex::dev::constants.genesis_amount)
					.link (1)
					.sign (lumex::dev::genesis_key.prv, lumex::dev::genesis_key.pub)
					.work (*pool.generate (send1->hash ()))
					.build ();
	ASSERT_EQ (lumex::block_status::gap_source, ledger.process (transaction, receive1));
}

TEST (ledger, state_receive_bad_amount_fail)
{
	auto ctx = lumex::test::ledger_empty ();
	auto & ledger = ctx.ledger ();
	auto & store = ctx.store ();
	auto transaction = ledger.tx_begin_write ();
	auto & pool = ctx.pool ();
	lumex::block_builder builder;
	auto send1 = builder
				 .send ()
				 .previous (lumex::dev::genesis->hash ())
				 .destination (lumex::dev::genesis_key.pub)
				 .balance (lumex::dev::constants.genesis_amount - lumex::Klumex_ratio)
				 .sign (lumex::dev::genesis_key.prv, lumex::dev::genesis_key.pub)
				 .work (*pool.generate (lumex::dev::genesis->hash ()))
				 .build ();
	ASSERT_EQ (lumex::block_status::progress, ledger.process (transaction, send1));
	ASSERT_TRUE (ledger.any.block_exists (transaction, send1->hash ()));
	auto send2 = ledger.any.block_get (transaction, send1->hash ());
	ASSERT_NE (nullptr, send2);
	ASSERT_EQ (*send1, *send2);
	ASSERT_EQ (lumex::dev::constants.genesis_amount - lumex::Klumex_ratio, ledger.any.block_balance (transaction, send1->hash ()));
	ASSERT_EQ (lumex::Klumex_ratio, ledger.any.block_amount (transaction, send1->hash ()));
	ASSERT_EQ (lumex::dev::constants.genesis_amount - lumex::Klumex_ratio, ledger.weight (lumex::dev::genesis_key.pub));
	auto receive1 = builder
					.state ()
					.account (lumex::dev::genesis_key.pub)
					.previous (send1->hash ())
					.representative (lumex::dev::genesis_key.pub)
					.balance (lumex::dev::constants.genesis_amount - lumex::Klumex_ratio)
					.link (send1->hash ())
					.sign (lumex::dev::genesis_key.prv, lumex::dev::genesis_key.pub)
					.work (*pool.generate (send1->hash ()))
					.build ();
	ASSERT_EQ (lumex::block_status::balance_mismatch, ledger.process (transaction, receive1));
}

TEST (ledger, state_no_link_amount_fail)
{
	auto ctx = lumex::test::ledger_empty ();
	auto & ledger = ctx.ledger ();
	auto & store = ctx.store ();
	auto transaction = ledger.tx_begin_write ();
	auto & pool = ctx.pool ();
	lumex::block_builder builder;
	auto send1 = builder
				 .state ()
				 .account (lumex::dev::genesis_key.pub)
				 .previous (lumex::dev::genesis->hash ())
				 .representative (lumex::dev::genesis_key.pub)
				 .balance (lumex::dev::constants.genesis_amount - lumex::Klumex_ratio)
				 .link (lumex::dev::genesis_key.pub)
				 .sign (lumex::dev::genesis_key.prv, lumex::dev::genesis_key.pub)
				 .work (*pool.generate (lumex::dev::genesis->hash ()))
				 .build ();
	ASSERT_EQ (lumex::block_status::progress, ledger.process (transaction, send1));
	lumex::keypair rep;
	auto change1 = builder
				   .state ()
				   .account (lumex::dev::genesis_key.pub)
				   .previous (send1->hash ())
				   .representative (rep.pub)
				   .balance (lumex::dev::constants.genesis_amount)
				   .link (0)
				   .sign (lumex::dev::genesis_key.prv, lumex::dev::genesis_key.pub)
				   .work (*pool.generate (send1->hash ()))
				   .build ();
	ASSERT_EQ (lumex::block_status::balance_mismatch, ledger.process (transaction, change1));
}

TEST (ledger, state_receive_wrong_account_fail)
{
	auto ctx = lumex::test::ledger_empty ();
	auto & ledger = ctx.ledger ();
	auto & store = ctx.store ();
	auto transaction = ledger.tx_begin_write ();
	auto & pool = ctx.pool ();
	lumex::block_builder builder;
	auto send1 = builder
				 .state ()
				 .account (lumex::dev::genesis_key.pub)
				 .previous (lumex::dev::genesis->hash ())
				 .representative (lumex::dev::genesis_key.pub)
				 .balance (lumex::dev::constants.genesis_amount - lumex::Klumex_ratio)
				 .link (lumex::dev::genesis_key.pub)
				 .sign (lumex::dev::genesis_key.prv, lumex::dev::genesis_key.pub)
				 .work (*pool.generate (lumex::dev::genesis->hash ()))
				 .build ();
	ASSERT_EQ (lumex::block_status::progress, ledger.process (transaction, send1));
	ASSERT_TRUE (ledger.any.block_exists (transaction, send1->hash ()));
	auto send2 = ledger.any.block_get (transaction, send1->hash ());
	ASSERT_NE (nullptr, send2);
	ASSERT_EQ (*send1, *send2);
	ASSERT_EQ (lumex::dev::constants.genesis_amount - lumex::Klumex_ratio, ledger.any.block_balance (transaction, send1->hash ()));
	ASSERT_EQ (lumex::Klumex_ratio, ledger.any.block_amount (transaction, send1->hash ()));
	ASSERT_EQ (lumex::dev::constants.genesis_amount - lumex::Klumex_ratio, ledger.weight (lumex::dev::genesis_key.pub));
	lumex::keypair key;
	auto receive1 = builder
					.state ()
					.account (key.pub)
					.previous (0)
					.representative (lumex::dev::genesis_key.pub)
					.balance (lumex::Klumex_ratio)
					.link (send1->hash ())
					.sign (key.prv, key.pub)
					.work (*pool.generate (key.pub))
					.build ();
	ASSERT_EQ (lumex::block_status::unreceivable, ledger.process (transaction, receive1));
}

TEST (ledger, state_open_state_fork)
{
	auto ctx = lumex::test::ledger_empty ();
	auto & ledger = ctx.ledger ();
	auto & store = ctx.store ();
	auto transaction = ledger.tx_begin_write ();
	auto & pool = ctx.pool ();
	lumex::keypair destination;
	lumex::block_builder builder;
	auto send1 = builder
				 .state ()
				 .account (lumex::dev::genesis_key.pub)
				 .previous (lumex::dev::genesis->hash ())
				 .representative (lumex::dev::genesis_key.pub)
				 .balance (lumex::dev::constants.genesis_amount - lumex::Klumex_ratio)
				 .link (destination.pub)
				 .sign (lumex::dev::genesis_key.prv, lumex::dev::genesis_key.pub)
				 .work (*pool.generate (lumex::dev::genesis->hash ()))
				 .build ();
	ASSERT_EQ (lumex::block_status::progress, ledger.process (transaction, send1));
	auto open1 = builder
				 .state ()
				 .account (destination.pub)
				 .previous (0)
				 .representative (lumex::dev::genesis_key.pub)
				 .balance (lumex::Klumex_ratio)
				 .link (send1->hash ())
				 .sign (destination.prv, destination.pub)
				 .work (*pool.generate (destination.pub))
				 .build ();
	ASSERT_EQ (lumex::block_status::progress, ledger.process (transaction, open1));
	auto open2 = builder
				 .open ()
				 .source (send1->hash ())
				 .representative (lumex::dev::genesis_key.pub)
				 .account (destination.pub)
				 .sign (destination.prv, destination.pub)
				 .work (*pool.generate (destination.pub))
				 .build ();
	ASSERT_EQ (lumex::block_status::fork, ledger.process (transaction, open2));
	ASSERT_EQ (open1->root (), open2->root ());
}

TEST (ledger, state_state_open_fork)
{
	auto ctx = lumex::test::ledger_empty ();
	auto & ledger = ctx.ledger ();
	auto & store = ctx.store ();
	auto transaction = ledger.tx_begin_write ();
	auto & pool = ctx.pool ();
	lumex::keypair destination;
	lumex::block_builder builder;
	auto send1 = builder
				 .state ()
				 .account (lumex::dev::genesis_key.pub)
				 .previous (lumex::dev::genesis->hash ())
				 .representative (lumex::dev::genesis_key.pub)
				 .balance (lumex::dev::constants.genesis_amount - lumex::Klumex_ratio)
				 .link (destination.pub)
				 .sign (lumex::dev::genesis_key.prv, lumex::dev::genesis_key.pub)
				 .work (*pool.generate (lumex::dev::genesis->hash ()))
				 .build ();
	ASSERT_EQ (lumex::block_status::progress, ledger.process (transaction, send1));
	auto open1 = builder
				 .open ()
				 .source (send1->hash ())
				 .representative (lumex::dev::genesis_key.pub)
				 .account (destination.pub)
				 .sign (destination.prv, destination.pub)
				 .work (*pool.generate (destination.pub))
				 .build ();
	ASSERT_EQ (lumex::block_status::progress, ledger.process (transaction, open1));
	auto open2 = builder
				 .state ()
				 .account (destination.pub)
				 .previous (0)
				 .representative (lumex::dev::genesis_key.pub)
				 .balance (lumex::Klumex_ratio)
				 .link (send1->hash ())
				 .sign (destination.prv, destination.pub)
				 .work (*pool.generate (destination.pub))
				 .build ();
	ASSERT_EQ (lumex::block_status::fork, ledger.process (transaction, open2));
	ASSERT_EQ (open1->root (), open2->root ());
	ASSERT_EQ (store.account.count (transaction), ledger.account_count ());
}

TEST (ledger, state_open_previous_fail)
{
	auto ctx = lumex::test::ledger_empty ();
	auto & ledger = ctx.ledger ();
	auto & store = ctx.store ();
	auto transaction = ledger.tx_begin_write ();
	auto & pool = ctx.pool ();
	lumex::keypair destination;
	lumex::block_builder builder;
	auto send1 = builder
				 .state ()
				 .account (lumex::dev::genesis_key.pub)
				 .previous (lumex::dev::genesis->hash ())
				 .representative (lumex::dev::genesis_key.pub)
				 .balance (lumex::dev::constants.genesis_amount - lumex::Klumex_ratio)
				 .link (destination.pub)
				 .sign (lumex::dev::genesis_key.prv, lumex::dev::genesis_key.pub)
				 .work (*pool.generate (lumex::dev::genesis->hash ()))
				 .build ();
	ASSERT_EQ (lumex::block_status::progress, ledger.process (transaction, send1));
	auto open1 = builder
				 .state ()
				 .account (destination.pub)
				 .previous (1)
				 .representative (lumex::dev::genesis_key.pub)
				 .balance (lumex::Klumex_ratio)
				 .link (send1->hash ())
				 .sign (destination.prv, destination.pub)
				 .work (*pool.generate (1))
				 .build ();
	ASSERT_EQ (lumex::block_status::gap_previous, ledger.process (transaction, open1));
}

TEST (ledger, state_open_source_fail)
{
	auto ctx = lumex::test::ledger_empty ();
	auto & ledger = ctx.ledger ();
	auto & store = ctx.store ();
	auto transaction = ledger.tx_begin_write ();
	auto & pool = ctx.pool ();
	lumex::keypair destination;
	lumex::block_builder builder;
	auto send1 = builder
				 .state ()
				 .account (lumex::dev::genesis_key.pub)
				 .previous (lumex::dev::genesis->hash ())
				 .representative (lumex::dev::genesis_key.pub)
				 .balance (lumex::dev::constants.genesis_amount - lumex::Klumex_ratio)
				 .link (destination.pub)
				 .sign (lumex::dev::genesis_key.prv, lumex::dev::genesis_key.pub)
				 .work (*pool.generate (lumex::dev::genesis->hash ()))
				 .build ();
	ASSERT_EQ (lumex::block_status::progress, ledger.process (transaction, send1));
	auto open1 = builder
				 .state ()
				 .account (destination.pub)
				 .previous (0)
				 .representative (lumex::dev::genesis_key.pub)
				 .balance (0)
				 .link (0)
				 .sign (destination.prv, destination.pub)
				 .work (*pool.generate (destination.pub))
				 .build ();
	ASSERT_EQ (lumex::block_status::gap_source, ledger.process (transaction, open1));
}

TEST (ledger, state_send_change)
{
	auto ctx = lumex::test::ledger_empty ();
	auto & ledger = ctx.ledger ();
	auto & store = ctx.store ();
	auto transaction = ledger.tx_begin_write ();
	auto & pool = ctx.pool ();
	lumex::keypair rep;
	lumex::block_builder builder;
	auto send1 = builder
				 .state ()
				 .account (lumex::dev::genesis_key.pub)
				 .previous (lumex::dev::genesis->hash ())
				 .representative (rep.pub)
				 .balance (lumex::dev::constants.genesis_amount - lumex::Klumex_ratio)
				 .link (lumex::dev::genesis_key.pub)
				 .sign (lumex::dev::genesis_key.prv, lumex::dev::genesis_key.pub)
				 .work (*pool.generate (lumex::dev::genesis->hash ()))
				 .build ();
	ASSERT_EQ (lumex::block_status::progress, ledger.process (transaction, send1));
	ASSERT_TRUE (ledger.any.block_exists (transaction, send1->hash ()));
	auto send2 = ledger.any.block_get (transaction, send1->hash ());
	ASSERT_NE (nullptr, send2);
	ASSERT_EQ (*send1, *send2);
	ASSERT_EQ (lumex::dev::constants.genesis_amount - lumex::Klumex_ratio, ledger.any.block_balance (transaction, send1->hash ()));
	ASSERT_EQ (lumex::Klumex_ratio, ledger.any.block_amount (transaction, send1->hash ()));
	ASSERT_EQ (0, ledger.weight (lumex::dev::genesis_key.pub));
	ASSERT_EQ (lumex::dev::constants.genesis_amount - lumex::Klumex_ratio, ledger.weight (rep.pub));
	ASSERT_EQ (2, send2->sideband ().height);
	ASSERT_TRUE (send2->is_send ());
	ASSERT_FALSE (send2->is_receive ());
	ASSERT_FALSE (send2->sideband ().details.is_epoch);
}

TEST (ledger, state_receive_change)
{
	auto ctx = lumex::test::ledger_empty ();
	auto & ledger = ctx.ledger ();
	auto & store = ctx.store ();
	auto transaction = ledger.tx_begin_write ();
	auto & pool = ctx.pool ();
	lumex::block_builder builder;
	auto send1 = builder
				 .state ()
				 .account (lumex::dev::genesis_key.pub)
				 .previous (lumex::dev::genesis->hash ())
				 .representative (lumex::dev::genesis_key.pub)
				 .balance (lumex::dev::constants.genesis_amount - lumex::Klumex_ratio)
				 .link (lumex::dev::genesis_key.pub)
				 .sign (lumex::dev::genesis_key.prv, lumex::dev::genesis_key.pub)
				 .work (*pool.generate (lumex::dev::genesis->hash ()))
				 .build ();
	ASSERT_EQ (lumex::block_status::progress, ledger.process (transaction, send1));
	ASSERT_TRUE (ledger.any.block_exists (transaction, send1->hash ()));
	auto send2 = ledger.any.block_get (transaction, send1->hash ());
	ASSERT_NE (nullptr, send2);
	ASSERT_EQ (*send1, *send2);
	ASSERT_EQ (lumex::dev::constants.genesis_amount - lumex::Klumex_ratio, ledger.any.block_balance (transaction, send1->hash ()));
	ASSERT_EQ (lumex::Klumex_ratio, ledger.any.block_amount (transaction, send1->hash ()));
	ASSERT_EQ (lumex::dev::constants.genesis_amount - lumex::Klumex_ratio, ledger.weight (lumex::dev::genesis_key.pub));
	lumex::keypair rep;
	auto receive1 = builder
					.state ()
					.account (lumex::dev::genesis_key.pub)
					.previous (send1->hash ())
					.representative (rep.pub)
					.balance (lumex::dev::constants.genesis_amount)
					.link (send1->hash ())
					.sign (lumex::dev::genesis_key.prv, lumex::dev::genesis_key.pub)
					.work (*pool.generate (send1->hash ()))
					.build ();
	ASSERT_EQ (lumex::block_status::progress, ledger.process (transaction, receive1));
	ASSERT_TRUE (ledger.any.block_exists (transaction, receive1->hash ()));
	auto receive2 = ledger.any.block_get (transaction, receive1->hash ());
	ASSERT_NE (nullptr, receive2);
	ASSERT_EQ (*receive1, *receive2);
	ASSERT_EQ (lumex::dev::constants.genesis_amount, ledger.any.block_balance (transaction, receive1->hash ()));
	ASSERT_EQ (lumex::Klumex_ratio, ledger.any.block_amount (transaction, receive1->hash ()));
	ASSERT_EQ (0, ledger.weight (lumex::dev::genesis_key.pub));
	ASSERT_EQ (lumex::dev::constants.genesis_amount, ledger.weight (rep.pub));
	ASSERT_EQ (3, receive2->sideband ().height);
	ASSERT_FALSE (receive2->is_send ());
	ASSERT_TRUE (receive2->is_receive ());
	ASSERT_FALSE (receive2->sideband ().details.is_epoch);
}

TEST (ledger, state_open_old)
{
	auto ctx = lumex::test::ledger_empty ();
	auto & ledger = ctx.ledger ();
	auto & store = ctx.store ();
	auto transaction = ledger.tx_begin_write ();
	auto & pool = ctx.pool ();
	lumex::keypair destination;
	lumex::block_builder builder;
	auto send1 = builder
				 .state ()
				 .account (lumex::dev::genesis_key.pub)
				 .previous (lumex::dev::genesis->hash ())
				 .representative (lumex::dev::genesis_key.pub)
				 .balance (lumex::dev::constants.genesis_amount - lumex::Klumex_ratio)
				 .link (destination.pub)
				 .sign (lumex::dev::genesis_key.prv, lumex::dev::genesis_key.pub)
				 .work (*pool.generate (lumex::dev::genesis->hash ()))
				 .build ();
	ASSERT_EQ (lumex::block_status::progress, ledger.process (transaction, send1));
	auto open1 = builder
				 .open ()
				 .source (send1->hash ())
				 .representative (lumex::dev::genesis_key.pub)
				 .account (destination.pub)
				 .sign (destination.prv, destination.pub)
				 .work (*pool.generate (destination.pub))
				 .build ();
	ASSERT_EQ (lumex::block_status::progress, ledger.process (transaction, open1));
	ASSERT_EQ (lumex::Klumex_ratio, ledger.any.block_balance (transaction, open1->hash ()));
	ASSERT_EQ (lumex::Klumex_ratio, ledger.any.block_amount (transaction, open1->hash ()));
	ASSERT_EQ (lumex::dev::constants.genesis_amount, ledger.weight (lumex::dev::genesis_key.pub));
}

TEST (ledger, state_receive_old)
{
	auto ctx = lumex::test::ledger_empty ();
	auto & ledger = ctx.ledger ();
	auto & store = ctx.store ();
	auto transaction = ledger.tx_begin_write ();
	auto & pool = ctx.pool ();
	lumex::keypair destination;
	lumex::block_builder builder;
	auto send1 = builder
				 .state ()
				 .account (lumex::dev::genesis_key.pub)
				 .previous (lumex::dev::genesis->hash ())
				 .representative (lumex::dev::genesis_key.pub)
				 .balance (lumex::dev::constants.genesis_amount - lumex::Klumex_ratio)
				 .link (destination.pub)
				 .sign (lumex::dev::genesis_key.prv, lumex::dev::genesis_key.pub)
				 .work (*pool.generate (lumex::dev::genesis->hash ()))
				 .build ();
	ASSERT_EQ (lumex::block_status::progress, ledger.process (transaction, send1));
	auto send2 = builder
				 .state ()
				 .account (lumex::dev::genesis_key.pub)
				 .previous (send1->hash ())
				 .representative (lumex::dev::genesis_key.pub)
				 .balance (lumex::dev::constants.genesis_amount - (2 * lumex::Klumex_ratio))
				 .link (destination.pub)
				 .sign (lumex::dev::genesis_key.prv, lumex::dev::genesis_key.pub)
				 .work (*pool.generate (send1->hash ()))
				 .build ();
	ASSERT_EQ (lumex::block_status::progress, ledger.process (transaction, send2));
	auto open1 = builder
				 .open ()
				 .source (send1->hash ())
				 .representative (lumex::dev::genesis_key.pub)
				 .account (destination.pub)
				 .sign (destination.prv, destination.pub)
				 .work (*pool.generate (destination.pub))
				 .build ();
	ASSERT_EQ (lumex::block_status::progress, ledger.process (transaction, open1));
	auto receive1 = builder
					.receive ()
					.previous (open1->hash ())
					.source (send2->hash ())
					.sign (destination.prv, destination.pub)
					.work (*pool.generate (open1->hash ()))
					.build ();
	ASSERT_EQ (lumex::block_status::progress, ledger.process (transaction, receive1));
	ASSERT_EQ (2 * lumex::Klumex_ratio, ledger.any.block_balance (transaction, receive1->hash ()));
	ASSERT_EQ (lumex::Klumex_ratio, ledger.any.block_amount (transaction, receive1->hash ()));
	ASSERT_EQ (lumex::dev::constants.genesis_amount, ledger.weight (lumex::dev::genesis_key.pub));
}

TEST (ledger, state_rollback_send)
{
	auto ctx = lumex::test::ledger_empty ();
	auto & ledger = ctx.ledger ();
	auto & store = ctx.store ();
	auto transaction = ledger.tx_begin_write ();
	auto & pool = ctx.pool ();
	lumex::block_builder builder;
	auto send1 = builder
				 .state ()
				 .account (lumex::dev::genesis_key.pub)
				 .previous (lumex::dev::genesis->hash ())
				 .representative (lumex::dev::genesis_key.pub)
				 .balance (lumex::dev::constants.genesis_amount - lumex::Klumex_ratio)
				 .link (lumex::dev::genesis_key.pub)
				 .sign (lumex::dev::genesis_key.prv, lumex::dev::genesis_key.pub)
				 .work (*pool.generate (lumex::dev::genesis->hash ()))
				 .build ();
	ASSERT_EQ (lumex::block_status::progress, ledger.process (transaction, send1));
	ASSERT_TRUE (ledger.any.block_exists (transaction, send1->hash ()));
	auto send2 = ledger.any.block_get (transaction, send1->hash ());
	ASSERT_NE (nullptr, send2);
	ASSERT_EQ (*send1, *send2);
	ASSERT_EQ (lumex::dev::constants.genesis_amount - lumex::Klumex_ratio, ledger.any.account_balance (transaction, lumex::dev::genesis_key.pub));
	ASSERT_EQ (lumex::dev::constants.genesis_amount - lumex::Klumex_ratio, ledger.weight (lumex::dev::genesis_key.pub));
	auto info = ledger.any.pending_get (transaction, lumex::pending_key (lumex::dev::genesis_key.pub, send1->hash ()));
	ASSERT_TRUE (info);
	ASSERT_EQ (lumex::dev::genesis_key.pub, info->source);
	ASSERT_EQ (lumex::Klumex_ratio, info->amount.number ());
	ASSERT_FALSE (ledger.rollback (transaction, send1->hash ()));
	ASSERT_FALSE (ledger.any.block_exists (transaction, send1->hash ()));
	ASSERT_EQ (lumex::dev::constants.genesis_amount, ledger.any.account_balance (transaction, lumex::dev::genesis_key.pub));
	ASSERT_EQ (lumex::dev::constants.genesis_amount, ledger.weight (lumex::dev::genesis_key.pub));
	ASSERT_FALSE (ledger.any.pending_get (transaction, lumex::pending_key{ lumex::dev::genesis_key.pub, send1->hash () }));
	ASSERT_FALSE (ledger.any.block_successor (transaction, lumex::dev::genesis->hash ()));
	ASSERT_EQ (store.account.count (transaction), ledger.account_count ());
}

TEST (ledger, state_rollback_receive)
{
	auto ctx = lumex::test::ledger_empty ();
	auto & ledger = ctx.ledger ();
	auto & store = ctx.store ();
	auto transaction = ledger.tx_begin_write ();
	auto & pool = ctx.pool ();
	lumex::block_builder builder;
	auto send1 = builder
				 .state ()
				 .account (lumex::dev::genesis_key.pub)
				 .previous (lumex::dev::genesis->hash ())
				 .representative (lumex::dev::genesis_key.pub)
				 .balance (lumex::dev::constants.genesis_amount - lumex::Klumex_ratio)
				 .link (lumex::dev::genesis_key.pub)
				 .sign (lumex::dev::genesis_key.prv, lumex::dev::genesis_key.pub)
				 .work (*pool.generate (lumex::dev::genesis->hash ()))
				 .build ();
	ASSERT_EQ (lumex::block_status::progress, ledger.process (transaction, send1));
	auto receive1 = builder
					.state ()
					.account (lumex::dev::genesis_key.pub)
					.previous (send1->hash ())
					.representative (lumex::dev::genesis_key.pub)
					.balance (lumex::dev::constants.genesis_amount)
					.link (send1->hash ())
					.sign (lumex::dev::genesis_key.prv, lumex::dev::genesis_key.pub)
					.work (*pool.generate (send1->hash ()))
					.build ();
	ASSERT_EQ (lumex::block_status::progress, ledger.process (transaction, receive1));
	ASSERT_FALSE (ledger.any.pending_get (transaction, lumex::pending_key{ lumex::dev::genesis_key.pub, receive1->hash () }));
	ASSERT_FALSE (ledger.rollback (transaction, receive1->hash ()));
	auto info = ledger.any.pending_get (transaction, lumex::pending_key (lumex::dev::genesis_key.pub, send1->hash ()));
	ASSERT_TRUE (info);
	ASSERT_EQ (lumex::dev::genesis_key.pub, info->source);
	ASSERT_EQ (lumex::Klumex_ratio, info->amount.number ());
	ASSERT_FALSE (ledger.any.block_exists (transaction, receive1->hash ()));
	ASSERT_EQ (lumex::dev::constants.genesis_amount - lumex::Klumex_ratio, ledger.any.account_balance (transaction, lumex::dev::genesis_key.pub));
	ASSERT_EQ (lumex::dev::constants.genesis_amount - lumex::Klumex_ratio, ledger.weight (lumex::dev::genesis_key.pub));
	ASSERT_EQ (store.account.count (transaction), ledger.account_count ());
}

TEST (ledger, state_rollback_received_send)
{
	auto ctx = lumex::test::ledger_empty ();
	auto & ledger = ctx.ledger ();
	auto & store = ctx.store ();
	auto transaction = ledger.tx_begin_write ();
	auto & pool = ctx.pool ();
	lumex::keypair key;
	lumex::block_builder builder;
	auto send1 = builder
				 .state ()
				 .account (lumex::dev::genesis_key.pub)
				 .previous (lumex::dev::genesis->hash ())
				 .representative (lumex::dev::genesis_key.pub)
				 .balance (lumex::dev::constants.genesis_amount - lumex::Klumex_ratio)
				 .link (key.pub)
				 .sign (lumex::dev::genesis_key.prv, lumex::dev::genesis_key.pub)
				 .work (*pool.generate (lumex::dev::genesis->hash ()))
				 .build ();
	ASSERT_EQ (lumex::block_status::progress, ledger.process (transaction, send1));
	auto receive1 = builder
					.state ()
					.account (key.pub)
					.previous (0)
					.representative (key.pub)
					.balance (lumex::Klumex_ratio)
					.link (send1->hash ())
					.sign (key.prv, key.pub)
					.work (*pool.generate (key.pub))
					.build ();
	ASSERT_EQ (lumex::block_status::progress, ledger.process (transaction, receive1));
	ASSERT_FALSE (ledger.any.pending_get (transaction, lumex::pending_key{ lumex::dev::genesis_key.pub, receive1->hash () }));
	ASSERT_FALSE (ledger.rollback (transaction, send1->hash ()));
	ASSERT_FALSE (ledger.any.pending_get (transaction, lumex::pending_key{ lumex::dev::genesis_key.pub, send1->hash () }));
	ASSERT_FALSE (ledger.any.block_exists (transaction, send1->hash ()));
	ASSERT_FALSE (ledger.any.block_exists (transaction, receive1->hash ()));
	ASSERT_EQ (lumex::dev::constants.genesis_amount, ledger.any.account_balance (transaction, lumex::dev::genesis_key.pub));
	ASSERT_EQ (lumex::dev::constants.genesis_amount, ledger.weight (lumex::dev::genesis_key.pub));
	ASSERT_FALSE (ledger.any.account_balance (transaction, key.pub));
	ASSERT_EQ (0, ledger.weight (key.pub));
	ASSERT_EQ (store.account.count (transaction), ledger.account_count ());
}

TEST (ledger, state_rep_change_rollback)
{
	auto ctx = lumex::test::ledger_empty ();
	auto & ledger = ctx.ledger ();
	auto & store = ctx.store ();
	auto transaction = ledger.tx_begin_write ();
	auto & pool = ctx.pool ();
	lumex::keypair rep;
	lumex::block_builder builder;
	auto change1 = builder
				   .state ()
				   .account (lumex::dev::genesis_key.pub)
				   .previous (lumex::dev::genesis->hash ())
				   .representative (rep.pub)
				   .balance (lumex::dev::constants.genesis_amount)
				   .link (0)
				   .sign (lumex::dev::genesis_key.prv, lumex::dev::genesis_key.pub)
				   .work (*pool.generate (lumex::dev::genesis->hash ()))
				   .build ();
	ASSERT_EQ (lumex::block_status::progress, ledger.process (transaction, change1));
	ASSERT_FALSE (ledger.rollback (transaction, change1->hash ()));
	ASSERT_FALSE (ledger.any.block_exists (transaction, change1->hash ()));
	ASSERT_EQ (lumex::dev::constants.genesis_amount, ledger.any.account_balance (transaction, lumex::dev::genesis_key.pub));
	ASSERT_EQ (lumex::dev::constants.genesis_amount, ledger.weight (lumex::dev::genesis_key.pub));
	ASSERT_EQ (0, ledger.weight (rep.pub));
}

TEST (ledger, state_open_rollback)
{
	auto ctx = lumex::test::ledger_empty ();
	auto & ledger = ctx.ledger ();
	auto & store = ctx.store ();
	auto transaction = ledger.tx_begin_write ();
	auto & pool = ctx.pool ();
	lumex::keypair destination;
	lumex::block_builder builder;
	auto send1 = builder
				 .state ()
				 .account (lumex::dev::genesis_key.pub)
				 .previous (lumex::dev::genesis->hash ())
				 .representative (lumex::dev::genesis_key.pub)
				 .balance (lumex::dev::constants.genesis_amount - lumex::Klumex_ratio)
				 .link (destination.pub)
				 .sign (lumex::dev::genesis_key.prv, lumex::dev::genesis_key.pub)
				 .work (*pool.generate (lumex::dev::genesis->hash ()))
				 .build ();
	ASSERT_EQ (lumex::block_status::progress, ledger.process (transaction, send1));
	auto open1 = builder
				 .state ()
				 .account (destination.pub)
				 .previous (0)
				 .representative (lumex::dev::genesis_key.pub)
				 .balance (lumex::Klumex_ratio)
				 .link (send1->hash ())
				 .sign (destination.prv, destination.pub)
				 .work (*pool.generate (destination.pub))
				 .build ();
	ASSERT_EQ (lumex::block_status::progress, ledger.process (transaction, open1));
	ASSERT_FALSE (ledger.rollback (transaction, open1->hash ()));
	ASSERT_FALSE (ledger.any.block_exists (transaction, open1->hash ()));
	ASSERT_FALSE (ledger.any.account_balance (transaction, destination.pub));
	ASSERT_EQ (lumex::dev::constants.genesis_amount - lumex::Klumex_ratio, ledger.weight (lumex::dev::genesis_key.pub));
	auto info = ledger.any.pending_get (transaction, lumex::pending_key (destination.pub, send1->hash ()));
	ASSERT_TRUE (info);
	ASSERT_EQ (lumex::dev::genesis_key.pub, info->source);
	ASSERT_EQ (lumex::Klumex_ratio, info->amount.number ());
	ASSERT_EQ (store.account.count (transaction), ledger.account_count ());
}

TEST (ledger, state_send_change_rollback)
{
	auto ctx = lumex::test::ledger_empty ();
	auto & ledger = ctx.ledger ();
	auto & store = ctx.store ();
	auto transaction = ledger.tx_begin_write ();
	auto & pool = ctx.pool ();
	lumex::keypair rep;
	lumex::block_builder builder;
	auto send1 = builder
				 .state ()
				 .account (lumex::dev::genesis_key.pub)
				 .previous (lumex::dev::genesis->hash ())
				 .representative (rep.pub)
				 .balance (lumex::dev::constants.genesis_amount - lumex::Klumex_ratio)
				 .link (lumex::dev::genesis_key.pub)
				 .sign (lumex::dev::genesis_key.prv, lumex::dev::genesis_key.pub)
				 .work (*pool.generate (lumex::dev::genesis->hash ()))
				 .build ();
	ASSERT_EQ (lumex::block_status::progress, ledger.process (transaction, send1));
	ASSERT_FALSE (ledger.rollback (transaction, send1->hash ()));
	ASSERT_FALSE (ledger.any.block_exists (transaction, send1->hash ()));
	ASSERT_EQ (lumex::dev::constants.genesis_amount, ledger.any.account_balance (transaction, lumex::dev::genesis_key.pub));
	ASSERT_EQ (lumex::dev::constants.genesis_amount, ledger.weight (lumex::dev::genesis_key.pub));
	ASSERT_EQ (0, ledger.weight (rep.pub));
	ASSERT_EQ (store.account.count (transaction), ledger.account_count ());
}

TEST (ledger, state_receive_change_rollback)
{
	auto ctx = lumex::test::ledger_empty ();
	auto & ledger = ctx.ledger ();
	auto & store = ctx.store ();
	auto transaction = ledger.tx_begin_write ();
	auto & pool = ctx.pool ();
	lumex::block_builder builder;
	auto send1 = builder
				 .state ()
				 .account (lumex::dev::genesis_key.pub)
				 .previous (lumex::dev::genesis->hash ())
				 .representative (lumex::dev::genesis_key.pub)
				 .balance (lumex::dev::constants.genesis_amount - lumex::Klumex_ratio)
				 .link (lumex::dev::genesis_key.pub)
				 .sign (lumex::dev::genesis_key.prv, lumex::dev::genesis_key.pub)
				 .work (*pool.generate (lumex::dev::genesis->hash ()))
				 .build ();
	ASSERT_EQ (lumex::block_status::progress, ledger.process (transaction, send1));
	lumex::keypair rep;
	auto receive1 = builder
					.state ()
					.account (lumex::dev::genesis_key.pub)
					.previous (send1->hash ())
					.representative (rep.pub)
					.balance (lumex::dev::constants.genesis_amount)
					.link (send1->hash ())
					.sign (lumex::dev::genesis_key.prv, lumex::dev::genesis_key.pub)
					.work (*pool.generate (send1->hash ()))
					.build ();
	ASSERT_EQ (lumex::block_status::progress, ledger.process (transaction, receive1));
	ASSERT_FALSE (ledger.rollback (transaction, receive1->hash ()));
	ASSERT_FALSE (ledger.any.block_exists (transaction, receive1->hash ()));
	ASSERT_EQ (lumex::dev::constants.genesis_amount - lumex::Klumex_ratio, ledger.any.account_balance (transaction, lumex::dev::genesis_key.pub));
	ASSERT_EQ (lumex::dev::constants.genesis_amount - lumex::Klumex_ratio, ledger.weight (lumex::dev::genesis_key.pub));
	ASSERT_EQ (0, ledger.weight (rep.pub));
	ASSERT_EQ (store.account.count (transaction), ledger.account_count ());
}

TEST (ledger, rollback_depth_exceeded)
{
	auto ctx = lumex::test::ledger_empty ();
	auto & ledger = ctx.ledger ();
	auto & store = ctx.store ();
	auto transaction = ledger.tx_begin_write ();
	auto & pool = ctx.pool ();

	// Chain: G->A0->A1
	lumex::keypair key1, key2;
	lumex::keypair rep1, rep2;
	lumex::block_builder builder;
	auto send1 = builder
				 .state ()
				 .account (lumex::dev::genesis_key.pub)
				 .previous (lumex::dev::genesis->hash ())
				 .representative (lumex::dev::genesis_key.pub)
				 .balance (lumex::dev::constants.genesis_amount - lumex::Klumex_ratio)
				 .link (key1.pub)
				 .sign (lumex::dev::genesis_key.prv, lumex::dev::genesis_key.pub)
				 .work (*pool.generate (lumex::dev::genesis->hash ()))
				 .build ();
	ASSERT_EQ (lumex::block_status::progress, ledger.process (transaction, send1));
	auto receive1 = builder
					.state ()
					.account (key1.pub)
					.previous (0)
					.representative (rep1.pub)
					.balance (lumex::Klumex_ratio)
					.link (send1->hash ())
					.sign (key1.prv, key1.pub)
					.work (*pool.generate (key1.pub))
					.build ();
	ASSERT_EQ (lumex::block_status::progress, ledger.process (transaction, receive1));
	auto send2 = builder
				 .state ()
				 .account (key1.pub)
				 .previous (receive1->hash ())
				 .representative (rep1.pub)
				 .balance (0)
				 .link (key2.pub)
				 .sign (key1.prv, key1.pub)
				 .work (*pool.generate (receive1->hash ()))
				 .build ();
	ASSERT_EQ (lumex::block_status::progress, ledger.process (transaction, send2));
	auto receive2 = builder
					.state ()
					.account (key2.pub)
					.previous (0)
					.representative (rep2.pub)
					.balance (lumex::Klumex_ratio)
					.link (send2->hash ())
					.sign (key2.prv, key2.pub)
					.work (*pool.generate (key2.pub))
					.build ();
	ASSERT_EQ (lumex::block_status::progress, ledger.process (transaction, receive2));

	// This call should fail because the rollback depth exceeds the limit
	std::deque<std::shared_ptr<lumex::block>> rollback_list;
	ASSERT_TRUE (ledger.rollback (transaction, send1->hash (), rollback_list, 0, 1));
	ASSERT_EQ (rollback_list.size (), 0);
	ASSERT_EQ (ledger.any.account_balance (transaction, key1.pub), 0);
	ASSERT_EQ (ledger.any.account_balance (transaction, key2.pub), lumex::Klumex_ratio);
	ASSERT_EQ (ledger.any.account_balance (transaction, lumex::dev::genesis_key.pub), lumex::dev::constants.genesis_amount - lumex::Klumex_ratio);
	ASSERT_EQ (ledger.weight (rep1.pub), 0);
	ASSERT_EQ (ledger.weight (rep2.pub), lumex::Klumex_ratio);
	ASSERT_EQ (ledger.weight (lumex::dev::genesis_key.pub), lumex::dev::constants.genesis_amount - lumex::Klumex_ratio);
	ASSERT_FALSE (ledger.any.pending_get (transaction, lumex::pending_key (key1.pub, send1->hash ())));
	ASSERT_FALSE (ledger.any.pending_get (transaction, lumex::pending_key (key2.pub, send2->hash ())));
}

TEST (ledger, epoch_blocks_v1_general)
{
	auto ctx = lumex::test::ledger_empty ();
	auto & ledger = ctx.ledger ();
	auto & store = ctx.store ();
	auto transaction = ledger.tx_begin_write ();
	auto & pool = ctx.pool ();
	lumex::keypair destination;
	lumex::block_builder builder;
	auto epoch1 = builder
				  .state ()
				  .account (lumex::dev::genesis_key.pub)
				  .previous (lumex::dev::genesis->hash ())
				  .representative (lumex::dev::genesis_key.pub)
				  .balance (lumex::dev::constants.genesis_amount)
				  .link (ledger.epoch_link (lumex::epoch::epoch_1))
				  .sign (lumex::dev::genesis_key.prv, lumex::dev::genesis_key.pub)
				  .work (*pool.generate (lumex::dev::genesis->hash ()))
				  .build ();
	ASSERT_EQ (lumex::block_status::progress, ledger.process (transaction, epoch1));
	ASSERT_FALSE (epoch1->is_send ());
	ASSERT_FALSE (epoch1->is_receive ());
	ASSERT_TRUE (epoch1->sideband ().details.is_epoch);
	ASSERT_EQ (lumex::epoch::epoch_1, epoch1->sideband ().details.epoch);
	ASSERT_EQ (lumex::epoch::epoch_0, epoch1->sideband ().source_epoch); // Not used for epoch blocks
	auto epoch2 = builder
				  .state ()
				  .account (lumex::dev::genesis_key.pub)
				  .previous (epoch1->hash ())
				  .representative (lumex::dev::genesis_key.pub)
				  .balance (lumex::dev::constants.genesis_amount)
				  .link (ledger.epoch_link (lumex::epoch::epoch_1))
				  .sign (lumex::dev::genesis_key.prv, lumex::dev::genesis_key.pub)
				  .work (*pool.generate (epoch1->hash ()))
				  .build ();
	ASSERT_EQ (lumex::block_status::block_position, ledger.process (transaction, epoch2));
	auto genesis_info = ledger.any.account_get (transaction, lumex::dev::genesis_key.pub);
	ASSERT_TRUE (genesis_info);
	ASSERT_EQ (genesis_info->epoch (), lumex::epoch::epoch_1);
	ASSERT_FALSE (ledger.rollback (transaction, epoch1->hash ()));
	genesis_info = ledger.any.account_get (transaction, lumex::dev::genesis_key.pub);
	ASSERT_TRUE (genesis_info);
	ASSERT_EQ (genesis_info->epoch (), lumex::epoch::epoch_0);
	ASSERT_EQ (lumex::block_status::progress, ledger.process (transaction, epoch1));
	genesis_info = ledger.any.account_get (transaction, lumex::dev::genesis_key.pub);
	ASSERT_TRUE (genesis_info);
	ASSERT_EQ (genesis_info->epoch (), lumex::epoch::epoch_1);
	ASSERT_FALSE (epoch1->is_send ());
	ASSERT_FALSE (epoch1->is_receive ());
	ASSERT_TRUE (epoch1->sideband ().details.is_epoch);
	ASSERT_EQ (lumex::epoch::epoch_1, epoch1->sideband ().details.epoch);
	ASSERT_EQ (lumex::epoch::epoch_0, epoch1->sideband ().source_epoch); // Not used for epoch blocks
	auto change1 = builder
				   .change ()
				   .previous (epoch1->hash ())
				   .representative (lumex::dev::genesis_key.pub)
				   .sign (lumex::dev::genesis_key.prv, lumex::dev::genesis_key.pub)
				   .work (*pool.generate (epoch1->hash ()))
				   .build ();
	ASSERT_EQ (lumex::block_status::block_position, ledger.process (transaction, change1));
	auto send1 = builder
				 .state ()
				 .account (lumex::dev::genesis_key.pub)
				 .previous (epoch1->hash ())
				 .representative (lumex::dev::genesis_key.pub)
				 .balance (lumex::dev::constants.genesis_amount - lumex::Klumex_ratio)
				 .link (destination.pub)
				 .sign (lumex::dev::genesis_key.prv, lumex::dev::genesis_key.pub)
				 .work (*pool.generate (epoch1->hash ()))
				 .build ();
	ASSERT_EQ (lumex::block_status::progress, ledger.process (transaction, send1));
	ASSERT_TRUE (send1->is_send ());
	ASSERT_FALSE (send1->is_receive ());
	ASSERT_FALSE (send1->sideband ().details.is_epoch);
	ASSERT_EQ (lumex::epoch::epoch_1, send1->sideband ().details.epoch);
	ASSERT_EQ (lumex::epoch::epoch_0, send1->sideband ().source_epoch); // Not used for send blocks
	auto open1 = builder
				 .open ()
				 .source (send1->hash ())
				 .representative (lumex::dev::genesis_key.pub)
				 .account (destination.pub)
				 .sign (destination.prv, destination.pub)
				 .work (*pool.generate (destination.pub))
				 .build ();
	ASSERT_EQ (lumex::block_status::unreceivable, ledger.process (transaction, open1));
	auto epoch3 = builder
				  .state ()
				  .account (destination.pub)
				  .previous (0)
				  .representative (lumex::dev::genesis_key.pub)
				  .balance (0)
				  .link (ledger.epoch_link (lumex::epoch::epoch_1))
				  .sign (lumex::dev::genesis_key.prv, lumex::dev::genesis_key.pub)
				  .work (*pool.generate (destination.pub))
				  .build ();
	ASSERT_EQ (lumex::block_status::representative_mismatch, ledger.process (transaction, epoch3));
	auto epoch4 = builder
				  .state ()
				  .account (destination.pub)
				  .previous (0)
				  .representative (0)
				  .balance (0)
				  .link (ledger.epoch_link (lumex::epoch::epoch_1))
				  .sign (lumex::dev::genesis_key.prv, lumex::dev::genesis_key.pub)
				  .work (*pool.generate (destination.pub))
				  .build ();
	ASSERT_EQ (lumex::block_status::progress, ledger.process (transaction, epoch4));
	ASSERT_FALSE (epoch4->is_send ());
	ASSERT_FALSE (epoch4->is_receive ());
	ASSERT_TRUE (epoch4->sideband ().details.is_epoch);
	ASSERT_EQ (lumex::epoch::epoch_1, epoch4->sideband ().details.epoch);
	ASSERT_EQ (lumex::epoch::epoch_0, epoch4->sideband ().source_epoch); // Not used for epoch blocks
	auto receive1 = builder
					.receive ()
					.previous (epoch4->hash ())
					.source (send1->hash ())
					.sign (destination.prv, destination.pub)
					.work (*pool.generate (epoch4->hash ()))
					.build ();
	ASSERT_EQ (lumex::block_status::block_position, ledger.process (transaction, receive1));
	auto receive2 = builder
					.state ()
					.account (destination.pub)
					.previous (epoch4->hash ())
					.representative (destination.pub)
					.balance (lumex::Klumex_ratio)
					.link (send1->hash ())
					.sign (destination.prv, destination.pub)
					.work (*pool.generate (epoch4->hash ()))
					.build ();
	ASSERT_EQ (lumex::block_status::progress, ledger.process (transaction, receive2));
	ASSERT_EQ (lumex::epoch::epoch_1, receive2->sideband ().details.epoch);
	ASSERT_EQ (lumex::epoch::epoch_1, receive2->sideband ().source_epoch);
	ASSERT_EQ (0, ledger.any.block_balance (transaction, epoch4->hash ()).value ().number ());
	ASSERT_EQ (lumex::Klumex_ratio, ledger.any.block_balance (transaction, receive2->hash ()));
	ASSERT_EQ (lumex::Klumex_ratio, ledger.any.block_amount (transaction, receive2->hash ()));
	ASSERT_EQ (lumex::dev::constants.genesis_amount - lumex::Klumex_ratio, ledger.weight (lumex::dev::genesis_key.pub));
	ASSERT_EQ (lumex::Klumex_ratio, ledger.weight (destination.pub));
	ASSERT_FALSE (receive2->is_send ());
	ASSERT_TRUE (receive2->is_receive ());
	ASSERT_FALSE (receive2->sideband ().details.is_epoch);
}

TEST (ledger, epoch_blocks_v2_general)
{
	auto ctx = lumex::test::ledger_empty ();
	auto & ledger = ctx.ledger ();
	auto & store = ctx.store ();
	auto transaction = ledger.tx_begin_write ();
	auto & pool = ctx.pool ();
	lumex::keypair destination;
	lumex::block_builder builder;
	auto epoch1 = builder
				  .state ()
				  .account (lumex::dev::genesis_key.pub)
				  .previous (lumex::dev::genesis->hash ())
				  .representative (lumex::dev::genesis_key.pub)
				  .balance (lumex::dev::constants.genesis_amount)
				  .link (ledger.epoch_link (lumex::epoch::epoch_2))
				  .sign (lumex::dev::genesis_key.prv, lumex::dev::genesis_key.pub)
				  .work (*pool.generate (lumex::dev::genesis->hash ()))
				  .build ();
	// Trying to upgrade from epoch 0 to epoch 2. It is a requirement epoch upgrades are sequential unless the account is unopened
	ASSERT_EQ (lumex::block_status::block_position, ledger.process (transaction, epoch1));
	// Set it to the first epoch and it should now succeed
	epoch1 = builder
			 .state ()
			 .account (lumex::dev::genesis_key.pub)
			 .previous (lumex::dev::genesis->hash ())
			 .representative (lumex::dev::genesis_key.pub)
			 .balance (lumex::dev::constants.genesis_amount)
			 .link (ledger.epoch_link (lumex::epoch::epoch_1))
			 .sign (lumex::dev::genesis_key.prv, lumex::dev::genesis_key.pub)
			 .work (epoch1->work)
			 .build ();
	ASSERT_EQ (lumex::block_status::progress, ledger.process (transaction, epoch1));
	ASSERT_EQ (lumex::epoch::epoch_1, epoch1->sideband ().details.epoch);
	ASSERT_EQ (lumex::epoch::epoch_0, epoch1->sideband ().source_epoch); // Not used for epoch blocks
	auto epoch2 = builder
				  .state ()
				  .account (lumex::dev::genesis_key.pub)
				  .previous (epoch1->hash ())
				  .representative (lumex::dev::genesis_key.pub)
				  .balance (lumex::dev::constants.genesis_amount)
				  .link (ledger.epoch_link (lumex::epoch::epoch_2))
				  .sign (lumex::dev::genesis_key.prv, lumex::dev::genesis_key.pub)
				  .work (*pool.generate (epoch1->hash ()))
				  .build ();
	ASSERT_EQ (lumex::block_status::progress, ledger.process (transaction, epoch2));
	ASSERT_EQ (lumex::epoch::epoch_2, epoch2->sideband ().details.epoch);
	ASSERT_EQ (lumex::epoch::epoch_0, epoch2->sideband ().source_epoch); // Not used for epoch blocks
	auto epoch3 = builder
				  .state ()
				  .account (lumex::dev::genesis_key.pub)
				  .previous (epoch2->hash ())
				  .representative (lumex::dev::genesis_key.pub)
				  .balance (lumex::dev::constants.genesis_amount)
				  .link (ledger.epoch_link (lumex::epoch::epoch_2))
				  .sign (lumex::dev::genesis_key.prv, lumex::dev::genesis_key.pub)
				  .work (*pool.generate (epoch2->hash ()))
				  .build ();
	ASSERT_EQ (lumex::block_status::block_position, ledger.process (transaction, epoch3));
	auto genesis_info = ledger.any.account_get (transaction, lumex::dev::genesis_key.pub);
	ASSERT_TRUE (genesis_info);
	ASSERT_EQ (genesis_info->epoch (), lumex::epoch::epoch_2);
	ASSERT_FALSE (ledger.rollback (transaction, epoch1->hash ()));
	genesis_info = ledger.any.account_get (transaction, lumex::dev::genesis_key.pub);
	ASSERT_TRUE (genesis_info);
	ASSERT_EQ (genesis_info->epoch (), lumex::epoch::epoch_0);
	ASSERT_EQ (lumex::block_status::progress, ledger.process (transaction, epoch1));
	genesis_info = ledger.any.account_get (transaction, lumex::dev::genesis_key.pub);
	ASSERT_TRUE (genesis_info);
	ASSERT_EQ (genesis_info->epoch (), lumex::epoch::epoch_1);
	auto change1 = builder
				   .change ()
				   .previous (epoch1->hash ())
				   .representative (lumex::dev::genesis_key.pub)
				   .sign (lumex::dev::genesis_key.prv, lumex::dev::genesis_key.pub)
				   .work (*pool.generate (epoch1->hash ()))
				   .build ();
	ASSERT_EQ (lumex::block_status::block_position, ledger.process (transaction, change1));
	auto send1 = builder
				 .state ()
				 .account (lumex::dev::genesis_key.pub)
				 .previous (epoch1->hash ())
				 .representative (lumex::dev::genesis_key.pub)
				 .balance (lumex::dev::constants.genesis_amount - lumex::Klumex_ratio)
				 .link (destination.pub)
				 .sign (lumex::dev::genesis_key.prv, lumex::dev::genesis_key.pub)
				 .work (*pool.generate (epoch1->hash ()))
				 .build ();
	ASSERT_EQ (lumex::block_status::progress, ledger.process (transaction, send1));
	ASSERT_EQ (lumex::epoch::epoch_1, send1->sideband ().details.epoch);
	ASSERT_EQ (lumex::epoch::epoch_0, send1->sideband ().source_epoch); // Not used for send blocks
	auto open1 = builder
				 .open ()
				 .source (send1->hash ())
				 .representative (lumex::dev::genesis_key.pub)
				 .account (destination.pub)
				 .sign (destination.prv, destination.pub)
				 .work (*pool.generate (destination.pub))
				 .build ();
	ASSERT_EQ (lumex::block_status::unreceivable, ledger.process (transaction, open1));
	auto epoch4 = builder
				  .state ()
				  .account (destination.pub)
				  .previous (0)
				  .representative (0)
				  .balance (0)
				  .link (ledger.epoch_link (lumex::epoch::epoch_1))
				  .sign (lumex::dev::genesis_key.prv, lumex::dev::genesis_key.pub)
				  .work (*pool.generate (destination.pub))
				  .build ();
	ASSERT_EQ (lumex::block_status::progress, ledger.process (transaction, epoch4));
	ASSERT_EQ (lumex::epoch::epoch_1, epoch4->sideband ().details.epoch);
	ASSERT_EQ (lumex::epoch::epoch_0, epoch4->sideband ().source_epoch); // Not used for epoch blocks
	auto epoch5 = builder
				  .state ()
				  .account (destination.pub)
				  .previous (epoch4->hash ())
				  .representative (lumex::dev::genesis_key.pub)
				  .balance (0)
				  .link (ledger.epoch_link (lumex::epoch::epoch_2))
				  .sign (lumex::dev::genesis_key.prv, lumex::dev::genesis_key.pub)
				  .work (*pool.generate (epoch4->hash ()))
				  .build ();
	ASSERT_EQ (lumex::block_status::representative_mismatch, ledger.process (transaction, epoch5));
	auto epoch6 = builder
				  .state ()
				  .account (destination.pub)
				  .previous (epoch4->hash ())
				  .representative (0)
				  .balance (0)
				  .link (ledger.epoch_link (lumex::epoch::epoch_2))
				  .sign (lumex::dev::genesis_key.prv, lumex::dev::genesis_key.pub)
				  .work (*pool.generate (epoch4->hash ()))
				  .build ();
	ASSERT_EQ (lumex::block_status::progress, ledger.process (transaction, epoch6));
	ASSERT_EQ (lumex::epoch::epoch_2, epoch6->sideband ().details.epoch);
	ASSERT_EQ (lumex::epoch::epoch_0, epoch6->sideband ().source_epoch); // Not used for epoch blocks
	auto receive1 = builder
					.receive ()
					.previous (epoch6->hash ())
					.source (send1->hash ())
					.sign (destination.prv, destination.pub)
					.work (*pool.generate (epoch6->hash ()))
					.build ();
	ASSERT_EQ (lumex::block_status::block_position, ledger.process (transaction, receive1));
	auto receive2 = builder
					.state ()
					.account (destination.pub)
					.previous (epoch6->hash ())
					.representative (destination.pub)
					.balance (lumex::Klumex_ratio)
					.link (send1->hash ())
					.sign (destination.prv, destination.pub)
					.work (*pool.generate (epoch6->hash ()))
					.build ();
	ASSERT_EQ (lumex::block_status::progress, ledger.process (transaction, receive2));
	ASSERT_EQ (lumex::epoch::epoch_2, receive2->sideband ().details.epoch);
	ASSERT_EQ (lumex::epoch::epoch_1, receive2->sideband ().source_epoch);
	ASSERT_EQ (0, ledger.any.block_balance (transaction, epoch6->hash ()).value ().number ());
	ASSERT_EQ (lumex::Klumex_ratio, ledger.any.block_balance (transaction, receive2->hash ()));
	ASSERT_EQ (lumex::Klumex_ratio, ledger.any.block_amount (transaction, receive2->hash ()));
	ASSERT_EQ (lumex::dev::constants.genesis_amount - lumex::Klumex_ratio, ledger.weight (lumex::dev::genesis_key.pub));
	ASSERT_EQ (lumex::Klumex_ratio, ledger.weight (destination.pub));
}

TEST (ledger, epoch_blocks_receive_upgrade)
{
	auto ctx = lumex::test::ledger_empty ();
	auto & ledger = ctx.ledger ();
	auto & store = ctx.store ();
	auto transaction = ledger.tx_begin_write ();
	auto & pool = ctx.pool ();
	lumex::keypair destination;
	lumex::block_builder builder;
	auto send1 = builder
				 .state ()
				 .account (lumex::dev::genesis_key.pub)
				 .previous (lumex::dev::genesis->hash ())
				 .representative (lumex::dev::genesis_key.pub)
				 .balance (lumex::dev::constants.genesis_amount - lumex::Klumex_ratio)
				 .link (destination.pub)
				 .sign (lumex::dev::genesis_key.prv, lumex::dev::genesis_key.pub)
				 .work (*pool.generate (lumex::dev::genesis->hash ()))
				 .build ();
	ASSERT_EQ (lumex::block_status::progress, ledger.process (transaction, send1));
	auto epoch1 = builder
				  .state ()
				  .account (lumex::dev::genesis_key.pub)
				  .previous (send1->hash ())
				  .representative (lumex::dev::genesis_key.pub)
				  .balance (lumex::dev::constants.genesis_amount - lumex::Klumex_ratio)
				  .link (ledger.epoch_link (lumex::epoch::epoch_1))
				  .sign (lumex::dev::genesis_key.prv, lumex::dev::genesis_key.pub)
				  .work (*pool.generate (send1->hash ()))
				  .build ();
	ASSERT_EQ (lumex::block_status::progress, ledger.process (transaction, epoch1));
	auto send2 = builder
				 .state ()
				 .account (lumex::dev::genesis_key.pub)
				 .previous (epoch1->hash ())
				 .representative (lumex::dev::genesis_key.pub)
				 .balance (lumex::dev::constants.genesis_amount - lumex::Klumex_ratio * 2)
				 .link (destination.pub)
				 .sign (lumex::dev::genesis_key.prv, lumex::dev::genesis_key.pub)
				 .work (*pool.generate (epoch1->hash ()))
				 .build ();
	ASSERT_EQ (lumex::block_status::progress, ledger.process (transaction, send2));
	ASSERT_EQ (lumex::epoch::epoch_1, send2->sideband ().details.epoch);
	ASSERT_EQ (lumex::epoch::epoch_0, send2->sideband ().source_epoch); // Not used for send blocks
	auto open1 = builder
				 .open ()
				 .source (send1->hash ())
				 .representative (destination.pub)
				 .account (destination.pub)
				 .sign (destination.prv, destination.pub)
				 .work (*pool.generate (destination.pub))
				 .build ();
	ASSERT_EQ (lumex::block_status::progress, ledger.process (transaction, open1));
	ASSERT_EQ (lumex::epoch::epoch_0, open1->sideband ().details.epoch);
	ASSERT_EQ (lumex::epoch::epoch_0, open1->sideband ().source_epoch);
	auto receive1 = builder
					.receive ()
					.previous (open1->hash ())
					.source (send2->hash ())
					.sign (destination.prv, destination.pub)
					.work (*pool.generate (open1->hash ()))
					.build ();
	ASSERT_EQ (lumex::block_status::unreceivable, ledger.process (transaction, receive1));
	auto receive2 = builder
					.state ()
					.account (destination.pub)
					.previous (open1->hash ())
					.representative (destination.pub)
					.balance (lumex::Klumex_ratio * 2)
					.link (send2->hash ())
					.sign (destination.prv, destination.pub)
					.work (*pool.generate (open1->hash ()))
					.build ();
	ASSERT_EQ (lumex::block_status::progress, ledger.process (transaction, receive2));
	ASSERT_EQ (lumex::epoch::epoch_1, receive2->sideband ().details.epoch);
	ASSERT_EQ (lumex::epoch::epoch_1, receive2->sideband ().source_epoch);
	auto destination_info = ledger.any.account_get (transaction, destination.pub);
	ASSERT_TRUE (destination_info);
	ASSERT_EQ (destination_info->epoch (), lumex::epoch::epoch_1);
	ASSERT_FALSE (ledger.rollback (transaction, receive2->hash ()));
	destination_info = ledger.any.account_get (transaction, destination.pub);
	ASSERT_TRUE (destination_info);
	ASSERT_EQ (destination_info->epoch (), lumex::epoch::epoch_0);
	auto pending_send2 = ledger.any.pending_get (transaction, lumex::pending_key (destination.pub, send2->hash ()));
	ASSERT_TRUE (pending_send2);
	ASSERT_EQ (lumex::dev::genesis_key.pub, pending_send2->source);
	ASSERT_EQ (lumex::Klumex_ratio, pending_send2->amount.number ());
	ASSERT_EQ (lumex::epoch::epoch_1, pending_send2->epoch);
	ASSERT_EQ (lumex::block_status::progress, ledger.process (transaction, receive2));
	ASSERT_EQ (lumex::epoch::epoch_1, receive2->sideband ().details.epoch);
	ASSERT_EQ (lumex::epoch::epoch_1, receive2->sideband ().source_epoch);
	destination_info = ledger.any.account_get (transaction, destination.pub);
	ASSERT_TRUE (destination_info);
	ASSERT_EQ (destination_info->epoch (), lumex::epoch::epoch_1);
	lumex::keypair destination2;
	auto send3 = builder
				 .state ()
				 .account (destination.pub)
				 .previous (receive2->hash ())
				 .representative (destination.pub)
				 .balance (lumex::Klumex_ratio)
				 .link (destination2.pub)
				 .sign (destination.prv, destination.pub)
				 .work (*pool.generate (receive2->hash ()))
				 .build ();
	ASSERT_EQ (lumex::block_status::progress, ledger.process (transaction, send3));
	auto open2 = builder
				 .open ()
				 .source (send3->hash ())
				 .representative (destination2.pub)
				 .account (destination2.pub)
				 .sign (destination2.prv, destination2.pub)
				 .work (*pool.generate (destination2.pub))
				 .build ();
	ASSERT_EQ (lumex::block_status::unreceivable, ledger.process (transaction, open2));
	// Upgrade to epoch 2 and send to destination. Try to create an open block from an epoch 2 source block.
	lumex::keypair destination3;
	auto epoch2 = builder
				  .state ()
				  .account (lumex::dev::genesis_key.pub)
				  .previous (send2->hash ())
				  .representative (lumex::dev::genesis_key.pub)
				  .balance (lumex::dev::constants.genesis_amount - lumex::Klumex_ratio * 2)
				  .link (ledger.epoch_link (lumex::epoch::epoch_2))
				  .sign (lumex::dev::genesis_key.prv, lumex::dev::genesis_key.pub)
				  .work (*pool.generate (send2->hash ()))
				  .build ();
	ASSERT_EQ (lumex::block_status::progress, ledger.process (transaction, epoch2));
	auto send4 = builder
				 .state ()
				 .account (lumex::dev::genesis_key.pub)
				 .previous (epoch2->hash ())
				 .representative (lumex::dev::genesis_key.pub)
				 .balance (lumex::dev::constants.genesis_amount - lumex::Klumex_ratio * 3)
				 .link (destination3.pub)
				 .sign (lumex::dev::genesis_key.prv, lumex::dev::genesis_key.pub)
				 .work (*pool.generate (epoch2->hash ()))
				 .build ();
	ASSERT_EQ (lumex::block_status::progress, ledger.process (transaction, send4));
	auto open3 = builder
				 .open ()
				 .source (send4->hash ())
				 .representative (destination3.pub)
				 .account (destination3.pub)
				 .sign (destination3.prv, destination3.pub)
				 .work (*pool.generate (destination3.pub))
				 .build ();
	ASSERT_EQ (lumex::block_status::unreceivable, ledger.process (transaction, open3));
	// Send it to an epoch 1 account
	auto send5 = builder
				 .state ()
				 .account (lumex::dev::genesis_key.pub)
				 .previous (send4->hash ())
				 .representative (lumex::dev::genesis_key.pub)
				 .balance (lumex::dev::constants.genesis_amount - lumex::Klumex_ratio * 4)
				 .link (destination.pub)
				 .sign (lumex::dev::genesis_key.prv, lumex::dev::genesis_key.pub)
				 .work (*pool.generate (send4->hash ()))
				 .build ();
	ASSERT_EQ (lumex::block_status::progress, ledger.process (transaction, send5));
	destination_info = ledger.any.account_get (transaction, destination.pub);
	ASSERT_TRUE (destination_info);
	ASSERT_EQ (destination_info->epoch (), lumex::epoch::epoch_1);
	auto receive3 = builder
					.state ()
					.account (destination.pub)
					.previous (send3->hash ())
					.representative (destination.pub)
					.balance (lumex::Klumex_ratio * 2)
					.link (send5->hash ())
					.sign (destination.prv, destination.pub)
					.work (*pool.generate (send3->hash ()))
					.build ();
	ASSERT_EQ (lumex::block_status::progress, ledger.process (transaction, receive3));
	ASSERT_EQ (lumex::epoch::epoch_2, receive3->sideband ().details.epoch);
	ASSERT_EQ (lumex::epoch::epoch_2, receive3->sideband ().source_epoch);
	destination_info = ledger.any.account_get (transaction, destination.pub);
	ASSERT_TRUE (destination_info);
	ASSERT_EQ (destination_info->epoch (), lumex::epoch::epoch_2);
	// Upgrade an unopened account straight to epoch 2
	lumex::keypair destination4;
	auto send6 = builder
				 .state ()
				 .account (lumex::dev::genesis_key.pub)
				 .previous (send5->hash ())
				 .representative (lumex::dev::genesis_key.pub)
				 .balance (lumex::dev::constants.genesis_amount - lumex::Klumex_ratio * 5)
				 .link (destination4.pub)
				 .sign (lumex::dev::genesis_key.prv, lumex::dev::genesis_key.pub)
				 .work (*pool.generate (send5->hash ()))
				 .build ();
	ASSERT_EQ (lumex::block_status::progress, ledger.process (transaction, send6));
	auto epoch4 = builder
				  .state ()
				  .account (destination4.pub)
				  .previous (0)
				  .representative (0)
				  .balance (0)
				  .link (ledger.epoch_link (lumex::epoch::epoch_2))
				  .sign (lumex::dev::genesis_key.prv, lumex::dev::genesis_key.pub)
				  .work (*pool.generate (destination4.pub))
				  .build ();
	ASSERT_EQ (lumex::block_status::progress, ledger.process (transaction, epoch4));
	ASSERT_EQ (lumex::epoch::epoch_2, epoch4->sideband ().details.epoch);
	ASSERT_EQ (lumex::epoch::epoch_0, epoch4->sideband ().source_epoch); // Not used for epoch blocks
	ASSERT_EQ (store.account.count (transaction), ledger.account_count ());
}

TEST (ledger, epoch_blocks_fork)
{
	auto ctx = lumex::test::ledger_empty ();
	auto & ledger = ctx.ledger ();
	auto & store = ctx.store ();
	auto transaction = ledger.tx_begin_write ();
	auto & pool = ctx.pool ();
	lumex::keypair destination;
	lumex::block_builder builder;
	auto send1 = builder
				 .send ()
				 .previous (lumex::dev::genesis->hash ())
				 .destination (lumex::account{})
				 .balance (lumex::dev::constants.genesis_amount)
				 .sign (lumex::dev::genesis_key.prv, lumex::dev::genesis_key.pub)
				 .work (*pool.generate (lumex::dev::genesis->hash ()))
				 .build ();
	ASSERT_EQ (lumex::block_status::progress, ledger.process (transaction, send1));
	auto epoch1 = builder
				  .state ()
				  .account (lumex::dev::genesis_key.pub)
				  .previous (lumex::dev::genesis->hash ())
				  .representative (lumex::dev::genesis_key.pub)
				  .balance (lumex::dev::constants.genesis_amount)
				  .link (ledger.epoch_link (lumex::epoch::epoch_1))
				  .sign (lumex::dev::genesis_key.prv, lumex::dev::genesis_key.pub)
				  .work (*pool.generate (lumex::dev::genesis->hash ()))
				  .build ();
	ASSERT_EQ (lumex::block_status::fork, ledger.process (transaction, epoch1));
	auto epoch2 = builder
				  .state ()
				  .account (lumex::dev::genesis_key.pub)
				  .previous (lumex::dev::genesis->hash ())
				  .representative (lumex::dev::genesis_key.pub)
				  .balance (lumex::dev::constants.genesis_amount)
				  .link (ledger.epoch_link (lumex::epoch::epoch_2))
				  .sign (lumex::dev::genesis_key.prv, lumex::dev::genesis_key.pub)
				  .work (*pool.generate (lumex::dev::genesis->hash ()))
				  .build ();
	ASSERT_EQ (lumex::block_status::fork, ledger.process (transaction, epoch2));
	auto epoch3 = builder
				  .state ()
				  .account (lumex::dev::genesis_key.pub)
				  .previous (send1->hash ())
				  .representative (lumex::dev::genesis_key.pub)
				  .balance (lumex::dev::constants.genesis_amount)
				  .link (ledger.epoch_link (lumex::epoch::epoch_1))
				  .sign (lumex::dev::genesis_key.prv, lumex::dev::genesis_key.pub)
				  .work (*pool.generate (send1->hash ()))
				  .build ();
	ASSERT_EQ (lumex::block_status::progress, ledger.process (transaction, epoch3));
	ASSERT_EQ (lumex::epoch::epoch_1, epoch3->sideband ().details.epoch);
	ASSERT_EQ (lumex::epoch::epoch_0, epoch3->sideband ().source_epoch); // Not used for epoch state blocks
	auto epoch4 = builder
				  .state ()
				  .account (lumex::dev::genesis_key.pub)
				  .previous (send1->hash ())
				  .representative (lumex::dev::genesis_key.pub)
				  .balance (lumex::dev::constants.genesis_amount)
				  .link (ledger.epoch_link (lumex::epoch::epoch_2))
				  .sign (lumex::dev::genesis_key.prv, lumex::dev::genesis_key.pub)
				  .work (*pool.generate (send1->hash ()))
				  .build ();
	ASSERT_EQ (lumex::block_status::fork, ledger.process (transaction, epoch2));
}

TEST (ledger, successor_epoch)
{
	lumex::test::system system (1);
	auto & node1 (*system.nodes[0]);
	lumex::keypair key1;
	lumex::work_pool pool{ lumex::dev::network_params.network, std::numeric_limits<unsigned>::max () };
	lumex::block_builder builder;
	auto send1 = builder
				 .send ()
				 .previous (lumex::dev::genesis->hash ())
				 .destination (key1.pub)
				 .balance (lumex::dev::constants.genesis_amount - 1)
				 .sign (lumex::dev::genesis_key.prv, lumex::dev::genesis_key.pub)
				 .work (*pool.generate (lumex::dev::genesis->hash ()))
				 .build ();
	auto open = builder
				.state ()
				.account (key1.pub)
				.previous (0)
				.representative (key1.pub)
				.balance (1)
				.link (send1->hash ())
				.sign (key1.prv, key1.pub)
				.work (*pool.generate (key1.pub))
				.build ();
	auto change = builder
				  .state ()
				  .account (key1.pub)
				  .previous (open->hash ())
				  .representative (key1.pub)
				  .balance (1)
				  .link (0)
				  .sign (key1.prv, key1.pub)
				  .work (*pool.generate (open->hash ()))
				  .build ();
	auto open_hash = open->hash ();
	auto send2 = builder
				 .send ()
				 .previous (send1->hash ())
				 .destination (reinterpret_cast<lumex::account const &> (open_hash))
				 .balance (lumex::dev::constants.genesis_amount - 2)
				 .sign (lumex::dev::genesis_key.prv, lumex::dev::genesis_key.pub)
				 .work (*pool.generate (send1->hash ()))
				 .build ();
	auto epoch_open = builder
					  .state ()
					  .account (reinterpret_cast<lumex::account const &> (open_hash))
					  .previous (0)
					  .representative (0)
					  .balance (0)
					  .link (node1.ledger.epoch_link (lumex::epoch::epoch_1))
					  .sign (lumex::dev::genesis_key.prv, lumex::dev::genesis_key.pub)
					  .work (*pool.generate (open->hash ()))
					  .build ();
	auto transaction = node1.ledger.tx_begin_write ();
	ASSERT_EQ (lumex::block_status::progress, node1.ledger.process (transaction, send1));
	ASSERT_EQ (lumex::block_status::progress, node1.ledger.process (transaction, open));
	ASSERT_EQ (lumex::block_status::progress, node1.ledger.process (transaction, change));
	ASSERT_EQ (lumex::block_status::progress, node1.ledger.process (transaction, send2));
	ASSERT_EQ (lumex::block_status::progress, node1.ledger.process (transaction, epoch_open));
	ASSERT_EQ (*change, *node1.ledger.any.block_get (transaction, node1.ledger.any.block_successor (transaction, change->qualified_root ()).value ()));
	ASSERT_EQ (*epoch_open, *node1.ledger.any.block_get (transaction, node1.ledger.any.block_successor (transaction, epoch_open->qualified_root ()).value ()));
	ASSERT_EQ (lumex::epoch::epoch_1, epoch_open->sideband ().details.epoch);
	ASSERT_EQ (lumex::epoch::epoch_0, epoch_open->sideband ().source_epoch); // Not used for epoch state blocks
}

TEST (ledger, epoch_open_pending)
{
	lumex::block_builder builder{};
	lumex::test::system system{ 1 };
	auto & node1 = *system.nodes[0];
	lumex::work_pool pool{ lumex::dev::network_params.network, std::numeric_limits<unsigned>::max () };
	lumex::keypair key1{};
	auto epoch_open = builder.state ()
					  .account (key1.pub)
					  .previous (0)
					  .representative (0)
					  .balance (0)
					  .link (node1.ledger.epoch_link (lumex::epoch::epoch_1))
					  .sign (lumex::dev::genesis_key.prv, lumex::dev::genesis_key.pub)
					  .work (*pool.generate (key1.pub))
					  .build ();
	auto process_result = node1.ledger.process (node1.ledger.tx_begin_write (), epoch_open);
	ASSERT_EQ (lumex::block_status::gap_epoch_open_pending, process_result);
	node1.block_processor.add (epoch_open, lumex::block_source::test);
	// Waits for the block to get saved in the database
	ASSERT_TIMELY_EQ (10s, 1, node1.unchecked.count ());
	ASSERT_FALSE (node1.block_or_pruned_exists (epoch_open->hash ()));
	// Open block should be inserted into unchecked
	auto blocks = node1.unchecked.get (lumex::hash_or_account (epoch_open->account_field ().value ()).hash);
	ASSERT_EQ (blocks.size (), 1);
	ASSERT_EQ (blocks[0].block->full_hash (), epoch_open->full_hash ());
	// New block to process epoch open
	auto send1 = builder.state ()
				 .account (lumex::dev::genesis_key.pub)
				 .previous (lumex::dev::genesis->hash ())
				 .representative (lumex::dev::genesis_key.pub)
				 .balance (lumex::dev::constants.genesis_amount - 100)
				 .link (key1.pub)
				 .sign (lumex::dev::genesis_key.prv, lumex::dev::genesis_key.pub)
				 .work (*pool.generate (lumex::dev::genesis->hash ()))
				 .build ();
	node1.block_processor.add (send1, lumex::block_source::test);
	ASSERT_TIMELY (10s, node1.block_or_pruned_exists (epoch_open->hash ()));
}

TEST (ledger, block_hash_account_conflict)
{
	lumex::block_builder builder;
	lumex::test::system system (1);
	auto & node1 (*system.nodes[0]);
	lumex::keypair key1;
	lumex::keypair key2;
	lumex::work_pool pool{ lumex::dev::network_params.network, std::numeric_limits<unsigned>::max () };

	/*
	 * Generate a send block whose destination is a block hash already
	 * in the ledger and not an account
	 */
	auto send1 = builder.state ()
				 .account (lumex::dev::genesis_key.pub)
				 .previous (lumex::dev::genesis->hash ())
				 .representative (lumex::dev::genesis_key.pub)
				 .balance (lumex::dev::constants.genesis_amount - 100)
				 .link (key1.pub)
				 .sign (lumex::dev::genesis_key.prv, lumex::dev::genesis_key.pub)
				 .work (*pool.generate (lumex::dev::genesis->hash ()))
				 .build ();

	auto receive1 = builder.state ()
					.account (key1.pub)
					.previous (0)
					.representative (lumex::dev::genesis_key.pub)
					.balance (100)
					.link (send1->hash ())
					.sign (key1.prv, key1.pub)
					.work (*pool.generate (key1.pub))
					.build ();

	/*
	 * Note that the below link is a block hash when this is intended
	 * to represent a send state block. This can generally never be
	 * received , except by epoch blocks, which can sign an open block
	 * for arbitrary accounts.
	 */
	auto send2 = builder.state ()
				 .account (key1.pub)
				 .previous (receive1->hash ())
				 .representative (lumex::dev::genesis_key.pub)
				 .balance (90)
				 .link (receive1->hash ())
				 .sign (key1.prv, key1.pub)
				 .work (*pool.generate (receive1->hash ()))
				 .build ();

	/*
	 * Generate an epoch open for the account with the same value as the block hash
	 */
	auto receive1_hash = receive1->hash ();
	auto open_epoch1 = builder.state ()
					   .account (reinterpret_cast<lumex::account const &> (receive1_hash))
					   .previous (0)
					   .representative (0)
					   .balance (0)
					   .link (node1.ledger.epoch_link (lumex::epoch::epoch_1))
					   .sign (lumex::dev::genesis_key.prv, lumex::dev::genesis_key.pub)
					   .work (*pool.generate (receive1->hash ()))
					   .build ();

	node1.work_generate_blocking (*send1);
	node1.work_generate_blocking (*receive1);
	node1.work_generate_blocking (*send2);
	node1.work_generate_blocking (*open_epoch1);
	ASSERT_EQ (lumex::block_status::progress, node1.process (send1));
	ASSERT_EQ (lumex::block_status::progress, node1.process (receive1));
	ASSERT_EQ (lumex::block_status::progress, node1.process (send2));
	ASSERT_EQ (lumex::block_status::progress, node1.process (open_epoch1));
	ASSERT_TRUE (lumex::test::start_elections (system, node1, { send1, receive1, send2, open_epoch1 }));
	auto election1 = node1.active.election (send1->qualified_root ());
	ASSERT_NE (nullptr, election1);
	auto election2 = node1.active.election (receive1->qualified_root ());
	ASSERT_NE (nullptr, election2);
	auto election3 = node1.active.election (send2->qualified_root ());
	ASSERT_NE (nullptr, election3);
	auto election4 = node1.active.election (open_epoch1->qualified_root ());
	ASSERT_NE (nullptr, election4);
	auto winner1 (election1->winner ());
	auto winner2 (election2->winner ());
	auto winner3 (election3->winner ());
	auto winner4 (election4->winner ());
	ASSERT_EQ (*send1, *winner1);
	ASSERT_EQ (*receive1, *winner2);
	ASSERT_EQ (*send2, *winner3);
	ASSERT_EQ (*open_epoch1, *winner4);
}

TEST (ledger, unchecked_epoch)
{
	lumex::test::system system (1);
	auto & node1 (*system.nodes[0]);
	lumex::keypair destination;
	lumex::block_builder builder;
	auto send1 = builder
				 .state ()
				 .account (lumex::dev::genesis_key.pub)
				 .previous (lumex::dev::genesis->hash ())
				 .representative (lumex::dev::genesis_key.pub)
				 .balance (lumex::dev::constants.genesis_amount - lumex::Klumex_ratio)
				 .link (destination.pub)
				 .sign (lumex::dev::genesis_key.prv, lumex::dev::genesis_key.pub)
				 .work (0)
				 .build ();
	node1.work_generate_blocking (*send1);
	auto open1 = builder
				 .state ()
				 .account (destination.pub)
				 .previous (0)
				 .representative (destination.pub)
				 .balance (lumex::Klumex_ratio)
				 .link (send1->hash ())
				 .sign (destination.prv, destination.pub)
				 .work (0)
				 .build ();
	node1.work_generate_blocking (*open1);
	auto epoch1 = builder
				  .state ()
				  .account (destination.pub)
				  .previous (open1->hash ())
				  .representative (destination.pub)
				  .balance (lumex::Klumex_ratio)
				  .link (node1.ledger.epoch_link (lumex::epoch::epoch_1))
				  .sign (lumex::dev::genesis_key.prv, lumex::dev::genesis_key.pub)
				  .work (0)
				  .build ();
	node1.work_generate_blocking (*epoch1);
	node1.block_processor.add (epoch1, lumex::block_source::test);
	{
		// Waits for the epoch1 block to pass through block_processor and unchecked.put queues
		ASSERT_TIMELY_EQ (10s, 1, node1.unchecked.count ());
		auto blocks = node1.unchecked.get (epoch1->previous ());
		ASSERT_EQ (blocks.size (), 1);
	}
	node1.block_processor.add (send1, lumex::block_source::test);
	node1.block_processor.add (open1, lumex::block_source::test);
	ASSERT_TIMELY (5s, node1.ledger.any.block_exists (node1.ledger.tx_begin_read (), epoch1->hash ()));
	{
		// Waits for the last blocks to pass through block_processor and unchecked.put queues
		ASSERT_TIMELY_EQ (10s, 0, node1.unchecked.count ());
		auto info = node1.ledger.any.account_get (node1.ledger.tx_begin_read (), destination.pub);
		ASSERT_TRUE (info);
		ASSERT_EQ (info->epoch (), lumex::epoch::epoch_1);
	}
}

TEST (ledger, unchecked_epoch_invalid)
{
	lumex::test::system system;
	lumex::node_config node_config = system.default_config ();
	node_config.backlog_scan->enable = false;
	auto & node1 (*system.add_node (node_config));
	lumex::keypair destination;
	lumex::block_builder builder;
	auto send1 = builder
				 .state ()
				 .account (lumex::dev::genesis_key.pub)
				 .previous (lumex::dev::genesis->hash ())
				 .representative (lumex::dev::genesis_key.pub)
				 .balance (lumex::dev::constants.genesis_amount - lumex::Klumex_ratio)
				 .link (destination.pub)
				 .sign (lumex::dev::genesis_key.prv, lumex::dev::genesis_key.pub)
				 .work (0)
				 .build ();
	node1.work_generate_blocking (*send1);
	auto open1 = builder
				 .state ()
				 .account (destination.pub)
				 .previous (0)
				 .representative (destination.pub)
				 .balance (lumex::Klumex_ratio)
				 .link (send1->hash ())
				 .sign (destination.prv, destination.pub)
				 .work (0)
				 .build ();
	node1.work_generate_blocking (*open1);
	// Epoch block with account own signature
	auto epoch1 = builder
				  .state ()
				  .account (destination.pub)
				  .previous (open1->hash ())
				  .representative (destination.pub)
				  .balance (lumex::Klumex_ratio)
				  .link (node1.ledger.epoch_link (lumex::epoch::epoch_1))
				  .sign (destination.prv, destination.pub)
				  .work (0)
				  .build ();
	node1.work_generate_blocking (*epoch1);
	// Pseudo epoch block (send subtype, destination - epoch link)
	auto epoch2 = builder
				  .state ()
				  .account (destination.pub)
				  .previous (open1->hash ())
				  .representative (destination.pub)
				  .balance (lumex::Klumex_ratio - 1)
				  .link (node1.ledger.epoch_link (lumex::epoch::epoch_1))
				  .sign (destination.prv, destination.pub)
				  .work (0)
				  .build ();
	node1.work_generate_blocking (*epoch2);
	node1.block_processor.add (epoch1, lumex::block_source::test);
	node1.block_processor.add (epoch2, lumex::block_source::test);
	{
		// Waits for the last blocks to pass through block_processor and unchecked.put queues
		ASSERT_TIMELY_EQ (10s, 2, node1.unchecked.count ());
		auto blocks = node1.unchecked.get (epoch1->previous ());
		ASSERT_EQ (blocks.size (), 2);
	}
	node1.block_processor.add (send1, lumex::block_source::test);
	node1.block_processor.add (open1, lumex::block_source::test);
	// Waits for the last blocks to pass through block_processor and unchecked.put queues
	ASSERT_TIMELY (10s, node1.ledger.any.block_exists (node1.ledger.tx_begin_read (), epoch2->hash ()));
	{
		auto transaction = node1.ledger.tx_begin_read ();
		ASSERT_FALSE (node1.ledger.any.block_exists (transaction, epoch1->hash ()));
		auto unchecked_count = node1.unchecked.count ();
		ASSERT_EQ (unchecked_count, 0);
		ASSERT_EQ (unchecked_count, node1.unchecked.count ());
		auto info = node1.ledger.any.account_get (transaction, destination.pub);
		ASSERT_TRUE (info);
		ASSERT_NE (info->epoch (), lumex::epoch::epoch_1);
		auto epoch2_store = node1.ledger.any.block_get (transaction, epoch2->hash ());
		ASSERT_NE (nullptr, epoch2_store);
		ASSERT_EQ (lumex::epoch::epoch_0, epoch2_store->sideband ().details.epoch);
		ASSERT_TRUE (epoch2_store->is_send ());
		ASSERT_FALSE (epoch2_store->sideband ().details.is_epoch);
		ASSERT_FALSE (epoch2_store->is_receive ());
	}
}

TEST (ledger, unchecked_open)
{
	lumex::test::system system (1);
	auto & node1 (*system.nodes[0]);
	lumex::keypair destination;
	lumex::block_builder builder;
	auto send1 = builder
				 .state ()
				 .account (lumex::dev::genesis_key.pub)
				 .previous (lumex::dev::genesis->hash ())
				 .representative (lumex::dev::genesis_key.pub)
				 .balance (lumex::dev::constants.genesis_amount - lumex::Klumex_ratio)
				 .link (destination.pub)
				 .sign (lumex::dev::genesis_key.prv, lumex::dev::genesis_key.pub)
				 .work (0)
				 .build ();
	node1.work_generate_blocking (*send1);
	auto open1 = builder
				 .open ()
				 .source (send1->hash ())
				 .representative (destination.pub)
				 .account (destination.pub)
				 .sign (destination.prv, destination.pub)
				 .work (0)
				 .build ();
	node1.work_generate_blocking (*open1);
	// Invalid signature for open block
	auto open2 = builder
				 .open ()
				 .source (send1->hash ())
				 .representative (lumex::dev::genesis_key.pub)
				 .account (destination.pub)
				 .sign (destination.prv, destination.pub)
				 .work (0)
				 .build ();
	node1.work_generate_blocking (*open2);
	open2->signature.bytes[0] ^= 1;
	node1.block_processor.add (open2, lumex::block_source::test); // Insert open2 in to the queue before open1
	node1.block_processor.add (open1, lumex::block_source::test);
	{
		// Waits for the last blocks to pass through block_processor and unchecked.put queues
		ASSERT_TIMELY_EQ (10s, 1, node1.unchecked.count ());
		// Get the next peer for attempting a tcp bootstrap connection
		auto blocks = node1.unchecked.get (open1->source_field ().value ());
		ASSERT_EQ (blocks.size (), 1);
	}
	node1.block_processor.add (send1, lumex::block_source::test);
	// Waits for the send1 block to pass through block_processor and unchecked.put queues
	ASSERT_TIMELY (5s, node1.ledger.any.block_exists (node1.ledger.tx_begin_read (), open1->hash ()));
	ASSERT_EQ (0, node1.unchecked.count ());
}

TEST (ledger, unchecked_receive)
{
	lumex::test::system system{ 1 };
	auto & node1 = *system.nodes[0];
	lumex::keypair destination{};
	lumex::block_builder builder;
	auto send1 = builder
				 .state ()
				 .account (lumex::dev::genesis_key.pub)
				 .previous (lumex::dev::genesis->hash ())
				 .representative (lumex::dev::genesis_key.pub)
				 .balance (lumex::dev::constants.genesis_amount - lumex::Klumex_ratio)
				 .link (destination.pub)
				 .sign (lumex::dev::genesis_key.prv, lumex::dev::genesis_key.pub)
				 .work (0)
				 .build ();
	node1.work_generate_blocking (*send1);
	auto send2 = builder
				 .state ()
				 .account (lumex::dev::genesis_key.pub)
				 .previous (send1->hash ())
				 .representative (lumex::dev::genesis_key.pub)
				 .balance (lumex::dev::constants.genesis_amount - 2 * lumex::Klumex_ratio)
				 .link (destination.pub)
				 .sign (lumex::dev::genesis_key.prv, lumex::dev::genesis_key.pub)
				 .work (0)
				 .build ();
	node1.work_generate_blocking (*send2);
	auto open1 = builder
				 .open ()
				 .source (send1->hash ())
				 .representative (destination.pub)
				 .account (destination.pub)
				 .sign (destination.prv, destination.pub)
				 .work (0)
				 .build ();
	node1.work_generate_blocking (*open1);
	auto receive1 = builder
					.receive ()
					.previous (open1->hash ())
					.source (send2->hash ())
					.sign (destination.prv, destination.pub)
					.work (0)
					.build ();
	node1.work_generate_blocking (*receive1);
	node1.block_processor.add (send1, lumex::block_source::test);
	node1.block_processor.add (receive1, lumex::block_source::test);
	auto check_block_is_listed = [&] (lumex::store::transaction const & transaction_a, lumex::block_hash const & block_hash_a) {
		return !node1.unchecked.get (block_hash_a).empty ();
	};
	// Previous block for receive1 is unknown, signature cannot be validated
	{
		// Waits for the last blocks to pass through block_processor and unchecked.put queues
		ASSERT_TIMELY (15s, check_block_is_listed (node1.store.tx_begin_read (), receive1->previous ()));
		auto blocks = node1.unchecked.get (receive1->previous ());
		ASSERT_EQ (blocks.size (), 1);
	}
	// Waits for the open1 block to pass through block_processor and unchecked.put queues
	node1.block_processor.add (open1, lumex::block_source::test);
	ASSERT_TIMELY (15s, check_block_is_listed (node1.store.tx_begin_read (), receive1->source_field ().value ()));
	// Previous block for receive1 is known, signature was validated
	{
		auto transaction = node1.store.tx_begin_read ();
		auto blocks (node1.unchecked.get (receive1->source_field ().value ()));
		ASSERT_EQ (blocks.size (), 1);
	}
	node1.block_processor.add (send2, lumex::block_source::test);
	ASSERT_TIMELY (10s, node1.ledger.any.block_exists (node1.ledger.tx_begin_read (), receive1->hash ()));
	ASSERT_EQ (0, node1.unchecked.count ());
}

TEST (ledger, confirmation_height_not_updated)
{
	auto ctx = lumex::test::ledger_empty ();
	auto & ledger = ctx.ledger ();
	auto & store = ctx.store ();
	auto transaction = ledger.tx_begin_write ();
	auto & pool = ctx.pool ();
	auto account_info = ledger.any.account_get (transaction, lumex::dev::genesis_key.pub);
	ASSERT_TRUE (account_info);
	lumex::keypair key;
	lumex::block_builder builder;
	auto send1 = builder
				 .send ()
				 .previous (account_info->head)
				 .destination (key.pub)
				 .balance (50)
				 .sign (lumex::dev::genesis_key.prv, lumex::dev::genesis_key.pub)
				 .work (*pool.generate (account_info->head))
				 .build ();
	lumex::confirmation_height_info confirmation_height_info;
	ASSERT_FALSE (store.confirmation_height.get (transaction, lumex::dev::genesis_key.pub, confirmation_height_info));
	ASSERT_EQ (1, confirmation_height_info.height);
	ASSERT_EQ (lumex::dev::genesis->hash (), confirmation_height_info.frontier);
	ASSERT_EQ (lumex::block_status::progress, ledger.process (transaction, send1));
	ASSERT_FALSE (store.confirmation_height.get (transaction, lumex::dev::genesis_key.pub, confirmation_height_info));
	ASSERT_EQ (1, confirmation_height_info.height);
	ASSERT_EQ (lumex::dev::genesis->hash (), confirmation_height_info.frontier);
	auto open1 = builder
				 .open ()
				 .source (send1->hash ())
				 .representative (lumex::dev::genesis_key.pub)
				 .account (key.pub)
				 .sign (key.prv, key.pub)
				 .work (*pool.generate (key.pub))
				 .build ();
	ASSERT_EQ (lumex::block_status::progress, ledger.process (transaction, open1));
	ASSERT_TRUE (store.confirmation_height.get (transaction, key.pub, confirmation_height_info));
	ASSERT_EQ (0, confirmation_height_info.height);
	ASSERT_EQ (lumex::block_hash (0), confirmation_height_info.frontier);
}

TEST (ledger, zero_rep_weight)
{
	lumex::test::system system (1);
	auto & node1 (*system.nodes[0]);
	lumex::block_builder builder;
	auto block1 = builder.state ()
				  .account (lumex::dev::genesis_key.pub)
				  .previous (lumex::dev::genesis->hash ())
				  .representative (0)
				  .balance (lumex::dev::constants.genesis_amount)
				  .link (0)
				  .sign (lumex::dev::genesis_key.prv, lumex::dev::genesis_key.pub)
				  .work (*system.work.generate (lumex::dev::genesis->hash ()))
				  .build ();
	auto transaction = node1.ledger.tx_begin_write ();
	ASSERT_EQ (lumex::block_status::progress, node1.ledger.process (transaction, block1));
	ASSERT_EQ (0, node1.ledger.rep_weights.get (lumex::dev::genesis_key.pub));
	ASSERT_EQ (0, node1.ledger.rep_weights.get (0));
	auto block2 = builder.state ()
				  .account (lumex::dev::genesis_key.pub)
				  .previous (block1->hash ())
				  .representative (lumex::dev::genesis_key.pub)
				  .balance (lumex::dev::constants.genesis_amount)
				  .link (0)
				  .sign (lumex::dev::genesis_key.prv, lumex::dev::genesis_key.pub)
				  .work (*system.work.generate (block1->hash ()))
				  .build ();
	ASSERT_EQ (lumex::block_status::progress, node1.ledger.process (transaction, block2));
	ASSERT_EQ (lumex::dev::constants.genesis_amount, node1.ledger.rep_weights.get (lumex::dev::genesis_key.pub));
	ASSERT_EQ (0, node1.ledger.rep_weights.get (0));
}

TEST (ledger, work_validation)
{
	auto ctx = lumex::test::ledger_empty ();
	auto & ledger = ctx.ledger ();
	auto & store = ctx.store ();
	auto & pool = ctx.pool ();
	lumex::block_builder builder;
	auto gen = lumex::dev::genesis_key;
	lumex::keypair key;

	// With random work the block doesn't pass, then modifies the block with sufficient work and ensures a correct result
	auto process_block = [&store, &ledger, &pool] (std::shared_ptr<lumex::block> block_a, lumex::block_details const details_a) {
		auto threshold = lumex::dev::network_params.work.threshold (block_a->work_version (), details_a);
		// Rarely failed with random work, so modify until it doesn't have enough difficulty
		while (lumex::dev::network_params.work.difficulty (*block_a) >= threshold)
		{
			block_a->block_work_set (block_a->block_work () + 1);
		}
		EXPECT_EQ (lumex::block_status::insufficient_work, ledger.process (ledger.tx_begin_write (), block_a));
		block_a->block_work_set (*pool.generate (block_a->root (), threshold));
		EXPECT_EQ (lumex::block_status::progress, ledger.process (ledger.tx_begin_write (), block_a));
	};

	std::error_code ec;

	auto send = builder.send ()
				.previous (lumex::dev::genesis->hash ())
				.destination (gen.pub)
				.balance (lumex::dev::constants.genesis_amount - 1)
				.sign (gen.prv, gen.pub)
				.work (0)
				.build (ec);
	ASSERT_FALSE (ec);

	auto receive = builder.receive ()
				   .previous (send->hash ())
				   .source (send->hash ())
				   .sign (gen.prv, gen.pub)
				   .work (0)
				   .build (ec);
	ASSERT_FALSE (ec);

	auto change = builder.change ()
				  .previous (receive->hash ())
				  .representative (key.pub)
				  .sign (gen.prv, gen.pub)
				  .work (0)
				  .build (ec);
	ASSERT_FALSE (ec);

	auto state = builder.state ()
				 .account (gen.pub)
				 .previous (change->hash ())
				 .representative (gen.pub)
				 .balance (lumex::dev::constants.genesis_amount - 1)
				 .link (key.pub)
				 .sign (gen.prv, gen.pub)
				 .work (0)
				 .build (ec);
	ASSERT_FALSE (ec);

	auto open = builder.open ()
				.account (key.pub)
				.source (state->hash ())
				.representative (key.pub)
				.sign (key.prv, key.pub)
				.work (0)
				.build (ec);
	ASSERT_FALSE (ec);

	auto epoch = builder.state ()
				 .account (key.pub)
				 .previous (open->hash ())
				 .balance (1)
				 .representative (key.pub)
				 .link (ledger.epoch_link (lumex::epoch::epoch_1))
				 .sign (gen.prv, gen.pub)
				 .work (0)
				 .build (ec);
	ASSERT_FALSE (ec);

	process_block (send, {});
	process_block (receive, {});
	process_block (change, {});
	process_block (state, lumex::block_details (lumex::epoch::epoch_0, true, false, false));
	process_block (open, {});
	process_block (epoch, lumex::block_details (lumex::epoch::epoch_1, false, false, true));
}

TEST (ledger, block_dependencies)
{
	auto ctx = lumex::test::ledger_empty ();
	auto & ledger = ctx.ledger ();
	auto & pool = ctx.pool ();
	auto transaction = ledger.tx_begin_write ();
	lumex::block_builder builder;
	lumex::block_hash zero{ 0 };

	// Genesis open block has no dependencies
	{
		auto deps = lumex::dev::genesis->dependencies ();
		ASSERT_EQ (deps[0], zero);
		ASSERT_EQ (deps[1], zero);
	}

	lumex::keypair key1;

	// Legacy send
	auto legacy_send = builder
					   .send ()
					   .previous (lumex::dev::genesis->hash ())
					   .destination (key1.pub)
					   .balance (lumex::dev::constants.genesis_amount - 100)
					   .sign (lumex::dev::genesis_key.prv, lumex::dev::genesis_key.pub)
					   .work (*pool.generate (lumex::dev::genesis->hash ()))
					   .build ();
	ASSERT_EQ (lumex::block_status::progress, ledger.process (transaction, legacy_send));
	{
		auto deps = legacy_send->dependencies ();
		ASSERT_EQ (deps[0], lumex::dev::genesis->hash ());
		ASSERT_EQ (deps[1], zero);
	}

	// Legacy open (non-genesis)
	auto legacy_open = builder
					   .open ()
					   .source (legacy_send->hash ())
					   .representative (key1.pub)
					   .account (key1.pub)
					   .sign (key1.prv, key1.pub)
					   .work (*pool.generate (key1.pub))
					   .build ();
	ASSERT_EQ (lumex::block_status::progress, ledger.process (transaction, legacy_open));
	{
		auto deps = legacy_open->dependencies ();
		ASSERT_EQ (deps[0], zero);
		ASSERT_EQ (deps[1], legacy_send->hash ());
	}

	auto legacy_send2 = builder
						.send ()
						.previous (legacy_send->hash ())
						.destination (key1.pub)
						.balance (lumex::dev::constants.genesis_amount - 200)
						.sign (lumex::dev::genesis_key.prv, lumex::dev::genesis_key.pub)
						.work (*pool.generate (legacy_send->hash ()))
						.build ();
	ASSERT_EQ (lumex::block_status::progress, ledger.process (transaction, legacy_send2));

	// Legacy receive
	auto legacy_receive = builder
						  .receive ()
						  .previous (legacy_open->hash ())
						  .source (legacy_send2->hash ())
						  .sign (key1.prv, key1.pub)
						  .work (*pool.generate (legacy_open->hash ()))
						  .build ();
	ASSERT_EQ (lumex::block_status::progress, ledger.process (transaction, legacy_receive));
	{
		auto deps = legacy_receive->dependencies ();
		ASSERT_EQ (deps[0], legacy_open->hash ());
		ASSERT_EQ (deps[1], legacy_send2->hash ());
	}

	// Legacy change
	auto legacy_change = builder
						 .change ()
						 .previous (legacy_receive->hash ())
						 .representative (lumex::dev::genesis_key.pub)
						 .sign (key1.prv, key1.pub)
						 .work (*pool.generate (legacy_receive->hash ()))
						 .build ();
	ASSERT_EQ (lumex::block_status::progress, ledger.process (transaction, legacy_change));
	{
		auto deps = legacy_change->dependencies ();
		ASSERT_EQ (deps[0], legacy_receive->hash ());
		ASSERT_EQ (deps[1], zero);
	}

	lumex::keypair key2;

	// State send
	auto state_send = builder.state ()
					  .account (lumex::dev::genesis_key.pub)
					  .previous (legacy_send2->hash ())
					  .representative (lumex::dev::genesis_key.pub)
					  .balance (lumex::dev::constants.genesis_amount - 300)
					  .link (key2.pub)
					  .sign (lumex::dev::genesis_key.prv, lumex::dev::genesis_key.pub)
					  .work (*pool.generate (legacy_send2->hash ()))
					  .build ();
	ASSERT_EQ (lumex::block_status::progress, ledger.process (transaction, state_send));
	{
		ASSERT_TRUE (state_send->is_send ());
		auto deps = state_send->dependencies ();
		ASSERT_EQ (deps[0], legacy_send2->hash ());
		ASSERT_EQ (deps[1], zero);
	}

	// State receive opening an account
	auto state_open_receive = builder.state ()
							  .account (key2.pub)
							  .previous (0)
							  .representative (key2.pub)
							  .balance (100)
							  .link (state_send->hash ())
							  .sign (key2.prv, key2.pub)
							  .work (*pool.generate (key2.pub))
							  .build ();
	ASSERT_EQ (lumex::block_status::progress, ledger.process (transaction, state_open_receive));
	{
		ASSERT_TRUE (state_open_receive->is_receive ());
		auto deps = state_open_receive->dependencies ();
		ASSERT_EQ (deps[0], zero);
		ASSERT_EQ (deps[1], state_send->hash ());
	}

	auto state_send2 = builder.state ()
					   .account (lumex::dev::genesis_key.pub)
					   .previous (state_send->hash ())
					   .representative (lumex::dev::genesis_key.pub)
					   .balance (lumex::dev::constants.genesis_amount - 400)
					   .link (key2.pub)
					   .sign (lumex::dev::genesis_key.prv, lumex::dev::genesis_key.pub)
					   .work (*pool.generate (state_send->hash ()))
					   .build ();
	ASSERT_EQ (lumex::block_status::progress, ledger.process (transaction, state_send2));

	// State receive on existing account
	auto state_receive = builder.state ()
						 .account (key2.pub)
						 .previous (state_open_receive->hash ())
						 .representative (key2.pub)
						 .balance (200)
						 .link (state_send2->hash ())
						 .sign (key2.prv, key2.pub)
						 .work (*pool.generate (state_open_receive->hash ()))
						 .build ();
	ASSERT_EQ (lumex::block_status::progress, ledger.process (transaction, state_receive));
	{
		ASSERT_TRUE (state_receive->is_receive ());
		auto deps = state_receive->dependencies ();
		ASSERT_EQ (deps[0], state_open_receive->hash ());
		ASSERT_EQ (deps[1], state_send2->hash ());
	}

	// State change
	auto state_change = builder.state ()
						.account (key2.pub)
						.previous (state_receive->hash ())
						.representative (lumex::dev::genesis_key.pub)
						.balance (200)
						.link (0)
						.sign (key2.prv, key2.pub)
						.work (*pool.generate (state_receive->hash ()))
						.build ();
	ASSERT_EQ (lumex::block_status::progress, ledger.process (transaction, state_change));
	{
		ASSERT_TRUE (state_change->is_change ());
		auto deps = state_change->dependencies ();
		ASSERT_EQ (deps[0], state_receive->hash ());
		ASSERT_EQ (deps[1], zero);
	}

	// Epoch v1 on existing account
	auto epoch1 = builder.state ()
				  .account (lumex::dev::genesis_key.pub)
				  .previous (state_send2->hash ())
				  .representative (lumex::dev::genesis_key.pub)
				  .balance (lumex::dev::constants.genesis_amount - 400)
				  .link (ledger.epoch_link (lumex::epoch::epoch_1))
				  .sign (lumex::dev::genesis_key.prv, lumex::dev::genesis_key.pub)
				  .work (*pool.generate (state_send2->hash ()))
				  .build ();
	ASSERT_EQ (lumex::block_status::progress, ledger.process (transaction, epoch1));
	{
		ASSERT_TRUE (epoch1->is_epoch ());
		ASSERT_FALSE (epoch1->is_receive ());
		auto deps = epoch1->dependencies ();
		ASSERT_EQ (deps[0], state_send2->hash ());
		ASSERT_EQ (deps[1], zero);
	}

	// Epoch v2 on existing account
	auto epoch2 = builder.state ()
				  .account (lumex::dev::genesis_key.pub)
				  .previous (epoch1->hash ())
				  .representative (lumex::dev::genesis_key.pub)
				  .balance (lumex::dev::constants.genesis_amount - 400)
				  .link (ledger.epoch_link (lumex::epoch::epoch_2))
				  .sign (lumex::dev::genesis_key.prv, lumex::dev::genesis_key.pub)
				  .work (*pool.generate (epoch1->hash ()))
				  .build ();
	ASSERT_EQ (lumex::block_status::progress, ledger.process (transaction, epoch2));
	{
		ASSERT_TRUE (epoch2->is_epoch ());
		auto deps = epoch2->dependencies ();
		ASSERT_EQ (deps[0], epoch1->hash ());
		ASSERT_EQ (deps[1], zero);
	}

	lumex::keypair key3;

	// Send to key3 to create a pending entry for epoch open
	auto send_for_epoch = builder.state ()
						  .account (lumex::dev::genesis_key.pub)
						  .previous (epoch2->hash ())
						  .representative (lumex::dev::genesis_key.pub)
						  .balance (lumex::dev::constants.genesis_amount - 500)
						  .link (key3.pub)
						  .sign (lumex::dev::genesis_key.prv, lumex::dev::genesis_key.pub)
						  .work (*pool.generate (epoch2->hash ()))
						  .build ();
	ASSERT_EQ (lumex::block_status::progress, ledger.process (transaction, send_for_epoch));

	// Epoch v1 opening an account
	auto epoch_open = builder.state ()
					  .account (key3.pub)
					  .previous (0)
					  .representative (0)
					  .balance (0)
					  .link (ledger.epoch_link (lumex::epoch::epoch_1))
					  .sign (lumex::dev::genesis_key.prv, lumex::dev::genesis_key.pub)
					  .work (*pool.generate (key3.pub))
					  .build ();
	ASSERT_EQ (lumex::block_status::progress, ledger.process (transaction, epoch_open));
	{
		ASSERT_TRUE (epoch_open->is_epoch ());
		ASSERT_FALSE (epoch_open->is_receive ());
		auto deps = epoch_open->dependencies ();
		ASSERT_EQ (deps[0], zero);
		ASSERT_EQ (deps[1], zero);
	}

	// Send to an account whose public key equals the epoch v1 link
	auto send_to_epoch_link = builder.state ()
							  .account (lumex::dev::genesis_key.pub)
							  .previous (send_for_epoch->hash ())
							  .representative (lumex::dev::genesis_key.pub)
							  .balance (lumex::dev::constants.genesis_amount - 600)
							  .link (ledger.epoch_link (lumex::epoch::epoch_1))
							  .sign (lumex::dev::genesis_key.prv, lumex::dev::genesis_key.pub)
							  .work (*pool.generate (send_for_epoch->hash ()))
							  .build ();
	ASSERT_EQ (lumex::block_status::progress, ledger.process (transaction, send_to_epoch_link));
	{
		ASSERT_TRUE (send_to_epoch_link->is_send ());
		ASSERT_FALSE (send_to_epoch_link->is_epoch ());
		auto deps = send_to_epoch_link->dependencies ();
		ASSERT_EQ (deps[0], send_for_epoch->hash ());
		ASSERT_EQ (deps[1], zero);
	}
}

TEST (ledger, dependencies_cemented)
{
	auto ctx = lumex::test::ledger_empty ();
	auto & ledger = ctx.ledger ();
	auto & store = ctx.store ();
	auto transaction = ledger.tx_begin_write ();
	lumex::block_builder builder;
	ASSERT_TRUE (ledger.dependencies_cemented (transaction, *lumex::dev::genesis));
	auto & pool = ctx.pool ();
	lumex::keypair key1;
	auto send1 = builder.state ()
				 .account (lumex::dev::genesis_key.pub)
				 .previous (lumex::dev::genesis->hash ())
				 .representative (lumex::dev::genesis_key.pub)
				 .balance (lumex::dev::constants.genesis_amount - 100)
				 .link (key1.pub)
				 .sign (lumex::dev::genesis_key.prv, lumex::dev::genesis_key.pub)
				 .work (*pool.generate (lumex::dev::genesis->hash ()))
				 .build ();
	ASSERT_EQ (lumex::block_status::progress, ledger.process (transaction, send1));
	ASSERT_TRUE (ledger.dependencies_cemented (transaction, *send1));
	auto send2 = builder.state ()
				 .account (lumex::dev::genesis_key.pub)
				 .previous (send1->hash ())
				 .representative (lumex::dev::genesis_key.pub)
				 .balance (lumex::dev::constants.genesis_amount - 200)
				 .link (key1.pub)
				 .sign (lumex::dev::genesis_key.prv, lumex::dev::genesis_key.pub)
				 .work (*pool.generate (send1->hash ()))
				 .build ();
	ASSERT_EQ (lumex::block_status::progress, ledger.process (transaction, send2));
	ASSERT_FALSE (ledger.dependencies_cemented (transaction, *send2));
	auto receive1 = builder.state ()
					.account (key1.pub)
					.previous (0)
					.representative (lumex::dev::genesis_key.pub)
					.balance (100)
					.link (send1->hash ())
					.sign (key1.prv, key1.pub)
					.work (*pool.generate (key1.pub))
					.build ();
	ASSERT_EQ (lumex::block_status::progress, ledger.process (transaction, receive1));
	ASSERT_FALSE (ledger.dependencies_cemented (transaction, *receive1));
	ledger.cement (transaction, send1->hash ());
	ASSERT_TRUE (ledger.dependencies_cemented (transaction, *receive1));
	auto receive2 = builder.state ()
					.account (key1.pub)
					.previous (receive1->hash ())
					.representative (lumex::dev::genesis_key.pub)
					.balance (200)
					.link (send2->hash ())
					.sign (key1.prv, key1.pub)
					.work (*pool.generate (receive1->hash ()))
					.build ();
	ASSERT_EQ (lumex::block_status::progress, ledger.process (transaction, receive2));
	ASSERT_FALSE (ledger.dependencies_cemented (transaction, *receive2));
	ledger.cement (transaction, receive1->hash ());
	ASSERT_FALSE (ledger.dependencies_cemented (transaction, *receive2));
	ledger.cement (transaction, send2->hash ());
	ASSERT_TRUE (ledger.dependencies_cemented (transaction, *receive2));
}

TEST (ledger, dependencies_cemented_pruning)
{
	lumex::logger logger;
	lumex::stats stats{ logger };
	auto store = lumex::test::make_store (logger, stats);
	// Topo index is incompatible with pruning
	lumex::ledger ledger (*store, lumex::dev::network_params, stats, logger, lumex::ledger_options{ .enable_topo_index = false });
	ledger.pruning = true;
	auto transaction = ledger.tx_begin_write ();
	lumex::block_builder builder;
	lumex::work_pool pool{ lumex::dev::network_params.network, std::numeric_limits<unsigned>::max () };
	lumex::keypair key1;
	auto send1 = builder.state ()
				 .account (lumex::dev::genesis_key.pub)
				 .previous (lumex::dev::genesis->hash ())
				 .representative (lumex::dev::genesis_key.pub)
				 .balance (lumex::dev::constants.genesis_amount - 100)
				 .link (key1.pub)
				 .sign (lumex::dev::genesis_key.prv, lumex::dev::genesis_key.pub)
				 .work (*pool.generate (lumex::dev::genesis->hash ()))
				 .build ();
	ASSERT_EQ (lumex::block_status::progress, ledger.process (transaction, send1));
	auto send2 = builder.state ()
				 .account (lumex::dev::genesis_key.pub)
				 .previous (send1->hash ())
				 .representative (lumex::dev::genesis_key.pub)
				 .balance (lumex::dev::constants.genesis_amount - 200)
				 .link (key1.pub)
				 .sign (lumex::dev::genesis_key.prv, lumex::dev::genesis_key.pub)
				 .work (*pool.generate (send1->hash ()))
				 .build ();
	ASSERT_EQ (lumex::block_status::progress, ledger.process (transaction, send2));
	ledger.cement (transaction, send2->hash ());
	ASSERT_TRUE (ledger.cemented.block_exists_or_pruned (transaction, send1->hash ()));
	ASSERT_EQ (2, ledger.pruning_action (transaction, send2->hash (), 1));
	auto receive1 = builder.state ()
					.account (key1.pub)
					.previous (0)
					.representative (lumex::dev::genesis_key.pub)
					.balance (100)
					.link (send1->hash ())
					.sign (key1.prv, key1.pub)
					.work (*pool.generate (key1.pub))
					.build ();
	ASSERT_EQ (lumex::block_status::progress, ledger.process (transaction, receive1));
	ASSERT_TRUE (ledger.dependencies_cemented (transaction, *receive1));
}

TEST (ledger, block_confirmed)
{
	auto ctx = lumex::test::ledger_empty ();
	auto & ledger = ctx.ledger ();
	auto & store = ctx.store ();
	auto transaction = ledger.tx_begin_write ();
	lumex::block_builder builder;
	ASSERT_TRUE (ledger.cemented.block_exists_or_pruned (transaction, lumex::dev::genesis->hash ()));
	auto & pool = ctx.pool ();
	lumex::keypair key1;
	auto send1 = builder.state ()
				 .account (lumex::dev::genesis_key.pub)
				 .previous (lumex::dev::genesis->hash ())
				 .representative (lumex::dev::genesis_key.pub)
				 .balance (lumex::dev::constants.genesis_amount - 100)
				 .link (key1.pub)
				 .sign (lumex::dev::genesis_key.prv, lumex::dev::genesis_key.pub)
				 .work (*pool.generate (lumex::dev::genesis->hash ()))
				 .build ();
	// Must be safe against non-existing blocks
	ASSERT_FALSE (ledger.cemented.block_exists_or_pruned (transaction, send1->hash ()));
	ASSERT_EQ (lumex::block_status::progress, ledger.process (transaction, send1));
	ASSERT_FALSE (ledger.cemented.block_exists_or_pruned (transaction, send1->hash ()));
	ledger.cement (transaction, send1->hash ());
	ASSERT_TRUE (ledger.cemented.block_exists_or_pruned (transaction, send1->hash ()));
}

TEST (ledger, cache)
{
	auto ctx = lumex::test::ledger_empty ();
	auto & ledger = ctx.ledger ();
	auto & store = ctx.store ();
	auto & stats = ctx.stats ();
	auto & logger = ctx.logger ();
	auto & pool = ctx.pool ();
	lumex::block_builder builder;

	size_t const total = 100;

	// Check existing ledger (incremental cache update) and reload on a new ledger
	for (size_t i (0); i < total; ++i)
	{
		auto account_count = 1 + i;
		auto block_count = 1 + 2 * (i + 1) - 2;
		auto cemented_count = 1 + 2 * (i + 1) - 2;
		auto genesis_weight = lumex::dev::constants.genesis_amount - i;
		auto pruned_count = i;

		auto cache_check = [&, i] (lumex::ledger const & ledger) {
			ASSERT_EQ (account_count, ledger.account_count ());
			ASSERT_EQ (block_count, ledger.block_count ());
			ASSERT_EQ (cemented_count, ledger.cemented_count ());
			ASSERT_EQ (genesis_weight, ledger.rep_weights.get (lumex::dev::genesis_key.pub));
			ASSERT_EQ (pruned_count, ledger.pruned_count ());
		};

		cache_check (ledger);
		cache_check (lumex::ledger (store, lumex::dev::network_params, stats, logger));

		lumex::keypair key;
		auto const latest = ledger.any.account_head (ledger.tx_begin_read (), lumex::dev::genesis_key.pub);
		auto send = builder.state ()
					.account (lumex::dev::genesis_key.pub)
					.previous (latest)
					.representative (lumex::dev::genesis_key.pub)
					.balance (lumex::dev::constants.genesis_amount - (i + 1))
					.link (key.pub)
					.sign (lumex::dev::genesis_key.prv, lumex::dev::genesis_key.pub)
					.work (*pool.generate (latest))
					.build ();
		auto open = builder.state ()
					.account (key.pub)
					.previous (0)
					.representative (key.pub)
					.balance (1)
					.link (send->hash ())
					.sign (key.prv, key.pub)
					.work (*pool.generate (key.pub))
					.build ();
		{
			auto transaction = ledger.tx_begin_write ();
			ASSERT_EQ (lumex::block_status::progress, ledger.process (transaction, send));
		}

		++block_count;
		--genesis_weight;
		cache_check (ledger);
		cache_check (lumex::ledger (store, lumex::dev::network_params, stats, logger));

		{
			auto transaction = ledger.tx_begin_write ();
			ASSERT_EQ (lumex::block_status::progress, ledger.process (transaction, open));
		}

		++block_count;
		++account_count;
		cache_check (ledger);
		cache_check (lumex::ledger (store, lumex::dev::network_params, stats, logger));

		{
			auto transaction = ledger.tx_begin_write ();
			ledger.cement (transaction, send->hash ());
			ASSERT_TRUE (ledger.cemented.block_exists_or_pruned (transaction, send->hash ()));
		}

		++cemented_count;
		cache_check (ledger);
		cache_check (lumex::ledger (store, lumex::dev::network_params, stats, logger));

		{
			auto transaction = ledger.tx_begin_write ();
			ledger.cement (transaction, open->hash ());
			ASSERT_TRUE (ledger.cemented.block_exists_or_pruned (transaction, open->hash ()));
		}

		++cemented_count;
		cache_check (ledger);
		cache_check (lumex::ledger (store, lumex::dev::network_params, stats, logger));

		{
			auto transaction = ledger.tx_begin_write ();
			ledger.pruning_action (transaction, open->hash (), 1);
		}
		++pruned_count;
		cache_check (ledger);
		cache_check (lumex::ledger (store, lumex::dev::network_params, stats, logger));
	}
}

TEST (ledger, pruning_action)
{
	lumex::logger logger;
	lumex::stats stats{ logger };
	auto store = lumex::test::make_store (logger, stats);
	// Topo index is incompatible with pruning
	lumex::ledger ledger (*store, lumex::dev::network_params, stats, logger, lumex::ledger_options{ .enable_topo_index = false });
	ledger.pruning = true;
	auto transaction = ledger.tx_begin_write ();
	lumex::work_pool pool{ lumex::dev::network_params.network, std::numeric_limits<unsigned>::max () };
	lumex::block_builder builder;
	auto send1 = builder
				 .state ()
				 .account (lumex::dev::genesis_key.pub)
				 .previous (lumex::dev::genesis->hash ())
				 .representative (lumex::dev::genesis_key.pub)
				 .balance (lumex::dev::constants.genesis_amount - lumex::Klumex_ratio)
				 .link (lumex::dev::genesis_key.pub)
				 .sign (lumex::dev::genesis_key.prv, lumex::dev::genesis_key.pub)
				 .work (*pool.generate (lumex::dev::genesis->hash ()))
				 .build ();
	ASSERT_EQ (lumex::block_status::progress, ledger.process (transaction, send1));
	ASSERT_TRUE (ledger.any.block_exists (transaction, send1->hash ()));
	auto send1_stored (store->block.get (transaction, send1->hash ()));
	ASSERT_NE (nullptr, send1_stored);
	ASSERT_EQ (*send1, *send1_stored);
	ASSERT_TRUE (ledger.any.pending_get (transaction, lumex::pending_key{ lumex::dev::genesis_key.pub, send1->hash () }));
	auto send2 = builder
				 .state ()
				 .account (lumex::dev::genesis_key.pub)
				 .previous (send1->hash ())
				 .representative (lumex::dev::genesis_key.pub)
				 .balance (lumex::dev::constants.genesis_amount - lumex::Klumex_ratio * 2)
				 .link (lumex::dev::genesis_key.pub)
				 .sign (lumex::dev::genesis_key.prv, lumex::dev::genesis_key.pub)
				 .work (*pool.generate (send1->hash ()))
				 .build ();
	ASSERT_EQ (lumex::block_status::progress, ledger.process (transaction, send2));
	ASSERT_TRUE (ledger.any.block_exists (transaction, send2->hash ()));
	// Pruning action
	ledger.cement (transaction, send1->hash ());
	ASSERT_EQ (1, ledger.pruning_action (transaction, send1->hash (), 1));
	ASSERT_EQ (0, ledger.pruning_action (transaction, lumex::dev::genesis->hash (), 1));
	ASSERT_TRUE (ledger.any.pending_get (transaction, lumex::pending_key{ lumex::dev::genesis_key.pub, send1->hash () }));
	ASSERT_FALSE (ledger.any.block_exists (transaction, send1->hash ()));
	ASSERT_TRUE (ledger.any.block_exists_or_pruned (transaction, send1->hash ()));
	// Pruned ledger start without proper flags emulation
	ledger.pruning = false;
	ASSERT_TRUE (ledger.any.block_exists_or_pruned (transaction, send1->hash ()));
	ledger.pruning = true;
	ASSERT_TRUE (store->pruned.exists (transaction, send1->hash ()));
	ASSERT_TRUE (ledger.any.block_exists (transaction, lumex::dev::genesis->hash ()));
	ASSERT_TRUE (ledger.any.block_exists (transaction, send2->hash ()));
	// Receiving pruned block
	auto receive1 = builder
					.state ()
					.account (lumex::dev::genesis_key.pub)
					.previous (send2->hash ())
					.representative (lumex::dev::genesis_key.pub)
					.balance (lumex::dev::constants.genesis_amount - lumex::Klumex_ratio)
					.link (send1->hash ())
					.sign (lumex::dev::genesis_key.prv, lumex::dev::genesis_key.pub)
					.work (*pool.generate (send2->hash ()))
					.build ();
	ASSERT_EQ (lumex::block_status::progress, ledger.process (transaction, receive1));
	ASSERT_TRUE (ledger.any.block_exists (transaction, receive1->hash ()));
	auto receive1_stored (store->block.get (transaction, receive1->hash ()));
	ASSERT_NE (nullptr, receive1_stored);
	ASSERT_EQ (*receive1, *receive1_stored);
	ASSERT_FALSE (ledger.any.pending_get (transaction, lumex::pending_key{ lumex::dev::genesis_key.pub, send1->hash () }));
	ASSERT_EQ (4, receive1_stored->sideband ().height);
	ASSERT_FALSE (receive1_stored->is_send ());
	ASSERT_TRUE (receive1_stored->is_receive ());
	ASSERT_FALSE (receive1_stored->sideband ().details.is_epoch);
	// Middle block pruning
	ASSERT_TRUE (ledger.any.block_exists (transaction, send2->hash ()));
	ledger.cement (transaction, send2->hash ());
	ASSERT_EQ (1, ledger.pruning_action (transaction, send2->hash (), 1));
	ASSERT_TRUE (store->pruned.exists (transaction, send2->hash ()));
	ASSERT_FALSE (ledger.any.block_exists (transaction, send2->hash ()));
	ASSERT_EQ (store->account.count (transaction), ledger.account_count ());
	ASSERT_EQ (store->pruned.count (transaction), ledger.pruned_count ());
	ASSERT_EQ (store->block.count (transaction), ledger.block_count () - ledger.pruned_count ());
}

TEST (ledger, pruning_large_chain)
{
	lumex::logger logger;
	lumex::stats stats{ logger };
	auto store = lumex::test::make_store (logger, stats);
	// Topo index is incompatible with pruning
	lumex::ledger ledger (*store, lumex::dev::network_params, stats, logger, lumex::ledger_options{ .enable_topo_index = false });
	ledger.pruning = true;
	auto transaction = ledger.tx_begin_write ();
	lumex::work_pool pool{ lumex::dev::network_params.network, std::numeric_limits<unsigned>::max () };
	size_t send_receive_pairs (20);
	auto last_hash (lumex::dev::genesis->hash ());
	lumex::block_builder builder;
	for (auto i (0); i < send_receive_pairs; i++)
	{
		auto send = builder
					.state ()
					.account (lumex::dev::genesis_key.pub)
					.previous (last_hash)
					.representative (lumex::dev::genesis_key.pub)
					.balance (lumex::dev::constants.genesis_amount - lumex::Klumex_ratio)
					.link (lumex::dev::genesis_key.pub)
					.sign (lumex::dev::genesis_key.prv, lumex::dev::genesis_key.pub)
					.work (*pool.generate (last_hash))
					.build ();
		ASSERT_EQ (lumex::block_status::progress, ledger.process (transaction, send));
		ASSERT_TRUE (ledger.any.block_exists (transaction, send->hash ()));
		auto receive = builder
					   .state ()
					   .account (lumex::dev::genesis_key.pub)
					   .previous (send->hash ())
					   .representative (lumex::dev::genesis_key.pub)
					   .balance (lumex::dev::constants.genesis_amount)
					   .link (send->hash ())
					   .sign (lumex::dev::genesis_key.prv, lumex::dev::genesis_key.pub)
					   .work (*pool.generate (send->hash ()))
					   .build ();
		ASSERT_EQ (lumex::block_status::progress, ledger.process (transaction, receive));
		ASSERT_TRUE (ledger.any.block_exists (transaction, receive->hash ()));
		last_hash = receive->hash ();
	}
	ASSERT_EQ (0, store->pruned.count (transaction));
	ASSERT_EQ (send_receive_pairs * 2 + 1, store->block.count (transaction));
	ledger.cement (transaction, last_hash);
	// Pruning action
	ASSERT_EQ (send_receive_pairs * 2, ledger.pruning_action (transaction, last_hash, 5));
	ASSERT_TRUE (store->pruned.exists (transaction, last_hash));
	ASSERT_TRUE (ledger.any.block_exists (transaction, lumex::dev::genesis->hash ()));
	ASSERT_FALSE (ledger.any.block_exists (transaction, last_hash));
	ASSERT_EQ (store->pruned.count (transaction), ledger.pruned_count ());
	ASSERT_EQ (store->block.count (transaction), ledger.block_count () - ledger.pruned_count ());
	ASSERT_EQ (send_receive_pairs * 2, store->pruned.count (transaction));
	ASSERT_EQ (1, store->block.count (transaction)); // Genesis
}

TEST (ledger, pruning_source_rollback)
{
	lumex::logger logger;
	lumex::stats stats{ logger };
	auto store = lumex::test::make_store (logger, stats);
	// Topo index is incompatible with pruning
	lumex::ledger ledger (*store, lumex::dev::network_params, stats, logger, lumex::ledger_options{ .enable_topo_index = false });
	ledger.pruning = true;
	auto transaction = ledger.tx_begin_write ();
	lumex::work_pool pool{ lumex::dev::network_params.network, std::numeric_limits<unsigned>::max () };
	lumex::block_builder builder;
	auto epoch1 = builder
				  .state ()
				  .account (lumex::dev::genesis_key.pub)
				  .previous (lumex::dev::genesis->hash ())
				  .representative (lumex::dev::genesis_key.pub)
				  .balance (lumex::dev::constants.genesis_amount)
				  .link (ledger.epoch_link (lumex::epoch::epoch_1))
				  .sign (lumex::dev::genesis_key.prv, lumex::dev::genesis_key.pub)
				  .work (*pool.generate (lumex::dev::genesis->hash ()))
				  .build ();
	ASSERT_EQ (lumex::block_status::progress, ledger.process (transaction, epoch1));
	auto send1 = builder
				 .state ()
				 .account (lumex::dev::genesis_key.pub)
				 .previous (epoch1->hash ())
				 .representative (lumex::dev::genesis_key.pub)
				 .balance (lumex::dev::constants.genesis_amount - lumex::Klumex_ratio)
				 .link (lumex::dev::genesis_key.pub)
				 .sign (lumex::dev::genesis_key.prv, lumex::dev::genesis_key.pub)
				 .work (*pool.generate (epoch1->hash ()))
				 .build ();
	ASSERT_EQ (lumex::block_status::progress, ledger.process (transaction, send1));
	ASSERT_TRUE (ledger.any.pending_get (transaction, lumex::pending_key{ lumex::dev::genesis_key.pub, send1->hash () }));
	auto send2 = builder
				 .state ()
				 .account (lumex::dev::genesis_key.pub)
				 .previous (send1->hash ())
				 .representative (lumex::dev::genesis_key.pub)
				 .balance (lumex::dev::constants.genesis_amount - lumex::Klumex_ratio * 2)
				 .link (lumex::dev::genesis_key.pub)
				 .sign (lumex::dev::genesis_key.prv, lumex::dev::genesis_key.pub)
				 .work (*pool.generate (send1->hash ()))
				 .build ();
	ASSERT_EQ (lumex::block_status::progress, ledger.process (transaction, send2));
	ASSERT_TRUE (ledger.any.block_exists (transaction, send2->hash ()));
	ledger.cement (transaction, send1->hash ());
	// Pruning action
	ASSERT_EQ (2, ledger.pruning_action (transaction, send1->hash (), 1));
	ASSERT_FALSE (ledger.any.block_exists (transaction, send1->hash ()));
	ASSERT_TRUE (store->pruned.exists (transaction, send1->hash ()));
	ASSERT_FALSE (ledger.any.block_exists (transaction, epoch1->hash ()));
	ASSERT_TRUE (store->pruned.exists (transaction, epoch1->hash ()));
	ASSERT_TRUE (ledger.any.block_exists (transaction, lumex::dev::genesis->hash ()));
	auto info = ledger.any.pending_get (transaction, lumex::pending_key (lumex::dev::genesis_key.pub, send1->hash ()));
	ASSERT_TRUE (info);
	ASSERT_EQ (lumex::dev::genesis_key.pub, info->source);
	ASSERT_EQ (lumex::Klumex_ratio, info->amount.number ());
	ASSERT_EQ (lumex::epoch::epoch_1, info->epoch);
	// Receiving pruned block
	auto receive1 = builder
					.state ()
					.account (lumex::dev::genesis_key.pub)
					.previous (send2->hash ())
					.representative (lumex::dev::genesis_key.pub)
					.balance (lumex::dev::constants.genesis_amount - lumex::Klumex_ratio)
					.link (send1->hash ())
					.sign (lumex::dev::genesis_key.prv, lumex::dev::genesis_key.pub)
					.work (*pool.generate (send2->hash ()))
					.build ();
	ASSERT_EQ (lumex::block_status::progress, ledger.process (transaction, receive1));
	ASSERT_FALSE (ledger.any.pending_get (transaction, lumex::pending_key{ lumex::dev::genesis_key.pub, send1->hash () }));
	ASSERT_EQ (2, ledger.pruned_count ());
	ASSERT_EQ (5, ledger.block_count ());
	// Rollback receive block
	ASSERT_FALSE (ledger.rollback (transaction, receive1->hash ()));
	auto info2 = ledger.any.pending_get (transaction, lumex::pending_key (lumex::dev::genesis_key.pub, send1->hash ()));
	ASSERT_TRUE (info2);
	ASSERT_NE (lumex::dev::genesis_key.pub, info2->source); // Tradeoff to not store pruned blocks accounts
	ASSERT_EQ (lumex::Klumex_ratio, info2->amount.number ());
	ASSERT_EQ (lumex::epoch::epoch_1, info2->epoch);
	// Process receive block again
	ASSERT_EQ (lumex::block_status::progress, ledger.process (transaction, receive1));
	ASSERT_FALSE (ledger.any.pending_get (transaction, lumex::pending_key{ lumex::dev::genesis_key.pub, send1->hash () }));
	ASSERT_EQ (2, ledger.pruned_count ());
	ASSERT_EQ (5, ledger.block_count ());
}

TEST (ledger, pruning_source_rollback_legacy)
{
	lumex::logger logger;
	lumex::stats stats{ logger };
	auto store = lumex::test::make_store (logger, stats);
	// Topo index is incompatible with pruning
	lumex::ledger ledger (*store, lumex::dev::network_params, stats, logger, lumex::ledger_options{ .enable_topo_index = false });
	ledger.pruning = true;
	auto transaction = ledger.tx_begin_write ();
	lumex::work_pool pool{ lumex::dev::network_params.network, std::numeric_limits<unsigned>::max () };
	lumex::block_builder builder;
	auto send1 = builder
				 .send ()
				 .previous (lumex::dev::genesis->hash ())
				 .destination (lumex::dev::genesis_key.pub)
				 .balance (lumex::dev::constants.genesis_amount - lumex::Klumex_ratio)
				 .sign (lumex::dev::genesis_key.prv, lumex::dev::genesis_key.pub)
				 .work (*pool.generate (lumex::dev::genesis->hash ()))
				 .build ();
	ASSERT_EQ (lumex::block_status::progress, ledger.process (transaction, send1));
	ASSERT_TRUE (ledger.any.pending_get (transaction, lumex::pending_key{ lumex::dev::genesis_key.pub, send1->hash () }));
	lumex::keypair key1;
	auto send2 = builder
				 .send ()
				 .previous (send1->hash ())
				 .destination (key1.pub)
				 .balance (lumex::dev::constants.genesis_amount - 2 * lumex::Klumex_ratio)
				 .sign (lumex::dev::genesis_key.prv, lumex::dev::genesis_key.pub)
				 .work (*pool.generate (send1->hash ()))
				 .build ();
	ASSERT_EQ (lumex::block_status::progress, ledger.process (transaction, send2));
	ASSERT_TRUE (ledger.any.block_exists (transaction, send2->hash ()));
	ASSERT_TRUE (ledger.any.pending_get (transaction, lumex::pending_key{ key1.pub, send2->hash () }));
	auto send3 = builder
				 .send ()
				 .previous (send2->hash ())
				 .destination (lumex::dev::genesis_key.pub)
				 .balance (lumex::dev::constants.genesis_amount - 3 * lumex::Klumex_ratio)
				 .sign (lumex::dev::genesis_key.prv, lumex::dev::genesis_key.pub)
				 .work (*pool.generate (send2->hash ()))
				 .build ();
	ASSERT_EQ (lumex::block_status::progress, ledger.process (transaction, send3));
	ASSERT_TRUE (ledger.any.block_exists (transaction, send3->hash ()));
	ASSERT_TRUE (ledger.any.pending_get (transaction, lumex::pending_key{ lumex::dev::genesis_key.pub, send3->hash () }));
	ledger.cement (transaction, send2->hash ());
	// Pruning action
	ASSERT_EQ (2, ledger.pruning_action (transaction, send2->hash (), 1));
	ASSERT_FALSE (ledger.any.block_exists (transaction, send2->hash ()));
	ASSERT_TRUE (store->pruned.exists (transaction, send2->hash ()));
	ASSERT_FALSE (ledger.any.block_exists (transaction, send1->hash ()));
	ASSERT_TRUE (store->pruned.exists (transaction, send1->hash ()));
	ASSERT_TRUE (ledger.any.block_exists (transaction, lumex::dev::genesis->hash ()));
	auto info1 = ledger.any.pending_get (transaction, lumex::pending_key (lumex::dev::genesis_key.pub, send1->hash ()));
	ASSERT_TRUE (info1);
	ASSERT_EQ (lumex::dev::genesis_key.pub, info1->source);
	ASSERT_EQ (lumex::Klumex_ratio, info1->amount.number ());
	ASSERT_EQ (lumex::epoch::epoch_0, info1->epoch);
	auto info2 = ledger.any.pending_get (transaction, lumex::pending_key (key1.pub, send2->hash ()));
	ASSERT_TRUE (info2);
	ASSERT_EQ (lumex::dev::genesis_key.pub, info2->source);
	ASSERT_EQ (lumex::Klumex_ratio, info2->amount.number ());
	ASSERT_EQ (lumex::epoch::epoch_0, info2->epoch);
	// Receiving pruned block
	auto receive1 = builder
					.receive ()
					.previous (send3->hash ())
					.source (send1->hash ())
					.sign (lumex::dev::genesis_key.prv, lumex::dev::genesis_key.pub)
					.work (*pool.generate (send3->hash ()))
					.build ();
	ASSERT_EQ (lumex::block_status::progress, ledger.process (transaction, receive1));
	ASSERT_FALSE (ledger.any.pending_get (transaction, lumex::pending_key{ lumex::dev::genesis_key.pub, send1->hash () }));
	ASSERT_EQ (2, ledger.pruned_count ());
	ASSERT_EQ (5, ledger.block_count ());
	// Rollback receive block
	ASSERT_FALSE (ledger.rollback (transaction, receive1->hash ()));
	auto info3 = ledger.any.pending_get (transaction, lumex::pending_key (lumex::dev::genesis_key.pub, send1->hash ()));
	ASSERT_TRUE (info3);
	ASSERT_NE (lumex::dev::genesis_key.pub, info3->source); // Tradeoff to not store pruned blocks accounts
	ASSERT_EQ (lumex::Klumex_ratio, info3->amount.number ());
	ASSERT_EQ (lumex::epoch::epoch_0, info3->epoch);
	// Process receive block again
	ASSERT_EQ (lumex::block_status::progress, ledger.process (transaction, receive1));
	ASSERT_FALSE (ledger.any.pending_get (transaction, lumex::pending_key{ lumex::dev::genesis_key.pub, send1->hash () }));
	ASSERT_EQ (2, ledger.pruned_count ());
	ASSERT_EQ (5, ledger.block_count ());
	// Receiving pruned block (open)
	auto open1 = builder
				 .open ()
				 .source (send2->hash ())
				 .representative (lumex::dev::genesis_key.pub)
				 .account (key1.pub)
				 .sign (key1.prv, key1.pub)
				 .work (*pool.generate (key1.pub))
				 .build ();
	ASSERT_EQ (lumex::block_status::progress, ledger.process (transaction, open1));
	ASSERT_FALSE (ledger.any.pending_get (transaction, lumex::pending_key{ key1.pub, send2->hash () }));
	ASSERT_EQ (2, ledger.pruned_count ());
	ASSERT_EQ (6, ledger.block_count ());
	// Rollback open block
	ASSERT_FALSE (ledger.rollback (transaction, open1->hash ()));
	auto info4 = ledger.any.pending_get (transaction, lumex::pending_key (key1.pub, send2->hash ()));
	ASSERT_TRUE (info4);
	ASSERT_NE (lumex::dev::genesis_key.pub, info4->source); // Tradeoff to not store pruned blocks accounts
	ASSERT_EQ (lumex::Klumex_ratio, info4->amount.number ());
	ASSERT_EQ (lumex::epoch::epoch_0, info4->epoch);
	// Process open block again
	ASSERT_EQ (lumex::block_status::progress, ledger.process (transaction, open1));
	ASSERT_FALSE (ledger.any.pending_get (transaction, lumex::pending_key{ key1.pub, send2->hash () }));
	ASSERT_EQ (2, ledger.pruned_count ());
	ASSERT_EQ (6, ledger.block_count ());
}

TEST (ledger, pruning_legacy_blocks)
{
	lumex::logger logger;
	lumex::stats stats{ logger };
	auto store = lumex::test::make_store (logger, stats);
	// Topo index is incompatible with pruning
	lumex::ledger ledger (*store, lumex::dev::network_params, stats, logger, lumex::ledger_options{ .enable_topo_index = false });
	ledger.pruning = true;
	lumex::keypair key1;
	auto transaction = ledger.tx_begin_write ();
	lumex::work_pool pool{ lumex::dev::network_params.network, std::numeric_limits<unsigned>::max () };
	lumex::block_builder builder;
	auto send1 = builder
				 .send ()
				 .previous (lumex::dev::genesis->hash ())
				 .destination (lumex::dev::genesis_key.pub)
				 .balance (lumex::dev::constants.genesis_amount - lumex::Klumex_ratio)
				 .sign (lumex::dev::genesis_key.prv, lumex::dev::genesis_key.pub)
				 .work (*pool.generate (lumex::dev::genesis->hash ()))
				 .build ();
	ASSERT_EQ (lumex::block_status::progress, ledger.process (transaction, send1));
	ASSERT_TRUE (ledger.any.pending_get (transaction, lumex::pending_key{ lumex::dev::genesis_key.pub, send1->hash () }));
	auto receive1 = builder
					.receive ()
					.previous (send1->hash ())
					.source (send1->hash ())
					.sign (lumex::dev::genesis_key.prv, lumex::dev::genesis_key.pub)
					.work (*pool.generate (send1->hash ()))
					.build ();
	ASSERT_EQ (lumex::block_status::progress, ledger.process (transaction, receive1));
	auto change1 = builder
				   .change ()
				   .previous (receive1->hash ())
				   .representative (key1.pub)
				   .sign (lumex::dev::genesis_key.prv, lumex::dev::genesis_key.pub)
				   .work (*pool.generate (receive1->hash ()))
				   .build ();
	ASSERT_EQ (lumex::block_status::progress, ledger.process (transaction, change1));
	auto send2 = builder
				 .send ()
				 .previous (change1->hash ())
				 .destination (key1.pub)
				 .balance (lumex::dev::constants.genesis_amount - lumex::Klumex_ratio)
				 .sign (lumex::dev::genesis_key.prv, lumex::dev::genesis_key.pub)
				 .work (*pool.generate (change1->hash ()))
				 .build ();
	ASSERT_EQ (lumex::block_status::progress, ledger.process (transaction, send2));
	auto open1 = builder
				 .open ()
				 .source (send2->hash ())
				 .representative (lumex::dev::genesis_key.pub)
				 .account (key1.pub)
				 .sign (key1.prv, key1.pub)
				 .work (*pool.generate (key1.pub))
				 .build ();
	ASSERT_EQ (lumex::block_status::progress, ledger.process (transaction, open1));
	auto send3 = builder
				 .send ()
				 .previous (open1->hash ())
				 .destination (lumex::dev::genesis_key.pub)
				 .balance (0)
				 .sign (key1.prv, key1.pub)
				 .work (*pool.generate (open1->hash ()))
				 .build ();
	ASSERT_EQ (lumex::block_status::progress, ledger.process (transaction, send3));
	ledger.cement (transaction, open1->hash ());
	// Pruning action
	ASSERT_EQ (3, ledger.pruning_action (transaction, change1->hash (), 2));
	ASSERT_EQ (1, ledger.pruning_action (transaction, open1->hash (), 1));
	ASSERT_TRUE (ledger.any.block_exists (transaction, lumex::dev::genesis->hash ()));
	ASSERT_FALSE (ledger.any.block_exists (transaction, send1->hash ()));
	ASSERT_TRUE (store->pruned.exists (transaction, send1->hash ()));
	ASSERT_FALSE (ledger.any.block_exists (transaction, receive1->hash ()));
	ASSERT_TRUE (store->pruned.exists (transaction, receive1->hash ()));
	ASSERT_FALSE (ledger.any.block_exists (transaction, change1->hash ()));
	ASSERT_TRUE (store->pruned.exists (transaction, change1->hash ()));
	ASSERT_TRUE (ledger.any.block_exists (transaction, send2->hash ()));
	ASSERT_FALSE (ledger.any.block_exists (transaction, open1->hash ()));
	ASSERT_TRUE (store->pruned.exists (transaction, open1->hash ()));
	ASSERT_TRUE (ledger.any.block_exists (transaction, send3->hash ()));
	ASSERT_EQ (4, ledger.pruned_count ());
	ASSERT_EQ (7, ledger.block_count ());
	ASSERT_EQ (store->pruned.count (transaction), ledger.pruned_count ());
	ASSERT_EQ (store->block.count (transaction), ledger.block_count () - ledger.pruned_count ());
}

TEST (ledger, pruning_safe_functions)
{
	lumex::logger logger;
	lumex::stats stats{ logger };
	auto store = lumex::test::make_store (logger, stats);
	// Topo index is incompatible with pruning
	lumex::ledger ledger (*store, lumex::dev::network_params, stats, logger, lumex::ledger_options{ .enable_topo_index = false });
	ledger.pruning = true;
	auto transaction = ledger.tx_begin_write ();
	lumex::work_pool pool{ lumex::dev::network_params.network, std::numeric_limits<unsigned>::max () };
	lumex::block_builder builder;
	auto send1 = builder
				 .state ()
				 .account (lumex::dev::genesis_key.pub)
				 .previous (lumex::dev::genesis->hash ())
				 .representative (lumex::dev::genesis_key.pub)
				 .balance (lumex::dev::constants.genesis_amount - lumex::Klumex_ratio)
				 .link (lumex::dev::genesis_key.pub)
				 .sign (lumex::dev::genesis_key.prv, lumex::dev::genesis_key.pub)
				 .work (*pool.generate (lumex::dev::genesis->hash ()))
				 .build ();
	ASSERT_EQ (lumex::block_status::progress, ledger.process (transaction, send1));
	ASSERT_TRUE (ledger.any.block_exists (transaction, send1->hash ()));
	auto send2 = builder
				 .state ()
				 .account (lumex::dev::genesis_key.pub)
				 .previous (send1->hash ())
				 .representative (lumex::dev::genesis_key.pub)
				 .balance (lumex::dev::constants.genesis_amount - lumex::Klumex_ratio * 2)
				 .link (lumex::dev::genesis_key.pub)
				 .sign (lumex::dev::genesis_key.prv, lumex::dev::genesis_key.pub)
				 .work (*pool.generate (send1->hash ()))
				 .build ();
	ASSERT_EQ (lumex::block_status::progress, ledger.process (transaction, send2));
	ASSERT_TRUE (ledger.any.block_exists (transaction, send2->hash ()));
	ledger.cement (transaction, send1->hash ());
	// Pruning action
	ASSERT_EQ (1, ledger.pruning_action (transaction, send1->hash (), 1));
	ASSERT_FALSE (ledger.any.block_exists (transaction, send1->hash ()));
	ASSERT_TRUE (ledger.any.block_exists_or_pruned (transaction, send1->hash ())); // true for pruned
	ASSERT_TRUE (store->pruned.exists (transaction, send1->hash ()));
	ASSERT_TRUE (ledger.any.block_exists (transaction, lumex::dev::genesis->hash ()));
	ASSERT_TRUE (ledger.any.block_exists (transaction, send2->hash ()));
	// Safe ledger actions
	ASSERT_FALSE (ledger.any.block_balance (transaction, send1->hash ()));
	ASSERT_EQ (lumex::dev::constants.genesis_amount - lumex::Klumex_ratio * 2, ledger.any.block_balance (transaction, send2->hash ()).value ().number ());
	ASSERT_FALSE (ledger.any.block_amount (transaction, send2->hash ()));
	ASSERT_FALSE (ledger.any.block_account (transaction, send1->hash ()));
	ASSERT_EQ (lumex::dev::genesis_key.pub, ledger.any.block_account (transaction, send2->hash ()).value ());
}

TEST (ledger, random_blocks)
{
	lumex::logger logger;
	lumex::stats stats{ logger };
	auto store = lumex::test::make_store (logger, stats);
	// Topo index is incompatible with pruning
	lumex::ledger ledger (*store, lumex::dev::network_params, stats, logger, lumex::ledger_options{ .enable_topo_index = false });
	ledger.pruning = true;
	auto transaction = ledger.tx_begin_write ();
	lumex::work_pool pool{ lumex::dev::network_params.network, std::numeric_limits<unsigned>::max () };
	lumex::block_builder builder;
	auto send1 = builder
				 .state ()
				 .account (lumex::dev::genesis_key.pub)
				 .previous (lumex::dev::genesis->hash ())
				 .representative (lumex::dev::genesis_key.pub)
				 .balance (lumex::dev::constants.genesis_amount - lumex::Klumex_ratio)
				 .link (lumex::dev::genesis_key.pub)
				 .sign (lumex::dev::genesis_key.prv, lumex::dev::genesis_key.pub)
				 .work (*pool.generate (lumex::dev::genesis->hash ()))
				 .build ();
	ASSERT_EQ (lumex::block_status::progress, ledger.process (transaction, send1));
	ASSERT_TRUE (ledger.any.block_exists (transaction, send1->hash ()));
	auto send2 = builder
				 .state ()
				 .account (lumex::dev::genesis_key.pub)
				 .previous (send1->hash ())
				 .representative (lumex::dev::genesis_key.pub)
				 .balance (lumex::dev::constants.genesis_amount - lumex::Klumex_ratio * 2)
				 .link (lumex::dev::genesis_key.pub)
				 .sign (lumex::dev::genesis_key.prv, lumex::dev::genesis_key.pub)
				 .work (*pool.generate (send1->hash ()))
				 .build ();
	ASSERT_EQ (lumex::block_status::progress, ledger.process (transaction, send2));
	ASSERT_TRUE (ledger.any.block_exists (transaction, send2->hash ()));
	ledger.cement (transaction, send1->hash ());
	// Pruning action
	ASSERT_EQ (1, ledger.pruning_action (transaction, send1->hash (), 1));
	ASSERT_FALSE (ledger.any.block_exists (transaction, send1->hash ()));
	ASSERT_TRUE (store->pruned.exists (transaction, send1->hash ()));
	ASSERT_TRUE (ledger.any.block_exists (transaction, lumex::dev::genesis->hash ()));
	ASSERT_TRUE (ledger.any.block_exists (transaction, send2->hash ()));
	// Prunned block will not be included in the random selection because it's not in the blocks set
	{
		bool done = false;
		size_t iteration = 0;
		while (!done && iteration < 42)
		{
			++iteration;
			auto blocks = ledger.random_blocks (transaction, 10);
			ASSERT_EQ (blocks.size (), 10); // Random blocks should repeat if the ledger is smaller than the requested count
			auto first = blocks.front ();
			done = (first->hash () == send1->hash ());
		}
		ASSERT_FALSE (done);
	}
	// Genesis and send2 should be included in the random selection
	{
		bool done = false;
		size_t iteration = 0;
		while (!done)
		{
			++iteration;
			auto blocks = ledger.random_blocks (transaction, 1);
			ASSERT_EQ (blocks.size (), 1);
			auto first = blocks.front ();
			done = (first->hash () == send2->hash ());
			ASSERT_LE (iteration, 1000);
		}
	}
	{
		bool done = false;
		size_t iteration = 0;
		while (!done)
		{
			++iteration;
			auto blocks = ledger.random_blocks (transaction, 1);
			ASSERT_EQ (blocks.size (), 1);
			auto first = blocks.front ();
			done = (first->hash () == lumex::dev::genesis->hash ());
			ASSERT_LE (iteration, 1000);
		}
	}
}

TEST (ledger, is_send_genesis)
{
	auto ctx = lumex::test::ledger_empty ();
	auto & ledger = ctx.ledger ();
	auto & store = ctx.store ();
	auto tx = store.tx_begin_read ();
	ASSERT_FALSE (lumex::dev::genesis->is_send ());
}

TEST (ledger, is_send_state)
{
	auto ctx = lumex::test::ledger_send_receive ();
	auto & ledger = ctx.ledger ();
	auto & store = ctx.store ();
	auto tx = store.tx_begin_read ();
	ASSERT_TRUE (ctx.blocks ()[0]->is_send ());
	ASSERT_FALSE (ctx.blocks ()[1]->is_send ());
}

TEST (ledger, is_send_legacy)
{
	auto ctx = lumex::test::ledger_send_receive_legacy ();
	auto & ledger = ctx.ledger ();
	auto & store = ctx.store ();
	auto tx = store.tx_begin_read ();
	ASSERT_TRUE (ctx.blocks ()[0]->is_send ());
	ASSERT_FALSE (ctx.blocks ()[1]->is_send ());
}

TEST (ledger, head_block)
{
	auto ctx = lumex::test::ledger_empty ();
	auto & ledger = ctx.ledger ();
	auto & store = ctx.store ();
	auto tx = ledger.tx_begin_read ();
	ASSERT_EQ (*lumex::dev::genesis, *ledger.any.block_get (tx, ledger.any.account_head (tx, lumex::dev::genesis_key.pub)));
}

TEST (ledger, copy_to_rocksdb)
{
	auto path = lumex::unique_path ();
	lumex::logger logger;
	lumex::stats stats{ logger };
	boost::asio::ip::address_v6 address (boost::asio::ip::make_address_v6 ("::ffff:127.0.0.1"));
	uint16_t port = 100;
	lumex::work_pool pool{ lumex::dev::network_params.network, std::numeric_limits<unsigned>::max () };

	// Create default backend and store
	auto store = lumex::test::make_store (logger, stats, path);
	lumex::ledger ledger{ *store, lumex::dev::network_params, stats, logger };

	auto send = lumex::state_block_builder ()
				.account (lumex::dev::genesis_key.pub)
				.previous (lumex::dev::genesis->hash ())
				.representative (0)
				.link (lumex::account (10))
				.balance (lumex::dev::constants.genesis_amount - 100)
				.sign (lumex::dev::genesis_key.prv, lumex::dev::genesis_key.pub)
				.work (*pool.generate (lumex::dev::genesis->hash ()))
				.build ();

	lumex::endpoint_key endpoint_key (address.to_bytes (), port);
	auto version = lumex::store::ledger_store::version_current;

	{
		auto transaction = ledger.tx_begin_write ();

		store->confirmation_height.put (transaction, lumex::dev::genesis_key.pub, { 2, send->hash () });
		store->online_weight.put (transaction, 100, lumex::amount (2));
		store->peer.put (transaction, endpoint_key, 37);
		store->pending.put (transaction, lumex::pending_key (lumex::dev::genesis_key.pub, send->hash ()), lumex::pending_info (lumex::dev::genesis_key.pub, 100, lumex::epoch::epoch_0));
		store->pruned.put (transaction, send->hash ());
		store->version.put_version (transaction, version);
		send->sideband_set ({});
		store->block.put (transaction, send->hash (), *send);
		store->final_vote.put (transaction, send->qualified_root (), lumex::block_hash (2));
	}

	// Create RocksDB backend and copy data
	{
		auto rocksdb_backend = std::make_unique<lumex::store::rocksdb::backend_rocksdb> (path / "rocksdb_copy", lumex::rocksdb_config{}, logger);
		rocksdb_backend->open (lumex::store::ledger_store::schema_current, lumex::store::open_mode::read_write);

		// Copy the backend
		store->backend.copy_to (*rocksdb_backend);
	}

	// Open RocksDB store for verification (creates new backend)
	{
		auto rocksdb_backend = std::make_unique<lumex::store::rocksdb::backend_rocksdb> (path / "rocksdb_copy", lumex::rocksdb_config{}, logger);
		auto rocksdb_store = std::make_unique<lumex::store::ledger_store> (std::move (rocksdb_backend), lumex::store::open_mode::read_only, stats, logger);

		auto rocksdb_transaction = rocksdb_store->tx_begin_read ();

		ASSERT_TRUE (rocksdb_store->pending.get (rocksdb_transaction, lumex::pending_key (lumex::dev::genesis_key.pub, send->hash ())));

		for (auto i = rocksdb_store->online_weight.begin (rocksdb_transaction); i != rocksdb_store->online_weight.end (rocksdb_transaction); ++i)
		{
			ASSERT_EQ (i->first, 100);
			ASSERT_EQ (i->second, 2);
		}

		ASSERT_EQ (rocksdb_store->online_weight.count (rocksdb_transaction), 1);

		auto block1 = rocksdb_store->block.get (rocksdb_transaction, send->hash ());
		ASSERT_NE (block1, nullptr);
		ASSERT_EQ (*send, *block1);

		ASSERT_TRUE (rocksdb_store->peer.exists (rocksdb_transaction, endpoint_key));
		ASSERT_EQ (rocksdb_store->version.get_version (rocksdb_transaction), version);

		lumex::confirmation_height_info confirmation_height_info;
		ASSERT_FALSE (rocksdb_store->confirmation_height.get (rocksdb_transaction, lumex::dev::genesis_key.pub, confirmation_height_info));
		ASSERT_EQ (confirmation_height_info.height, 2);
		ASSERT_EQ (confirmation_height_info.frontier, send->hash ());

		ASSERT_TRUE (rocksdb_store->final_vote.get (rocksdb_transaction, send->qualified_root ()).has_value ());
		ASSERT_EQ (rocksdb_store->final_vote.get (rocksdb_transaction, send->qualified_root ()).value (), lumex::block_hash (2));
	}
}

// Tests that a fairly complex ledger topology can be cemented in a single batch
TEST (ledger, cement_unbounded)
{
	auto ctx = lumex::test::ledger_diamond (5);
	auto & ledger = ctx.ledger ();
	auto bottom = ctx.blocks ().back ();

	std::deque<std::shared_ptr<lumex::block>> confirmed;
	{
		auto tx = ledger.tx_begin_write ();
		confirmed = ledger.cement (tx, bottom->hash ());
	}
	ASSERT_TRUE (ledger.cemented.block_exists (ledger.tx_begin_read (), bottom->hash ()));
	// Check that all blocks got confirmed in a single call
	ASSERT_TRUE (std::all_of (ctx.blocks ().begin (), ctx.blocks ().end (), [&] (auto const & block) {
		return std::find_if (confirmed.begin (), confirmed.end (), [&] (auto const & block2) {
			return block2->hash () == block->hash ();
		})
		!= confirmed.end ();
	}));
}

// Tests that bounded cementing works when recursion stack is large
TEST (ledger, cement_bounded)
{
	auto ctx = lumex::test::ledger_single_chain (64);
	auto & ledger = ctx.ledger ();
	auto bottom = ctx.blocks ().back ();

	std::deque<std::shared_ptr<lumex::block>> confirmed1, confirmed2, confirmed3;

	{
		// This should only cement some of the dependencies
		auto tx = ledger.tx_begin_write ();
		confirmed1 = ledger.cement (tx, bottom->hash (), /* max cementing batch size */ 3);
	}
	ASSERT_FALSE (ledger.cemented.block_exists (ledger.tx_begin_read (), bottom->hash ()));
	ASSERT_EQ (confirmed1.size (), 3);
	// Only topmost dependencies should get cemented during this call
	ASSERT_TRUE (std::all_of (confirmed1.begin (), confirmed1.end (), [&] (auto const & block) {
		return ledger.dependencies_cemented (ledger.tx_begin_read (), *block);
	}));

	{
		// This should cement a few more dependencies
		auto tx = ledger.tx_begin_write ();
		confirmed2 = ledger.cement (tx, bottom->hash (), /* max cementing batch size */ 16);
	}
	ASSERT_FALSE (ledger.cemented.block_exists (ledger.tx_begin_read (), bottom->hash ()));
	ASSERT_EQ (confirmed2.size (), 16);
	// Only topmost dependencies should get cemented during this call
	ASSERT_TRUE (std::all_of (confirmed2.begin (), confirmed2.end (), [&] (auto const & block) {
		return ledger.dependencies_cemented (ledger.tx_begin_read (), *block);
	}));

	{
		// This should cement the remaining dependencies
		auto tx = ledger.tx_begin_write ();
		confirmed3 = ledger.cement (tx, bottom->hash (), /* max cementing batch size */ 64);
	}
	ASSERT_TRUE (ledger.cemented.block_exists (ledger.tx_begin_read (), bottom->hash ()));
	ASSERT_LE (confirmed3.size (), 64);
	// Every block in the ledger should be cemented
	ASSERT_TRUE (std::all_of (ctx.blocks ().begin (), ctx.blocks ().end (), [&] (auto const & block) {
		return ledger.cemented.block_exists (ledger.tx_begin_read (), block->hash ());
	}));
}

// Tests that bounded cementing works when number of blocks is large but tree height is small (recursion stack is small)
TEST (ledger, cement_bounded_diamond)
{
	auto ctx = lumex::test::ledger_diamond (4);
	auto & ledger = ctx.ledger ();
	auto bottom = ctx.blocks ().back ();

	std::deque<std::shared_ptr<lumex::block>> confirmed1, confirmed2, confirmed3, confirmed4;

	{
		// This should only cement some of the dependencies
		auto tx = ledger.tx_begin_write ();
		confirmed1 = ledger.cement (tx, bottom->hash (), /* max cementing batch size */ 3);
	}
	ASSERT_FALSE (ledger.cemented.block_exists (ledger.tx_begin_read (), bottom->hash ()));
	ASSERT_EQ (confirmed1.size (), 3);
	// Only topmost dependencies should get cemented during this call
	ASSERT_TRUE (std::all_of (confirmed1.begin (), confirmed1.end (), [&] (auto const & block) {
		return ledger.dependencies_cemented (ledger.tx_begin_read (), *block);
	}));

	{
		// This should cement a few more dependencies
		auto tx = ledger.tx_begin_write ();
		confirmed2 = ledger.cement (tx, bottom->hash (), /* max cementing batch size */ 16);
	}
	ASSERT_FALSE (ledger.cemented.block_exists (ledger.tx_begin_read (), bottom->hash ()));
	ASSERT_EQ (confirmed2.size (), 16);
	// Only topmost dependencies should get cemented during this call
	ASSERT_TRUE (std::all_of (confirmed2.begin (), confirmed2.end (), [&] (auto const & block) {
		return ledger.dependencies_cemented (ledger.tx_begin_read (), *block);
	}));

	{
		// A few more bounded calls should confirm the whole tree
		auto tx = ledger.tx_begin_write ();
		confirmed3 = ledger.cement (tx, bottom->hash (), /* max cementing batch size */ 64);
	}
	ASSERT_FALSE (ledger.cemented.block_exists (ledger.tx_begin_read (), bottom->hash ()));
	ASSERT_EQ (confirmed3.size (), 64);
	// Only topmost dependencies should get cemented during this call
	ASSERT_TRUE (std::all_of (confirmed2.begin (), confirmed2.end (), [&] (auto const & block) {
		return ledger.dependencies_cemented (ledger.tx_begin_read (), *block);
	}));

	{
		auto tx = ledger.tx_begin_write ();
		confirmed4 = ledger.cement (tx, bottom->hash (), /* max cementing batch size */ 64);
	}
	ASSERT_TRUE (ledger.cemented.block_exists (ledger.tx_begin_read (), bottom->hash ()));
	ASSERT_LT (confirmed4.size (), 64);
	// Every block in the ledger should be cemented
	ASSERT_TRUE (std::all_of (ctx.blocks ().begin (), ctx.blocks ().end (), [&] (auto const & block) {
		return ledger.cemented.block_exists (ledger.tx_begin_read (), block->hash ());
	}));
}

// Test that nullopt can be returned when there are no receivable entries
TEST (ledger_receivable, upper_bound_account_none)
{
	auto ctx = lumex::test::ledger_empty ();
	ASSERT_EQ (ctx.ledger ().any.receivable_end (), ctx.ledger ().any.receivable_upper_bound (ctx.ledger ().tx_begin_read (), 0));
}

// Test behavior of ledger::receivable_upper_bound when there are receivable entries for multiple accounts
TEST (ledger_receivable, upper_bound_account_key)
{
	auto ctx = lumex::test::ledger_empty ();
	lumex::block_builder builder;
	lumex::keypair key;
	auto send1 = builder
				 .state ()
				 .account (lumex::dev::genesis_key.pub)
				 .previous (lumex::dev::genesis->hash ())
				 .representative (lumex::dev::genesis_key.pub)
				 .balance (lumex::dev::constants.genesis_amount - lumex::Klumex_ratio)
				 .link (key.pub)
				 .sign (lumex::dev::genesis_key.prv, lumex::dev::genesis_key.pub)
				 .work (*ctx.pool ().generate (lumex::dev::genesis->hash ()))
				 .build ();
	ASSERT_EQ (lumex::block_status::progress, ctx.ledger ().process (ctx.ledger ().tx_begin_write (), send1));
	auto send2 = builder
				 .state ()
				 .account (lumex::dev::genesis_key.pub)
				 .previous (send1->hash ())
				 .representative (lumex::dev::genesis_key.pub)
				 .balance (lumex::dev::constants.genesis_amount - 2 * lumex::Klumex_ratio)
				 .link (lumex::dev::genesis_key.pub)
				 .sign (lumex::dev::genesis_key.prv, lumex::dev::genesis_key.pub)
				 .work (*ctx.pool ().generate (send1->hash ()))
				 .build ();
	ASSERT_EQ (lumex::block_status::progress, ctx.ledger ().process (ctx.ledger ().tx_begin_write (), send2));
	auto tx = ctx.ledger ().tx_begin_read ();
	auto & ledger = ctx.ledger ();
	auto next1 = ledger.any.receivable_upper_bound (tx, lumex::dev::genesis_key.pub);
	auto next2 = ledger.any.receivable_upper_bound (tx, key.pub);
	// Depending on which is greater but only one should have a value
	ASSERT_TRUE (next1 == ledger.any.receivable_end () xor next2 == ledger.any.receivable_end ());
	// The account returned should be after the one we searched for
	ASSERT_TRUE (next1 == ledger.any.receivable_end () || next1->first.account == key.pub);
	ASSERT_TRUE (next2 == ledger.any.receivable_end () || next2->first.account == lumex::dev::genesis_key.pub);
	auto next3 = ledger.any.receivable_upper_bound (tx, lumex::dev::genesis_key.pub, 0);
	auto next4 = ledger.any.receivable_upper_bound (tx, key.pub, 0);
	// Neither account has more than one receivable
	ASSERT_TRUE (next3 != ledger.any.receivable_end () && next4 != ledger.any.receivable_end ());
	auto next5 = ledger.any.receivable_upper_bound (tx, next3->first.account, next3->first.hash);
	auto next6 = ledger.any.receivable_upper_bound (tx, next4->first.account, next4->first.hash);
	ASSERT_TRUE (next5 == ledger.any.receivable_end () && next6 == ledger.any.receivable_end ());
	ASSERT_EQ (ledger.any.receivable_end (), ++next3);
	ASSERT_EQ (ledger.any.receivable_end (), ++next4);
}

// Test that multiple receivable entries for the same account
TEST (ledger_receivable, key_two)
{
	auto ctx = lumex::test::ledger_empty ();
	lumex::block_builder builder;
	lumex::keypair key;
	auto send1 = builder
				 .state ()
				 .account (lumex::dev::genesis_key.pub)
				 .previous (lumex::dev::genesis->hash ())
				 .representative (lumex::dev::genesis_key.pub)
				 .balance (lumex::dev::constants.genesis_amount - lumex::Klumex_ratio)
				 .link (key.pub)
				 .sign (lumex::dev::genesis_key.prv, lumex::dev::genesis_key.pub)
				 .work (*ctx.pool ().generate (lumex::dev::genesis->hash ()))
				 .build ();
	ASSERT_EQ (lumex::block_status::progress, ctx.ledger ().process (ctx.ledger ().tx_begin_write (), send1));
	auto send2 = builder
				 .state ()
				 .account (lumex::dev::genesis_key.pub)
				 .previous (send1->hash ())
				 .representative (lumex::dev::genesis_key.pub)
				 .balance (lumex::dev::constants.genesis_amount - 2 * lumex::Klumex_ratio)
				 .link (key.pub)
				 .sign (lumex::dev::genesis_key.prv, lumex::dev::genesis_key.pub)
				 .work (*ctx.pool ().generate (send1->hash ()))
				 .build ();
	ASSERT_EQ (lumex::block_status::progress, ctx.ledger ().process (ctx.ledger ().tx_begin_write (), send2));
	auto tx = ctx.ledger ().tx_begin_read ();
	auto & ledger = ctx.ledger ();
	auto next1 = ledger.any.receivable_upper_bound (tx, key.pub, 0);
	ASSERT_TRUE (next1 != ledger.any.receivable_end () && next1->first.account == key.pub);
	auto next2 = ledger.any.receivable_upper_bound (tx, key.pub, next1->first.hash);
	ASSERT_TRUE (next2 != ledger.any.receivable_end () && next2->first.account == key.pub);
	ASSERT_NE (next1->first.hash, next2->first.hash);
	ASSERT_EQ (next2, ++next1);
	ASSERT_EQ (ledger.any.receivable_end (), ++next1);
	ASSERT_EQ (ledger.any.receivable_end (), ++next2);
}

TEST (ledger_receivable, any_none)
{
	auto ctx = lumex::test::ledger_empty ();
	ASSERT_FALSE (ctx.ledger ().any.receivable_exists (ctx.ledger ().tx_begin_read (), lumex::dev::genesis_key.pub));
}

TEST (ledger_receivable, any_one)
{
	auto ctx = lumex::test::ledger_empty ();
	lumex::block_builder builder;
	lumex::keypair key;
	auto send1 = builder
				 .state ()
				 .account (lumex::dev::genesis_key.pub)
				 .previous (lumex::dev::genesis->hash ())
				 .representative (lumex::dev::genesis_key.pub)
				 .balance (lumex::dev::constants.genesis_amount - lumex::Klumex_ratio)
				 .link (lumex::dev::genesis_key.pub)
				 .sign (lumex::dev::genesis_key.prv, lumex::dev::genesis_key.pub)
				 .work (*ctx.pool ().generate (lumex::dev::genesis->hash ()))
				 .build ();
	ASSERT_EQ (lumex::block_status::progress, ctx.ledger ().process (ctx.ledger ().tx_begin_write (), send1));
	ASSERT_TRUE (ctx.ledger ().any.receivable_exists (ctx.ledger ().tx_begin_read (), lumex::dev::genesis_key.pub));
	ASSERT_FALSE (ctx.ledger ().any.receivable_exists (ctx.ledger ().tx_begin_read (), key.pub));
}

TEST (ledger_transaction, write_refresh)
{
	auto ctx = lumex::test::ledger_empty ();
	lumex::block_builder builder;
	lumex::keypair key;
	auto send1 = builder
				 .state ()
				 .account (lumex::dev::genesis_key.pub)
				 .previous (lumex::dev::genesis->hash ())
				 .representative (lumex::dev::genesis_key.pub)
				 .balance (lumex::dev::constants.genesis_amount - lumex::Klumex_ratio)
				 .link (lumex::dev::genesis_key.pub)
				 .sign (lumex::dev::genesis_key.prv, lumex::dev::genesis_key.pub)
				 .work (*ctx.pool ().generate (lumex::dev::genesis->hash ()))
				 .build ();
	auto send2 = builder
				 .state ()
				 .account (lumex::dev::genesis_key.pub)
				 .previous (send1->hash ())
				 .representative (lumex::dev::genesis_key.pub)
				 .balance (lumex::dev::constants.genesis_amount - 2 * lumex::Klumex_ratio)
				 .link (key.pub)
				 .sign (lumex::dev::genesis_key.prv, lumex::dev::genesis_key.pub)
				 .work (*ctx.pool ().generate (send1->hash ()))
				 .build ();

	auto transaction = ctx.ledger ().tx_begin_write ();
	ASSERT_EQ (lumex::block_status::progress, ctx.ledger ().process (transaction, send1));
	// Force refresh
	ASSERT_TRUE (transaction.refresh_if_needed (0ms));
	ASSERT_FALSE (transaction.refresh_if_needed ()); // Should not refresh again too soon
	// Refreshed transaction should work just fine
	ASSERT_EQ (lumex::block_status::progress, ctx.ledger ().process (transaction, send2));
}

TEST (ledger_transaction, write_wait_order)
{
	lumex::test::system system;

	auto ctx = lumex::test::ledger_empty ();

	std::atomic<bool> acquired1{ false };
	std::atomic<bool> acquired2{ false };
	std::atomic<bool> acquired3{ false };

	std::latch latch1{ 1 };
	std::latch latch2{ 1 };
	std::latch latch3{ 1 };

	auto fut1 = std::async (std::launch::async, [&] {
		auto tx = ctx.ledger ().tx_begin_write (lumex::store::writer::generic);
		acquired1 = true;
		latch1.wait (); // Wait for the signal to drop tx
	});
	WAIT (250ms); // Allow thread to start

	auto fut2 = std::async (std::launch::async, [&ctx, &acquired2, &latch2] {
		auto tx = ctx.ledger ().tx_begin_write (lumex::store::writer::block_processor);
		acquired2 = true;
		latch2.wait (); // Wait for the signal to drop tx
	});
	WAIT (250ms); // Allow thread to start

	auto fut3 = std::async (std::launch::async, [&ctx, &acquired3, &latch3] {
		auto tx = ctx.ledger ().tx_begin_write (lumex::store::writer::confirmation_height);
		acquired3 = true;
		latch3.wait (); // Wait for the signal to drop tx
	});
	WAIT (250ms); // Allow thread to start

	// First transaction should be ready immediately, others should be waiting
	ASSERT_TIMELY (5s, acquired1.load ());
	ASSERT_NEVER (250ms, acquired2.load ());
	ASSERT_NEVER (250ms, acquired3.load ());

	// Signal to continue and drop the first transaction
	latch1.count_down ();
	ASSERT_TIMELY (5s, acquired2.load ());
	ASSERT_NEVER (250ms, acquired3.load ());

	// Signal to continue and drop the second transaction
	latch2.count_down ();
	ASSERT_TIMELY (5s, acquired3.load ());

	// Signal to continue and drop the third transaction
	latch3.count_down ();
}

TEST (ledger_transaction, multithreaded_interleaving)
{
	lumex::test::system system;

	auto ctx = lumex::test::ledger_empty ();

	int constexpr num_threads = 2;
	int constexpr num_iterations = 10;
	int constexpr num_blocks = 10;

	std::deque<std::thread> threads;
	for (int i = 0; i < num_threads; ++i)
	{
		threads.emplace_back ([&] {
			for (int n = 0; n < num_iterations; ++n)
			{
				auto tx = ctx.ledger ().tx_begin_write (lumex::store::writer::testing);
				for (unsigned k = 0; k < num_blocks; ++k)
				{
					ctx.store ().account.put (tx, lumex::account{ k }, lumex::account_info{});
				}
				for (unsigned k = 0; k < num_blocks; ++k)
				{
					ctx.store ().account.del (tx, lumex::account{ k });
				}
			}
		});
	}

	for (auto & thread : threads)
	{
		thread.join ();
	}
}

TEST (bootstrap_weights, bootstrap_weights)
{
	// Live network should return non-empty weights with sane values
	{
		auto weights = lumex::get_bootstrap_weights (lumex::network_type::lumex_live_network);
		ASSERT_GT (weights.max_blocks, 100'000'000); // Should be at least 100M blocks
		ASSERT_GT (weights.representatives.size (), 100); // Should have many representatives
		ASSERT_LT (weights.representatives.size (), 10000);
		lumex::uint128_t total_weight{ 0 };
		for (auto const & [account, weight] : weights.representatives)
		{
			ASSERT_GT (weight, 0);
			total_weight += weight;
		}
		ASSERT_GT (total_weight, lumex::lumex_ratio * 100'000'000); // Should represent significant voting weight (>100M lumex)
		ASSERT_LT (total_weight, lumex::lumex_ratio * 340'000'000); // Should not exceed max supply (~340M lumex)
	}
	// Beta network should return non-empty weights
	{
		auto weights = lumex::get_bootstrap_weights (lumex::network_type::lumex_beta_network);
		ASSERT_GT (weights.max_blocks, 0);
		ASSERT_FALSE (weights.representatives.empty ());
		for (auto const & [account, weight] : weights.representatives)
		{
			ASSERT_GT (weight, 0);
		}
	}
	// Dev/test/invalid networks should return empty weights
	{
		auto weights = lumex::get_bootstrap_weights (lumex::network_type::lumex_dev_network);
		ASSERT_EQ (weights.max_blocks, 0);
		ASSERT_TRUE (weights.representatives.empty ());
	}
	{
		auto weights = lumex::get_bootstrap_weights (lumex::network_type::lumex_test_network);
		ASSERT_EQ (weights.max_blocks, 0);
		ASSERT_TRUE (weights.representatives.empty ());
	}
	{
		auto weights = lumex::get_bootstrap_weights (lumex::network_type::invalid);
		ASSERT_EQ (weights.max_blocks, 0);
		ASSERT_TRUE (weights.representatives.empty ());
	}
}

TEST (ledger, block_find_by_hash)
{
	auto ctx = lumex::test::ledger_empty ();
	auto & ledger = ctx.ledger ();
	auto & pool = ctx.pool ();
	lumex::block_builder builder;
	lumex::keypair key1;

	// send1 to key1, open1 opens key1's account
	auto send1 = builder.state ()
				 .account (lumex::dev::genesis_key.pub)
				 .previous (lumex::dev::genesis->hash ())
				 .representative (lumex::dev::genesis_key.pub)
				 .balance (lumex::dev::constants.genesis_amount - 100)
				 .link (key1.pub)
				 .sign (lumex::dev::genesis_key.prv, lumex::dev::genesis_key.pub)
				 .work (*pool.generate (lumex::dev::genesis->hash ()))
				 .build ();
	auto open1 = builder.state ()
				 .account (key1.pub)
				 .previous (0)
				 .representative (key1.pub)
				 .balance (100)
				 .link (send1->hash ())
				 .sign (key1.prv, key1.pub)
				 .work (*pool.generate (key1.pub))
				 .build ();

	auto txn = ledger.tx_begin_write ();
	ASSERT_EQ (lumex::block_status::progress, ledger.process (txn, send1));
	ASSERT_EQ (lumex::block_status::progress, ledger.process (txn, open1));

	// Hash finds send1, root would find open1 via account lookup — hash must win
	auto result = ledger.block_find (txn, send1->hash (), lumex::root (key1.pub));
	ASSERT_NE (nullptr, result);
	ASSERT_EQ (*result, *send1);
}

TEST (ledger, block_find_by_successor)
{
	auto ctx = lumex::test::ledger_empty ();
	auto & ledger = ctx.ledger ();
	auto & pool = ctx.pool ();
	lumex::block_builder builder;

	auto send1 = builder.state ()
				 .account (lumex::dev::genesis_key.pub)
				 .previous (lumex::dev::genesis->hash ())
				 .representative (lumex::dev::genesis_key.pub)
				 .balance (lumex::dev::constants.genesis_amount - 1)
				 .link (lumex::dev::genesis_key.pub)
				 .sign (lumex::dev::genesis_key.prv, lumex::dev::genesis_key.pub)
				 .work (*pool.generate (lumex::dev::genesis->hash ()))
				 .build ();

	auto txn = ledger.tx_begin_write ();
	ASSERT_EQ (lumex::block_status::progress, ledger.process (txn, send1));

	// Wrong hash but root is the predecessor — finds successor (send1)
	auto result = ledger.block_find (txn, lumex::block_hash (1), lumex::root (lumex::dev::genesis->hash ()));
	ASSERT_NE (nullptr, result);
	ASSERT_EQ (*result, *send1);
}

TEST (ledger, block_find_by_account)
{
	auto ctx = lumex::test::ledger_empty ();
	auto & ledger = ctx.ledger ();
	auto & pool = ctx.pool ();
	lumex::block_builder builder;
	lumex::keypair key1;

	auto send1 = builder.state ()
				 .account (lumex::dev::genesis_key.pub)
				 .previous (lumex::dev::genesis->hash ())
				 .representative (lumex::dev::genesis_key.pub)
				 .balance (lumex::dev::constants.genesis_amount - 100)
				 .link (key1.pub)
				 .sign (lumex::dev::genesis_key.prv, lumex::dev::genesis_key.pub)
				 .work (*pool.generate (lumex::dev::genesis->hash ()))
				 .build ();

	auto open1 = builder.state ()
				 .account (key1.pub)
				 .previous (0)
				 .representative (key1.pub)
				 .balance (100)
				 .link (send1->hash ())
				 .sign (key1.prv, key1.pub)
				 .work (*pool.generate (key1.pub))
				 .build ();

	auto txn = ledger.tx_begin_write ();
	ASSERT_EQ (lumex::block_status::progress, ledger.process (txn, send1));
	ASSERT_EQ (lumex::block_status::progress, ledger.process (txn, open1));

	// Wrong hash but root matches account — finds the open block
	auto result = ledger.block_find (txn, lumex::block_hash (1), lumex::root (key1.pub));
	ASSERT_NE (nullptr, result);
	ASSERT_EQ (*result, *open1);
}

TEST (ledger, block_find_not_found)
{
	auto ctx = lumex::test::ledger_empty ();
	auto & ledger = ctx.ledger ();
	auto & pool = ctx.pool ();
	lumex::block_builder builder;
	lumex::keypair key1;

	// Add blocks to the ledger that could confuse the search
	auto send1 = builder.state ()
				 .account (lumex::dev::genesis_key.pub)
				 .previous (lumex::dev::genesis->hash ())
				 .representative (lumex::dev::genesis_key.pub)
				 .balance (lumex::dev::constants.genesis_amount - 100)
				 .link (key1.pub)
				 .sign (lumex::dev::genesis_key.prv, lumex::dev::genesis_key.pub)
				 .work (*pool.generate (lumex::dev::genesis->hash ()))
				 .build ();
	auto open1 = builder.state ()
				 .account (key1.pub)
				 .previous (0)
				 .representative (key1.pub)
				 .balance (100)
				 .link (send1->hash ())
				 .sign (key1.prv, key1.pub)
				 .work (*pool.generate (key1.pub))
				 .build ();

	auto txn = ledger.tx_begin_write ();
	ASSERT_EQ (lumex::block_status::progress, ledger.process (txn, send1));
	ASSERT_EQ (lumex::block_status::progress, ledger.process (txn, open1));

	// Unknown hash and zero root
	ASSERT_EQ (nullptr, ledger.block_find (txn, lumex::block_hash (1), lumex::root (0)));

	// Unknown hash and root that doesn't match any account or predecessor
	lumex::keypair key2;
	ASSERT_EQ (nullptr, ledger.block_find (txn, lumex::block_hash (1), lumex::root (key2.pub)));

	// Existing root but wrong hash — root has no successor (open1 is the head)
	ASSERT_EQ (nullptr, ledger.block_find (txn, lumex::block_hash (1), lumex::root (open1->hash ())));
}

// Epoch open blocks can be created without the account's private key.
// block_find should return the epoch open when searching by root-as-account.
TEST (ledger, block_find_epoch_open)
{
	auto ctx = lumex::test::ledger_empty ();
	auto & ledger = ctx.ledger ();
	auto & pool = ctx.pool ();
	lumex::block_builder builder;
	lumex::keypair key1;

	// Send to key1 so there's a pending entry (required for epoch open)
	auto send1 = builder.state ()
				 .account (lumex::dev::genesis_key.pub)
				 .previous (lumex::dev::genesis->hash ())
				 .representative (lumex::dev::genesis_key.pub)
				 .balance (lumex::dev::constants.genesis_amount - 100)
				 .link (key1.pub)
				 .sign (lumex::dev::genesis_key.prv, lumex::dev::genesis_key.pub)
				 .work (*pool.generate (lumex::dev::genesis->hash ()))
				 .build ();

	// Epoch open for key1 — signed by epoch signer, not by key1
	auto epoch_open = builder.state ()
					  .account (key1.pub)
					  .previous (0)
					  .representative (0)
					  .balance (0)
					  .link (ledger.epoch_link (lumex::epoch::epoch_1))
					  .sign (lumex::dev::genesis_key.prv, lumex::dev::genesis_key.pub)
					  .work (*pool.generate (key1.pub))
					  .build ();

	auto txn = ledger.tx_begin_write ();
	ASSERT_EQ (lumex::block_status::progress, ledger.process (txn, send1));
	ASSERT_EQ (lumex::block_status::progress, ledger.process (txn, epoch_open));

	// Direct hash lookup finds the epoch open
	auto result1 = ledger.block_find (txn, epoch_open->hash (), lumex::root (0));
	ASSERT_NE (nullptr, result1);
	ASSERT_EQ (*result1, *epoch_open);

	// Root-as-account lookup finds the epoch open (unknown hash)
	auto result2 = ledger.block_find (txn, lumex::block_hash (1), lumex::root (key1.pub));
	ASSERT_NE (nullptr, result2);
	ASSERT_EQ (*result2, *epoch_open);
}

// When both a regular open and a subsequent block exist for an account that was epoch-opened,
// block_find should return the correct block based on the search criteria.
TEST (ledger, block_find_epoch_open_then_receive)
{
	auto ctx = lumex::test::ledger_empty ();
	auto & ledger = ctx.ledger ();
	auto & pool = ctx.pool ();
	lumex::block_builder builder;
	lumex::keypair key1;

	auto send1 = builder.state ()
				 .account (lumex::dev::genesis_key.pub)
				 .previous (lumex::dev::genesis->hash ())
				 .representative (lumex::dev::genesis_key.pub)
				 .balance (lumex::dev::constants.genesis_amount - 100)
				 .link (key1.pub)
				 .sign (lumex::dev::genesis_key.prv, lumex::dev::genesis_key.pub)
				 .work (*pool.generate (lumex::dev::genesis->hash ()))
				 .build ();

	// Epoch open for key1
	auto epoch_open = builder.state ()
					  .account (key1.pub)
					  .previous (0)
					  .representative (0)
					  .balance (0)
					  .link (ledger.epoch_link (lumex::epoch::epoch_1))
					  .sign (lumex::dev::genesis_key.prv, lumex::dev::genesis_key.pub)
					  .work (*pool.generate (key1.pub))
					  .build ();

	// Receive on key1 after epoch open
	auto receive1 = builder.state ()
					.account (key1.pub)
					.previous (epoch_open->hash ())
					.representative (key1.pub)
					.balance (100)
					.link (send1->hash ())
					.sign (key1.prv, key1.pub)
					.work (*pool.generate (epoch_open->hash ()))
					.build ();

	auto txn = ledger.tx_begin_write ();
	ASSERT_EQ (lumex::block_status::progress, ledger.process (txn, send1));
	ASSERT_EQ (lumex::block_status::progress, ledger.process (txn, epoch_open));
	ASSERT_EQ (lumex::block_status::progress, ledger.process (txn, receive1));

	// Direct hash lookup for epoch_open
	auto result1 = ledger.block_find (txn, epoch_open->hash (), lumex::root (0));
	ASSERT_NE (nullptr, result1);
	ASSERT_EQ (*result1, *epoch_open);

	// Direct hash lookup for receive1
	auto result2 = ledger.block_find (txn, receive1->hash (), lumex::root (0));
	ASSERT_NE (nullptr, result2);
	ASSERT_EQ (*result2, *receive1);

	// Root-as-account lookup returns the open block (epoch_open)
	auto result3 = ledger.block_find (txn, lumex::block_hash (1), lumex::root (key1.pub));
	ASSERT_NE (nullptr, result3);
	ASSERT_EQ (*result3, *epoch_open);

	// Root as predecessor (epoch_open hash) returns successor (receive1)
	auto result4 = ledger.block_find (txn, lumex::block_hash (1), lumex::root (epoch_open->hash ()));
	ASSERT_NE (nullptr, result4);
	ASSERT_EQ (*result4, *receive1);
}

// A block hash can collide with an account public key.
// When root matches both a block hash (successor path) and an account (open block path), the successor lookup takes priority.
TEST (ledger, block_find_root_hash_account_collision)
{
	auto ctx = lumex::test::ledger_empty ();
	auto & ledger = ctx.ledger ();
	auto & pool = ctx.pool ();
	lumex::block_builder builder;

	// Create send1 from genesis — its hash will also be used as an "account" public key
	auto send1 = builder.state ()
				 .account (lumex::dev::genesis_key.pub)
				 .previous (lumex::dev::genesis->hash ())
				 .representative (lumex::dev::genesis_key.pub)
				 .balance (lumex::dev::constants.genesis_amount - 100)
				 .link (lumex::dev::genesis_key.pub)
				 .sign (lumex::dev::genesis_key.prv, lumex::dev::genesis_key.pub)
				 .work (*pool.generate (lumex::dev::genesis->hash ()))
				 .build ();

	// send1->hash() is now a valid block hash in the ledger
	// Use it as an account public key: send funds to "account" send1->hash()
	auto send2 = builder.state ()
				 .account (lumex::dev::genesis_key.pub)
				 .previous (send1->hash ())
				 .representative (lumex::dev::genesis_key.pub)
				 .balance (lumex::dev::constants.genesis_amount - 200)
				 .link (send1->hash ()) // send to "account" whose pub key == send1's hash
				 .sign (lumex::dev::genesis_key.prv, lumex::dev::genesis_key.pub)
				 .work (*pool.generate (send1->hash ()))
				 .build ();

	// Epoch open the account whose public key == send1->hash()
	// This doesn't require the private key
	lumex::account colliding_account{ send1->hash () };
	auto epoch_open = builder.state ()
					  .account (colliding_account)
					  .previous (0)
					  .representative (0)
					  .balance (0)
					  .link (ledger.epoch_link (lumex::epoch::epoch_1))
					  .sign (lumex::dev::genesis_key.prv, lumex::dev::genesis_key.pub)
					  .work (*pool.generate (send1->hash ()))
					  .build ();

	auto txn = ledger.tx_begin_write ();
	ASSERT_EQ (lumex::block_status::progress, ledger.process (txn, send1));
	ASSERT_EQ (lumex::block_status::progress, ledger.process (txn, send2));
	ASSERT_EQ (lumex::block_status::progress, ledger.process (txn, epoch_open));

	// Now send1->hash() is both:
	// - A block hash (send1) whose successor is send2
	// - An account public key whose open block is epoch_open

	// Root = send1->hash(), unknown hash:
	// Successor path finds send2 (successor of send1)
	auto result = ledger.block_find (txn, lumex::block_hash (1), lumex::root (send1->hash ()));
	ASSERT_NE (nullptr, result);
	ASSERT_EQ (*result, *send2); // successor wins over account lookup

	// But providing the correct hash for epoch_open finds it directly
	auto result2 = ledger.block_find (txn, epoch_open->hash (), lumex::root (send1->hash ()));
	ASSERT_NE (nullptr, result2);
	ASSERT_EQ (*result2, *epoch_open);
}

/*
 * A freshly-initialized ledger must have `topo_index` enabled, and the flag
 * must survive closing and reopening the store
 */
TEST (ledger, topo_index_flag_persists)
{
	lumex::logger logger;
	lumex::stats stats{ logger };
	auto const path = lumex::unique_path ();

	// Ledger initialization should create the store with topo_index enabled
	{
		auto store = lumex::test::make_store (logger, stats, path);
		lumex::ledger ledger{ *store, lumex::dev::network_params, stats, logger };
		ASSERT_TRUE (ledger.flags.topo_index);
	}
	// Reopen store and ledger, confirm flag is still set
	{
		auto store = lumex::test::make_store (logger, stats, path);
		lumex::ledger ledger{ *store, lumex::dev::network_params, stats, logger };
		ASSERT_TRUE (ledger.flags.topo_index);
	}
}

/*
 * Genesis block must carry topo_height = 1 in its sideband and have a topology entry.
 * Every subsequently-processed block must satisfy topo_height = 1 + max(deps' topo_height)
 * and have a matching entry in the topology table.
 */
TEST (ledger, topo_height_legacy_chain)
{
	auto ctx = lumex::test::ledger_empty ();
	auto & ledger = ctx.ledger ();
	auto & store = ctx.store ();
	ASSERT_TRUE (ledger.flags.topo_index);

	// Genesis, confirm both the sideband value and the matching topology row
	{
		auto txn = ledger.tx_begin_read ();
		auto genesis = ledger.any.block_get (txn, lumex::dev::genesis->hash ());
		ASSERT_TRUE (genesis);
		ASSERT_EQ (1, genesis->sideband ().topo_height);
		ASSERT_TRUE (store.topology.exists (txn, { 1, lumex::dev::genesis->hash () }));
	}

	auto txn = ledger.tx_begin_write ();
	auto & pool = ctx.pool ();
	auto info1 = ledger.any.account_get (txn, lumex::dev::genesis_key.pub);
	ASSERT_TRUE (info1);
	lumex::keypair key2;
	lumex::block_builder builder;

	// Genesis → send. dep is genesis (topo=1) → send.topo = 2
	auto send = builder.send ()
				.previous (info1->head)
				.destination (key2.pub)
				.balance (lumex::dev::constants.genesis_amount - lumex::Klumex_ratio)
				.sign (lumex::dev::genesis_key.prv, lumex::dev::genesis_key.pub)
				.work (*pool.generate (info1->head))
				.build ();
	ASSERT_EQ (lumex::block_status::progress, ledger.process (txn, send));
	ASSERT_EQ (2, send->sideband ().topo_height);
	ASSERT_TRUE (store.topology.exists (txn, { 2, send->hash () }));

	// Open is a pure receive: dep is the send (topo=2) → open.topo = 3
	auto open = builder.open ()
				.source (send->hash ())
				.representative (key2.pub)
				.account (key2.pub)
				.sign (key2.prv, key2.pub)
				.work (*pool.generate (key2.pub))
				.build ();
	ASSERT_EQ (lumex::block_status::progress, ledger.process (txn, open));
	ASSERT_EQ (3, open->sideband ().topo_height);
	ASSERT_TRUE (store.topology.exists (txn, { 3, open->hash () }));

	// A second genesis send → dep is the first send (topo=2) → send2.topo = 3
	auto send2 = builder.send ()
				 .previous (send->hash ())
				 .destination (key2.pub)
				 .balance (lumex::dev::constants.genesis_amount - 2 * lumex::Klumex_ratio)
				 .sign (lumex::dev::genesis_key.prv, lumex::dev::genesis_key.pub)
				 .work (*pool.generate (send->hash ()))
				 .build ();
	ASSERT_EQ (lumex::block_status::progress, ledger.process (txn, send2));
	ASSERT_EQ (3, send2->sideband ().topo_height);
	ASSERT_TRUE (store.topology.exists (txn, { 3, send2->hash () }));

	// Receive on key2 has two deps: previous = open (topo=3), source = send2 (topo=3) -> receive.topo = max(3, 3) + 1 = 4
	auto receive = builder.receive ()
				   .previous (open->hash ())
				   .source (send2->hash ())
				   .sign (key2.prv, key2.pub)
				   .work (*pool.generate (open->hash ()))
				   .build ();
	ASSERT_EQ (lumex::block_status::progress, ledger.process (txn, receive));
	ASSERT_EQ (4, receive->sideband ().topo_height);
	ASSERT_TRUE (store.topology.exists (txn, { 4, receive->hash () }));

	// Change on key2's chain: only dep is previous = receive (topo=4) → topo = 5
	lumex::keypair key3;
	auto change = builder.change ()
				  .previous (receive->hash ())
				  .representative (key3.pub)
				  .sign (key2.prv, key2.pub)
				  .work (*pool.generate (receive->hash ()))
				  .build ();
	ASSERT_EQ (lumex::block_status::progress, ledger.process (txn, change));
	ASSERT_EQ (5, change->sideband ().topo_height);
	ASSERT_TRUE (store.topology.exists (txn, { 5, change->hash () }));
}

/*
 * State blocks must carry topo_height computed over their dependencies.
 * For send/change subtypes only `previous` is a dep;
 * For receive subtypes (including state open) both `previous` and `link` are deps.
 */
TEST (ledger, topo_height_state_blocks)
{
	auto ctx = lumex::test::ledger_empty ();
	auto & ledger = ctx.ledger ();
	auto & store = ctx.store ();
	ASSERT_TRUE (ledger.flags.topo_index);

	auto txn = ledger.tx_begin_write ();
	auto & pool = ctx.pool ();
	lumex::keypair key2;
	lumex::block_builder builder;

	// State send from genesis. dep is genesis (topo=1) → topo = 2
	auto send = builder.state ()
				.account (lumex::dev::genesis_key.pub)
				.previous (lumex::dev::genesis->hash ())
				.representative (lumex::dev::genesis_key.pub)
				.balance (lumex::dev::constants.genesis_amount - lumex::Klumex_ratio)
				.link (key2.pub)
				.sign (lumex::dev::genesis_key.prv, lumex::dev::genesis_key.pub)
				.work (*pool.generate (lumex::dev::genesis->hash ()))
				.build ();
	ASSERT_EQ (lumex::block_status::progress, ledger.process (txn, send));
	ASSERT_EQ (2, send->sideband ().topo_height);
	ASSERT_TRUE (store.topology.exists (txn, { 2, send->hash () }));

	// State open (receive subtype, no previous). dep is the send (topo=2) → topo = 3
	auto open = builder.state ()
				.account (key2.pub)
				.previous (0)
				.representative (key2.pub)
				.balance (lumex::Klumex_ratio)
				.link (send->hash ())
				.sign (key2.prv, key2.pub)
				.work (*pool.generate (key2.pub))
				.build ();
	ASSERT_EQ (lumex::block_status::progress, ledger.process (txn, open));
	ASSERT_EQ (3, open->sideband ().topo_height);
	ASSERT_TRUE (store.topology.exists (txn, { 3, open->hash () }));

	// Second state send from genesis. dep is the first send (topo=2) → topo = 3
	auto send2 = builder.state ()
				 .account (lumex::dev::genesis_key.pub)
				 .previous (send->hash ())
				 .representative (lumex::dev::genesis_key.pub)
				 .balance (lumex::dev::constants.genesis_amount - 2 * lumex::Klumex_ratio)
				 .link (key2.pub)
				 .sign (lumex::dev::genesis_key.prv, lumex::dev::genesis_key.pub)
				 .work (*pool.generate (send->hash ()))
				 .build ();
	ASSERT_EQ (lumex::block_status::progress, ledger.process (txn, send2));
	ASSERT_EQ (3, send2->sideband ().topo_height);
	ASSERT_TRUE (store.topology.exists (txn, { 3, send2->hash () }));

	// State receive on key2 (mid-chain)
	// two deps: previous = open (topo=3), link source = send2 (topo=3). Result is max(3, 3) + 1 = 4
	auto receive = builder.state ()
				   .account (key2.pub)
				   .previous (open->hash ())
				   .representative (key2.pub)
				   .balance (2 * lumex::Klumex_ratio)
				   .link (send2->hash ())
				   .sign (key2.prv, key2.pub)
				   .work (*pool.generate (open->hash ()))
				   .build ();
	ASSERT_EQ (lumex::block_status::progress, ledger.process (txn, receive));
	ASSERT_EQ (4, receive->sideband ().topo_height);
	ASSERT_TRUE (store.topology.exists (txn, { 4, receive->hash () }));

	// State change on key2 (no balance change, link == 0)
	// Only dep is previous = receive (topo=4) → topo = 5
	lumex::keypair key3;
	auto change = builder.state ()
				  .account (key2.pub)
				  .previous (receive->hash ())
				  .representative (key3.pub)
				  .balance (2 * lumex::Klumex_ratio)
				  .link (0)
				  .sign (key2.prv, key2.pub)
				  .work (*pool.generate (receive->hash ()))
				  .build ();
	ASSERT_EQ (lumex::block_status::progress, ledger.process (txn, change));
	ASSERT_EQ (5, change->sideband ().topo_height);
	ASSERT_TRUE (store.topology.exists (txn, { 5, change->hash () }));
}

/*
 * Rollback must remove the topology entry
 */
TEST (ledger, topo_height_rollback_removes_topology_entry)
{
	auto ctx = lumex::test::ledger_empty ();
	auto & ledger = ctx.ledger ();
	auto & store = ctx.store ();
	ASSERT_TRUE (ledger.flags.topo_index);

	auto txn = ledger.tx_begin_write ();
	auto & pool = ctx.pool ();
	auto info1 = ledger.any.account_get (txn, lumex::dev::genesis_key.pub);
	ASSERT_TRUE (info1);
	lumex::keypair key2;
	lumex::block_builder builder;

	auto send = builder.send ()
				.previous (info1->head)
				.destination (key2.pub)
				.balance (lumex::dev::constants.genesis_amount - lumex::Klumex_ratio)
				.sign (lumex::dev::genesis_key.prv, lumex::dev::genesis_key.pub)
				.work (*pool.generate (info1->head))
				.build ();
	ASSERT_EQ (lumex::block_status::progress, ledger.process (txn, send));
	auto const send_topo = send->sideband ().topo_height;
	ASSERT_EQ (2, send_topo);
	ASSERT_TRUE (store.topology.exists (txn, { send_topo, send->hash () }));

	ASSERT_FALSE (ledger.rollback (txn, send->hash ()));
	ASSERT_FALSE (store.topology.exists (txn, { send_topo, send->hash () }));

	// Genesis's entry must remain untouched
	ASSERT_TRUE (store.topology.exists (txn, { 1, lumex::dev::genesis->hash () }));
}

/*
 * Epoch upgrade on an existing account chain.
 * The block has `previous` pointer, so its topo_height must be `previous.topo_height + 1`
 */
TEST (ledger, topo_height_epoch_upgrade)
{
	auto ctx = lumex::test::ledger_empty ();
	auto & ledger = ctx.ledger ();
	auto & store = ctx.store ();
	ASSERT_TRUE (ledger.flags.topo_index);

	auto txn = ledger.tx_begin_write ();
	auto & pool = ctx.pool ();
	lumex::keypair key1;
	lumex::block_builder builder;

	// Genesis (topo=1) → state send to key1 (topo=2)
	auto send = builder.state ()
				.account (lumex::dev::genesis_key.pub)
				.previous (lumex::dev::genesis->hash ())
				.representative (lumex::dev::genesis_key.pub)
				.balance (lumex::dev::constants.genesis_amount - lumex::Klumex_ratio)
				.link (key1.pub)
				.sign (lumex::dev::genesis_key.prv, lumex::dev::genesis_key.pub)
				.work (*pool.generate (lumex::dev::genesis->hash ()))
				.build ();
	ASSERT_EQ (lumex::block_status::progress, ledger.process (txn, send));
	ASSERT_EQ (2, send->sideband ().topo_height);

	// State open for key1 (topo=3, source = send)
	auto open = builder.state ()
				.account (key1.pub)
				.previous (0)
				.representative (key1.pub)
				.balance (lumex::Klumex_ratio)
				.link (send->hash ())
				.sign (key1.prv, key1.pub)
				.work (*pool.generate (key1.pub))
				.build ();
	ASSERT_EQ (lumex::block_status::progress, ledger.process (txn, open));
	ASSERT_EQ (3, open->sideband ().topo_height);

	// Epoch upgrade on key1: previous = open. topo must be open.topo + 1 = 4
	auto epoch1 = builder.state ()
				  .account (key1.pub)
				  .previous (open->hash ())
				  .representative (key1.pub)
				  .balance (lumex::Klumex_ratio)
				  .link (ledger.epoch_link (lumex::epoch::epoch_1))
				  .sign (lumex::dev::genesis_key.prv, lumex::dev::genesis_key.pub)
				  .work (*pool.generate (open->hash ()))
				  .build ();
	ASSERT_EQ (lumex::block_status::progress, ledger.process (txn, epoch1));
	ASSERT_EQ (lumex::epoch::epoch_1, epoch1->sideband ().details.epoch);
	ASSERT_EQ (4, epoch1->sideband ().topo_height);
	ASSERT_TRUE (store.topology.exists (txn, { 4, epoch1->hash () }));

	// Sequential epoch upgrade on the same chain: previous = epoch1. topo must be epoch1.topo + 1 = 5
	auto epoch2 = builder.state ()
				  .account (key1.pub)
				  .previous (epoch1->hash ())
				  .representative (key1.pub)
				  .balance (lumex::Klumex_ratio)
				  .link (ledger.epoch_link (lumex::epoch::epoch_2))
				  .sign (lumex::dev::genesis_key.prv, lumex::dev::genesis_key.pub)
				  .work (*pool.generate (epoch1->hash ()))
				  .build ();
	ASSERT_EQ (lumex::block_status::progress, ledger.process (txn, epoch2));
	ASSERT_EQ (lumex::epoch::epoch_2, epoch2->sideband ().details.epoch);
	ASSERT_EQ (5, epoch2->sideband ().topo_height);
	ASSERT_TRUE (store.topology.exists (txn, { 5, epoch2->hash () }));
}

/*
 * Epoch open on an unopened account is the one block type with no usable
 * block-graph dependency: `previous == 0` and `link` carries the epoch payload
 * (not a block hash). The protocol still anchors it to the dependency graph
 * implicitly (it requires pending entries), so this case is treated as a known
 * rootless chain start and indexed at topo_height = 1.
 */
TEST (ledger, topo_height_epoch_open)
{
	auto ctx = lumex::test::ledger_empty ();
	auto & ledger = ctx.ledger ();
	auto & store = ctx.store ();
	ASSERT_TRUE (ledger.flags.topo_index);

	auto txn = ledger.tx_begin_write ();
	auto & pool = ctx.pool ();
	lumex::keypair key1;
	lumex::block_builder builder;

	// Genesis → send to key1 (creates the pending entry)
	auto send = builder.state ()
				.account (lumex::dev::genesis_key.pub)
				.previous (lumex::dev::genesis->hash ())
				.representative (lumex::dev::genesis_key.pub)
				.balance (lumex::dev::constants.genesis_amount - lumex::Klumex_ratio)
				.link (key1.pub)
				.sign (lumex::dev::genesis_key.prv, lumex::dev::genesis_key.pub)
				.work (*pool.generate (lumex::dev::genesis->hash ()))
				.build ();
	ASSERT_EQ (lumex::block_status::progress, ledger.process (txn, send));
	ASSERT_EQ (2, send->sideband ().topo_height);

	// Epoch open on the still-unopened key1 account — rootless in the graph
	auto epoch_open = builder.state ()
					  .account (key1.pub)
					  .previous (0)
					  .representative (0)
					  .balance (0)
					  .link (ledger.epoch_link (lumex::epoch::epoch_1))
					  .sign (lumex::dev::genesis_key.prv, lumex::dev::genesis_key.pub)
					  .work (*pool.generate (key1.pub))
					  .build ();
	ASSERT_EQ (lumex::block_status::progress, ledger.process (txn, epoch_open));

	// Anchored at 1 so the rest of the chain remains indexable
	ASSERT_EQ (1, epoch_open->sideband ().topo_height);
	ASSERT_TRUE (store.topology.exists (txn, { 1, epoch_open->hash () }));

	// With epoch_open=1 and send=2, the receive's max(prev=1, source=2) + 1 = 3
	auto receive = builder.state ()
				   .account (key1.pub)
				   .previous (epoch_open->hash ())
				   .representative (key1.pub)
				   .balance (lumex::Klumex_ratio)
				   .link (send->hash ())
				   .sign (key1.prv, key1.pub)
				   .work (*pool.generate (epoch_open->hash ()))
				   .build ();
	ASSERT_EQ (lumex::block_status::progress, ledger.process (txn, receive));
	ASSERT_EQ (3, receive->sideband ().topo_height);
	ASSERT_TRUE (store.topology.exists (txn, { 3, receive->hash () }));
}

/*
 * When a dependency has topo_height = 0 (the "unindexed" sentinel — produced
 * e.g. by processing under a disabled flag, by partial migration, or by a
 * pruned dep), the new block must also be assigned topo_height = 0.
 *
 * Setup: process a block with the flag disabled (so it lands with topo=0),
 * then re-enable the flag and process a successor. The successor's `previous`
 * has topo=0 → poison must propagate
 */
TEST (ledger, topo_height_unindexed_dependency_propagates_zero)
{
	lumex::logger logger;
	lumex::stats stats{ logger };
	auto store = lumex::test::make_store (logger, stats);
	lumex::work_pool pool{ lumex::dev::network_params.network, 1 };
	lumex::block_builder builder;
	lumex::keypair key2;

	// Initialize the ledger with topo index disabled
	{
		lumex::ledger ledger{ *store, lumex::dev::network_params, stats, logger };
	}
	{
		auto txn = store->tx_begin_write ();
		store->version.put_flag (txn, lumex::store::meta_key::topo_index_enabled, false);
	}

	std::shared_ptr<lumex::block> send;
	{
		lumex::ledger ledger{ *store, lumex::dev::network_params, stats, logger };
		ASSERT_FALSE (ledger.flags.topo_index);
		auto txn = ledger.tx_begin_write ();
		auto info1 = ledger.any.account_get (txn, lumex::dev::genesis_key.pub);
		ASSERT_TRUE (info1);
		send = builder.send ()
			   .previous (info1->head)
			   .destination (key2.pub)
			   .balance (lumex::dev::constants.genesis_amount - lumex::Klumex_ratio)
			   .sign (lumex::dev::genesis_key.prv, lumex::dev::genesis_key.pub)
			   .work (*pool.generate (info1->head))
			   .build ();
		ASSERT_EQ (lumex::block_status::progress, ledger.process (txn, send));

		// Sanity: while disabled, the send lands with the unindexed sentinel
		ASSERT_EQ (0, send->sideband ().topo_height);
	}

	{
		auto txn = store->tx_begin_write ();
		store->version.put_flag (txn, lumex::store::meta_key::topo_index_enabled, true);
	}
	{
		lumex::ledger ledger{ *store, lumex::dev::network_params, stats, logger };
		ASSERT_TRUE (ledger.flags.topo_index);
		auto txn = ledger.tx_begin_write ();
		auto open = builder.open ()
					.source (send->hash ())
					.representative (key2.pub)
					.account (key2.pub)
					.sign (key2.prv, key2.pub)
					.work (*pool.generate (key2.pub))
					.build ();
		ASSERT_EQ (lumex::block_status::progress, ledger.process (txn, open));

		// Source has topo=0 → propagates regardless of flag state
		ASSERT_EQ (0, open->sideband ().topo_height);
		ASSERT_FALSE (store->topology.exists (txn, { 0, open->hash () }));
	}
}

/*
 * When the topology index is disabled (e.g. an older ledger that hasn't been migrated yet),
 * the processor must not compute topo_heights and must not write to the topology table
 */
TEST (ledger, topo_height_disabled_flag_skips_indexing)
{
	lumex::logger logger;
	lumex::stats stats{ logger };
	auto store = lumex::test::make_store (logger, stats);

	// Initialize the ledger with topo index disabled
	{
		lumex::ledger ledger{ *store, lumex::dev::network_params, stats, logger };
	}
	{
		auto txn = store->tx_begin_write ();
		store->version.put_flag (txn, lumex::store::meta_key::topo_index_enabled, false);
	}

	lumex::ledger ledger{ *store, lumex::dev::network_params, stats, logger };
	ASSERT_FALSE (ledger.flags.topo_index);

	auto txn = ledger.tx_begin_write ();
	lumex::work_pool pool{ lumex::dev::network_params.network, 1 };
	auto info1 = ledger.any.account_get (txn, lumex::dev::genesis_key.pub);
	ASSERT_TRUE (info1);
	lumex::keypair key2;
	lumex::block_builder builder;

	auto send = builder.send ()
				.previous (info1->head)
				.destination (key2.pub)
				.balance (lumex::dev::constants.genesis_amount - lumex::Klumex_ratio)
				.sign (lumex::dev::genesis_key.prv, lumex::dev::genesis_key.pub)
				.work (*pool.generate (info1->head))
				.build ();
	ASSERT_EQ (lumex::block_status::progress, ledger.process (txn, send));

	// Zero topo_height; no topology row written
	ASSERT_EQ (0, send->sideband ().topo_height);
	ASSERT_FALSE (store->topology.exists (txn, { 0, send->hash () }));
}

/*
 * Migrating a legacy chain (send/open/send/receive/change) must produce the same topo_heights
 * a freshly-indexed ledger would have, and write matching topology entries
 * The flag and topology entries must also survive closing and reopening the store
 */
TEST (ledger, populate_topo_index_legacy_chain)
{
	lumex::logger logger;
	lumex::stats stats{ logger };
	auto const path = lumex::unique_path ();
	lumex::work_pool pool{ lumex::dev::network_params.network, 1 };
	lumex::block_builder builder;
	lumex::keypair key2;
	lumex::keypair key3;

	std::shared_ptr<lumex::block> send;
	std::shared_ptr<lumex::block> open;
	std::shared_ptr<lumex::block> send2;
	std::shared_ptr<lumex::block> receive;
	std::shared_ptr<lumex::block> change;

	auto store = lumex::test::make_store (logger, stats, path);
	{
		lumex::ledger ledger{ *store, lumex::dev::network_params, stats, logger };
	}
	{
		auto txn = store->tx_begin_write ();
		store->version.put_flag (txn, lumex::store::meta_key::topo_index_enabled, false);
	}

	// Build the chain with the flag disabled so every block lands with topo_height = 0
	{
		lumex::ledger ledger{ *store, lumex::dev::network_params, stats, logger };
		ASSERT_FALSE (ledger.flags.topo_index);

		auto txn = ledger.tx_begin_write ();
		auto info1 = ledger.any.account_get (txn, lumex::dev::genesis_key.pub);
		ASSERT_TRUE (info1);

		send = builder.send ()
			   .previous (info1->head)
			   .destination (key2.pub)
			   .balance (lumex::dev::constants.genesis_amount - lumex::Klumex_ratio)
			   .sign (lumex::dev::genesis_key.prv, lumex::dev::genesis_key.pub)
			   .work (*pool.generate (info1->head))
			   .build ();
		ASSERT_EQ (lumex::block_status::progress, ledger.process (txn, send));

		open = builder.open ()
			   .source (send->hash ())
			   .representative (key2.pub)
			   .account (key2.pub)
			   .sign (key2.prv, key2.pub)
			   .work (*pool.generate (key2.pub))
			   .build ();
		ASSERT_EQ (lumex::block_status::progress, ledger.process (txn, open));

		send2 = builder.send ()
				.previous (send->hash ())
				.destination (key2.pub)
				.balance (lumex::dev::constants.genesis_amount - 2 * lumex::Klumex_ratio)
				.sign (lumex::dev::genesis_key.prv, lumex::dev::genesis_key.pub)
				.work (*pool.generate (send->hash ()))
				.build ();
		ASSERT_EQ (lumex::block_status::progress, ledger.process (txn, send2));

		receive = builder.receive ()
				  .previous (open->hash ())
				  .source (send2->hash ())
				  .sign (key2.prv, key2.pub)
				  .work (*pool.generate (open->hash ()))
				  .build ();
		ASSERT_EQ (lumex::block_status::progress, ledger.process (txn, receive));

		change = builder.change ()
				 .previous (receive->hash ())
				 .representative (key3.pub)
				 .sign (key2.prv, key2.pub)
				 .work (*pool.generate (receive->hash ()))
				 .build ();
		ASSERT_EQ (lumex::block_status::progress, ledger.process (txn, change));
	}

	// Sanity: every non-genesis block landed unindexed
	{
		auto txn = store->tx_begin_read ();
		ASSERT_EQ (0, store->block.get (txn, send->hash ())->sideband ().topo_height);
		ASSERT_EQ (0, store->block.get (txn, open->hash ())->sideband ().topo_height);
		ASSERT_EQ (0, store->block.get (txn, send2->hash ())->sideband ().topo_height);
		ASSERT_EQ (0, store->block.get (txn, receive->hash ())->sideband ().topo_height);
		ASSERT_EQ (0, store->block.get (txn, change->hash ())->sideband ().topo_height);
	}

	// Run migration
	{
		lumex::ledger ledger{ *store, lumex::dev::network_params, stats, logger };
		ASSERT_FALSE (ledger.flags.topo_index);
		ledger.populate_topo_index ();
		ASSERT_TRUE (ledger.flags.topo_index);
	}

	auto txn = store->tx_begin_read ();

	// Genesis: kept at 1 (set during initialize, not touched)
	auto genesis = store->block.get (txn, lumex::dev::genesis->hash ());
	ASSERT_EQ (1, genesis->sideband ().topo_height);
	ASSERT_TRUE (store->topology.exists (txn, { 1, lumex::dev::genesis->hash () }));

	// genesis(1) → send(2) → open(3) → send2(3) → receive(4) → change(5)
	ASSERT_EQ (2, store->block.get (txn, send->hash ())->sideband ().topo_height);
	ASSERT_TRUE (store->topology.exists (txn, { 2, send->hash () }));
	ASSERT_EQ (3, store->block.get (txn, open->hash ())->sideband ().topo_height);
	ASSERT_TRUE (store->topology.exists (txn, { 3, open->hash () }));
	ASSERT_EQ (3, store->block.get (txn, send2->hash ())->sideband ().topo_height);
	ASSERT_TRUE (store->topology.exists (txn, { 3, send2->hash () }));
	ASSERT_EQ (4, store->block.get (txn, receive->hash ())->sideband ().topo_height);
	ASSERT_TRUE (store->topology.exists (txn, { 4, receive->hash () }));
	ASSERT_EQ (5, store->block.get (txn, change->hash ())->sideband ().topo_height);
	ASSERT_TRUE (store->topology.exists (txn, { 5, change->hash () }));
}

/*
 * State-block ledger built before migration is correctly indexed
 * Covers state send / state open (receive) / state send / state receive / state change
 */
TEST (ledger, populate_topo_index_state_blocks)
{
	lumex::logger logger;
	lumex::stats stats{ logger };
	lumex::work_pool pool{ lumex::dev::network_params.network, 1 };
	lumex::block_builder builder;
	lumex::keypair key2;

	auto store = lumex::test::make_store (logger, stats);
	{
		lumex::ledger ledger{ *store, lumex::dev::network_params, stats, logger };
	}
	{
		auto txn = store->tx_begin_write ();
		store->version.put_flag (txn, lumex::store::meta_key::topo_index_enabled, false);
	}

	std::shared_ptr<lumex::block> send;
	std::shared_ptr<lumex::block> open;
	std::shared_ptr<lumex::block> send2;
	std::shared_ptr<lumex::block> receive;
	std::shared_ptr<lumex::block> change;
	{
		lumex::ledger ledger{ *store, lumex::dev::network_params, stats, logger };
		ASSERT_FALSE (ledger.flags.topo_index);
		auto txn = ledger.tx_begin_write ();

		send = builder.state ()
			   .account (lumex::dev::genesis_key.pub)
			   .previous (lumex::dev::genesis->hash ())
			   .representative (lumex::dev::genesis_key.pub)
			   .balance (lumex::dev::constants.genesis_amount - lumex::Klumex_ratio)
			   .link (key2.pub)
			   .sign (lumex::dev::genesis_key.prv, lumex::dev::genesis_key.pub)
			   .work (*pool.generate (lumex::dev::genesis->hash ()))
			   .build ();
		ASSERT_EQ (lumex::block_status::progress, ledger.process (txn, send));

		open = builder.state ()
			   .account (key2.pub)
			   .previous (0)
			   .representative (key2.pub)
			   .balance (lumex::Klumex_ratio)
			   .link (send->hash ())
			   .sign (key2.prv, key2.pub)
			   .work (*pool.generate (key2.pub))
			   .build ();
		ASSERT_EQ (lumex::block_status::progress, ledger.process (txn, open));

		send2 = builder.state ()
				.account (lumex::dev::genesis_key.pub)
				.previous (send->hash ())
				.representative (lumex::dev::genesis_key.pub)
				.balance (lumex::dev::constants.genesis_amount - 2 * lumex::Klumex_ratio)
				.link (key2.pub)
				.sign (lumex::dev::genesis_key.prv, lumex::dev::genesis_key.pub)
				.work (*pool.generate (send->hash ()))
				.build ();
		ASSERT_EQ (lumex::block_status::progress, ledger.process (txn, send2));

		receive = builder.state ()
				  .account (key2.pub)
				  .previous (open->hash ())
				  .representative (key2.pub)
				  .balance (2 * lumex::Klumex_ratio)
				  .link (send2->hash ())
				  .sign (key2.prv, key2.pub)
				  .work (*pool.generate (open->hash ()))
				  .build ();
		ASSERT_EQ (lumex::block_status::progress, ledger.process (txn, receive));

		lumex::keypair key3;
		change = builder.state ()
				 .account (key2.pub)
				 .previous (receive->hash ())
				 .representative (key3.pub)
				 .balance (2 * lumex::Klumex_ratio)
				 .link (0)
				 .sign (key2.prv, key2.pub)
				 .work (*pool.generate (receive->hash ()))
				 .build ();
		ASSERT_EQ (lumex::block_status::progress, ledger.process (txn, change));
	}

	{
		lumex::ledger ledger{ *store, lumex::dev::network_params, stats, logger };
		ledger.populate_topo_index ();
		ASSERT_TRUE (ledger.flags.topo_index);
	}

	auto txn = store->tx_begin_read ();

	// genesis(1) → send(2) → open(3) [open's prev=0, link=send(2) → max(0,2)+1 = 3]
	// send2(3) [prev=send(2)] → receive(4) [prev=open(3), link=send2(3)] → change(5) [prev=receive(4)]
	ASSERT_EQ (2, store->block.get (txn, send->hash ())->sideband ().topo_height);
	ASSERT_EQ (3, store->block.get (txn, open->hash ())->sideband ().topo_height);
	ASSERT_EQ (3, store->block.get (txn, send2->hash ())->sideband ().topo_height);
	ASSERT_EQ (4, store->block.get (txn, receive->hash ())->sideband ().topo_height);
	ASSERT_EQ (5, store->block.get (txn, change->hash ())->sideband ().topo_height);
	ASSERT_TRUE (store->topology.exists (txn, { 2, send->hash () }));
	ASSERT_TRUE (store->topology.exists (txn, { 3, open->hash () }));
	ASSERT_TRUE (store->topology.exists (txn, { 3, send2->hash () }));
	ASSERT_TRUE (store->topology.exists (txn, { 4, receive->hash () }));
	ASSERT_TRUE (store->topology.exists (txn, { 5, change->hash () }));
}

/*
 * Migration must handle epoch upgrades on existing chains and rootless epoch open on unopened accounts
 * Rootless epoch open must be anchored at topo=1, mirroring the live processor behavior
 */
TEST (ledger, populate_topo_index_with_epoch_blocks)
{
	lumex::logger logger;
	lumex::stats stats{ logger };
	auto store = lumex::test::make_store (logger, stats);
	lumex::work_pool pool{ lumex::dev::network_params.network, 1 };
	lumex::block_builder builder;
	lumex::keypair key1;

	{
		lumex::ledger ledger{ *store, lumex::dev::network_params, stats, logger };
	}
	{
		auto txn = store->tx_begin_write ();
		store->version.put_flag (txn, lumex::store::meta_key::topo_index_enabled, false);
	}

	std::shared_ptr<lumex::block> send;
	std::shared_ptr<lumex::block> epoch_upgrade;
	std::shared_ptr<lumex::block> epoch_open;
	std::shared_ptr<lumex::block> receive;
	{
		lumex::ledger ledger{ *store, lumex::dev::network_params, stats, logger };
		ASSERT_FALSE (ledger.flags.topo_index);
		auto txn = ledger.tx_begin_write ();

		// Genesis send to key1
		send = builder.state ()
			   .account (lumex::dev::genesis_key.pub)
			   .previous (lumex::dev::genesis->hash ())
			   .representative (lumex::dev::genesis_key.pub)
			   .balance (lumex::dev::constants.genesis_amount - lumex::Klumex_ratio)
			   .link (key1.pub)
			   .sign (lumex::dev::genesis_key.prv, lumex::dev::genesis_key.pub)
			   .work (*pool.generate (lumex::dev::genesis->hash ()))
			   .build ();
		ASSERT_EQ (lumex::block_status::progress, ledger.process (txn, send));

		// Epoch upgrade on the genesis chain (still has prev = send)
		epoch_upgrade = builder.state ()
						.account (lumex::dev::genesis_key.pub)
						.previous (send->hash ())
						.representative (lumex::dev::genesis_key.pub)
						.balance (lumex::dev::constants.genesis_amount - lumex::Klumex_ratio)
						.link (ledger.epoch_link (lumex::epoch::epoch_1))
						.sign (lumex::dev::genesis_key.prv, lumex::dev::genesis_key.pub)
						.work (*pool.generate (send->hash ()))
						.build ();
		ASSERT_EQ (lumex::block_status::progress, ledger.process (txn, epoch_upgrade));

		// Rootless epoch open on key1 (still unopened)
		epoch_open = builder.state ()
					 .account (key1.pub)
					 .previous (0)
					 .representative (0)
					 .balance (0)
					 .link (ledger.epoch_link (lumex::epoch::epoch_1))
					 .sign (lumex::dev::genesis_key.prv, lumex::dev::genesis_key.pub)
					 .work (*pool.generate (key1.pub))
					 .build ();
		ASSERT_EQ (lumex::block_status::progress, ledger.process (txn, epoch_open));

		// Receive on key1 chain after the epoch open
		receive = builder.state ()
				  .account (key1.pub)
				  .previous (epoch_open->hash ())
				  .representative (key1.pub)
				  .balance (lumex::Klumex_ratio)
				  .link (send->hash ())
				  .sign (key1.prv, key1.pub)
				  .work (*pool.generate (epoch_open->hash ()))
				  .build ();
		ASSERT_EQ (lumex::block_status::progress, ledger.process (txn, receive));
	}

	{
		lumex::ledger ledger{ *store, lumex::dev::network_params, stats, logger };
		ledger.populate_topo_index ();
		ASSERT_TRUE (ledger.flags.topo_index);
	}

	auto txn = store->tx_begin_read ();
	// genesis(1) → send(2) → epoch_upgrade(3) [prev=send(2)]
	// epoch_open(1) [rootless: prev=0, link=epoch] → receive(3) [prev=epoch_open(1), link=send(2) → max(1,2)+1=3]
	ASSERT_EQ (2, store->block.get (txn, send->hash ())->sideband ().topo_height);
	ASSERT_EQ (3, store->block.get (txn, epoch_upgrade->hash ())->sideband ().topo_height);
	ASSERT_EQ (1, store->block.get (txn, epoch_open->hash ())->sideband ().topo_height);
	ASSERT_EQ (3, store->block.get (txn, receive->hash ())->sideband ().topo_height);
	ASSERT_TRUE (store->topology.exists (txn, { 2, send->hash () }));
	ASSERT_TRUE (store->topology.exists (txn, { 3, epoch_upgrade->hash () }));
	ASSERT_TRUE (store->topology.exists (txn, { 1, epoch_open->hash () }));
	ASSERT_TRUE (store->topology.exists (txn, { 3, receive->hash () }));
}

/*
 * Migration over a ledger where some blocks are pre-indexed (topo_height != 0) and others are not
 * The DFS must skip already-indexed blocks via the outer-loop fast path, and must use
 * pre-indexed blocks' topo_height as deps when resolving their unindexed descendants
 *
 * Setup:
 *   Phase A (flag ON):  genesis(1) → send(2) → open(3)
 *   Phase B (flag OFF): send2(0) → receive(0) → change(0)
 *
 * After migration:
 *   genesis(1), send(2), open(3) unchanged; send2(3), receive(4), change(5) freshly computed
 */
TEST (ledger, populate_topo_index_mixed_indexed_and_unindexed)
{
	lumex::logger logger;
	lumex::stats stats{ logger };
	auto store = lumex::test::make_store (logger, stats);
	lumex::work_pool pool{ lumex::dev::network_params.network, 1 };
	lumex::block_builder builder;
	lumex::keypair key2;
	lumex::keypair key3;

	std::shared_ptr<lumex::block> send;
	std::shared_ptr<lumex::block> open;
	std::shared_ptr<lumex::block> send2;
	std::shared_ptr<lumex::block> receive;
	std::shared_ptr<lumex::block> change;

	// Phase A: process the head of each chain with the flag ON so they pick up real topo_heights
	{
		lumex::ledger ledger{ *store, lumex::dev::network_params, stats, logger };
		ASSERT_TRUE (ledger.flags.topo_index);
		auto txn = ledger.tx_begin_write ();
		auto info1 = ledger.any.account_get (txn, lumex::dev::genesis_key.pub);
		ASSERT_TRUE (info1);

		send = builder.send ()
			   .previous (info1->head)
			   .destination (key2.pub)
			   .balance (lumex::dev::constants.genesis_amount - lumex::Klumex_ratio)
			   .sign (lumex::dev::genesis_key.prv, lumex::dev::genesis_key.pub)
			   .work (*pool.generate (info1->head))
			   .build ();
		ASSERT_EQ (lumex::block_status::progress, ledger.process (txn, send));
		ASSERT_EQ (2, send->sideband ().topo_height);

		open = builder.open ()
			   .source (send->hash ())
			   .representative (key2.pub)
			   .account (key2.pub)
			   .sign (key2.prv, key2.pub)
			   .work (*pool.generate (key2.pub))
			   .build ();
		ASSERT_EQ (lumex::block_status::progress, ledger.process (txn, open));
		ASSERT_EQ (3, open->sideband ().topo_height);
	}

	// Flip the flag off so the next batch lands unindexed
	{
		auto txn = store->tx_begin_write ();
		store->version.put_flag (txn, lumex::store::meta_key::topo_index_enabled, false);
	}

	// Phase B: process more blocks with the flag off -- topo_height must stay 0
	{
		lumex::ledger ledger{ *store, lumex::dev::network_params, stats, logger };
		ASSERT_FALSE (ledger.flags.topo_index);
		auto txn = ledger.tx_begin_write ();

		send2 = builder.send ()
				.previous (send->hash ())
				.destination (key2.pub)
				.balance (lumex::dev::constants.genesis_amount - 2 * lumex::Klumex_ratio)
				.sign (lumex::dev::genesis_key.prv, lumex::dev::genesis_key.pub)
				.work (*pool.generate (send->hash ()))
				.build ();
		ASSERT_EQ (lumex::block_status::progress, ledger.process (txn, send2));
		ASSERT_EQ (0, send2->sideband ().topo_height);

		receive = builder.receive ()
				  .previous (open->hash ())
				  .source (send2->hash ())
				  .sign (key2.prv, key2.pub)
				  .work (*pool.generate (open->hash ()))
				  .build ();
		ASSERT_EQ (lumex::block_status::progress, ledger.process (txn, receive));
		ASSERT_EQ (0, receive->sideband ().topo_height);

		change = builder.change ()
				 .previous (receive->hash ())
				 .representative (key3.pub)
				 .sign (key2.prv, key2.pub)
				 .work (*pool.generate (receive->hash ()))
				 .build ();
		ASSERT_EQ (lumex::block_status::progress, ledger.process (txn, change));
		ASSERT_EQ (0, change->sideband ().topo_height);
	}

	// Run migration
	{
		lumex::ledger ledger{ *store, lumex::dev::network_params, stats, logger };
		ASSERT_FALSE (ledger.flags.topo_index);
		ledger.populate_topo_index ();
		ASSERT_TRUE (ledger.flags.topo_index);
	}

	auto txn = store->tx_begin_read ();

	// Pre-indexed blocks: untouched
	ASSERT_EQ (1, store->block.get (txn, lumex::dev::genesis->hash ())->sideband ().topo_height);
	ASSERT_EQ (2, store->block.get (txn, send->hash ())->sideband ().topo_height);
	ASSERT_EQ (3, store->block.get (txn, open->hash ())->sideband ().topo_height);

	// Unindexed blocks: freshly computed against the pre-indexed deps
	// send2: prev = send(2) → 3
	// receive: max(prev = open(3), src = send2(3)) → 4
	// change: prev = receive(4) → 5
	ASSERT_EQ (3, store->block.get (txn, send2->hash ())->sideband ().topo_height);
	ASSERT_EQ (4, store->block.get (txn, receive->hash ())->sideband ().topo_height);
	ASSERT_EQ (5, store->block.get (txn, change->hash ())->sideband ().topo_height);

	// Topology table populated for every block
	ASSERT_TRUE (store->topology.exists (txn, { 1, lumex::dev::genesis->hash () }));
	ASSERT_TRUE (store->topology.exists (txn, { 2, send->hash () }));
	ASSERT_TRUE (store->topology.exists (txn, { 3, open->hash () }));
	ASSERT_TRUE (store->topology.exists (txn, { 3, send2->hash () }));
	ASSERT_TRUE (store->topology.exists (txn, { 4, receive->hash () }));
	ASSERT_TRUE (store->topology.exists (txn, { 5, change->hash () }));
}

/*
 * After dropping, blocks remain in the ledger and `populate_topo_index` can re-build the index
 */
TEST (ledger, drop_topo_index)
{
	lumex::logger logger;
	lumex::stats stats{ logger };
	auto const path = lumex::unique_path ();
	lumex::work_pool pool{ lumex::dev::network_params.network, 1 };
	lumex::block_builder builder;
	lumex::keypair key2;

	std::shared_ptr<lumex::block> send;
	std::shared_ptr<lumex::block> open;

	auto store = lumex::test::make_store (logger, stats, path);

	// Build a small chain on a fresh ledger (topo_index enabled by default)
	{
		lumex::ledger ledger{ *store, lumex::dev::network_params, stats, logger };
		ASSERT_TRUE (ledger.flags.topo_index);

		auto txn = ledger.tx_begin_write ();
		auto info1 = ledger.any.account_get (txn, lumex::dev::genesis_key.pub);
		ASSERT_TRUE (info1);

		send = builder.send ()
			   .previous (info1->head)
			   .destination (key2.pub)
			   .balance (lumex::dev::constants.genesis_amount - lumex::Klumex_ratio)
			   .sign (lumex::dev::genesis_key.prv, lumex::dev::genesis_key.pub)
			   .work (*pool.generate (info1->head))
			   .build ();
		ASSERT_EQ (lumex::block_status::progress, ledger.process (txn, send));

		open = builder.open ()
			   .source (send->hash ())
			   .representative (key2.pub)
			   .account (key2.pub)
			   .sign (key2.prv, key2.pub)
			   .work (*pool.generate (key2.pub))
			   .build ();
		ASSERT_EQ (lumex::block_status::progress, ledger.process (txn, open));
	}

	// Sanity: topology table populated, flag set
	{
		auto txn = store->tx_begin_read ();
		ASSERT_LT (0, store->topology.count (txn));
		ASSERT_TRUE (store->topology.exists (txn, { 1, lumex::dev::genesis->hash () }));
		ASSERT_TRUE (store->topology.exists (txn, { 2, send->hash () }));
		ASSERT_TRUE (store->topology.exists (txn, { 3, open->hash () }));
	}

	// Drop the index
	{
		lumex::ledger ledger{ *store, lumex::dev::network_params, stats, logger };
		ASSERT_TRUE (ledger.flags.topo_index);
		ledger.drop_topo_index ();
		ASSERT_FALSE (ledger.flags.topo_index);
	}

	// Topology table empty, meta flag false, blocks still present
	{
		auto txn = store->tx_begin_read ();
		ASSERT_EQ (0, store->topology.count (txn));
		ASSERT_FALSE (store->topology.exists (txn, { 1, lumex::dev::genesis->hash () }));
		ASSERT_FALSE (store->topology.exists (txn, { 2, send->hash () }));
		ASSERT_FALSE (store->topology.exists (txn, { 3, open->hash () }));
		ASSERT_FALSE (store->version.get_flag (txn, lumex::store::meta_key::topo_index_enabled));
		ASSERT_TRUE (store->block.exists (txn, send->hash ()));
		ASSERT_TRUE (store->block.exists (txn, open->hash ()));
	}

	// Re-populate after drop: round-trip restores the index
	{
		lumex::ledger ledger{ *store, lumex::dev::network_params, stats, logger };
		ASSERT_FALSE (ledger.flags.topo_index);
		ledger.populate_topo_index ();
		ASSERT_TRUE (ledger.flags.topo_index);
	}
	{
		auto txn = store->tx_begin_read ();
		ASSERT_TRUE (store->topology.exists (txn, { 1, lumex::dev::genesis->hash () }));
		ASSERT_TRUE (store->topology.exists (txn, { 2, send->hash () }));
		ASSERT_TRUE (store->topology.exists (txn, { 3, open->hash () }));
	}
}