#include <lumex/core_test/fakes/work_peer.hpp>
...
TEST (distributed_work, stopped)
{
	...
	ASSERT_TRUE (system.nodes[0]->distributed_work.make (lumex::work_version::work_1, lumex::block_hash (), {}, lumex::dev::network_params.work.base, {}));
}
...
TEST (distributed_work, no_peers)
{
	...
	ASSERT_FALSE (node->distributed_work.make (lumex::work_version::work_1, hash, node->config.work_peers, node->network_params.work.base, callback, lumex::account ()));
	ASSERT_TIMELY (5s, done);
	ASSERT_GE (lumex::dev::network_params.work.difficulty (lumex::work_version::work_1, hash, *work), node->network_params.work.base);
...
TEST (distributed_work, no_peers_disabled)
{
	...
	ASSERT_TRUE (node.distributed_work.make (lumex::work_version::work_1, lumex::block_hash (), node.config.work_peers, lumex::dev::network_params.work.base, {}));
}
...
TEST (distributed_work, no_peers_cancel)
{
	...
	ASSERT_FALSE (node.distributed_work.make (lumex::work_version::work_1, hash, node.config.work_peers, lumex::difficulty::from_multiplier (1e6, node.network_params.work.base), callback_to_cancel));
	ASSERT_EQ (1, node.distributed_work.size ());
...
TEST (distributed_work, no_peers_multi)
{
	...
	{
		ASSERT_FALSE (node->distributed_work.make (lumex::work_version::work_1, hash, node->config.work_peers, lumex::difficulty::from_multiplier (10, node->network_params.work.base), callback));
	}
	...
	for (unsigned i{ 0 }; i < total; ++i)
	{
		lumex::block_hash hash_i (i + 1);
		ASSERT_FALSE (node->distributed_work.make (lumex::work_version::work_1, hash_i, node->config.work_peers, node->network_params.work.base, callback));
	}
	ASSERT_TIMELY_EQ (5s, count, total);
	system.deadline_set (5s);
	while (node->distributed_work.size () > 0)
...
TEST (distributed_work, peer)
{
	...
	ASSERT_FALSE (node->distributed_work.make (lumex::work_version::work_1, hash, peers, node->network_params.work.base, callback, lumex::account ()));
	ASSERT_TIMELY (5s, done);
	ASSERT_GE (lumex::dev::network_params.work.difficulty (lumex::work_version::work_1, hash, *work), node->network_params.work.base);
...
TEST (distributed_work, DISABLED_peer_malicious)
{
	...
	ASSERT_FALSE (node->distributed_work.make (lumex::work_version::work_1, hash, peers, node->network_params.work.base, {}, lumex::account ()));
...
// Issue for investigating it: https://github.com/lumexcurrency/lumex-node/issues/3630
