...
{
	lumex::stats stats;
	lumex::frontier_scan_config config;
	lumex::bootstrap::frontier_scan_index frontier_scan;

	explicit test_context (lumex::frontier_scan_config config_a = {}) :
		stats{ lumex::default_logger () },
		config{ config_a },
...
TEST (bootstrap_frontier_scan, next_basic)
{
	...
	ASSERT_EQ (second.number (), lumex::account{ "7FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF" });
...
TEST (bootstrap_frontier_scan, process_basic)
{
	...
	std::deque<std::pair<lumex::account, lumex::block_hash>> response;
	response.push_back ({ lumex::account{ 2 }, lumex::block_hash{ 1 } });
	response.push_back ({ lumex::account{ 3 }, lumex::block_hash{ 2 } });
...
TEST (bootstrap_frontier_scan, range_wrap_around)
{
	...
	std::deque<std::pair<lumex::account, lumex::block_hash>> response;
	response.push_back ({ lumex::account{ std::numeric_limits<lumex::uint256_t>::max () }, lumex::block_hash{ 1 } });
...
TEST (bootstrap_frontier_scan, candidate_trimming)
{
	...
	std::deque<std::pair<lumex::account, lumex::block_hash>> response1;
	for (int i = 0; i <= 9; i += 3)
	{
		response1.push_back ({ lumex::account{ start.number () + i }, lumex::block_hash{ static_cast<uint64_t> (i) } });
	}
	ASSERT_FALSE (frontier_scan.process (start, response1));

	// Response contains: 1, 3, 5, 7, 9
	std::deque<std::pair<lumex::account, lumex::block_hash>> response2;
	for (int i = 0; i <= 8; i += 2)
	{
		response2.push_back ({ lumex::account{ start.number () + i }, lumex::block_hash{ static_cast<uint64_t> (i) } });
	}
...
TEST (bootstrap_frontier_scan, heads_distribution)
{
	...
	std::vector<lumex::account> initial_accounts;
	for (int i = 0; i < 4; i++)
	{
...
TEST (bootstrap_frontier_scan, invalid_response_ordering)
{
	...
	std::deque<std::pair<lumex::account, lumex::block_hash>> response;
	response.push_back ({ lumex::account{ start.number () + 2 }, lumex::block_hash{ 1 } });
	response.push_back ({ lumex::account{ start.number () + 1 }, lumex::block_hash{ 2 } }); // Out of order
...
TEST (bootstrap_frontier_scan, empty_responses)
{
	...
	std::deque<std::pair<lumex::account, lumex::block_hash>> empty_response;
	...
	std::deque<std::pair<lumex::account, lumex::block_hash>> response;
	response.push_back ({ lumex::account{ start.number () + 1 }, lumex::block_hash{ 1 } });
	ASSERT_TRUE (frontier_scan.process (start, response));
