...
lumex::store::column_schema const test_schema{
...
lumex::uint256_union make_key (uint64_t value)
{
...
lumex::uint256_union make_value (uint64_t value)
{
...
{
	auto backend = lumex::test::make_backend ();
	backend->create (test_schema, 1);
	backend->open (test_schema, lumex::store::open_mode::read_write);

	auto key = make_key (1);
	auto value = make_value (42);

	auto write_tx = backend->tx_begin_write ();
	auto put_status = backend->put (write_tx, lumex::store::table::accounts, lumex::store::db_val{ key }, lumex::store::db_val{ value });
	ASSERT_TRUE (backend->success (put_status));
	write_tx.commit ();

	auto read_tx = backend->tx_begin_read ();
	lumex::store::db_val result;
	auto get_status = backend->get (read_tx, lumex::store::table::accounts, lumex::store::db_val{ key }, result);
	ASSERT_TRUE (backend->success (get_status));
	ASSERT_EQ (result.size (), sizeof (lumex::uint256_union));
	auto result_value = result.convert_to<lumex::uint256_union> ();
...
TEST (backend, put_overwrite)
{
	...
	{
		auto write_tx = backend->tx_begin_write ();
		backend->put (write_tx, lumex::store::table::accounts, lumex::store::db_val{ key }, lumex::store::db_val{ value1 });
	}
	{
		auto write_tx = backend->tx_begin_write ();
		backend->put (write_tx, lumex::store::table::accounts, lumex::store::db_val{ key }, lumex::store::db_val{ value2 });
	}

	auto read_tx = backend->tx_begin_read ();
	lumex::store::db_val result;
	backend->get (read_tx, lumex::store::table::accounts, lumex::store::db_val{ key }, result);
	auto result_value = result.convert_to<lumex::uint256_union> ();
...
TEST (backend, get_non_existent)
{
	...
	auto read_tx = backend->tx_begin_read ();
	lumex::store::db_val result;
	auto status = backend->get (read_tx, lumex::store::table::accounts, lumex::store::db_val{ key }, result);
...
TEST (backend, exists_after_put)
{
	...
	auto write_tx = backend->tx_begin_write ();
	EXPECT_FALSE (backend->exists (write_tx, lumex::store::table::accounts, lumex::store::db_val{ key }));
	backend->put (write_tx, lumex::store::table::accounts, lumex::store::db_val{ key }, lumex::store::db_val{ value });
	EXPECT_TRUE (backend->exists (write_tx, lumex::store::table::accounts, lumex::store::db_val{ key }));
}
...
TEST (backend, delete_existing)
{
	...
	}
	{
		auto write_tx = backend->tx_begin_write ();
		EXPECT_TRUE (backend->exists (write_tx, lumex::store::table::accounts, lumex::store::db_val{ key }));
		auto status = backend->del (write_tx, lumex::store::table::accounts, lumex::store::db_val{ key });
...
TEST (backend, exists_after_delete)
{
	...
	{
		auto write_tx = backend->tx_begin_write ();
		backend->del (write_tx, lumex::store::table::accounts, lumex::store::db_val{ key });
	}

	auto read_tx = backend->tx_begin_read ();
	EXPECT_FALSE (backend->exists (read_tx, lumex::store::table::accounts, lumex::store::db_val{ key }));
}
...
TEST (backend, empty_value)
{
	...
	{
		auto key = make_key (1);
		lumex::store::db_val empty_value{ nullptr };

		auto write_tx = backend->tx_begin_write ();
		auto put_status = backend->put (write_tx, lumex::store::table::accounts, lumex::store::db_val{ key }, empty_value);
		ASSERT_TRUE (backend->success (put_status));
		write_tx.commit ();
	...
	{
		auto key = make_key (2);
		lumex::store::db_val empty_value{ std::span<uint8_t const>{} };
...
TEST (backend, binary_null_bytes)
{
	...
	lumex::store::db_val key{ key_data };
	lumex::store::db_val value{ value_data };

	auto write_tx = backend->tx_begin_write ();
	auto put_status = backend->put (write_tx, lumex::store::table::accounts, key, value);
	ASSERT_TRUE (backend->success (put_status));
	write_tx.commit ();

	auto read_tx = backend->tx_begin_read ();
	lumex::store::db_val result;
	auto get_status = backend->get (read_tx, lumex::store::table::accounts, key, result);
	ASSERT_TRUE (backend->success (get_status));
...
TEST (backend, binary_all_bytes)
{
	...
	auto key = make_key (1);
	lumex::store::db_val value{ all_bytes };

	auto write_tx = backend->tx_begin_write ();
	auto put_status = backend->put (write_tx, lumex::store::table::accounts, lumex::store::db_val{ key }, value);
	ASSERT_TRUE (backend->success (put_status));
	write_tx.commit ();
...
TEST (backend, large_value)
{
	...
	auto key = make_key (1);
	lumex::store::db_val value{ large_data };
...
TEST (backend, probe_key)
{
	...
	// Insert 64-byte keys with different first bytes
	{
		...
		for (uint8_t first_byte : { 0x20, 0x60, 0xA0, 0xE0 })
		{
			...
			backend->put (write_tx, lumex::store::table::accounts, lumex::store::db_val{ key }, lumex::store::db_val{ make_value (first_byte) });
		}
	}
	...
	// Probe with 32-byte key (shorter than actual 64-byte keys)
	{
		...
		auto it = backend->begin (read_tx, lumex::store::table::accounts, lumex::store::db_val{ probe });
...
TEST (backend, iterator_empty_table)
{
	...
	auto read_tx = backend->tx_begin_read ();
	auto begin_it = backend->begin (read_tx, lumex::store::table::accounts);
	auto end_it = backend->end (read_tx, lumex::store::table::accounts);
...
TEST (backend, iterator_single_entry)
{
	...
	auto read_tx = backend->tx_begin_read ();
	auto it = backend->begin (read_tx, lumex::store::table::accounts);
	auto end_it = backend->end (read_tx, lumex::store::table::accounts);
	...
	auto [k, v] = *it;
	EXPECT_EQ (k.size (), sizeof (lumex::uint256_union));
...
TEST (backend, iterator_forward)
{
	...
	std::vector<lumex::uint256_union> keys;
...

	{
		...
		{
			backend->put (write_tx, lumex::store::table::accounts, lumex::store::db_val{ key }, lumex::store::db_val{ make_value (0) });
		}
	}
	...
	std::vector<lumex::uint256_union> found_keys;
	for (; it != end_it; ++it)
	{
		auto [k, v] = *it;
		auto found_key = lumex::store::db_val{ k }.convert_to<lumex::uint256_union> ();
...
TEST (backend, iterator_decrement_from_end)
{
	...
	{
		auto write_tx = backend->tx_begin_write ();
		backend->put (write_tx, lumex::store::table::accounts, lumex::store::db_val{ key1 }, lumex::store::db_val{ make_value (0) });
		backend->put (write_tx, lumex::store::table::accounts, lumex::store::db_val{ key2 }, lumex::store::db_val{ make_value (0) });
		backend->put (write_tx, lumex::store::table::accounts, lumex::store::db_val{ key3 }, lumex::store::db_val{ make_value (0) });
	}
...
TEST (backend, iterator_lower_bound_exact)
{
	...
	auto read_tx = backend->tx_begin_read ();
	auto it = backend->begin (read_tx, lumex::store::table::accounts, lumex::store::db_val{ key2 });
...
TEST (backend, iterator_lower_bound_past_all)
{
	...
	auto it = backend->begin (read_tx, lumex::store::table::accounts, lumex::store::db_val{ large_key });
...
TEST (backend, tx_write_commit)
{
	...
	auto read_tx = backend->tx_begin_read ();
	EXPECT_TRUE (backend->exists (read_tx, lumex::store::table::accounts, lumex::store::db_val{ key }));
}
...
TEST (backend, tx_refresh)
{
	...
	read_tx.refresh ();
	backend->get (read_tx, lumex::store::table::accounts, lumex::store::db_val{ key }, result);
	result_value = result.convert_to<lumex::uint256_union> ();
	EXPECT_EQ (result_value, value2);
}
...
TEST (backend, tx_multiple_read)
{
	...
	EXPECT_TRUE (backend->exists (read_tx1, lumex::store::table::accounts, lumex::store::db_val{ key }));
	EXPECT_TRUE (backend->exists (read_tx2, lumex::store::table::accounts, lumex::store::db_val{ key }));
}
...
TEST (backend, tx_read_isolation)
{
	...
	{
		auto write_tx = backend->tx_begin_write ();
		backend->put (write_tx, lumex::store::table::accounts, lumex::store::db_val{ key1 }, lumex::store::db_val{ value1 });
	}
	...
	{
		auto write_tx = backend->tx_begin_write ();
		backend->put (write_tx, lumex::store::table::accounts, lumex::store::db_val{ key1 }, lumex::store::db_val{ value2 });
		backend->put (write_tx, lumex::store::table::accounts, lumex::store::db_val{ key2 }, lumex::store::db_val{ value2 });
		backend->put (write_tx, lumex::store::table::accounts, lumex::store::db_val{ key3 }, lumex::store::db_val{ value2 });
	}

	// Test get() isolation - should see old value for key1
	{
		lumex::store::db_val result;
		auto status = backend->get (read_tx, lumex::store::table::accounts, lumex::store::db_val{ key1 }, result);
	...
	{
		EXPECT_TRUE (backend->exists (read_tx, lumex::store::table::accounts, lumex::store::db_val{ key1 }));
		EXPECT_FALSE (backend->exists (read_tx, lumex::store::table::accounts, lumex::store::db_val{ key2 }));
	}

	// Test iterator isolation - should only see key1, not key2 or key3
	{
		auto it = backend->begin (read_tx, lumex::store::table::accounts);
		auto end = backend->end (read_tx, lumex::store::table::accounts);
...
TEST (backend, tx_write_read_isolation)
{
	...
	// Another write transaction commits changes AFTER our write_tx started
	{
		...
		backend->put (other_write_tx, lumex::store::table::accounts, lumex::store::db_val{ key1 }, lumex::store::db_val{ value2 });
		backend->put (other_write_tx, lumex::store::table::accounts, lumex::store::db_val{ key2 }, lumex::store::db_val{ value2 });
		backend->put (other_write_tx, lumex::store::table::accounts, lumex::store::db_val{ key3 }, lumex::store::db_val{ value2 });
	}

	// Test get() isolation within write transaction - should see old value for key1
	{
		lumex::store::db_val result;
		auto status = backend->get (write_tx, lumex::store::table::accounts, lumex::store::db_val{ key1 }, result);
		ASSERT_TRUE (backend->success (status));
		auto result_value = result.convert_to<lumex::uint256_union> ();
		EXPECT_EQ (result_value, value1);
	}
	...
	{
		EXPECT_TRUE (backend->exists (write_tx, lumex::store::table::accounts, lumex::store::db_val{ key1 }));
		EXPECT_FALSE (backend->exists (write_tx, lumex::store::table::accounts, lumex::store::db_val{ key2 }));
	}

	// Test iterator isolation within write transaction
	{
		auto it = backend->begin (write_tx, lumex::store::table::accounts);
		auto end = backend->end (write_tx, lumex::store::table::accounts);
...
TEST (backend, count_empty)
{
	...
	auto read_tx = backend->tx_begin_read ();
	EXPECT_EQ (backend->count (read_tx, lumex::store::table::accounts), 0);
}
...
TEST (backend, count_accuracy)
{
...

	{
		...
		{
			backend->put (write_tx, lumex::store::table::accounts, lumex::store::db_val{ make_key (i) }, lumex::store::db_val{ make_value (i) });
		}
	}

	auto read_tx = backend->tx_begin_read ();
	EXPECT_EQ (backend->count (read_tx, lumex::store::table::accounts), 10);

	// Delete some entries
	{
		auto write_tx = backend->tx_begin_write ();
		backend->del (write_tx, lumex::store::table::accounts, lumex::store::db_val{ make_key (0) });
		backend->del (write_tx, lumex::store::table::accounts, lumex::store::db_val{ make_key (1) });
	}

	auto read_tx2 = backend->tx_begin_read ();
	EXPECT_EQ (backend->count (read_tx2, lumex::store::table::accounts), 8);
}
...
TEST (backend, empty_true_false)
{
	...
	{
		auto read_tx = backend->tx_begin_read ();
		EXPECT_TRUE (backend->empty (read_tx, lumex::store::table::accounts));
	}
	{
		auto write_tx = backend->tx_begin_write ();
		backend->put (write_tx, lumex::store::table::accounts, lumex::store::db_val{ make_key (1) }, lumex::store::db_val{ make_value (1) });
	}
	{
		auto read_tx = backend->tx_begin_read ();
		EXPECT_FALSE (backend->empty (read_tx, lumex::store::table::accounts));
	}
}
...
TEST (backend, empty_vs_count)
{
	...
	// empty() should be reliable even when count() might return estimates
	{
		...
		auto count = backend->count (read_tx, lumex::store::table::accounts);
		EXPECT_EQ (count, 0);
	}
...
TEST (backend, clear_table)
{
	...
	{
		auto status = backend->clear (lumex::store::table::accounts);
		EXPECT_TRUE (backend->success (status));
	}
...
TEST (backend, drop_table_not_in_schema)
{
	...
	lumex::store::column_schema const old_schema{
		{ lumex::store::table::meta, "meta" },
		{ lumex::store::table::accounts, "accounts" },
		{ lumex::store::table::frontiers, "frontiers" }, // Dropped in v24
	};
	...
	auto backend = lumex::test::make_backend (path);
	{
		...
		backend->open (old_schema, lumex::store::open_mode::read_write);

		auto write_tx = backend->tx_begin_write ();
		backend->put (write_tx, lumex::store::table::frontiers, lumex::store::db_val{ make_key (1) }, lumex::store::db_val{ make_value (1) });
	}
	backend->close ();
...
TEST (backend, open_already_open_throws)
{
	...
	EXPECT_THROW (backend->open (test_schema, lumex::store::open_mode::read_write), std::runtime_error);
}
...
TEST (backend, open_nonexistent_readonly_throws)
{
	...
	// Opening a non-existent database in read-only mode should throw lumex::error with db_not_found
	EXPECT_THROW (
	{
		try
		{
			backend->open (test_schema, lumex::store::open_mode::read_only);
		}
		catch (lumex::error const & e)
		{
			EXPECT_EQ (e, lumex::error_backend::db_not_found);
			throw;
		}
	},
	lumex::error);
...
{
	auto src_path = lumex::unique_path ();
	auto dst_path = lumex::unique_path ();

	auto src_backend = lumex::test::make_backend (src_path);
	src_backend->create (test_schema, 1);
	src_backend->open (test_schema, lumex::store::open_mode::read_write);

	auto dst_backend = lumex::test::make_backend (dst_path);
	dst_backend->create (test_schema, 1);
	dst_backend->open (test_schema, lumex::store::open_mode::read_write);

	// Add data to destination backend
	{
		auto write_tx = dst_backend->tx_begin_write ();
		dst_backend->put (write_tx, lumex::store::table::accounts, lumex::store::db_val{ make_key (1) }, lumex::store::db_val{ make_value (1) });
	}
