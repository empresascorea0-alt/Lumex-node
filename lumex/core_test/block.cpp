...
TEST (transaction_block, empty)
{
	...
	ASSERT_FALSE (lumex::validate_message (key1.pub, hash, block->signature));
	block->signature.bytes[32] ^= 0x1;
	ASSERT_TRUE (lumex::validate_message (key1.pub, hash, block->signature));
}
...
TEST (block, send_serialize)
{
	...
	ASSERT_NE (0, size);
	lumex::bufferstream stream2 (data, size);
	bool error (false);
	lumex::send_block block2 (error, stream2);
	ASSERT_FALSE (error);
...
TEST (block, send_serialize_json)
{
	...
	bool error (false);
	lumex::send_block block2 (error, tree1);
	ASSERT_FALSE (error);
	ASSERT_EQ (*block1, block2);
}
...
TEST (block, receive_serialize)
{
	...
	bool error (false);
	lumex::receive_block block2 (error, stream2);
	ASSERT_FALSE (error);
	ASSERT_EQ (*block1, block2);
}
...
TEST (block, receive_serialize_json)
{
	...
	boost::property_tree::read_json (istream, tree1);
	bool error (false);
	lumex::receive_block block2 (error, tree1);
	ASSERT_FALSE (error);
	ASSERT_EQ (*block1, block2);
}
...
TEST (block, open_serialize_json)
{
	...
	boost::property_tree::read_json (istream, tree1);
	bool error (false);
	lumex::open_block block2 (error, tree1);
	ASSERT_FALSE (error);
	ASSERT_EQ (*block1, block2);
}
...
TEST (block, change_serialize_json)
{
	...
	boost::property_tree::read_json (istream, tree1);
	bool error (false);
	lumex::change_block block2 (error, tree1);
	ASSERT_FALSE (error);
	ASSERT_EQ (*block1, block2);
}
...
TEST (send_block, deserialize)
{
	...
	ASSERT_EQ (lumex::send_block::size, bytes.size ());
	lumex::bufferstream stream2 (bytes.data (), bytes.size ());
	bool error (false);
	lumex::send_block block2 (error, stream2);
	ASSERT_FALSE (error);
...
TEST (receive_block, deserialize)
{
	...
	ASSERT_EQ (lumex::receive_block::size, bytes.size ());
	lumex::bufferstream stream2 (bytes.data (), bytes.size ());
	bool error (false);
	lumex::receive_block block2 (error, stream2);
	ASSERT_FALSE (error);
...
TEST (open_block, deserialize)
{
	...
	ASSERT_EQ (lumex::open_block::size, bytes.size ());
	lumex::bufferstream stream (bytes.data (), bytes.size ());
	bool error (false);
	lumex::open_block block2 (error, stream);
	ASSERT_FALSE (error);
	ASSERT_EQ (*block1, block2);
}
...
TEST (change_block, deserialize)
{
	...
	ASSERT_EQ (lumex::change_block::size, bytes.size ());
	auto data (bytes.data ());
	auto size (bytes.size ());
	ASSERT_NE (nullptr, data);
	ASSERT_NE (0, size);
	lumex::bufferstream stream2 (data, size);
	bool error (false);
	lumex::change_block block2 (error, stream2);
	ASSERT_FALSE (error);
	ASSERT_EQ (*block1, block2);
}
...
{
	lumex::messages::frontier_req request1{ lumex::dev::network_params.network };
	request1.start = 1;
	request1.age = 2;
	request1.count = 3;
	std::vector<uint8_t> bytes;
	...
	ASSERT_FALSE (error);
	lumex::messages::frontier_req request2 (error, stream, header);
	ASSERT_FALSE (error);
...
TEST (block, publish_req_serialization)
{
	...
	auto block = builder
				 ...
				 .work (3)
				 .build ();
	lumex::messages::publish req{ lumex::dev::network_params.network, block };
	std::vector<uint8_t> bytes;
	{
		lumex::vectorstream stream (bytes);
	...
	ASSERT_FALSE (error);
	lumex::messages::publish req2 (error, stream2, header);
	ASSERT_FALSE (error);
	ASSERT_EQ (req, req2);
...
TEST (block, difficulty)
{
	...
	auto block = builder
				 ...
				 .work (5)
				 .build ();
	ASSERT_EQ (lumex::dev::network_params.work.difficulty (*block), lumex::dev::network_params.work.difficulty (block->work_version (), block->root (), block->block_work ()));
}
...
TEST (state_block, serialization)
{
	...
	ASSERT_EQ (lumex::block_hash (1), block1->previous ());
	ASSERT_EQ (key2.pub, block1->hashables.representative);
	ASSERT_EQ (lumex::amount (2), block1->hashables.balance);
	ASSERT_EQ (lumex::uint256_union (4), block1->hashables.link);
	std::vector<uint8_t> bytes;
	{
		lumex::vectorstream stream (bytes);
		block1->serialize (stream);
	...
	ASSERT_EQ (lumex::state_block::size, bytes.size ());
	bool error1 (false);
	lumex::bufferstream stream (bytes.data (), bytes.size ());
	lumex::state_block block2 (error1, stream);
	ASSERT_FALSE (error1);
	ASSERT_EQ (*block1, block2);
	...
	bool error2 (false);
	lumex::state_block block3 (error2, tree);
	ASSERT_FALSE (error2);
...
{
	ASSERT_EQ (lumex::work_version::work_1, lumex::send_block ().work_version ());
	ASSERT_EQ (lumex::work_version::work_1, lumex::receive_block ().work_version ());
	ASSERT_EQ (lumex::work_version::work_1, lumex::change_block ().work_version ());
	ASSERT_EQ (lumex::work_version::work_1, lumex::open_block ().work_version ());
	ASSERT_EQ (lumex::work_version::work_1, lumex::state_block ().work_version ());
}

TEST (block_uniquer, null)
{
	lumex::block_uniquer uniquer;
...
TEST (block_uniquer, single)
{
	...
	auto block1 = builder
				  ...
				  .work (0)
				  .build ();
	auto block2 (std::make_shared<lumex::state_block> (*block1));
	ASSERT_NE (block1, block2);
	ASSERT_EQ (*block1, *block2);
	std::weak_ptr<lumex::state_block> block3 (block2);
...
TEST (block_uniquer, cleanup)
{
	...
	std::this_thread::sleep_for (lumex::block_uniquer::cleanup_cutoff);
...
TEST (block_builder, zeroed_state_block)
{
	...
	ASSERT_FALSE (lumex::validate_message (key.pub, zero_block_build->hash (), zero_block_build->signature));
}
...
TEST (block_builder, state_missing_rep)
{
	...
	auto block = builder
				 ...
				 .work (0)
				 .build (ec);
	ASSERT_EQ (ec, lumex::error_common::missing_representative);
}
...
TEST (block_builder, state_equality)
{
	...
	lumex::state_block block1 (key1.pub, 1, key2.pub, 2, 4, key1.prv, key1.pub, 5);
...
TEST (block_builder, state_errors)
{
	...
	ASSERT_EQ (ec, lumex::error_common::bad_account_number);
...
TEST (block_builder, open_equality)
{
	...
	// With constructor
	lumex::keypair key1, key2;
	lumex::open_block block1 (1, key1.pub, key2.pub, key1.prv, key1.pub, 5);
...
TEST (block_builder, change_equality)
{
	...
	// With constructor
	lumex::keypair key1, key2;
	lumex::change_block block1 (1, key1.pub, key1.prv, key1.pub, 5);
...
TEST (block_builder, send)
{
	...
	ASSERT_FALSE (block->source_field ());
	ASSERT_EQ (block->destination_field ().value ().to_account (), "lumex_1gys8r4crpxhp94n4uho5cshaho81na6454qni5gu9n53gksoyy1wcd4udyb");
	ASSERT_FALSE (block->link_field ());
}
...
TEST (block_builder, send_equality)
{
	...
	// With constructor
	lumex::keypair key1, key2;
	lumex::send_block block1 (1, key1.pub, 2, key1.prv, key1.pub, 5);
...
TEST (block_builder, receive_equality)
{
	...
	// With constructor
	lumex::keypair key1;
	lumex::receive_block block1 (1, 2, key1.prv, key1.pub, 5);
