...
TEST (random_pool, multithreading)
{
	...
	for (auto i = 0; i < 100; ++i)
	{
		threads.emplace_back ([] () {
			lumex::uint256_union number;
			lumex::random_pool::generate_block (number.bytes.data (), number.bytes.size ());
		});
	}
	for (auto & i : threads)
	{
...
TEST (random_pool, generate_word64)
{
	...
	{
		auto random = lumex::random_pool::generate_word64 (1, 9);
...
TEST (random_pool, generate_word64_big_number)
{
	...
	auto big_random = lumex::random_pool::generate_word64 (min, max);
