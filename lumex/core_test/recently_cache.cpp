...
namespace
{
std::shared_ptr<lumex::block> make_test_block ()
{
...
lumex::election_status make_test_election_status (std::chrono::milliseconds duration = std::chrono::milliseconds (100))
{
...
{
	lumex::recently_confirmed_cache cache (10);
	ASSERT_EQ (0, cache.size ());
}

TEST (recently_confirmed_cache, put)
{
	lumex::recently_confirmed_cache cache (3);
	std::vector<lumex::qualified_root> roots;
	std::vector<lumex::block_hash> hashes;

	// Add entries and test size limit
	for (int i = 0; i < 5; ++i)
	{
		auto root = lumex::test::random_qualified_root ();
		auto hash = lumex::test::random_hash ();
...
{
	lumex::recently_confirmed_cache cache (1000);
	for (int i = 1; i <= 100; ++i)
	{
...
{
	lumex::recently_cemented_cache cache (10);
	ASSERT_EQ (0, cache.size ());
}

TEST (recently_cemented_cache, put)
{
	lumex::recently_cemented_cache cache (3);
	std::vector<lumex::election_status> statuses;
