...
TEST (priority_scheduler, activate_multiple_buckets)
{
	...
	lumex::keypair account_a, account_b;
	...
	auto send_to_a = builder.make_block ()
					 ...
					 .build ();
	ASSERT_EQ (lumex::block_status::progress, node.process (send_to_a));
	...
	auto open_a = builder.make_block ()
				  ...
				  .build ();
	ASSERT_EQ (lumex::block_status::progress, node.process (open_a));
	...
	auto send_to_b = builder.make_block ()
					 ...
					 .build ();
	ASSERT_EQ (lumex::block_status::progress, node.process (send_to_b));
	...
	auto open_b = builder.make_block ()
				  ...
				  .build ();
	ASSERT_EQ (lumex::block_status::progress, node.process (open_b));

	// Cement the initial opens so their successors are eligible for activation
	lumex::test::confirm (node, { send_to_a, open_a, send_to_b, open_b });
	...
	auto next_a = builder.make_block ()
				  ...
				  .build ();
	ASSERT_EQ (lumex::block_status::progress, node.process (next_a));
	...
	auto next_b = builder.make_block ()
				  ...
				  .build ();
	ASSERT_EQ (lumex::block_status::progress, node.process (next_b));
...
TEST (priority_scheduler, queue_activations)
{
	...
	lumex::state_block_builder builder;
	lumex::keypair keypair1, keypair2;
	...
	lumex::test::confirm (node, { send1, open1, send2, open2 });
	...
	auto next_block1 = builder.make_block ()
					   ...
					   .build ();
	ASSERT_EQ (lumex::block_status::progress, node.process (next_block1));
	...
	auto next_block2 = builder.make_block ()
					   ...
					   .build ();
	ASSERT_EQ (lumex::block_status::progress, node.process (next_block2));
	...
	std::shared_ptr<lumex::election> election1;
...
TEST (priority_scheduler, cancel_worst_election)
{
	...
	lumex::state_block_builder builder;
	lumex::keypair account_c, account_d;
	...
	auto send_to_c = builder.make_block ()
					 ...
					 .work (*system.work.generate (lumex::dev::genesis->hash ()))
					 .build ();
	ASSERT_EQ (lumex::block_status::progress, node.process (send_to_c));
	auto open_c = builder.make_block ()
				  ...
				  .build ();
	ASSERT_EQ (lumex::block_status::progress, node.process (open_c));
	...
	auto send_to_d = builder.make_block ()
					 ...
					 .build ();
	ASSERT_EQ (lumex::block_status::progress, node.process (send_to_d));
	auto open_d = builder.make_block ()
				  ...
				  .build ();
	ASSERT_EQ (lumex::block_status::progress, node.process (open_d));

	// Cement the opens
	lumex::test::confirm (node, { send_to_c, open_c, send_to_d, open_d });
	...
	auto worse_priority_block = builder.make_block ()
								...
								.build ();
	ASSERT_EQ (lumex::block_status::progress, node.process (worse_priority_block));
	...
	auto better_priority_block = builder.make_block ()
								 ...
								 .build ();
	ASSERT_EQ (lumex::block_status::progress, node.process (better_priority_block));
	...
	constexpr lumex::bucket_index bucket = 7;
	// Higher priority timestamp = worse priority according to ordering (lower goes first)
	lumex::priority_timestamp worse_priority = std::numeric_limits<lumex::priority_timestamp>::max ();
	lumex::priority_timestamp better_priority = 0;
...
TEST (priority_scheduler, activate_successors)
{
	...
	lumex::state_block_builder builder;
	lumex::keypair destination_account;
	...
	auto send_to_destination = builder.make_block ()
							   ...
							   .work (*system.work.generate (lumex::dev::genesis->hash ()))
							   .build ();
	ASSERT_EQ (lumex::block_status::progress, node.process (send_to_destination));
	lumex::test::confirm (node, { send_to_destination });
	...
	auto open_destination = builder.make_block ()
							...
							.build ();
	ASSERT_EQ (lumex::block_status::progress, node.process (open_destination));
	...
	auto next_genesis_block = builder.make_block ()
							  ...
							  .build ();
	ASSERT_EQ (lumex::block_status::progress, node.process (next_genesis_block));
...
TEST (priority_scheduler, stress_test)
{
	...
	auto blocks = lumex::test::setup_independent_blocks (system, node, num_blocks);

	// Worker pool for async activation - decouples notification from activation
	lumex::thread_pool workers{ 1, lumex::thread_role::name::unknown };
	lumex::test::start_stop_guard guard{ workers };
