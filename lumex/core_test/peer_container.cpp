...
TEST (peer_container, empty_peers)
{
	...
	lumex::network & network (system.nodes[0]->network);
...
TEST (peer_container, no_self_incoming)
{
	...
	auto error = system.poll_until_true (2s, [&node] {
		auto result = node.network.tcp_channels.find_channel (lumex::transport::map_endpoint_to_tcp (node.network.endpoint ()));
...
// Tests the function network not_a_peer function used by the lumex::transport::tcp_channels.insert ()
TEST (peer_container, reserved_ip_is_not_a_peer)
{
	lumex::test::system system{ 1 };
	auto not_a_peer = [&node = system.nodes[0]] (lumex::endpoint endpoint_a) -> bool {
	...
	ASSERT_TRUE (not_a_peer (lumex::transport::map_endpoint_to_v6 (lumex::endpoint (boost::asio::ip::address (boost::asio::ip::address_v4 (0x00000001)), 10000))));
	ASSERT_TRUE (not_a_peer (lumex::transport::map_endpoint_to_v6 (lumex::endpoint (boost::asio::ip::address (boost::asio::ip::address_v4 (0xc0000201)), 10000))));
	ASSERT_TRUE (not_a_peer (lumex::transport::map_endpoint_to_v6 (lumex::endpoint (boost::asio::ip::address (boost::asio::ip::address_v4 (0xc6336401)), 10000))));
	ASSERT_TRUE (not_a_peer (lumex::transport::map_endpoint_to_v6 (lumex::endpoint (boost::asio::ip::address (boost::asio::ip::address_v4 (0xcb007101)), 10000))));
	ASSERT_TRUE (not_a_peer (lumex::transport::map_endpoint_to_v6 (lumex::endpoint (boost::asio::ip::address (boost::asio::ip::address_v4 (0xe9fc0001)), 10000))));
	ASSERT_TRUE (not_a_peer (lumex::transport::map_endpoint_to_v6 (lumex::endpoint (boost::asio::ip::address (boost::asio::ip::address_v4 (0xf0000001)), 10000))));
	ASSERT_TRUE (not_a_peer (lumex::transport::map_endpoint_to_v6 (lumex::endpoint (boost::asio::ip::address (boost::asio::ip::address_v4 (0xffffffff)), 10000))));

	// Test with a valid IP address
	ASSERT_FALSE (not_a_peer (lumex::transport::map_endpoint_to_v6 (lumex::endpoint (boost::asio::ip::address (boost::asio::ip::address_v4 (0x08080808)), 10000))));
}
...
TEST (peer_container, DISABLED_tcp_channel_cleanup_works)
{
	...
	auto outer_node1 = lumex::test::add_outer_node (system, config1, node_flags);
	auto config2 = config1;
	config2.network_params.network.keepalive_period = std::chrono::minutes (10);
	auto outer_node2 = lumex::test::add_outer_node (system, config2, node_flags);
	auto now = std::chrono::steady_clock::now ();
	auto channel1 = lumex::test::establish_tcp (system, node1, outer_node1->network.endpoint ());
	...
	auto channel2 = lumex::test::establish_tcp (system, node1, outer_node2->network.endpoint ());
...
TEST (channels, fill_random_clear)
{
	...
	std::fill (target.begin (), target.end (), lumex::endpoint (boost::asio::ip::address_v6::loopback (), 10000));
	system.nodes[0]->network.random_fill (target);
	ASSERT_TRUE (std::all_of (target.begin (), target.end (), [] (lumex::endpoint const & endpoint_a) { return endpoint_a == lumex::endpoint (boost::asio::ip::address_v6::any (), 0); }));
}
...
TEST (channels, fill_random_full)
{
	...
	for (int i = 0; i < 8; ++i)
	{
		...
		ASSERT_NE (nullptr, lumex::test::establish_tcp (system, *system.nodes[0], outer_node->network.endpoint ()));
	}
	ASSERT_TIMELY_EQ (5s, 8, system.nodes[0]->network.tcp_channels.size ());

	// create an array of 8 endpoints with a known filler value
	auto filler_endpoint = lumex::endpoint (boost::asio::ip::address_v6::loopback (), 10000);
	std::array<lumex::endpoint, 8> target;
	...
	auto is_filler = [&filler_endpoint] (lumex::endpoint const & endpoint_a) {
...
TEST (channels, fill_random_part)
{
	...
	std::fill (target.begin (), target.end (), lumex::endpoint (boost::asio::ip::address_v6::loopback (), 10000));
	system.nodes[0]->network.random_fill (target);
	ASSERT_TRUE (std::none_of (target.begin (), target.begin () + half, [] (lumex::endpoint const & endpoint_a) { return endpoint_a == lumex::endpoint (boost::asio::ip::address_v6::loopback (), 10000); }));
	ASSERT_TRUE (std::none_of (target.begin (), target.begin () + half, [] (lumex::endpoint const & endpoint_a) { return endpoint_a == lumex::endpoint (boost::asio::ip::address_v6::loopback (), 0); }));
	ASSERT_TRUE (std::all_of (target.begin () + half, target.end (), [] (lumex::endpoint const & endpoint_a) { return endpoint_a == lumex::endpoint (boost::asio::ip::address_v6::any (), 0); }));
}
...
TEST (peer_container, DISABLED_list_fanout)
{
	...
	auto add_peer = [&node, &system] () {
		auto outer_node = lumex::test::add_outer_node (system);
		auto channel = lumex::test::establish_tcp (system, *node, outer_node->network.endpoint ());
	};
...
TEST (peer_container, reachout)
{
	...
	auto outer_node1 = lumex::test::add_outer_node (system);
	ASSERT_NE (nullptr, lumex::test::establish_tcp (system, node1, outer_node1->network.endpoint ()));
	...
	auto outer_node2 = lumex::test::add_outer_node (system);
	...
	ASSERT_NE (nullptr, lumex::test::establish_tcp (system, node1, outer_node2->network.endpoint ()));
