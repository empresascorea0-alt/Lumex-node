...
namespace
{
lumex::wallet::wallets make_wallets (lumex::node & node)
{
	return lumex::wallet::wallets (node, node.wallets_backend, node.ledger, node.config, node.network_params, node.online_reps, node.network, node.stats, node.logger);
}
}
...
TEST (wallets, open_create)
{
	...
	auto id = lumex::random_wallet_id ();
...
TEST (wallets, create_from_json)
{
	...
	auto & node = *system.nodes[0];
	node.wallets.stop (); // Stop node wallets to avoid race condition with local wallets sharing same LMDB environment
	lumex::wallet_id id (1);
	std::string json;
	lumex::public_key account;
	{
		auto wallets = make_wallets (node);
		auto wallet = wallets.create (id);
		ASSERT_NE (nullptr, wallet);
...
TEST (wallets, DISABLED_reload)
{
	...
	{
		lumex::lock_guard<lumex::mutex> lock_wallet (node1.wallets.mutex);
		lumex::inactive_node node (node1.get_data_path (), lumex::inactive_node_flag_defaults ());
...
TEST (wallets, vote_minimum)
{
	...
	auto send1 = builder
				 ...
				 .balance (std::numeric_limits<lumex::uint128_t>::max () - node1.config.vote_minimum.number ())
				 .link (key1.pub)
	...
	auto send2 = builder
				 ...
				 .previous (send1->hash ())
				 .representative (lumex::dev::genesis_key.pub)
				 .balance (std::numeric_limits<lumex::uint128_t>::max () - 2 * node1.config.vote_minimum.number () + 1)
				 .link (key2.pub)
				 .sign (lumex::dev::genesis_key.prv, lumex::dev::genesis_key.pub)
	...
	auto open2 = builder
				 ...
				 .build ();
	ASSERT_EQ (lumex::block_status::progress, node1.process (open2));
	auto wallet (node1.wallets.items.begin ()->second);
	ASSERT_EQ (0, wallet->reps ().size ());
	ASSERT_TRUE (wallet->insert_adhoc (lumex::dev::genesis_key.prv));
...
TEST (wallets, rep_scan)
{
	...
	auto send = builder
				...
				.previous (lumex::dev::genesis->hash ())
				.representative (lumex::dev::genesis_key.pub)
				.balance (lumex::dev::constants.genesis_amount - node.config.vote_minimum.number ())
				.link (key.pub)
				.sign (lumex::dev::genesis_key.prv, lumex::dev::genesis_key.pub)
				.work (*system.work.generate (lumex::dev::genesis->hash ()))
				.build ();
	...
	node.stats.clear ();
	ASSERT_TIMELY (5s, node.stats.count (lumex::stat::type::wallet, lumex::stat::detail::loop_reps) > 1);
...
TEST (wallets, receivable_scan)
{
	...
	node.stats.clear ();
	ASSERT_TIMELY (5s, node.stats.count (lumex::stat::type::wallet, lumex::stat::detail::loop_receivable) > 1);

	// Verify the background scan automatically created the receive block
	ASSERT_TIMELY_EQ (5s, node.balance (lumex::dev::genesis_key.pub), lumex::dev::constants.genesis_amount);
...
TEST (wallets, signer_none)
{
	...
	int called = 0;
	sign ([&] (lumex::public_key const &, lumex::raw_key const &) {
		++called;
	});
	ASSERT_EQ (0, called);
}
...
TEST (wallets, signer_multiple)
{
	...
	system.wallet (0)->send_sync (lumex::dev::genesis_key.pub, rep2.pub, amount);
	...
	std::set<lumex::account> accounts;
	sign ([&] (lumex::public_key const & pub, lumex::raw_key const &) {
		accounts.insert (pub);
	});

	ASSERT_EQ (2, accounts.size ());
	ASSERT_TRUE (accounts.count (lumex::dev::genesis_key.pub));
...
TEST (wallets, signer_below_weight)
{
	lumex::test::system system;
	lumex::node_config config = system.default_config ();
	config.vote_minimum = lumex::Klumex_ratio; // 1000 lumex
	auto & node = *system.add_node (config);

	lumex::keypair zero_weight;
	lumex::keypair below_minimum;
	system.wallet (0)->insert_adhoc (lumex::dev::genesis_key.prv);
	...
	auto const amount = lumex::Klumex_ratio - 1;
	system.wallet (0)->send_sync (lumex::dev::genesis_key.pub, below_minimum.pub, amount);
...
TEST (wallets, rep_keys_cache_rekey_preserves)
{
	...
	lumex::public_key pub_before;
	lumex::raw_key prv_before;
	{
		auto sign = node.wallets.signer ();
		sign ([&] (lumex::public_key const & pub, lumex::raw_key const & prv) {
			pub_before = pub;
			prv_before = prv;
		});
	}
	ASSERT_EQ (lumex::dev::genesis_key.pub, pub_before);
	...
	lumex::public_key pub_after;
	lumex::raw_key prv_after;
	{
		auto sign = node.wallets.signer ();
		sign ([&] (lumex::public_key const & pub, lumex::raw_key const & prv) {
			pub_after = pub;
	...
	ASSERT_EQ (lumex::dev::genesis_key.prv, prv_after);
}
...
TEST (wallets, rep_keys_cache_deterministic_insert_index)
{
	...
	lumex::raw_key seed;
	seed = 1;
	auto det_prv = lumex::deterministic_key (seed, 0);
	auto det_pub = lumex::pub_key (det_prv);
...
TEST (wallets, rep_keys_cache_key_correctness)
{
	...
	auto sign = node.wallets.signer ();
	std::map<lumex::public_key, lumex::raw_key> key_pairs;
	sign ([&] (lumex::public_key const & pub, lumex::raw_key const & prv) {
		key_pairs[pub] = prv;
	});

	ASSERT_EQ (1, key_pairs.size ());
	ASSERT_TRUE (key_pairs.count (lumex::dev::genesis_key.pub));

	// Verify the cached private key matches the original
	ASSERT_EQ (lumex::dev::genesis_key.prv, key_pairs[lumex::dev::genesis_key.pub]);

	// Verify the private key derives back to the correct public key
	ASSERT_EQ (lumex::dev::genesis_key.pub, lumex::pub_key (key_pairs[lumex::dev::genesis_key.pub]));
}
...
TEST (wallets, rep_keys_cache_weight_lost)
{
	...
	auto send_back = builder
					 ...
					 .build ();
	ASSERT_EQ (lumex::block_status::progress, node.process (send_back));
