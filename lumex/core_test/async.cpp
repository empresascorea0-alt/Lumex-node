...
public:
	...
	lumex::logger logger;
	lumex::thread_runner runner{ io_ctx, logger, 1 };
	lumex::async::strand strand{ io_ctx->get_executor () };
};
}
...
TEST (async, sleep)
{
	...
	ctx.strand,
	[&] () -> asio::awaitable<void> {
		co_await lumex::async::sleep_for (500ms);
	},
	asio::use_future);
...
TEST (async, cancellation)
{
	...
	lumex::async::cancellation cancellation{ ctx.strand };

	auto fut = asio::co_spawn (
	ctx.strand,
	[&] () -> asio::awaitable<void> {
		co_await lumex::async::sleep_for (10s);
	},
...
TEST (async, task)
{
	...
	lumex::async::task task{ ctx.strand };

	// Default state, empty task
	ASSERT_FALSE (task.joinable ());
	ASSERT_FALSE (task.running ());

	task = lumex::async::task (ctx.strand, [&] () -> asio::awaitable<void> {
		co_await lumex::async::sleep_for (500ms);
	});
...
TEST (async, task_cancel)
{
	...
	lumex::async::task task = lumex::async::task (ctx.strand, [&] () -> asio::awaitable<void> {
		co_await lumex::async::sleep_for (10s);
	});
