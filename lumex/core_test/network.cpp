...
{
	lumex::test::system system{};
	auto const port = lumex::test::speculatively_choose_a_free_tcp_bind_port ();
	ASSERT_NE (port, 0);
	auto const node = system.add_node ([&] { lumex::node_config c; c.peering_port = port; return c; }());
...
TEST (network, send_node_id_handshake_tcp)
{
	...
	auto initial (node0->stats.count (lumex::stat::type::message, lumex::stat::detail::node_id_handshake, lumex::stat::dir::in));
	auto initial_node1 (node1->stats.count (lumex::stat::type::message, lumex::stat::detail::node_id_handshake, lumex::stat::dir::in));
	auto initial_keepalive (node0->stats.count (lumex::stat::type::message, lumex::stat::detail::keepalive, lumex::stat::dir::in));
	std::weak_ptr<lumex::node> node_w (node0);
	node0->network.tcp_channels.start_tcp (node1->network.endpoint ());
	ASSERT_EQ (0, node0->network.size ());
	ASSERT_EQ (0, node1->network.size ());
	ASSERT_TIMELY (10s, node0->stats.count (lumex::stat::type::tcp_server_message, lumex::stat::detail::node_id_handshake, lumex::stat::dir::in) >= initial + 2);
	ASSERT_TIMELY (5s, node1->stats.count (lumex::stat::type::tcp_server_message, lumex::stat::detail::node_id_handshake, lumex::stat::dir::in) >= initial_node1 + 1);
	ASSERT_TIMELY (5s, node0->stats.count (lumex::stat::type::message, lumex::stat::detail::keepalive, lumex::stat::dir::in) >= initial_keepalive + 2);
	ASSERT_TIMELY (5s, node1->stats.count (lumex::stat::type::message, lumex::stat::detail::keepalive, lumex::stat::dir::in) >= initial_keepalive + 2);
...
TEST (network, last_contacted)
{
	...
	lumex::node_config node1_config = system.default_config ();
	node1_config.tcp->max_inbound_connections = 0; // Prevent ephemeral node1->node0 channel repacement with incoming connection
	auto node1 (std::make_shared<lumex::node> (lumex::unique_path (), node1_config, system.work, lumex::node_flags{}));
	node1->start ();
	system.nodes.push_back (node1);

	auto channel1 = lumex::test::establish_tcp (system, *node1, node0->network.endpoint ());
...

	{
		...
		std::shared_ptr<lumex::transport::tcp_socket> sock0 = channel0->socket;
		std::shared_ptr<lumex::transport::tcp_socket> sock1 = channel1->socket;
	...
	auto keepalive_count = node0->stats.count (lumex::stat::type::message, lumex::stat::detail::keepalive, lumex::stat::dir::in);
	...
	ASSERT_TIMELY (3s, node0->stats.count (lumex::stat::type::message, lumex::stat::detail::keepalive, lumex::stat::dir::in) >= keepalive_count + 3);
...
TEST (network, multi_keepalive)
{
	...
	ASSERT_TIMELY (10s, node0->network.size () == 1 && node0->stats.count (lumex::stat::type::message, lumex::stat::detail::keepalive) >= 1);
	...
	ASSERT_TIMELY (10s, node1->network.size () == 2 && node0->network.size () == 2 && node2->network.size () == 2 && node0->stats.count (lumex::stat::type::message, lumex::stat::detail::keepalive) >= 2);
}
...
TEST (network, send_discarded_publish)
{
				 ...
				 .build ();
	{
		...
		auto sent = node1.network.flood_block (block, lumex::transport::traffic_type::test);
		ASSERT_EQ (1, sent);
		ASSERT_EQ (lumex::dev::genesis->hash (), node1.ledger.any.account_head (transaction, lumex::dev::genesis_key.pub));
		ASSERT_EQ (lumex::dev::genesis->hash (), node2.latest (lumex::dev::genesis_key.pub));
	}
	ASSERT_TIMELY (10s, node2.stats.count (lumex::stat::type::message, lumex::stat::detail::publish, lumex::stat::dir::in) != 0);
	auto transaction = node1.ledger.tx_begin_read ();
	ASSERT_EQ (lumex::dev::genesis->hash (), node1.ledger.any.account_head (transaction, lumex::dev::genesis_key.pub));
	ASSERT_EQ (lumex::dev::genesis->hash (), node2.latest (lumex::dev::genesis_key.pub));
}
...
TEST (network, send_valid_confirm_ack)
{
	...
	lumex::block_hash latest1 (node1.latest (lumex::dev::genesis_key.pub));
	lumex::block_builder builder;
	auto block2 = builder
				  .send ()
				  .previous (latest1)
				  ...
				  .build ();
	lumex::block_hash latest2 (node2.latest (lumex::dev::genesis_key.pub));
	node1.process_active (std::make_shared<lumex::send_block> (*block2));
	// Keep polling until latest block changes
	ASSERT_TIMELY (10s, node2.latest (lumex::dev::genesis_key.pub) != latest2);
	// Make sure the balance has decreased after processing the block.
	ASSERT_EQ (50, node2.balance (lumex::dev::genesis_key.pub));
}
...
TEST (network, send_valid_publish)
{
	...
	lumex::block_hash latest2 (node2.latest (lumex::dev::genesis_key.pub));
	node2.process_active (std::make_shared<lumex::send_block> (*block2));
	ASSERT_TIMELY (10s, node1.stats.count (lumex::stat::type::message, lumex::stat::detail::publish, lumex::stat::dir::in) != 0);
...
TEST (network, send_insufficient_work)
{
	...
	lumex::messages::publish publish{ lumex::dev::network_params.network, block };
	tcp_channel->send (publish, lumex::transport::traffic_type::test);

	ASSERT_TIMELY_EQ (5s, 1, node2.stats.count (lumex::stat::type::tcp_server_message_error, lumex::stat::detail::insufficient_work));
}
...
TEST (receivable_processor, confirm_insufficient_pos)
{
	...
	ASSERT_EQ (lumex::block_status::progress, node1.process (block1));
	auto election = lumex::test::start_election (system, node1, block1->hash ());
	lumex::keypair key1;
	auto vote = lumex::test::make_final_vote (key1, { block1 });
	lumex::messages::confirm_ack con1{ lumex::dev::network_params.network, vote };
	auto channel1 = std::make_shared<lumex::transport::inproc::channel> (node1, node1);
...
TEST (receivable_processor, confirm_sufficient_pos)
{
	...
	ASSERT_EQ (lumex::block_status::progress, node1.process (block1));
	auto election = lumex::test::start_election (system, node1, block1->hash ());
	auto vote = lumex::test::make_final_vote (lumex::dev::genesis_key, { block1 });
	lumex::messages::confirm_ack con1{ lumex::dev::network_params.network, vote };
	auto channel1 = std::make_shared<lumex::transport::inproc::channel> (node1, node1);
	ASSERT_EQ (1, election->votes ().size ());
	node1.inbound (con1, channel1);
...
TEST (receivable_processor, send_with_receive)
{
	...
	auto & node1 (*system.nodes[0]);
	auto & node2 (*system.nodes[1]);
	auto amount (std::numeric_limits<lumex::uint128_t>::max ());
	lumex::keypair key2;
	system.wallet (0)->insert_adhoc (lumex::dev::genesis_key.prv);
	lumex::block_hash latest1 (node1.latest (lumex::dev::genesis_key.pub));
	lumex::block_builder builder;
	auto block1 = builder
				  ...
				  .work (*system.work.generate (latest1))
				  .build ();
	ASSERT_EQ (amount, node1.balance (lumex::dev::genesis_key.pub));
	ASSERT_EQ (0, node1.balance (key2.pub));
	ASSERT_EQ (amount, node2.balance (lumex::dev::genesis_key.pub));
	ASSERT_EQ (0, node2.balance (key2.pub));
	node1.process_active (block1);
	ASSERT_TIMELY (5s, lumex::test::exists (node1, { block1 }));
	node2.process_active (block1);
	ASSERT_TIMELY (5s, lumex::test::exists (node2, { block1 }));
	ASSERT_EQ (amount - node1.config.receive_minimum.number (), node1.balance (lumex::dev::genesis_key.pub));
	ASSERT_EQ (0, node1.balance (key2.pub));
	ASSERT_EQ (amount - node1.config.receive_minimum.number (), node2.balance (lumex::dev::genesis_key.pub));
	ASSERT_EQ (0, node2.balance (key2.pub));
	system.wallet (1)->insert_adhoc (key2.prv);
...
TEST (network, receive_weight_change)
{
	...
	ASSERT_TIMELY (10s, std::all_of (system.nodes.begin (), system.nodes.end (), [&] (std::shared_ptr<lumex::node> const & node_a) { return node_a->weight (key2.pub) == system.nodes[0]->config.receive_minimum.number (); }));
}
...
TEST (parse_endpoint, valid)
{
	...
	ASSERT_FALSE (lumex::parse_endpoint (string, endpoint));
...
TEST (parse_endpoint, invalid_port)
{
	...
	lumex::endpoint endpoint;
	ASSERT_TRUE (lumex::parse_endpoint (string, endpoint));
}
...
TEST (network, ipv6)
{
	...
	lumex::endpoint endpoint1 (address, 16384);
	std::vector<uint8_t> bytes1;
	{
		lumex::vectorstream stream (bytes1);
		lumex::write (stream, address.to_bytes ());
	}
	...
	lumex::bufferstream stream (bytes1.data (), bytes1.size ());
	auto error (lumex::try_read (stream, bytes2));
	ASSERT_FALSE (error);
	lumex::endpoint endpoint2 (boost::asio::ip::address_v6 (bytes2), 16384);
	ASSERT_EQ (endpoint1, endpoint2);
}

TEST (network, ipv6_from_ipv4)
{
	lumex::endpoint endpoint1 (boost::asio::ip::address_v4::loopback (), 16000);
	ASSERT_TRUE (endpoint1.address ().is_v4 ());
	lumex::endpoint endpoint2 (boost::asio::ip::make_address_v6 (boost::asio::ip::v4_mapped, endpoint1.address ().to_v4 ()), 16000);
...
TEST (network, peer_max_tcp_attempts)
{
	...
	for (auto i = 0; i < node_config.network->max_peers_per_ip; ++i)
	{
		...
		lumex::node_config temp_config = system.default_config ();
	...
	ASSERT_FALSE (node->network.tcp_channels.track_reachout (lumex::endpoint (node->network.endpoint ().address (), system.get_available_port ())));
	ASSERT_LE (1, node->stats.count (lumex::stat::type::tcp, lumex::stat::detail::max_per_ip, lumex::stat::dir::out));
}
...
TEST (network, peer_max_tcp_attempts_subnetwork)
{
	...
	for (auto i (0); i < node->config.network->max_peers_per_subnetwork; ++i)
	{
		...
		lumex::endpoint endpoint (address, system.get_available_port ());
		ASSERT_TRUE (node->network.tcp_channels.track_reachout (endpoint));
	}

	ASSERT_EQ (0, node->network.size ());
	ASSERT_EQ (0, node->stats.count (lumex::stat::type::tcp, lumex::stat::detail::max_per_subnetwork, lumex::stat::dir::out));
	ASSERT_FALSE (node->network.tcp_channels.track_reachout (lumex::endpoint (boost::asio::ip::make_address_v6 ("::ffff:127.0.0.1"), system.get_available_port ())));
	ASSERT_EQ (1, node->stats.count (lumex::stat::type::tcp, lumex::stat::detail::max_per_subnetwork, lumex::stat::dir::out));
}

namespace
{
// Skip the first 8 bytes of the message header, which is the common header for all messages
std::vector<uint8_t> message_payload_to_bytes (lumex::messages::message const & message)
{
	...
	debug_assert (bytes.size () > lumex::messages::message_header::size);
	return std::vector<uint8_t> (bytes.begin () + lumex::messages::message_header::size, bytes.end ());
}
}
...
TEST (network, duplicate_detection)
{
	...
	lumex::messages::publish publish{ lumex::dev::network_params.network, lumex::dev::genesis };

	ASSERT_EQ (0, node1.stats.count (lumex::stat::type::filter, lumex::stat::detail::duplicate_publish_message));
	...
	ASSERT_EQ (0, node1.stats.count (lumex::stat::type::filter, lumex::stat::detail::duplicate_publish_message));
	tcp_channel->send (publish, lumex::transport::traffic_type::test);
	ASSERT_ALWAYS_EQ (100ms, node1.stats.count (lumex::stat::type::filter, lumex::stat::detail::duplicate_publish_message), 0);
	tcp_channel->send (publish, lumex::transport::traffic_type::test);
	ASSERT_TIMELY_EQ (2s, node1.stats.count (lumex::stat::type::filter, lumex::stat::detail::duplicate_publish_message), 1);
}
...
TEST (network, duplicate_revert_publish)
{
	...
	auto other_node (std::make_shared<lumex::node> (system.get_available_port (), lumex::unique_path (), system.work, lumex::node_flags{}));
	other_node->start ();
	system.nodes.push_back (other_node);
	auto channel = lumex::test::establish_tcp (system, *other_node, node.network.endpoint ());
	ASSERT_NE (nullptr, channel);
	ASSERT_EQ (0, publish.digest);
	node.inbound (publish, lumex::test::fake_channel (node));
	ASSERT_TRUE (node.network.filter.apply (bytes.data (), bytes.size ()));
...
TEST (network, duplicate_vote_detection)
{
	...
	auto vote = lumex::test::make_vote (lumex::dev::genesis_key, { lumex::dev::genesis->hash () });
	lumex::messages::confirm_ack message{ lumex::dev::network_params.network, vote };

	ASSERT_EQ (0, node1.stats.count (lumex::stat::type::filter, lumex::stat::detail::duplicate_confirm_ack_message));
	...
	ASSERT_EQ (0, node1.stats.count (lumex::stat::type::filter, lumex::stat::detail::duplicate_confirm_ack_message));
	tcp_channel->send (message, lumex::transport::traffic_type::test);
	ASSERT_ALWAYS_EQ (100ms, node1.stats.count (lumex::stat::type::filter, lumex::stat::detail::duplicate_confirm_ack_message), 0);
	tcp_channel->send (message, lumex::transport::traffic_type::test);
	ASSERT_TIMELY_EQ (2s, node1.stats.count (lumex::stat::type::filter, lumex::stat::detail::duplicate_confirm_ack_message), 1);
}
...
TEST (network, duplicate_revert_vote)
{
	...
	auto vote1 = lumex::test::make_vote (lumex::dev::genesis_key, { lumex::dev::genesis->hash () }, 1);
	lumex::messages::confirm_ack message1{ lumex::dev::network_params.network, vote1 };
	auto bytes1 = message_payload_to_bytes (message1);

	auto vote2 = lumex::test::make_vote (lumex::dev::genesis_key, { lumex::dev::genesis->hash () }, 2);
	lumex::messages::confirm_ack message2{ lumex::dev::network_params.network, vote2 };
	...
	tcp_channel->send (message1, lumex::transport::traffic_type::test);
	ASSERT_ALWAYS_EQ (100ms, node1.stats.count (lumex::stat::type::filter, lumex::stat::detail::duplicate_confirm_ack_message), 0);
	ASSERT_TIMELY (5s, node1.network.filter.check (bytes1.data (), bytes1.size ()));

	// Second vote should get dropped from processor queue
	tcp_channel->send (message2, lumex::transport::traffic_type::test);
	ASSERT_ALWAYS_EQ (100ms, node1.stats.count (lumex::stat::type::filter, lumex::stat::detail::duplicate_confirm_ack_message), 0);
...
{
	lumex::test::system system;
	lumex::messages::publish message{ lumex::dev::network_params.network, lumex::dev::genesis };
	...
	auto & node = *system.add_node (node_config);
	lumex::transport::inproc::channel channel1{ node, node };
	lumex::transport::inproc::channel channel2{ node, node };
	// Send droppable messages
	for (auto i = 0; i < message_limit; i += 2) // number of channels
	{
		channel1.send (message, lumex::transport::traffic_type::test);
		channel2.send (message, lumex::transport::traffic_type::test);
	}
	// Only sent messages below limit, so we don't expect any drops
	ASSERT_TIMELY_EQ (1s, 0, node.stats.count (lumex::stat::type::message_drop, lumex::stat::detail::publish, lumex::stat::dir::out));

	// Send droppable message; drop stats should increase by one now
	channel1.send (message, lumex::transport::traffic_type::test);
	ASSERT_TIMELY_EQ (1s, 1, node.stats.count (lumex::stat::type::message_drop, lumex::stat::detail::publish, lumex::stat::dir::out));
...
TEST (network, DISABLED_bandwidth_limiter_2_messages)
{
	...
	channel1.send (message, lumex::transport::traffic_type::test);
	channel2.send (message, lumex::transport::traffic_type::test);
	ASSERT_TIMELY_EQ (1s, 2, node.stats.count (lumex::stat::type::message_drop, lumex::stat::detail::publish, lumex::stat::dir::out));
}
...
TEST (peer_exclusion, validate)
{
	...
	lumex::peer_exclusion excluded_peers{ max_size };

	for (auto i = 0; i < max_size + 1; ++i)
	{
		lumex::tcp_endpoint endpoint{ boost::asio::ip::make_address_v6 (boost::asio::ip::v4_mapped, boost::asio::ip::address_v4 (i)), 0 };
	...
	lumex::tcp_endpoint oldest{ boost::asio::ip::make_address_v6 (boost::asio::ip::v4_mapped, boost::asio::ip::address_v4 (0x0)), 0 };
	...
	lumex::tcp_endpoint first{ boost::asio::ip::make_address_v6 (boost::asio::ip::v4_mapped, boost::asio::ip::address_v4 (0x1)), 0 };
	ASSERT_NE (excluded_peers.score (first), 0);

	lumex::tcp_endpoint second{ boost::asio::ip::make_address_v6 (boost::asio::ip::v4_mapped, boost::asio::ip::address_v4 (0x2)), 0 };
...
TEST (network, tcp_no_accept_excluded_peers)
{
	...
	node1->start ();
	system.nodes.push_back (node1);
	auto endpoint1_tcp (lumex::transport::map_endpoint_to_tcp (node1->network.endpoint ()));
	while (!node0->network.excluded_peers.check (endpoint1_tcp))
	{
		node0->network.excluded_peers.add (endpoint1_tcp);
	}
	ASSERT_EQ (0, node0->stats.count (lumex::stat::type::tcp_listener_rejected, lumex::stat::detail::excluded));
	node1->network.merge_peer (node0->network.endpoint ());
	ASSERT_TIMELY (5s, node0->stats.count (lumex::stat::type::tcp_listener_rejected, lumex::stat::detail::excluded) >= 1);
...
TEST (network, cleanup_purge)
{
	...
	node1.network.cleanup (test_start);
	ASSERT_EQ (1, node1.network.size ());
	ASSERT_EQ (0, node1.stats.count (lumex::stat::type::tcp_channels_purge));

	node1.network.cleanup (std::chrono::steady_clock::now ());
	ASSERT_EQ (1, node1.stats.count (lumex::stat::type::tcp_channels_purge, lumex::stat::detail::idle));
}
...
TEST (network, loopback_channel)
{
	...
	auto & node1 = *system.nodes[0];
	auto & node2 = *system.nodes[1];
	lumex::transport::inproc::channel channel1 (node1, node1);
	ASSERT_EQ (channel1.get_type (), lumex::transport::transport_type::loopback);
	...
	lumex::transport::inproc::channel channel2 (node2, node2);
	++node1.network.port;
...
TEST (network, filter_invalid_network_bytes)
{
	...
	lumex::messages::keepalive keepalive{ lumex::dev::network_params.network };
	const_cast<lumex::network_type &> (keepalive.header.network) = lumex::network_type::invalid;
	channel->send (keepalive, lumex::transport::traffic_type::test);

	ASSERT_TIMELY_EQ (5s, 1, node1.stats.count (lumex::stat::type::tcp_server_message_error, lumex::stat::detail::invalid_network));
}
...
TEST (network, filter_invalid_version_using)
{
	...
	lumex::messages::keepalive keepalive{ lumex::dev::network_params.network };
	const_cast<uint8_t &> (keepalive.header.version_using) = lumex::dev::network_params.network.protocol_version_min - 1;
	channel->send (keepalive, lumex::transport::traffic_type::test);

	ASSERT_TIMELY_EQ (5s, 1, node1.stats.count (lumex::stat::type::tcp_server_message_error, lumex::stat::detail::outdated_version));
}
...
TEST (network, fill_keepalive_self)
{
	...
	auto get_keepalive = [&system] (lumex::node & node) {
		std::array<lumex::endpoint, 8> target;
...
TEST (network, purge_dead_channel)
{
	...
	node1.observers.socket_connected.add ([&] (auto const & socket) {
		system.logger.debug (lumex::log::type::test, "Connected socket: {}", lumex::streamed (socket));
	});
	...
	std::deque<std::shared_ptr<lumex::transport::tcp_socket>> sockets;
...
TEST (network, flood_vote)
{
	...
	auto vote = lumex::test::make_vote (lumex::dev::genesis_key, { lumex::dev::genesis->hash () });
	ASSERT_EQ (2, node.network.flood_vote (vote, lumex::transport::traffic_type::test));
	ASSERT_EQ (3, node.network.flood_vote_all (vote, lumex::transport::traffic_type::test));
...
TEST (network, disable_reachout)
{
	...
	ASSERT_EQ (0, node.stats.count (lumex::stat::type::network, lumex::stat::detail::reachout_live));
	ASSERT_EQ (0, node.stats.count (lumex::stat::type::network, lumex::stat::detail::reachout_cached));
	ASSERT_EQ (0, node.stats.count (lumex::stat::type::network, lumex::stat::detail::reachout_preconfigured));
}
