...
TEST (active_elections, confirm_election_by_request)
{
	...
	lumex::node_config node_config1;
	...
	ASSERT_TIMELY (5s, lumex::test::confirmed (node1, { send1 }));
	...
	node2.rep_crawler.force_add_rep (lumex::dev::genesis_key.pub, *peers.cbegin ());
	...
	ASSERT_TIMELY (5s, lumex::test::confirmed (node1, { send1 }));
	ASSERT_TIMELY (5s, lumex::test::confirmed (node2, { send1 }));
}
...
TEST (active_elections, confirm_frontier)
{
...

	{
		...
		ASSERT_TRUE (lumex::test::process (node1, { send->clone () }));
		lumex::test::confirm (node1.ledger, send->hash ());
	}
	...
	node_flags2.disable_rep_crawler = true;
	lumex::node_config node_config2;
	...
	std::shared_ptr<lumex::election> election2;
	...
	ASSERT_FALSE (peers.empty ());
	node2.rep_crawler.force_add_rep (lumex::dev::genesis_key.pub, *peers.begin ());

	ASSERT_TIMELY (5s, lumex::test::confirmed (node2, { send }));
...
TEST (active_elections, confirm_fork)
{
	...
	auto fork1 = builder.make_block ()
				 ...
				 .previous (lumex::dev::genesis->hash ())
				 .representative (lumex::dev::genesis_key.pub)
				 .balance (lumex::dev::constants.genesis_amount - 100) // –100 RAW
				 .link (dest.pub)
				 .sign (lumex::dev::genesis_key.prv, lumex::dev::genesis_key.pub)
				 .work (*system.work.generate (lumex::dev::genesis->hash ()))
				 .build ();
	...
	auto fork2 = builder.make_block ()
				 ...
				 .previous (lumex::dev::genesis->hash ())
				 .representative (lumex::dev::genesis_key.pub)
				 .balance (lumex::dev::constants.genesis_amount - 200) // –200 RAW
				 .link (dest.pub)
				 .sign (lumex::dev::genesis_key.prv, lumex::dev::genesis_key.pub)
				 .work (*system.work.generate (lumex::dev::genesis->hash ()))
				 .build ();
	...
	auto vote_fork2 = lumex::test::make_final_vote (lumex::dev::genesis_key, { fork2 });
	node.vote_processor.vote (vote_fork2,
	std::make_shared<lumex::transport::inproc::channel> (node, node));
...
TEST (active_elections, confirm_fork_cache)
{
	...
	auto vote_fork2 = lumex::test::make_final_vote (lumex::dev::genesis_key, { fork1 });
	node.vote_processor.vote (vote_fork2,
	std::make_shared<lumex::transport::inproc::channel> (node, node));
...
TEST (active_elections, DISABLED_keep_local)
{
	...
	auto const send1 = wallet.send_action (lumex::dev::genesis_key.pub, key1.pub, node.config.receive_minimum.number ());
	auto const send2 = wallet.send_action (lumex::dev::genesis_key.pub, key2.pub, node.config.receive_minimum.number ());
	auto const send3 = wallet.send_action (lumex::dev::genesis_key.pub, key3.pub, node.config.receive_minimum.number ());
	auto const send4 = wallet.send_action (lumex::dev::genesis_key.pub, key4.pub, node.config.receive_minimum.number ());
	auto const send5 = wallet.send_action (lumex::dev::genesis_key.pub, key5.pub, node.config.receive_minimum.number ());
	auto const send6 = wallet.send_action (lumex::dev::genesis_key.pub, key6.pub, node.config.receive_minimum.number ());
	...
	lumex::test::confirm (node.ledger, send6);
...
TEST (active_elections, cached_vote_basic)
{
	...
	ASSERT_EQ (1, node.stats.count (lumex::stat::type::election_vote, lumex::stat::detail::cache));
}
...
TEST (active_elections, cached_vote_non_final)
{
	...
	// Non-final vote
	auto vote = std::make_shared<lumex::vote> (lumex::dev::genesis_key.pub, lumex::dev::genesis_key.prv, 0, 0, std::vector<lumex::block_hash> (1, send->hash ()));
	node.vote_processor.vote (vote, std::make_shared<lumex::transport::inproc::channel> (node, node));
	ASSERT_TIMELY_EQ (5s, node.vote_cache.size (), 1);
	...
	ASSERT_TIMELY_EQ (5s, node.stats.count (lumex::stat::type::election_vote, lumex::stat::detail::cache), 1);
	ASSERT_TIMELY_EQ (5s, lumex::dev::constants.genesis_amount - 100, election->tally ().begin ()->first);
	ASSERT_FALSE (election->confirmed ());
}
...
TEST (active_elections, cached_vote_fork)
{
	...
	auto const latest = node.latest (lumex::dev::genesis_key.pub);
	lumex::keypair key{};
	...
	auto const vote = lumex::test::make_final_vote (lumex::dev::genesis_key, { send1 });
	node.vote_processor.vote (vote, std::make_shared<lumex::transport::inproc::channel> (node, node));
	ASSERT_TIMELY_EQ (5s, node.vote_cache.size (), 1);
...
TEST (active_elections, cached_vote_existing)
{
	...
	auto send = builder.send ()
				.previous (latest)
				.destination (key.pub)
				.balance (lumex::dev::constants.genesis_amount - 100 * lumex::Klumex_ratio)
				.sign (lumex::dev::genesis_key.prv, lumex::dev::genesis_key.pub)
				.work (*system.work.generate (latest))
				.build ();
	auto open = builder.state ()
				...
				.balance (100 * lumex::Klumex_ratio)
				.link (send->hash ())
				.sign (key.prv, key.pub)
				.work (*system.work.generate (key.pub))
				.build ();
	ASSERT_EQ (lumex::block_status::progress, node.ledger.process (node.ledger.tx_begin_write (), send));
	ASSERT_EQ (lumex::block_status::progress, node.ledger.process (node.ledger.tx_begin_write (), open));
	auto election = lumex::test::start_election (system, node, send->hash ());
	ASSERT_GT (node.weight (key.pub), node.minimum_principal_weight ());
	// Insert vote
	auto vote1 = lumex::test::make_vote (key, { send }, lumex::vote::timestamp_min * 1, 0);
	node.vote_processor.vote (vote1, std::make_shared<lumex::transport::inproc::channel> (node, node));
	ASSERT_TIMELY_EQ (5s, election->votes ().size (), 2);
	ASSERT_EQ (1, node.stats.count (lumex::stat::type::election, lumex::stat::detail::vote));
	auto last_vote1 (election->votes ()[key.pub]);
	ASSERT_EQ (send->hash (), last_vote1.hash);
	ASSERT_EQ (lumex::vote::timestamp_min * 1, last_vote1.timestamp);
	...
	ASSERT_EQ (0, node.stats.count (lumex::stat::type::election_vote, lumex::stat::detail::cache));
}
...
TEST (active_elections, cached_vote_multiple)
{
	...
	ASSERT_TRUE (lumex::test::process (node, { send1, send2, open }));
	ASSERT_TIMELY (5s, lumex::test::exists (node, { send1, send2, open }));

	// Process votes
	auto vote1 = lumex::test::make_vote (key1, { send1 }, 0, 0);
	node.vote_processor.vote (vote1, std::make_shared<lumex::transport::inproc::channel> (node, node));

	auto vote2 = lumex::test::make_vote (lumex::dev::genesis_key, { send1 }, 0, 0);
	node.vote_processor.vote (vote2, std::make_shared<lumex::transport::inproc::channel> (node, node));

	ASSERT_TIMELY_EQ (5s, node.vote_cache.find (send1->hash ()).size (), 2);
	ASSERT_EQ (1, node.vote_cache.size ());
	auto election = lumex::test::start_election (system, node, send1->hash ());
	ASSERT_TIMELY_EQ (5s, 3, election->votes ().size ()); // 2 votes and 1 default not_an_acount
	ASSERT_EQ (2, node.stats.count (lumex::stat::type::election_vote, lumex::stat::detail::cache));
}
...
TEST (active_elections, cached_vote_election_start)
{
	...
	auto amount = ((node.online_reps.trended () / 100) * node.config.hinted_scheduler->hinting_threshold_percent) / 2 + 1000 * lumex::Klumex_ratio;
	auto send1 = send_block_builder.make_block ()
				 .previous (latest)
				 .destination (key1.pub)
				 .balance (lumex::dev::constants.genesis_amount - amount)
				 .sign (lumex::dev::genesis_key.prv, lumex::dev::genesis_key.pub)
				 .work (*system.work.generate (latest))
				 .build ();
	auto send2 = send_block_builder.make_block ()
				 ...
				 .balance (lumex::dev::constants.genesis_amount - 2 * amount)
				 .sign (lumex::dev::genesis_key.prv, lumex::dev::genesis_key.pub)
				 .work (*system.work.generate (send1->hash ()))
				 .build ();
	...
	ASSERT_EQ (lumex::block_status::progress, node.process (open1));
	ASSERT_EQ (lumex::block_status::progress, node.process (open2));
	...
	// Inactive votes
	auto vote1 = lumex::test::make_vote (key1, { open1, open2, send4 });
	node.vote_processor.vote (vote1, std::make_shared<lumex::transport::inproc::channel> (node, node));
	...
	auto vote2 = lumex::test::make_vote (key2, { open1, open2, send4 });
	node.vote_processor.vote (vote2, std::make_shared<lumex::transport::inproc::channel> (node, node));
	...
	auto vote0 = lumex::test::make_final_vote (lumex::dev::genesis_key, { open1, open2, send4 });
	node.vote_processor.vote (vote0, std::make_shared<lumex::transport::inproc::channel> (node, node));
	...
	ASSERT_TIMELY (5s, lumex::test::confirmed (node, { send1, send2, open1, open2 }));
...
TEST (active_elections, vote_replays)
{
	...
	// send Klumex_ratio raw from genesis to key
	auto send1 = builder.make_block ()
				 .account (lumex::dev::genesis_key.pub)
				 .previous (lumex::dev::genesis->hash ())
				 .representative (lumex::dev::genesis_key.pub)
	...
	// create open block for key receing Klumex_ratio raw
	...
	lumex::test::process (node, { send1, open1 });
	ASSERT_TRUE (lumex::test::start_elections (system, node, { send1, open1 }));
	ASSERT_EQ (2, node.active.size ());

	// First vote is not a replay and should not confirm the election, second vote should be a replay
	auto vote_send1 = lumex::test::make_vote (lumex::dev::genesis_key, { send1 }, 0, 0);
	ASSERT_EQ (lumex::vote_code::vote, node.vote_router.vote (vote_send1).at (send1->hash ()));
	ASSERT_EQ (lumex::vote_code::replay, node.vote_router.vote (vote_send1).at (send1->hash ()));

	// Now send a final vote to actually confirm the election
	auto final_vote_send1 = lumex::test::make_final_vote (lumex::dev::genesis_key, { send1 });
	...
	ASSERT_EQ (lumex::vote_code::late, node.vote_router.vote (vote_send1).at (send1->hash ()));

	// Open new account
	auto vote_open1 = lumex::test::make_final_vote (lumex::dev::genesis_key, { open1 });
	ASSERT_EQ (lumex::vote_code::vote, node.vote_router.vote (vote_open1).at (open1->hash ()));
	ASSERT_TIMELY (5s, node.active.empty ());
	ASSERT_EQ (lumex::vote_code::late, node.vote_router.vote (vote_open1).at (open1->hash ()));
	ASSERT_EQ (lumex::Klumex_ratio, node.ledger.weight (key.pub));
	...
	ASSERT_TRUE (lumex::test::start_elections (system, node, { send2 }));
	ASSERT_EQ (1, node.active.size ());

	// vote2_send2 is a non final vote with little weight, vote1_send2 is the vote that confirms the election
	auto vote1_send2 = lumex::test::make_final_vote (lumex::dev::genesis_key, { send2 });
	auto vote2_send2 = lumex::test::make_vote (key, { send2 }, 0, 0);
	ASSERT_EQ (lumex::vote_code::vote, node.vote_router.vote (vote2_send2).at (send2->hash ())); // this vote cannot confirm the election
	ASSERT_EQ (1, node.active.size ());
	ASSERT_EQ (lumex::vote_code::replay, node.vote_router.vote (vote2_send2).at (send2->hash ())); // this vote cannot confirm the election
	ASSERT_EQ (1, node.active.size ());
	ASSERT_EQ (lumex::vote_code::vote, node.vote_router.vote (vote1_send2).at (send2->hash ())); // this vote confirms the election
	...
	ASSERT_TIMELY (5s, node.active.empty ());
	ASSERT_EQ (lumex::vote_code::late, node.vote_router.vote (vote1_send2).at (send2->hash ()));
	ASSERT_EQ (lumex::vote_code::late, node.vote_router.vote (vote2_send2).at (send2->hash ()));

	// Removing blocks as recently confirmed makes every vote indeterminate
	node.active.recently_confirmed.clear ();

	ASSERT_EQ (lumex::vote_code::indeterminate, node.vote_router.vote (vote_send1).at (send1->hash ()));
	ASSERT_EQ (lumex::vote_code::indeterminate, node.vote_router.vote (vote_open1).at (open1->hash ()));
	ASSERT_EQ (lumex::vote_code::indeterminate, node.vote_router.vote (vote1_send2).at (send2->hash ()));
	ASSERT_EQ (lumex::vote_code::indeterminate, node.vote_router.vote (vote2_send2).at (send2->hash ()));
}
...
TEST (active_elections, dropped_cleanup)
{
	...
	{
		lumex::vectorstream stream (block_bytes);
	...
	auto election = lumex::test::start_election (system, node, hash);
	...
	ASSERT_EQ (1, node.stats.count (lumex::stat::type::active_elections_dropped, lumex::stat::detail::manual));
	...
	election = lumex::test::start_election (system, node, hash);
	ASSERT_NE (nullptr, election);
...
TEST (active_elections, republish_winner)
{
	...
	ASSERT_TIMELY (5s, lumex::test::exists (node1, { send1 }));
	ASSERT_TIMELY_EQ (3s, node2.stats.count (lumex::stat::type::message, lumex::stat::detail::publish, lumex::stat::dir::in), 1);
	...
	ASSERT_EQ (1, node2.stats.count (lumex::stat::type::message, lumex::stat::detail::publish, lumex::stat::dir::in));
	...
	ASSERT_NE (nullptr, election);
	auto vote = lumex::test::make_final_vote (lumex::dev::genesis_key, { fork });
	node1.vote_processor.vote (vote, std::make_shared<lumex::transport::inproc::channel> (node1, node1));
	ASSERT_TIMELY (5s, election->confirmed ());
...
TEST (active_elections, fork_filter_cleanup)
{
	...
	{
		lumex::vectorstream stream{ send_block_bytes };
...
TEST (active_elections, fork_replacement_tally)
{
	...
	std::vector<lumex::keypair> keys (reps_count);
	...
	for (auto i (0); i < reps_count; i++)
	{
		...
		// Confirmation
		auto vote = lumex::test::make_final_vote (lumex::dev::genesis_key, { send, open });
		node1.vote_processor.vote (vote, std::make_shared<lumex::transport::inproc::channel> (node1, node1));
	}
	...
	auto send_last = builder.make_block ()
					 ...
					 .previous (latest)
					 .representative (lumex::dev::genesis_key.pub)
					 .balance (balance - 2 * lumex::Klumex_ratio)
					 .link (key.pub)
					 .sign (lumex::dev::genesis_key.prv, lumex::dev::genesis_key.pub)
					 .work (*system.work.generate (latest))
					 .build ();
	...
	for (auto i (0); i < reps_count; i++)
	{
		auto fork = builder.make_block ()
					...
					.previous (latest)
					.representative (lumex::dev::genesis_key.pub)
					.balance (balance - lumex::Klumex_ratio - i)
					.link (key.pub)
					.sign (lumex::dev::genesis_key.prv, lumex::dev::genesis_key.pub)
					.work (*system.work.generate (latest))
					.build ();
	...
	for (auto i (0); i < reps_count; i++)
	{
		auto fork = builder.make_block ()
					...
					.work (*system.work.generate (latest))
					.build ();
		auto vote = lumex::test::make_vote (keys[i], { fork }, 0, 0);
		node1.vote_processor.vote (vote, std::make_shared<lumex::transport::inproc::channel> (node1, node1));
	...
	ASSERT_TRUE (node2.network.flood_block (send_last, lumex::transport::traffic_type::test));
	ASSERT_TIMELY (3s, node1.stats.count (lumex::stat::type::message, lumex::stat::detail::publish, lumex::stat::dir::in) > 0);

	// Correct block without votes is ignored
	std::unordered_map<lumex::block_hash, std::shared_ptr<lumex::block>> blocks1;
	...
	auto vote = lumex::test::make_vote (lumex::dev::genesis_key, { send_last }, 0, 0);
	node1.vote_processor.vote (vote, std::make_shared<lumex::transport::inproc::channel> (node1, node1));
	// ensure vote arrives before the block
	ASSERT_TIMELY_EQ (5s, 1, node1.vote_cache.find (send_last->hash ()).size ());
	node1.network.filter.clear ();
	ASSERT_TRUE (node2.network.flood_block (send_last, lumex::transport::traffic_type::test));
	ASSERT_TIMELY (5s, node1.stats.count (lumex::stat::type::message, lumex::stat::detail::publish, lumex::stat::dir::in) > 1);
	...
	ASSERT_TRUE (votes2.find (lumex::dev::genesis_key.pub) != votes2.end ());
}
...
TEST (active_elections, confirmation_consistency)
{
	...
	{
		auto block (system.wallet (0)->send_action (lumex::dev::genesis_key.pub, lumex::public_key (), node.config.receive_minimum.number ()));
...
TEST (active_elections, confirm_new)
{
	...
	auto send_copy = lumex::send_block_builder ().make_block ().from (*send).build ();
...
TEST (active_elections, conflicting_block_vote_existing_election)
{
	...
	auto fork = builder.make_block ()
				...
				.work (*system.work.generate (lumex::dev::genesis->hash ()))
				.build ();
	auto vote_fork = lumex::test::make_final_vote (lumex::dev::genesis_key, { fork });

	ASSERT_EQ (lumex::block_status::progress, node.process_local (send).value ());
	ASSERT_TIMELY_EQ (5s, 1, node.active.size ());
	...
	ASSERT_EQ (lumex::block_status::fork, node.process_local (fork).value ());
...
TEST (active_elections, activate_account_chain)
{
	...
	auto election1 = lumex::test::start_election (system, node, send->hash ());
	ASSERT_EQ (1, node.active.size ());
...
TEST (active_elections, activate_inactive)
{
	...
	ASSERT_ALWAYS_EQ (50ms, 0, node.stats.count (lumex::stat::type::confirmation_observer, lumex::stat::detail::active_conf_height, lumex::stat::dir::out));
...
TEST (active_elections, list_active)
{
	...
	ASSERT_TRUE (lumex::test::start_elections (system, node, { send, send2, open }));
...
TEST (active_elections, vacancy)
{
	...
	ASSERT_EQ (lumex::block_status::progress, node.process (send));
	ASSERT_EQ (1, node.active.vacancy (lumex::election_behavior::priority));
	ASSERT_EQ (0, node.active.size ());
	auto election1 = lumex::test::start_election (system, node, send->hash ());
	ASSERT_TIMELY (1s, updated);
	updated = false;
	ASSERT_EQ (0, node.active.vacancy (lumex::election_behavior::priority));
	ASSERT_EQ (1, node.active.size ());
...
TEST (active_elections, limit_vote_hinted_elections)
{
	...
	// Enough weight to trigger election hinting but not enough to confirm block on its own
	const auto amount = ((node.online_reps.trended () / 100) * node.config.hinted_scheduler->hinting_threshold_percent) + 1000 * lumex::Klumex_ratio;
	lumex::keypair rep1 = lumex::test::setup_rep (system, node, amount / 2);
	lumex::keypair rep2 = lumex::test::setup_rep (system, node, amount / 2);

	auto blocks = lumex::test::setup_independent_blocks (system, node, 2);
	auto open0 = blocks[0];
	auto open1 = blocks[1];
	...
	// Inactive vote
	auto vote1 = lumex::test::make_vote (rep1, { open0, open1 });
	node.vote_processor.vote (vote1, lumex::test::fake_channel (node));
	...
	ASSERT_FALSE (lumex::test::confirmed (node, { open0, open1 }));

	// This vote should trigger election hinting for first receive block
	auto vote2 = lumex::test::make_vote (rep2, { open0 });
	node.vote_processor.vote (vote2, lumex::test::fake_channel (node));
	// Ensure an election got started for open0 block
	ASSERT_TIMELY_EQ (5s, node.active.size (), 1);
	ASSERT_TIMELY (5s, lumex::test::active (node, { open0 }));

	// This vote should trigger election hinting but not become active due to limit of active hinted elections
	auto vote3 = lumex::test::make_vote (rep2, { open1 });
	node.vote_processor.vote (vote3, lumex::test::fake_channel (node));
	// Ensure no new election are getting started
	ASSERT_NEVER (1s, lumex::test::active (node, { open1 }));
	ASSERT_EQ (node.active.size (), 1);

	// This final vote should confirm the first receive block
	auto vote4 = lumex::test::make_final_vote (lumex::dev::genesis_key, { open0 });
	node.vote_processor.vote (vote4, lumex::test::fake_channel (node));
	// Ensure election for open0 block got confirmed
	ASSERT_TIMELY (5s, lumex::test::confirmed (node, { open0 }));

	// Now a second block should get vote hinted
	ASSERT_TIMELY (5s, lumex::test::active (node, { open1 }));

	// Ensure there was no overflow of elections
	ASSERT_EQ (0, node.stats.count (lumex::stat::type::active_elections_dropped, lumex::stat::detail::priority));
}
...
TEST (active_elections, bound_election_winners)
{
	...
	auto blocks = lumex::test::setup_independent_blocks (system, node, 10);
...

	{
		...
		auto guard = node.ledger.tx_begin_write (lumex::store::writer::testing);
		...
		ASSERT_TRUE (node.active.vacancy (lumex::election_behavior::priority) > 0);
		...
		ASSERT_TIMELY (5s, node.active.vacancy (lumex::election_behavior::priority) < 0);
...
TEST (active_elections, stale_election)
{
	...
	// Check initial state
	ASSERT_EQ (0, node.stats.count (lumex::stat::type::active_elections, lumex::stat::detail::stale));
	...
	ASSERT_TIMELY (5s, node.stats.count (lumex::stat::type::active_elections, lumex::stat::detail::stale) > 0);
...
TEST (active_elections, stale_election_multiple)
{
	...
	lumex::locked<std::set<lumex::qualified_root>> stale_detected;
	...
	ASSERT_TIMELY (5s, node.stats.count (lumex::stat::type::active_elections, lumex::stat::detail::stale) >= blocks.size ());
...
TEST (active_elections, transition_optimistic_to_priority)
{
	...
	ASSERT_TRUE (lumex::test::process (node, { block }));
	...
	ASSERT_EQ (lumex::election_behavior::optimistic, election->behavior ());

	// Verify initial sizes
	ASSERT_EQ (1, node.active.size (lumex::election_behavior::optimistic));
	ASSERT_EQ (0, node.active.size (lumex::election_behavior::priority));

	// Transition to priority
	auto transition_result = node.active.insert (block, lumex::election_behavior::priority);
	ASSERT_FALSE (transition_result.inserted);
	ASSERT_EQ (election, transition_result.election);

	// Verify transition
	ASSERT_EQ (lumex::election_behavior::priority, election->behavior ());
	ASSERT_EQ (1, node.stats.count (lumex::stat::type::active_elections, lumex::stat::detail::transition_priority));
	ASSERT_EQ (0, node.active.size (lumex::election_behavior::optimistic));
	ASSERT_EQ (1, node.active.size (lumex::election_behavior::priority));
}
...
TEST (active_elections, transition_hinted_to_priority)
{
	...
	ASSERT_TRUE (result.inserted);
	auto election = result.election;
	ASSERT_EQ (lumex::election_behavior::hinted, election->behavior ());

	// Verify initial sizes
	ASSERT_EQ (1, node.active.size (lumex::election_behavior::hinted));
	ASSERT_EQ (0, node.active.size (lumex::election_behavior::priority));
	...
	ASSERT_EQ (lumex::election_behavior::priority, election->behavior ());
	ASSERT_EQ (1, node.stats.count (lumex::stat::type::active_elections, lumex::stat::detail::transition_priority));
	ASSERT_EQ (0, node.active.size (lumex::election_behavior::hinted));
	ASSERT_EQ (1, node.active.size (lumex::election_behavior::priority));
}
...
TEST (active_elections, cancel_cemented_races)
{
	...
	auto const chains = lumex::test::setup_chains (system, node, chain_count,
	blocks_per_chain,
	lumex::dev::genesis_key,
	false); // Don't auto-confirm

	// Collect all blocks for random access
	std::vector<std::shared_ptr<lumex::block>> all_blocks;
...
TEST (active_elections, cancel_already_cemented)
{
	...
	auto const chains = lumex::test::setup_chains (system, node, chain_count, blocks_per_chain, lumex::dev::genesis_key, false);
	...
	auto election = node.active.insert (last_block, lumex::election_behavior::priority);
	...
	ASSERT_GT (node.stats.count (lumex::stat::type::active_elections, lumex::stat::detail::cancel_checkup), 0)
