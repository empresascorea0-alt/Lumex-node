...
{
public:
	void add (lumex::messages::asc_pull_ack const & ack)
	{
		lumex::lock_guard<lumex::mutex> lock{ mutex };
		responses.push_back (ack);
	}

	std::vector<lumex::messages::asc_pull_ack> get ()
	{
		lumex::lock_guard<lumex::mutex> lock{ mutex };
		return responses;
	}
	...
	void connect (lumex::bootstrap_server & server)
	{
...
private:
	lumex::mutex mutex;
	std::vector<lumex::messages::asc_pull_ack> responses;
};
...
bool compare_blocks (auto const & blocks_a, auto const & blocks_b, int skip = 0)
{
	...
	for (int n = 0; n < count; ++n)
	{
		...
		// lumex::block does not have != operator
		if (!(block_a == block_b))
		{
			return false;
		}
...
TEST (bootstrap_server, serve_account_blocks)
{
	...
	auto chains = lumex::test::setup_chains (system, node, 1, 128);
	auto [first_account, first_blocks] = chains.front ();

	// Request blocks from account root
	lumex::messages::asc_pull_req request{ node.network_params.network };
	request.id = 7;
	request.type = lumex::messages::asc_pull_type::blocks;

	lumex::messages::asc_pull_req::blocks_payload request_payload{};
	request_payload.start = first_account;
	request_payload.count = lumex::bootstrap_server::max_blocks;
	request_payload.start_type = lumex::messages::asc_pull_req::hash_type::account;

	request.payload = request_payload;
	request.update_header ();

	node.inbound (request, lumex::test::fake_channel (node));
	...
	ASSERT_EQ (response.type, lumex::messages::asc_pull_type::blocks);

	lumex::messages::asc_pull_ack::blocks_payload response_payload;
	ASSERT_NO_THROW (response_payload = std::get<lumex::messages::asc_pull_ack::blocks_payload> (response.payload));
...
TEST (bootstrap_server, serve_hash)
{
	...
	auto chains = lumex::test::setup_chains (system, node, 1, 256);
	auto [account, blocks] = chains.front ();

	// Skip a few blocks to request hash in the middle of the chain
	blocks = lumex::block_list_t{ std::next (blocks.begin (), 9), blocks.end () };
	...
	request_payload.count = lumex::bootstrap_server::max_blocks;
	request_payload.start_type = lumex::messages::asc_pull_req::hash_type::block;
...
TEST (bootstrap_server, serve_missing)
{
	...
	lumex::messages::asc_pull_req::blocks_payload request_payload{};
	request_payload.start = lumex::test::random_hash ();
	request_payload.count = lumex::bootstrap_server::max_blocks;
	request_payload.start_type = lumex::messages::asc_pull_req::hash_type::block;
...
TEST (bootstrap_server, serve_multiple)
{
	...
	auto chains = lumex::test::setup_chains (system, node, 32, 16);
...
TEST (bootstrap_server, serve_account_info)
{
	...
	lumex::messages::asc_pull_req request{ node.network_params.network };
	request.id = 7;
	request.type = lumex::messages::asc_pull_type::account_info;

	lumex::messages::asc_pull_req::account_info_payload request_payload{};
	request_payload.target = account;
	request_payload.target_type = lumex::messages::asc_pull_req::hash_type::account;
	...
	// Ensure we got response exactly for what we asked for
	ASSERT_EQ (response.id, 7);
	ASSERT_EQ (response.type, lumex::messages::asc_pull_type::account_info);

	lumex::messages::asc_pull_ack::account_info_payload response_payload;
	ASSERT_NO_THROW (response_payload = std::get<lumex::messages::asc_pull_ack::account_info_payload> (response.payload));
...
TEST (bootstrap_server, serve_account_info_missing)
{
	...
	lumex::messages::asc_pull_req::account_info_payload request_payload{};
	request_payload.target = lumex::test::random_account ();
	request_payload.target_type = lumex::messages::asc_pull_req::hash_type::account;
...
TEST (bootstrap_server, serve_frontiers)
{
	...
	auto chains = lumex::test::setup_chains (system, node, /* chain count */ 32, /* block count */ 4);

	// Request all frontiers
	lumex::messages::asc_pull_req request{ node.network_params.network };
	request.id = 7;
	request.type = lumex::messages::asc_pull_type::frontiers;

	lumex::messages::asc_pull_req::frontiers_payload request_payload{};
	request_payload.count = lumex::bootstrap_server::max_frontiers;
	...
	// Ensure we got response exactly for what we asked for
	ASSERT_EQ (response.id, 7);
	ASSERT_EQ (response.type, lumex::messages::asc_pull_type::frontiers);

	lumex::messages::asc_pull_ack::frontiers_payload response_payload;
	ASSERT_NO_THROW (response_payload = std::get<lumex::messages::asc_pull_ack::frontiers_payload> (response.payload));

	ASSERT_EQ (response_payload.frontiers.size (), chains.size () + 1); // +1 for genesis

	// Ensure frontiers match what we expect
	std::map<lumex::account, lumex::block_hash> expected_frontiers;
	for (auto & [account, blocks] : chains)
	{
		expected_frontiers[account] = blocks.back ()->hash ();
	}
	expected_frontiers[lumex::dev::genesis_key.pub] = node.latest (lumex::dev::genesis_key.pub);
...
TEST (bootstrap_server, serve_frontiers_invalid_count)
{
	...
	auto chains = lumex::test::setup_chains (system, node, /* chain count */ 4, /* block count */ 4);
	...
	ASSERT_TIMELY_EQ (5s, node.stats.count (lumex::stat::type::bootstrap_server, lumex::stat::detail::invalid), 1);
	...
	// Count larger than allowed
	{
		...
		lumex::messages::asc_pull_req::frontiers_payload request_payload{};
		request_payload.count = lumex::bootstrap_server::max_frontiers + 1;
		request_payload.start = 0;
	...
	ASSERT_TIMELY_EQ (5s, node.stats.count (lumex::stat::type::bootstrap_server, lumex::stat::detail::invalid), 2);
	...
	ASSERT_TIMELY_EQ (5s, node.stats.count (lumex::stat::type::bootstrap_server, lumex::stat::detail::invalid), 3);
