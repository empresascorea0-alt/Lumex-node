...
lumex::test::test_response::test_response (boost::property_tree::ptree const & request_a, boost::asio::io_context & io_ctx_a) :
	request (request_a),
	sock (io_ctx_a)
{
}

lumex::test::test_response::test_response (boost::property_tree::ptree const & request_a, uint16_t port_a, boost::asio::io_context & io_ctx_a) :
	request (request_a),
	sock (io_ctx_a)
{
	run (port_a);
...
void lumex::test::test_response::run (uint16_t port_a)
{
	sock.async_connect (lumex::tcp_endpoint (boost::asio::ip::address_v6::loopback (), port_a), [this] (boost::system::error_code const & ec) {
		if (!ec)
		{
