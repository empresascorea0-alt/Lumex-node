...
TEST (rpc, wrapped_task)
{
	...
	auto handler_l (std::make_shared<lumex::json_handler> (node, node_rpc_config, "", response_handler_l));
	auto task (handler_l->create_worker_task ([] (std::shared_ptr<lumex::json_handler> const &) {
...
TEST (rpc, account_balance)
{
	...
	request.put ("account", lumex::dev::genesis_key.pub.to_account ());
...
TEST (rpc, account_create)
{
	...
	ASSERT_EQ (std::error_code (lumex::error_common::invalid_index).message (), response2.get<std::string> ("error"));
}
...
TEST (rpc, account_weight)
{
	...
	auto block = builder
				 ...
				 .build ();
	ASSERT_EQ (lumex::block_status::progress, node1->process (block));
...
TEST (rpc, validate_account_invalid)
{
	...
	auto node = add_ipc_enabled_node (system);
	auto const rpc_ctx = add_rpc (system, node);
	std::string account = lumex::dev::genesis_key.pub.to_account ();
	account[0] ^= 0x1;
	boost::property_tree::ptree request;
	request.put ("action", "validate_account_number");
...
TEST (rpc, send)
{
	...
	request.put ("source", lumex::dev::genesis_key.pub.to_account ());
	request.put ("destination", lumex::dev::genesis_key.pub.to_account ());
	request.put ("amount", "100");
	ASSERT_EQ (node->balance (lumex::dev::genesis_key.pub), lumex::dev::constants.genesis_amount);
	auto response (wait_response (system, rpc_ctx, request, 10s));
	std::string block_text (response.get<std::string> ("block"));
	lumex::block_hash block;
	ASSERT_FALSE (block.decode_hex (block_text));
	ASSERT_TRUE (node->block_or_pruned_exists (block));
	ASSERT_EQ (node->latest (lumex::dev::genesis_key.pub), block);
	ASSERT_NE (node->balance (lumex::dev::genesis_key.pub), lumex::dev::constants.genesis_amount);
}
...
TEST (rpc, send_fail)
{
	...
	request.put ("amount", "100");
	auto response (wait_response (system, rpc_ctx, request, 10s));
	ASSERT_EQ (std::error_code (lumex::error_common::account_not_found_wallet).message (), response.get<std::string> ("error"));
}
...
TEST (rpc, send_work)
{
	...
	request.put ("work", "1");
	auto response (wait_response (system, rpc_ctx, request, 10s));
	ASSERT_EQ (std::error_code (lumex::error_common::invalid_work).message (), response.get<std::string> ("error"));
	request.erase ("work");
	request.put ("work", lumex::to_string_hex (*node->work_generate_blocking (node->latest (lumex::dev::genesis_key.pub))));
...
TEST (rpc, send_work_disabled)
{
	...
	request.put ("amount", "100");
	auto response (wait_response (system, rpc_ctx, request, 10s));
	ASSERT_EQ (std::error_code (lumex::error_common::disabled_work_generation).message (), response.get<std::string> ("error"));
}
...
TEST (rpc, send_idempotent)
{
	...
	request.put ("action", "send");
	request.put ("source", lumex::dev::genesis_key.pub.to_account ());
	request.put ("destination", lumex::account{}.to_account ());
	request.put ("amount", (lumex::dev::constants.genesis_amount - (lumex::dev::constants.genesis_amount / 4)).convert_to<std::string> ());
	...
	ASSERT_FALSE (block.decode_hex (block_text));
	ASSERT_TRUE (node->block_or_pruned_exists (block));
	ASSERT_EQ (node->balance (lumex::dev::genesis_key.pub), lumex::dev::constants.genesis_amount / 4);
	auto response2 (wait_response (system, rpc_ctx, request));
	...
	ASSERT_EQ (std::error_code (lumex::error_common::insufficient_balance).message (), response3.get<std::string> ("error"));
}
...
TEST (rpc, send_epoch_2)
{
	...
	std::shared_ptr<lumex::block> epoch1, epoch2;
	ASSERT_TRUE (epoch1 = system.upgrade_genesis_epoch (*node, lumex::epoch::epoch_1));
	ASSERT_TRUE (epoch2 = system.upgrade_genesis_epoch (*node, lumex::epoch::epoch_2));
	system.wallet (0)->insert_adhoc (lumex::dev::genesis_key.prv, false);
	ASSERT_TIMELY (5s, lumex::test::confirmed (*node, { epoch1, epoch2 }));

	auto target_difficulty = lumex::dev::network_params.work.threshold (lumex::work_version::work_1, lumex::block_details (lumex::epoch::epoch_2, true, false, false));
	...
	request.put ("action", "send");
	request.put ("source", lumex::dev::genesis_key.pub.to_account ());
	request.put ("destination", lumex::keypair ().pub.to_account ());
	request.put ("amount", "1");

	// Test that the correct error is given if there is insufficient work
	auto latest = node->ledger.any.account_head (node->ledger.tx_begin_read (), lumex::dev::genesis_key.pub);
	auto insufficient = system.work_generate_limited (latest, min_difficulty, target_difficulty);
	request.put ("work", lumex::to_string_hex (insufficient));
	{
		auto response (wait_response (system, rpc_ctx, request));
		std::error_code ec (lumex::error_common::invalid_work);
...
TEST (rpc, wallet_representative)
{
	...
	ASSERT_EQ (account_text1, lumex::dev::genesis_key.pub.to_account ());
}
...
TEST (rpc, wallet_representative_set_force)
{
	...
	while (representative != key.pub)
	{
		...
		auto info = node->ledger.any.account_get (transaction, lumex::dev::genesis_key.pub);
		if (info)
		{
...
TEST (rpc, wallet_create_seed)
{
	...
	auto node = add_ipc_enabled_node (system);
	lumex::raw_key seed;
	lumex::random_pool::generate_block (seed.bytes.data (), seed.bytes.size ());
	auto prv = lumex::deterministic_key (seed, 0);
	auto pub (lumex::pub_key (prv));
	auto const rpc_ctx = add_rpc (system, node);
	boost::property_tree::ptree request;
	request.put ("action", "wallet_create");
...
TEST (rpc, wallet_export)
{
	...
	auto new_wallet = node->wallets.create (lumex::random_wallet_id ());
	ASSERT_NE (nullptr, new_wallet);
	ASSERT_FALSE (new_wallet->import (wallet_json, ""));
	ASSERT_TRUE (new_wallet->exists (lumex::dev::genesis_key.pub));
}
...
TEST (rpc, account_move)
{
	...
	destination->insert_adhoc (lumex::dev::genesis_key.prv);
	lumex::keypair key;
	auto source_id = lumex::random_wallet_id ();
	...
	ASSERT_TRUE (destination->exists (lumex::dev::genesis_key.pub));
...
TEST (rpc, block)
{
	...
	request.put ("hash", node->latest (lumex::dev::genesis_key.pub).to_string ());
	auto response (wait_response (system, rpc_ctx, request));
...
TEST (rpc, block_account)
{
	...
	request.put ("hash", lumex::dev::genesis->hash ().to_string ());
	auto response (wait_response (system, rpc_ctx, request));
...
TEST (rpc, chain)
{
	...
	system.wallet (0)->insert_adhoc (lumex::dev::genesis_key.prv);
	lumex::keypair key;
	auto genesis (node->latest (lumex::dev::genesis_key.pub));
	ASSERT_FALSE (genesis.is_zero ());
	auto block (system.wallet (0)->send_action (lumex::dev::genesis_key.pub, key.pub, 1));
	...
	std::vector<lumex::block_hash> blocks;
	for (auto i (blocks_node.begin ()), n (blocks_node.end ()); i != n; ++i)
	{
		blocks.push_back (lumex::block_hash (i->second.get<std::string> ("")));
	}
...
TEST (rpc, frontier)
{
	lumex::test::system system;
	auto node = add_ipc_enabled_node (system);
	std::unordered_map<lumex::account, lumex::block_hash> source;
	{
		...
		for (auto i (0); i < 1000; ++i)
		{
			...
			lumex::random_pool::generate_block (hash.bytes.data (), hash.bytes.size ());
			source[key.pub] = hash;
			node->store.account.put (transaction, key.pub, lumex::account_info (hash, 0, 0, 0, 0, 0, lumex::epoch::epoch_0));
		}
	}
	lumex::keypair key;
	auto const rpc_ctx = add_rpc (system, node);
	...
	request.put ("account", lumex::account{}.to_account ());
	request.put ("count", std::to_string (std::numeric_limits<uint64_t>::max ()));
	auto response (wait_response (system, rpc_ctx, request));
	...
	ASSERT_EQ (1, frontiers.erase (lumex::dev::genesis_key.pub));
...
TEST (rpc, history)
{
	...
	system.wallet (0)->insert_adhoc (lumex::dev::genesis_key.prv);
	auto change (system.wallet (0)->change_action (lumex::dev::genesis_key.pub, lumex::dev::genesis_key.pub));
	ASSERT_NE (nullptr, change);
	auto send (system.wallet (0)->send_action (lumex::dev::genesis_key.pub, lumex::dev::genesis_key.pub, node0->config.receive_minimum.number ()));
	ASSERT_NE (nullptr, send);
	auto receive (system.wallet (0)->receive_action (send->hash (), lumex::dev::genesis_key.pub, node0->config.receive_minimum.number (), send->destination ()));
	...
	auto usend = builder
				 ...
				 .link (lumex::dev::genesis_key.pub)
				 .sign (lumex::dev::genesis_key.prv, lumex::dev::genesis_key.pub)
				 .work (*node0->work_generate_blocking (node0->latest (lumex::dev::genesis_key.pub)))
				 .build ();
	...
	auto uchange = builder
				   ...
				   .representative (lumex::keypair ().pub)
				   .balance (lumex::dev::constants.genesis_amount)
				   .link (0)
				   .sign (lumex::dev::genesis_key.prv, lumex::dev::genesis_key.pub)
				   ...
				   .build ();
	{
		...
		ASSERT_EQ (lumex::block_status::progress, node0->ledger.process (transaction, usend));
		ASSERT_EQ (lumex::block_status::progress, node0->ledger.process (transaction, ureceive));
		ASSERT_EQ (lumex::block_status::progress, node0->ledger.process (transaction, uchange));
	}
	...
	ASSERT_EQ (lumex::dev::genesis_key.pub.to_account (), std::get<1> (history_l[0]));
	ASSERT_EQ (lumex::Klumex_ratio.convert_to<std::string> (), std::get<2> (history_l[0]));
	ASSERT_EQ (5, history_l.size ());
	ASSERT_EQ ("send", std::get<0> (history_l[1]));
	ASSERT_EQ (usend->hash ().to_string (), std::get<3> (history_l[1]));
	ASSERT_EQ (lumex::dev::genesis_key.pub.to_account (), std::get<1> (history_l[1]));
	ASSERT_EQ (lumex::Klumex_ratio.convert_to<std::string> (), std::get<2> (history_l[1]));
	ASSERT_EQ ("receive", std::get<0> (history_l[2]));
	ASSERT_EQ (lumex::dev::genesis_key.pub.to_account (), std::get<1> (history_l[2]));
	...
	ASSERT_EQ (lumex::dev::genesis_key.pub.to_account (), std::get<1> (history_l[3]));
	...
	ASSERT_EQ (lumex::dev::genesis_key.pub.to_account (), std::get<1> (history_l[4]));
	ASSERT_EQ (lumex::dev::constants.genesis_amount.convert_to<std::string> (), std::get<2> (history_l[4]));
	ASSERT_EQ (lumex::dev::genesis->hash ().to_string (), std::get<3> (history_l[4]));
}
...
TEST (rpc, account_history)
{
	...
	auto send2 (system.wallet (0)->send_action (lumex::dev::genesis_key.pub, account2, node0->config.receive_minimum.number ()));
	...
	// Test filter for receive state blocks
	{
		...
		boost::property_tree::ptree other_account;
		other_account.put ("", lumex::dev::genesis_key.pub.to_account ());
		boost::property_tree::ptree filtered_accounts;
		filtered_accounts.push_back (std::make_pair ("", other_account));
		request.add_child ("account_filter", filtered_accounts);
		request.put ("count", 100);
...
TEST (rpc, history_count)
{
	...
	auto change (system.wallet (0)->change_action (lumex::dev::genesis_key.pub, lumex::dev::genesis_key.pub));
	ASSERT_NE (nullptr, change);
	auto send (system.wallet (0)->send_action (lumex::dev::genesis_key.pub, lumex::dev::genesis_key.pub, node->config.receive_minimum.number ()));
	ASSERT_NE (nullptr, send);
	auto receive (system.wallet (0)->receive_action (send->hash (), lumex::dev::genesis_key.pub, node->config.receive_minimum.number (), send->destination ()));
	ASSERT_NE (nullptr, receive);
	auto const rpc_ctx = add_rpc (system, node);
	boost::property_tree::ptree request;
	request.put ("action", "history");
...
TEST (rpc, history_pruning)
{
	...
	auto change = builder
				  ...
				  .representative (lumex::dev::genesis_key.pub)
				  .sign (lumex::dev::genesis_key.prv, lumex::dev::genesis_key.pub)
				  .work (*node0->work.generate (lumex::dev::genesis->hash ()))
				  .build ();
	...
	auto send = builder
				...
				.balance (lumex::dev::constants.genesis_amount - node0->config.receive_minimum.number ())
				.sign (lumex::dev::genesis_key.prv, lumex::dev::genesis_key.pub)
	...
	lumex::test::process_live (*node0, blocks);
	ASSERT_TIMELY (5s, lumex::test::exists (*node0, blocks));
	system.wallet (0)->insert_adhoc (lumex::dev::genesis_key.prv);

	lumex::test::confirm (node0->ledger, blocks);
	...
	ASSERT_EQ (lumex::dev::genesis_key.pub.to_account (), entry.get<std::string> ("account", "N/A"));
	...
	ASSERT_EQ (lumex::dev::genesis_key.pub.to_account (), entry.get<std::string> ("account", "N/A"));
	ASSERT_EQ (lumex::Klumex_ratio.convert_to<std::string> (), entry.get<std::string> ("amount", "N/A"));
...
TEST (rpc, account_history_state_open)
{
	...
	lumex::keypair key;
	auto node0 = add_ipc_enabled_node (system);
	auto blocks = lumex::test::setup_new_account (system, *node0, 1, lumex::dev::genesis_key, key, key.pub, true);
	auto const rpc_ctx = add_rpc (system, node0);
	boost::property_tree::ptree request;
	request.put ("action", "account_history");
	request.put ("account", key.pub.to_account ());
...
TEST (rpc, process_block)
{
	...
	auto const rpc_ctx = add_rpc (system, node1);
	lumex::keypair key;
	auto latest (node1->latest (lumex::dev::genesis_key.pub));
	lumex::block_builder builder;
	auto send = builder
				.send ()
				.previous (latest)
	...
	{
		auto response (wait_response (system, rpc_ctx, request));
		ASSERT_TIMELY_EQ (10s, node1->latest (lumex::dev::genesis_key.pub), send->hash ());
	...
	{
		auto response (wait_response (system, rpc_ctx, request));
		std::error_code ec (lumex::error_blocks::invalid_block);
...
TEST (rpc, process_block_async)
{
	...
	{
		auto response (wait_response (system, rpc_ctx, request));
		std::error_code ec (lumex::error_common::is_not_state_block);
		ASSERT_EQ (ec.message (), response.get<std::string> ("error"));
	}
	...
	request.put ("block", json1);
	{
		...
		ASSERT_TIMELY_EQ (10s, node1->latest (lumex::dev::genesis_key.pub), state_send->hash ());
	}
}
...
TEST (rpc, process_republish)
{
	...
	request.put ("block", json);
	auto response (wait_response (system, rpc_ctx, request));
	ASSERT_TIMELY_EQ (10s, node2.latest (lumex::dev::genesis_key.pub), send->hash ());
}
...
TEST (rpc, process_subtype_send)
{
	...
	auto response (wait_response (system, rpc_ctx, request));
	std::error_code ec (lumex::error_rpc::invalid_subtype_balance);
	ASSERT_EQ (response.get<std::string> ("error"), ec.message ());
	...
	ASSERT_TIMELY_EQ (10s, system.nodes[1]->latest (lumex::dev::genesis_key.pub), send->hash ());
}
...
TEST (rpc, process_subtype_open)
{
	...
	auto send = builder
				...
				.work (*node1->work_generate_blocking (latest))
				.build ();
	ASSERT_EQ (lumex::block_status::progress, node1->process (send));
	ASSERT_EQ (lumex::block_status::progress, node2.process (send));
	auto const rpc_ctx = add_rpc (system, node1);
...
TEST (rpc, process_subtype_receive)
{
	...
	request.put ("subtype", "open");
	auto response2 (wait_response (system, rpc_ctx, request));
	ec = lumex::error_rpc::invalid_subtype_previous;
	ASSERT_EQ (response2.get<std::string> ("error"), ec.message ());
	request.put ("subtype", "receive");
	auto response3 (wait_response (system, rpc_ctx, request));
	ASSERT_EQ (receive->hash ().to_string (), response3.get<std::string> ("hash"));
	ASSERT_TIMELY_EQ (10s, node2.latest (lumex::dev::genesis_key.pub), receive->hash ());
}
...
TEST (rpc, process_ledger_insufficient_work)
{
	...
	auto send = builder
				...
				.build ();
	ASSERT_LT (lumex::dev::network_params.work.difficulty (*send), max_difficulty);
	ASSERT_GE (lumex::dev::network_params.work.difficulty (*send), min_difficulty);
	boost::property_tree::ptree request;
	request.put ("action", "process");
	std::string json;
	send->serialize_json (json);
	...
	request.put ("subtype", "send");
	auto response (wait_response (system, rpc_ctx, request));
	std::error_code ec (lumex::error_process::insufficient_work);
	ASSERT_EQ (1, response.count ("error"));
	ASSERT_EQ (response.get<std::string> ("error"), ec.message ());
}
...
TEST (rpc, keepalive)
{
	lumex::test::system system;
	auto node0 = add_ipc_enabled_node (system);
	auto node1 (std::make_shared<lumex::node> (system.get_available_port (), lumex::unique_path (), system.work, lumex::node_flags{}));
	node1->start ();
...
TEST (rpc, version)
{
	...
	ASSERT_EQ (boost::str (boost::format ("Lumex %1%") % LUMEX_VERSION_STRING), response1.json.get<std::string> ("node_vendor"));
	...
	auto genesis_open (node1->latest (lumex::dev::genesis_key.pub));
...
TEST (rpc, work_generate)
{
	...
	auto verify_response = [&node, &rpc_ctx, &system] (auto & request, auto & hash) {
		...
		uint64_t work;
		ASSERT_FALSE (lumex::from_string_hex (work_text, work));
		auto result_difficulty (lumex::dev::network_params.work.difficulty (lumex::work_version::work_1, hash, work));
		auto response_difficulty_text (response.template get<std::string> ("difficulty"));
		uint64_t response_difficulty;
		ASSERT_FALSE (lumex::from_string_hex (response_difficulty_text, response_difficulty));
		ASSERT_EQ (result_difficulty, response_difficulty);
		auto multiplier = response.template get<double> ("multiplier");
		ASSERT_NEAR (lumex::difficulty::to_multiplier (result_difficulty, node->default_difficulty (lumex::work_version::work_1)), multiplier, 1e-6);
	};
...
TEST (rpc, work_generate_with_peers_defaults_distributed)
{
	...
	uint64_t work;
	ASSERT_FALSE (lumex::from_string_hex (work_text, work));
	ASSERT_GE (lumex::dev::network_params.work.difficulty (lumex::work_version::work_1, hash, work), lumex::dev::network_params.work.threshold_base (lumex::work_version::work_1));
}
...
TEST (rpc, work_generate_difficulty)
{
	...
	request.put ("hash", hash.to_string ());
	{
		...
		request.put ("difficulty", lumex::to_string_hex (difficulty));
		auto response (wait_response (system, rpc_ctx, request, 10s));
		auto work_text (response.get<std::string> ("work"));
		uint64_t work;
		ASSERT_FALSE (lumex::from_string_hex (work_text, work));
		...
		ASSERT_NEAR (lumex::difficulty::to_multiplier (result_difficulty, node->default_difficulty (lumex::work_version::work_1)), multiplier, 1e-10);
	...
	{
		uint64_t difficulty (node->max_work_generate_difficulty (lumex::work_version::work_1) + 1);
		request.put ("difficulty", lumex::to_string_hex (difficulty));
		auto response (wait_response (system, rpc_ctx, request));
		std::error_code ec (lumex::error_rpc::difficulty_limit);
		ASSERT_EQ (response.get<std::string> ("error"), ec.message ());
	}
}
...
TEST (rpc, work_generate_multiplier)
{
	...
	request.put ("hash", hash.to_string ());
	{
		...
		request.put ("difficulty", lumex::to_string_hex (static_cast<uint64_t> (0xff00000000000000)));
	...
	}
	{
		...
		auto response (wait_response (system, rpc_ctx, request));
		std::error_code ec (lumex::error_rpc::bad_multiplier_format);
		ASSERT_EQ (response.get<std::string> ("error"), ec.message ());
	}
	{
		double max_multiplier (lumex::difficulty::to_multiplier (node->max_work_generate_difficulty (lumex::work_version::work_1), node->default_difficulty (lumex::work_version::work_1)));
...
TEST (rpc, work_generate_block_high)
{
	...
	auto block = builder
				 ...
				 .build ();
	lumex::block_hash hash (block->root ().as_block_hash ());
	auto block_difficulty (lumex::dev::network_params.work.difficulty (lumex::work_version::work_1, hash, block->block_work ()));
	boost::property_tree::ptree request;
	request.put ("action", "work_generate");
	request.put ("hash", hash.to_string ());
	request.put ("json_block", "true");
	...
	request.add_child ("block", json);
	{
		auto response (wait_response (system, rpc_ctx, request));
		ASSERT_EQ (1, response.count ("error"));
		ASSERT_EQ (std::error_code (lumex::error_rpc::block_work_enough).message (), response.get<std::string> ("error"));
	}
}
...
TEST (rpc, work_generate_block_low)
{
	...
	block->block_work_set (system.work_generate_limited (block->root ().as_block_hash (), threshold, lumex::difficulty::from_multiplier (node->config.max_work_generate_multiplier / 10, threshold)));
	lumex::block_hash hash (block->root ().as_block_hash ());
	auto block_difficulty (lumex::dev::network_params.work.difficulty (*block));
	boost::property_tree::ptree request;
	request.put ("action", "work_generate");
	request.put ("hash", hash.to_string ());
	request.put ("difficulty", lumex::to_string_hex (block_difficulty + 1));
	request.put ("json_block", "false");
	std::string json;
	block->serialize_json (json);
	request.put ("block", json);
...
TEST (rpc, work_generate_block_root_mismatch)
{
	...
	request.put ("block", json);
	{
		auto response (wait_response (system, rpc_ctx, request));
		ASSERT_EQ (1, response.count ("error"));
		ASSERT_EQ (std::error_code (lumex::error_rpc::block_root_mismatch).message (), response.get<std::string> ("error"));
	}
}
...
TEST (rpc, work_generate_block_ledger_epoch_2)
{
	...
	lumex::keypair key;
	system.wallet (0)->insert_adhoc (lumex::dev::genesis_key.prv);
	auto send_block (system.wallet (0)->send_action (lumex::dev::genesis_key.pub, key.pub, lumex::Klumex_ratio));
	...
	auto block = builder
				 ...
				 .work (0)
				 .build ();
	auto threshold (lumex::dev::network_params.work.threshold (block->work_version (), lumex::block_details (lumex::epoch::epoch_2, false, true, false)));
...
TEST (rpc, work_cancel)
{
	...
	while (!done)
	{
		system.work.generate (lumex::work_version::work_1, hash1, node1->network_params.work.base, [&done] (std::optional<uint64_t> work_a) {
			done = !work_a;
		});
...
TEST (rpc, work_peer_bad)
{
	...
	node2.work_generate (lumex::work_version::work_1, hash1, node2.network_params.work.base, [&work] (std::optional<uint64_t> work_a) {
		ASSERT_TRUE (work_a.has_value ());
		work = work_a.value ();
	});
	ASSERT_TIMELY (5s, lumex::dev::network_params.work.difficulty (lumex::work_version::work_1, hash1, work) >= lumex::dev::network_params.work.threshold_base (lumex::work_version::work_1));
}

// Test disabled because it's failing intermittently.
// PR in which it got disabled: https://github.com/lumexcurrency/lumex-node/pull/3629
// Issue for investigating it: https://github.com/lumexcurrency/lumex-node/issues/3639
TEST (rpc, DISABLED_work_peer_one)
{
	...
	lumex::keypair key1;
	std::atomic<uint64_t> work (0);
	node2.work_generate (lumex::work_version::work_1, key1.pub, node1->network_params.work.base, [&work] (std::optional<uint64_t> work_a) {
		ASSERT_TRUE (work_a.has_value ());
		work = work_a.value ();
	});
	ASSERT_TIMELY (5s, lumex::dev::network_params.work.difficulty (lumex::work_version::work_1, key1.pub, work) >= lumex::dev::network_params.work.threshold_base (lumex::work_version::work_1));
}

// Test disabled because it's failing intermittently.
// PR in which it got disabled: https://github.com/lumexcurrency/lumex-node/pull/3629
// Issue for investigating it: https://github.com/lumexcurrency/lumex-node/issues/3636
TEST (rpc, DISABLED_work_peer_many)
{
	lumex::test::system system1 (1);
	lumex::test::system system2;
	lumex::test::system system3 (1);
	lumex::test::system system4 (1);
	...
	{
		lumex::keypair key1;
		node1.work_generate (lumex::work_version::work_1, key1.pub, node1.network_params.work.base, [&work] (std::optional<uint64_t> work_a) {
			work = *work_a;
		});
		while (lumex::dev::network_params.work.difficulty (lumex::work_version::work_1, key1.pub, work) < lumex::dev::network_params.work.threshold_base (lumex::work_version::work_1))
		{
			system1.poll ();
			system2.poll ();
			system3.poll ();
...
// Test disabled because it's failing intermittently.
// PR in which it got disabled: https://github.com/lumexcurrency/lumex-node/pull/3629
// Issue for investigating it: https://github.com/lumexcurrency/lumex-node/issues/3637
TEST (rpc, DISABLED_work_version_invalid)
{
	...
	request.put ("version", "work_invalid");
	{
		auto response (wait_response (system, rpc_ctx, request));
		ASSERT_EQ (1, response.count ("error"));
		ASSERT_EQ (std::error_code (lumex::error_rpc::bad_work_version).message (), response.get<std::string> ("error"));
	}
...
{
	lumex::test::system system;
	lumex::node_config node_config0 = system.default_config ();
	node_config0.receive_minimum = lumex::dev::constants.genesis_amount; // Prevent auto-receive & receive1 block conflicts
...
TEST (rpc, available_supply)
{
	...
	auto block2 (system.wallet (0)->send_action (lumex::dev::genesis_key.pub, 0, 100)); // Sending to burning 0 account
	auto response3 (wait_response (system, rpc_ctx, request1, 10s));
	ASSERT_EQ ("1", response3.get<std::string> ("available"));
}

TEST (rpc, lumex_to_raw)
{
	lumex::test::system system;
	auto node1 = add_ipc_enabled_node (system);
	auto const rpc_ctx = add_rpc (system, node1);
	boost::property_tree::ptree request1;
	request1.put ("action", "lumex_to_raw");
	request1.put ("amount", "1");
	auto response1 (wait_response (system, rpc_ctx, request1));
	ASSERT_EQ (lumex::lumex_ratio.convert_to<std::string> (), response1.get<std::string> ("amount"));
}

TEST (rpc, raw_to_lumex)
{
	lumex::test::system system;
	auto node1 = add_ipc_enabled_node (system);
	auto const rpc_ctx = add_rpc (system, node1);
	boost::property_tree::ptree request1;
	request1.put ("action", "raw_to_lumex");
	request1.put ("amount", lumex::lumex_ratio.convert_to<std::string> ());
	auto response1 (wait_response (system, rpc_ctx, request1));
...
TEST (rpc, account_representative_set)
{
	...
	// create a 2nd account and send it some lumex
	lumex::keypair key2;
	wallet.insert_adhoc (key2.prv);
	auto key2_open_block_hash = wallet.send_sync (lumex::dev::genesis_key.pub, key2.pub, node->config.receive_minimum.number ());
	...
	ASSERT_EQ (lumex::dev::genesis_key.pub, key2_open_block->representative_field ().value ());
...
TEST (rpc, account_representative_set_epoch_2_insufficient_work)
{
	...
	// Upgrade the genesis account to epoch 2
	std::shared_ptr<lumex::block> head;
	ASSERT_NE (nullptr, system.upgrade_genesis_epoch (*node, lumex::epoch::epoch_1));
	ASSERT_NE (nullptr, (head = system.upgrade_genesis_epoch (*node, lumex::epoch::epoch_2)));
	ASSERT_TIMELY (5s, node->block_confirmed (head->hash ()));

	auto target_difficulty = lumex::dev::network_params.work.threshold (lumex::work_version::work_1, lumex::block_details (lumex::epoch::epoch_2, false, false, false));
	ASSERT_LT (node->network_params.work.entry, target_difficulty);
	auto min_difficulty = node->network_params.work.entry;
	...
	request.put ("action", "account_representative_set");
	request.put ("account", lumex::dev::genesis_key.pub.to_account ());
	request.put ("representative", lumex::keypair ().pub.to_account ());
...
TEST (rpc, representatives)
{
	...
	std::vector<lumex::account> representatives;
	...
	ASSERT_EQ (lumex::dev::genesis_key.pub, representatives[0]);
}
...
TEST (rpc, wallet_frontiers)
{
	lumex::test::system system0;
	auto node = add_ipc_enabled_node (system0);
	system0.wallet (0)->insert_adhoc (lumex::dev::genesis_key.prv);
	auto const rpc_ctx = add_rpc (system0, node);
	boost::property_tree::ptree request;
	...
	auto response (wait_response (system0, rpc_ctx, request));
	auto & frontiers_node (response.get_child ("frontiers"));
	std::vector<lumex::account> frontiers;
	for (auto i (frontiers_node.begin ()), n (frontiers_node.end ()); i != n; ++i)
	{
		frontiers.push_back (lumex::account (i->second.get<std::string> ("")));
	}
	ASSERT_EQ (1, frontiers.size ());
	ASSERT_EQ (node->latest (lumex::dev::genesis_key.pub), frontiers[0].as_union ());
}
...
TEST (rpc, work_validate)
{
	...
	request.put ("action", "work_validate");
	request.put ("hash", hash.to_string ());
	request.put ("work", lumex::to_string_hex (work1));
	{
		auto response (wait_response (system, rpc_ctx, request));
		...
		uint64_t difficulty;
		ASSERT_FALSE (lumex::from_string_hex (difficulty_text, difficulty));
		ASSERT_GE (difficulty, node1->default_difficulty (lumex::work_version::work_1));
		double multiplier (response.get<double> ("multiplier"));
		ASSERT_NEAR (multiplier, lumex::difficulty::to_multiplier (difficulty, node1->default_difficulty (lumex::work_version::work_1)), 1e-6);
	}
	uint64_t work2 (0);
	request.put ("work", lumex::to_string_hex (work2));
	{
		auto response (wait_response (system, rpc_ctx, request));
		ASSERT_EQ (0, response.count ("valid"));
		...
		uint64_t difficulty;
		ASSERT_FALSE (lumex::from_string_hex (difficulty_text, difficulty));
		ASSERT_GE (node1->default_difficulty (lumex::work_version::work_1), difficulty);
		double multiplier (response.get<double> ("multiplier"));
		ASSERT_NEAR (multiplier, lumex::difficulty::to_multiplier (difficulty, node1->default_difficulty (lumex::work_version::work_1)), 1e-6);
	}
	auto result_difficulty (lumex::dev::network_params.work.difficulty (lumex::work_version::work_1, hash, work1));
	ASSERT_GE (result_difficulty, node1->default_difficulty (lumex::work_version::work_1));
	request.put ("work", lumex::to_string_hex (work1));
	request.put ("difficulty", lumex::to_string_hex (result_difficulty));
	{
		auto response (wait_response (system, rpc_ctx, request));
	...
	request.put ("work", lumex::to_string_hex (work1));
	request.put ("difficulty", lumex::to_string_hex (difficulty4));
	{
		auto response (wait_response (system, rpc_ctx, request));
		ASSERT_EQ (result_difficulty >= difficulty4, response.get<bool> ("valid"));
		ASSERT_EQ (result_difficulty >= node1->default_difficulty (lumex::work_version::work_1), response.get<bool> ("valid_all"));
	...
	request.put ("work", lumex::to_string_hex (work3));
	{
		auto response (wait_response (system, rpc_ctx, request));
		ASSERT_TRUE (response.get<bool> ("valid"));
		ASSERT_TRUE (response.get<bool> ("valid_all"));
...
TEST (rpc, work_validate_epoch_2)
{
	...
	request.put ("work", lumex::to_string_hex (work));
	{
		auto response (wait_response (system, rpc_ctx, request));
		ASSERT_EQ (0, response.count ("valid"));
		ASSERT_FALSE (response.get<bool> ("valid_all"));
		...
		ASSERT_FALSE (lumex::from_string_hex (difficulty_text, difficulty));
		double multiplier (response.get<double> ("multiplier"));
		ASSERT_NEAR (multiplier, lumex::difficulty::to_multiplier (difficulty, node->network_params.work.epoch_2), 1e-6);
	};
	// After upgrading, the higher difficulty is used to validate and calculate the multiplier
	ASSERT_NE (nullptr, system.upgrade_genesis_epoch (*node, lumex::epoch::epoch_2));
	{
		auto response (wait_response (system, rpc_ctx, request));
		ASSERT_EQ (0, response.count ("valid"));
		ASSERT_FALSE (response.get<bool> ("valid_all"));
		...
		ASSERT_FALSE (lumex::from_string_hex (difficulty_text, difficulty));
		double multiplier (response.get<double> ("multiplier"));
		ASSERT_NEAR (multiplier, lumex::difficulty::to_multiplier (difficulty, node->default_difficulty (lumex::work_version::work_1)), 1e-6);
	};
}
...
TEST (rpc, republish)
{
	...
	auto open = builder
				...
				.work (*node1->work_generate_blocking (key.pub))
				.build ();
	ASSERT_EQ (lumex::block_status::progress, node1->process (open));
	auto const rpc_ctx = add_rpc (system, node1);
	boost::property_tree::ptree request;
	request.put ("action", "republish");
	request.put ("hash", send->hash ().to_string ());
	auto response (wait_response (system, rpc_ctx, request));
	ASSERT_TIMELY (10s, system.nodes[1]->balance (lumex::dev::genesis_key.pub) != lumex::dev::constants.genesis_amount);
	auto & blocks_node (response.get_child ("blocks"));
	std::vector<lumex::block_hash> blocks;
	for (auto i (blocks_node.begin ()), n (blocks_node.end ()); i != n; ++i)
	{
	...
	ASSERT_EQ (1, blocks.size ());
	ASSERT_EQ (lumex::dev::genesis->hash (), blocks[0]);
...
TEST (rpc, accounts_balances)
{
	...
	entry1.put ("", lumex::dev::genesis_key.pub.to_account ());
	...
	auto const account_not_found = "lumex_1os6txqxyuesnxrtshnfb5or1hesc1647wpk9rsr84pmki6eairwha79hk3j";
	...
	auto genesis_entry = response.get_child (boost::str (boost::format ("balances.%1%") % lumex::dev::genesis_key.pub.to_account ()));
...
TEST (rpc, accounts_balances_with_errors)
{
	...
	// Adds a bad account string for getting an error response (the lumex_ address checksum is wrong)
	boost::property_tree::ptree entry;
	auto const bad_account_number = "lumex_3e3j5tkog48pnny9dmfzj1r16pg8t1e76dz5tmac6iq689wyjfpiij4txtd1";
	...
	auto get_error_message = [] (lumex::error_common error_common) -> std::string {
	...
	ASSERT_EQ (get_error_message (lumex::error_common::bad_account_number), bad_account_number_error_text);
}
...
TEST (rpc, accounts_balances_unopened_account_with_receivables)
{
	...
	lumex::keypair unopened_account;
...
TEST (rpc, accounts_representatives)
{
	...
	// Adds a valid account present in the ledger.
	entry.put ("", lumex::dev::genesis_key.pub.to_account ());
	accounts.push_back (std::make_pair ("", entry));
	request.add_child ("accounts", accounts);
	auto response (wait_response (system, rpc_ctx, request));
	// Ensures the response is correct.
	auto response_representative (response.get_child ("representatives").get<std::string> (lumex::dev::genesis_key.pub.to_account ()));
	ASSERT_EQ (response_representative, lumex::dev::genesis_key.pub.to_account ());
...
TEST (rpc, accounts_representatives_with_errors)
{
	...
	auto const bad_account_number = "lumex_36uccgpjzhjsdbj44wm1y5hyz8gefx3wjpp1jircxt84nopxkxti5bzq1rnz";
	entry2.put ("", bad_account_number);
	accounts_l.push_back (std::make_pair ("", entry2));

	// Adds a valid key but that isn't on the ledger. It won't be found.
	auto const account_not_found = "lumex_1hrts7hcoozxccnffoq9hqhngnn9jz783usapejm57ejtqcyz9dpso1bibuy";
	...
	ASSERT_EQ (response.get_child ("representatives").count (lumex::dev::genesis_key.pub.to_account ()), 1);
	auto rep_text = response.get_child ("representatives").get<std::string> (lumex::dev::genesis_key.pub.to_account ());
	ASSERT_EQ (rep_text, lumex::dev::genesis_key.pub.to_account ());
	...
	ASSERT_EQ (response.get_child ("errors").get<std::string> (bad_account_number), make_error_code (lumex::error_common::bad_account_number).message ());
	ASSERT_EQ (response.get_child ("errors").get<std::string> (account_not_found), make_error_code (lumex::error_common::account_not_found).message ());
}
...
TEST (rpc, accounts_frontiers)
{
	...
	ASSERT_EQ (response.get_child ("frontiers").count (lumex::dev::genesis_key.pub.to_account ()), 1);
	auto frontier_text = response.get_child ("frontiers").get<std::string> (lumex::dev::genesis_key.pub.to_account ());
	ASSERT_EQ (lumex::block_hash{ frontier_text }, node->latest (lumex::dev::genesis_key.pub));
...
TEST (rpc, blocks)
{
	...
	entry.put ("", node->latest (lumex::dev::genesis_key.pub).to_string ());
	...
	for (auto & blocks : response.get_child ("blocks"))
	{
		...
		ASSERT_EQ (node->latest (lumex::dev::genesis_key.pub).to_string (), hash_text);
...
TEST (rpc, wallet_info)
{
	...
	auto send (system.wallet (0)->send_action (lumex::dev::genesis_key.pub, key.pub, lumex::Klumex_ratio));
	...
	auto send2 (system.wallet (0)->send_action (lumex::dev::genesis_key.pub, key.pub, 1));
...
TEST (rpc, wallet_balances)
{
	...
	for (auto & balances : response.get_child ("balances"))
	{
		...
		ASSERT_EQ (lumex::dev::genesis_key.pub.to_account (), account_text);
	...
	auto send (system0.wallet (0)->send_action (lumex::dev::genesis_key.pub, key.pub, 1));
...
TEST (rpc, pending_exists)
{
	...
	lumex::keypair key1;
	system.wallet (0)->insert_adhoc (lumex::dev::genesis_key.prv);
	auto hash0 (node->latest (lumex::dev::genesis_key.pub));
	auto block1 (system.wallet (0)->send_action (lumex::dev::genesis_key.pub, key1.pub, 100));
	...
	node->ledger.any.pending_get (node->ledger.tx_begin_read (), lumex::pending_key{ lumex::dev::genesis_key.pub, block1->hash () });
...
TEST (rpc, wallet_pending)
{
	...
	auto block1 = system.wallet (0)->send_action (lumex::dev::genesis_key.pub, key1.pub, 100);
	ASSERT_TIMELY_EQ (5s, node->ledger.cemented.account_height (node->ledger.tx_begin_read (), lumex::dev::genesis_key.pub), 2);
	auto const rpc_ctx = add_rpc (system, node);
	boost::property_tree::ptree request;
	...
	lumex::block_hash hash1{ pending.second.begin ()->second.get<std::string> ("") };
...
TEST (rpc, work_get)
{
	...
	auto node = add_ipc_enabled_node (system);
	system.wallet (0)->insert_adhoc (lumex::dev::genesis_key.prv);
	system.wallet (0)->work_cache_blocking (lumex::dev::genesis_key.pub, node->latest (lumex::dev::genesis_key.pub));
	auto const rpc_ctx = add_rpc (system, node);
	boost::property_tree::ptree request;
	...
	auto work_result = node->wallets.items.begin ()->second->get_work (lumex::dev::genesis_key.pub);
	ASSERT_TRUE (work_result);
	ASSERT_EQ (lumex::to_string_hex (work_result.value ()), work_text);
}
...
TEST (rpc, work_set)
{
	...
	request.put ("wallet", node->wallets.items.begin ()->first.to_string ());
	request.put ("account", lumex::dev::genesis_key.pub.to_account ());
	request.put ("work", lumex::to_string_hex (work0));
	auto response (wait_response (system, rpc_ctx, request));
	std::string success (response.get<std::string> ("success"));
	ASSERT_TRUE (success.empty ());
	auto work1_result = node->wallets.items.begin ()->second->get_work (lumex::dev::genesis_key.pub);
...
TEST (rpc, wallet_republish)
{
	...
	auto node1 = add_ipc_enabled_node (system);
	lumex::keypair key;
	while (key.pub < lumex::dev::genesis_key.pub)
	{
		lumex::keypair key1;
		key.pub = key1.pub;
		key.prv = key1.prv;
...
TEST (rpc, delegators)
{
	...
	ASSERT_EQ ("100", delegators.get<std::string> (lumex::dev::genesis_key.pub.to_account ()));
...
TEST (rpc, delegators_parameters)
{
	...
	if (lumex::dev::genesis_key.pub.number () < key.pub.number ())
	{
		ASSERT_EQ ("100", delegators2.get<std::string> (lumex::dev::genesis_key.pub.to_account ()));
	}
	else
	{
	...
	if (lumex::dev::genesis_key.pub.number () > key.pub.number ())
	{
		last_account = lumex::dev::genesis_key.pub;
	}
	request.put ("start", lumex::account (last_account.number () - 1).to_account ());
...
TEST (rpc, account_info)
{
	...
	// Test for a non existing account
	{
		...
		ASSERT_EQ (error.value (), std::error_code (lumex::error_common::account_not_found).message ());
	}
	...
	auto send = builder
				...
				.build ();
	ASSERT_EQ (lumex::block_status::progress, node1->process (send));
	auto time = lumex::seconds_since_epoch ();
	...
	request.put ("account", lumex::dev::genesis_key.pub.to_account ());
	{
		...
		ASSERT_EQ (lumex::dev::genesis->hash ().to_string (), open_block);
		std::string representative_block (response.get<std::string> ("representative_block"));
		ASSERT_EQ (lumex::dev::genesis->hash ().to_string (), representative_block);
		...
		ASSERT_EQ (lumex::dev::genesis->hash ().to_string (), confirmation_height_frontier);
	...
	request.put ("representative", "1");
	{
		...
		ASSERT_EQ (lumex::dev::genesis_key.pub.to_account (), representative2);
	}

	// Test for confirmed only blocks
	lumex::keypair key1;
	{
		latest = node1->latest (lumex::dev::genesis_key.pub);
		auto send1 = builder
					 .send ()
					 .previous (latest)
		...
		auto state_change = builder
							...
							.build ();
		ASSERT_EQ (lumex::block_status::progress, node1->process (state_change));
	...
	request.put ("include_confirmed", true);
	{
		...
		ASSERT_EQ (confirmed_representative, lumex::dev::genesis_key.pub.to_account ());

		auto confirmed_frontier (response.get<std::string> ("confirmed_frontier"));
		ASSERT_EQ (lumex::dev::genesis->hash ().to_string (), confirmed_frontier);
...
TEST (rpc, json_block_input)
{
	...
	auto send = builder
				.state ()
				.account (lumex::dev::genesis_key.pub)
				.previous (node1->latest (lumex::dev::genesis_key.pub))
				.representative (lumex::dev::genesis_key.pub)
				.balance (lumex::dev::constants.genesis_amount - lumex::Klumex_ratio)
				.link (key.pub)
				.sign (lumex::dev::genesis_key.prv, lumex::dev::genesis_key.pub)
	...
	lumex::state_block block (json_error, response.get_child ("block"));
	ASSERT_FALSE (json_error);

	ASSERT_FALSE (lumex::validate_message (key.pub, send->hash (), block.block_signature ()));
...
TEST (rpc, json_block_output)
{
	...
	bool json_error{ false };
	lumex::send_block send_from_json (json_error, response.get_child ("contents"));
	ASSERT_FALSE (json_error);
}
...
TEST (rpc, blocks_info)
{
	...
	auto check_blocks = [node] (boost::property_tree::ptree & response) {
		for (auto & blocks : response.get_child ("blocks"))
		{
			...
			ASSERT_EQ (lumex::dev::constants.genesis_amount.convert_to<std::string> (), amount_text);
			...
			ASSERT_EQ (lumex::dev::constants.genesis_amount.convert_to<std::string> (), balance_text);
			...
			ASSERT_EQ (lumex::block_hash (0).to_string (), successor_text); // Genesis block doesn't have successor yet
		}
	};
	boost::property_tree::ptree request;
	...
	std::string random_hash = lumex::block_hash ().to_string ();
	...
	{
		auto response (wait_response (system, rpc_ctx, request));
		ASSERT_EQ (std::error_code (lumex::error_blocks::not_found).message (), response.get<std::string> ("error"));
	}
	...
	request.put ("receive_hash", "1");
	{
		...
		for (auto & blocks : response.get_child ("blocks"))
		{
			...
			ASSERT_EQ (lumex::block_hash (0).to_string (), receive_hash);
		}
	}
}
...
TEST (rpc, blocks_info_receive_hash)
{
	...
	// do 4 sends
	auto send1 = system.wallet (0)->send_action (lumex::dev::genesis_key.pub, key1.pub, 1);
	auto send2 = system.wallet (0)->send_action (lumex::dev::genesis_key.pub, key1.pub, 2);
	auto send3 = system.wallet (0)->send_action (lumex::dev::genesis_key.pub, key1.pub, 3);
	auto send4 = system.wallet (0)->send_action (lumex::dev::genesis_key.pub, key1.pub, 4);
...
TEST (rpc, blocks_info_subtype)
{
	...
	system.wallet (0)->insert_adhoc (lumex::dev::genesis_key.prv);
	system.wallet (0)->insert_adhoc (key.prv);
	auto send (system.wallet (0)->send_action (lumex::dev::genesis_key.pub, lumex::dev::genesis_key.pub, lumex::Klumex_ratio));
	ASSERT_NE (nullptr, send);
	auto receive (system.wallet (0)->receive_action (send->hash (), key.pub, lumex::Klumex_ratio, send->destination ()));
	ASSERT_NE (nullptr, receive);
	auto change (system.wallet (0)->change_action (lumex::dev::genesis_key.pub, key.pub));
	ASSERT_NE (nullptr, change);
	auto const rpc_ctx = add_rpc (system, node1);
	boost::property_tree::ptree request;
	request.put ("action", "blocks_info");
	...
	ASSERT_EQ (change_successor, lumex::block_hash (0).to_string ()); // Change block doesn't have successor yet
}
...
TEST (rpc, block_info_pruning)
{
	...
	node_config0.receive_minimum = lumex::dev::constants.genesis_amount; // Prevent auto-receive & receive1 block conflicts
	auto & node0 = *system.add_node (node_config0);
	lumex::node_config node_config1 = system.default_config ();
	...
	bool json_error{ false };
	lumex::receive_block receive_from_json (json_error, response2.get_child ("contents"));
	ASSERT_FALSE (json_error);
	...
	ASSERT_EQ (successor_text, lumex::block_hash (0).to_string ()); // receive1 block doesn't have successor yet
}
...
TEST (rpc, populate_backlog)
{
	...
	lumex::block_builder builder;
	auto latest (node->latest (lumex::dev::genesis_key.pub));
	auto genesis_balance (lumex::dev::constants.genesis_amount);
...
TEST (rpc, ledger)
{
	...
	auto open = builder
				...
				.work (*node->work_generate_blocking (key.pub))
				.build ();
	ASSERT_EQ (lumex::block_status::progress, node->process (open));
	auto time = lumex::seconds_since_epoch ();
	auto const rpc_ctx = add_rpc (system, node);
	boost::property_tree::ptree request;
	...
	request.put ("representative", true);
	{
		...
		for (auto & account : response.get_child ("accounts"))
		{
			...
			ASSERT_EQ (lumex::dev::genesis_key.pub.to_account (), representative.value ());
		}
	}
	// Test threshold
	request.put ("count", 2);
...
TEST (rpc, block_create)
{
	...
	request.put ("work", lumex::to_string_hex (send_work));
	auto response (wait_response (system, rpc_ctx, request));
	std::string send_hash (response.get<std::string> ("hash"));
	ASSERT_EQ (send->hash ().to_string (), send_hash);
	std::string send_difficulty (response.get<std::string> ("difficulty"));
	ASSERT_EQ (lumex::to_string_hex (lumex::dev::network_params.work.difficulty (*send)), send_difficulty);
	auto send_text (response.get<std::string> ("block"));
	boost::property_tree::ptree block_l;
	std::stringstream block_stream (send_text);
	boost::property_tree::read_json (block_stream, block_l);
	auto send_block (lumex::deserialize_block_json (block_l));
	...
	request1.put ("representative", lumex::dev::genesis_key.pub.to_account ());
	request1.put ("source", send->hash ().to_string ());
	request1.put ("work", lumex::to_string_hex (open_work));
	auto response1 (wait_response (system, rpc_ctx, request1));
	...
	auto open_block (lumex::deserialize_block_json (block_l));
	...
	request1.put ("work", lumex::to_string_hex (change_work));
	...
	auto change_block (lumex::deserialize_block_json (block_l));
	ASSERT_EQ (change->hash (), change_block->hash ());
	ASSERT_EQ (lumex::block_status::progress, node1->process (change));
	auto send2 = builder
				 .send ()
				 .previous (send->hash ())
				 .destination (key.pub)
	...
	request2.put ("work", lumex::to_string_hex (*node1->work_generate_blocking (change->hash ())));
	...
	auto receive_block (lumex::deserialize_block_json (block_l));
...
TEST (rpc, block_create_state)
{
	...
	request.put ("wallet", node->wallets.items.begin ()->first.to_string ());
	request.put ("account", lumex::dev::genesis_key.pub.to_account ());
	request.put ("previous", lumex::dev::genesis->hash ().to_string ());
	request.put ("representative", lumex::dev::genesis_key.pub.to_account ());
	request.put ("balance", (lumex::dev::constants.genesis_amount - lumex::Klumex_ratio).convert_to<std::string> ());
	request.put ("link", key.pub.to_account ());
	request.put ("work", lumex::to_string_hex (*node->work_generate_blocking (lumex::dev::genesis->hash ())));
	auto response (wait_response (system, rpc_ctx, request));
	...
	boost::property_tree::ptree block_l;
	boost::property_tree::read_json (block_stream, block_l);
	auto state_block (lumex::deserialize_block_json (block_l));
	ASSERT_NE (nullptr, state_block);
	ASSERT_EQ (lumex::block_type::state, state_block->type ());
	ASSERT_EQ (state_hash, state_block->hash ().to_string ());
	auto process_result (node->process (state_block));
	ASSERT_EQ (lumex::block_status::progress, process_result);
}
...
TEST (rpc, block_create_state_open)
{
	...
	request.put ("representative", lumex::dev::genesis_key.pub.to_account ());
	request.put ("balance", lumex::Klumex_ratio.convert_to<std::string> ());
	request.put ("link", send_block->hash ().to_string ());
	request.put ("work", lumex::to_string_hex (*node->work_generate_blocking (key.pub)));
	auto response (wait_response (system, rpc_ctx, request));
	std::string state_hash (response.get<std::string> ("hash"));
	auto state_text (response.get<std::string> ("block"));
	std::stringstream block_stream (state_text);
	...
	ASSERT_EQ (lumex::block_type::state, state_block->type ());
	ASSERT_EQ (state_hash, state_block->hash ().to_string ());
	auto difficulty (lumex::dev::network_params.work.difficulty (*state_block));
	ASSERT_GT (difficulty, lumex::dev::network_params.work.threshold (state_block->work_version (), lumex::block_details (lumex::epoch::epoch_0, false, true, false)));
	ASSERT_TRUE (node->latest (key.pub).is_zero ());
	auto process_result (node->process (state_block));
	ASSERT_EQ (lumex::block_status::progress, process_result);
	ASSERT_EQ (state_block->sideband ().details.epoch, lumex::epoch::epoch_0);
...
TEST (rpc, block_create_state_request_work)
{
	...
	std::shared_ptr<lumex::state_block> epoch2;
	{
		lumex::test::system system (1);
		system.upgrade_genesis_epoch (*system.nodes.front (), lumex::epoch::epoch_1);
		epoch2 = system.upgrade_genesis_epoch (*system.nodes.front (), lumex::epoch::epoch_2);
	}
	...
	for (auto previous : previous_test_input)
	{
		...
		boost::property_tree::read_json (block_stream, block_l);
		auto block (lumex::deserialize_block_json (block_l));
		ASSERT_NE (nullptr, block);
		ASSERT_GE (lumex::dev::network_params.work.difficulty (*block), node->default_difficulty (lumex::work_version::work_1));
	}
}
...
TEST (rpc, block_create_open_epoch_v2)
{
	...
	ASSERT_EQ (state_hash, state_block->hash ().to_string ());
	auto difficulty (lumex::dev::network_params.work.difficulty (*state_block));
	ASSERT_GT (difficulty, lumex::dev::network_params.work.threshold (state_block->work_version (), lumex::block_details (lumex::epoch::epoch_2, false, true, false)));
	ASSERT_TRUE (node->latest (key.pub).is_zero ());
	auto process_result (node->process (state_block));
	ASSERT_EQ (lumex::block_status::progress, process_result);
	ASSERT_EQ (state_block->sideband ().details.epoch, lumex::epoch::epoch_2);
	ASSERT_TRUE (state_block->is_receive ());
	ASSERT_FALSE (node->latest (key.pub).is_zero ());
}
...
TEST (rpc, block_create_receive_epoch_v2)
{
	...
	ASSERT_EQ (lumex::block_status::progress, node->process (open));
	ASSERT_NE (nullptr, system.upgrade_genesis_epoch (*node, lumex::epoch::epoch_2));
	auto send_block_2 (system.wallet (0)->send_action (lumex::dev::genesis_key.pub, key.pub, lumex::Klumex_ratio));
	auto const rpc_ctx = add_rpc (system, node);
	boost::property_tree::ptree request;
	request.put ("action", "block_create");
	request.put ("type", "state");
	...
	request.put ("representative", lumex::dev::genesis_key.pub.to_account ());
	request.put ("balance", (2 * lumex::Klumex_ratio).convert_to<std::string> ());
...
TEST (rpc, block_create_send_epoch_v2)
{
	...
	request.put ("link", lumex::dev::genesis_key.pub.to_string ());
	auto response (wait_response (system, rpc_ctx, request));
	std::string state_hash (response.get<std::string> ("hash"));
	auto state_text (response.get<std::string> ("block"));
	std::stringstream block_stream (state_text);
	...
	ASSERT_EQ (state_hash, state_block->hash ().to_string ());
	auto difficulty (lumex::dev::network_params.work.difficulty (*state_block));
	ASSERT_GT (difficulty, lumex::dev::network_params.work.threshold (state_block->work_version (), lumex::block_details (lumex::epoch::epoch_2, true, false, false)));
	auto process_result (node->process (state_block));
	ASSERT_EQ (lumex::block_status::progress, process_result);
	ASSERT_EQ (state_block->sideband ().details.epoch, lumex::epoch::epoch_2);
...
TEST (rpc, wallet_create_fail)
{
	...
	for (int i = 0; i < 127; i++)
	{
		node->wallets.create (lumex::random_wallet_id ());
	}
	auto const rpc_ctx = add_rpc (system, node);
	boost::property_tree::ptree request;
	request.put ("action", "wallet_create");
	auto response (wait_response (system, rpc_ctx, request));
	ASSERT_EQ (std::error_code (lumex::error_common::wallet_lmdb_max_dbs).message (), response.get<std::string> ("error"));
}
...
TEST (rpc, wallet_add_watch)
{
	...
	std::string success (response.get<std::string> ("success"));
	ASSERT_TRUE (success.empty ());
	ASSERT_TRUE (system.wallet (0)->exists (lumex::dev::genesis_key.pub));

	// Make sure using special wallet key as pubkey fails
	lumex::public_key bad_key (1);
	...
	std::error_code ec (lumex::error_common::bad_public_key);
...
TEST (rpc, online_reps)
{
	...
	ASSERT_EQ (node2->online_reps.online (), lumex::dev::constants.genesis_amount - lumex::Klumex_ratio);
	auto const rpc_ctx = add_rpc (system, node2);
	boost::property_tree::ptree request;
	...
	ASSERT_EQ (lumex::dev::genesis_key.pub.to_account (), item->second.get<std::string> (""));
	...
	ASSERT_EQ (lumex::dev::genesis_key.pub.to_account (), item2->first);
	auto weight2 (item2->second.get<std::string> ("weight"));
	ASSERT_EQ (node2->weight (lumex::dev::genesis_key.pub).convert_to<std::string> (), weight2);
	// Test accounts filter
	auto new_rep = system.wallet (1)->deterministic_insert ().value ();
	auto send (system.wallet (0)->send_action (lumex::dev::genesis_key.pub, new_rep, node1->config.receive_minimum.number ()));
	ASSERT_NE (nullptr, send);
	...
	auto change (system.wallet (0)->change_action (lumex::dev::genesis_key.pub, new_rep));
	ASSERT_NE (nullptr, change);
...
TEST (rpc, confirmation_history)
{
	...
	auto block (system.wallet (0)->send_action (lumex::dev::genesis_key.pub, key.pub, lumex::Klumex_ratio));
	...
	lumex::amount tally_num;
	tally_num.decode_dec (tally);
	debug_assert (tally_num == lumex::dev::constants.genesis_amount || tally_num == (lumex::dev::constants.genesis_amount - lumex::Klumex_ratio));
	system.stop ();
}
...
TEST (rpc, confirmation_history_hash)
{
	...
	system.wallet (0)->insert_adhoc (lumex::dev::genesis_key.prv);
	ASSERT_TRUE (node->active.recently_cemented.list ().empty ());
	auto send1 (system.wallet (0)->send_action (lumex::dev::genesis_key.pub, key.pub, lumex::Klumex_ratio));
	auto send2 (system.wallet (0)->send_action (lumex::dev::genesis_key.pub, key.pub, lumex::Klumex_ratio));
	auto send3 (system.wallet (0)->send_action (lumex::dev::genesis_key.pub, key.pub, lumex::Klumex_ratio));
	...
	lumex::amount tally_num;
	tally_num.decode_dec (tally);
	debug_assert (tally_num == lumex::dev::constants.genesis_amount || tally_num == (lumex::dev::constants.genesis_amount - lumex::Klumex_ratio) || tally_num == (lumex::dev::constants.genesis_amount - 2 * lumex::Klumex_ratio) || tally_num == (lumex::dev::constants.genesis_amount - 3 * lumex::Klumex_ratio));
	system.stop ();
}
...
TEST (rpc, block_confirm_confirmed)
{
	...
	{
		auto transaction = node->ledger.tx_begin_read ();
		ASSERT_TRUE (node->ledger.cemented.block_exists_or_pruned (transaction, lumex::dev::genesis->hash ()));
	}
	ASSERT_EQ (0, node->stats.count (lumex::stat::type::http_callbacks_ec));
	auto const rpc_ctx = add_rpc (system, node);
	boost::property_tree::ptree request;
	request.put ("action", "block_confirm");
	request.put ("hash", lumex::dev::genesis->hash ().to_string ());
	...
	ASSERT_EQ (lumex::dev::genesis->hash (), confirmed.begin ()->winner->hash ());
	// Check callback
	...
	ASSERT_TIMELY (5s, node->stats.count (lumex::stat::type::http_callbacks_ec) != 0);
}
...
TEST (rpc, unchecked_get)
{
	...
	request.put ("hash", open->hash ().to_string ());
	{
		...
		auto timestamp (response.get<lumex::seconds_t> ("modified_timestamp"));
		ASSERT_LE (timestamp, lumex::seconds_since_epoch ());
	}
	request.put ("json_block", true);
	{
		auto response (wait_response (system, rpc_ctx, request));
...
TEST (rpc, unopened)
{
	...
	auto node = add_ipc_enabled_node (system);
	system.wallet (0)->insert_adhoc (lumex::dev::genesis_key.prv);
	lumex::account account1 (1), account2 (account1.number () + 1);
	auto genesis (node->latest (lumex::dev::genesis_key.pub));
	ASSERT_FALSE (genesis.is_zero ());
	auto send (system.wallet (0)->send_action (lumex::dev::genesis_key.pub, account1, 1));
	ASSERT_NE (nullptr, send);
	auto send2 (system.wallet (0)->send_action (lumex::dev::genesis_key.pub, account2, 10));
	ASSERT_NE (nullptr, send2);
	auto const rpc_ctx = add_rpc (system, node);
	{
		boost::property_tree::ptree request;
	...
	}
	{
		...
		boost::property_tree::ptree request;
		request.put ("action", "unopened");
		request.put ("account", lumex::account (account2.number () + 1).to_account ());
		auto response (wait_response (system, rpc_ctx, request));
		auto & accounts (response.get_child ("accounts"));
...
TEST (rpc, unopened_seek)
{
	...
	auto node = add_ipc_enabled_node (system);
	system.wallet (0)->insert_adhoc (lumex::dev::genesis_key.prv);
	lumex::account last_account{ std::numeric_limits<lumex::uint256_t>::max () };
	auto genesis (node->latest (lumex::dev::genesis_key.pub));
	ASSERT_FALSE (genesis.is_zero ());
	auto send (system.wallet (0)->send_action (lumex::dev::genesis_key.pub, last_account, 1));
	ASSERT_NE (nullptr, send);
	auto const rpc_ctx = add_rpc (system, node);
	{
		boost::property_tree::ptree request;
...
TEST (rpc, unopened_burn)
{
	...
	auto genesis (node->latest (lumex::dev::genesis_key.pub));
	ASSERT_FALSE (genesis.is_zero ());
	auto send (system.wallet (0)->send_action (lumex::dev::genesis_key.pub, lumex::dev::constants.burn_account, 1));
	ASSERT_NE (nullptr, send);
	auto const rpc_ctx = add_rpc (system, node);
	boost::property_tree::ptree request;
	request.put ("action", "unopened");
...
TEST (rpc, wallet_history)
{
	...
	auto node = add_ipc_enabled_node (system, node_config);
	system.wallet (0)->insert_adhoc (lumex::dev::genesis_key.prv);
	uint64_t timestamp = lumex::seconds_since_epoch ();
	auto send (system.wallet (0)->send_action (lumex::dev::genesis_key.pub, lumex::dev::genesis_key.pub, node->config.receive_minimum.number ()));
	ASSERT_NE (nullptr, send);
	auto receive (system.wallet (0)->receive_action (send->hash (), lumex::dev::genesis_key.pub, node->config.receive_minimum.number (), send->destination ()));
	ASSERT_NE (nullptr, receive);
	lumex::keypair key;
	auto send2 (system.wallet (0)->send_action (lumex::dev::genesis_key.pub, key.pub, node->config.receive_minimum.number ()));
	ASSERT_NE (nullptr, send2);
	system.deadline_set (10s);
	auto const rpc_ctx = add_rpc (system, node);
	boost::property_tree::ptree request;
	...
	ASSERT_EQ (lumex::dev::genesis_key.pub.to_account (), std::get<4> (history_l[0]));
	...
	ASSERT_EQ (lumex::dev::genesis_key.pub.to_account (), std::get<4> (history_l[1]));
	...
	ASSERT_EQ (lumex::dev::genesis_key.pub.to_account (), std::get<4> (history_l[2]));
	...
	ASSERT_EQ (lumex::dev::genesis_key.pub.to_account (), std::get<1> (history_l[3]));
	ASSERT_EQ (lumex::dev::constants.genesis_amount.convert_to<std::string> (), std::get<2> (history_l[3]));
	ASSERT_EQ (lumex::dev::genesis->hash ().to_string (), std::get<3> (history_l[3]));
	ASSERT_EQ (lumex::dev::genesis_key.pub.to_account (), std::get<4> (history_l[3]));
}
...
TEST (rpc, sign_hash)
{
	...
	request.put ("key", key.prv.to_string ());
	auto response (wait_response (system, rpc_ctx, request, 10s));
	std::error_code ec (lumex::error_rpc::sign_hash_disabled);
	ASSERT_EQ (response.get<std::string> ("error"), ec.message ());
	...
	ASSERT_FALSE (lumex::validate_message (key.pub, send->hash (), signature));
}
...
TEST (rpc, memory_stats)
{
	...
	auto vote = lumex::test::make_vote (key, { hashes }, 0, 0);
...
TEST (rpc, stats_samples)
{
	...
	node->stats.sample (lumex::stat::sample::active_election_duration, 1, { 0, 10 });
	node->stats.sample (lumex::stat::sample::active_election_duration, 2, { 0, 10 });
	node->stats.sample (lumex::stat::sample::active_election_duration, 3, { 0, 10 });
	node->stats.sample (lumex::stat::sample::active_election_duration, 4, { 0, 10 });

	node->stats.sample (lumex::stat::sample::bootstrap_tag_duration, 5, { 0, 999 });
	node->stats.sample (lumex::stat::sample::bootstrap_tag_duration, 5, { 0, 999 });
...
TEST (rpc, block_confirmed)
{
	...
	auto response (wait_response (system, rpc_ctx, request));
	ASSERT_EQ (std::error_code (lumex::error_blocks::invalid_block_hash).message (), response.get<std::string> ("error"));

	request.put ("hash", "0");
	auto response1 (wait_response (system, rpc_ctx, request));
	ASSERT_EQ (std::error_code (lumex::error_blocks::not_found).message (), response1.get<std::string> ("error"));
	...
	{
		auto transaction = node->ledger.tx_begin_write ();
		lumex::block_hash latest (node->ledger.any.account_head (transaction, lumex::dev::genesis_key.pub));
		auto send1 = builder
					 .send ()
					 .previous (latest)
					 .destination (key.pub)
		...
		auto open1 = builder
					 ...
					 .work (*system.work.generate (key.pub))
					 .build ();
		ASSERT_EQ (lumex::block_status::progress, node->ledger.process (transaction, open1));
	}
...
TEST (rpc, database_txn_tracker)
{
	// First try when database tracking is disabled
	{
		...
		{
			auto response (wait_response (system, rpc_ctx, request));
			std::error_code ec (lumex::error_common::tracking_not_enabled);
			ASSERT_EQ (response.get<std::string> ("error"), ec.message ());
		}
	}
	...
	auto check_not_correct_amount = [&system, &rpc_ctx, &request] () {
		auto response (wait_response (system, rpc_ctx, request));
		std::error_code ec (lumex::error_common::invalid_amount);
		ASSERT_EQ (response.get<std::string> ("error"), ec.message ());
	};
	...
	std::thread thread ([&store = node->store, &keep_txn_alive_promise, &txn_created_promise] () {
		...
		lumex::thread_role::set (lumex::thread_role::name::rpc_process_container);
	...
	auto thread_name = lumex::thread_role::get_string (lumex::thread_role::name::rpc_process_container);
...
TEST (rpc, active_difficulty)
{
	...
	auto node = add_ipc_enabled_node (system);
	auto const rpc_ctx = add_rpc (system, node);
	ASSERT_EQ (node->default_difficulty (lumex::work_version::work_1), node->network_params.work.epoch_2);
	boost::property_tree::ptree request;
	...
	auto expected_multiplier{ 1.0 };
	{
		...
		ASSERT_FALSE (lumex::from_string_hex (network_minimum_text, network_minimum));
		ASSERT_EQ (node->default_difficulty (lumex::work_version::work_1), network_minimum);
		auto network_receive_minimum_text (response.get<std::string> ("network_receive_minimum"));
		uint64_t network_receive_minimum;
		ASSERT_FALSE (lumex::from_string_hex (network_receive_minimum_text, network_receive_minimum));
		ASSERT_EQ (node->default_receive_difficulty (lumex::work_version::work_1), network_receive_minimum);
		auto multiplier (response.get<double> ("multiplier"));
		...
		ASSERT_FALSE (lumex::from_string_hex (network_current_text, network_current));
		ASSERT_EQ (lumex::difficulty::from_multiplier (expected_multiplier, node->default_difficulty (lumex::work_version::work_1)), network_current);
		auto network_receive_current_text (response.get<std::string> ("network_receive_current"));
		uint64_t network_receive_current;
		ASSERT_FALSE (lumex::from_string_hex (network_receive_current_text, network_receive_current));
		auto network_receive_current_multiplier (lumex::difficulty::to_multiplier (network_receive_current, network_receive_minimum));
		auto network_receive_current_normalized_multiplier (lumex::dev::network_params.work.normalized_multiplier (network_receive_current_multiplier, network_receive_minimum));
...
TEST (rpc, simultaneous_calls)
{
	...
	lumex::node_rpc_config node_rpc_config;
	lumex::ipc::ipc_server ipc_server (*node, node_rpc_config);
	lumex::rpc_config rpc_config{ lumex::dev::network_params.network, system.get_available_port (), true };
	...
	lumex::ipc_rpc_processor ipc_rpc_processor (system.io_ctx, rpc_config, ipc_tcp_port.value ());
	auto rpc = std::make_shared<lumex::rpc> (system.io_ctx, rpc_config, ipc_rpc_processor);
	lumex::test::start_stop_guard stop_guard{ *rpc };
...
TEST (rpc, deprecated_account_format)
{
	...
	std::string account_text (lumex::dev::genesis_key.pub.to_account ());
	account_text[4] = '-';
	request.put ("account", account_text);
	auto response2 (wait_response (system, rpc_ctx, request));
	std::string frontier (response2.get<std::string> ("frontier"));
	ASSERT_EQ (lumex::dev::genesis->hash ().to_string (), frontier);
...
TEST (rpc, epoch_upgrade)
{
	...
	auto send3 = builder
				 ...
				 .previous (send2->hash ())
				 .representative (lumex::dev::genesis_key.pub)
				 .balance (lumex::dev::constants.genesis_amount - 3)
				 .link (0)
				 .sign (lumex::dev::genesis_key.prv, lumex::dev::genesis_key.pub)
				 .work (*system.work.generate (send2->hash ()))
				 .build (); // to burn (0)
	ASSERT_EQ (lumex::block_status::progress, node->process (send3));
	lumex::account max_account (std::numeric_limits<lumex::uint256_t>::max ());
	auto send4 = builder
				 .state ()
				 .account (lumex::dev::genesis_key.pub)
				 .previous (send3->hash ())
				 .representative (lumex::dev::genesis_key.pub)
				 .balance (lumex::dev::constants.genesis_amount - 4)
				 .link (max_account)
				 .sign (lumex::dev::genesis_key.prv, lumex::dev::genesis_key.pub)
				 .work (*system.work.generate (send3->hash ()))
				 .build (); // to max account
	ASSERT_EQ (lumex::block_status::progress, node->process (send4));
	auto open = builder
				.state ()
				.account (key1.pub)
				.previous (0)
	...
	// Check accounts epochs
	{
		...
		{
			lumex::account_info info (i->second);
	...
	// Check upgrade
	{
		...
		ASSERT_TRUE (node->store.account.exists (transaction, std::numeric_limits<lumex::uint256_t>::max ()));
		ASSERT_FALSE (node->store.account.exists (transaction, 0));
	}

	// Epoch 2 upgrade
	auto genesis_latest (node->latest (lumex::dev::genesis_key.pub));
	auto send5 = builder
				 .state ()
				 .account (lumex::dev::genesis_key.pub)
				 .previous (genesis_latest)
				 .representative (lumex::dev::genesis_key.pub)
				 .balance (lumex::dev::constants.genesis_amount - 5)
				 .link (0)
				 .sign (lumex::dev::genesis_key.prv, lumex::dev::genesis_key.pub)
				 .work (*system.work.generate (genesis_latest))
				 .build (); // to burn (0)
	ASSERT_EQ (lumex::block_status::progress, node->process (send5));
	auto send6 = builder
				 .state ()
				 .account (lumex::dev::genesis_key.pub)
				 .previous (send5->hash ())
				 .representative (lumex::dev::genesis_key.pub)
				 .balance (lumex::dev::constants.genesis_amount - 6)
				 .link (key1.pub)
				 .sign (lumex::dev::genesis_key.prv, lumex::dev::genesis_key.pub)
				 .work (*system.work.generate (send5->hash ()))
				 .build (); // to key1 (again)
	ASSERT_EQ (lumex::block_status::progress, node->process (send6));
	...
	auto send7 = builder
				 ...
				 .build (); // to key3
	ASSERT_EQ (lumex::block_status::progress, node->process (send7));
	{
		// Check pending entry
		auto transaction = node->ledger.tx_begin_read ();
		auto info = node->ledger.any.pending_get (transaction, lumex::pending_key (key3.pub, send7->hash ()));
		ASSERT_TRUE (info);
...
TEST (rpc, receive)
{
	...
	auto send1 (wallet->send_action (lumex::dev::genesis_key.pub, key1.pub, node->config.receive_minimum.number (), *node->work_generate_blocking (lumex::dev::genesis->hash ())));
	ASSERT_TIMELY (5s, node->balance (lumex::dev::genesis_key.pub) != lumex::dev::constants.genesis_amount);
	ASSERT_TIMELY (10s, !node->store.account.exists (node->store.tx_begin_read (), key1.pub));
	// Send below minimum receive amount
	auto send2 (wallet->send_action (lumex::dev::genesis_key.pub, key1.pub, node->config.receive_minimum.number () - 1, *node->work_generate_blocking (send1->hash ())));
	auto const rpc_ctx = add_rpc (system, node);
	boost::property_tree::ptree request;
	...
	request.put ("block", send2->hash ().to_string ());
	{
		...
		ASSERT_TRUE (info);
		ASSERT_EQ (info->head, lumex::block_hash{ receive_text });
	}
	// Trying to receive the same block should fail with unreceivable
	{
		auto response (wait_response (system, rpc_ctx, request));
		ASSERT_EQ (std::error_code (lumex::error_process::unreceivable).message (), response.get<std::string> ("error"));
	}
	// Trying to receive a non-existing block should fail
	request.put ("block", lumex::block_hash (send2->hash ().number () + 1).to_string ());
	{
		auto response (wait_response (system, rpc_ctx, request));
		ASSERT_EQ (std::error_code (lumex::error_blocks::not_found).message (), response.get<std::string> ("error"));
	}
...
TEST (rpc, receive_unopened)
{
	...
	lumex::keypair key1;
	auto send1 (wallet->send_action (lumex::dev::genesis_key.pub, key1.pub, node->config.receive_minimum.number () - 1, *node->work_generate_blocking (lumex::dev::genesis->hash ())));
	ASSERT_TIMELY (5s, !node->balance (lumex::dev::genesis_key.pub) != lumex::dev::constants.genesis_amount);
	...
	request.put ("block", send1->hash ().to_string ());
	{
		...
		ASSERT_EQ (info->representative, lumex::dev::genesis_key.pub);
	}

	// Test receiving for an unopened with a different wallet representative
	lumex::keypair key2;
	auto prev_amount (node->balance (lumex::dev::genesis_key.pub));
	auto send2 (wallet->send_action (lumex::dev::genesis_key.pub, key2.pub, node->config.receive_minimum.number () - 1, *node->work_generate_blocking (send1->hash ())));
	ASSERT_TIMELY (5s, !node->balance (lumex::dev::genesis_key.pub) != prev_amount);
...
TEST (rpc, receive_work_disabled)
{
	...
	auto send1 (wallet->send_action (lumex::dev::genesis_key.pub, key1.pub, node->config.receive_minimum.number () - 1, *worker_node.work_generate_blocking (lumex::dev::genesis->hash ()), false));
...
TEST (rpc, receive_pruned)
{
	...
	auto send1 (wallet1->send_action (lumex::dev::genesis_key.pub, key1.pub, node2->config.receive_minimum.number (), *node2->work_generate_blocking (lumex::dev::genesis->hash ())));
	ASSERT_TIMELY (5s, node2->balance (lumex::dev::genesis_key.pub) != lumex::dev::constants.genesis_amount);
	ASSERT_TIMELY (10s, node2->store.account.exists (node2->store.tx_begin_read (), key1.pub));
	// Send below minimum receive amount
	auto send2 (wallet1->send_action (lumex::dev::genesis_key.pub, key1.pub, node2->config.receive_minimum.number () - 1, *node2->work_generate_blocking (send1->hash ())));
	// Extra send frontier
	auto send3 (wallet1->send_action (lumex::dev::genesis_key.pub, key1.pub, node2->config.receive_minimum.number (), *node2->work_generate_blocking (send1->hash ())));
	// Pruning
...
TEST (rpc, telemetry_single)
{
	...
	{
		auto response (wait_response (system, rpc_ctx, request, 10s));
		ASSERT_EQ (std::error_code (lumex::error_rpc::requires_port_and_address).message (), response.get<std::string> ("error"));
	}
	...
	{
		auto response (wait_response (system, rpc_ctx, request, 10s));
		ASSERT_EQ (std::error_code (lumex::error_common::invalid_ip_address).message (), response.get<std::string> ("error"));
	}
	...
	{
		auto response (wait_response (system, rpc_ctx, request, 10s));
		ASSERT_EQ (std::error_code (lumex::error_common::invalid_port).message (), response.get<std::string> ("error"));
	}
	...
	request.put ("port", node->network.endpoint ().port ());
	{
		...
		lumex::jsonconfig config (response);
		...
		ASSERT_TRUE (lumex::test::compare_telemetry (telemetry_data, *node));
	}
}
...
TEST (rpc, telemetry_all)
{
	...
	request.put ("action", "telemetry");
	{
		...
		ASSERT_FALSE (telemetry_data.deserialize_json (config, should_ignore_identification_metrics));
		ASSERT_TRUE (lumex::test::compare_telemetry_data (telemetry_data, node->local_telemetry ()));
	}
...
TEST (rpc, telemetry_self)
{
	...
	lumex::test::establish_tcp (system, *node1, outer_node->network.endpoint ());
	...
	auto const should_ignore_identification_metrics = false;
	{
		...
		lumex::jsonconfig config (response);
		ASSERT_FALSE (data.deserialize_json (config, should_ignore_identification_metrics));
		ASSERT_TRUE (lumex::test::compare_telemetry (data, *node1));
	}
	...
	{
		auto response (wait_response (system, rpc_ctx, request, 10s));
		ASSERT_EQ (std::error_code (lumex::error_rpc::peer_not_found).message (), response.get<std::string> ("error"));
	}
}
...
TEST (rpc, confirmation_active)
{
	...
	auto send2 = builder
				 ...
				 .work (*system.work.generate (send1->hash ()))
				 .build ();
	lumex::test::process (*node1, { send1, send2 });
...
TEST (rpc, confirmation_info)
{
	...
	auto rep1 = lumex::test::setup_rep (system, *node, 100 * lumex::Klumex_ratio);
	auto rep2 = lumex::test::setup_rep (system, *node, 1000 * lumex::Klumex_ratio);
	auto rep3 = lumex::test::setup_rep (system, *node, 2000 * lumex::Klumex_ratio);
	auto rep4 = lumex::keypair{}; // Representative with zero weight
	auto rep5 = lumex::test::setup_rep (system, *node, 1500 * lumex::Klumex_ratio); // Additional final representative

	lumex::block_builder builder;
	auto send = builder
				.state ()
				.account (lumex::dev::genesis_key.pub)
				.previous (node->latest (lumex::dev::genesis_key.pub))
				.link (lumex::public_key{})
				.representative (lumex::dev::genesis_key.pub)
				.balance (lumex::dev::constants.genesis_amount - 10000 * lumex::Klumex_ratio)
				.sign (lumex::dev::genesis_key.prv, lumex::dev::genesis_key.pub)
				.work (*system.work.generate (node->latest (lumex::dev::genesis_key.pub)))
				.build ();
	...
	auto vote1 = lumex::test::make_vote (rep1, { send });
	auto vote2 = lumex::test::make_vote (rep2, { send });
	auto vote3 = lumex::test::make_final_vote (rep3, { send });
	auto vote4 = lumex::test::make_vote (rep4, { send }); // Add vote from zero-weight representative
	auto vote5 = lumex::test::make_final_vote (rep5, { send }); // Add another final vote

	node->vote_processor.vote_blocking (vote1, lumex::test::fake_channel (*node));
	node->vote_processor.vote_blocking (vote2, lumex::test::fake_channel (*node));
	node->vote_processor.vote_blocking (vote3, lumex::test::fake_channel (*node));
	node->vote_processor.vote_blocking (vote4, lumex::test::fake_channel (*node));
	node->vote_processor.vote_blocking (vote5, lumex::test::fake_channel (*node));
...
TEST (rpc, bootstrap_priorities)
{
	...
	auto chains = lumex::test::setup_chains (system, node_server, chain_count, blocks_per_chain);
