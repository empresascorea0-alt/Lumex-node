...
class json_handler : public std::enable_shared_from_this<lumex::json_handler>
{
public:
	json_handler (
	lumex::node &, lumex::node_rpc_config const &, std::string const &, std::function<void (std::string const &)> const &, std::function<void ()> stop_callback = [] () {});
	...
	std::shared_ptr<lumex::wallet::wallet> wallet_impl ();
	bool wallet_locked_impl (std::shared_ptr<lumex::wallet::wallet> const &);
	bool wallet_account_impl (std::shared_ptr<lumex::wallet::wallet> const &, lumex::account const &);
	lumex::account account_impl (std::string = "", std::error_code = lumex::error_common::bad_account_number);
	lumex::account_info account_info_impl (secure::transaction const &, lumex::account const &);
	lumex::amount amount_impl ();
	std::shared_ptr<lumex::block> block_impl (bool = true);
	lumex::block_hash hash_impl (std::string = "hash");
	lumex::amount threshold_optional_impl ();
	...
	uint64_t difficulty_optional_impl (lumex::work_version const);
	uint64_t difficulty_ledger (lumex::block const &);
	double multiplier_optional_impl (lumex::work_version const, uint64_t &);
	lumex::work_version work_version_optional_impl (lumex::work_version const default_a);
	bool enable_sign_hash{ false };
	std::function<void ()> stop_callback;
	lumex::node_rpc_config const & node_rpc_config;
	std::function<void ()> create_worker_task (std::function<void (std::shared_ptr<lumex::json_handler> const &)> const &);
};

class inprocess_rpc_handler final : public lumex::rpc_handler_interface
{
public:
	inprocess_rpc_handler (
	lumex::node & node_a, lumex::ipc::ipc_server & ipc_server_a, lumex::node_rpc_config const & node_rpc_config_a, std::function<void ()> stop_callback_a = [] () {}) :
		node (node_a),
	...
	void rpc_instance (lumex::rpc & rpc_a) override
	{
		rpc = rpc_a;
	}
...
private:
	...
	std::optional<std::reference_wrapper<lumex::rpc>> rpc;
	std::function<void ()> stop_callback;
	lumex::node_rpc_config const & node_rpc_config;
};
}
