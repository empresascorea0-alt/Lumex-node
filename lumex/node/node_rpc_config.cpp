...
lumex::error lumex::node_rpc_config::serialize_toml (lumex::tomlconfig & toml) const
{
	toml.put ("enable_sign_hash", enable_sign_hash, "Allow or disallow signing of hashes.\ntype:bool");

	lumex::tomlconfig child_process_l;
	...
	child_process_l.put ("rpc_path", child_process.rpc_path, "Path to the lumex_rpc executable. Must be set if child process is enabled.\ntype:string,path");
	toml.put_child ("child_process", child_process_l);
	return toml.get_error ();
}

lumex::error lumex::node_rpc_config::deserialize_toml (lumex::tomlconfig & toml)
{
...
void lumex::node_rpc_config::set_request_callback (std::function<void (boost::property_tree::ptree const &)> callback_a)
{
