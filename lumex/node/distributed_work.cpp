...
std::shared_ptr<request_type> lumex::distributed_work::peer_request::get_prepared_json_request (std::string const & request_string_a) const
{
...
lumex::distributed_work::distributed_work (lumex::node & node_a, lumex::work_request const & request_a, std::chrono::seconds const & backoff_a) :
	node (node_a),
	...
	elapsed (lumex::timer_state::started, "distributed work generation timer")
{
	debug_assert (!finished);
	debug_assert (status == work_generation_status::ongoing);
}

lumex::distributed_work::~distributed_work ()
{
	debug_assert (status != work_generation_status::ongoing);
	if (auto node_l = node_w.lock ())
	{
		if (!node_l->stopped && node_l->websocket.server && node_l->websocket.server->any_subscriber (lumex::websocket::topic::work))
		{
			lumex::websocket::message_builder builder{ node_l->ledger };
...
void lumex::distributed_work::start ()
{
	...
	for (auto const & peer : need_resolve)
	{
		...
		if (!ec)
		{
			do_request (lumex::tcp_endpoint (parsed_address, peer.second));
		}
		else
		{
			...
			node.network.resolver.async_resolve (peer.first, std::to_string (peer.second), [peer, this_l, &extra = resolved_extra] (boost::system::error_code const & ec, boost::asio::ip::tcp::resolver::results_type results) {
				if (!ec)
				{
					...
					for (auto const & i : results)
					{
						...
						first = false;
						this_l->do_request (lumex::tcp_endpoint (i.endpoint ().address (), i.endpoint ().port ()));
					}
				}
				else
				{
					this_l->node.logger.error (lumex::log::type::distributed_work, "Error resolving work peer: {}:{} ({})",
...
void lumex::distributed_work::start_local ()
{
	auto this_l (shared_from_this ());
...
void lumex::distributed_work::do_request (lumex::tcp_endpoint const & endpoint_a)
{
	auto this_l (shared_from_this ());
	...
	[this_l, connection] (boost::system::error_code const & ec) {
		if (!ec && !this_l->stopped)
		{
			std::string request_string;
			{
				...
				rpc_request.put ("difficulty", lumex::to_string_hex (this_l->request.difficulty));
			...
			[this_l, connection, peer_request] (boost::system::error_code const & ec, std::size_t size_a) {
				if (!ec && !this_l->stopped)
				{
					...
					boost::asio::bind_executor (this_l->strand, [this_l, connection] (boost::system::error_code const & ec, std::size_t size_a) {
						if (!ec && !this_l->stopped)
						{
							...
							{
								this_l->node.logger.error (lumex::log::type::distributed_work, "Work peer responded with an error {} ({})",
								connection->endpoint,
								ec.message ());
				...
				{
					this_l->node.logger.error (lumex::log::type::distributed_work, "Unable to write to work peer {} ({})",
					connection->endpoint,
					ec.message ());
		...
		{
			this_l->node.logger.error (lumex::log::type::distributed_work, "Unable to connect to work peer {} ({})",
			connection->endpoint,
			ec.message ());
...
void lumex::distributed_work::do_cancel (lumex::tcp_endpoint const & endpoint_a)
{
	auto this_l (shared_from_this ());
	...
	[this_l, cancelling_l] (boost::system::error_code const & ec) {
		if (!ec)
		{
			...
			[this_l, peer_cancel, cancelling_l] (boost::system::error_code const & ec, std::size_t bytes_transferred) {
				...
				{
					this_l->node.logger.error (lumex::log::type::distributed_work, "Unable to send work cancel to work peer {} ({})",
...
void lumex::distributed_work::success (std::string const & body_a, lumex::tcp_endpoint const & endpoint_a)
{
	bool error = true;
	try
	{
		...
		uint64_t work;
		if (!lumex::from_string_hex (work_text, work))
		{
			if (lumex::dev::network_params.work.difficulty (request.version, request.root, work) >= request.difficulty)
			{
				error = false;
			...
			{
				node.logger.error (lumex::log::type::distributed_work, "Incorrect work response from {} for root {} with difficulty {}: {}",
				endpoint_a,
				request.root,
				lumex::to_string_hex (request.difficulty),
				work_text);
			}
		}
		else
		{
			node.logger.error (lumex::log::type::distributed_work, "Work response from {} wasn't a number: {}",
			endpoint_a,
			work_text);
		}
	}
	...
	{
		node.logger.error (lumex::log::type::distributed_work, "Work response from {} wasn't parsable: {}",
		endpoint_a,
		body_a);
	}
	if (error)
...
void lumex::distributed_work::stop_once (bool const local_stop_a)
{
				...
				boost::asio::post (strand, boost::asio::bind_executor (strand, [this_l, connection_l] {
					...
					if (connection_l->socket.is_open ())
					{
						...
						if (!ec)
						{
							...
							if (ec)
							{
								this_l->node.logger.error (lumex::log::type::distributed_work, "Error closing socket with work peer: {} ({})",
						...
						{
							this_l->node.logger.error (lumex::log::type::distributed_work, "Error cancelling operation with work peer: {} ({})",
							connection_l->endpoint,
							ec.message ());
						}
					}
...
void lumex::distributed_work::set_once (uint64_t const work_a, std::string const & source_a)
{
	if (!finished.exchange (true))
	{
		elapsed.stop ();

		node.logger.info (lumex::log::type::distributed_work, "Work generation for {}, with a threshold difficulty of {} (multiplier {}x) complete: {} ms",
		request.root,
		lumex::to_string_hex (request.difficulty),
		lumex::to_string (lumex::difficulty::to_multiplier (request.difficulty, node.default_difficulty (request.version)), 2),
		elapsed.value ().count ());
...
void lumex::distributed_work::cancel ()
{
	if (!finished.exchange (true))
	{
		elapsed.stop ();

		node.logger.info (lumex::log::type::distributed_work, "Work generation for {} was cancelled after {} ms",
		request.root,
		elapsed.value ().count ());
...
void lumex::distributed_work::failure ()
{
...
void lumex::distributed_work::handle_failure ()
{
	if (!finished)
	{
		...
		{
			node.logger.info (lumex::log::type::distributed_work, "Work peer(s) failed to generate work for root {}, retrying... (backoff: {}s)",
			request.root,
			backoff.count ());

			status = work_generation_status::failure_peers;

			std::weak_ptr<lumex::node> node_weak (node.shared ());
...
void lumex::distributed_work::add_bad_peer (lumex::tcp_endpoint const & endpoint_a)
{
	lumex::lock_guard<lumex::mutex> guard{ mutex };
