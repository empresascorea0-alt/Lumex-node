...
bool lumex::opencl_loaded{ true };
#else
bool lumex::opencl_loaded{ false };
#endif
...
enum IV_Derived {
    lumex_xor_iv0 = 0x6a09e667f2bdc900UL,  // iv1 ^ 0x1010000 ^ outlen
    lumex_xor_iv4 = 0x510e527fade682f9UL,  // iv4 ^ inbytes
    lumex_xor_iv6 = 0xe07c265404be4294UL,  // iv6 ^ ~0
};
...
{
    ulong2 vv[8] = {
        {lumex_xor_iv0, iv1}, {iv2, iv3},          {iv4, iv5},
        {iv6, iv7},          {iv0, iv1},          {iv2, iv3},
        {lumex_xor_iv4, iv5}, {lumex_xor_iv6, iv7},
    };
    ...
    return lumex_xor_iv0 ^ vv[0].s0 ^ vv[4].s0;
}
#undef G32
#undef G2v
#undef G2v_split
...
__kernel void lumex_work(__constant ulong *attempt,
...
lumex::opencl_environment::opencl_environment (bool & error_a)
{
	if (lumex::opencl_loaded)
	{
		...
		{
			lumex::opencl_platform platform;
			platform.platform = *i;
...
void lumex::opencl_environment::dump (std::ostream & stream)
{
	if (lumex::opencl_loaded)
	{
		auto index (0);
...
lumex::opencl_work::opencl_work (bool & error_a, lumex::opencl_config const & config_a, lumex::opencl_environment & environment_a, lumex::logger & logger_a, lumex::work_thresholds & work) :
	config (config_a),
	context (0),
	attempt_buffer (0),
	result_buffer (0),
	...
	work{ work }
{
	...
	if (!error_a)
	{
		...
		if (!error_a)
		{
			lumex::random_pool::generate_block (reinterpret_cast<uint8_t *> (rand.s.data ()), rand.s.size () * sizeof (decltype (rand.s)::value_type));
			...
			if (!error_a)
			{
				...
				if (!error_a)
				{
					...
					if (!error_a)
					{
						...
						{
							cl_int item_error (0);
							std::size_t item_size (sizeof (lumex::uint256_union));
								...
								if (!error_a)
								{
									...
									if (!error_a)
									{
										...
										if (!error_a)
										{
											...
											kernel = clCreateKernel (program, "lumex_work", &kernel_error);
												...
												if (!error_a)
												{
													...
													if (!error_a)
													{
														...
														if (!error_a)
														{
															...
															{
																logger.error (lumex::log::type::opencl_work, "Bind argument 3 error: {}", arg3_error);
															}
														}
														else
														{
															logger.error (lumex::log::type::opencl_work, "Bind argument 2 error: {}", arg2_error);
														}
													}
													else
													{
														logger.error (lumex::log::type::opencl_work, "Bind argument 1 error: {}", arg1_error);
													}
												}
												else
												{
													logger.error (lumex::log::type::opencl_work, "Bind argument 0 error: {}", arg0_error);
												}
											}
											else
											{
												logger.error (lumex::log::type::opencl_work, "Create kernel error: {}", kernel_error);
											}
										}
										else
										{
											logger.error (lumex::log::type::opencl_work, "Build program error: {}", clBuildProgramError);
											for (auto i (selected_devices.begin ()), n (selected_devices.end ()); i != n; ++i)
											{
												...
												logger.info (lumex::log::type::opencl_work, "Device log: {}", log.data ());
											}
										}
									}
									else
									{
										logger.error (lumex::log::type::opencl_work, "Create program error: {}", program_error);
									}
								}
								else
								{
									logger.error (lumex::log::type::opencl_work, "Difficulty buffer error: {}", difficulty_error);
								}
							}
							else
							{
								logger.error (lumex::log::type::opencl_work, "Item buffer error: {}", item_error);
							}
						}
						else
						{
							logger.error (lumex::log::type::opencl_work, "Result buffer error: {}", result_error);
						}
					}
					else
					{
						logger.error (lumex::log::type::opencl_work, "Attempt buffer error: {}", attempt_error);
					}
				}
				else
				{
					logger.error (lumex::log::type::opencl_work, "Unable to create command queue: {}", queue_error);
				}
			}
			else
			{
				logger.error (lumex::log::type::opencl_work, "Unable to create context: {}", createContextError);
			}
		}
		else
		{
			logger.error (lumex::log::type::opencl_work, "Requested device {} and only have {}", config.device, platform.devices.size ());
		}
	}
	else
	{
		logger.error (lumex::log::type::opencl_work, "Requested platform {} and only have {}", config.platform, environment_a.platforms.size ());
	}
}

lumex::opencl_work::~opencl_work ()
{
	if (kernel != 0)
	{
...
std::optional<uint64_t> lumex::opencl_work::generate_work (lumex::work_version const version_a, lumex::root const & root_a, uint64_t const difficulty_a)
{
	std::atomic<int> ticket_l{ 0 };
	return generate_work (version_a, root_a, difficulty_a, ticket_l);
}

std::optional<uint64_t> lumex::opencl_work::generate_work (lumex::work_version const version_a, lumex::root const & root_a, uint64_t const difficulty_a, std::atomic<int> & ticket_a)
{
	...
	while (work.difficulty (version_a, root_a, result) < difficulty_a && !error && ticket_a == ticket_l)
	{
		...
		{
			cl_int write_error2 = clEnqueueWriteBuffer (queue, item_buffer, false, 0, sizeof (lumex::root), root_a.bytes.data (), 0, nullptr, nullptr);
				...
				if (write_error3 == CL_SUCCESS)
				{
					...
					if (enqueue_error == CL_SUCCESS)
					{
						...
						if (read_error1 == CL_SUCCESS)
						{
							...
							{
								error = true;
								logger.error (lumex::log::type::opencl_work, "Error finishing queue: {}", finishError);
							}
						}
						else
						{
							error = true;
							logger.error (lumex::log::type::opencl_work, "Error reading result: {}", read_error1);
						}
					}
					else
					{
						error = true;
						logger.error (lumex::log::type::opencl_work, "Error enqueueing kernel: {}", enqueue_error);
					}
				}
				else
				{
					error = true;
					logger.error (lumex::log::type::opencl_work, "Error writing difficulty: {}", write_error3);
				}
			}
			else
			{
				error = true;
				logger.error (lumex::log::type::opencl_work, "Error writing item: {}", write_error2);
			}
		}
		else
		{
			error = true;
			logger.error (lumex::log::type::opencl_work, "Error writing attempt: {}", write_error1);
		}
	}
...
std::unique_ptr<lumex::opencl_work> lumex::opencl_work::create (bool create_a, lumex::opencl_config const & config_a, lumex::logger & logger_a, lumex::work_thresholds & work)
{
	std::unique_ptr<lumex::opencl_work> result;
	if (create_a)
	{
		auto error (false);
		...
		logger_a.info (lumex::log::type::opencl_work, "OpenCL environment: {}", stream.str ());

		if (!error)
		{
			result.reset (new lumex::opencl_work (error, config_a, environment, logger_a, work));
			if (error)
			{
				result.reset ();
			}
