...
lumex::transport::tcp_channel::tcp_channel (lumex::node & node_a, std::shared_ptr<lumex::transport::tcp_socket> socket_a) :
	channel (node_a),
	socket{ socket_a },
...
lumex::transport::tcp_channel::~tcp_channel ()
{
	close ();
}

void lumex::transport::tcp_channel::close ()
{
	stop ();
	socket->close ();
}

void lumex::transport::tcp_channel::close_async ()
{
	socket->close_async ();
}

void lumex::transport::tcp_channel::start ()
{
	sending_task = lumex::async::task (strand, [this] (lumex::async::condition & condition) {
		return start_sending (condition); // This is not a coroutine, but a corotuine factory
	});
}

asio::awaitable<void> lumex::transport::tcp_channel::start_sending (lumex::async::condition & condition)
{
...
void lumex::transport::tcp_channel::stop ()
{
...
bool lumex::transport::tcp_channel::max (lumex::transport::traffic_type traffic_type)
{
	lumex::lock_guard<lumex::mutex> guard{ mutex };
	return queue.max (traffic_type);
}

bool lumex::transport::tcp_channel::send_impl (lumex::messages::message const & message, lumex::transport::traffic_type type, lumex::transport::channel::callback_t callback)
{
	...
	if (!queue.full (type))
	{
		...
		node.stats.inc (lumex::stat::type::tcp_channel, lumex::stat::detail::queued, lumex::stat::dir::out);
		node.stats.inc (lumex::stat::type::tcp_channel_queued, to_stat_detail (type), lumex::stat::dir::out);
	...
	else
	{
		...
		node.stats.inc (lumex::stat::type::tcp_channel, lumex::stat::detail::drop, lumex::stat::dir::out);
		node.stats.inc (lumex::stat::type::tcp_channel_drop, to_stat_detail (type), lumex::stat::dir::out);
	}
	return false;
}

asio::awaitable<void> lumex::transport::tcp_channel::run_sending (lumex::async::condition & condition)
{
...
asio::awaitable<boost::system::error_code> lumex::transport::tcp_channel::send_one (traffic_type type, tcp_channel_queue::entry_t const & item)
{
	debug_assert (strand.running_in_this_thread ());
	...
	while (allocated_bandwidth < size)
	{
		...
		{
			node.stats.inc (lumex::stat::type::tcp_channel_wait, lumex::stat::detail::wait_bandwidth, lumex::stat::dir::out);
			co_await lumex::async::sleep_for (100ms); // TODO: Exponential backoff
		}
	}
	allocated_bandwidth -= size;

	node.stats.inc (lumex::stat::type::tcp_channel, lumex::stat::detail::send, lumex::stat::dir::out);
	node.stats.inc (lumex::stat::type::tcp_channel_send, to_stat_detail (type), lumex::stat::dir::out);
	...
	if (!ec)
	{
		node.stats.add (lumex::stat::type::traffic_tcp_type, to_stat_detail (type), lumex::stat::dir::out, size_written);
		set_last_packet_sent (std::chrono::steady_clock::now ());
	}
	else
	{
		node.stats.inc (lumex::stat::type::tcp_channel_ec, lumex::to_stat_detail (ec), lumex::stat::dir::out);
	}
...
bool lumex::transport::tcp_channel::alive () const
{
	return socket->alive ();
}

lumex::endpoint lumex::transport::tcp_channel::get_remote_endpoint () const
{
	lumex::lock_guard<lumex::mutex> lock{ mutex };
	return remote_endpoint;
}

lumex::endpoint lumex::transport::tcp_channel::get_local_endpoint () const
{
	lumex::lock_guard<lumex::mutex> lock{ mutex };
	return local_endpoint;
}

std::string lumex::transport::tcp_channel::to_string () const
{
	return lumex::util::to_str (get_remote_endpoint ());
}

void lumex::transport::tcp_channel::operator() (lumex::object_stream & obs) const
{
	lumex::transport::channel::operator() (obs); // Write common data
...
lumex::transport::tcp_channel_queue::tcp_channel_queue ()
{
...
bool lumex::transport::tcp_channel_queue::empty () const
{
	return total_size == 0;
}

size_t lumex::transport::tcp_channel_queue::size () const
{
	return total_size;
}

size_t lumex::transport::tcp_channel_queue::size (traffic_type type) const
{
	return queues.at (type).second.size ();
}

bool lumex::transport::tcp_channel_queue::max (traffic_type type) const
{
	return size (type) >= max_size;
}

bool lumex::transport::tcp_channel_queue::full (traffic_type type) const
{
	return size (type) >= full_size;
}

void lumex::transport::tcp_channel_queue::push (traffic_type type, entry_t entry)
{
...
auto lumex::transport::tcp_channel_queue::next () -> value_t
{
...
auto lumex::transport::tcp_channel_queue::next_batch (size_t max_count) -> batch_t
{
...
size_t lumex::transport::tcp_channel_queue::priority (traffic_type type) const
{
	switch (type)
	{
