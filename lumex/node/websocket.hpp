...
#include <lumex/node/websocket_stream.hpp>
...
namespace websocket
{
	...
	{
	public:
		message (lumex::websocket::topic topic_a) :
			topic (topic_a)
		{
		}
		message (lumex::websocket::topic topic_a, boost::property_tree::ptree & tree_a) :
			topic (topic_a), contents (tree_a)
		{
		}

		std::string to_string () const;
		lumex::websocket::topic topic;
	...
	{
	public:
		message_builder (lumex::ledger & ledger);

		message block_confirmed (std::shared_ptr<lumex::block> const & block, lumex::account const & account, lumex::amount const & amount, std::string subtype, lumex::election_status const & election_status, std::vector<lumex::vote_with_weight_info> const & election_votes, lumex::websocket::confirmation_options const & options);
		message started_election (lumex::block_hash const & hash);
		message stopped_election (lumex::block_hash const & hash);
		message vote_received (std::shared_ptr<lumex::vote> const & vote, lumex::vote_code code);
		message work_generation (lumex::work_version const version, lumex::block_hash const & root, uint64_t work, uint64_t difficulty, uint64_t publish_threshold, std::chrono::milliseconds const & duration, std::string const & peer, std::vector<std::string> const & bad_peers, bool completed = true, bool cancelled = false);
		message work_cancelled (lumex::work_version const version, lumex::block_hash const & root, uint64_t difficulty, uint64_t publish_threshold, std::chrono::milliseconds const & duration, std::vector<std::string> const & bad_peers);
		message work_failed (lumex::work_version const version, lumex::block_hash const & root, uint64_t difficulty, uint64_t publish_threshold, std::chrono::milliseconds const & duration, std::vector<std::string> const & bad_peers);
		...
		message telemetry_received (lumex::messages::telemetry_data const &, lumex::endpoint const &);
		message new_block_arrived (lumex::block const & block_a);
	...
	{
	public:
		confirmation_options (lumex::wallet::wallets & wallets_a, lumex::logger &);
		confirmation_options (boost::property_tree::ptree const & options_a, lumex::wallet::wallets & wallets_a, lumex::logger &);
	...
	{
	public:
		vote_options (boost::property_tree::ptree const & options_a, lumex::logger &);
	...
	public:
		...
		explicit session (lumex::websocket::listener & listener_a, socket_type socket_a, boost::asio::ssl::context & ctx_a);
#endif
		/** Constructor that takes ownership over \p socket_a */
		explicit session (lumex::websocket::listener & listener_a, socket_type socket_a, lumex::logger &);
		...
		void write (lumex::websocket::message message_a);

	private:
		/** The owning listener */
		lumex::websocket::listener & ws_listener;
		/** Websocket stream, supporting both plain and tls connections */
		lumex::websocket::stream ws;
		lumex::logger & logger;
		...
		lumex::mutex subscriptions_mutex;
	...
	{
	public:
		listener (lumex::logger &, lumex::node &, lumex::wallet::wallets & wallets_a, boost::asio::io_context & io_ctx_a, boost::asio::ip::tcp::endpoint endpoint_a);
		...
		void broadcast_confirmation (std::shared_ptr<lumex::block> const & block, lumex::account const & account, lumex::amount const & amount, std::string const & subtype, lumex::election_status const & election_status, std::vector<lumex::vote_with_weight_info> const & election_votes);

		/** Broadcast \p message to all session subscribing to the message topic. */
		void broadcast (lumex::websocket::message message_a);
		...
		lumex::wallet::wallets & get_wallets () const
		{
			return wallets;
		}
		 ...
		 */
		bool any_subscriber (lumex::websocket::topic const & topic_a) const
		{
			return subscriber_count (topic_a) > 0;
		}
		/** Getter for subscriber count of a specific topic*/
		std::size_t subscriber_count (lumex::websocket::topic const & topic_a) const
		{
	...
	private:
		...
		friend lumex::websocket::session;

		/** Adds to subscription count of a specific topic*/
		void increase_subscriber_count (lumex::websocket::topic const & topic_a);
		/** Removes from subscription count of a specific topic*/
		void decrease_subscriber_count (lumex::websocket::topic const & topic_a);
...
{
public:
	websocket_server (lumex::websocket::config &, lumex::node &, lumex::node_observers &, lumex::wallet::wallets &, lumex::ledger &, boost::asio::io_context &, lumex::logger &);

	void start ();
	void stop ();

private: // Dependencies
	lumex::websocket::config const & config;
...
public:
	...
	std::shared_ptr<lumex::websocket::listener> server;
};
} // namespace lumex
