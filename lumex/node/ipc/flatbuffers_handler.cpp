...
namespace
{
auto handler_map = lumex::ipc::action_handler::handler_map ();
...
lumex::ipc::flatbuffers_handler::flatbuffers_handler (lumex::node & node_a, lumex::ipc::ipc_server & ipc_server_a, std::shared_ptr<lumex::ipc::subscriber> const & subscriber_a, lumex::ipc::ipc_config const & ipc_config_a) :
	node (node_a),
...
std::shared_ptr<flatbuffers::Parser> lumex::ipc::flatbuffers_handler::make_flatbuffers_parser (lumex::ipc::ipc_config const & ipc_config_a)
{
	...
	if (!api_path)
	{
		throw lumex::error ("Internal IPC error: unable to find api path");
	}

	const std::string api_path_str = api_path->string ();
	char const * include_directories[] = { api_path_str.c_str (), nullptr };
	std::string schemafile;
	if (!flatbuffers::LoadFile ((*api_path / "lumexapi.fbs").string ().c_str (), false, &schemafile))
	{
		throw lumex::error ("Internal IPC error: unable to load schema file");
	}
	...
	if (!parse_success)
	{
		...
		throw lumex::error (parser_error);
	}
	return parser;
}

void lumex::ipc::flatbuffers_handler::process_json (uint8_t const * message_buffer_a, std::size_t buffer_size_a,
...
void lumex::ipc::flatbuffers_handler::process (uint8_t const * message_buffer_a, std::size_t buffer_size_a,
std::function<void (std::shared_ptr<flatbuffers::FlatBufferBuilder> const &)> const & response_handler)
{
	...
	try
	{
		...
		if (ipc_config.flatbuffers.verify_buffers)
		{
			...
			{
				throw lumex::error ("Envelope buffer did not pass verifier");
			}
		}

		auto incoming = lumexapi::GetEnvelope (message_buffer_a);
		if (incoming == nullptr)
		{
			lumex::error err ("Invalid message");
		...
		{
			lumex::error err ("Unknown message type");
			actionhandler->make_error (err.error_code_as_int (), err.get_message ());
		}
	}
	catch (lumex::error const & err)
