...
lumex::ipc::broker::broker (lumex::node & node_a) :
	node (node_a)
{
}

std::shared_ptr<flatbuffers::Parser> lumex::ipc::subscriber::get_parser (lumex::ipc::ipc_config const & ipc_config_a)
{
	if (!parser)
	{
		parser = lumex::ipc::flatbuffers_handler::make_flatbuffers_parser (ipc_config_a);
	}
	return parser;
}

void lumex::ipc::broker::start ()
{
	node.observers.blocks.add ([this_l = shared_from_this ()] (lumex::election_status const & status_a, std::vector<lumex::vote_with_weight_info> const & votes_a, lumex::account const & account_a, lumex::amount const & amount_a, bool is_state_send_a, bool is_state_epoch_a) {
		...
		try
		{
			...
			{
				auto confirmation (std::make_shared<lumexapi::EventConfirmationT> ());
				...
				switch (status_a.type)
				{
					case lumex::election_status_type::active_confirmed_quorum:
						confirmation->confirmation_type = lumexapi::TopicConfirmationType::TopicConfirmationType_active_quorum;
						break;
					case lumex::election_status_type::active_confirmation_height:
						confirmation->confirmation_type = lumexapi::TopicConfirmationType::TopicConfirmationType_active_confirmation_height;
						break;
					case lumex::election_status_type::inactive_confirmation_height:
						confirmation->confirmation_type = lumexapi::TopicConfirmationType::TopicConfirmationType_inactive;
						break;
					default:
						debug_assert (false);
						break;
				...
				confirmation->confirmation_type = lumexapi::TopicConfirmationType::TopicConfirmationType_active_quorum;
				confirmation->block = lumex::ipc::flatbuffers_builder::block_to_union (*status_a.winner, amount_a, is_state_send_a, is_state_epoch_a);
				confirmation->election_info = std::make_unique<lumexapi::ElectionInfoT> ();
		...
		catch (lumex::error const & err)
		{
			this_l->node.logger.error (lumex::log::type::ipc, "Could not broadcast message: {}", err.get_message ());
		}
	});
}

template <typename COLL, typename TOPIC_TYPE>
void subscribe_or_unsubscribe (lumex::logger & logger, COLL & subscriber_collection, std::weak_ptr<lumex::ipc::subscriber> const & subscriber_a, TOPIC_TYPE topic_a)
{
								 ...
								 [&logger = logger, topic_a, subscriber_a] (auto & sub) {
									 ...
									 if (subscriber_l)
									 {
										 if (auto calling_subscriber_l = subscriber_a.lock ())
										 {
											 ...
											 if (remove)
											 {
												 logger.info (lumex::log::type::ipc, "Subscriber ubsubscribed #{}", calling_subscriber_l->get_id ());
											 }
										 }
									 }
									 else
...
void lumex::ipc::broker::subscribe (std::weak_ptr<lumex::ipc::subscriber> const & subscriber_a, std::shared_ptr<lumexapi::TopicConfirmationT> const & confirmation_a)
{
...
void lumex::ipc::broker::broadcast (std::shared_ptr<lumexapi::EventConfirmationT> const & confirmation_a)
{
	using Filter = lumexapi::TopicConfirmationTypeFilter;
	decltype (confirmation_a->election_info) election_info;
	lumexapi::BlockUnion block;
	...
	while (itr != confirmation_subscribers->end ())
	{
		if (auto subscriber_l = itr->subscriber.lock ())
		{
			auto should_filter = [this, &itr, confirmation_a] () {
				...
				if ((confirmation_a->confirmation_type == lumexapi::TopicConfirmationType::TopicConfirmationType_active_quorum || confirmation_a->confirmation_type == lumexapi::TopicConfirmationType::TopicConfirmationType_active_confirmation_height) && (all_filter || active_filter))
				{
					should_filter_conf_type_l = false;
				}
				else if (confirmation_a->confirmation_type == lumexapi::TopicConfirmationType::TopicConfirmationType_inactive && (all_filter || inactive_filter))
				{
					should_filter_conf_type_l = false;
				}
			...
			{
				auto fb (lumex::ipc::flatbuffer_producer::make_buffer (*confirmation_a));

				if (subscriber_l->get_active_encoding () == lumex::ipc::payload_encoding::flatbuffers_json)
				{
					...
					{
						throw lumex::error ("Couldn't serialize response to JSON");
					}

					subscriber_l->async_send_message (reinterpret_cast<uint8_t const *> (json->data ()), json->size (), [json] (lumex::error const & err) {});
				}
				else
				{
					subscriber_l->async_send_message (fb->GetBufferPointer (), fb->GetSize (), [fb] (lumex::error const & err) {});
				}
			}
			...
			if (block.type != lumexapi::Block::Block_NONE)
			{
...
std::size_t lumex::ipc::broker::confirmation_subscriber_count () const
{
	return confirmation_subscribers->size ();
}

void lumex::ipc::broker::service_register (std::string const & service_name_a, std::weak_ptr<lumex::ipc::subscriber> const & subscriber_a)
{
...
void lumex::ipc::broker::service_stop (std::string const & service_name_a)
{
	...
	for (auto & subcription : subscribers.get ())
	{
		if (auto subscriber_l = subcription.subscriber.lock ())
		{
			...
			{
				lumexapi::EventServiceStopT event_stop;
				auto fb (lumex::ipc::flatbuffer_producer::make_buffer (event_stop));
				subscriber_l->async_send_message (fb->GetBufferPointer (), fb->GetSize (), [fb] (lumex::error const & err) {});
...
void lumex::ipc::broker::subscribe (std::weak_ptr<lumex::ipc::subscriber> const & subscriber_a, std::shared_ptr<lumexapi::TopicServiceStopT> const & service_stop_a)
{
	auto subscribers = service_stop_subscribers.lock ();
