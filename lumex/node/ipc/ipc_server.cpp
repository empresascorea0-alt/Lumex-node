...
#include <lumex/boost/asio/local/stream_protocol.hpp>
...
#include <lumex/node/ipc/flatbuffers_handler.hpp>
...
class session final : public lumex::ipc::socket_base, public std::enable_shared_from_this<session<SOCKET_TYPE>>
{
public:
	session (lumex::ipc::ipc_server & server_a, std::shared_ptr<boost::asio::io_context> io_ctx_a, lumex::ipc::ipc_config_transport & config_transport_a) :
	...
	{
		node.logger.debug (lumex::log::type::ipc, "Creating session with id: {}", session_id.load ());
	}
	...
	std::shared_ptr<lumex::ipc::subscriber> get_subscriber ()
	{
		class subscriber_impl final : public lumex::ipc::subscriber, public std::enable_shared_from_this<subscriber_impl>
		{
		public:
			...
			virtual void async_send_message (uint8_t const * data_a, std::size_t length_a, std::function<void (lumex::error const &)> broadcast_completion_handler_a) override
			{
			...
			lumex::ipc::payload_encoding get_active_encoding () const override
			{
				lumex::ipc::payload_encoding encoding{ lumex::ipc::payload_encoding::flatbuffers };
				if (auto session_l = session_m.lock ())
				{
		...
		static lumex::mutex subscriber_mutex;
		lumex::unique_lock<lumex::mutex> lock{ subscriber_mutex };
	 ...
	 * @note This function explicitly doesn't use lumex::shared_const_buffer, as buffers usually originate from Flatbuffers
	...
	void write_queued_messages ()
	{
		...
		lumex::unsafe_async_write (socket, msg.buffer,
	...
	void async_read_exactly (void * buff_a, std::size_t size_a, std::chrono::seconds timeout_a, std::function<void ()> const & callback_a)
	{
		...
		[this_l, callback_a] (boost::system::error_code const & ec, std::size_t bytes_transferred_a) {
			...
			{
				this_l->node.logger.error (lumex::log::type::ipc, "Error reading: ", ec.message ());
			}
	...
	void handle_json_query (bool allow_unsafe)
	{
		...
		// This is called when lumex::rpc_handler#process_request is done. We convert to
		...
		auto response_handler_l ([this_l, request_id_l] (std::string const & body) {
			...
			this_l->node.logger.debug (lumex::log::type::ipc, "IPC/RPC request {} completed in: {} {}",
			request_id_l,
			...
			this_l->queued_write (boost::asio::buffer (buffer->data (), buffer->size ()), [this_l, buffer] (boost::system::error_code const & error_a, std::size_t size_a) {
				...
				{
					this_l->node.logger.error (lumex::log::type::ipc, "Write failed: ", error_a.message ());
				}
			});
		...
		node.stats.inc (lumex::stat::type::ipc, lumex::stat::detail::invocations);
		auto body (std::string (reinterpret_cast<char *> (buffer.data ()), buffer.size ()));

		// Note that if the rpc action is async, the shared_ptr<json_handler> lifetime will be extended by the action handler
		auto handler (std::make_shared<lumex::json_handler> (node, server.node_rpc_config, body, response_handler_l, [server_w = server.weak_from_this ()] () {
	...
	void read_next_request ()
	{
		...
		async_read_exactly (buffer.data (), buffer.size (), std::chrono::seconds::max (), [this_l] () {
			auto encoding (this_l->buffer[lumex::ipc::preamble_offset::encoding]);
			this_l->active_encoding = static_cast<lumex::ipc::payload_encoding> (encoding);
			if (this_l->buffer[lumex::ipc::preamble_offset::lead] != 'N' || this_l->buffer[lumex::ipc::preamble_offset::reserved_1] != 0 || this_l->buffer[lumex::ipc::preamble_offset::reserved_2] != 0)
			{
				this_l->node.logger.error (lumex::log::type::ipc, "Invalid preamble");
			}
			else if (encoding == static_cast<uint8_t> (lumex::ipc::payload_encoding::json_v1) || encoding == static_cast<uint8_t> (lumex::ipc::payload_encoding::json_v1_unsafe))
			{
				auto allow_unsafe (encoding == static_cast<uint8_t> (lumex::ipc::payload_encoding::json_v1_unsafe));
				// Length of payload
			...
			else if (encoding == static_cast<uint8_t> (lumex::ipc::payload_encoding::flatbuffers) || encoding == static_cast<uint8_t> (lumex::ipc::payload_encoding::flatbuffers_json))
			{
				// Length of payload
				this_l->async_read_exactly (&this_l->buffer_size, sizeof (this_l->buffer_size), [this_l, encoding] () {
					...
					this_l->async_read_exactly (this_l->buffer.data (), this_l->buffer_size, [this_l, encoding] () {
						...
						{
							this_l->flatbuffers_handler = std::make_shared<lumex::ipc::flatbuffers_handler> (this_l->node, this_l->server, this_l->get_subscriber (), *this_l->node.config.ipc_config);
						}

						if (encoding == static_cast<uint8_t> (lumex::ipc::payload_encoding::flatbuffers_json))
						{
							this_l->flatbuffers_handler->process_json (this_l->buffer.data (), this_l->buffer_size, [this_l] (std::shared_ptr<std::string> const & body) {
								this_l->node.logger.debug (lumex::log::type::ipc, "IPC/Flatbuffer request completed in: {} {}",
								this_l->session_timer.stop ().count (),
								this_l->session_timer.unit ());
			...
			{
				this_l->node.logger.error (lumex::log::type::ipc, "Unsupported payload encoding");
			}
		});
	}
...
private:
	...
	lumex::locked<std::string> service_name;
	 ...
	 */
	std::atomic<lumex::ipc::payload_encoding> active_encoding;

	/** Timer for measuring the duration of ipc calls */
	lumex::timer<std::chrono::microseconds> session_timer;
	...
	lumex::ipc::ipc_config_transport & config_transport;

	/** Handler for Flatbuffers requests. This is created lazily on the first request. */
	std::shared_ptr<lumex::ipc::flatbuffers_handler> flatbuffers_handler;

	/** Session subscriber */
	std::shared_ptr<lumex::ipc::subscriber> subscriber;
};

/** Domain and TCP socket transport */
template <typename ACCEPTOR_TYPE, typename SOCKET_TYPE, typename ENDPOINT_TYPE>
class socket_transport : public lumex::ipc::transport
{
public:
	socket_transport (lumex::ipc::ipc_server & server_a, ENDPOINT_TYPE endpoint_a, lumex::ipc::ipc_config_transport & config_transport_a, int concurrency_a) :
		server (server_a),
		config_transport (config_transport_a)
	{
		...
		runner = std::make_unique<lumex::thread_runner> (io_ctx, server.logger, static_cast<unsigned> (std::max (1, concurrency_a)), lumex::thread_role::name::io_ipc);
	}
	...
	void accept ()
	{
		...
		std::weak_ptr<lumex::node> lumex_weak = server.node.shared ();
		acceptor->async_accept (new_session->get_socket (), [this, new_session, lumex_weak] (boost::system::error_code const & ec) {
			auto node = lumex_weak.lock ();
			if (!node)
			{
				return;
			}
			...
			{
				node->logger.error (lumex::log::type::ipc, "Acceptor error: {}", ec.message ());
			}
			...
			{
				node->logger.info (lumex::log::type::ipc, "Shutting down");
			}
		});
	}
...
private:
	...
	lumex::locked<std::list<std::weak_ptr<session<SOCKET_TYPE>>>> sessions;
};
...
lumex::ipc::ipc_server::ipc_server (lumex::node & node_a, lumex::node_rpc_config const & node_rpc_config_a) :
	node (node_a),
	node_rpc_config (node_rpc_config_a),
	broker (std::make_shared<lumex::ipc::broker> (node_a))
{
	try
	{
		lumex::error access_config_error (reload_access_config ());
...
#else
			node.logger.error (lumex::log::type::ipc_server, "Domain sockets are not supported on this platform");
#endif
		}
		...
		node.logger.debug (lumex::log::type::ipc_server, "Server started");
	...
	{
		node.logger.error (lumex::log::type::ipc_server, "Error: {}", ex.what ());
	}
}

lumex::ipc::ipc_server::~ipc_server ()
{
	node.logger.debug (lumex::log::type::ipc_server, "Server stopped");

	stop ();
}

void lumex::ipc::ipc_server::stop ()
{
...
std::optional<std::uint16_t> lumex::ipc::ipc_server::listening_tcp_port () const
{
...
std::shared_ptr<lumex::ipc::broker> lumex::ipc::ipc_server::get_broker ()
{
	return broker;
}

lumex::ipc::access & lumex::ipc::ipc_server::get_access ()
{
	return access;
}

lumex::error lumex::ipc::ipc_server::reload_access_config ()
{
	lumex::error access_config_error (lumex::ipc::read_access_config_toml (node.get_data_path (), access));
	if (access_config_error)
	{
		node.logger.error (lumex::log::type::ipc_server, "Invalid access configuration file: {}", access_config_error.get_message ());
	}
