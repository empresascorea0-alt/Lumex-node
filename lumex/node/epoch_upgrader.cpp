...
lumex::epoch_upgrader::epoch_upgrader (lumex::node & node_a, lumex::ledger & ledger_a, lumex::store::ledger_store & store_a, lumex::network_params & network_params_a, lumex::logger & logger_a) :
	node{ node_a },
	ledger{ ledger_a },
	store{ store_a },
...
void lumex::epoch_upgrader::stop ()
{
	stopped = true;
...
bool lumex::epoch_upgrader::start (lumex::raw_key const & prv_a, lumex::epoch epoch_a, uint64_t count_limit, uint64_t threads)
{
...
void lumex::epoch_upgrader::upgrade_impl (lumex::raw_key const & prv_a, lumex::epoch epoch_a, uint64_t count_limit, uint64_t threads)
{
	lumex::thread_role::set (lumex::thread_role::name::epoch_upgrader);
	auto upgrader_process = [this] (std::atomic<uint64_t> & counter, std::shared_ptr<lumex::block> const & epoch, uint64_t difficulty, lumex::public_key const & signer_a, lumex::root const & root_a, lumex::account const & account_a) {
		epoch->block_work_set (node.work_generate_blocking (lumex::work_version::work_1, root_a, difficulty).value_or (0));
		bool valid_signature (!lumex::validate_message (signer_a, epoch->hash (), epoch->block_signature ()));
		bool valid_work (node.network_params.work.difficulty (*epoch) >= difficulty);
		lumex::block_status result (lumex::block_status::old);
		...
		{
			bool fork (result == lumex::block_status::fork);

			logger.error (lumex::log::type::epoch_upgrader, "Failed to upgrade account {} (valid signature: {}, valid work: {}, fork: {})",
			account_a,
			valid_signature,
			valid_work,
			fork);
	...
	raw_key = prv_a;
	auto signer (lumex::pub_key (prv_a));
	debug_assert (signer == ledger.epoch_signer (link));

	lumex::mutex upgrader_mutex;
	lumex::condition_variable upgrader_condition;
	...
	boost::multi_index::indexed_by<
		...
		boost::multi_index::hashed_unique<boost::multi_index::tag<account_tag>,
			boost::multi_index::member<account_upgrade_item, lumex::account, &account_upgrade_item::account>>>>
	accounts_list;
	// clang-format on
	...
	while (!finished_upgrade && !stopped)
	{
		...
		{
			{
				...
				for (auto i (store.account.begin (transaction)), n (store.account.end (transaction)); i != n && accounts_list.size () < count_limit; ++i)
				{
					...
					{
						release_assert (lumex::epochs::is_sequential (info.epoch (), epoch_a));
			...
			{
				lumex::account const & account (i->account);
				auto info = ledger.any.account_get (ledger.tx_begin_read (), account);
				if (info && info->epoch () < epoch_a)
				{
					++attempts;
					auto difficulty (node.network_params.work.threshold (lumex::work_version::work_1, lumex::block_details (epoch_a, false, false, true)));
					lumex::root const & root (info->head);
					std::shared_ptr<lumex::block> epoch = builder.state ()
														 .account (account)
														 .previous (info->head)
					...
					{
						{
							lumex::unique_lock<lumex::mutex> lock{ upgrader_mutex };
							++workers;
						...
						node.workers.post ([&upgrader_process, &upgrader_mutex, &upgrader_condition, &upgraded_accounts, &workers, epoch, difficulty, signer, root, account] () {
							...
							{
								lumex::lock_guard<lumex::mutex> lock{ upgrader_mutex };
								--workers;
							}
			...
			{
				logger.info (lumex::log::type::epoch_upgrader, "{} accounts were upgraded to new epoch, {} remain...",
			...
			{
				logger.info (lumex::log::type::epoch_upgrader, "{} total accounts were upgraded to new epoch", total_upgraded_accounts);
		...
		while (!finished_pending && count_limit != 0 && !stopped)
		{
			...
			for (auto current = ledger.any.receivable_upper_bound (ledger.tx_begin_read (), 0), end = ledger.any.receivable_end (); current != end && attempts < upgrade_batch_size && attempts < count_limit && !stopped;)
			{
				...
				if (!store.account.exists (ledger.tx_begin_read (), key.account))
				{
					if (info.epoch < epoch_a)
					{
						++attempts;
						release_assert (lumex::epochs::is_sequential (info.epoch, epoch_a));
						auto difficulty (network_params.work.threshold (lumex::work_version::work_1, lumex::block_details (epoch_a, false, false, true)));
						lumex::root const & root (key.account);
						lumex::account const & account (key.account);
						std::shared_ptr<lumex::block> epoch = builder.state ()
															 .account (key.account)
															 .previous (0)
															 .representative (0)
				...
				else
				{
					...
					if (key.account.number () == std::numeric_limits<lumex::uint256_t>::max ())
					{
						break;
					}
					else
			...
			{
				logger.info (lumex::log::type::epoch_upgrader, "{} unopened accounts with pending blocks were upgraded to new epoch...", total_upgraded_pending);
			}
			else
			{
				logger.info (lumex::log::type::epoch_upgrader, "{} total unopened accounts with pending blocks were upgraded to new epoch", total_upgraded_pending);
	...
	logger.info (lumex::log::type::epoch_upgrader, "Epoch upgrade is completed");
}
