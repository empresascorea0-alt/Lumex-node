...
#include <lumex/lib/json_error_response.hpp>
...
namespace
{
void construct_json (lumex::container_info_component * component, boost::property_tree::ptree & parent);
using ipc_json_handler_no_arg_func_map = std::unordered_map<std::string, std::function<void (lumex::json_handler *)>>;
...
bool block_confirmed (lumex::node & node, lumex::secure::transaction & transaction, lumex::block_hash const & hash, bool include_active, bool include_only_confirmed);
char const * epoch_as_string (lumex::epoch);
}

lumex::json_handler::json_handler (lumex::node & node_a, lumex::node_rpc_config const & node_rpc_config_a, std::string const & body_a, std::function<void (std::string const &)> const & response_a, std::function<void ()> stop_callback_a) :
	body (body_a),
	node (node_a),
	response (response_a),
...
std::function<void ()> lumex::json_handler::create_worker_task (std::function<void (std::shared_ptr<lumex::json_handler> const &)> const & action_a)
{
...
void lumex::json_handler::process_request (bool unsafe_a)
{
	try
	{
		...
		else
		{
			...
			else if (action == "lumex_to_raw")
			{
				lumex_to_raw ();
			}
			else if (action == "raw_to_lumex")
			{
				raw_to_lumex ();
			}
...
void lumex::json_handler::response_errors ()
{
	if (!ec && response_l.empty ())
	{
		// Return an error code if no response data was given
		ec = lumex::error_rpc::empty_response;
	}
	if (ec)
	{
...
std::shared_ptr<lumex::wallet::wallet> lumex::json_handler::wallet_impl ()
{
	if (!ec)
	{
		...
		if (!wallet.decode_hex (wallet_text))
		{
			...
			{
				ec = lumex::error_common::wallet_not_found;
			}
		}
		else
		{
			ec = lumex::error_common::bad_wallet_number;
		}
	}
	return nullptr;
}

bool lumex::json_handler::wallet_locked_impl (std::shared_ptr<lumex::wallet::wallet> const & wallet_a)
{
	bool result (false);
	if (!ec)
	{
		...
		{
			ec = lumex::error_common::wallet_locked;
			result = true;
		}
	}
	return result;
...
bool lumex::json_handler::wallet_account_impl (std::shared_ptr<lumex::wallet::wallet> const & wallet_a, lumex::account const & account_a)
{
	bool result (false);
	if (!ec)
	{
		...
		{
			ec = lumex::error_common::account_not_found_wallet;
		}
	}
	return result;
}

lumex::account lumex::json_handler::account_impl (std::string account_text, std::error_code ec_a)
{
	...
	if (!ec)
	{
		...
		{
			// lumex- and xrb- prefixes are deprecated
...
lumex::account_info lumex::json_handler::account_info_impl (secure::transaction const & transaction_a, lumex::account const & account_a)
{
	...
	if (!ec)
	{
		...
		if (!info)
		{
			ec = lumex::error_common::account_not_found;
		}
		else
		{
			result = *info;
...
lumex::amount lumex::json_handler::amount_impl ()
{
	lumex::amount result (0);
	if (!ec)
	{
		std::string amount_text (request.get<std::string> ("amount"));
		if (result.decode_dec (amount_text))
		{
			ec = lumex::error_common::invalid_amount;
		}
	}
	return result;
}

std::shared_ptr<lumex::block> lumex::json_handler::block_impl (bool signature_work_required)
{
	bool const json_block_l = request.get<bool> ("json_block", false);
	std::shared_ptr<lumex::block> result{ nullptr };
	if (!ec)
	{
		...
		else
		{
			...
			{
				ec = lumex::error_blocks::invalid_block;
			}
		}
		if (!ec)
		{
			...
			result = lumex::deserialize_block_json (block_l);
			if (result == nullptr)
			{
				ec = lumex::error_blocks::invalid_block;
			}
...
lumex::block_hash lumex::json_handler::hash_impl (std::string search_text)
{
	...
	if (!ec)
	{
		...
		{
			ec = lumex::error_blocks::invalid_block_hash;
		}
	}
	return result;
}

lumex::amount lumex::json_handler::threshold_optional_impl ()
{
	lumex::amount result (0);
	...
	if (!ec && threshold_text.has_value ())
	{
		...
		{
			ec = lumex::error_common::bad_threshold;
		}
	}
	return result;
}

uint64_t lumex::json_handler::work_optional_impl ()
{
	uint64_t result (0);
	auto work_text (request.get_optional<std::string> ("work"));
	if (!ec && work_text.has_value ())
	{
		if (lumex::from_string_hex (work_text.value (), result))
		{
			ec = lumex::error_common::bad_work_format;
		}
	}
	return result;
}

uint64_t lumex::json_handler::difficulty_optional_impl (lumex::work_version const version_a)
{
	...
	{
		if (lumex::from_string_hex (difficulty_text.value (), difficulty))
		{
			ec = lumex::error_rpc::bad_difficulty_format;
		}
	}
	return difficulty;
}

uint64_t lumex::json_handler::difficulty_ledger (lumex::block const & block_a)
{
	lumex::block_details details (lumex::epoch::epoch_0, false, false, false);
	bool details_found (false);
	auto transaction = node.ledger.tx_begin_read ();
	// Previous block find
	std::shared_ptr<lumex::block> block_previous (nullptr);
	...
	if (link && !link.value ().is_zero () && !details.is_send)
	{
		...
		if (block_link != nullptr && node.ledger.any.pending_get (transaction, lumex::pending_key{ account, link.value ().as_block_hash () }))
		{
...
double lumex::json_handler::multiplier_optional_impl (lumex::work_version const version_a, uint64_t & difficulty)
{
	double multiplier (1.);
	...
	if (!ec && multiplier_text.has_value ())
	{
		...
		{
			difficulty = lumex::difficulty::from_multiplier (multiplier, node.default_difficulty (version_a));
		}
		else
		{
			ec = lumex::error_rpc::bad_multiplier_format;
		}
	}
	return multiplier;
}

lumex::work_version lumex::json_handler::work_version_optional_impl (lumex::work_version const default_a)
{
	lumex::work_version result = default_a;
	auto version_text (request.get_optional<std::string> ("version"));
	if (!ec && version_text.has_value ())
	{
		if (*version_text == lumex::to_string (lumex::work_version::work_1))
		{
			result = lumex::work_version::work_1;
		}
		else
		{
			ec = lumex::error_rpc::bad_work_version;
		}
	}
	return result;
}
...
uint64_t lumex::json_handler::count_impl ()
{
	uint64_t result (0);
	if (!ec)
	{
		...
		{
			ec = lumex::error_common::invalid_count;
		}
	}
	return result;
}

uint64_t lumex::json_handler::count_optional_impl (uint64_t result)
{
...
uint64_t lumex::json_handler::offset_optional_impl (uint64_t result)
{
	...
	if (!ec && offset_text.has_value ())
	{
		...
		{
			ec = lumex::error_rpc::invalid_offset;
		}
	}
	return result;
}

void lumex::json_handler::account_balance ()
{
...
void lumex::json_handler::account_block_count ()
{
	auto account (account_impl ());
	if (!ec)
	{
...
void lumex::json_handler::account_create ()
{
	node.workers.post (create_worker_task ([] (std::shared_ptr<lumex::json_handler> const & rpc_l) {
		...
		if (!rpc_l->ec)
		{
			...
			if (index_text.has_value ())
			{
				...
				{
					rpc_l->ec = lumex::error_common::invalid_index;
				}
				else
				{
...
void lumex::json_handler::account_get ()
{
	...
	{
		ec = lumex::error_common::bad_public_key;
	}
	response_errors ();
}

void lumex::json_handler::account_info ()
{
	auto account (account_impl ());
	if (!ec)
	{
		...
		if (!ec)
		{
			...
			lumex::amount balance_l (info.balance);
			std::string balance = balance_l.to_string_dec ();

			response_l.put ("balance", balance);

			lumex::amount confirmed_balance_l;
			...
			std::shared_ptr<lumex::block> confirmed_frontier_block;
			...
			if (representative)
			{
				...
				if (include_confirmed)
				{
					lumex::account confirmed_representative{};
...
void lumex::json_handler::account_key ()
{
	auto account (account_impl ());
	if (!ec)
	{
...
void lumex::json_handler::account_list ()
{
...
void lumex::json_handler::account_move ()
{
	node.workers.post (create_worker_task ([] (std::shared_ptr<lumex::json_handler> const & rpc_l) {
		auto wallet (rpc_l->wallet_impl ());
		if (!rpc_l->ec)
		{
			...
			if (!source.decode_hex (source_text))
			{
				...
				if (existing != rpc_l->node.wallets.items.end ())
				{
					...
					std::vector<lumex::public_key> accounts;
				...
				{
					rpc_l->ec = lumex::error_rpc::source_not_found;
				}
			}
			else
			{
				rpc_l->ec = lumex::error_rpc::bad_source;
			}
		}
		rpc_l->response_errors ();
	}));
...
void lumex::json_handler::account_remove ()
{
	node.workers.post (create_worker_task ([] (std::shared_ptr<lumex::json_handler> const & rpc_l) {
		auto wallet (rpc_l->wallet_impl ());
...
void lumex::json_handler::account_representative ()
{
	auto account (account_impl ());
	if (!ec)
	{
...
void lumex::json_handler::account_representative_set ()
{
	node.workers.post (create_worker_task ([work_generation_enabled = node.work_generation_enabled ()] (std::shared_ptr<lumex::json_handler> const & rpc_l) {
		auto wallet (rpc_l->wallet_impl ());
		auto account (rpc_l->account_impl ());
		std::string representative_text (rpc_l->request.get<std::string> ("representative"));
		auto representative (rpc_l->account_impl (representative_text, lumex::error_rpc::bad_representative_number));
		if (!rpc_l->ec)
		{
			...
			if (!rpc_l->ec && work)
			{
				...
				if (!rpc_l->ec)
				{
					...
					if (!rpc_l->ec)
					{
						lumex::block_details details (info.epoch (), false, false, false);
						if (rpc_l->node.network_params.work.difficulty (lumex::work_version::work_1, info.head, work) < rpc_l->node.network_params.work.threshold (lumex::work_version::work_1, details))
						{
							rpc_l->ec = lumex::error_common::invalid_work;
						}
					}
				}
			}
			else if (!rpc_l->ec) // work == 0
			{
				...
				{
					rpc_l->ec = lumex::error_common::disabled_work_generation;
				}
			}
			if (!rpc_l->ec)
			{
				...
				wallet->change_async (
				account, representative, [response_a, response_data] (std::shared_ptr<lumex::block> const & block) {
					if (block != nullptr)
					{
...
void lumex::json_handler::account_weight ()
{
	auto account (account_impl ());
	if (!ec)
	{
...
void lumex::json_handler::accounts_balances ()
{
...
void lumex::json_handler::accounts_representatives ()
{
...
void lumex::json_handler::accounts_create ()
{
	node.workers.post (create_worker_task ([] (std::shared_ptr<lumex::json_handler> const & rpc_l) {
		auto wallet = rpc_l->wallet_impl ();
...
void lumex::json_handler::accounts_frontiers ()
{
...
void lumex::json_handler::accounts_pending ()
{
	response_l.put ("deprecated", "1");
	accounts_receivable ();
}

void lumex::json_handler::accounts_receivable ()
{
	...
	for (auto & accounts : request.get_child ("accounts"))
	{
		...
		if (!ec)
		{
			...
			for (auto i (node.store.pending.begin (transaction, lumex::pending_key (account, 0))), n (node.store.pending.end (transaction)); i != n && lumex::pending_key (i->first).account == account && peers_l.size () < count; ++i)
			{
			...
			if (sorting && !simple)
			{
				if (source)
				{
					peers_l.sort ([] (auto const & child1, auto const & child2) -> bool {
						return child1.second.template get<lumex::uint128_t> ("amount") > child2.second.template get<lumex::uint128_t> ("amount");
					});
				}
				else
				{
					peers_l.sort ([] (auto const & child1, auto const & child2) -> bool {
						return child1.second.template get<lumex::uint128_t> ("") > child2.second.template get<lumex::uint128_t> ("");
					});
				}
			}
			if (!peers_l.empty ())
...
void lumex::json_handler::active_difficulty ()
{
	auto include_trend (request.get<bool> ("include_trend", false));
	auto const multiplier_active = 1.0;
	auto const default_difficulty (node.default_difficulty (lumex::work_version::work_1));
	auto const default_receive_difficulty (node.default_receive_difficulty (lumex::work_version::work_1));
	...
	response_l.put ("deprecated", "1");
	response_l.put ("network_minimum", lumex::to_string_hex (default_difficulty));
	response_l.put ("network_receive_minimum", lumex::to_string_hex (default_receive_difficulty));
	response_l.put ("network_current", lumex::to_string_hex (lumex::difficulty::from_multiplier (multiplier_active, default_difficulty)));
	response_l.put ("network_receive_current", lumex::to_string_hex (lumex::difficulty::from_multiplier (receive_current_denormalized, default_receive_difficulty)));
...
void lumex::json_handler::available_supply ()
{
	auto genesis_balance (node.balance (node.network_params.ledger.genesis->account ())); // Cold storage genesis
	auto landing_balance (node.balance (lumex::account ("059F68AAB29DE0D3A27443625C7EA9CDDB6517A8B76FE37727EF6A4D76832AD5"))); // Active unavailable account
	auto faucet_balance (node.balance (lumex::account ("8E319CE6F3025E5B2DF66DA7AB1467FE48F1679C13DD43BFDB29FA2E9FC40D3B"))); // Faucet account
	auto burned_balance ((node.balance_pending (lumex::account{}, false)).second); // Burning 0 account
	auto available (lumex::dev::constants.genesis_amount - genesis_balance - landing_balance - faucet_balance - burned_balance);
	response_l.put ("available", available.convert_to<std::string> ());
	response_errors ();
}

void lumex::json_handler::block_info ()
{
	auto hash (hash_impl ());
	if (!ec)
	{
		...
		if (block != nullptr)
		{
			...
			response_l.put ("successor", node.store.successor.get (transaction, hash).value_or (lumex::block_hash{ 0 }).to_string ());
			...
			{
				auto subtype (lumex::state_subtype (block->sideband ().details));
				response_l.put ("subtype", subtype);
			}
		}
		else
		{
			ec = lumex::error_blocks::not_found;
		}
	}
	response_errors ();
}

void lumex::json_handler::block_confirm ()
{
	auto hash (hash_impl ());
	if (!ec)
	{
		...
		if (block_l != nullptr)
		{
			...
			else
			{
				...
				lumex::election_status status{ block_l, lumex::election_status_type::active_confirmation_height };
				...
				if (amount)
				{
					if (auto state = dynamic_cast<lumex::state_block *> (block_l.get ()))
					{
...
void lumex::json_handler::blocks ()
{
	bool const json_block_l = request.get<bool> ("json_block", false);
	...
	for (boost::property_tree::ptree::value_type & hashes : request.get_child ("hashes"))
	{
		if (!ec)
		{
			...
			{
				ec = lumex::error_blocks::bad_hash_number;
			}
		}
	}
...
void lumex::json_handler::blocks_info ()
{
	bool const pending = request.get<bool> ("pending", false);
	bool const receivable = request.get<bool> ("receivable", pending);
		...
		if (!ec)
		{
			...
			if (!hash.decode_hex (hash_text))
			{
				...
				if (block != nullptr)
				{
					...
					entry.put ("successor", node.store.successor.get (transaction, hash).value_or (lumex::block_hash{ 0 }).to_string ());
					auto confirmed (node.ledger.cemented.block_exists_or_pruned (transaction, hash));
					...
					if (receivable || receive_hash)
					{
						if (!block->is_send ())
						{
							...
							if (receive_hash)
							{
								entry.put ("receive_hash", lumex::block_hash (0).to_string ());
							}
						}
						else if (node.ledger.any.pending_get (transaction, lumex::pending_key{ block->destination (), hash }))
						{
							if (receivable)
							{
								entry.put ("pending", "1");
						...
						else
						{
							...
							if (receive_hash)
							{
								std::shared_ptr<lumex::block> receive_block = node.ledger.find_receive_block_by_send_hash (transaction, block->destination (), hash);
								std::string receive_hash = receive_block ? receive_block->hash ().to_string () : lumex::block_hash (0).to_string ();
...
void lumex::json_handler::block_account ()
{
	auto hash (hash_impl ());
	if (!ec)
	{
...
void lumex::json_handler::block_count ()
{
...
void lumex::json_handler::block_create ()
{
	std::string type (request.get<std::string> ("type"));
	lumex::wallet_id wallet (0);
	// Default to work_1 if not specified
	auto work_version (work_version_optional_impl (lumex::work_version::work_1));
	...
	{
		representative = account_impl (representative_text.value (), lumex::error_rpc::bad_representative_number);
	}
	...
	{
		destination = account_impl (destination_text.value (), lumex::error_rpc::bad_destination);
	}
	lumex::block_hash source (0);
	auto source_text (request.get_optional<std::string> ("source"));
	if (!ec && source_text.has_value ())
	{
		if (source.decode_hex (source_text.value ()))
		{
			ec = lumex::error_rpc::bad_source;
		}
	}
	lumex::amount amount (0);
	...
	{
		ec = lumex::error_common::disabled_work_generation;
	}
	...
	if (!ec && key_text.has_value ())
	{
		...
		{
			ec = lumex::error_common::bad_private_key;
		}
	}
	...
	if (!ec && previous_text.has_value ())
	{
		...
		{
			ec = lumex::error_rpc::bad_previous;
		}
	}
	...
	if (!ec && balance_text.has_value ())
	{
		...
		{
			ec = lumex::error_rpc::invalid_balance;
		}
	}
	lumex::link link (0);
	...
	if (!ec)
	{
		...
		auto block_response_put_l = [rpc_l, this] (lumex::block const & block_a) {
			boost::property_tree::ptree response_l;
			response_l.put ("hash", block_a.hash ().to_string ());
			response_l.put ("difficulty", lumex::to_string_hex (rpc_l->node.network_params.work.difficulty (block_a)));
			bool json_block_l = request.get<bool> ("json_block", false);
			if (json_block_l)
			{
				boost::property_tree::ptree block_node_l;
		...
		auto get_callback_l = [rpc_l, block_response_put_l] (std::shared_ptr<lumex::block> const & block_a) {
			...
			return [block_a, rpc_l, block_response_put_l] (std::optional<uint64_t> const & work_a) {
				if (block_a != nullptr)
				{
					...
					{
						rpc_l->ec = lumex::error_common::failure_work_generation;
					}
				}
				else
				{
					rpc_l->ec = lumex::error_common::generic;
				}
				if (rpc_l->ec)
				{
					rpc_l->response_errors ();
		...
		if (prv != 0)
		{
			lumex::account pub (lumex::pub_key (prv));
			...
			else if (previous_text.has_value () && balance_text.has_value () && type == "send")
			{
				...
				{
					ec = lumex::error_rpc::block_create_balance_mismatch;
				}
			}
			...
			if (!ec && account_text.has_value ())
			{
				if (account != pub)
				{
					ec = lumex::error_rpc::block_create_public_key_mismatch;
				}
			}
			lumex::block_builder builder_l;
			std::shared_ptr<lumex::block> block_l{ nullptr };
			lumex::root root_l;
			...
			if (type == "state")
			{
				...
				{
					ec = lumex::error_rpc::block_create_requirements_state;
				}
			}
			else if (type == "open")
			{
				...
				{
					ec = lumex::error_rpc::block_create_requirements_open;
				}
			}
			else if (type == "receive")
			{
				...
				{
					ec = lumex::error_rpc::block_create_requirements_receive;
				}
			}
			else if (type == "change")
			{
				...
				{
					ec = lumex::error_rpc::block_create_requirements_change;
				}
			}
			else if (type == "send")
			{
				if (destination != 0 && previous != 0 && balance != 0 && amount != 0)
				{
					...
					{
						ec = lumex::error_common::insufficient_balance;
					}
				}
				else
				{
					ec = lumex::error_rpc::block_create_requirements_send;
				}
			}
			else
			{
				ec = lumex::error_blocks::invalid_type;
			}
			if (!ec && (!ec_build || ec_build == lumex::error_common::missing_work))
			{
				if (work == 0)
				{
					...
					node.work_generate (work_version, root_l, difficulty_l, get_callback_l (block_l), lumex::account (pub));
				}
				else
				{
		...
		{
			ec = lumex::error_rpc::block_create_key_required;
		}
	}
	// Because of callback
	if (ec)
...
void lumex::json_handler::block_hash ()
{
...
void lumex::json_handler::bootstrap ()
{
	...
	{
		uint16_t port;
		if (!lumex::parse_port (port_text, port))
		{
			ec = lumex::error_rpc::disabled_bootstrap_legacy;
		}
		else
		{
			ec = lumex::error_common::invalid_port;
		}
	}
	else
	{
		ec = lumex::error_common::invalid_ip_address;
	}
	response_errors ();
}

void lumex::json_handler::bootstrap_any ()
{
...
void lumex::json_handler::bootstrap_lazy ()
{
	auto hash (hash_impl ());
	bool const force = request.get<bool> ("force", false);
	ec = lumex::error_rpc::disabled_bootstrap_lazy;
	response_errors ();
}
 ...
 */
void lumex::json_handler::bootstrap_status ()
{
 ...
 * @warning This is an internal/diagnostic RPC, do not rely on its interface being stable
 */
void lumex::json_handler::bootstrap_priorities ()
{
	if (!ec)
	{
 ...
 * @warning This is an internal/diagnostic RPC, do not rely on its interface being stable
 */
void lumex::json_handler::bootstrap_reset ()
{
...
void lumex::json_handler::chain (bool successors)
{
...
void lumex::json_handler::confirmation_active ()
{
...
void lumex::json_handler::election_statistics ()
{
	auto active_elections = node.active.list_active ();
...
void lumex::json_handler::confirmation_history ()
{
	boost::property_tree::ptree elections;
...
void lumex::json_handler::confirmation_info ()
{
	...
	if (!root.decode_hex (root_text))
	{
		...
		if (election != nullptr && !election->confirmed ())
		{
			...
			for (auto const & [tally, block] : info.tally)
			{
				...
				if (representatives)
				{
					std::multimap<lumex::uint128_t, lumex::account, std::greater<lumex::uint128_t>> representatives;
					std::multimap<lumex::uint128_t, lumex::account, std::greater<lumex::uint128_t>> representatives_final;
		...
		{
			ec = lumex::error_rpc::confirmation_not_found;
		}
	}
	else
	{
		ec = lumex::error_rpc::invalid_root;
	}
	response_errors ();
}

void lumex::json_handler::confirmation_quorum ()
{
	...
	if (request.get<bool> ("peer_details", false))
	{
		...
		for (auto & peer : node.rep_crawler.representatives ())
		{
			...
			peer_node.put ("weight", lumex::amount{ node.ledger.weight (peer.account) }.to_string_dec ());
...
void lumex::json_handler::database_txn_tracker ()
{
	...
	{
		ec = lumex::error_common::tracking_not_enabled;
	}

	response_errors ();
}

void lumex::json_handler::delegators ()
{
	...
	if (!ec)
	{
		...
		for (auto i (node.store.account.begin (transaction, inc_sat (start_account.number ()))), n (node.store.account.end (transaction)); i != n && delegators.size () < count; ++i)
		{
			...
			if (info.representative == representative)
			{
				...
				{
					std::string balance = lumex::uint128_union (info.balance).to_string_dec ();
					lumex::account const & delegator (i->first);
...
void lumex::json_handler::delegators_count ()
{
	auto account (account_impl ());
	if (!ec)
	{
...
void lumex::json_handler::deterministic_key ()
{
	...
	if (!seed.decode_hex (seed_text))
	{
		try
		{
			...
			lumex::raw_key prv = lumex::deterministic_key (seed, index);
		...
		{
			ec = lumex::error_common::invalid_index;
		}
	}
	else
	{
		ec = lumex::error_common::bad_seed;
	}
	response_errors ();
}
 ...
 * @warning This is an internal/diagnostic RPC, do not rely on its interface being stable
 */
void lumex::json_handler::epoch_upgrade ()
{
	lumex::epoch epoch (lumex::epoch::invalid);
	uint8_t epoch_int (request.get<uint8_t> ("epoch"));
	switch (epoch_int)
	{
		case 1:
			epoch = lumex::epoch::epoch_1;
			break;
		case 2:
			epoch = lumex::epoch::epoch_2;
			break;
		default:
			break;
	}
	if (epoch != lumex::epoch::invalid)
	{
		...
		if (!ec && threads_text.has_value ())
		{
			...
			{
				ec = lumex::error_rpc::invalid_threads_count;
			}
		}
		std::string key_text (request.get<std::string> ("key"));
		lumex::raw_key prv;
		...
		{
			if (lumex::pub_key (prv) == node.ledger.epoch_signer (node.ledger.epoch_link (epoch)))
			{
			...
			{
				ec = lumex::error_rpc::invalid_epoch_signer;
			}
		}
		else
		{
	...
	{
		ec = lumex::error_rpc::invalid_epoch;
	}
	response_errors ();
}

void lumex::json_handler::frontiers ()
{
...
void lumex::json_handler::account_count ()
{
...
namespace
{
class history_visitor : public lumex::block_visitor
{
public:
	history_visitor (lumex::json_handler & handler_a, bool raw_a, lumex::secure::transaction & transaction_a, boost::property_tree::ptree & tree_a, lumex::block_hash const & hash_a, std::vector<lumex::public_key> const & accounts_filter_a) :
		handler (handler_a),
		raw (raw_a),
	...
	void send_block (lumex::send_block const & block_a)
	{
	...
	void receive_block (lumex::receive_block const & block_a)
	{
	...
	void open_block (lumex::open_block const & block_a)
	{
		if (raw)
		{
			tree.put ("type", "open");
		...
		else
		{
			...
			tree.put ("amount", lumex::dev::constants.genesis_amount.convert_to<std::string> ());
		}
	}
	void change_block (lumex::change_block const & block_a)
	{
	...
	void state_block (lumex::state_block const & block_a)
	{
		if (raw)
		{
			tree.put ("type", "state");
	...
	bool should_ignore_account (lumex::public_key const & account)
	{
		bool ignore (false);
	...
	lumex::json_handler & handler;
	bool raw;
	lumex::secure::transaction & transaction;
	boost::property_tree::ptree & tree;
	lumex::block_hash const & hash;
	std::vector<lumex::public_key> const & accounts_filter;
};
}

void lumex::json_handler::account_history ()
{
	std::vector<lumex::public_key> accounts_to_filter;
		...
		while (block != nullptr && count > 0)
		{
			...
			else
			{
				...
				if (!entry.empty ())
				{
					...
					if (output_raw)
					{
						entry.put ("work", lumex::to_string_hex (block->block_work ()));
...
void lumex::json_handler::keepalive ()
{
	if (!ec)
	{
		std::string address_text (request.get<std::string> ("address"));
...
void lumex::json_handler::key_create ()
{
	lumex::keypair pair;
...
void lumex::json_handler::key_expand ()
{
	std::string key_text (request.get<std::string> ("key"));
	lumex::raw_key prv;
	if (!prv.decode_hex (key_text))
...
void lumex::json_handler::ledger ()
{
	auto count (count_optional_impl ());
	auto threshold (threshold_optional_impl ());
	if (!ec)
	{
		...
		if (modified_since_text.has_value ())
		{
			...
			{
				ec = lumex::error_rpc::invalid_timestamp;
			}
		}
		bool const sorting = request.get<bool> ("sorting", false);
		bool const representative = request.get<bool> ("representative", false);
		...
		{
			std::vector<std::pair<lumex::uint128_union, lumex::account>> ledger_l;
			for (auto i (node.store.account.begin (transaction, start)), n (node.store.account.end (transaction)); i != n; ++i)
			{
				lumex::account_info const & info (i->second);
				lumex::uint128_union balance (info.balance);
			...
			for (auto i (ledger_l.begin ()), n (ledger_l.end ()); i != n && accounts.size () < count; ++i)
			{
				...
				{
					lumex::account const & account (i->second);
					boost::property_tree::ptree response_a;
					if (receivable)
					{
						auto account_receivable = node.ledger.account_receivable (transaction, account);
...
void lumex::json_handler::lumex_to_raw ()
{
	auto amount (amount_impl ());
	if (!ec)
	{
		auto result (amount.number () * lumex::lumex_ratio);
		...
		{
			ec = lumex::error_common::invalid_amount_big;
		}
	}
	response_errors ();
}

void lumex::json_handler::raw_to_lumex ()
{
	auto amount (amount_impl ());
	if (!ec)
	{
 ...
 * @warning This is an internal/diagnostic RPC, do not rely on its interface being stable
 */
void lumex::json_handler::node_id ()
{
	if (!ec)
	{
 ...
 * @warning This is an internal/diagnostic RPC, do not rely on its interface being stable
 */
void lumex::json_handler::node_id_delete ()
{
	response_l.put ("deprecated", "1");
	response_errors ();
}

void lumex::json_handler::password_change ()
{
	node.workers.post (create_worker_task ([] (std::shared_ptr<lumex::json_handler> const & rpc_l) {
		auto wallet (rpc_l->wallet_impl ());
		if (!rpc_l->ec)
		{
			...
			if (!rpc_l->ec)
			{
				...
				if (!error)
				{
					rpc_l->node.logger.warn (lumex::log::type::rpc, "Wallet password changed");
				}
			}
		}
		rpc_l->response_errors ();
...
void lumex::json_handler::password_enter ()
{
	node.workers.post (create_worker_task ([] (std::shared_ptr<lumex::json_handler> const & rpc_l) {
		auto wallet (rpc_l->wallet_impl ());
		if (!rpc_l->ec)
...
void lumex::json_handler::password_valid (bool wallet_locked)
{
	auto wallet (wallet_impl ());
	if (!ec)
	{
...
void lumex::json_handler::peers ()
{
	...
	for (auto i (peers_list.begin ()), n (peers_list.end ()); i != n; ++i)
	{
		...
		if (peer_details)
		{
			...
			debug_assert (channel->get_type () == lumex::transport::transport_type::tcp);
...
void lumex::json_handler::pending ()
{
	response_l.put ("deprecated", "1");
	receivable ();
}

void lumex::json_handler::receivable ()
{
	auto account (account_impl ());
	auto count (count_optional_impl ());
	auto offset (offset_optional_impl (0));
	...
	if (!ec)
	{
		...
		std::vector<std::pair<std::string, lumex::uint128_t>> hash_amount_pairs;
		for (auto i (node.store.pending.begin (transaction, lumex::pending_key (account, 0))), n (node.store.pending.end (transaction)); i != n && lumex::pending_key (i->first).account == account && (should_sort || peers_l.size () < count); ++i)
		{
			lumex::pending_key const & key (i->first);
			if (block_confirmed (node, transaction, key.hash, include_active, include_only_confirmed))
			{
		...
		if (should_sort)
		{
			if (source || min_version)
			{
				std::stable_sort (hash_ptree_pairs.begin (), hash_ptree_pairs.end (), [] (auto const & lhs, auto const & rhs) {
					return lhs.second.template get<lumex::uint128_t> ("amount") > rhs.second.template get<lumex::uint128_t> ("amount");
				});
...
void lumex::json_handler::pending_exists ()
{
	response_l.put ("deprecated", "1");
	receivable_exists ();
}

void lumex::json_handler::receivable_exists ()
{
	auto hash (hash_impl ());
	bool const include_active = request.get<bool> ("include_active", false);
	bool const include_only_confirmed = request.get<bool> ("include_only_confirmed", true);
	if (!ec)
	{
		...
		if (block != nullptr)
		{
			...
			if (block->is_send ())
			{
				exists = node.ledger.any.pending_get (transaction, lumex::pending_key{ block->destination (), hash }).has_value ();
			}
...
void lumex::json_handler::process ()
{
	node.workers.post (create_worker_task ([] (std::shared_ptr<lumex::json_handler> const & rpc_l) {
		...
		if (!rpc_l->ec && block->type () == lumex::block_type::state)
		{
			std::string subtype_text (rpc_l->request.get<std::string> ("subtype", ""));
			if (!subtype_text.empty ())
			{
				std::shared_ptr<lumex::state_block> block_state (std::static_pointer_cast<lumex::state_block> (block));
				...
				{
					rpc_l->ec = lumex::error_process::gap_previous;
				}
				else
				{
					...
					if (subtype_text == "send")
					{
						...
						{
							rpc_l->ec = lumex::error_rpc::invalid_subtype_balance;
						}
					...
					else if (subtype_text == "open")
					{
						...
						{
							rpc_l->ec = lumex::error_rpc::invalid_subtype_previous;
						}
					}
					...
					else if (subtype_text == "epoch")
					{
						...
						{
							rpc_l->ec = lumex::error_rpc::invalid_subtype_epoch_link;
						}
					}
					else
					{
						rpc_l->ec = lumex::error_rpc::invalid_subtype;
					}
				}
			}
		}
		if (!rpc_l->ec)
		{
			if (!rpc_l->node.network_params.work.validate_entry (*block))
			{
				if (!is_async)
				{
					...
					if (!result_maybe)
					{
						rpc_l->ec = lumex::error_rpc::stopped;
					}
					else
					{
						...
						switch (result)
						{
							...
							{
								rpc_l->ec = lumex::error_process::gap_source;
								break;
							}
							case lumex::block_status::old:
							{
								rpc_l->ec = lumex::error_process::old;
								break;
							}
							case lumex::block_status::bad_signature:
							{
								rpc_l->ec = lumex::error_process::bad_signature;
								break;
							}
							case lumex::block_status::negative_spend:
							{
								// TODO once we get RPC versioning, this should be changed to "negative spend"
								rpc_l->ec = lumex::error_process::negative_spend;
								break;
							}
							case lumex::block_status::balance_mismatch:
							{
								rpc_l->ec = lumex::error_process::balance_mismatch;
								break;
							}
							case lumex::block_status::unreceivable:
							{
								rpc_l->ec = lumex::error_process::unreceivable;
								break;
							}
							case lumex::block_status::block_position:
							{
								rpc_l->ec = lumex::error_process::block_position;
								break;
							}
							case lumex::block_status::gap_epoch_open_pending:
							{
								rpc_l->ec = lumex::error_process::gap_epoch_open_pending;
								break;
							}
							case lumex::block_status::fork:
							{
								...
								{
									rpc_l->ec = lumex::error_process::fork;
								}
								break;
							}
							case lumex::block_status::insufficient_work:
							{
								rpc_l->ec = lumex::error_process::insufficient_work;
								break;
							}
							case lumex::block_status::opened_burn_account:
								rpc_l->ec = lumex::error_process::opened_burn_account;
								break;
							default:
							{
								rpc_l->ec = lumex::error_process::other;
								break;
							}
						}
					}
				...
				else
				{
					...
					{
						rpc_l->ec = lumex::error_common::is_not_state_block;
					}
				}
			}
			else
			{
				rpc_l->ec = lumex::error_blocks::work_low;
			}
		}
		rpc_l->response_errors ();
	}));
...
void lumex::json_handler::pruned_exists ()
{
	auto hash (hash_impl ());
	if (!ec)
	{
		...
		{
			ec = lumex::error_rpc::pruning_disabled;
		}
	}
	response_errors ();
}

void lumex::json_handler::receive ()
{
	auto wallet (wallet_impl ());
	auto account (account_impl ());
	auto hash (hash_impl ("block"));
	if (!ec)
	{
		...
		if (!ec)
		{
			...
			{
				auto pending_info = node.ledger.any.pending_get (block_transaction, lumex::pending_key (account, hash));
				if (pending_info)
				{
					auto work (work_optional_impl ());
					if (!ec && work)
					{
						lumex::root head;
						lumex::epoch epoch = pending_info->epoch;
						...
						lumex::block_details details (epoch, false, true, false);
						if (node.network_params.work.difficulty (lumex::work_version::work_1, head, work) < node.network_params.work.threshold (lumex::work_version::work_1, details))
						{
							ec = lumex::error_common::invalid_work;
						}
					}
					else if (!ec) // && work == 0
					{
					...
					if (!ec)
					{
						...
						lumex::account representative (wallet->get_representative ());
						bool generate_work (work == 0); // Disable work generation if "work" option is provided
						auto response_a (response);
						wallet->receive_async (
						hash, representative, lumex::dev::constants.genesis_amount, account, [response_a] (std::shared_ptr<lumex::block> const & block_a) {
							if (block_a != nullptr)
							{
								boost::property_tree::ptree response_l;
				...
				{
					ec = lumex::error_process::unreceivable;
				}
			}
			else
			{
...
void lumex::json_handler::receive_minimum ()
{
	if (!ec)
	{
...
void lumex::json_handler::receive_minimum_set ()
{
	auto amount (amount_impl ());
	if (!ec)
	{
...
void lumex::json_handler::representatives ()
{
	auto count (count_optional_impl ());
	if (!ec)
	{
		...
		if (!sorting) // Simple
		{
			std::map<lumex::account, lumex::uint128_t> ordered (rep_amounts.begin (), rep_amounts.end ());
		...
		{
			std::vector<std::pair<lumex::uint128_t, std::string>> representation;
...
void lumex::json_handler::representatives_online ()
{
...
void lumex::json_handler::republish ()
{
	...
	if (!ec && sources_text.has_value ())
	{
		...
		{
			ec = lumex::error_rpc::invalid_sources;
		}
	}
	...
	if (!ec && destinations_text.has_value ())
	{
		...
		{
			ec = lumex::error_rpc::invalid_destinations;
		}
	}
	auto hash (hash_impl ());
	if (!ec)
	{
		...
		if (block != nullptr)
		{
			std::deque<std::shared_ptr<lumex::block>> republish_bundle;
			for (auto i (0); !hash.is_zero () && i < count; ++i)
			{
				block = node.ledger.any.block_get (transaction, hash);
				if (sources != 0) // Republish source chain
				{
					lumex::block_hash source = block->source_field ().value_or (block->link_field ().value_or (0).as_block_hash ());
				...
				if (destinations != 0) // Republish destination chain
				{
					...
					auto destination = block_b->is_send () ? block_b->destination () : lumex::account (0);
					if (!destination.is_zero ())
					{
						if (!node.ledger.any.pending_get (transaction, lumex::pending_key{ destination, hash }))
						{
							lumex::block_hash previous (node.ledger.any.account_head (transaction, destination));
			...
			node.network.flood_block_many (std::move (republish_bundle), lumex::transport::traffic_type::block_broadcast_rpc, 25ms);
...
void lumex::json_handler::search_pending ()
{
	response_l.put ("deprecated", "1");
	search_receivable ();
}

void lumex::json_handler::search_receivable ()
{
	auto wallet (wallet_impl ());
	if (!ec)
	{
...
void lumex::json_handler::search_pending_all ()
{
	response_l.put ("deprecated", "1");
	search_receivable_all ();
}

void lumex::json_handler::search_receivable_all ()
{
	if (!ec)
	{
...
void lumex::json_handler::send ()
{
	...
	auto source (account_impl (source_text, lumex::error_rpc::bad_source));
	std::string destination_text (request.get<std::string> ("destination"));
	auto destination (account_impl (destination_text, lumex::error_rpc::bad_destination));
	if (!ec)
	{
		auto work (work_optional_impl ());
		lumex::uint128_t balance (0);
		...
		if (!ec)
		{
			...
			if (!ec && work)
			{
				lumex::block_details details (info.epoch (), true, false, false);
				if (node.network_params.work.difficulty (lumex::work_version::work_1, info.head, work) < node.network_params.work.threshold (lumex::work_version::work_1, details))
				{
					ec = lumex::error_common::invalid_work;
				}
			}
		...
		if (!ec)
		{
			...
			wallet->send_async (
			source, destination, amount.number (), [balance, amount, response_a, response_data] (std::shared_ptr<lumex::block> const & block_a) {
				if (block_a != nullptr)
				{
				...
				else
				{
					...
					{
						std::error_code ec (lumex::error_common::insufficient_balance);
...
void lumex::json_handler::sign ()
{
	...
	{
		ec = lumex::error_rpc::sign_hash_disabled;
	}
	if (!ec)
	{
		lumex::raw_key prv;
		...
		{
			lumex::public_key pub (lumex::pub_key (prv));
			lumex::signature signature (lumex::sign_message (prv, pub, hash));
...
void lumex::json_handler::stats ()
{
	...
	{
		ec = lumex::error_rpc::invalid_missing_type;
	}

	response_errors ();
}

void lumex::json_handler::stats_clear ()
{
	node.stats.clear ();
	response_l.put ("success", "");
	std::stringstream ostream;
...
void lumex::json_handler::telemetry ()
{
	...
	if (address_text.has_value () || port_text.has_value ())
	{
		...
		if (address_text.has_value () && port_text.has_value ())
		{
			...
			if (!lumex::parse_port (*port_text, port))
			{
				...
				if (!lumex::parse_address (*address_text, address))
				{
		...
		{
			ec = lumex::error_rpc::requires_port_and_address;
		}

		if (!ec)
		{
			auto maybe_telemetry = node.telemetry.get_telemetry (lumex::transport::map_endpoint_to_v6 (endpoint));
			if (maybe_telemetry)
			{
				...
				{
					ec = lumex::error_rpc::generic;
				}
			}
			else
			{
				ec = lumex::error_rpc::peer_not_found;
			}
...
void lumex::json_handler::unchecked ()
{
	bool const json_block_l = request.get<bool> ("json_block", false);
	auto count (count_optional_impl ());
	if (!ec)
	{
		...
		[&unchecked, &json_block_l] (lumex::unchecked_key const & key, lumex::unchecked_info const & info) {
			if (json_block_l)
			{
				boost::property_tree::ptree block_node_l;
...
void lumex::json_handler::unchecked_clear ()
{
	node.workers.post (create_worker_task ([] (std::shared_ptr<lumex::json_handler> const & rpc_l) {
...
void lumex::json_handler::unchecked_get ()
{
	bool const json_block_l = request.get<bool> ("json_block", false);
	auto hash (hash_impl ());
	if (!ec)
	{
		bool done = false;
		node.unchecked.for_each (
		[&] (lumex::unchecked_key const & key, lumex::unchecked_info const & info) {
			if (key.hash == hash)
			{
				response_l.put ("modified_timestamp", std::to_string (info.modified ()));
...
void lumex::json_handler::unopened ()
{
	auto count{ count_optional_impl () };
	auto threshold{ threshold_optional_impl () };
	lumex::account start{ 1 }; // exclude burn account by default
	auto account_text (request.get_optional<std::string> ("account"));
	if (account_text.has_value ())
	{
		start = account_impl (account_text.value ());
	...
	{
		auto transaction = node.store.tx_begin_read ();
		auto iterator = node.store.pending.begin (transaction, lumex::pending_key (start, 0));
		auto end = node.store.pending.end (transaction);
		lumex::account current_account = start;
		lumex::uint128_t current_account_sum{ 0 };
		boost::property_tree::ptree accounts;
		while (iterator != end && accounts.size () < count)
		{
			lumex::pending_key key{ iterator->first };
			lumex::account account{ key.account };
			lumex::pending_info info{ iterator->second };
			if (node.store.account.exists (transaction, account))
			{
				if (account.number () == std::numeric_limits<lumex::uint256_t>::max ())
				{
					break;
				}
				// Skip existing accounts
				iterator = node.store.pending.begin (transaction, lumex::pending_key (inc_sat (account.number ()), 0));
			}
			else
			{
...
void lumex::json_handler::validate_account_number ()
{
	auto account (account_impl ());
	(void)account;
...
void lumex::json_handler::wallet_add ()
{
	node.workers.post (create_worker_task ([] (std::shared_ptr<lumex::json_handler> const & rpc_l) {
		auto wallet = rpc_l->wallet_impl ();
		if (!rpc_l->ec)
		{
			...
			{
				rpc_l->ec = lumex::error_common::bad_private_key;
			}
		}
		rpc_l->response_errors ();
	}));
...
void lumex::json_handler::wallet_add_watch ()
{
	node.workers.post (create_worker_task ([] (std::shared_ptr<lumex::json_handler> const & rpc_l) {
		auto wallet (rpc_l->wallet_impl ());
		if (!rpc_l->ec)
		{
			if (!wallet->is_locked ())
			{
				for (auto & accounts : rpc_l->request.get_child ("accounts"))
				{
					...
					if (!rpc_l->ec)
					{
						...
						{
							rpc_l->ec = lumex::error_common::bad_public_key;
						}
					}
				}
				if (!rpc_l->ec)
			...
			{
				rpc_l->ec = lumex::error_common::wallet_locked;
			}
		}
		rpc_l->response_errors ();
	}));
...
void lumex::json_handler::wallet_info ()
{
	auto wallet (wallet_impl ());
	if (!ec)
	{
		lumex::uint128_t balance (0);
		lumex::uint128_t receivable (0);
		uint64_t count (0);
		...
		for (auto const & account : wallet->accounts ())
		{
			...
			lumex::confirmation_height_info confirmation_info{};
			...
			lumex::wallet::key_type key_type_l (wallet->key_type (account));
			if (key_type_l == lumex::wallet::key_type::deterministic)
			{
				deterministic_count++;
			}
			else if (key_type_l == lumex::wallet::key_type::adhoc)
			{
				adhoc_count++;
			}
...
void lumex::json_handler::wallet_balances ()
{
	auto wallet (wallet_impl ());
	auto threshold (threshold_optional_impl ());
	if (!ec)
	{
		...
		for (auto const & account : wallet->accounts ())
		{
			lumex::uint128_t balance = node.ledger.any.account_balance (block_transaction, account).value_or (0).number ();
			if (balance >= threshold.number ())
			{
				boost::property_tree::ptree entry;
				lumex::uint128_t receivable = node.ledger.account_receivable (block_transaction, account);
...
void lumex::json_handler::wallet_change_seed ()
{
	node.workers.post (create_worker_task ([] (std::shared_ptr<lumex::json_handler> const & rpc_l) {
		auto wallet (rpc_l->wallet_impl ());
		if (!rpc_l->ec)
		{
			...
			if (!seed.decode_hex (seed_text))
			{
				...
				if (!wallet->is_locked ())
				{
					lumex::public_key account (wallet->change_seed (seed, count));
					rpc_l->response_l.put ("success", "");
			...
			{
				rpc_l->ec = lumex::error_common::bad_seed;
			}
		}
		rpc_l->response_errors ();
	}));
...
void lumex::json_handler::wallet_contains ()
{
	auto account (account_impl ());
	auto wallet (wallet_impl ());
	if (!ec)
...
void lumex::json_handler::wallet_create ()
{
	node.workers.post (create_worker_task ([] (std::shared_ptr<lumex::json_handler> const & rpc_l) {
		lumex::raw_key seed;
		...
		if (!rpc_l->ec)
		{
			...
			{
				rpc_l->ec = lumex::error_common::wallet_lmdb_max_dbs;
			}
			if (!rpc_l->ec && seed_text.has_value ())
			{
				lumex::public_key account (wallet->change_seed (seed));
				rpc_l->response_l.put ("last_restored_account", account.to_account ());
				auto index (wallet->get_deterministic_index ());
				debug_assert (index > 0);
				rpc_l->response_l.put ("restored_count", std::to_string (index));
...
void lumex::json_handler::wallet_destroy ()
{
	node.workers.post (create_worker_task ([] (std::shared_ptr<lumex::json_handler> const & rpc_l) {
		...
		if (!wallet.decode_hex (wallet_text))
		{
			...
			{
				rpc_l->ec = lumex::error_common::wallet_not_found;
			}
		}
		else
		{
			rpc_l->ec = lumex::error_common::bad_wallet_number;
		}
		rpc_l->response_errors ();
	}));
}

void lumex::json_handler::wallet_export ()
{
	auto wallet (wallet_impl ());
	if (!ec)
	{
...
void lumex::json_handler::wallet_frontiers ()
{
	auto wallet (wallet_impl ());
	if (!ec)
	{
...
void lumex::json_handler::wallet_history ()
{
	uint64_t modified_since (0);
	auto modified_since_text (request.get_optional<std::string> ("modified_since"));
	if (modified_since_text.has_value ())
		...
		for (auto const & account : wallet->accounts ())
		{
			...
			if (info)
			{
				...
				while (timestamp >= modified_since && !hash.is_zero ())
				{
					...
					{
						boost::property_tree::ptree entry;
						std::vector<lumex::public_key> no_filter;
...
void lumex::json_handler::wallet_key_valid ()
{
	auto wallet (wallet_impl ());
	if (!ec)
	{
...
void lumex::json_handler::wallet_ledger ()
{
	bool const representative = request.get<bool> ("representative", false);
	bool const weight = request.get<bool> ("weight", false);
	bool const pending = request.get<bool> ("pending", false);
...
void lumex::json_handler::wallet_lock ()
{
	auto wallet (wallet_impl ());
	if (!ec)
	{
		...
		node.logger.warn (lumex::log::type::rpc, "Wallet locked");
	}
	response_errors ();
}

void lumex::json_handler::wallet_pending ()
{
	response_l.put ("deprecated", "1");
	wallet_receivable ();
}

void lumex::json_handler::wallet_receivable ()
{
	auto wallet (wallet_impl ());
	auto count (count_optional_impl ());
	auto threshold (threshold_optional_impl ());
	...
	if (!ec)
	{
		...
		{
			boost::property_tree::ptree peers_l;
			for (auto ii (node.store.pending.begin (block_transaction, lumex::pending_key (account, 0))), nn (node.store.pending.end (block_transaction)); ii != nn && lumex::pending_key (ii->first).account == account && peers_l.size () < count; ++ii)
			{
				lumex::pending_key key (ii->first);
				if (block_confirmed (node, block_transaction, key.hash, include_active, include_only_confirmed))
				{
					...
					{
						lumex::pending_info info (ii->second);
						if (info.amount.number () >= threshold.number ())
						{
							if (source || min_version)
							{
...
void lumex::json_handler::wallet_representative ()
{
	auto wallet (wallet_impl ());
	if (!ec)
	{
...
void lumex::json_handler::wallet_representative_set ()
{
	node.workers.post (create_worker_task ([] (std::shared_ptr<lumex::json_handler> const & rpc_l) {
		auto wallet (rpc_l->wallet_impl ());
		std::string representative_text (rpc_l->request.get<std::string> ("representative"));
		...
		if (!rpc_l->ec)
		{
			...
			if (!rpc_l->ec && update_existing_accounts)
			{
				...
				{
					wallet->change_async (
					account, representative, [] (std::shared_ptr<lumex::block> const &) {}, 0, false);
				}
			}
		}
		rpc_l->response_errors ();
...
void lumex::json_handler::wallet_republish ()
{
	auto wallet (wallet_impl ());
	auto count (count_impl ());
	if (!ec)
	{
		...
		node.network.flood_block_many (std::move (republish_bundle), lumex::transport::traffic_type::keepalive, 25ms);
		response_l.add_child ("blocks", blocks);
	}
	response_errors ();
}

void lumex::json_handler::wallet_seed ()
{
	auto wallet = wallet_impl ();
	if (!ec)
	{
...
void lumex::json_handler::wallet_work_get ()
{
	auto wallet (wallet_impl ());
	if (!ec)
	{
		...
		for (auto const & account : wallet->accounts ())
		{
			...
			works.put (account.to_account (), lumex::to_string_hex (work));
		}
...
void lumex::json_handler::work_generate ()
{
	std::optional<lumex::account> account;
	...
	if (!ec)
	{
		...
		if (!ec && (difficulty > node.max_work_generate_difficulty (work_version) || difficulty < node.network_params.work.threshold_entry (work_version, lumex::block_type::state)))
		{
			ec = lumex::error_rpc::difficulty_limit;
		}
		...
		if (!ec && request.count ("block"))
		{
			...
			if (block != nullptr)
			{
				...
				{
					ec = lumex::error_rpc::block_root_mismatch;
				}
				...
				{
					ec = lumex::error_rpc::block_work_version_mismatch;
				}
				// Difficulty calculation
				...
				{
					ec = lumex::error_rpc::block_work_enough;
				}
			}
		}
		if (!ec && response_l.empty ())
		{
			...
			auto callback = [rpc_l, hash, work_version, this] (std::optional<uint64_t> const & work_a) {
				if (work_a)
				{
					...
					response_l.put ("work", lumex::to_string_hex (work));
					std::stringstream ostream;
					auto result_difficulty (rpc_l->node.network_params.work.difficulty (work_version, hash, work));
					response_l.put ("difficulty", lumex::to_string_hex (result_difficulty));
					auto result_multiplier = lumex::difficulty::to_multiplier (result_difficulty, node.default_difficulty (work_version));
					response_l.put ("multiplier", lumex::to_string (result_multiplier));
					boost::property_tree::write_json (ostream, response_l);
					rpc_l->response (ostream.str ());
				}
				else
			...
			if (!use_peers)
			{
				if (node.local_work_generation_enabled ())
				{
					...
					if (error)
					{
						ec = lumex::error_common::failure_work_generation;
					}
				}
				else
				{
					ec = lumex::error_common::disabled_local_work_generation;
				}
			}
			else
			{
...
void lumex::json_handler::work_cancel ()
{
	auto hash (hash_impl ());
	if (!ec)
	{
...
void lumex::json_handler::work_get ()
{
	auto wallet (wallet_impl ());
	auto account (account_impl ());
	if (!ec)
	{
		...
		if (work_result)
		{
			response_l.put ("work", lumex::to_string_hex (work_result.value ()));
		}
		else
		{
...
void lumex::json_handler::work_set ()
{
	node.workers.post (create_worker_task ([] (std::shared_ptr<lumex::json_handler> const & rpc_l) {
		auto wallet (rpc_l->wallet_impl ());
		auto account (rpc_l->account_impl ());
		...
		if (!rpc_l->ec)
		{
			...
			{
				rpc_l->ec = lumex::error_common::account_not_found_wallet;
			}
			else
			{
...
void lumex::json_handler::work_validate ()
{
	auto hash (hash_impl ());
	auto work (work_optional_impl ());
	// Default to work_1 if not specified
	...
	if (!ec)
	{
		...
		response_l.put ("valid_receive", (result_difficulty >= node.network_params.work.threshold (work_version, lumex::block_details (lumex::epoch::epoch_2, false, true, false))) ? "1" : "0");
		response_l.put ("difficulty", lumex::to_string_hex (result_difficulty));
		auto result_multiplier = lumex::difficulty::to_multiplier (result_difficulty, node.default_difficulty (work_version));
		response_l.put ("multiplier", lumex::to_string (result_multiplier));
	}
...
void lumex::json_handler::work_peer_add ()
{
	std::string address_text = request.get<std::string> ("address");
	std::string port_text = request.get<std::string> ("port");
	uint16_t port;
...
void lumex::json_handler::work_peers ()
{
...
void lumex::json_handler::work_peers_clear ()
{
...
void lumex::json_handler::populate_backlog ()
{
...
void lumex::inprocess_rpc_handler::process_request (std::string const &, std::string const & body_a, std::function<void (std::string const &)> response_a)
{
	// Note that if the rpc action is async, the shared_ptr<json_handler> lifetime will be extended by the action handler
	auto handler (std::make_shared<lumex::json_handler> (node, node_rpc_config, body_a, response_a, [this] () {
		this->stop_callback ();
		this->stop ();
	}));
...
void lumex::inprocess_rpc_handler::process_request_v2 (rpc_handler_request_params const & params_a, std::string const & body_a, std::function<void (std::shared_ptr<std::string> const &)> response_a)
{
	std::string body_l = params_a.json_envelope (body_a);
	auto handler (std::make_shared<lumex::ipc::flatbuffers_handler> (node, ipc_server, nullptr, node.config.ipc_config));
...
void construct_json (lumex::container_info_component * component, boost::property_tree::ptree & parent)
{
	...
	{
		auto & leaf_info = static_cast<lumex::container_info_leaf *> (component)->get_info ();
	...
	auto composite = static_cast<lumex::container_info_composite *> (component);
...
ipc_json_handler_no_arg_func_map create_ipc_json_handler_no_arg_func_map ()
{
	...
	no_arg_funcs.emplace ("account_balance", &lumex::json_handler::account_balance);
	no_arg_funcs.emplace ("account_block_count", &lumex::json_handler::account_block_count);
	no_arg_funcs.emplace ("account_count", &lumex::json_handler::account_count);
	no_arg_funcs.emplace ("account_create", &lumex::json_handler::account_create);
	no_arg_funcs.emplace ("account_get", &lumex::json_handler::account_get);
	no_arg_funcs.emplace ("account_history", &lumex::json_handler::account_history);
	no_arg_funcs.emplace ("account_info", &lumex::json_handler::account_info);
	no_arg_funcs.emplace ("account_key", &lumex::json_handler::account_key);
	no_arg_funcs.emplace ("account_list", &lumex::json_handler::account_list);
	no_arg_funcs.emplace ("account_move", &lumex::json_handler::account_move);
	no_arg_funcs.emplace ("account_remove", &lumex::json_handler::account_remove);
	no_arg_funcs.emplace ("account_representative", &lumex::json_handler::account_representative);
	no_arg_funcs.emplace ("account_representative_set", &lumex::json_handler::account_representative_set);
	no_arg_funcs.emplace ("account_weight", &lumex::json_handler::account_weight);
	no_arg_funcs.emplace ("accounts_balances", &lumex::json_handler::accounts_balances);
	no_arg_funcs.emplace ("accounts_representatives", &lumex::json_handler::accounts_representatives);
	no_arg_funcs.emplace ("accounts_create", &lumex::json_handler::accounts_create);
	no_arg_funcs.emplace ("accounts_frontiers", &lumex::json_handler::accounts_frontiers);
	no_arg_funcs.emplace ("accounts_pending", &lumex::json_handler::accounts_pending);
	no_arg_funcs.emplace ("accounts_receivable", &lumex::json_handler::accounts_receivable);
	no_arg_funcs.emplace ("active_difficulty", &lumex::json_handler::active_difficulty);
	no_arg_funcs.emplace ("available_supply", &lumex::json_handler::available_supply);
	no_arg_funcs.emplace ("block_info", &lumex::json_handler::block_info);
	no_arg_funcs.emplace ("block", &lumex::json_handler::block_info);
	no_arg_funcs.emplace ("block_confirm", &lumex::json_handler::block_confirm);
	no_arg_funcs.emplace ("blocks", &lumex::json_handler::blocks);
	no_arg_funcs.emplace ("blocks_info", &lumex::json_handler::blocks_info);
	no_arg_funcs.emplace ("block_account", &lumex::json_handler::block_account);
	no_arg_funcs.emplace ("block_count", &lumex::json_handler::block_count);
	no_arg_funcs.emplace ("block_create", &lumex::json_handler::block_create);
	no_arg_funcs.emplace ("block_hash", &lumex::json_handler::block_hash);
	no_arg_funcs.emplace ("bootstrap", &lumex::json_handler::bootstrap);
	no_arg_funcs.emplace ("bootstrap_any", &lumex::json_handler::bootstrap_any);
	no_arg_funcs.emplace ("bootstrap_lazy", &lumex::json_handler::bootstrap_lazy);
	no_arg_funcs.emplace ("bootstrap_status", &lumex::json_handler::bootstrap_status);
	no_arg_funcs.emplace ("confirmation_active", &lumex::json_handler::confirmation_active);
	no_arg_funcs.emplace ("confirmation_history", &lumex::json_handler::confirmation_history);
	no_arg_funcs.emplace ("confirmation_info", &lumex::json_handler::confirmation_info);
	no_arg_funcs.emplace ("confirmation_quorum", &lumex::json_handler::confirmation_quorum);
	no_arg_funcs.emplace ("database_txn_tracker", &lumex::json_handler::database_txn_tracker);
	no_arg_funcs.emplace ("delegators", &lumex::json_handler::delegators);
	no_arg_funcs.emplace ("delegators_count", &lumex::json_handler::delegators_count);
	no_arg_funcs.emplace ("deterministic_key", &lumex::json_handler::deterministic_key);
	no_arg_funcs.emplace ("election_statistics", &lumex::json_handler::election_statistics);
	no_arg_funcs.emplace ("epoch_upgrade", &lumex::json_handler::epoch_upgrade);
	no_arg_funcs.emplace ("frontiers", &lumex::json_handler::frontiers);
	no_arg_funcs.emplace ("frontier_count", &lumex::json_handler::account_count);
	no_arg_funcs.emplace ("keepalive", &lumex::json_handler::keepalive);
	no_arg_funcs.emplace ("key_create", &lumex::json_handler::key_create);
	no_arg_funcs.emplace ("key_expand", &lumex::json_handler::key_expand);
	no_arg_funcs.emplace ("ledger", &lumex::json_handler::ledger);
	no_arg_funcs.emplace ("node_id", &lumex::json_handler::node_id);
	no_arg_funcs.emplace ("node_id_delete", &lumex::json_handler::node_id_delete);
	no_arg_funcs.emplace ("password_change", &lumex::json_handler::password_change);
	no_arg_funcs.emplace ("password_enter", &lumex::json_handler::password_enter);
	no_arg_funcs.emplace ("wallet_unlock", &lumex::json_handler::password_enter);
	no_arg_funcs.emplace ("peers", &lumex::json_handler::peers);
	no_arg_funcs.emplace ("pending", &lumex::json_handler::pending);
	no_arg_funcs.emplace ("pending_exists", &lumex::json_handler::pending_exists);
	no_arg_funcs.emplace ("receivable", &lumex::json_handler::receivable);
	no_arg_funcs.emplace ("receivable_exists", &lumex::json_handler::receivable_exists);
	no_arg_funcs.emplace ("process", &lumex::json_handler::process);
	no_arg_funcs.emplace ("pruned_exists", &lumex::json_handler::pruned_exists);
	no_arg_funcs.emplace ("receive", &lumex::json_handler::receive);
	no_arg_funcs.emplace ("receive_minimum", &lumex::json_handler::receive_minimum);
	no_arg_funcs.emplace ("receive_minimum_set", &lumex::json_handler::receive_minimum_set);
	no_arg_funcs.emplace ("representatives", &lumex::json_handler::representatives);
	no_arg_funcs.emplace ("representatives_online", &lumex::json_handler::representatives_online);
	no_arg_funcs.emplace ("republish", &lumex::json_handler::republish);
	no_arg_funcs.emplace ("search_pending", &lumex::json_handler::search_pending);
	no_arg_funcs.emplace ("search_receivable", &lumex::json_handler::search_receivable);
	no_arg_funcs.emplace ("search_pending_all", &lumex::json_handler::search_pending_all);
	no_arg_funcs.emplace ("search_receivable_all", &lumex::json_handler::search_receivable_all);
	no_arg_funcs.emplace ("send", &lumex::json_handler::send);
	no_arg_funcs.emplace ("sign", &lumex::json_handler::sign);
	no_arg_funcs.emplace ("stats", &lumex::json_handler::stats);
	no_arg_funcs.emplace ("stats_clear", &lumex::json_handler::stats_clear);
	no_arg_funcs.emplace ("stop", &lumex::json_handler::stop);
	no_arg_funcs.emplace ("telemetry", &lumex::json_handler::telemetry);
	no_arg_funcs.emplace ("unchecked", &lumex::json_handler::unchecked);
	no_arg_funcs.emplace ("unchecked_clear", &lumex::json_handler::unchecked_clear);
	no_arg_funcs.emplace ("unchecked_get", &lumex::json_handler::unchecked_get);
	no_arg_funcs.emplace ("unchecked_keys", &lumex::json_handler::unchecked_keys);
	no_arg_funcs.emplace ("unopened", &lumex::json_handler::unopened);
	no_arg_funcs.emplace ("uptime", &lumex::json_handler::uptime);
	no_arg_funcs.emplace ("validate_account_number", &lumex::json_handler::validate_account_number);
	no_arg_funcs.emplace ("version", &lumex::json_handler::version);
	no_arg_funcs.emplace ("wallet_add", &lumex::json_handler::wallet_add);
	no_arg_funcs.emplace ("wallet_add_watch", &lumex::json_handler::wallet_add_watch);
	no_arg_funcs.emplace ("wallet_balances", &lumex::json_handler::wallet_balances);
	no_arg_funcs.emplace ("wallet_change_seed", &lumex::json_handler::wallet_change_seed);
	no_arg_funcs.emplace ("wallet_contains", &lumex::json_handler::wallet_contains);
	no_arg_funcs.emplace ("wallet_create", &lumex::json_handler::wallet_create);
	no_arg_funcs.emplace ("wallet_destroy", &lumex::json_handler::wallet_destroy);
	no_arg_funcs.emplace ("wallet_export", &lumex::json_handler::wallet_export);
	no_arg_funcs.emplace ("wallet_frontiers", &lumex::json_handler::wallet_frontiers);
	no_arg_funcs.emplace ("wallet_history", &lumex::json_handler::wallet_history);
	no_arg_funcs.emplace ("wallet_info", &lumex::json_handler::wallet_info);
	no_arg_funcs.emplace ("wallet_balance_total", &lumex::json_handler::wallet_info);
	no_arg_funcs.emplace ("wallet_key_valid", &lumex::json_handler::wallet_key_valid);
	no_arg_funcs.emplace ("wallet_ledger", &lumex::json_handler::wallet_ledger);
	no_arg_funcs.emplace ("wallet_lock", &lumex::json_handler::wallet_lock);
	no_arg_funcs.emplace ("wallet_pending", &lumex::json_handler::wallet_pending);
	no_arg_funcs.emplace ("wallet_receivable", &lumex::json_handler::wallet_receivable);
	no_arg_funcs.emplace ("wallet_representative", &lumex::json_handler::wallet_representative);
	no_arg_funcs.emplace ("wallet_representative_set", &lumex::json_handler::wallet_representative_set);
	no_arg_funcs.emplace ("wallet_republish", &lumex::json_handler::wallet_republish);
	no_arg_funcs.emplace ("wallet_work_get", &lumex::json_handler::wallet_work_get);
	no_arg_funcs.emplace ("work_generate", &lumex::json_handler::work_generate);
	no_arg_funcs.emplace ("work_cancel", &lumex::json_handler::work_cancel);
	no_arg_funcs.emplace ("work_get", &lumex::json_handler::work_get);
	no_arg_funcs.emplace ("work_set", &lumex::json_handler::work_set);
	no_arg_funcs.emplace ("work_validate", &lumex::json_handler::work_validate);
	no_arg_funcs.emplace ("work_peer_add", &lumex::json_handler::work_peer_add);
	no_arg_funcs.emplace ("work_peers", &lumex::json_handler::work_peers);
	no_arg_funcs.emplace ("work_peers_clear", &lumex::json_handler::work_peers_clear);
	no_arg_funcs.emplace ("populate_backlog", &lumex::json_handler::populate_backlog);
	no_arg_funcs.emplace ("bootstrap_priorities", &lumex::json_handler::bootstrap_priorities);
	no_arg_funcs.emplace ("bootstrap_reset", &lumex::json_handler::bootstrap_reset);
	return no_arg_funcs;
}
...
char const * epoch_as_string (lumex::epoch epoch)
{
	switch (epoch)
	{
