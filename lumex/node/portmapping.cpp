...
std::string lumex::mapping_protocol::to_string ()
{
	std::stringstream ss;
...
lumex::port_mapping::port_mapping (lumex::node & node_a) :
	node (node_a),
...
void lumex::port_mapping::start ()
{
	...
	thread = std::thread ([this] {
		lumex::thread_role::set (lumex::thread_role::name::port_mapping);
		run ();
	});
}

void lumex::port_mapping::stop ()
{
	{
...
void lumex::port_mapping::shutdown ()
{
	node.logger.debug (lumex::log::type::upnp, "UPnP shutdown...");
	...
	for (auto & protocol : protocols | boost::adaptors::filtered ([] (auto const & p) { return p.enabled; }))
	{
		if (protocol.external_port != 0)
		{
			...
			if (delete_error_l)
			{
				node.logger.warn (lumex::log::type::upnp, "UPnP shutdown {} port mapping failed: {} ({})",
				protocol.name,
				delete_error_l,
			...
			{
				node.logger.info (lumex::log::type::upnp, "UPnP shutdown {} port mapping successful: {}:{}",
				protocol.name,
...
std::string lumex::port_mapping::get_config_port (std::string const & node_port_a)
{
	return node.config.external_port != 0 ? std::to_string (node.config.external_port) : node_port_a;
}

std::string lumex::port_mapping::to_string ()
{
	std::stringstream ss;
...
void lumex::port_mapping::refresh_devices ()
{
	upnp_state upnp_l;
	...
	node.logger.log ((check_count % 15 == 0) ? lumex::log::level::info : lumex::log::level::debug,
	lumex::log::type::upnp, "UPnP local address: {}, discovery: {}, IGD search: {}",
	...
	{
		node.logger.debug (lumex::log::type::upnp, "UPnP device url: {}, st: {}, usn: {}", i->descURL, i->st, i->usn);
	}
...
lumex::endpoint lumex::port_mapping::external_address ()
{
	lumex::endpoint result_l (boost::asio::ip::address_v6{}, 0);
	lumex::lock_guard<lumex::mutex> guard_l (mutex);
	for (auto & protocol : protocols | boost::adaptors::filtered ([] (auto const & p) { return p.enabled; }))
	{
		if (protocol.external_port != 0)
		{
			result_l = lumex::endpoint (protocol.external_address, protocol.external_port);
		}
	}
	return result_l;
}

void lumex::port_mapping::refresh_mapping ()
{
	lumex::lock_guard<lumex::mutex> guard_l (mutex);
	...
	for (auto & protocol : protocols | boost::adaptors::filtered ([] (auto const & p) { return p.enabled; }))
	{
		auto upnp_description = fmt::format ("Lumex Node ({})", node.network_params.network.get_current_network_as_string ());
		std::string address_str = address.to_string ();
		...
		if (add_port_mapping_error_l == UPNPCOMMAND_SUCCESS)
		{
			...
			node.logger.info (lumex::log::type::upnp, "UPnP {} {}:{} mapped to: {}",
			protocol.name,
			protocol.external_address,
			config_port_l,
			node_port_l);
		...
		else
		{
			...
			node.logger.warn (lumex::log::type::upnp, "UPnP {} {}:{} failed: {} ({})",
			protocol.name,
			protocol.external_address,
			config_port_l,
...
bool lumex::port_mapping::check_lost_or_old_mapping ()
{
	bool result_l (false);
	lumex::lock_guard<lumex::mutex> guard_l (mutex);
	auto node_port_l (std::to_string (node.network.endpoint ().port ()));
	...
	for (auto & protocol : protocols | boost::adaptors::filtered ([] (auto const & p) { return p.enabled; }))
	{
		...
		if (verify_port_mapping_error_l != UPNPCOMMAND_SUCCESS)
		{
			...
			node.logger.warn (lumex::log::type::upnp, "UPnP get specific port mapping failed: {} ({})",
		...
		if (!recent_lease)
		{
			...
			node.logger.info (lumex::log::type::upnp, "UPnP lease time getting old, remaining time: {}, lease time: {}, below the threshold: {}",
		...
		else
		{
			...
			node.logger.warn (lumex::log::type::upnp, "UPnP get external ip address failed: {} ({})",
			external_ip_error_l,
			strupnperror (external_ip_error_l));
		}

		node.logger.debug (lumex::log::type::upnp, "UPnP {} mapping verification response: {}, external ip response: {}, external ip: {}, internal ip: {}, remaining lease: {}",
		protocol.name,
		verify_port_mapping_error_l,
		external_ip_error_l,
...
void lumex::port_mapping::check_mapping ()
{
	...
	if (upnp.devices != nullptr)
	{
		...
		{
			node.logger.info (lumex::log::type::upnp, "UPnP no need to refresh the mapping");
		}
	}
	else
	{
		// Bump logging level periodically
		node.logger.log ((check_count % 15 == 0) ? lumex::log::level::info : lumex::log::level::debug,
		lumex::log::type::upnp, "UPnP no IGD devices found");
	}

	++check_count;
}

void lumex::port_mapping::run ()
{
	lumex::unique_lock<lumex::mutex> lock{ mutex };
	while (!stopped)
	{
		node.stats.inc (lumex::stat::type::port_mapping, lumex::stat::detail::loop);
...
std::string lumex::upnp_state::to_string ()
{
	std::stringstream ss;
...
lumex::upnp_state & lumex::upnp_state::operator= (lumex::upnp_state && other_a)
{
	if (this == &other_a)
	{
		return *this;
