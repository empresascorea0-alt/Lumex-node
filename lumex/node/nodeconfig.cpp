...
std::string const default_live_peer_network = lumex::env::get ("LUMEX_DEFAULT_PEER").value_or ("peering.lumex.org");
std::string const default_beta_peer_network = lumex::env::get ("LUMEX_DEFAULT_PEER").value_or ("peering-beta.lumex.org");
std::string const default_test_peer_network = lumex::env::get ("LUMEX_DEFAULT_PEER").value_or ("peering-test.lumex.org");
}

lumex::node_config::node_config (lumex::network_params const & network_params) :
	...
	local_block_broadcaster{ network_params.network }
{
	switch (network_params.network.network ())
	{
		...
		case lumex::network_type::lumex_beta_network:
		{
			...
			lumex::account offline_representative;
			release_assert (!offline_representative.decode_account ("lumex_1defau1t9off1ine9rep99999999999999999999999999999999wgmuzxxy"));
...
lumex::node_config::node_config (node_config const &) = default;
lumex::node_config::node_config (node_config &&) noexcept = default;
lumex::node_config::~node_config () = default;

lumex::error lumex::node_config::serialize_toml (lumex::tomlconfig & toml) const
{
	...
	lumex::tomlconfig experimental_l;
...
lumex::error lumex::node_config::deserialize_toml (lumex::tomlconfig & toml)
{
	try
	{
...
void lumex::node_config::deserialize_address (std::string const & entry_a, std::vector<std::pair<std::string, uint16_t>> & container_a)
{
...
lumex::account lumex::node_config::random_representative () const
{
	debug_assert (!preconfigured_representatives.empty ());
	std::size_t index (lumex::random_pool::generate_word32 (0, static_cast<CryptoPP::word32> (preconfigured_representatives.size () - 1)));
	auto result (preconfigured_representatives[index]);
	return result;
}

std::optional<unsigned> lumex::node_config::env_io_threads ()
{
	static auto const value = [] () {
		auto value = lumex::env::get<unsigned> ("LUMEX_IO_THREADS");
		if (value)
		{
			std::cerr << "IO threads overridden by LUMEX_IO_THREADS environment variable: " << *value << std::endl;
		}
		return value;
	}();
	return value;
