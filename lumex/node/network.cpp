#include <lumex/crypto_lib/random_pool_shuffle.hpp>
...
std::size_t lumex::network::confirm_req_hashes_max{ 255 };
std::size_t lumex::network::confirm_ack_hashes_max{ 255 };

/*
 * network
 */

lumex::network::network (lumex::node & node_a, uint16_t port_a) :
	...
	port{ port_a }
{
	node.observers.channel_connected.add ([this] (std::shared_ptr<lumex::transport::channel> const & channel) {
		node.stats.inc (lumex::stat::type::network, lumex::stat::detail::connected);
		node.logger.debug (lumex::log::type::network, "Connected to: {}", channel);
	});
}
...
void lumex::network::start ()
{
	cleanup_thread = std::thread ([this] () {
		lumex::thread_role::set (lumex::thread_role::name::network_cleanup);
		run_cleanup ();
	});

	keepalive_thread = std::thread ([this] () {
		lumex::thread_role::set (lumex::thread_role::name::network_keepalive);
		run_keepalive ();
	});
	...
	if (!node.flags.disable_reachout && config.peer_reachout.count () > 0)
	{
		reachout_thread = std::thread ([this] () {
			lumex::thread_role::set (lumex::thread_role::name::network_reachout);
			run_reachout ();
		});
	}
	else
	{
		node.logger.warn (lumex::log::type::network, "Peer reachout is disabled");
	}
	...
	{
		node.logger.warn (lumex::log::type::network, "Cached peer reachout is disabled");
	}
	...
	{
		node.logger.warn (lumex::log::type::network, "Realtime TCP is disabled");
	}
}

void lumex::network::stop ()
{
	{
...
void lumex::network::run_cleanup ()
{
	...
	while (!stopped)
	{
		...
		node.stats.inc (lumex::stat::type::network, lumex::stat::detail::loop_cleanup);
...
void lumex::network::run_keepalive ()
{
	lumex::unique_lock<lumex::mutex> lock{ mutex };
	while (!stopped)
	{
		...
		node.stats.inc (lumex::stat::type::network, lumex::stat::detail::loop_keepalive);
...
void lumex::network::run_reachout ()
{
	lumex::unique_lock<lumex::mutex> lock{ mutex };
	while (!stopped)
	{
		...
		node.stats.inc (lumex::stat::type::network, lumex::stat::detail::loop_reachout);
		...
		if (keepalive)
		{
			for (auto const & peer : keepalive->peers)
			{
				...
				node.stats.inc (lumex::stat::type::network, lumex::stat::detail::reachout_live);
...
void lumex::network::run_reachout_cached ()
{
	lumex::unique_lock<lumex::mutex> lock{ mutex };
	while (!stopped)
	{
		...
		node.stats.inc (lumex::stat::type::network, lumex::stat::detail::loop_reachout_cached);
		...
		for (auto const & peer : cached_peers)
		{
			...
			node.stats.inc (lumex::stat::type::network, lumex::stat::detail::reachout_cached);
...
void lumex::network::run_reachout_preconfigured ()
{
	lumex::unique_lock<lumex::mutex> lock{ mutex };
	while (!stopped)
	{
		...
		node.stats.inc (lumex::stat::type::network, lumex::stat::detail::loop_reachout_preconfigured);
...
void lumex::network::trigger_reachout ()
{
	node.stats.inc (lumex::stat::type::network, lumex::stat::detail::trigger_reachout);
	reachout_preconfigured_interval.reset ();
	condition.notify_all ();
}

void lumex::network::reachout (std::string const & address_a, uint16_t port_a)
{
	...
	resolver.async_resolve (address_a, std::to_string (port_a), [this, node_l, address_a, port_a] (boost::system::error_code const & ec, boost::asio::ip::tcp::resolver::results_type results) {
		if (!ec)
		{
			...
			{
				auto endpoint (lumex::transport::map_endpoint_to_v6 (i.endpoint ()));
		...
		{
			node.logger.error (lumex::log::type::network, "Error resolving address for reachout: {}:{} ({})", address_a, port_a, ec.message ());
		}
	});
}

void lumex::network::reachout_preconfigured ()
{
	...
	node.stats.inc (lumex::stat::type::network, lumex::stat::detail::reachout_preconfigured);
...
void lumex::network::send_keepalive (std::shared_ptr<lumex::transport::channel> const & channel) const
{
...
void lumex::network::send_keepalive_self (std::shared_ptr<lumex::transport::channel> const & channel) const
{
	lumex::messages::keepalive message{ node.network_params.network };
...
bool lumex::network::check_capacity (lumex::transport::traffic_type type, size_t target_count) const
{
...
bool lumex::network::check_capacity_fanout (lumex::transport::traffic_type type, float scale) const
{
	return check_capacity (type, fanout (scale));
}

bool lumex::network::check_capacity_ratio (lumex::transport::traffic_type type, float ratio) const
{
	auto const target_count = static_cast<size_t> (std::ceil (size () * ratio));
	return check_capacity (type, target_count);
}

size_t lumex::network::flood_message (lumex::messages::message const & message, lumex::transport::traffic_type type, float scale) const
{
...
size_t lumex::network::flood_message_all (lumex::messages::message const & message, lumex::transport::traffic_type type) const
{
...
size_t lumex::network::flood_keepalive (float scale) const
{
	lumex::messages::keepalive message{ node.network_params.network };
	random_fill (message.peers);
	return flood_message (message, lumex::transport::traffic_type::keepalive, scale);
}

size_t lumex::network::flood_keepalive_self (float scale) const
{
	lumex::messages::keepalive message{ node.network_params.network };
	fill_keepalive_self (message.peers);
	return flood_message (message, lumex::transport::traffic_type::keepalive, scale);
...
size_t lumex::network::flood_vote (std::shared_ptr<lumex::vote> const & vote, lumex::transport::traffic_type type, bool rebroadcasted) const
{
	lumex::messages::confirm_ack message{ node.network_params.network, vote, rebroadcasted };
	return flood_message (message, type);
}

size_t lumex::network::flood_vote_all (std::shared_ptr<lumex::vote> const & vote, lumex::transport::traffic_type type, bool rebroadcasted) const
{
	lumex::messages::confirm_ack message{ node.network_params.network, vote, rebroadcasted };
	return flood_message_all (message, type);
}

size_t lumex::network::flood_vote_non_pr (std::shared_ptr<lumex::vote> const & vote, float scale, lumex::transport::traffic_type type) const
{
	lumex::messages::confirm_ack message{ node.network_params.network, vote };
...
size_t lumex::network::flood_vote_pr (std::shared_ptr<lumex::vote> const & vote, lumex::transport::traffic_type type) const
{
	lumex::messages::confirm_ack message{ node.network_params.network, vote };
...
size_t lumex::network::flood_block (std::shared_ptr<lumex::block> const & block, lumex::transport::traffic_type type) const
{
	lumex::messages::publish message{ node.network_params.network, block };
	return flood_message (message, type);
}

size_t lumex::network::flood_block_initial (std::shared_ptr<lumex::block> const & block) const
{
	lumex::messages::publish message{ node.network_params.network, block, /* is_originator */ true };

	size_t result = 0;
	for (auto const & rep : node.rep_crawler.principal_representatives ())
	{
		bool sent = rep.channel->send (message, lumex::transport::traffic_type::block_broadcast_initial);
		result += sent;
	}
	for (auto & peer : list_non_pr (fanout (1.0)))
	{
		bool sent = peer->send (message, lumex::transport::traffic_type::block_broadcast_initial);
		result += sent;
	}
	return result;
}

size_t lumex::network::flood_block_all (std::shared_ptr<lumex::block> const & block, lumex::transport::traffic_type type) const
{
	lumex::messages::publish message{ node.network_params.network, block };
	return flood_message_all (message, type);
}

void lumex::network::flood_block_many (std::deque<std::shared_ptr<lumex::block>> blocks, lumex::transport::traffic_type type, std::chrono::milliseconds delay, std::function<void ()> callback) const
{
	if (blocks.empty ())
	{
		return;
	...
	if (!blocks.empty ())
	{
		std::weak_ptr<lumex::node> node_w (node.shared ());
...
void lumex::network::merge_peers (std::array<lumex::endpoint, 8> const & peers_a)
{
...
bool lumex::network::merge_peer (lumex::endpoint const & peer)
{
	if (track_reachout (peer))
	{
		node.stats.inc (lumex::stat::type::network, lumex::stat::detail::merge_peer);
		node.logger.debug (lumex::log::type::network, "Initiating peer merge: {}", peer);
		bool started = tcp_channels.start_tcp (peer);
		if (!started)
		{
			node.stats.inc (lumex::stat::type::tcp, lumex::stat::detail::merge_peer_failed);
			node.logger.debug (lumex::log::type::network, "Peer merge failed: {}", peer);
		}
		return started;
	}
...
bool lumex::network::not_a_peer (lumex::endpoint const & endpoint_a, bool allow_local_peers) const
{
	bool result (false);
	...
	else if (lumex::transport::reserved_address (endpoint_a, allow_local_peers))
	{
		result = true;
	}
...
bool lumex::network::track_reachout (lumex::endpoint const & endpoint_a)
{
...
std::deque<std::shared_ptr<lumex::transport::channel>> lumex::network::list (std::size_t max_count, channel_filter filter) const
{
	auto result = tcp_channels.list (filter);
	lumex::random_pool_shuffle (result.begin (), result.end ()); // Randomize returned peer order
...
std::deque<std::shared_ptr<lumex::transport::channel>> lumex::network::list_non_pr (std::size_t max_count, channel_filter filter) const
{
	auto result = tcp_channels.list (filter);

	auto partition_point = std::partition (result.begin (), result.end (),
	[this] (std::shared_ptr<lumex::transport::channel> const & channel) {
...
std::deque<std::shared_ptr<lumex::transport::channel>> lumex::network::list (std::size_t max_count, uint8_t minimum_version) const
{
	return list (max_count, [minimum_version] (auto const & channel) { return channel->get_network_version () >= minimum_version; });
}

std::deque<std::shared_ptr<lumex::transport::channel>> lumex::network::list_non_pr (std::size_t max_count, uint8_t minimum_version) const
{
...
std::size_t lumex::network::fanout (float scale) const
{
...
std::unordered_set<std::shared_ptr<lumex::transport::channel>> lumex::network::random_set (std::size_t max_count, uint8_t minimum_version) const
{
	return tcp_channels.random_set (max_count, minimum_version);
}

void lumex::network::random_fill (std::array<lumex::endpoint, 8> & target_a) const
{
...
void lumex::network::fill_keepalive_self (std::array<lumex::endpoint, 8> & target_a) const
{
	...
	target_a[1] = lumex::endpoint (boost::asio::ip::address_v6{}, 0); // For node v19 (response channels)
	if (node.config.external_address != boost::asio::ip::address_v6{}.to_string () && node.config.external_port != 0)
	{
		target_a[0] = lumex::endpoint (boost::asio::ip::make_address_v6 (node.config.external_address), node.config.external_port);
	}
	else
	{
		...
		{
			target_a[0] = lumex::endpoint (boost::asio::ip::address_v6{}, port);
			boost::system::error_code ec;
			auto external_v6 = boost::asio::ip::make_address_v6 (external_address.address ().to_string (), ec);
			target_a[1] = lumex::endpoint (external_v6, external_address.port ());
		}
		else
		{
			target_a[0] = lumex::endpoint (boost::asio::ip::address_v6{}, port);
...
lumex::tcp_endpoint lumex::network::bootstrap_peer ()
{
	return tcp_channels.bootstrap_peer ();
}

std::shared_ptr<lumex::transport::channel> lumex::network::find_channel (lumex::endpoint const & endpoint_a)
{
	return tcp_channels.find_channel (lumex::transport::map_endpoint_to_tcp (endpoint_a));
}

std::shared_ptr<lumex::transport::channel> lumex::network::find_node_id (lumex::account const & node_id_a)
{
	return tcp_channels.find_node_id (node_id_a);
}

lumex::endpoint lumex::network::endpoint () const
{
	return lumex::endpoint{ boost::asio::ip::address_v6::loopback (), port };
}

void lumex::network::cleanup (std::chrono::steady_clock::time_point const & cutoff)
{
...
std::size_t lumex::network::size () const
{
	return tcp_channels.size ();
}

float lumex::network::size_log () const
{
	auto size_l = std::max (static_cast<size_t> (1u), size ()); // Clamp size to domain of std::log
	return static_cast<float> (std::log (size_l));
}

bool lumex::network::empty () const
{
	return size () == 0;
}

void lumex::network::erase (lumex::transport::channel const & channel_a)
{
	auto const channel_type = channel_a.get_type ();
	if (channel_type == lumex::transport::transport_type::tcp)
	{
		tcp_channels.erase (channel_a.get_remote_endpoint ());
	}
}

void lumex::network::exclude (std::shared_ptr<lumex::transport::channel> const & channel)
{
...
bool lumex::network::verify_handshake_response (const lumex::messages::node_id_handshake::response_payload & response, const lumex::endpoint & remote_endpoint)
{
	// Prevent connection with ourselves
	if (response.node_id == node.node_id.pub)
	{
		node.stats.inc (lumex::stat::type::handshake, lumex::stat::detail::invalid_node_id);
		return false; // Fail
	}
	...
	{
		node.stats.inc (lumex::stat::type::handshake, lumex::stat::detail::invalid_genesis);
		return false; // Fail
	}
	...
	if (!cookie)
	{
		node.stats.inc (lumex::stat::type::handshake, lumex::stat::detail::missing_cookie);
		return false; // Fail
	}

	if (!response.validate (*cookie))
	{
		node.stats.inc (lumex::stat::type::handshake, lumex::stat::detail::invalid_signature);
		return false; // Fail
	}

	node.stats.inc (lumex::stat::type::handshake, lumex::stat::detail::ok);
	return true; // OK
}

std::optional<lumex::messages::node_id_handshake::query_payload> lumex::network::prepare_handshake_query (const lumex::endpoint & remote_endpoint)
{
	if (auto cookie = syn_cookies.assign (remote_endpoint); cookie)
	{
		lumex::messages::node_id_handshake::query_payload query{ *cookie };
		return query;
	}
	return std::nullopt;
}

lumex::messages::node_id_handshake::response_payload lumex::network::prepare_handshake_response (const lumex::messages::node_id_handshake::query_payload & query, lumex::messages::handshake_version version) const
{
	using handshake_version = lumex::messages::handshake_version;
	using response_payload = lumex::messages::node_id_handshake::response_payload;
	...
	switch (version)
	{
		case handshake_version::v3:
		{
			...
			v3.salt = lumex::random_pool::generate<uint256_union> ();
		...
		case handshake_version::v2:
		{
			...
			v2.salt = lumex::random_pool::generate<uint256_union> ();
...
lumex::container_info lumex::network::container_info () const
{
...
lumex::syn_cookies::syn_cookies (std::size_t max_cookies_per_ip_a, lumex::logger & logger_a) :
	max_cookies_per_ip (max_cookies_per_ip_a),
	logger (logger_a)
{
}

std::optional<lumex::uint256_union> lumex::syn_cookies::assign (lumex::endpoint const & endpoint_a)
{
	auto ip_addr (endpoint_a.address ());
	debug_assert (ip_addr.is_v6 ());
	lumex::lock_guard<lumex::mutex> lock{ syn_cookie_mutex };
	unsigned & ip_cookies = cookies_per_ip[ip_addr];
	std::optional<lumex::uint256_union> result;
	if (ip_cookies < max_cookies_per_ip)
	{
		if (cookies.find (endpoint_a) == cookies.end ())
		{
			lumex::uint256_union query;
...
bool lumex::syn_cookies::validate (lumex::endpoint const & endpoint_a, lumex::account const & node_id, lumex::signature const & sig)
{
	auto ip_addr (endpoint_a.address ());
	debug_assert (ip_addr.is_v6 ());
	lumex::lock_guard<lumex::mutex> lock{ syn_cookie_mutex };
	...
	if (cookie_it != cookies.end () && !lumex::validate_message (node_id, cookie_it->second.cookie, sig))
	{
		result = false;
...
void lumex::syn_cookies::purge (std::chrono::steady_clock::time_point const & cutoff_a)
{
	lumex::lock_guard<lumex::mutex> lock{ syn_cookie_mutex };
...
std::optional<lumex::uint256_union> lumex::syn_cookies::cookie (const lumex::endpoint & endpoint_a)
{
	auto ip_addr (endpoint_a.address ());
	debug_assert (ip_addr.is_v6 ());
	lumex::lock_guard<lumex::mutex> lock{ syn_cookie_mutex };
...
std::size_t lumex::syn_cookies::cookies_size () const
{
	lumex::lock_guard<lumex::mutex> lock{ syn_cookie_mutex };
	return cookies.size ();
}

lumex::container_info lumex::syn_cookies::container_info () const
{
	lumex::lock_guard<lumex::mutex> syn_cookie_guard{ syn_cookie_mutex };
...
lumex::error lumex::network_config::serialize (lumex::tomlconfig & toml) const
{
...
lumex::error lumex::network_config::deserialize (lumex::tomlconfig & toml)
{
