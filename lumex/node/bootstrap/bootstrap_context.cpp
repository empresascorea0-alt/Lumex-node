...
{
bootstrap_context::bootstrap_context (lumex::node_config const & node_config_a, lumex::ledger & ledger_a,
lumex::ledger_notifications & ledger_notifications_a, lumex::block_processor & block_processor_a, lumex::network & network_a, lumex::stats & stat_a, lumex::logger & logger_a) :
	...
	workers{ 1, lumex::thread_role::name::bootstrap_worker }
{
...
void bootstrap_context::start ()
{
	...
	if (!config.enable)
	{
		logger.warn (lumex::log::type::bootstrap, "Bootstrap is disabled, node will not be able to synchronize with the network");
		return;
	}
	...
	cleanup_thread = std::thread ([this] () {
		lumex::thread_role::set (lumex::thread_role::name::bootstrap_cleanup);
		run_cleanup ();
	});
}
...
void bootstrap_context::stop ()
{
	...
	frontier_strat.stop ();
	lumex::join_or_pass (cleanup_thread);
...
void bootstrap_context::reset ()
{
	...
	stats.inc (lumex::stat::type::bootstrap, lumex::stat::detail::reset);
	logger.info (lumex::log::type::bootstrap, "Resetting bootstrap state");
...
bool bootstrap_context::send (std::shared_ptr<lumex::transport::channel> const & channel, lumex::messages::asc_pull_req && message, async_tag tag)
{
	...
	message, lumex::transport::traffic_type::bootstrap_requests, [this, id = tag.id] (auto const & ec, auto size) {
		lumex::lock_guard<lumex::mutex> lock{ mutex };
		if (auto it = tags.get<tag_id> ().find (id); it != tags.get<tag_id> ().end ())
		{
			stats.inc (lumex::stat::type::bootstrap_request_ec, lumex::to_stat_detail (ec), lumex::stat::dir::out);
			if (!ec)
			{
				stats.inc (lumex::stat::type::bootstrap, lumex::stat::detail::request_success, lumex::stat::dir::out);
			...
			{
				stats.inc (lumex::stat::type::bootstrap, lumex::stat::detail::request_failed, lumex::stat::dir::out);
	...
	if (sent)
	{
		stats.inc (lumex::stat::type::bootstrap, lumex::stat::detail::request);
		stats.inc (lumex::stat::type::bootstrap_request, to_stat_detail (tag.type));
	}
	else
	{
		stats.inc (lumex::stat::type::bootstrap, lumex::stat::detail::request_failed);
	}
...
{
	wait ([this] () {
		return block_processor.size (lumex::block_source::bootstrap) < config.block_processor_threshold;
	});
}

std::shared_ptr<lumex::transport::channel> bootstrap_context::wait_channel ()
{
...
size_t bootstrap_context::count_tags (lumex::account const & account, query_source source) const
{
...
size_t bootstrap_context::count_tags (lumex::block_hash const & hash, query_source source) const
{
	debug_assert (!mutex.try_lock ());
...
bool bootstrap_context::request (lumex::account account, size_t count, std::shared_ptr<lumex::transport::channel> const & channel, query_source source)
{
	debug_assert (count > 0);
	debug_assert (count <= lumex::bootstrap_server::max_blocks);
...

	{
		...
		if (auto info = ledger.store.account.get (transaction, account))
		{
			...
			{
				stats.inc (lumex::stat::type::bootstrap_request_blocks, lumex::stat::detail::optimistic);

				tag.type = query_type::blocks_by_hash;
				payload.start = info->head;
				tag.hash = info->head;

				logger.debug (lumex::log::type::bootstrap, "Requesting blocks for {} starting from account frontier: {} (optimistic: {}) from: {}",
				account,
				payload.start,
				optimistic_request,
				channel);
			...
			{
				stats.inc (lumex::stat::type::bootstrap_request_blocks, lumex::stat::detail::safe);
				...
				if (auto conf_info = ledger.store.confirmation_height.get (transaction, account))
				{
					...
					logger.debug (lumex::log::type::bootstrap, "Requesting blocks for {} starting from confirmation frontier: {} (optimistic: {}) from: {}",
					account,
					payload.start,
					optimistic_request,
					channel);
				...
				else
				{
					...
					logger.debug (lumex::log::type::bootstrap, "Requesting blocks for {} starting from account root (optimistic: {}) from: {}",
					account,
					optimistic_request,
					channel);
				}
		...
		{
			stats.inc (lumex::stat::type::bootstrap_request_blocks, lumex::stat::detail::base);

			tag.type = query_type::blocks_by_account;
			payload.start = account;

			logger.debug (lumex::log::type::bootstrap, "Requesting blocks for {} from: {}", account, channel);
		}
	}
	...
	// Build the message
	lumex::messages::asc_pull_req message{ network_constants };
	message.id = tag.id;
	message.type = lumex::messages::asc_pull_type::blocks;

	lumex::messages::asc_pull_req::blocks_payload msg_pld;
	msg_pld.start = payload.start;
	msg_pld.count = payload.count;
	msg_pld.start_type = tag.type == query_type::blocks_by_hash ? lumex::messages::asc_pull_req::hash_type::block : lumex::messages::asc_pull_req::hash_type::account;
 ...
 */
void bootstrap_context::inspect (secure::transaction const & tx, lumex::block_status const & result, lumex::block const & block, lumex::block_source source)
{
	debug_assert (!mutex.try_lock ());
	...
	switch (result)
	{
		case lumex::block_status::progress:
		{
			...
			if (source != lumex::block_source::live)
			{
		...
		case lumex::block_status::gap_source:
		{
			...
			if (source == lumex::block_source::bootstrap)
			{
		...
		case lumex::block_status::gap_previous:
		{
			...
			if (source == lumex::block_source::live && !accounts.priority_half_full () && !accounts.blocked_half_full ())
			{
...
void bootstrap_context::cleanup ()
{
	...
	while (!tags_by_order.empty () && should_timeout (tags_by_order.front ()))
	{
		...
		stats.inc (lumex::stat::type::bootstrap, lumex::stat::detail::timeout);
		stats.inc (lumex::stat::type::bootstrap_timeout, to_stat_detail (tag.type));
...
void bootstrap_context::run_cleanup ()
{
	...
	while (!stopped)
	{
		stats.inc (lumex::stat::type::bootstrap, lumex::stat::detail::loop_cleanup);
		cleanup ();
		condition.wait_for (lock, lumex::is_dev_run () ? 500ms : 5s, [this] () { return stopped; });
	}
}

void bootstrap_context::process (lumex::messages::asc_pull_ack const & message, std::shared_ptr<lumex::transport::channel> const & channel)
{
	lumex::unique_lock<lumex::mutex> lock{ mutex };
	...
	{
		stats.inc (lumex::stat::type::bootstrap, lumex::stat::detail::missing_tag);
		return;
	}

	stats.inc (lumex::stat::type::bootstrap, lumex::stat::detail::reply);
	...
	struct payload_verifier
	{
		...
		bool operator() (const lumex::messages::asc_pull_ack::blocks_payload & response) const
		{
			return type == query_type::blocks_by_hash || type == query_type::blocks_by_account;
		}
		bool operator() (const lumex::messages::asc_pull_ack::account_info_payload & response) const
		{
			return type == query_type::account_info_by_hash;
		}
		bool operator() (const lumex::messages::asc_pull_ack::frontiers_payload & response) const
		{
			return type == query_type::frontiers;
		}
		bool operator() (const lumex::messages::empty_payload & response) const
		{
	...
	if (!valid)
	{
		stats.inc (lumex::stat::type::bootstrap, lumex::stat::detail::invalid_response_type);
		return;
	}

	// Track bootstrap request response time
	stats.inc (lumex::stat::type::bootstrap_reply, to_stat_detail (tag.type));
	stats.sample (lumex::stat::sample::bootstrap_tag_duration, lumex::log::milliseconds_delta (tag.timestamp), { 0, config.request_timeout.count () });
	...
	{
		stats.inc (lumex::stat::type::bootstrap, lumex::stat::detail::invalid_response);
	}
...
bool bootstrap_context::process (lumex::messages::asc_pull_ack::blocks_payload const & response, async_tag const & tag)
{
	debug_assert (!mutex.try_lock ());
	...
	stats.inc (lumex::stat::type::bootstrap_process, lumex::stat::detail::blocks);
	...
	switch (result)
	{
		...
		{
			stats.inc (lumex::stat::type::bootstrap_verify_blocks, lumex::stat::detail::ok);
			stats.add (lumex::stat::type::bootstrap, lumex::stat::detail::blocks, lumex::stat::dir::in, response.blocks.size ());
			...
			block_processor.add_many (blocks, lumex::block_source::bootstrap, nullptr, [this, account = tag.account] (auto result) {
				// It's the last block submitted for this account chain, reset timestamp to allow more requests
				stats.inc (lumex::stat::type::bootstrap, lumex::stat::detail::timestamp_reset);
				{
		...
		{
			stats.inc (lumex::stat::type::bootstrap_verify_blocks, lumex::stat::detail::nothing_new);
			{
		...
		{
			stats.inc (lumex::stat::type::bootstrap_verify_blocks, lumex::stat::detail::invalid);
		}
		break;
	}
...
verify_result bootstrap_context::verify (lumex::messages::asc_pull_ack::blocks_payload const & response, async_tag const & tag) const
{
	release_assert (std::holds_alternative<blocks_tag_payload> (tag.payload));
	auto const & payload = std::get<blocks_tag_payload> (tag.payload);
	...
	lumex::block_hash previous_hash = blocks.front ()->hash ();
...
bool bootstrap_context::process (lumex::messages::asc_pull_ack::account_info_payload const & response, async_tag const & tag)
{
	return dependency_strat.process (response, tag);
}

bool bootstrap_context::process (lumex::messages::asc_pull_ack::frontiers_payload const & response, async_tag const & tag)
{
	return frontier_strat.process (response, tag);
}

bool bootstrap_context::process (lumex::messages::empty_payload const & response, async_tag const & tag)
{
	stats.inc (lumex::stat::type::bootstrap_process, lumex::stat::detail::empty);
...
lumex::container_info bootstrap_context::container_info () const
{
	lumex::lock_guard<lumex::mutex> lock{ mutex };
