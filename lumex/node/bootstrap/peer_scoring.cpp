...
lumex::bootstrap::peer_scoring::peer_scoring (bootstrap_config const & config_a, lumex::network_constants const & network_constants_a) :
	config{ config_a },
	network_constants{ network_constants_a }
{
}

void lumex::bootstrap::peer_scoring::reset ()
{
	scoring.clear ();
}

bool lumex::bootstrap::peer_scoring::limit_exceeded (std::shared_ptr<lumex::transport::channel> const & channel) const
{
...
bool lumex::bootstrap::peer_scoring::try_insert (std::shared_ptr<lumex::transport::channel> const & channel)
{
	auto & index = scoring.get<tag_channel> ();
...
bool lumex::bootstrap::peer_scoring::try_send_message (std::shared_ptr<lumex::transport::channel> const & channel)
{
	auto & index = scoring.get<tag_channel> ();
...
void lumex::bootstrap::peer_scoring::received_message (std::shared_ptr<lumex::transport::channel> const & channel)
{
	auto & index = scoring.get<tag_channel> ();
	if (auto existing = index.find (channel.get ()); existing != index.end ())
	{
...
std::shared_ptr<lumex::transport::channel> lumex::bootstrap::peer_scoring::channel ()
{
...
std::size_t lumex::bootstrap::peer_scoring::size () const
{
	return scoring.size ();
}

std::size_t lumex::bootstrap::peer_scoring::available () const
{
...
void lumex::bootstrap::peer_scoring::timeout ()
{
...
void lumex::bootstrap::peer_scoring::sync (std::deque<std::shared_ptr<lumex::transport::channel>> const & list)
{
...
lumex::container_info lumex::bootstrap::peer_scoring::container_info () const
{
...
lumex::bootstrap::peer_scoring::peer_score::peer_score (std::shared_ptr<lumex::transport::channel> const & channel_a, uint64_t outstanding_a, uint64_t request_count_total_a, uint64_t response_count_total_a) :
	channel{ channel_a },
