...
{
account_sets_index::account_sets_index (account_sets_config const & config_a, lumex::stats & stats_a) :
	config{ config_a },
	stats{ stats_a }
{
}
...
void account_sets_index::priority_up (lumex::account const & account)
{
	if (account.is_zero ())
	{
		return;
	...
	if (!blocked (account))
	{
		stats.inc (lumex::stat::type::bootstrap_account_sets, lumex::stat::detail::prioritize);
		...
		{
			stats.inc (lumex::stat::type::bootstrap_account_sets, lumex::stat::detail::priority_insert);
	...
	{
		stats.inc (lumex::stat::type::bootstrap_account_sets, lumex::stat::detail::prioritize_failed);
	}
}

void account_sets_index::priority_down (lumex::account const & account)
{
	if (account.is_zero ())
	{
		return;
	...
	if (auto it = priorities.get<tag_account> ().find (account); it != priorities.get<tag_account> ().end ())
	{
		stats.inc (lumex::stat::type::bootstrap_account_sets, lumex::stat::detail::deprioritize);
		...
		{
			stats.inc (lumex::stat::type::bootstrap_account_sets, lumex::stat::detail::erase_by_threshold);
	...
	{
		stats.inc (lumex::stat::type::bootstrap_account_sets, lumex::stat::detail::deprioritize_failed);
	}
}

void account_sets_index::priority_set (lumex::account const & account, double priority)
{
	if (account.is_zero ())
	{
		return;
	...
	if (!blocked (account))
	{
		...
		{
			stats.inc (lumex::stat::type::bootstrap_account_sets, lumex::stat::detail::priority_set);
...
void account_sets_index::priority_erase (lumex::account const & account)
{
	if (account.is_zero ())
	{
		return;
	...
	{
		stats.inc (lumex::stat::type::bootstrap_account_sets, lumex::stat::detail::priority_erase);
	}
}

void account_sets_index::block (lumex::account const & account, lumex::block_hash const & dependency, std::chrono::steady_clock::time_point now)
{
	...
	if (erased > 0)
	{
		stats.inc (lumex::stat::type::bootstrap_account_sets, lumex::stat::detail::erase_by_blocking);
		stats.inc (lumex::stat::type::bootstrap_account_sets, lumex::stat::detail::block);
	...
	{
		stats.inc (lumex::stat::type::bootstrap_account_sets, lumex::stat::detail::block_failed);
	}
}

void account_sets_index::unblock (lumex::account const & account, std::optional<lumex::block_hash> const & hash)
{
	if (account.is_zero ())
	{
		return;
	...
	{
		stats.inc (lumex::stat::type::bootstrap_account_sets, lumex::stat::detail::unblock);
		stats.inc (lumex::stat::type::bootstrap_account_sets, lumex::stat::detail::priority_unblocked);
	...
	{
		stats.inc (lumex::stat::type::bootstrap_account_sets, lumex::stat::detail::unblock_failed);
	}
}

void account_sets_index::timestamp_set (const lumex::account & account)
{
	debug_assert (!account.is_zero ());
...
void account_sets_index::timestamp_reset (const lumex::account & account)
{
	debug_assert (!account.is_zero ());
...
void account_sets_index::dependency_update (lumex::block_hash const & hash, lumex::account const & dependency_account)
{
	...
	if (it != end)
	{
		while (it != end)
		{
			...
			{
				stats.inc (lumex::stat::type::bootstrap_account_sets, lumex::stat::detail::dependency_update);
	...
	{
		stats.inc (lumex::stat::type::bootstrap_account_sets, lumex::stat::detail::dependency_update_failed);
	}
}
...
void account_sets_index::trim_overflow ()
{
	while (!priorities.empty () && priorities.size () > config.priorities_max)
	{
		...
		stats.inc (lumex::stat::type::bootstrap_account_sets, lumex::stat::detail::priority_overflow);
	...
	{
		// Erase the lowest priority entry
		stats.inc (lumex::stat::type::bootstrap_account_sets, lumex::stat::detail::blocking_overflow);
		blocking.pop_front ();
	}
}

auto account_sets_index::next_priority (std::function<bool (lumex::account const &)> const & filter) -> priority_result
{
	if (priorities.empty ())
	{
		return { 0 };
...
lumex::block_hash account_sets_index::next_blocking (std::function<bool (lumex::block_hash const &)> const & filter)
{
	if (blocking.empty ())
	{
		return { 0 };
	...
	auto [begin, end] = blocking.get<tag_dependency_account> ().equal_range (lumex::account{ 0 });
...
{
	stats.inc (lumex::stat::type::bootstrap_account_sets, lumex::stat::detail::sync_dependencies);

	std::size_t synced = 0;

	// Sample all accounts with a known dependency account (> account 0)
	auto begin = blocking.get<tag_dependency_account> ().upper_bound (lumex::account{ 0 });
	...
	for (auto const & entry : boost::make_iterator_range (begin, end))
	{
		...
		{
			stats.inc (lumex::stat::type::bootstrap_account_sets, lumex::stat::detail::dependency_synced);
...
{
	stats.inc (lumex::stat::type::bootstrap_account_sets, lumex::stat::detail::decay_blocking);
	...
	stats.add (lumex::stat::type::bootstrap_account_sets, lumex::stat::detail::blocking_decayed, result);

	return result;
}

bool account_sets_index::blocked (lumex::account const & account) const
{
	return blocking.get<tag_account> ().contains (account);
}

bool account_sets_index::prioritized (lumex::account const & account) const
{
...
double account_sets_index::priority (lumex::account const & account) const
{
	if (!blocked (account))
	{
...
lumex::container_info account_sets_index::container_info () const
{
	// Count blocking entries with their dependency account unknown
	auto blocking_unknown = blocking.get<tag_dependency_account> ().count (lumex::account{ 0 });
