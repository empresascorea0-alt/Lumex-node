...
namespace test
{
	...
	public:
		...
		{
			lumex::timer<UNIT> timer (lumex::timer_state::started);
			bool error = true;
	...
	void wait_peer_connections (lumex::test::system &);
	 ...
	 */
	bool process (lumex::node & node, std::vector<std::shared_ptr<lumex::block>> blocks);
	/*
	 ...
	 */
	bool process_live (lumex::node & node, std::vector<std::shared_ptr<lumex::block>> blocks);
	/*
	 ...
	 */
	bool confirmed (lumex::node & node, std::vector<std::shared_ptr<lumex::block>> blocks);
	/*
	 * Convenience function to check whether a list of hashes is confirmed.
	 * @return true if all blocks are confirmed, false otherwise
	 */
	bool confirmed (lumex::node & node, std::vector<lumex::block_hash> hashes);
	/*
	 ...
	 */
	bool exists (lumex::node & node, std::vector<lumex::block_hash> hashes);
	/*
	 * Convenience function to check whether a list of blocks exists in node ledger.
	 * @return true if all blocks are fully processed and inserted in the ledger, false otherwise
	 */
	bool exists (lumex::node & node, std::vector<std::shared_ptr<lumex::block>> blocks);
	/*
	 ...
	 */
	void confirm (lumex::ledger & ledger, std::vector<std::shared_ptr<lumex::block>> const blocks);
	void confirm (lumex::ledger & ledger, std::shared_ptr<lumex::block> const block);
	void confirm (lumex::ledger & ledger, lumex::block_hash const & hash);
	void confirm (lumex::node & node, std::vector<std::shared_ptr<lumex::block>> const blocks);
	/*
	 * Convenience function to check whether *all* of the hashes exists in node ledger or in the pruned table.
	 * @return true if all blocks are fully processed and inserted in the ledger, false otherwise
	 */
	bool block_or_pruned_all_exists (lumex::node & node, std::vector<lumex::block_hash> hashes);
	/*
	 * Convenience function to check whether *all* of the blocks exists in node ledger or their hash exists in the pruned table.
	 * @return true if all blocks are fully processed and inserted in the ledger, false otherwise
	 */
	bool block_or_pruned_all_exists (lumex::node & node, std::vector<std::shared_ptr<lumex::block>> blocks);
	/*
	 ...
	 */
	bool block_or_pruned_none_exists (lumex::node & node, std::vector<lumex::block_hash> hashes);
	/*
	 * Convenience function to check whether *none* of the blocks exists in node ledger or their hash exists in the pruned table.
	 * @return true if none of the blocks are processed and inserted in the ledger, false otherwise
	 */
	bool block_or_pruned_none_exists (lumex::node & node, std::vector<std::shared_ptr<lumex::block>> blocks);
	/*
	 ...
	 */
	bool activate (lumex::node & node, std::vector<lumex::block_hash> hashes);
	/*
	 * Convenience function to start elections for a list of hashes. Blocks are loaded from ledger.
	 * @return true if all blocks exist and were queued to election scheduler
	 */
	bool activate (lumex::node & node, std::vector<std::shared_ptr<lumex::block>> blocks);
	/*
	 ...
	 */
	bool active (lumex::node & node, std::vector<lumex::block_hash> hashes);
	/*
	 * Convenience function that checks whether all hashes from list have currently active elections
	 * @return true if all blocks have currently active elections, false othersie
	 */
	bool active (lumex::node & node, std::vector<std::shared_ptr<lumex::block>> blocks);
	/*
	 * Convenience function to create a new vote from list of blocks
	 */
	std::shared_ptr<lumex::vote> make_vote (lumex::keypair key, std::vector<std::shared_ptr<lumex::block>> blocks, uint64_t timestamp = 0, uint8_t duration = 0);
	/*
	 * Convenience function to create a new vote from list of block hashes
	 */
	std::shared_ptr<lumex::vote> make_vote (lumex::keypair key, std::vector<lumex::block_hash> hashes, uint64_t timestamp = 0, uint8_t duration = 0);
	/*
	 * Convenience function to create a new final vote from list of blocks
	 */
	std::shared_ptr<lumex::vote> make_final_vote (lumex::keypair key, std::vector<std::shared_ptr<lumex::block>> blocks);
	/*
	 * Convenience function to create a new final vote from list of block hashes
	 */
	std::shared_ptr<lumex::vote> make_final_vote (lumex::keypair key, std::vector<lumex::block_hash> hashes);
	/*
	 * Converts list of blocks to list of hashes
	 */
	std::vector<lumex::block_hash> blocks_to_hashes (std::vector<std::shared_ptr<lumex::block>> blocks);
	/*
	 * Clones list of blocks
	 */
	std::vector<std::shared_ptr<lumex::block>> clone (std::vector<std::shared_ptr<lumex::block>> blocks);
	/*
	 * Creates a new fake channel associated with `node`
	 */
	std::shared_ptr<lumex::transport::channel> fake_channel (lumex::node & node, lumex::account node_id = { 0 });
	/*
	 * Creates a new test channel associated with `node`
	 */
	std::shared_ptr<lumex::transport::test_channel> test_channel (lumex::node & node, lumex::account node_id = { 0 });
	/*
	 ...
	 */
	std::shared_ptr<lumex::election> start_election (lumex::test::system & system_a, lumex::node & node_a, const lumex::block_hash & hash_a);
	/*
	 ...
	 */
	[[nodiscard]] bool start_elections (lumex::test::system &, lumex::node &, std::vector<lumex::block_hash> const &, bool const forced_a = false);
	/*
	 ...
	 * NOTE: Each election is given 5 seconds to complete, if it does not complete in 5 seconds, it will return an error.
	 */
	[[nodiscard]] bool start_elections (lumex::test::system &, lumex::node &, std::vector<std::shared_ptr<lumex::block>> const &, bool const forced_a = false);

	/**
	 *  Return account_info for account "acc", if account is not found, a default initialised object is returned
	 */
	lumex::account_info account_info (lumex::node const & node, lumex::account const & acc);

	/**
	 * \brief Debugging function to print all entries in the pending table. Intended to be used to debug unit tests.
	 */
	void print_all_receivable_entries (const lumex::store::ledger_store & store);

	/**
	 * \brief Debugging function to print all accounts in a ledger. Intended to be used to debug unit tests.
	 */
	void print_all_account_info (const lumex::ledger & ledger);

	/**
	 * \brief Debugging function to print all blocks in a node. Intended to be used to debug unit tests.
	 */
	void print_all_blocks (const lumex::store::ledger_store & store);

	/**
	 * Returns all blocks in the ledger
	 */
	std::vector<std::shared_ptr<lumex::block>> all_blocks (lumex::node &);
