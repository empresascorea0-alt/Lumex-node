...
{
backend_rocksdb::backend_rocksdb (std::filesystem::path const & path, lumex::rocksdb_config const & config_a, lumex::logger & logger_a, lumex::store::txn_tracking_config const & txn_tracking_config_a) :
...
void backend_rocksdb::open_impl (column_schema schema, lumex::store::open_mode mode)
{
	...
	if (mode != lumex::store::open_mode::read_only)
	{
		...
		lumex::set_secure_perm_directory (database_path, error_chmod);
...
void backend_rocksdb::open_db (std::filesystem::path const & path, lumex::store::open_mode mode, ::rocksdb::Options const & options, std::vector<::rocksdb::ColumnFamilyDescriptor> column_families)
{
	::rocksdb::Status s;
...
::rocksdb::Options backend_rocksdb::get_db_options (lumex::store::open_mode mode)
{
	::rocksdb::Options db_options;

	db_options.create_if_missing = (mode != lumex::store::open_mode::read_only);
	db_options.create_missing_column_families = (mode != lumex::store::open_mode::read_only);
...
::rocksdb::ColumnFamilyHandle * backend_rocksdb::table_to_column_family (lumex::store::table table) const
{
...
int backend_rocksdb::get (lumex::store::transaction const & txn, lumex::store::table table, lumex::store::db_val const & key, lumex::store::db_val & value) const
{
...
int backend_rocksdb::put (lumex::store::write_transaction const & txn, lumex::store::table table, lumex::store::db_val const & key, lumex::store::db_val const & value)
{
	auto key_slice = to_slice (key);
...
int backend_rocksdb::del (lumex::store::write_transaction const & txn, lumex::store::table table, lumex::store::db_val const & key)
{
...
bool backend_rocksdb::exists (lumex::store::transaction const & txn, lumex::store::table table, lumex::store::db_val const & key) const
{
	::rocksdb::PinnableSlice slice;
	auto key_slice = to_slice (key);
	auto internals = rocksdb::tx (txn);
...
uint64_t backend_rocksdb::count (lumex::store::transaction const & txn, lumex::store::table table) const
{
...
bool backend_rocksdb::count_is_exact (lumex::store::table table) const
{
	switch (table)
	{
		case lumex::store::table::pruned:
		case lumex::store::table::final_votes:
			// These tables use rocksdb.estimate-num-keys which may be inaccurate
			return false;
		case lumex::store::table::accounts:
		case lumex::store::table::blocks:
		case lumex::store::table::confirmation_height:
		case lumex::store::table::default_unused:
		case lumex::store::table::meta:
		case lumex::store::table::online_weight:
		case lumex::store::table::peers:
		case lumex::store::table::pending:
		case lumex::store::table::vote:
		case lumex::store::table::rep_weights:
		case lumex::store::table::successor:
		case lumex::store::table::topology:
		case lumex::store::table::unchecked:
		case lumex::store::table::frontiers:
...
int backend_rocksdb::clear (lumex::store::table table)
{
...
lumex::store::iterator backend_rocksdb::begin (lumex::store::transaction const & txn, lumex::store::table table) const
{
	return lumex::store::iterator{ txn, iterator::begin (db.get (), rocksdb::tx (txn), table_to_column_family (table)) };
}

lumex::store::iterator backend_rocksdb::begin (lumex::store::transaction const & txn, lumex::store::table table, lumex::store::db_val const & key) const
{
	auto key_slice = to_slice (key);
	return lumex::store::iterator{ txn, iterator::lower_bound (db.get (), rocksdb::tx (txn), table_to_column_family (table), key_slice) };
}

lumex::store::iterator backend_rocksdb::end (lumex::store::transaction const & txn, lumex::store::table table) const
{
	return lumex::store::iterator{ txn, iterator::end (db.get (), rocksdb::tx (txn), table_to_column_family (table)) };
}
...
lumex::store::read_transaction backend_rocksdb::tx_begin_read () const
{
	return store::read_transaction{ std::make_unique<lumex::store::rocksdb::read_transaction_impl> (db.get (), txn_tracking_callbacks ()) };
}

lumex::store::write_transaction backend_rocksdb::tx_begin_write ()
{
	release_assert (transaction_db != nullptr);
	return store::write_transaction{ std::make_unique<lumex::store::rocksdb::write_transaction_impl> (transaction_db, txn_tracking_callbacks ()) };
}
...
void backend_rocksdb::copy_with_compaction (std::filesystem::path const & destination_path)
{
	...
	backend_rocksdb temp_backend (destination_path, config, lumex::default_logger ());
...
{
	tombstone_map.emplace (std::piecewise_construct, std::forward_as_tuple (lumex::store::table::blocks), std::forward_as_tuple (0, 25000));
	tombstone_map.emplace (std::piecewise_construct, std::forward_as_tuple (lumex::store::table::accounts), std::forward_as_tuple (0, 25000));
	tombstone_map.emplace (std::piecewise_construct, std::forward_as_tuple (lumex::store::table::pending), std::forward_as_tuple (0, 25000));
}

void backend_rocksdb::flush_tombstones_check (lumex::store::table table)
{
...
void backend_rocksdb::flush_table (lumex::store::table table)
{
