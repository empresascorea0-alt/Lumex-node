...
{
public:
	explicit matches_txn (lumex::store::transaction_impl const * txn_impl_a) :
		txn_impl (txn_impl_a)
	{
	}

	bool operator() (lumex::store::txn_stats const & txn_stats)
	{
...
lumex::store::txn_stats::txn_stats (transaction_impl const * txn_impl_a) :
	txn_impl (txn_impl_a),
	thread_name (lumex::thread_role::get_string ()),
	stacktrace (std::make_shared<boost::stacktrace::stacktrace> ())
{
	timer.start ();
}

bool lumex::store::txn_stats::is_write () const
{
...
lumex::store::txn_tracker::txn_tracker (lumex::logger & logger_a, txn_tracking_config const & config_a) :
	logger (logger_a),
	config (config_a)
{
}

void lumex::store::txn_tracker::serialize_json (boost::property_tree::ptree & json, std::chrono::milliseconds min_read_time, std::chrono::milliseconds min_write_time)
{
	...
	for (std::size_t i = 0; i < times_since_start.size (); ++i)
	{
		...
		if ((are_writes[i] && time_held_open >= min_write_time) || (!are_writes[i] && time_held_open >= min_read_time))
		{
			...
			lumex::jsonconfig stack (stacktrace_config);
...
void lumex::store::txn_tracker::log_if_held_long_enough (txn_stats const & stats) const
{
	...
	if ((is_write && time_open >= config.min_write_txn_time) || (!is_write && time_open >= config.min_read_txn_time))
	{
		...
		logger.warn (lumex::log::type::txn_tracker, "{}ms {} held on thread {}\n{}",
		time_open.count (),
		is_write ? "write lock" : "read",
		stats.thread_name,
		lumex::util::to_str (*stats.stacktrace));
	}
}

void lumex::store::txn_tracker::add (transaction_impl const * transaction_impl)
{
	lumex::lock_guard<lumex::mutex> guard (mutex);
...
void lumex::store::txn_tracker::erase (transaction_impl const * transaction_impl)
{
...
lumex::error lumex::store::txn_tracking_config::serialize_toml (lumex::tomlconfig & toml) const
{
...
lumex::error lumex::store::txn_tracking_config::deserialize_toml (lumex::tomlconfig & toml)
{
