#pragma once

#include <lumex/lib/id_dispenser.hpp>
#include <lumex/lib/lmdbconfig.hpp>
#include <lumex/store/lmdb/transaction_lmdb.hpp>

#include <filesystem>

namespace lumex::store::lmdb
{
/**
 * RAII wrapper for MDB_env
 */
class env final
{
public:
	/** Environment options, most of which originates from the config file. */
	class options final
	{
		friend class env;

	public:
		static options make ()
		{
			return options ();
		}

		options & set_config (lumex::lmdb_config config_a)
		{
			config = config_a;
			return *this;
		}

		options & set_read_only (bool read_only_a)
		{
			read_only = read_only_a;
			return *this;
		}

		options & set_use_no_mem_init (int use_no_mem_init_a)
		{
			use_no_mem_init = use_no_mem_init_a;
			return *this;
		}

		/** Used by the wallet to override the config map size */
		options & override_config_map_size (std::size_t map_size_a)
		{
			config.map_size = map_size_a;
			return *this;
		}

		/** Used by the wallet to override the sync strategy */
		options & override_config_sync (lumex::lmdb_config::sync_strategy sync_a)
		{
			config.sync = sync_a;
			return *this;
		}

	private:
		bool use_no_mem_init{ false };
		bool read_only{ false };
		lumex::lmdb_config config;
	};

	env (std::filesystem::path const &, env::options options_a = env::options::make ());
	void init (std::filesystem::path const &, env::options options_a = env::options::make ());
	~env ();
	operator MDB_env * () const;
	store::read_transaction tx_begin_read (lumex::store::txn_callbacks callbacks = {}) const;
	store::write_transaction tx_begin_write (lumex::store::txn_callbacks callbacks = {}) const;
	MDB_txn * tx (store::transaction const & transaction_a) const;
	void create_backup_file (std::filesystem::path const & filepath, lumex::logger & logger) const;

public:
	std::unique_ptr<MDB_env, decltype (&mdb_env_close)> environment{ nullptr, mdb_env_close };
	lumex::id_t const store_id{ lumex::next_id () };
	std::filesystem::path const database_path;

	using table_handle = MDB_dbi;
};
}
