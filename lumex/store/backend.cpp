...
{
lumex::store::column_schema const backend::schema_meta{ { lumex::store::table::meta, "meta" } };

backend::backend (lumex::logger & logger, lumex::store::txn_tracking_config const & txn_tracking_config)
{
	if (txn_tracking_config.enable)
	{
		tracker = std::make_unique<lumex::store::txn_tracker> (logger, txn_tracking_config);
	}
}

backend::~backend () = default;

void backend::open (column_schema schema, lumex::store::open_mode mode)
{
	if (is_open)
	{
...
void backend::create (column_schema schema, lumex::store::backend_version_t version)
{
	if (is_open)
	{
		throw std::runtime_error ("Backend is already open: " + get_database_path ());
	...
	open (schema, lumex::store::open_mode::read_write);
...
auto backend::fetch_meta () -> std::optional<backend_meta>
{
	...
	try
	{
		open (schema_meta, lumex::store::open_mode::read_only);
	}
	catch (lumex::error const & error)
	{
		if (error == lumex::error_backend::db_not_found)
		{
			return std::nullopt;
		}
		throw;
...
auto backend::get_mode () const -> std::optional<lumex::store::open_mode>
{
	return is_open ? std::optional{ current_mode } : std::nullopt;
}

auto backend::get_version (lumex::store::transaction const & txn) const -> lumex::store::backend_version_t
{
	return meta.get_version (txn);
}

void backend::set_version (lumex::store::write_transaction const & txn, lumex::store::backend_version_t version)
{
	meta.put_version (txn, version);
}

bool backend::empty (lumex::store::transaction const & txn, lumex::store::table table) const
{
	return begin (txn, table) == end (txn, table);
}

bool backend::empty (lumex::store::transaction const & txn) const
{
	release_assert (is_open, "backend is not open");
...
uint64_t backend::count_exact (lumex::store::transaction const & txn, lumex::store::table table) const
{
	uint64_t result = 0;
...
void backend::for_each_par (lumex::store::table table, std::function<void (read_transaction const &, iterator, iterator)> const & action) const
{
	// Split based on first byte of keys (0-255)
	// This works regardless of actual key type/length
	unsigned const thread_count = std::max (10u, std::min (40u, 10 * lumex::hardware_concurrency ()));
	...
	for (unsigned i = 0; i < thread_count; ++i)
	{
		...
		futures.emplace_back (std::async (std::launch::async, [this, table, &action, i, split, is_last] {
			lumex::thread_role::set (lumex::thread_role::name::db_parallel_traversal);
			...
			lumex::store::db_val start_key{ start_bytes };
			lumex::store::db_val end_key{ end_bytes };
...
void backend::copy_to (backend & destination, copy_progress_callback callback, size_t batch_size) const
{
	...
	for (auto const & [table, table_name] : schema)
	{
		...
		auto copy_action = [&] (lumex::store::read_transaction const &, iterator begin_it, iterator end_it) {
			...
			for (auto it = std::move (begin_it); it != end_it; ++it)
			{
				...
				auto status = destination.put (dst_txn, table, lumex::store::db_val{ key }, lumex::store::db_val{ value });
...
auto backend::txn_tracking_callbacks () const -> lumex::store::txn_callbacks
{
	txn_callbacks callbacks{};
	if (tracker)
	{
		callbacks.txn_start = [this] (lumex::store::transaction_impl const * txn) {
			tracker->add (txn);
		};
		callbacks.txn_end = [this] (lumex::store::transaction_impl const * txn) {
			tracker->erase (txn);
		};
	}
	return callbacks;
...
{
	switch (static_cast<lumex::error_backend> (ev))
	{
		case lumex::error_backend::generic:
			return "Generic backend error";
		case lumex::error_backend::db_not_found:
			return "Database not found";
		case lumex::error_backend::table_not_found:
			return "Table not found";
		case lumex::error_backend::failure:
