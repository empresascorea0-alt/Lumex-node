...
lumex::rpc_handler::rpc_handler (lumex::rpc_config const & rpc_config, std::string const & body_a, std::string const & request_id_a, std::function<void (std::string const &)> const & response_a, lumex::rpc_handler_interface & rpc_handler_interface_a, lumex::logger & logger) :
	body (body_a),
...
void lumex::rpc_handler::process_request (lumex::rpc_handler_request_params const & request_params)
{
	try
	{
		...
		else
		{
			if (request_params.rpc_version == 1)
			{
				...
				logger.log (rpc_config.rpc_logging.log_rpc ? lumex::log::level::info : lumex::log::level::debug,
				lumex::log::type::rpc_request, "Request {} : {}", request_id, filter_request (request));

				// Check if this is a RPC command which requires RPC enabled control
				std::error_code rpc_control_disabled_ec = lumex::error_rpc::rpc_control_disabled;
				...
				else
				{
					...
					{
						lumex::uint128_union random_id;
						lumex::random_pool::generate_block (random_id.bytes.data (), random_id.bytes.size ());
