...
#include <lumex/rpc/rpc_connection.hpp>
#include <lumex/rpc/rpc_handler.hpp>
...
lumex::rpc_connection::rpc_connection (lumex::rpc_config const & rpc_config, boost::asio::io_context & io_ctx, lumex::logger & logger, lumex::rpc_handler_interface & rpc_handler_interface) :
	socket (io_ctx),
...
void lumex::rpc_connection::parse_connection ()
{
	read (socket);
}

void lumex::rpc_connection::prepare_head (unsigned version, boost::beast::http::status status)
{
	res.version (version);
	res.result (status);
...
void lumex::rpc_connection::write_result (std::string body, unsigned version, boost::beast::http::status status)
{
...
void lumex::rpc_connection::write_completion_handler (std::shared_ptr<lumex::rpc_connection> const & rpc_connection)
{
	// Intentional no-op
}

template <typename STREAM_TYPE>
void lumex::rpc_connection::read (STREAM_TYPE & stream)
{
	...
	boost::beast::http::async_read_header (stream, buffer, *header_parser, boost::asio::bind_executor (strand, [this_l, &stream, header_parser] (boost::system::error_code const & ec, size_t bytes_transferred) {
		if (!ec)
		{
			if (boost::iequals (header_parser->get ()[boost::beast::http::field::expect], "100-continue"))
			{
				...
				continue_response->set (boost::beast::http::field::server, "lumex");
		...
		{
			this_l->logger.error (lumex::log::type::rpc_connection, "RPC header error: ", ec.message ());
			...
			lumex::json_error_response (response_handler, std::string ("Invalid header: ") + ec.message ());
		}
	}));
}

template <typename STREAM_TYPE>
void lumex::rpc_connection::parse_request (STREAM_TYPE & stream, std::shared_ptr<boost::beast::http::request_parser<boost::beast::http::empty_body>> const & header_parser)
{
	auto this_l (shared_from_this ());
	auto header_field_credentials_l (header_parser->get ()["lumex-api-key"]);
	auto header_corr_id_l (header_parser->get ()["lumex-correlation-id"]);
	...
	boost::beast::http::async_read (stream, buffer, *body_parser, boost::asio::bind_executor (strand, [this_l, body_parser, header_field_credentials_l, header_corr_id_l, path_l, &stream] (boost::system::error_code const & ec, size_t bytes_transferred) {
		if (!ec)
		{
			boost::asio::post (this_l->io_ctx, [this_l, body_parser, header_field_credentials_l, header_corr_id_l, path_l, &stream] () {
				...
				auto response_handler ([this_l, version, start, request_id, &stream] (std::string const & tree_a) {
					...
					this_l->logger.log (this_l->rpc_config.rpc_logging.log_rpc ? lumex::log::level::info : lumex::log::level::debug,
					lumex::log::type::rpc_request, "RPC request {} completed in {} microseconds", request_id, std::chrono::duration_cast<std::chrono::microseconds> (std::chrono::steady_clock::now () - start).count ());
				});
				...
				switch (method)
				{
					...
					{
						auto handler (std::make_shared<lumex::rpc_handler> (this_l->rpc_config, req.body (), request_id, response_handler, this_l->rpc_handler_interface, this_l->logger));
						lumex::rpc_handler_request_params request_params;
					...
					default:
					{
						lumex::json_error_response (response_handler, "Can only POST requests");
						break;
					}
				}
			});
		...
		{
			this_l->logger.error (lumex::log::type::rpc_connection, "RPC read error: ", ec.message ());
		}
	}));
}

template void lumex::rpc_connection::read (socket_type &);
template void lumex::rpc_connection::parse_request (socket_type &, std::shared_ptr<boost::beast::http::request_parser<boost::beast::http::empty_body>> const &);
#ifdef LUMEX_SECURE_RPC
template void lumex::rpc_connection::read (boost::asio::ssl::stream<socket_type &> &);
template void lumex::rpc_connection::parse_request (boost::asio::ssl::stream<socket_type &> &, std::shared_ptr<boost::beast::http::request_parser<boost::beast::http::empty_body>> const &);
#endif
