...
{
message_header::message_header (lumex::network_constants const & constants, message_type type_a) :
...
message_header::message_header (bool & error_a, lumex::stream & stream_a)
{
	if (!error_a)
	{
...
void message_header::serialize (lumex::stream & stream_a) const
{
	lumex::write (stream_a, boost::endian::native_to_big (static_cast<uint16_t> (network)));
	lumex::write (stream_a, version_max);
	lumex::write (stream_a, version_using);
	lumex::write (stream_a, version_min);
	lumex::write (stream_a, type);
	lumex::write (stream_a, static_cast<uint16_t> (extensions.to_ullong ()));
}

bool message_header::deserialize (lumex::stream & stream_a)
{
	auto error (false);
	try
	{
		uint16_t network_bytes;
		lumex::read (stream_a, network_bytes);
		network = static_cast<lumex::network_type> (boost::endian::big_to_native (network_bytes));
		lumex::read (stream_a, version_max);
		lumex::read (stream_a, version_using);
		lumex::read (stream_a, version_min);
		lumex::read (stream_a, type);
		uint16_t extensions_l;
		lumex::read (stream_a, extensions_l);
...
lumex::block_type message_header::block_type () const
{
	return static_cast<lumex::block_type> (((extensions & block_type_mask) >> 8).to_ullong ());
}

void message_header::block_type_set (lumex::block_type type_a)
{
...
std::size_t message_header::payload_length_bytes () const
{
	switch (type)
	{
		...
		{
			return lumex::block::size (block_type ());
		}
...
void message_header::operator() (lumex::object_stream & obs) const
{
	obs.write ("type", type);
