...
telemetry_req::telemetry_req (lumex::network_constants const & constants) :
...
bool telemetry_req::deserialize (lumex::stream & stream_a)
{
	debug_assert (header.type == message_type::telemetry_req);
	return false;
}

void telemetry_req::serialize (lumex::stream & stream_a) const
{
...
void telemetry_req::operator() (lumex::object_stream & obs) const
{
...
telemetry_ack::telemetry_ack (lumex::network_constants const & constants) :
	message (constants, message_type::telemetry_ack)
{
}

telemetry_ack::telemetry_ack (bool & error_a, lumex::stream & stream_a, message_header const & message_header) :
...
telemetry_ack::telemetry_ack (lumex::network_constants const & constants, telemetry_data const & telemetry_data_a) :
	message (constants, message_type::telemetry_ack),
...
void telemetry_ack::serialize (lumex::stream & stream_a) const
{
	header.serialize (stream_a);
	if (!is_empty_payload ())
	{
...
bool telemetry_ack::deserialize (lumex::stream & stream_a)
{
	auto error (false);
	...
	try
	{
		if (!is_empty_payload ())
		{
			data.deserialize (stream_a, lumex::narrow_cast<uint16_t> (header.extensions.to_ulong ()));
		}
	}
...
void telemetry_ack::operator() (lumex::object_stream & obs) const
{
	message::operator() (obs); // Write common data
...
void telemetry_data::deserialize (lumex::stream & stream_a, uint16_t payload_length_a)
{
...
void telemetry_data::serialize_without_signature (lumex::stream & stream_a) const
{
...
void telemetry_data::serialize (lumex::stream & stream_a) const
{
	write (stream_a, signature);
	serialize_without_signature (stream_a);
}

lumex::error telemetry_data::serialize_json (lumex::jsonconfig & json, bool ignore_identification_metrics_a) const
{
	...
	json.put ("active_difficulty", lumex::to_string_hex (active_difficulty));
	if (version >= telemetry_data_version::v2)
	{
...
lumex::error telemetry_data::deserialize_json (lumex::jsonconfig & json, bool ignore_identification_metrics_a)
{
	if (!ignore_identification_metrics_a)
	{
	...
	auto ec = lumex::from_string_hex (current_active_difficulty_text, active_difficulty);
	debug_assert (!ec);
...
void telemetry_data::sign (lumex::keypair const & node_id_a)
{
	...
	signature = lumex::sign_message (node_id_a.prv, node_id_a.pub, bytes.data (), bytes.size ());
}
...
bool telemetry_data::validate_signature () const
{
	...
	return lumex::validate_message (node_id, bytes.data (), bytes.size (), signature);
}
...
void telemetry_data::operator() (lumex::object_stream & obs) const
{
	// TODO: Telemetry data
}
}
...
namespace lumex::messages
{
telemetry_database_backend to_telemetry_database_backend (lumex::database_backend backend)
{
	switch (backend)
	{
...
std::string to_string (telemetry_maker maker)
{
	switch (maker)
	{
		...
		case telemetry_maker::lumex_node_light:
			return "lumex_node_light";
		case telemetry_maker::rs_lumex:
			return "rs_lumex";
	}
	return "invalid";
}
...
telemetry_maker telemetry_maker_from_string (std::string const & str)
{
	...
	if (str == "lumex_node_light")
		return telemetry_maker::lumex_node_light;
	if (str == "rs_lumex")
		return telemetry_maker::rs_lumex;
	return telemetry_maker::nf_node;
}
