...
bool lumex_qt::eventloop_processor::event (QEvent * event_a)
{
	debug_assert (dynamic_cast<lumex_qt::eventloop_event *> (event_a) != nullptr);
	static_cast<lumex_qt::eventloop_event *> (event_a)->action ();
	return true;
}

lumex_qt::eventloop_event::eventloop_event (std::function<void ()> const & action_a) :
	QEvent (QEvent::Type::User),
	action (action_a)
{
}

lumex_qt::self_pane::self_pane (lumex_qt::wallet & wallet_a, lumex::account const & account_a) :
	window (new QWidget),
	layout (new QVBoxLayout),
	self_layout (new QHBoxLayout),
	self_window (new QWidget),
	your_account_label (new QLabel ("Your Lumex account:")),
	...
	wallet (wallet_a)
{
	...
	version = new QLabel (fmt::format ("{} {} network", LUMEX_VERSION_STRING, network).c_str ());
...
void lumex_qt::self_pane::set_balance_text (std::pair<lumex::uint128_t, lumex::uint128_t> balance_a)
{
...
lumex_qt::accounts::accounts (lumex_qt::wallet & wallet_a) :
...
void lumex_qt::accounts::refresh ()
{
	...
	for (auto const & key : wallet.wallet_m->accounts ())
	{
		...
		{
			case lumex::wallet::key_type::adhoc:
			{
				brush.setColor ("red");
...
lumex_qt::import::import (lumex_qt::wallet & wallet_a) :
	window (new QWidget),
	layout (new QVBoxLayout),
...
lumex_qt::history::history (lumex::ledger & ledger_a, lumex::account const & account_a, lumex_qt::wallet & wallet_a) :
	window (new QWidget),
	layout (new QVBoxLayout),
	model (new QStandardItemModel),
	view (new QTableView),
...
namespace
{
class short_text_visitor : public lumex::block_visitor
{
public:
	short_text_visitor (lumex::secure::transaction const & transaction_a, lumex::ledger & ledger_a) :
	...
	void open_block (lumex::open_block const & block_a)
	{
		...
		else
		{
			...
			amount = lumex::dev::constants.genesis_amount;
		}
	}
...
lumex_qt::block_viewer::block_viewer (lumex_qt::wallet & wallet_a) :
	window (new QWidget),
	layout (new QVBoxLayout),
...
void lumex_qt::block_viewer::rebroadcast_action (lumex::block_hash const & hash_a)
{
	auto done (true);
	...
	if (block != nullptr)
	{
		wallet.node.network.flood_block (block, lumex::transport::traffic_type::block_broadcast_initial);
...
lumex_qt::account_viewer::account_viewer (lumex_qt::wallet & wallet_a) :
	window (new QWidget),
	layout (new QVBoxLayout),
	...
	wallet (wallet_a)
{
	...
	QObject::connect (refresh, &QPushButton::released, [this] () {
		...
		if (!account.decode_account (account_line->text ().toStdString ()))
		{
			...
			auto final_text (std::string ("Balance (LUMEX): ") + wallet.format_balance (balance.first));
...
lumex_qt::stats_viewer::stats_viewer (lumex_qt::wallet & wallet_a) :
	window (new QWidget),
	layout (new QVBoxLayout),
	refresh (new QPushButton ("Refresh")),
...
void lumex_qt::stats_viewer::refresh_stats ()
{
	model->removeRows (0, model->rowCount ());
...
lumex_qt::status::status (lumex_qt::wallet & wallet_a) :
	wallet (wallet_a)
{
	wallet.status->setToolTip ("Wallet status, block count (blocks downloaded)");
	active.insert (lumex_qt::status_types::nominal);
	set_text ();
}

void lumex_qt::status::erase (lumex_qt::status_types status_a)
{
	debug_assert (status_a != lumex_qt::status_types::nominal);
	auto erased (active.erase (status_a));
	(void)erased;
	set_text ();
}

void lumex_qt::status::insert (lumex_qt::status_types status_a)
{
	debug_assert (status_a != lumex_qt::status_types::nominal);
...
std::string lumex_qt::status::text ()
{
	...
	switch (*active.begin ())
	{
		case lumex_qt::status_types::disconnected:
			result = "Status: Disconnected";
			break;
		case lumex_qt::status_types::working:
			result = "Status: Generating proof of work";
			break;
		case lumex_qt::status_types::synchronizing:
			result = "Status: Synchronizing";
			break;
		case lumex_qt::status_types::locked:
			result = "Status: Wallet locked";
			break;
		case lumex_qt::status_types::vulnerable:
			result = "Status: Wallet password empty";
			break;
		case lumex_qt::status_types::active:
			result = "Status: Wallet active";
			break;
		case lumex_qt::status_types::nominal:
...
std::string lumex_qt::status::color ()
{
	debug_assert (!active.empty ());
	std::string result;
	switch (*active.begin ())
...
lumex_qt::wallet::wallet (QApplication & application_a, lumex_qt::eventloop_processor & processor_a, lumex::node & node_a, std::shared_ptr<lumex::wallet::wallet> const & wallet_a, lumex::public_key const & account_a) :
	rendering_ratio (lumex::lumex_ratio),
	node (node_a),
	wallet_m (wallet_a),
	account (account_a),
...
void lumex_qt::wallet::ongoing_refresh ()
{
	std::weak_ptr<lumex_qt::wallet> wallet_w (shared_from_this ());
...
void lumex_qt::wallet::start ()
{
	ongoing_refresh ();
	std::weak_ptr<lumex_qt::wallet> this_w (shared_from_this ());
					...
					if (actual <= balance)
					{
						if (!this_l->wallet_m->is_locked ())
						{
							...
							this_l->node.workers.post ([this_w, account_l, actual] () {
								if (auto this_l = this_w.lock ())
								{
									this_l->wallet_m->send_async (this_l->account, account_l, actual, [this_w] (std::shared_ptr<lumex::block> const & block_a) {
										if (auto this_l = this_w.lock ())
										{
	...
	node.observers.blocks.add ([this_w] (lumex::election_status const & status_a, std::vector<lumex::vote_with_weight_info> const & votes_a, lumex::account const & account_a, lumex::uint128_t const & amount_a, bool, bool) {
		if (auto this_l = this_w.lock ())
		{
	...
	node.observers.account_balance.add ([this_w] (lumex::account const & account_a, bool is_pending) {
		if (auto this_l = this_w.lock ())
		{
		...
		if (auto this_l = this_w.lock ())
		{
			this_l->application.postEvent (&this_l->processor, new eventloop_event ([this_w, active_a] () {
				if (auto this_l = this_w.lock ())
				{
					if (active_a)
					{
						this_l->active_status.insert (lumex_qt::status_types::active);
					}
					else
					{
						this_l->active_status.erase (lumex_qt::status_types::active);
					}
				}
			}));
		}
	});
	node.observers.channel_connected.add ([this_w] (std::shared_ptr<lumex::transport::channel> const &) {
		if (auto this_l = this_w.lock ())
		{
			this_l->application.postEvent (&this_l->processor, new eventloop_event ([this_w] () {
				if (auto this_l = this_w.lock ())
		...
		if (auto this_l = this_w.lock ())
		{
			this_l->application.postEvent (&this_l->processor, new eventloop_event ([this_w, working] () {
				if (auto this_l = this_w.lock ())
				{
					if (working)
					{
						this_l->active_status.insert (lumex_qt::status_types::working);
					}
					else
					{
						this_l->active_status.erase (lumex_qt::status_types::working);
					}
				}
			}));
		}
...
void lumex_qt::wallet::update_connected ()
{
	if (node.network.empty ())
	{
		active_status.insert (lumex_qt::status_types::disconnected);
	}
	else
	{
		active_status.erase (lumex_qt::status_types::disconnected);
	}
}

void lumex_qt::wallet::empty_password ()
{
...
void lumex_qt::wallet::change_rendering_ratio (lumex::uint128_t const & rendering_ratio_a)
{
...
std::string lumex_qt::wallet::format_balance (lumex::uint128_t const & balance) const
{
	auto balance_str = lumex::amount (balance).format_balance (rendering_ratio, 3, false);
	auto unit = std::string ("lumex");
	if (rendering_ratio == lumex::raw_ratio)
	{
		unit = std::string ("raw");
	}
...
void lumex_qt::wallet::push_main_stack (QWidget * widget_a)
{
	main_stack->addWidget (widget_a);
	main_stack->setCurrentIndex (main_stack->count () - 1);
}

void lumex_qt::wallet::pop_main_stack ()
{
	main_stack->removeWidget (main_stack->currentWidget ());
}

lumex_qt::settings::settings (lumex_qt::wallet & wallet_a) :
	window (new QWidget),
	layout (new QVBoxLayout),
	password (new QLineEdit),
	...
	wallet (wallet_a)
{
		...
		if (!this->wallet.wallet_m->is_locked ())
		{
			...
			else
			{
				if (new_password->text () == retype_password->text ())
				{
					...
					this->wallet.node.logger.warn (lumex::log::type::qt, "Wallet password changed");
	...
	QObject::connect (change_rep, &QPushButton::released, [this] () {
		lumex::account representative_l;
	...
	QObject::connect (lock_toggle, &QPushButton::released, [this] () {
		if (!this->wallet.wallet_m->is_locked ())
		{
			...
			this->wallet.node.logger.warn (lumex::log::type::qt, "Wallet locked");
...
void lumex_qt::settings::refresh_representative ()
{
...
void lumex_qt::settings::activate ()
{
	this->wallet.push_main_stack (window);
}

void lumex_qt::settings::update_locked (bool invalid, bool vulnerable)
{
	if (invalid)
	{
		this->wallet.active_status.insert (lumex_qt::status_types::locked);
	}
	else
	{
		this->wallet.active_status.erase (lumex_qt::status_types::locked);
	}
	if (vulnerable)
	{
		this->wallet.active_status.insert (lumex_qt::status_types::vulnerable);
	}
	else
	{
		this->wallet.active_status.erase (lumex_qt::status_types::vulnerable);
	}
}

lumex_qt::advanced_actions::advanced_actions (lumex_qt::wallet & wallet_a) :
	window (new QWidget),
	layout (new QVBoxLayout),
	...
	lumex_unit (new QRadioButton ("lumex")),
	...
	wallet (wallet_a)
{
	ratio_group->addButton (lumex_unit);
	ratio_group->setId (lumex_unit, ratio_group->buttons ().size () - 1);
	ratio_group->addButton (raw_unit);
	ratio_group->setId (raw_unit, ratio_group->buttons ().size () - 1);
	scale_layout->addWidget (scale_label);
	scale_layout->addWidget (lumex_unit);
	...
	QObject::connect (lumex_unit, &QRadioButton::toggled, [this] () {
		if (lumex_unit->isChecked ())
		{
			this->wallet.change_rendering_ratio (lumex::lumex_ratio);
			QSettings ().setValue (saved_ratio_key, ratio_group->id (lumex_unit));
		}
	});
	QObject::connect (raw_unit, &QRadioButton::toggled, [this] () {
		if (raw_unit->isChecked ())
		{
			this->wallet.change_rendering_ratio (lumex::raw_ratio);
	...
	{
		selected_ratio_button = lumex_unit;
	}
...
void lumex_qt::advanced_actions::refresh_peers ()
{
...
void lumex_qt::advanced_actions::refresh_ledger ()
{
	...
	for (auto i (wallet.node.ledger.store.account.begin (transaction)), j (wallet.node.ledger.store.account.end (transaction)); i != j; ++i)
	{
		...
		std::string balance = lumex::amount (info.balance.number () / wallet.rendering_ratio).to_string_dec ();
...
void lumex_qt::advanced_actions::refresh_stats ()
{
	wallet.stats_viewer.refresh_stats ();
}

lumex_qt::block_entry::block_entry (lumex_qt::wallet & wallet_a) :
	window (new QWidget),
	layout (new QVBoxLayout),
	block (new QPlainTextEdit),
	status (new QLabel),
	...
	wallet (wallet_a)
{
	...
	QObject::connect (process, &QPushButton::released, [this] () {
		...
		try
		{
			...
			auto block_l (lumex::deserialize_block_json (tree));
			if (block_l != nullptr)
			{
...
lumex_qt::block_creation::block_creation (lumex_qt::wallet & wallet_a) :
	window (new QWidget),
	layout (new QVBoxLayout),
...
void lumex_qt::block_creation::deactivate_all ()
{
	account_label->hide ();
	account->hide ();
	source_label->hide ();
...
void lumex_qt::block_creation::activate_send ()
{
	account_label->show ();
	account->show ();
	amount_label->show ();
...
void lumex_qt::block_creation::activate_receive ()
{
	source_label->show ();
	source->show ();
}

void lumex_qt::block_creation::activate_open ()
{
	source_label->show ();
	source->show ();
...
void lumex_qt::block_creation::activate_change ()
{
	account_label->show ();
	account->show ();
	representative_label->show ();
...
void lumex_qt::block_creation::create_send ()
{
	lumex::account account_l;
	auto error (account_l.decode_account (account->text ().toStdString ()));
	if (!error)
	{
		lumex::amount amount_l;
			...
			if (!error)
			{
				...
				if (key_result)
				{
					...
					if (amount_l.number () <= balance)
					{
						...
						(void)error;
						debug_assert (!error);
						lumex::state_block send (account_l, info.head, info.representative, balance - amount_l.number (), destination_l, key_result.value (), account_l, 0);
...
void lumex_qt::block_creation::create_receive ()
{
	lumex::block_hash source_l;
	...
	if (!error)
	{
		...
		if (block_l != nullptr)
		{
			...
			{
				lumex::pending_key pending_key (destination, source_l);
				if (auto pending = wallet.node.ledger.any.pending_get (block_transaction, pending_key))
				{
					...
					if (!error)
					{
						...
						if (key_result)
						{
							lumex::state_block receive (pending_key.account, info.head, info.representative, info.balance.number () + pending.value ().amount.number (), source_l, key_result.value (), pending_key.account, 0);
							lumex::block_details details;
...
void lumex_qt::block_creation::create_change ()
{
	lumex::account account_l;
	auto error (account_l.decode_account (account->text ().toStdString ()));
	if (!error)
	{
		...
		if (!error)
		{
			...
			if (!error)
			{
				...
				if (key_result)
				{
					lumex::state_block change (account_l, info.head, representative_l, info.balance, 0, key_result.value (), account_l, 0);
					lumex::block_details details;
					details.epoch = info.epoch ();
...
void lumex_qt::block_creation::create_open ()
{
	lumex::block_hash source_l;
	auto error (source_l.decode_hex (source->text ().toStdString ()));
	if (!error)
				...
				if (!destination.is_zero ())
				{
					...
					if (auto pending = wallet.node.ledger.any.pending_get (block_transaction, pending_key))
					{
						...
						if (error)
						{
							...
							if (key_result)
							{
								lumex::state_block open (pending_key.account, 0, representative_l, pending.value ().amount, source_l, key_result.value (), pending_key.account, 0);
								lumex::block_details details;
								details.is_receive = true;
